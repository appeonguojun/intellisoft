...
FOR i = 1 TO li_col_cnt
	...
	//For Label
	ls_retval = Mid(idw_detail.Describe( ls_field_nm + "_t.text" ) , 1, 5000)
...
public function integer of_set_parent_window (w_prac_data_design aw_win);parent_window_design = aw_win
...
ELSE
	...
	dw_detail.Modify( Mid( is_last_tab_label_name, 5, 100 ) + ".TabSequence = '" + sle_tab.Text + "'" )		
	//---------------------------- APPEON END ----------------------------
...
inv_data_entry.of_create_dynamic_dw(il_prac_id, il_data_view_id, ai_screen_id, dw_detail, dw_browse, True, 0 )
...
this.Control[iCurrent+2]=this.cb_screen_properties
this.Control[iCurrent+3]=this.sle_tab
this.Control[iCurrent+4]=this.st_tab
this.Control[iCurrent+5]=this.dw_properties
this.Control[iCurrent+6]=this.uo_screen_design_menu
this.Control[iCurrent+7]=this.cb_1
this.Control[iCurrent+8]=this.dw_fields_alias
this.Control[iCurrent+9]=this.dw_properties_report
this.Control[iCurrent+10]=this.dw_properties_dataview
this.Control[iCurrent+11]=this.dw_properties_modify
this.Control[iCurrent+12]=this.dw_properties_pmodify
end on
...
IF ib_relocating_fields = True THEN
	...
	END FOR	
	dw_properties.Retrieve(il_data_view_id, ii_screen_id)
	inv_data_entry.of_create_dynamic_dw(il_prac_id, il_data_view_id, ii_screen_id, dw_detail, dw_browse, True, 0 )	
...
IF NOT ib_resize_height AND NOT ib_resize_width AND ib_mouse_down AND Len(is_last_object[ii_obj_select_cnt]) > 0 AND is_last_object[ii_obj_select_cnt] <> "dw" AND is_option = "Move" AND	ib_control_down = False THEN		
	SetPointer(Cross!)
...
ELSEIF ib_resize_height AND ib_mouse_down THEN
	...
	IF Right( is_last_object[1], 2 ) <> "_t" AND	Pos( is_last_object[1], "obj" ) = 0 THEN
...
IF flags <> 9 AND ib_multi_select THEN
	...
	ls_selected_object = Mid(ls_selected_object, 1, Pos(ls_selected_object, "	" ) -1 )
	FOR i = 1 TO ii_obj_select_cnt 
...
IF ib_set_tab_order THEN
	IF Left( ls_selected_object_name, 4 ) = "tab_" THEN
		...
		ls_retval = This.Modify( Mid( is_last_tab_label_name, 5,100) + ".TabSequence = '" + sle_tab.Text + "'" )		
		//---------------------------- APPEON END ----------------------------
		...
		*/
		...
		sle_tab.Text = idw_detail.Describe( Mid( ls_selected_object_name, 5, 100 ) + ".TabSequence" )			
		//---------------------------- APPEON END ----------------------------
		sle_tab.SelectText( 1, 100 )
