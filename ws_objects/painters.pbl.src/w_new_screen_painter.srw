...
IF Left( ls_table_name, 4 ) <> "pdc_" THEN
...
FOR i = 1 TO li_rc	
	...
	ELSE
		IF tab_1.tabpage_screen.dw_sys_fields.GetItemString( i,"lookup_type" ) = "C" THEN
			...
			//------------------- APPEON BEGIN -------------------
			//Modified By Ken.Guo 10/24/2012
...
ls_ltr_field_sql = Mid( ls_ltr_field_sql + ls_field_sql, 1, Len( ls_ltr_field_sql + ls_field_sql )-3 )
ls_ltr_from_sql =  Mid( ls_ltr_from_sql +  ls_from_sql, 1, Len( ls_ltr_from_sql +  ls_from_sql )-3 )
ls_ltr_where_sql = Mid( ls_ltr_where_sql + ls_where_sql, 1, Len( ls_ltr_where_sql + ls_where_sql )-6 )
...
li_retval = tab_1.tabpage_screen.dw_ver_ltr.SaveAs( gs_dir_path + gs_DefDirName + "\Letters\data\" + ls_screen_name + ".txt", Text!, True)
...
ls_table_test = "v_"  + mid(ls_table,5)
...
// mskinner 07 april 2006 -- begin
// remove dba
ls_drop_sql = "drop VIEW v_"  + mid(ls_table,5) + ";"

ls_ltr_field_sql = "CREATE VIEW v_"  + mid(ls_table,5) + " AS SELECT ~n~r" 
// mskinner 07 april 2006 -- begin
...
if len(ls_sql) < 10 then messagebox("SQL","View SQL did not generate properly")
//ClipBoard( ls_sql )  
//return 1
if cnt > 0 then
	//get id for update
...
ls_dropview = "DROP VIEW v_"  + mid(ls_table,5) + ";"
...
ls_index_sql = "CREATE UNIQUE INDEX " + "idx_" + Mid( ls_table_name,5,100 ) + "_rec_id ON " + ls_table_name + "( rec_id ASC )"
...
ls_index_sql = "CREATE INDEX " + "idx_" + Mid( ls_table_name,5,100 ) + "_prac_id ON " + ls_table_name + "( prac_id ASC )"
...
ls_table = Mid( dw_table.GetText(), 5, 100 ) 
