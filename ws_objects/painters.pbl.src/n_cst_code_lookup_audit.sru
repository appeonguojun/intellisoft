...
if Len(adw_data.DataObject) > 0 then
	ids_data.DataObject = adw_data.DataObject
else
	ls_Syntax = adw_data.object.datawindow.syntax
	ids_data.Create(ls_Syntax, ls_ErrorText)
	if Len(ls_ErrorText) > 0 then Return -1
end if
ll_row = adw_data.getrow()
if adw_data.ShareData(ids_data) = -1 then Return -1
//Added By Jay Chen 10-23-2014 remark:when modify lookup painter last row data it auto locate to the first row.
if AppeonGetClientType() = "WEB" then
	if isvalid(w_lookup_maint) then
		w_lookup_maint.tab_1.tabpage_browse.dw_browse.ScrollToRow (ll_row)
		w_lookup_maint.tab_1.tabpage_browse.dw_browse.SetRow (ll_row)
		w_lookup_maint.tab_1.tabpage_detail.dw_detail.ScrollToRow (ll_row)
		w_lookup_maint.tab_1.tabpage_detail.dw_detail.SetRow (ll_row)
	end if
end if
