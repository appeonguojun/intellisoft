...
type cb_2 from commandbutton within pfc_cst_u_data_entry_report_painter
end type
...
FOR i = 1 TO li_col_cnt
	...
	dw_properties.SetItem(i, "data_view_report_fields_label_font_weight", Integer(idw_detail.Describe( ls_field_nm + "_t.font.weight" )) )	//Added By Ken.Guo 04.01.2008	
...
public function integer of_set_parent_window (w_prac_report_design aw_win);parent_window_design_report = aw_win
...
for i=1 to ll_rowcount
	...
	for j=1 to ll_colnum
		...
		ls_coltype=left(dw_properties.Describe(ls_colname+".coltype"),3) 
...
event dw_select_section::constructor;of_settransobject( sqlca)
...
is_curdw = dw_select_section.GetItemString(ll_row, "cur_dw") //modify
if pos(is_curdw,'_prf_')>0 then
	ls_tmpdw = dw_select_section.GetItemString(ll_row, "scr_dw")
	if isnull(ls_tmpdw) or trim(ls_tmpdw)='' then
		messagebox('','Current report not exist screen painter!')
		SetRedraw( True )
		return
	end if
