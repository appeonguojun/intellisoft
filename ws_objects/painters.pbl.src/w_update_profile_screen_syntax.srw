...
p1 = pos(s,"header(height",1 )
p2 = pos(s,"summary(height",1)
...
li_pos = pos(is_syntax,"%", 1)
li_pos2 = pos(is_syntax,"$", 1)

li_table = integer(mid(is_syntax, 1, li_pos -1))
is_screen = mid(is_syntax, li_pos + 1, li_pos2 - 1 - li_pos)

//messagebox("screen",is_screen)

is_syntax = mid(is_syntax,  li_pos2 + 1)
//clipboard(is_syntax)
...
//	j = LibraryImport( gs_dir_path + gs_DefDirName + "\report_temp.pbd","syntax_test", ImportDataWindow!, is_syntax, ErrorBuffer)
...
ls_file = gs_dir_path + gs_DefDirName + "\syntaxfile.isg"

filedelete(ls_file)

ll_len = len(is_syntax)
...
FOR li_counter = 1 to li_loops 
	ls_temp = mid(is_syntax,ll_start,32765)
