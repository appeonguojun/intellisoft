...
If len(lds_project_cols.describe('t1_col_name.coltype')) > 2 Then ls_col_name = 't1_col_name'
If len(lds_project_cols.describe('col_name.coltype')) > 2 Then ls_col_name = 'col_name'
If len(lds_project_cols.describe('syscolumns_col_name.coltype')) > 2 Then ls_col_name = 'syscolumns_col_name'
If len(lds_project_cols.describe('syscolumn_col_name.coltype')) > 2 Then ls_col_name = 'syscolumn_col_name'

For i = 1 to lds_project_cols.Rowcount()
	ls_import_project_cols += lds_project_cols.GetItemString(i,ls_col_name) + ','
Next
ls_import_project_cols = Mid(ls_import_project_cols,1,len(ls_import_project_cols) - 1)
...
If Upper(Left(ls_parm,3)) = 'ADD' Then
	is_type = 'ADD'
	This.Title = 'Add Project'
ElseIf Upper(Left(ls_parm,4)) = 'COPY' Then
	is_type = 'COPY'
	il_project_id = Long(Mid(ls_parm,5,Len(ls_parm) - 4 ))
	This.Title = 'Copy Project'
	Ids_project = Create DataStore
ElseIf Upper(Left(ls_parm,6)) = 'RENAME' Then
	is_type = 'RENAME'
	sle_p_name.text = Mid(ls_parm,7,Len(ls_parm) - 6 )
...
long backcolor = 67108864
string text = "Please enter project name:"
