...
// Get il_import_id
li_OldPos = Pos(Message.StringParm, "|")
il_import_id = Long(Left(Message.StringParm, li_OldPos - 1))
li_OldPos ++

// Get is_import_name
li_NewPos = Pos(Message.StringParm, "|", li_OldPos)
is_import_name = Mid(Message.StringParm, li_OldPos, li_NewPos - li_OldPos)

// Get is_imp_field_name
is_imp_field_name = Right(Message.StringParm, Len(Message.StringParm) - li_NewPos)
...
for i = 1 to lds_imp_data.RowCount()
	...
	if len(ls_value) < 1 or isnull(ls_value) then ls_Value = ''  //Start Code Change ----02.08.2008 #V8 maha - test for blanks blanks
	ll_FindRow = dw_data.Find("valuelist = '" + ls_Value + "'", 1, dw_data.RowCount())
...
li_Return = ln_getfilename.of_getsavefilename("Save As", ls_Path, ls_File, "TXT", "Text Files (*.TXT),*.TXT" )
