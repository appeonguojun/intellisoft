...
If dwo.name = 'b_ctx_search' Then
	...
	If ll_pos > 0 Then //Selected multi-rcontract
		...
		Return
	Else
		ll_pos = Pos(ls_data,'~t')
		If ll_pos > 0 Then
			ll_ctx_id = Long(Mid(ls_data,1,ll_pos - 1) )
...
If dwo.name = "b_contact_add" Then//added by gavins 20140605
	str_pass 	lstr_pass
	String		ls_Company
	...
	lstr_pass.s_string = "Select_Import_Contact"
	lstr_pass.contact_id[1] = GetItemNumber( row, 'ctxcta_contact_id' )
	lstr_pass.display_contact_detail = true	
	OpenWithParm ( w_contract_contact, lstr_pass)
	ll_contact_id = Message.doubleParm
	If ll_contact_id > 0 Then
		SetItem( row, 'ctxcta_contact_id', ll_Contact_id )
	End If
End If

If dwo.name = "b_contact_clear" Then//added by gavins 20140605
	decimal ld_null
	setnull( ld_null )
	SetItem( row, 'ctxcta_contact_id', ld_null )
End If
end event
