...
for i = 1 to dw_1.rowcount()
	...
	//test length 3 to 40
	if len(ls_field) < 3 or len(ls_field) > 40 then
		if lb_correct = true then 
			choose case len(ls_field)
				case 1
					ls_field+= "XX"
				case 2
					ls_field+= "X"
			end choose
			if len(ls_field) > 40 then
				debugbreak()
				ls_field = mid(ls_field,1,40)
			end if
		else
...
if len(is_mess_all) > 0 then ls_mess = "~r" + ls_mess //put return on beginning of string
