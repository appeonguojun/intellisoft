$PBExportHeader$w_ctx_category_select.srw
forward
global type w_ctx_category_select from w_response
end type
type st_1 from statictext within w_ctx_category_select
end type
type dw_view from u_dw within w_ctx_category_select
end type
type cb_select from commandbutton within w_ctx_category_select
end type
type cb_cancel from commandbutton within w_ctx_category_select
end type
type cb_ok from commandbutton within w_ctx_category_select
end type
end forward

global type w_ctx_category_select from w_response
integer width = 1362
string title = "Select Contract Category"
st_1 st_1
dw_view dw_view
cb_select cb_select
cb_cancel cb_cancel
...
global w_ctx_category_select w_ctx_category_select
...
on w_ctx_category_select.create
int iCurrent
call super::create
...
on w_ctx_category_select.destroy
call super::destroy
destroy(this.st_1)
...
If dw_view.RowCount() = 0 Then
	Messagebox('Error','Failed to retrieve Contract Category data.')
...
string text = "Only selected contract category can Auto-Syncing contract."
...
integer height = 1136
integer taborder = 10
string dataobject = "d_ctx_category_choose"
end type
...
If dw_view.Find('selected = 1', 1, dw_view.RowCount()) = 0 Then
	Messagebox('ContractLogix','You need to select at least one Contract Category.')
	Return
End If
