...
if gs_DBType = "ASA" then //No ASA for CL
...
//	CLOSE cur_keys;
	...
	DECLARE lp_pkeys PROCEDURE FOR sp_appeon_pkeys @table_name = :ls_table_name; //for a apb 60 bug - alfee 03.23.2010
	EXECUTE lp_pkeys;
	do while true
		...
		if SQLCA.SQLCode = 0 then
			...
			gnv_debug.of_output(true, 'failed to get the PK.' + sqlca.sqlerrtext )
			exit
		end if
	loop
	CLOSE lp_pkeys;
