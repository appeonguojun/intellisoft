...
type cb_preprocess from commandbutton within w_import_painter
end type
end forward
...
// Returns:  (none)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 07-31-2015
//--------------------------------------------------------------------
...
ll_result = GetFolder( "Select the folder for import defintion", ls_FilePath )
...
if FileExists(ls_filename) then
	...
	messagebox('Contractlogix','Import defintion file can not find in the directory ' + ls_FilePath + '.')
	return
end if
...
messagebox('Contractlogix','Import the import defintion from ' + ls_FilePath + ' success.')
end event
...
if ll_row < 1 then
	messagebox('Contractlogix','Please select import first.')
	return
else
...
messagebox('Contractlogix','Export the import defintion to ' + ls_FilePath + ' success.')
end event
...
//Added By Jay Chen 07-31-2015
if ai_Index = 1 then
	gnv_app.of_modify_menu_attr( m_pfe_cst_import_painter.m_file.m_importmap,'toolbaritemvisible', true)
	gnv_app.of_modify_menu_attr( m_pfe_cst_import_painter.m_file.m_exportmap,'toolbaritemvisible', true)
else
	gnv_app.of_modify_menu_attr( m_pfe_cst_import_painter.m_file.m_importmap,'toolbaritemvisible', false)
	gnv_app.of_modify_menu_attr( m_pfe_cst_import_painter.m_file.m_exportmap,'toolbaritemvisible', false)
end if
//end
...
choose case ls_table_name
	...
	case 'ctx_custom_multi_detail' //added by gavins 20130222
		...
		End If
	case 'security_users' 	//Added By Mark Lee 04/16/2014  //domain_name  //ic_ldap_user   Modified by gavins 2017/9/26 task 884
		ls_NotInList = "'user_password','image_flag','email_id','iverify_temp_id','default_search_facility','version_build','prac_id','pref_win_left','pref_win_top','last_pass_update' " +&
			...
			" ,'npdb_user_id','set_task','open_tdl','set_1','set_2','set_3','set_4'" +  &
			" ,'npdb_password','image_prompt_flag','lic_agr_img','lic_agr_asa','set_prelod_ctx' " + &
	...
	case 'ctx_am_document' //Added By Jay Chen 10-10-2014
		...
		" ,'esign_type','template_ctx_id','es_license_user_id','salesforce_id','alm_appointment','revision_control'  "	  //Modified by harry 2017-09-15   remove",'filename'"
	case else
		Return ""
end choose
...
dw_browse.SetItem(ll_InsertRow, "import_type", '1') //Added By Jay Chen 10-11-2014
end event
...
string dataobject = "d_import_browse_file"
...
string dataobject = "d_import_hdr_file"
boolean hscrollbar = true
boolean vscrollbar = false
boolean livescroll = false
borderstyle borderstyle = stylebox!
...
choose case dwo.name
	...
	case 'trigger_wf' //Added By Ken.Guo 2015-06-18
		If data = '1' Then
			Post Messagebox('Tips','When Trigger Work Flow is checked, New Contract type and Data Value Comparison type work flows will be triggered during the data import. Additionally, messages will be suppressed in this process.')
		End If
end choose
end event
...
//Commented by harry 2017-09-15
/*
//Added By Jay Chen 10-10-2014
if is_ImportType = '1' then
	dw_tables.setfilter(" table_id <> 1013 ")
	dw_tables.filter()
end if
//end
*/
...
If is_ImportType = '1' Then
	dw_tables.SetFilter("isoft_module = " + String(ii_curr_isoft_module))  //Modified By Harry 2017-09-15
...
choose case dwo.name
	case "imp_field_name"
		...
		if ll_FindRow > 0 then
			ll_findorder = this.getitemnumber(ll_FindRow,"field_order")
			this.Post SetItem(Row, "imp_field_name", "")
			this.Post SetColumn("imp_field_name")
			this.Post SetFocus()
			messagebox("Invalid Field Name","The Field Name (" + data + ") is already used for the field of the order " + string(ll_findorder) + ". Please use another name.") //Added By Jay Chen 01-09-2015
		end if
...
if ibn_freeform_move = true then 							//Added By Mark Lee 11/12/2015		 
...
this.Modify("sys_doc_id.visible=0")	//Added By Harry 2017-09-18
...
n_cst_import_credapp lnv_import_credapp
n_cst_import_contract lnv_import_contract
Long ll_basic_info  //Added by Harry 2017-09-13
...
if ii_curr_isoft_module = 1 then
	...
	lnv_import_contract = Create n_cst_import_contract
	//Added by Harry 2017-09-07 
	if tab_1.tabpage_field_data.dw_records.RowCount() > 0 then
		ll_basic_info =  tab_1.tabpage_field_data.dw_records.Find("isoft_table_id=1001", 1, tab_1.tabpage_field_data.dw_records.RowCount()) 
	else
		select isnull(count(*), 0) into :ll_basic_info from import_table_records where import_id = :il_curr_import_id and isoft_table_id = 1001;
	end if
	if tab_1.tabpage_hdr.dw_imp_hdr.GetItemNumber(tab_1.tabpage_hdr.dw_imp_hdr.GetRow(), 'add_protocol') = 1 and ll_basic_info > 0 then
		open(w_import_ctx_template)
		lnv_import_contract.il_ctx_template_id = message.doubleparm		
	else
		lnv_import_contract.il_ctx_template_id = 0
	end if
...
ls_SQLSelect = "SELECT " + ls_SQLSelect + ",0 sys_pc_id, 0 sys_facility_id, 0 sys_contact_id, 0 sys_hdr_id, '' as sys_user_id, 0 sys_doc_id FROM " + ls_TableName  //Added by harry 2017-09-18 add 0 sys_doc_id
