...
type variables
constant 	uLong 		WM_GETFONT =  49						//Added By Mark Lee 05/15/2013
end variables
...
public function integer of_exists_sql_keyword (string as_source)
public function string of_replace_env_var (string as_name)
end prototypes
...
public function boolean of_check_name (string as_data, integer ai_check_type, boolean ab_showmessage);//Check single quotes
...
public function integer of_exists_sql_keyword (string as_source);//====================================================================
// Function: of_exists_sql_keyword
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
//
//====================================================================
String 	ls_sql_keyword, ls_replace

if isnull(as_source) or trim(as_source) = '' then 
	return 0
end if

ls_sql_keyword = "[ADD],[EXCEPT],[PERCENT],[ALL],[EXEC],[PLAN],[ALTER],[EXECUTE],[PRECISION],[AND],[EXISTS],[PRIMARY],[ANY],[EXIT],[PRINT],[AS],[FETCH],[PROC]," + & 
						...
						"[DECLARE],[NULL],[TSEQUAL],[DEFAULT],[NULLIF],[UNION],[DELETE],[OF],[UNIQUE],[DENY],[OFF],[UPDATE],[DESC],[OFFSETS],[UPDATETEXT],[DISK]," + &
						"[ON],[USE],[DISTINCT],[OPEN],[USER],[DISTRIBUTED],[OPENDATASOURCE],[VALUES],[DOUBLE],[OPENQUERY],[VARYING],[DROP],[OPENROWSET],[VIEW]," + &
						"[DUMMY],[OPENXML],[WAITFOR],[DUMP],[OPTION],[WHEN],[ELSE],[OR],[WHERE],[END],[ORDER],[WHILE],[ERRLVL],[OUTER],[WITH],[ESCAPE],[OVER]," + &
						"[WRITETEXT],[]"


ls_replace = '['+trim(as_source)+']'

if pos(ls_sql_keyword,upper(ls_replace))  > 0 then 
	return -1
end if 
...
//--------------------------------------------------------------------
// Description:  Replace the env var with value
//--------------------------------------------------------------------
// Arguments:
//                as_name
//--------------------------------------------------------------------
// Returns:  string
//--------------------------------------------------------------------
// Author:         Ken.Guo           Date: 12/10/2013
//--------------------------------------------------------------------
...
String ls_name, ls_env_name, ls_env_value, ls_pre_env_name
Long ll_pos_s, ll_pos_e, ll_pre_pos
...
ll_pos_s = Pos(ls_name, '%')
ll_pos_e = Pos(ls_name, '%', ll_pos_s + 1 )
ls_env_name = Mid(ls_name, ll_pos_s, ll_pos_e - ll_pos_s + 1)

Do While (ll_pos_s > 0 and ll_pos_e > 0 and ll_pre_pos <> ll_pos_s and ls_pre_env_name <> ls_env_name)
	ll_pos_e = Pos(ls_name, '%', ll_pos_s + 1 )
	ls_env_name = Mid(ls_name, ll_pos_s + 1, ll_pos_e - ll_pos_s - 1)
	ls_env_value = lnv_systeminfo.of_get_envvar( ls_env_name )
	If ls_env_value <> '' Then
		ls_name = Replace(ls_name, ll_pos_s,ll_pos_e - ll_pos_s + 1,ls_env_value )
	End If
	
	//Save pre var
	ll_pre_pos = ll_pos_s
	ls_pre_env_name = ls_env_name	
