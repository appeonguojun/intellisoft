...
type variables
Integer				ii_Width = 1019, ii_SID
Boolean				ib_Dock_Prevent

//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// <RP> 8.8.2017
boolean				ib_modernUI, ib_ignoreIcons
//string					TRANSPARENCY_NORMAL = "50", &
//						TRANSPARENCY_HOT = "0"
long					COLOR_FRAME_BACKGROUND = rgb(247, 247, 247)
string					TEXT_COLOR_NORMAL = string(rgb(106, 106, 106)), &
						TEXT_COLOR_HOT = string(rgb(0, 0, 0)), &
						TEXT_COLOR_DISABLED = string(rgb(136, 136, 136)), &
						HEAD_BACKGROUND_COLOR = string(rgb(238, 238, 238)), &
						ROW_BACKGROUND_COLOR = string(rgb(255, 255, 255)), &
						HEAD_FONT_NAME = "Segoe UI", &
						HEAD_FONT_SIZE = "10", &
						HEAD_FONT_BOLD = "1", &
						ROW_FONT_NAME = "Segoe UI Semibold", &
						ROW_FONT_SIZE = "10", &
						ROW_FONT_BOLD = "0"
...
long					il_orig_titlebar_x = -1, &
						il_orig_titlebar_width = -1, &
						il_orig_titlebar_height = -1, &
						il_orig_items_y = -1, &
						il_orig_title_x = -1

long					il_orig_split_linecolor, &
						il_orig_backcolor
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
String					is_User ='contractlogix-super-master'
...
public function integer uof_setmodern ()
public function integer of_getmodernuisettings ()
end prototypes
...
public function integer of_click_item (menu am_menu, string as_click_items);Long				ll_i, ll_k, ll_o
...
//Add splitbar
string ls_showsplitbar  //Added By Jay Chen 02-24-2014
lb_Cascade = False
lb_LastCascade =False
If ll_Count > 0 Then ll_order = ids_Store.object.groupno[1]

//Added By Jay Chen 02-24-2014
ls_showsplitbar = gnv_user_option.of_Get_Option_value( is_User, 'EZMENU_SHOWSPLITBAR' )
if isnull(ls_showsplitbar) then ls_showsplitbar = ''
if  trim(ls_showsplitbar) = '' Or ls_showsplitbar = '1' then
	For ll_i = 1 To ll_Count
		If ids_Store.object.items_sorts3[ll_i] <> 100  And Not lb_Cascade Then
			ls_ParentMenu = ids_Store.object.parentmenu[ll_i]
			lb_Cascade = True
...
//Added By Jay Chen 02-24-2014 Remark:exchange the MDI Menu to the top
string ls_showmdimenutop
ls_showmdimenutop = gnv_user_option.of_Get_Option_value( is_User, 'EZMENU_SHOWMDIMENUTOP' )
If isnull(ls_showmdimenutop) Then ls_showmdimenutop = ''
If ls_showmdimenutop = '1' Then
	ll_Find = ids_Store.Find( "groupno = 2", 1, ids_store.Rowcount( ) )
	If ll_Find > 0 Then 
		ids_Store.RowsMove( ll_Find,ll_Find, primary!, ids_Store, 1, primary! )
		ids_Store.RowsMove( ll_Find + 1, ids_Store.RowCount( ), primary!, ids_Store, 1, primary! )
	end if
end if
//end add
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// <RP> 8.25.2017 - Add IF ib_modernUI check before certain statements...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
IF ib_modernUI THEN
	ls_Value = HEAD_FONT_NAME
ELSE
...
IF ib_modernUI THEN
	ls_Value = HEAD_BACKGROUND_COLOR
ELSE
...
// Portions will be overwritten by uof_setModern()
IF ib_modernUI THEN
	ls_Value = TEXT_COLOR_NORMAL
ELSE
...
//Added By Jay Chen 02-24-2014
string ls_color,ls_Value2
IF ib_modernUI THEN
	ls_Value2 = ROW_BACKGROUND_COLOR
ELSE
	ls_Value2 = gnv_user_option.of_Get_Option_value( is_User, 'EZMENU_' + 'rowbackcolor2' )
END IF
if isnull(ls_Value2) or ls_Value2 = '' then ls_Value2 = '16244694'
ls_color = "33554431~tif(groupno = 9, "+ls_Value2+", "+ls_Value+")"
If Len( ls_Value ) > 0 Then ls_modify += " itempic.background.color='"+ls_color+"'"
//end add

IF ib_modernUI THEN
	ls_Value = ROW_FONT_BOLD
ELSE
...
If Not( isnull( ls_toolbarvalue ) Or trim( ls_toolbarvalue ) = '' )  and am_menu.toolbaritemvisible  Then
	lb_Find = True
	If ls_toolbarvalue <> am_menu.toolbaritemtext Then am_menu.toolbaritemtext = ls_toolbarvalue + of_Getstring( ls_OriginalText, 'tips' )//modify by gavins 20131210
End If	
...
public function integer uof_setmodern ();//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Intertech - quick update to modernize look
//
// 8.10.2017 - Initial version by <RP>
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
IF ib_modernUI THEN
	this.object.l_split1.pen.color = rgb(222, 219, 222)
	this.object.l_split2.visible = 0
	ls_val = this.describe("itempic.x")
	IF isNumber(ls_val) THEN
		this.modify("p_titlebar.x=" + ls_val)
	END IF
	ls_val = this.describe("itempic.width")
	IF isNumber(ls_val) THEN
		this.modify("t_items_bg.width=" + ls_val)
		this.modify("p_titlebar.width=" + ls_val)
	END IF
	this.modify("p_titlebar.height=" + string(il_orig_titlebar_height + 12))
	ls_val = this.describe("items.y")
	ls_ret = this.modify("items.y=" + string(il_orig_items_y - 8))
	/* picture */
	ls_ret = this.modify("pic.y=" + string(il_orig_items_y - 4))
	ls_val = this.describe("t_title.x")
	ls_ret = this.modify("t_title.x=" + string(il_orig_title_x + 18))
	BAR_TOP = 24
	/* Remove 3-D border from p_titlebar */
	this.modify("p_titlebar.border='0'")
	this.object.datawindow.color = COLOR_FRAME_BACKGROUND

	ls_ret = this.modify("p_titlebar.y=" + string(BAR_TOP))
	ls_ret = this.modify("t_title.y=" + string(BAR_TOP + 12))
	ls_ret = this.modify("t_painter.y=" + string(BAR_TOP + 16))
	ls_ret = this.modify("t_refresh.y=" + string(BAR_TOP + 16))
	ls_ret = this.modify("t_contract.y=" + string(BAR_TOP + 12))
	ls_ret = this.modify("t_expand.y=" + string(BAR_TOP + 12))
	
	ls_val = this.describe("p_titlebar.y")
	IF isNumber(ls_val) THEN
		li_val = integer(ls_val)
	END IF
	ls_val = this.describe("p_titlebar.height")
	IF isNumber(ls_val) THEN
		li_val = li_val + integer(ls_val)
	END IF
	IF li_val > 0 THEN
		ls_ret = this.modify("datawindow.header.height=" + string(li_val) + "")
	ELSE
		ls_ret = this.modify("datawindow.header.height=128")
	END IF
	
	ls_val = TEXT_COLOR_NORMAL + "~t" + "if( enabled = 0, " + TEXT_COLOR_DISABLED + ", if( getrow() = currentrow(), " + TEXT_COLOR_HOT + ", " + TEXT_COLOR_NORMAL + " ) )"
	ls_ret = this.modify("items.color='" + ls_val + "'")
	ls_ret = this.modify("items.font.underline=no")
	
	/* Set text color */
	/* Handled in of_set_screen() */
//	this.modify("t_title.Color='" + TEXT_COLOR_NORMAL + "'")
//	this.modify("t_painter.Color='" + TEXT_COLOR_NORMAL + "'")
//	this.modify("t_refresh.Color='" + TEXT_COLOR_NORMAL + "'")
//	this.modify("t_contract.Color='" + TEXT_COLOR_NORMAL + "'")
//	this.modify("t_expand.Color='" + TEXT_COLOR_NORMAL + "'")	
END IF
...
public function integer of_getmodernuisettings ();//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// <RP> 8.25.2017
// Check preferences for modern UI settings
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
string ls_val, ls_ret

IF NOT isValid(gnv_user_option) THEN
	ib_modernUI = FALSE
	ib_ignoreIcons = FALSE
ELSE
	ls_val =gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_forcemodernstyle' )
	ib_modernUI = (ls_val = "1")
	ls_val =gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_ignorecustomicons' )
	ib_ignoreIcons = (ls_val = "1")
END IF

/* NOTE: Assume that this is called from of_set_menu_quick() AFTER DW sharing is turned off. */
IF ib_modernUI THEN
	IF this.dataobject <> "d_ezmenu_list_modern" THEN
		this.dataobject = "d_ezmenu_list_modern"
//		this.event constructor()
		/* script from constructor() */
		if AppeonGetClientType() = 'PB' then	
		...
		of_Set_screen( )
	END IF
	IF il_orig_titlebar_x < 0 THEN
		/* Values have not yet been saved. */
		ls_val = this.describe("p_titlebar.x")
		IF isNumber(ls_val) THEN
			il_orig_titlebar_x = Long(ls_val)
		END IF
		ls_val = this.describe("p_titlebar.width")
		IF isNumber(ls_val) THEN
			il_orig_titlebar_width = Long(ls_val)
		END IF
		ls_val = this.describe("p_titlebar.height")
		IF isNumber(ls_val) THEN
			il_orig_titlebar_height = Long(ls_val)
		END IF
		ls_val = this.describe("items.y")
		IF isNumber(ls_val) THEN
			il_orig_items_y = Long(ls_val)
		END IF
		ls_val = this.describe("t_title.x")
		IF isNumber(ls_val) THEN
			il_orig_title_x = Long(ls_val)
		END IF
		il_orig_split_linecolor = long(this.object.l_split1.pen.color)
		il_orig_backcolor = long(this.object.datawindow.color)
	END IF
ELSE
	IF this.dataobject <> "d_ezmenu_list" THEN
		this.dataobject = "d_ezmenu_list"
//		this.event constructor()
		/* script from constructor() */
		if AppeonGetClientType() = 'PB' then	
		//	This.Modify( "pic.visible=0 t_checked.x=" + This.Describe( "pic.x" )  )	
...
if dwo.name = 't_painter' then
	IF w_mdi.of_security_access( 6985 ) = 0 THEN
		Messagebox('Information',"You do not have permission to execute it.")
	Else
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// <RP> 8.10.2017
// Execute previous code conditionally
IF NOT ib_modernUI THEN
...
ELSE
	...
	// <RP> 8.10.2017
	IF ls_object = 't_contract' then
		IF this.describe("t_contract.Color") <> TEXT_COLOR_HOT then this.modify("t_contract.Color='" + TEXT_COLOR_HOT + "'")
	ELSE
		IF this.describe("t_contract.Color") <> TEXT_COLOR_NORMAL then this.modify("t_contract.Color='" + TEXT_COLOR_NORMAL + "'")
	END IF
	IF ls_object = 't_expand' then
		IF this.describe("t_expand.Color") <> TEXT_COLOR_HOT then this.modify("t_expand.Color='" + TEXT_COLOR_HOT + "'")
	ELSE
		IF this.describe("t_expand.Color") <> TEXT_COLOR_NORMAL then this.modify("t_expand.Color='" + TEXT_COLOR_NORMAL + "'")
	END IF
	IF ls_object = 't_refresh' then
		IF this.describe("t_refresh.Color") <> TEXT_COLOR_HOT then this.modify("t_refresh.Color='" + TEXT_COLOR_HOT + "'")
	ELSE
		IF this.describe("t_refresh.Color") <> TEXT_COLOR_NORMAL then this.modify("t_refresh.Color='" + TEXT_COLOR_NORMAL + "'")
	END IF
	IF ls_object = 't_painter' then
		IF this.describe("t_painter.Color") <> TEXT_COLOR_HOT then this.modify("t_painter.Color='" + TEXT_COLOR_HOT + "'")
	ELSE
		IF this.describe("t_painter.Color") <> TEXT_COLOR_NORMAL then this.modify("t_painter.Color='" + TEXT_COLOR_NORMAL + "'")
	END IF
	//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
END IF
// <RP> 8.10.2017
...
////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//// <RP> 8.25.2017 - get values of ib_moderUI and ib_ignoreIcons
////<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//of_getModernUISettings()
//
//////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
////ls_val = this.describe("p_titlebar.height")
////IF isNumber(ls_val) THEN
////	this.modify("p_titlebar.height=" + string(Integer(ls_val) + 12))
////END IF
////ls_val = this.describe("items.y")
////IF isNumber(ls_val) THEN
////	ls_ret = this.modify("items.y=" + string(integer(ls_val) - 8))
////	/* picture */
////	ls_ret = this.modify("pic.y=" + string(integer(ls_val) - 4))
////END IF
////ls_val = this.describe("t_title.x")
////IF isNumber(ls_val) THEN
////	ls_ret = this.modify("t_title.x=" + string(integer(ls_val) + 18))
////END IF
//IF ib_modernUI THEN
//	this.uof_setModern()
//END IF
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// <RP> 8.10.2017 - set row to a filler row, so that an item doesn't appear to be selected
long ll_row
IF ib_modernUI THEN
	ll_row = this.find("len(items) = 0", 1, this.rowCount())
	IF ll_row > 0 THEN
		this.setRow(ll_row)
	END IF
END IF
