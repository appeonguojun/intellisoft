...
if Trim(is_AppPath) = "" then
	...
	if Right(is_AppPath,1) <> "\" then is_AppPath += "\"
	
	ll_Pos = Pos(is_AppPath,"\")
	do while ll_Pos > 0
		is_AppPath = Replace(is_AppPath,ll_Pos,1,"$")
		ll_Pos = Pos(is_AppPath,"\")
	loop
...
do while IsValid(lgo_Temp) and TypeOf(lgo_Temp) <> Window!
	...
	if TypeOf(lgo_Temp) = datawindow! then
		...
		do while ll_Pos > 0
			ls_LibFile = Left(ls_LibList,ll_Pos - 1)
			ls_LibList = Mid(ls_LibList,ll_Pos + 1)
			ll_Pos = Pos(ls_LibList,",")
			
			ls_DataInfo = "~n" + LibraryDirectory(ls_LibFile, DirDataWindow!)
			
			if Pos(ls_DataInfo,"~n" + ls_DaObj + "~t") > 0 then
				if Pos(ls_LibPath,ls_LibFile) <= 0 then
...
if TypeOf(lgo_Temp) = Window! then
	...
	if Pos(ls_LibPath,lcd_ClassDef.LibraryName) <= 0 then
...
if lb_IsUserObj and IsValid(lgo_UserObj) then
	...
	do while IsValid(lcd_Temp) and lower(lcd_Temp.Name) <> 'userobject'
		...
		if Pos(ls_LibPath,lcd_Temp.LibraryName) <= 0 then
	...
	if Pos(ls_UserObjInfo,"---->") > 0 then ls_Temp += ls_UserObjInfo
end if
////////////////////////////////////////////////////////////////////////////
