...
forward
global type pfc_cst_u_application_audit from userobject
end type
...
IF POS( ls_phone_number, "(" ) > 0 THEN
	IF Mid(ls_phone_number, POS( ls_phone_number, "(" ) +1 ,3) = ls_local_area_code THEN
		ls_phone_number = Mid( ls_phone_number, POS( ls_phone_number, ")" ) +1, 100 )
	END IF
ELSE
	IF Mid(ls_phone_number,1,3) = ls_local_area_code THEN
		ls_phone_number = Mid( ls_phone_number, 4, 100 )
	END IF	
END IF
...
public function integer of_print_list ();IF dw_detail.rowcount() > 0 THEN
	dw_detail.Print()	
END IF
...
//Start Code Change ---- 10.30.2006 # maha
//if ac < 1 then
//	//open(w_select_fac_for_app_audit)
//	//Messagebox("No data","You cannot use this function if there are no template items.") //modified to get items if none exist
//	if gs_cust_type = "I" then //intelliapp
//		ls_app_type = "I"
//		li_facid = 1
//		//ii_parent = 1
//		select rec_id, apptmnt_type into :ll_appt_stat_id,:ls_app_type from pd_affil_stat where prac_id = :il_prac_id  and parent_facility_id = 1;
//	else //intellicred
//		li_facid = dw_orgs_facilities.getitemnumber(dw_orgs_facilities.getrow(),"audit_facility")
////Start Code Change ---- 01.26.2006 #222 maha		
//		li_parent = dw_orgs_facilities.getitemnumber(dw_orgs_facilities.getrow(),"parent_facility_id")  //maha 012606
////End Code Change---01.26.2006 		
//		select rec_id,apptmnt_type into :ll_appt_stat_id,:ls_app_type from pd_affil_stat where prac_id = :il_prac_id  and parent_facility_id = :li_parent and active_status in (1,4);
//	end if
//	if li_facid < 1 then return -1
//	//ls_app_type = "I" //maha added to select 061505
//else
//	ll_appt_stat_id = dw_detail.GetItemNumber( dw_detail.GetRow(), "appt_stat_id" )
//	li_facid = dw_detail.GetItemNumber( dw_detail.GetRow(), "appt_stat_id" )
//	select apptmnt_type, application_audit_facility into :ls_app_type,:li_facid from pd_affil_stat where rec_id = :ll_appt_stat_id;
//	
//end if
...
//ldw_rqrd_data.of_SetTransObject( SQLCA )
//
//li_row_cnt = ldw_rqrd_data.Retrieve(li_facid, ls_app_type )
...
////	IF NOT lb_find THEN
////		If dw_detail.Find ( "app_audit_id = " + string(ll_app_audit_id),  1, ac ) < 1 then
////			lb_find = TRUE
////		End If
////	END IF
////Next
...
////
//////060308
//////lds_last_app_audit_seq_no.Retrieve( ll_app_audit_id_arr, il_prac_id, ii_parent )	
////lds_last_app_audit_seq_no.Retrieve( ll_app_audit_id, il_prac_id, li_facid )	

////ll_record_id = gnv_app.of_get_id("RECORD_ID" , li_row_cnt) 
...
event constructor;IF Upper( gs_cust_type ) = "I" THEN
...
//---------------------------- APPEON END ----------------------------
	dw_browse.Retrieve( il_prac_id, This.GetItemNumber( currentrow, "parent_facility_id" ) )	
	dw_browse.SelectRow( 0, False )
	dw_browse.SelectRow(1, True)
	ll_rec_id = dw_browse.GetItemNumber( 1, "rec_id" )
	of_retrieve_detail( ll_Rec_id )
...
event pfc_postupdate;call super::pfc_postupdate;//Start Code Change ----09.10.2008 #V85 maha - modified to use new of_app_audit_complete function
Integer li_facility_id
Integer li_found
Integer li_rec_no
Integer li_comp[]
...
IF li_cnt = 1 THEN //no incompletes	
	...
	//Start Code Change ----04.09.2008 #V8 maha - reworked code logic
		...
		if li_ver_cnt > 0 then //if there are connected records //04.09.08 changed variable
			if ll_res = 0 then //if no default is set
				...
				if mid(message.stringparm,1,1) = "N" then 
					continue //cancel
				else
					ll_res = long(mid(message.stringparm,3)) //get the returned response code
				end if
			end if
...
event getfocus;call super::getfocus;IF IsValid( m_pfe_cst_app_audit ) THEN
...
event getfocus;call super::getfocus;IF IsValid( m_pfe_cst_app_audit ) THEN
	gnv_app.of_modify_menu_attr( m_pfe_cst_app_audit.m_edit.m_insert,'enabled', True)
	gnv_app.of_modify_menu_attr( m_pfe_cst_app_audit.m_edit.m_delete1,'enabled', True)
END IF
end event
