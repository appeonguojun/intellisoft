...
ls_Value = gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_' + 'SHOWMDIMENUTOP'  )
If Len( ls_Value ) > 0 Then dw_2.setItem( dw_2.GetRow( ), 'showmdimenutop' , long( ls_Value ) )
ls_Value = gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_' + 'SHOWSPLITBAR'  )
If Len( ls_Value ) > 0 Then dw_2.setItem( dw_2.GetRow( ), 'showsplitbar' , long( ls_Value ) )
//End add
...
//Added By Jay Chen 02-24-2014
ls_Value =gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_' + 'rowbackcolor2' )
If Len( ls_Value ) > 0 Then  dw_2.setItem( dw_2.GetRow( ), 'rowbackcolor2', long(ls_Value ) )
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// <RP> 8.25.2017
ls_Value =gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_forcemodernstyle' )
If Len( ls_Value ) > 0 Then dw_2.setItem( dw_2.GetRow( ), 'forcemodernstyle', long(ls_Value ) )
ls_Value =gnv_user_option.of_Get_Option_value( Is_User, 'EZMENU_ignorecustomicons' )
If Len( ls_Value ) > 0 Then dw_2.setItem( dw_2.GetRow( ), 'ignorecustomicons', long(ls_Value ) )
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
event pfc_postopen;call super::pfc_postopen;//Added By Ken.Guo 2016-03-16. setagain after dwobject changed.
...
dw_2.AcceptText( )
If dw_1.ModifiedCount( ) + dw_2.Modifiedcount( ) = 0 Then
	MessageBox( 'Tips', 'There is no data that need to be saved.' )
	Return
End If
...
If dw_2.ModifiedCount( ) > 0 Then
	...
	gnv_user_option.of_Set_Option_value( Is_User, 'EZMENU_' + 'SHOWMDIMENUTOP' , string(dw_2.GetItemNumber( dw_2.GetRow( ), 'showmdimenutop' )) ) //Added By Jay Chen 02-24-2014
	gnv_user_option.of_Set_Option_value( Is_User, 'EZMENU_' + 'SHOWSPLITBAR' , string(dw_2.GetItemNumber( dw_2.GetRow( ), 'showsplitbar' )) ) //Added By Jay Chen 02-24-2014
	...
	gnv_user_option.of_Set_Option_value( Is_User, 'EZMENU_' + 'rowbackcolor2' , string(dw_2.GetItemNumber( dw_2.GetRow( ), 'rowbackcolor2' ) ) ) //Added By Jay Chen 02-24-2014
	...
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	// <RP> 8.25.2017
	gnv_user_option.of_Set_Option_value( Is_User, 'EZMENU_' + 'forcemodernstyle' , string(dw_2.GetItemNumber( dw_2.GetRow( ), 'forcemodernstyle' ) ) )
	gnv_user_option.of_Set_Option_value( Is_User, 'EZMENU_' + 'ignorecustomicons' , string(dw_2.GetItemNumber( dw_2.GetRow( ), 'ignorecustomicons' ) ) )
	//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<	
...
If row > 0 Then
	...
	Choose Case lower(ls_Object)
		...
		case 't_rowbackcolor2' //Added By Jay Chen 02-24-2014
			ls_column = 'rowbackcolor2'
		case 'head_bold_down'
		...
		case 'b_default'
			...
			This.Object.rowbackcolor2[row] =Long( This.Object.rowbackcolor2.Initial) //Added By Jay Chen 02-24-2014
			...
			//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
			// <RP> 8.25.2017
			This.Object.forcemodernstyle[row] = Long(This.Object.forcemodernstyle.Initial)
			This.Object.ignoreusericons[row] = Long(This.Object.ignoreusericons.Initial)
			//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	End Choose
...
event itemchanged;call super::itemchanged;CHOOSE CASE dwo.name
	CASE "forcemodernstyle"
		/* Mirror the value as the default for column "ignorecustomicons" */
		this.function post setItem(row, "ignorecustomicons", long(data))
END CHOOSE
