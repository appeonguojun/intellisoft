...
Time idt_time,idt_resettime //Added By Jay Chen 04-14-2015
string is_format
boolean ib_date_close = true
...
of_resize() //Added By Jay Chen 04-15-2015
Return of_DropDown()
end event
...
string		ls_rc
Boolean lb_modify_to_ddlb = true //Added By Jay Chen 04-17-2015
...
If of_IsDateType(ls_coltype) Then
	...
	If ai_style = DDLB Or ai_style = DDLB_WITHARROW Then	
		//Added By Jay Chen 09-23-2014 do not display drop down when column's tabsequence is 0.
		string ls_taborder
		ls_taborder = idw_requestor.Describe (as_dwcolumn+".tabsequence")
		if ls_taborder = '0' then return 1
		//end
		...
		CHOOSE CASE Lower(ls_editstyle)
			...
			CASE 'editmask'
				...
				//Do not Support calendar for time format
				If Pos(Upper(ls_descret), 'HH' ) > 0 Then
					if left(Upper(ls_descret),3) = 'HH:' then   //add By Jay Chen 04-17-2015 support time format for begin with hh:
						lb_modify_to_ddlb = false
//					else    //modified by gavins 20151027
// 						Return 1 
					end if
				End If
...
//		//---------------------------- APPEON END ----------------------------	
		...
		if lb_modify_to_ddlb then ls_rc = idw_requestor.Modify (ls_modexp)
...
If IsValid(idw_requestor) Then
	...
	is_format = idw_requestor.describe(ls_colname + ".format") //Added By Jay Chen 04-15-2015
...
Integer li_h,li_m,li_s //Added By Jay Chen 04-14-2015
string ls_h,ls_m,ls_s
time lt_time
...
If IsValid(idw_requestor) Then
	...
	if ls_date <> '' and not isnull(ls_date) then ls_date = string(date(idw_requestor.GetText()))
	lt_time = time(datetime(idw_requestor.GetText()))
ElseIf IsValid(iem_requestor) Then
	ls_date = iem_requestor.Text
	if ls_date <> '' and not isnull(ls_date) then ls_date = string(date(iem_requestor.Text))
	lt_time = time(datetime(iem_requestor.Text))
Else
	Return -1
End If
...
//---------------------------- APPEON END ----------------------------
	...
	setnull(idt_resettime)
else
	if date(ls_date) = date("1900-01-01") then ls_date = string(today())
	...
	END CHOOSE
	idt_resettime = lt_time
end if

id_Today = Today()
//Added By Jay Chen 04-14-2015
idt_time = idt_resettime
if isnull(idt_time) or idt_time = time('00:00:00') then idt_time = Now()
li_h = Hour(idt_time)
li_m = Minute(idt_time )
li_s = Second(idt_time)
if li_h < 10 then
	ls_h = '0' + string(li_h)
else
	ls_h = string(li_h)
...
//Added By Jay Chen 04-14-2015
dw_cal.setitem( 1, 'hour', ls_h) 
dw_cal.setitem( 1, 'minute', ls_m) 
dw_cal.setitem( 1, 'second', ls_s) 
//end
...
//Added By Jay Chen 04-15-2015
time ldt_time
string ls_h,ls_m,ls_s
dw_cal.accepttext()
if dw_cal.rowcount() > 0 then 
	ls_h = dw_cal.getitemstring(1,"hour")
	ls_m = dw_cal.getitemstring(1,"minute")
	ls_s = dw_cal.getitemstring(1,"second")
end if
ldt_time = time(ls_h + ':' + ls_m + ':' + ls_s)
//end
...
If ab_setrequestor Then
	...
	//Added By Jay Chen 04-15-2015
	ls_time = string(ldt_time,'hh:mm:ss')
	if not ib_date_close then
//		if pos(lower(is_format),'hh') > 0 then  //hh:mm
		if left(Upper(is_format),3) = 'HH:' then
			ls_date = string(1901-01-01, is_dateformat) + ' ' + ls_time
		else
			ls_date = ls_date + ' ' + ls_time
		end if
	end if
	if dw_cal.dataobject = 'd_calendar_time' then
		ls_date = string(1901-01-01, is_dateformat) + ' ' + ls_time
	end if
	// Set the requestor with the new date.
	If IsValid(idw_requestor) Then
		idw_requestor.SetText(ls_date)
		idw_requestor.AcceptText() //Added by Ken.Guo on 2009-01-21
	ElseIf IsValid(iem_requestor) Then
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 04-15-2015
//--------------------------------------------------------------------
...
string ls_colname,ls_format,ls_modify
long i,ll_diffy

If IsValid(idw_requestor) Then
	ls_colname = idw_requestor.GetColumnName()
	ls_format = idw_requestor.describe(ls_colname + ".format")
//	if pos(lower(ls_format),'hh') > 0 then // hh:mm only visible time
	if left(lower(ls_format),3) = 'hh:' then
		dw_cal.dataobject = 'd_calendar_time'
		dw_cal.height = 226
		this.height = 226
	else
		dw_cal.dataobject = 'd_calendar'
		dw_cal.height = 632
		this.height = 632
	end if
	If dw_cal.RowCount()=0 Then 
		dw_cal.InsertRow(0)
		if id_date = Date("1900-01-01") then id_date = today() //Added By Jay Chen 05-21-2015
		of_DrawMonth(id_date)
		if not gnv_app.ib_hide_calendar then
			if dw_cal.dataobject = 'd_calendar' then
				dw_cal.object.b_expand.text = '-'
				dw_cal.height = 832
				this.height = 832
			end if
		else
...
//If appropriate, hide the Calendar.
If IsValid(inv_dropdown) Then
	If ib_closeonclick and ib_date_close Then
		// Set focus on the Requestor object which in turns hides the calendar.
		of_SetFocusOnRequestor()
	End If
End If
...
dw_cal.Modify( "month.protect=1 month.background.color=11632216 month.color=16777215" )
dw_cal.Modify( "year.protect=1 year.background.color=11632216 year.EditMask.Spin=no year.color=16777215" )
dw_cal.borderstyle = StyleRaised!
Return
end event
...
//Added By Jay Chen 04-16-2015
this.accepttext()
if row > 0 then
	li_h = long(this.getitemstring(1, "hour"))
	li_m = long(this.getitemstring(1, "minute"))
	li_s = long(this.getitemstring(1, "second"))
end if
//end
...
Choose Case ls_buttonname
	...
	case 'b_5'
		...
		this.setitem( 1, "second", ls_s)
	case 'b_clear'
		this.setitem( 1, "hour", '00')
		this.setitem( 1, "minute", '00')
		this.setitem( 1, "second", '00')
	case 'b_ok'
		If IsValid(inv_dropdown) Then
			of_SetDate(id_date, True)
			Parent.Visible = False 
		End If
	case 'b_expand'
		if dw_cal.object.b_expand.text = '+' then
			dw_cal.object.b_expand.text = '-'
			dw_cal.height = 832
			parent.height = 832
			ib_date_close = false
			gnv_app.ib_hide_calendar = false
		else
			dw_cal.object.b_expand.text = '+'
			dw_cal.height = 632
			parent.height = 632
			ib_date_close = true
			gnv_app.ib_hide_calendar = true
		end if
	//end add
...
Integer	li_days[12]={31,28,31,30,31,30,31,31,30,31,30,31}
Integer li_h,li_m,li_s //Added By Jay Chen 04-17-2015
...
//Added By Jay Chen 04-17-2015
if dwo.Name = 'hour' then
	li_h = Integer( Data )
	If li_h < 0 Or li_h > 23 Then Return 2
end if
if dwo.Name = 'minute' then
	li_m = Integer( Data )
	If li_m < 0 Or li_m > 59 Then Return 2
end if
if dwo.Name = 'second' then
	li_s = Integer( Data )
	If li_s < 0 Or li_s > 59 Then Return 2
end if
end event
...
//Added By Jay Chen 04-17-2015
If (dwo.Name = 'hour' or dwo.Name = 'minute' or dwo.Name = 'second')  and row > 0 Then
	If Len( data ) <= 2 And IsNumber( data ) Then dw_cal.Event ItemChanged( row, dwo, data )	
End If
