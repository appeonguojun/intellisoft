...
event ue_resize ( )
event ue_ini ( )
event ue_select_date ( date ad_date )
em_year em_year
pb_prev pb_prev
pb_next pb_next
dw_cal dw_cal
...
for ll_RowNo = 1 to ROWCOUNT
	for ll_ColNo = 1 to COLCOUNT
		if ll_RowNo = 1 then dw_cal.Modify(DAYNUMMARK + String(ll_ColNo) + ".Text = '" + Left(is_Week[ll_ColNo],3) + "'")
...
for ll_Cycle = 1 to UpperBound(is_Month)
	if Lower(Left(is_Month[ll_Cycle],3)) = Lower(Left(as_Month,3)) then Exit
next
...
ll_Pos = Pos(as_Tag,",")
al_Parm1 = long(Left(as_Tag,ll_Pos - 1))
as_Tag = Mid(as_Tag,ll_Pos + 1)

ll_Pos = Pos(as_Tag,",")
al_Parm2 = long(Left(as_Tag,ll_Pos - 1))
as_Tag = Mid(as_Tag,ll_Pos + 1)

ll_Pos = Pos(as_Tag,",")
al_Parm3 = long(Left(as_Tag,ll_Pos - 1))
as_Tag = Mid(as_Tag,ll_Pos + 1)
...
if ls_ObjName = Lower("RectangleFrame_t") or ls_ObjName = Lower("Today_t") then
	of_Draw_Today()
	Parent.Post Event ue_select_date(id_SelectedDate) //Added by Ken.Guo on 2009-01-20
elseif ls_ObjName = Lower("st_Month") then
	...
	if ll_XPos <= long(dw_Cal.Describe(ls_ObjName + ".X")) + long(dw_Cal.Describe(ls_ObjName + ".Width"))/2 then
		...
		ll_Length = Len(is_Month[ii_Month])
...
ll_Pos = Pos(ls_ObjName,DateCOLMARK)
If ll_Pos <= 0 Then Return

ll_NewColNo = long(Mid(ls_ObjName,ll_Pos + Len(DateCOLMARK)))
