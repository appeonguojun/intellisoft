...
ls_TDL_id = Mid(is_edit_type, Pos(is_edit_type, ":") + 1)
...
if dw_dashboard.RowCount() > 0 then
	if Len(dw_dashboard.GetItemString(1, "module")) > 0 then
...
this.Title += "("+ Mid(is_edit_type, 1, Pos(is_edit_type, ":") - 1) + ")"
//---------------------------- APPEON END ----------------------------
end event
...
ll_TDL_ID = Long(Mid(is_edit_type, Pos(is_edit_type, ":") + 1))
