...
integer width = 3671
integer height = 1988
string title = "Contract Logix Getting Started"
...
string ls_get_start_use_template 

cbx_start.backcolor = RGB(42,134,199)
st_tips.backcolor = RGB(42,134,199)
...
For i = 1 to UpperBound(ipb_module[])
	If ipb_module[i].tag = '1' or ipb_module[i].tag = '1,0' or ipb_module[i].tag = '1,1' Then
		...
		If w_mdi.of_security_access( 6955 ) = 0 Then  //Use Template
		...
		//Added By Jay Chen 06-25-2014
		if w_mdi.of_security_access( 6955 ) > 0 and w_mdi.of_security_access( 6988 ) > 0 then
			ls_get_start_use_template = gnv_user_option.of_get_option_value(gs_user_id, "get_start_use_template")
			if isnull(ls_get_start_use_template) or trim(ls_get_start_use_template ) = '' then ls_get_start_use_template = '0'
			if ls_get_start_use_template = '1'  then
				cbx_use_template.checked = True
			else
				cbx_use_template.checked = False
			end if
...
If Integer(cbx_use_template.tag) > 0 Then
	If This.Checked Then
		...
		gnv_user_option.of_Set_option_value(gs_user_id, "get_start_use_template",'1' ) //Added By Jay Chen 06-25-2014
	Else
		ipb_module[li_index].tag = '1,0'
		gnv_user_option.of_Set_option_value(gs_user_id, "get_start_use_template",'0' ) //Added By Jay Chen 06-25-2014
	End If
	gnv_user_option.of_save( ) //Added By Jay Chen 06-25-2014
End If
