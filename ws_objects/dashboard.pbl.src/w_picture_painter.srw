...
Choose Case w_mdi.of_security_access( 6987 ) 
	Case 0 ,1
		...
		cb_select.x = cb_refresh.x
		cb_refresh.x = cb_save.x
		cb_import_pic.enabled = False
		cb_update_pic.enabled = False
...
If cb_save.enabled = False or cb_save.visible = False Then
	...
	sle_filename.backcolor = This.backcolor
	mle_desc.backcolor = This.backcolor
...
event clicked;Long ll_id, ll_tabpage_used, ll_tdl_used, ll_ezmenu_pic_count,ll_pic_painter_used
String ls_id
...
gnv_appeondb.of_startqueue( )
	...
	Select Count(1) into :ll_pic_painter_used from conv_view_pic Where pic_painter_id = :ll_id;
gnv_appeondb.of_commitqueue( )

//Added By Jay Chen 12-17-2013
If ll_pic_painter_used > 0 Then
	Messagebox('Delete','It cannot be deleted because it is being used by the screen painter picture.',Exclamation!)
	Return
End If
...
//li_Return = GetFileOpenName("Select File(s)", &
//		+ ls_FilePath, ls_FileName[],"bmp","Graphic Files (*.bmp;*.gif;*.jpg;*.jpeg),*.bmp;*.gif;*.jpg;*.jpeg" )		
li_Return = ln_getfilename.of_getopenfilename("Select File(s)", &
		+ ls_FilePath, ls_FileName[],"bmp","Graphic Files (*.bmp;*.gif;*.jpg;*.jpeg),*.bmp;*.gif;*.jpg;*.jpeg" )	
gf_save_dir_path(ls_FilePath) //Added by Ken.Guo on 2009-03-10
