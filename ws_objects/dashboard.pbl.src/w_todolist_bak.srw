	...
	CHOOSE CASE ls_tdl_module
		CASE "01"		//IntelliCred
			...
			if len(trim(ls_ext_sql)) > 0 then
				if len(trim(is_where[ll_row])) > 0 then
					is_where[ll_row] += ls_ext_sql
				else
					is_where[ll_row] = mid(ls_ext_sql,5,len(ls_ext_sql) - 4)
				end if
...
IF NOT lb_tab_visible THEN
	Messagebox(title, "Access denied for Contract Folder->" + ls_tab_name + " tab.")
	iw_contract.tab_contract_details.Selecttab( 1 )
	Return 0
END IF
...
If ll_actionStatus > 0 Then
	...
	If li_Pos > 0 Then
		...
		li_PosBlank = Pos(tv_parent.Label,' ',li_Pos+1)
		If li_PosBlank < 1 Then 
			li_PosBlank = li_Pos
			DO WHILE isNumber(mid(tv_parent.Label,li_PosBlank+1,1))
				li_PosBlank++
			LOOP
		END IF
		
		li_old_Number = Integer(Mid(tv_parent.Label,li_Pos,li_PosBlank - li_Pos + 1))
