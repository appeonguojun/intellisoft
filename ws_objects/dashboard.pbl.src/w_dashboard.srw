...
IF gs_dbtype = "SQL" and appeongetclienttype() = "PB" and ll_length > 8000 THEN
	...
	For li_Cnt = 1 To li_loops				
		...
		IF SQLCA.SQLCode <> 0 THEN
			Messagebox( "ContractLogix", "Read logo file data from database failed." )
			Return ''
		END IF
		lblb_logo += lblb_data
	Next 
ELSE
	...
	WHERE id = 1;
	IF SQLCA.SQLCode <> 0 THEN
		Messagebox( "ContractLogix", "Read log file data from database failed." )
		Return ''
	END IF
END IF
...
If index > 0 Then
	...
	If UpperBound(This.Control[]) >= 3 Then 
		gnv_app.of_modify_menu_attr( lm_dashboard_tab.m_add,'enabled', False)
	End If
...
//Added By Jay Chen 03-16-2016 add '  ' before selectedtab text when select the tab
if AppeonGetClientType() = 'WEB' then 
	string ls_text
	long ll_row

	for ll_row = 1 to upperbound(tab_dashboard.control[])//modified by gavins 20160429
		ls_text = trim(tab_dashboard.control[ll_row].text)
		if left(ls_text,6) <> '      ' then
			tab_dashboard.control[ll_row].text =  '      ' +  ls_text + '    '  //Modified By Jay 2016-05-23  before 6 spaces and after 4 spaces for tab text 
		end if
	next
end if
end event
