...
event pfc_cst_print_report;tab_rpt.tabpage_report.dw_report.Modify("DataWindow.Print.Prompt=No")
		...
		CASE "d_crit_specialties"
			...
			ELSE
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_specialties = Mid( is_specialties, 1, Len( is_specialties ) -2 )
			END IF
		...
		CASE "d_crit_department", "d_crit_department_with_security"
			...
			ELSE
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_department = Mid( is_department, 1, Len( is_department ) -2 )
			END IF
...
//			end if
		...
		CASE "d_crit_net_dev_type"
			...
			ELSE
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_net_dev_types = Mid( is_net_dev_types, 1, Len( is_net_dev_types ) -2 )
		...
		CASE "d_crit_appmnt_status"
			...
			END IF						
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "history" ) = 1 THEN
			...
			if is_appmnt_status = "" then
				...
				Return -1
			ELSE
				is_appmnt_status =  Mid( is_appmnt_status, 1, Len( is_appmnt_status ) -1 )
			end if
		...
		CASE "d_crit_num_days"
			ls_val = tab_rpt.tabpage_criteria.dw_criteria.GetText()
				...
				IF ls_val = "" OR IsNull( ls_val ) THEN
					MessageBox("Selection Error", "You must select an Expiration period in days. ")
					Return -1
				END IF
		...
		CASE "d_crit_contract" //maha 021803 for ultimed
			...
			ELSE
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_contracts = Mid( is_contracts, 1, Len( is_contracts ) -2 )
			END IF
		...
		case "d_crit_priv_status" //maha 091803
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_priv_stat = Mid( is_priv_stat, 1, Len( is_priv_stat ) - 2 )
			end if
			//--------------------------- APPEON BEGIN ---------------------------
		...
		case "d_crit_division" //maha 091803
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_division = Mid( is_division, 1, Len( is_division ) -2 )
			end if
			//--------------------------- APPEON BEGIN ---------------------------
			//$< Add  > 2008-05-27 By: Scofield
			//$<Reason> At least select one check box.
		...
		case "d_crit_section" //maha 091803
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_section = Mid( is_section, 1, Len( is_section ) -2 )
			end if
			//--------------------------- APPEON BEGIN ---------------------------
			//$< Add  > 2008-05-27 By: Scofield
			//$<Reason> At least select one check box.
		...
		case "d_crit_staff_cat" //maha 091803
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_staff_cat = Mid( is_staff_cat, 1, Len( is_staff_cat ) -2 )
			end if
			//--------------------------- APPEON BEGIN ---------------------------
			//$< Add  > 2008-05-27 By: Scofield
			//$<Reason> At least select one check box.
		...
		case "d_crit_gender" //maha 020204
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_sex = Mid( is_sex, 1, Len( is_sex ) -2 )
			end if
			//--------------------------- APPEON BEGIN ---------------------------
			//$< Add  > 2008-05-27 By: Scofield
			//$<Reason> At least select one check box.
		...
		case "d_crit_affil_status" //maha 021704
			IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
				...
				FOR i = 1 TO li_rc
					...
					END IF
				END FOR
				is_affil_status = Mid( is_affil_status, 1, Len( is_affil_status ) -2 )
			end if
			//--------------------------- APPEON BEGIN ---------------------------
			//$< Add  > 2008-05-27 By: Scofield
			//$<Reason> At least select one check box.
...
ELSEIF ls_wizard_dw = "d_crit_org_facility" THEN
	...
	IF gs_cust_type = "C" THEN
		tab_rpt.tabpage_criteria.st_help_text.Text = 'Select Client'
	ELSEIF gs_cust_type = "I" THEN
		tab_rpt.tabpage_criteria.st_help_text.Text = 'Select Payor/Hospital'
	ELSE
		tab_rpt.tabpage_criteria.st_help_text.Text = 'Select Facility'
	END IF
	*/
...
CHOOSE CASE tab_rpt.tabpage_report.dw_report.DataObject
	...
	CASE "d_rpt_meetings_cme", "d_rpt_meetings_department","d_rpt_meetings_committee", "d_rpt_meetings_section","d_rpt_meetings_cme_lowell"
		...
		IF is_department <> "ALL" THEN
			...
			ls_sql = ls_sql + " AND ( code_lookup.code IN ( " + is_department + "))"	
			//---------------------------- APPEON END ----------------------------
		END IF
		...
		IF Not IsNull( id_meet_from ) THEN  //maha 012904
			...
			ls_sql = ls_sql + " AND ( ( meetings.start_date )  >= Cast('" + String( id_meet_from, 'yyyy/mm/dd' ) + "' AS DATETIME ) AND " +&
				                           "( meetings.start_date )  <= Cast('" + String( id_meet_to, 'yyyy/mm/dd' ) + "' AS DATETIME ) )"		
...
//		END IF		
	...
	CASE "d_rpt_quick_verification"
		res = tab_rpt.tabpage_report.dw_report.Reset()
	...
	CASE "d_rpt_expiring_provisional_appoints", "d_rpt_expiring_provisional_by_dept" //maha 091103 inova
		...
		IF Not IsNull( id_appt_from ) THEN	
			...
			"( pd_AFFIL_STAT.init_prov_period_to )  <= Cast('" + String( id_appt_to, 'yyyy/mm/dd' ) + "' AS DATETIME ) )"		
		...
		End If
		//---------------------------- APPEON END ----------------------------
	CASE "d_rpt_expiring_appointments_by_dept", "d_rpt_expiring_appointments", "d_rpt_expiring_appointments_by_month" //maha 091103 inova
		ls_sql = is_sql
		IF Upper( is_facility_name ) <> "ALL" THEN
			ls_sql = ls_sql + " AND (pd_affil_stat.parent_facility_id = " + String( il_facility_id ) + ")"
		END IF
		...
		IF Not IsNull( id_appt_from ) THEN
			...
			ls_sql = ls_sql + " AND	 ( ( pd_AFFIL_STAT.apptmnt_end_date )  >= Cast('" + String( id_appt_from, 'yyyy/mm/dd' ) + "' AS DATETIME ) AND " +&
				                           "( pd_AFFIL_STAT.apptmnt_end_date )  <= Cast('" + String( id_appt_to, 'yyyy/mm/dd' ) + "' AS DATETIME ) )"		
			//---------------------------- APPEON END ----------------------------
		end if	
	...
	CASE "d_rpt_labels"
		...
		IF tab_rpt.tabpage_report.dw_report.RowCount() > 0 THEN
			tab_rpt.tabpage_report.dw_report.SaveAs(gs_dir_path + gs_DefDirName + "\letters\data\mailing_labels.txt", TEXT!, True )	
			tab_rpt.tabpage_report.dw_report.SaveAs(gs_dir_path + gs_DefDirName + "\letters\data\mailing_labels.xls", excel!, True )
			if li_set6 = 5 then //maha readdd to allow old method
				...
				Run(ls_word_path + " " +  gs_dir_path + gs_DefDirName + "\letters\standard\mailing_labels.doc" + " /mpreview", Minimized!)
			else
				of_labels("mailing_labels.doc",gs_dir_path + gs_DefDirName + "\letters\standard\mailing_labels.doc")
			end if
		END IF
...
//		END IF
 ...
 //*maha 06/20/00
	CASE "d_rpt_grant_priv_list_by_clin_area"
		res = tab_rpt.tabpage_report.dw_report.retrieve(  il_prac_id,il_facility_ids_all )
 //*maha 07/31/02
	...
	CASE "d_rpt_verifs_inc_on_files_closed"
		tab_rpt.tabpage_report.dw_report.retrieve(  id_from, id_to , il_facility_ids_all,ii_appt_stat[])
	//*\maha
	//*maha 10/30/00  
...
//		tab_rpt.tabpage_report.dw_report.retrieve( il_facility_ids_all )
	...
	// MSKINNER 25 OCT 2004 -- END
	/*  Cut to of_contract_run()	
	case 'r_ctx_contracts_by_assn'
		 ...
		 if upperbound(il_contact_id) > 0 then
			...
			next
			 ...
			 inv_string.of_ArrayToString(ls_array, ls_delimiter, ls_out)
						 ls_sql = ls_sql + ' AND ctx_contract_contacts.contact_id in ' + '(' + ls_out  + ')' 
       end if
		 ...
		 if upperbound(ll_ctx_status) > 0 then
	      ...
	      ls_sql = ls_sql + ' AND ctx_basic_info.status in ' + '(' + ls_out  + ')' 	
...
//Start Code Change ----09.03.2008 #V85 maha	 - recred log report added
	CASE "d_rpt_recred_log"
		...
		ls_sql = ls_sql + " AND (sys_recred_log.date_run <= Cast('" + String( id_to, 'yyyy/mm/dd' ) + "' AS DATETIME ) )"
...
//Start Code Change ----10.29.2008 #V85 maha	 - CCVS custom
	CASE "d_rpt_ccvs_cust_app_detail"
		tab_rpt.tabpage_report.dw_report.SetTransObject( SQLCA )
		res = tab_rpt.tabpage_report.dw_report.Retrieve(il_prac_ids,il_facility_id  )
	CASE ELSE 
		...
		IF Not IsNull( id_from ) THEN
			...
			ls_sql = ls_sql + " AND ( ( net_dev_action_items.action_date )  >= Cast('" + String( id_from, 'yyyy/mm/dd' ) + "' AS DATETIME ) AND " +&
				                           "( net_dev_action_items.action_date )  <= Cast('" + String( id_to, 'yyyy/mm/dd' ) + "' AS DATETIME ) )"		
...
//***************	
			...
			ls_sql = ls_sql + " AND ( ( pd_affil_stat.init_prov_period_to )  >= Cast('" + String( id_prov_from, 'yyyy/mm/dd' ) + "' AS DATETIME ) AND " +&
				                           "( pd_affil_stat.init_prov_period_to )  <= Cast('" + String( id_prov_to, 'yyyy/mm/dd' ) + "' AS DATETIME )) AND " +&
												"( pd_affil_stat.prov_status = " + string(ii_prov_status) + ")"
...
if set10 = 1 then
		openwithparm(w_sql_msg, ls_sql)
		if res = -1 then messagebox("Program error","Data window syntax error.  Call technical support. ")
end if

if res < 0 then  messagebox("Program error","Data window syntax error.  Call technical support. ")
...
ls_path = gnv_string.of_replace_env_var(ls_path) //Added By Ken.Guo 12/10/2013
...
//if gi_citrix = 1 then
//	ls_path = gs_dir_path + gs_DefDirName + "\letters\standard\" + gs_user_id + "\"
...
Choose Case tab_rpt.tabpage_report.dw_report.DataObject
	...
	Case "d_rpt_ctx_feescheduleaudit"  
		...
		"  ctx_basic_info_ctx_id = A.ctx_id, " + &
	...
	Case "d_rpt_ctx_status_statistics_1","d_rpt_ctx_status_statistics_2","d_rpt_ctx_status_summary"
		...
		if tab_rpt.tabpage_report.dw_report.DataObject = "d_rpt_ctx_status_summary" then
			...
			if Len(Trim(ls_InsSynatx)) > 0 then
				as_sql += ls_InsSynatx
			...
			if Len(Trim(ls_InsSynatx)) > 0 then
				ll_OrderByPos = Pos(Upper(as_sql),"ORDER BY")
				if ll_OrderByPos > 0 then
					as_sql = Left(as_sql,ll_OrderByPos - 1) + ls_InsSynatx + Mid(as_sql,ll_OrderByPos)
				else
					as_sql += ls_InsSynatx
				end if
				tab_rpt.tabpage_report.dw_report.Modify( 'DataWindow.Table.Select = "' + as_sql + '"' )
...
Choose Case tab_rpt.tabpage_criteria.dw_criteria.DataObject
	...
	case "d_rpt_ctx_wiz_status1"
		IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
			...
			FOR i = 1 TO li_rc
				...
				END IF
			END FOR
			is_ctx_status = Left( is_ctx_status, Len( is_ctx_status ) -2 )
		end if
		//--------------------------- APPEON BEGIN ---------------------------
		//$< Add  > 2008-05-27 By: Scofield
		//$<Reason> At least select one check box.
	...
	case "d_rpt_ctx_wiz_status2"
		IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
			...
			FOR i = 1 TO li_rc
				...
				END IF
			END FOR
			is_ctx_Track_status = Left( is_ctx_Track_status, Len( is_ctx_Track_status ) -2 )
		end if
	...
	case "d_rpt_ctx_wiz_owner1"
		IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
			...
			FOR i = 1 TO li_rc
				...
				END IF
			END FOR
			is_ctx_owner = Left( is_ctx_owner, Len( is_ctx_owner ) -2 )
		end if
		//--------------------------- APPEON BEGIN ---------------------------
		//$< Add  > 2008-05-27 By: Scofield
		//$<Reason> At least select one check box.
	...
	case "d_rpt_ctx_wiz_category1"
		IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
			...
			FOR i = 1 TO li_rc
				...
				END IF
			END FOR
			is_ctx_category = Left( is_ctx_category, Len( is_ctx_category ) -2 )
		end if
		//--------------------------- APPEON BEGIN ---------------------------
		//$< Add  > 2008-05-27 By: Scofield
		//$<Reason> At least select one check box.
	...
	case "d_rpt_ctx_wiz_service1"
		IF tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 THEN
			...
			FOR i = 1 TO li_rc
				...
				END IF
			END FOR
			is_ctx_service = Left( is_ctx_service, Len( is_ctx_service ) -2 )
		end if
		//--------------------------- APPEON BEGIN ---------------------------
		//$< Add  > 2008-05-27 By: Scofield
		//$<Reason> At least select one check box.
	...
	Case "d_rpt_ctx_wiz_type1"
		if tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 then
			...
			next
			is_ctx_type = Left( is_ctx_type, Len( is_ctx_type ) -2 )
		end if
		IF is_ctx_type = "" THEN
			MessageBox("Selection Error", "You must at least select one check box." )
			Return -1
	...
	Case "d_rpt_ctx_wiz_user"
		if tab_rpt.tabpage_criteria.dw_criteria.GetItemNumber( 1, "selected" ) = 1 then
			...
			next
			is_ctx_user = Left( is_ctx_user, Len( is_ctx_user ) -2 )
		end if
		IF is_ctx_user = "" THEN
			MessageBox("Selection Error", "You must at least select one check box." )
			Return -1
...
event open;call super::open;Integer li_except_reports,li_except_current_user
This.x = 0
This.y = 0
...
/*
select set_13 into :ii_batch_button from icred_settings; 
*/

ii_batch_button = integer(gnv_data.of_getitem("icred_settings","set_13",false))
//---------------------------- APPEON END ----------------------------

if ii_batch_button = 0 then
	tab_rpt.tabpage_criteria.cb_batch.visible = false
end if
...
//--------------------------- APPEON BEGIN ---------------------------
//$<ID> PT-09
//$<modify> 02.10.2006 By: Liang QingShi
//$<reason> Performance tuning
...
IF ii_dept_security_on = 1 THEN
	...
	//(do this to determine if user has rights to all or only some of the departments)
...
//This.Post Event Activate()			//Added by Scofield on 2009-04-28
