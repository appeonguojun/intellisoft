...
FOR i = 1 TO li_rc
	...
	IF POS( ids_column_data.GetItemString( i, "compute_name" ), "=" ) > 0 THEN //trap for computed fields
...
//		ls_sql = ls_sql + "~t" + ids_sort_data.GetItemString( i, "table_name" ) + "."
	...
	ls_sort_synt = Left( ls_sort_synt, Len( ls_sort_synt ) - 1 ) + "~n"	//get rid of extra comma
...
FOR i = 1 TO li_row_cnt
	...
	IF isnull(ls_syntax) or len(ls_syntax) = 0 then //no syntax use default
		ls_report_dw = lds_profile_view.GetItemString( i, "report_dw" )
			IF ls_report_dw = "d_prf_" THEN
				Continue
			END IF	
	...
	//	debugbreak()
		...
		elseif  method = "B" then
			pos1 = pos(ls_syntax,"@" , 1)
			pos2 = pos(ls_syntax,"&" , 1)
			li_view_id = integer(mid(ls_syntax, 1, pos1 - 1))
			li_screen_id = integer(mid(ls_syntax, pos1 + 1, pos2 - pos1 - 1 ))
			ls_screen = mid(ls_syntax,  pos2 + 1)
		...
		choose case method
			case "A","B"
				of_library_check( gs_dir_path + gs_DefDirName + "\report_temp.pbd") 
				j = LibraryImport( gs_dir_path + gs_DefDirName + "\report_temp.pbd",ls_temp_name, ImportDataWindow!, ls_syntax, ErrorBuffer)
				//	messagebox(string(j),gs_dir_path + gs_DefDirName + "\report_temp.pbd")
...
pos_addr = pos(ls_syn1," FROM pd_address ",1)
pos_where = pos(ls_syn1," WHERE ",1)
pos_from = pos(ls_syn1," FROM ",1)
...
if pos_addr > 0 then
	...
	ls_from = mid(ls_syn1,pos_from  , pos_where - pos_from + 1)  //get the existing from statement
	
	
	pos_order = pos(ls_syn1," ORDER BY ",pos_where) 
	...
	pos5 = pos(ls_syn1,'"' , pos_order)
	//ls_order = mid(ls_syn1, pos_order,16)
	ls_order = mid(ls_syn1, pos_order, pos5 -  pos_order )
	//pos5 = pos_order + 17
	s5 = mid(ls_syn1, pos5 + 1)
	//End Code Change---04.08.2008
	////End Code Change---03.27.2008
	ls_where = mid(ls_syn1,pos_where  , pos_order - pos_where + 1)
	pos2 = pos(ls_where,".prac_id",1) 
	pos3 = pos2 + 11 //'.prac_id = '
	s2 = mid(ls_where,1 , pos3)
	
	pos4 = pos(ls_where,".facility_id",pos3) 
	if pos4 > 0 then //add facility variable if appropriate
		s4 = mid(ls_where, pos3, pos4 - pos3 + 1 ) + ".facility_id = :facility "
	else
		s4 = ""
	end if
...
public function integer of_profile_report_v8 (u_dw adw_report, integer ai_facility_id, long al_prac_id, integer ai_profile_view_id, boolean ai_print_peer_review_profile, datawindow adw_profile_syn, long al_view_id);//Start Code Change ---- 11.07.2007 #V7 maha new function will be used to create dynamic profiles.
Integer i,j
Integer li_row_cnt
integer li_report_id
integer li_screen_id
...
FOR i = 1 TO li_row_cnt	
	...
	ls_report_dw = lds_profile_view.GetItemString( i, "cur_dw" )
	IF ls_report_dw = "d_prf_" or ls_report_dw = "d_prf_att_quest_master" or ls_report_dw = "d_prf_personal" THEN
		Continue
	END IF	
//	messagebox(string(i),ls_report_dw)
//	of_create_dwdata(ls_report_dw)
	//modify dw properties
	if pos(ls_report_dw,'_prf_')>0 then
