...
Boolean ib_leave = False  // added by Ken.guo 2015-09-07
end variables
...
public subroutine of_set_ib_retrieved_dddw (boolean abn_flag)
public subroutine of_set_ib_retrieved_dddw_child (userobject auo, boolean abn_flag)
end prototypes
...
if ll_Cnts > 0 then
	...
	//else
		Post Open(w_ctxwfstepstatus, gw_contract) //Modified By Ken.Guo 11/29/2013
	//end if
end if
...
public subroutine of_set_ib_retrieved_dddw (boolean abn_flag);//====================================================================
// Function: of_set_ib_retrieved_dddw
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 07/29/2015
//--------------------------------------------------------------------
...
//
//====================================================================
PowerObject 				luo_PowerObject, luo_PowerObjectChild
u_dw_contract				ldw
object 						type_obj
long							i, j, k 
String							ls_classname
...
For i = 1 to UpperBound(This.Control[])	
	luo_PowerObject = This.Control[i]
	type_obj = luo_PowerObject.TypeOf() 
	ls_classname  = luo_PowerObject.classname( )
	CHOOSE CASE type_obj
		CASE DataWindow! 
			ldw = luo_PowerObject
			if ldw.ib_retrieve_drop_downs <> abn_flag  then 
				ldw.ib_retrieve_drop_downs = abn_flag
			end if 
		Case userobject!
			of_set_ib_retrieved_dddw_child(luo_PowerObject,abn_flag)
		Case else
...
public subroutine of_set_ib_retrieved_dddw_child (userobject auo, boolean abn_flag);//====================================================================
// Function: of_set_ib_retrieved_dddw_child
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    userobject    auo
// 	value    boolean       abn_flag
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
...
For i = 1 to UpperBound(auo.Control[])	
	luo_PowerObject = auo.Control[i]
	type_obj = luo_PowerObject.TypeOf() 
	ls_classname  = luo_PowerObject.classname( )
	CHOOSE CASE type_obj
		CASE DataWindow! 
			ldw = luo_PowerObject
//			if isvalid(ldw.ib_retrieve_drop_downs) then 
				ldw.ib_retrieve_drop_downs = abn_flag
//			end if 
		Case userobject!
			of_set_ib_retrieved_dddw_child(luo_PowerObject,abn_flag)
...
//Added By Ken.Guo 2015-09-07
If tabpage_details.uo_1.ib_new Then
	If newindex = 1 and oldindex = 2 Then
		If Messagebox('Leave','The new contract is not saved. You will lose the current data when you leave. ~r~nPlease confirm if you still want to leave.', question!, yesno!) = 1 Then
			tabpage_details.uo_1.inv_contract_details.is_mode = tabpage_details.uo_1. inv_contract_details.edit
			tabpage_details.uo_1.ib_new = False
			ib_leave = True		
			Return 0
		Else
			Return 1
...
choose case newindex
	...
	case 2
		...
		this.tabpage_search.is_masterchild_filter = this.tabpage_search.ids_Search.Describe( "datawindow.Table.Filter" )
		this.tabpage_search.is_masterchild_sort = this.tabpage_search.ids_Search.Describe( "datawindow.Table.Sort" )
		//end
		...
		//------------------------------------------------------------------------------------------------------------
		// <RP> 1.18.2017 ---------------------------------------------------------------------------------------
		// Workaround for possible Appeon bug.
		// The reference this.tabpage_fee_sched2.dw_fee_nm is inValid in Appeon, but not in PB
		long ll_rowCount
		IF isValid(this.tabpage_fee_sched2.dw_fee_nm) THEN
			ll_rowCount = this.tabpage_fee_sched2.dw_fee_nm.RowCount()
		ELSE
			ll_rowCount = 0
		END IF
//		f_message_console(this.className() + ".selectionchanged() - dw_fee_nm.rowcount() = " + string (ll_rowCount))
		//
...
If newindex = 1 and oldindex = 2 Then
	...
	//Added By Ken.Guo 2015-09-07. To leave  new  ctx.
	If ib_leave Then
		ib_leave = False
		ib_select2 = False
		tabpage_details.uo_1.ib_new = False
		m_pfe_cst_mdi_menu_contract.m_file.m_save.enabled = False
		tabpage_details.uo_1.dw_1.reset( )
		tabpage_details.uo_1.dw_ccc.reset( )
		tabpage_details.uo_1.dw_contract_dates.reset( )
		tabpage_details.uo_1.dw_custom.reset( )
		tabpage_details.uo_1.dw_occ.reset( )
		tabpage_details.uo_1.dw_products.reset()
		tabpage_details.uo_1.tab_1.tabpage_1.dw_1.reset( )
		tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.reset()
		tabpage_details.uo_1.tab_1.tabpage_notes.dw_1.reset()
		tabpage_details.uo_1.tab_1.tabpage_group_access.dw_1.reset() 	
		iw_parent.FUNCTION DYNAMIC	wf_manage_tabs(True) 
		tabpage_search.dw_search_det1.event rowfocuschanged( tabpage_search.dw_search_det1.getrow())
	End If
...
elseif newindex = 1 then 
	this.tabpage_search.dw_search_det1.setfocus( )			//Added By Mark Lee 09/23/2014
end if
Return success
...
If gb_workflow Then
	...
	li_count = UpperBound(this.tabpage_multi.il_screen_id)
	if li_count > 0 then
		for li_i = 1 to li_count
			ll_screen_id[Upperbound(ll_screen_id) + 1] = this.tabpage_multi.il_screen_id[li_i]
			ls_updateCol[this.tabpage_multi.il_screen_id[li_i]] = this.tabpage_multi.is_updatecol[this.tabpage_multi.il_screen_id[li_i]]
		next
	end if		
	...
	if UpperBound(ll_screen_id) > 0 then
		...
		If lnv_workflow.of_data_value_comparision(gi_contract_view_id, ll_screen_id, ls_updateCol,tabpage_search.il_ctx_id, 0, '03', 'I')  = 1 Then
...
//				gw_contract.tab_contract_details.Post of_OpenCtxWorkFlowStep(0,gw_contract.tab_contract_details.selectedtab) //Modified By Jay Chen 08-21-2015
		...
		//Added By Mark Lee 06/04/2015   //detail
//		if this.selectedtab = 2 then 
//			of_OpenCtxWorkFlowStep(0,2)
//		end if 
		...
		//Added By Ken.Guo 2016-02-16
		this.Tabpage_multi.il_screen_id[] = ll_empty[] 
		this.Tabpage_multi.is_updatecol[] = ls_empty[]	
...
event resize;call super::resize;//Added By Ken.Guo 11/29/2013
If isvalid(w_ctxwfstepstatus) Then
	w_ctxwfstepstatus.of_set_position()
End If
end event
