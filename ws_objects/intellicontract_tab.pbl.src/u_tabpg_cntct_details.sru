...
if IsValid(w_contract_contact.tab_1.tabpage_custom_facility) then
	dw_3 = w_contract_contact.tab_1.tabpage_custom_facility.dw_2
	dw_facility_detail.ShareData(dw_3)
end if
//end
...
event pfc_update;call super::pfc_update;string ls_user_id,ls_name1,ls_name2,ls_name3,ls_email
long ll_email_lookup_code,ll_find
boolean lb_update1,lb_update2,lb_update3,lb_update4

//Added By Jay Chen 11-25-2015 sync the contact data to requester.(only for name and email)
if dw_master.getrow() < 1 then return 1
 ls_user_id = dw_master.getItemString(dw_master.getrow(),"requester_user_d")
 if isnull(ls_user_id) then ls_user_id = ''
 if ls_user_id <> '' then
	if dw_master.getItemString(dw_master.getrow(),"first_name",primary!,false) <> dw_master.getItemString(dw_master.getrow(),"first_name",primary!,true) then lb_update1 = true
	if dw_master.getItemString(dw_master.getrow(),"middle_name",primary!,false) <> dw_master.getItemString(dw_master.getrow(),"middle_name",primary!,true) then lb_update2 = true
	if dw_master.getItemString(dw_master.getrow(),"last_name",primary!,false) <> dw_master.getItemString(dw_master.getrow(),"last_name",primary!,true) then lb_update3 = true	
	if lb_update1 or lb_update2 or lb_update3 then	
	  	ls_name1 = dw_master.getItemString(dw_master.getrow(),"first_name")
		ls_name2 = dw_master.getItemString(dw_master.getrow(),"middle_name")
		ls_name3 = dw_master.getItemString(dw_master.getrow(),"last_name")
		if isnull(ls_name1) then ls_name1 = ''
		if isnull(ls_name2) then ls_name2 = ''
		if isnull(ls_name3) then ls_name3 = ''
		if lb_update1 then
			update user_requester set first_name = :ls_name1 where user_id = :ls_user_id;
		end if
		if lb_update2 then
			update user_requester set middle_name = :ls_name2 where user_id = :ls_user_id;
		end if
		if lb_update3 then
			update user_requester set last_name = :ls_name3 where user_id = :ls_user_id;
		end if
	end if
	...
	ll_find = dw_1.find("via = " + string(ll_email_lookup_code), 1,dw_1.rowcount())
	if isnull(ll_find) then ll_find = 0
	if ll_find > 0 then
		if dw_1.getItemString(ll_find,"contact_email",primary!,false) <> dw_1.getItemString(ll_find,"contact_email",primary!,true) then lb_update4 = true
		ls_email = dw_1.getItemString(ll_find,"contact_email")
	end if
	if isnull(ls_email) then ls_email = ''
	if lb_update4 then
		update user_requester set email = :ls_email where user_id = :ls_user_id;
	end if
end if
...
event ue_sync_to_salesforce();//Added by Harry 20170321 for task 749
n_cst_webapi lnv_webapi
long ll_import_id, ll_facility_id, ll_contact_id, ll_count, ll_rtn
string ls_msg,ls_user,ls_pwd,ls_token 
string ls_salesforce_id, ls_sfurl

select a.import_id into :ll_import_id from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and b.isoft_table_id = 1003;
if isnull(ll_import_id) then ll_import_id = 0
if ll_import_id <= 0 then 
	messagebox('Contractlogix','The system can not find the mapping for the Contact synchronization. Please make sure you have configured it correctly in the Data Synchronization Painter.')
	return 
end if


ll_facility_id = this.getitemnumber(this.getrow(), 'facility_id')
ll_contact_id = this.getitemnumber(this.getrow(), 'contact_id')
ls_msg =  'Are you sure you want to sync this contact (ID = '+String(ll_contact_id)+') to SalesForce?'
...
ls_pwd += ls_token
if messagebox('Sync Contact', ls_msg,Exclamation!,Yesno!,2) = 1 then
	//check the contact_id if or not already sync to salesforce
	select count(*) into :ll_count from import_outbound_records where lower(ctx_tablename) = 'ctx_contacts' and lower(ctx_fieldname) = 'contact_id' and ctx_record_id = :ll_contact_id and export_type = 1;
	if isnull(ll_count) then ll_count = 0
	if ll_count > 0 then 
		if messagebox('Sync Contact', 'The contact (ID = '+String(ll_contact_id)+') has already been synchronized to SalesForce.~r~nAre you sure you want to overwrite the contact information on SalesForce?',Exclamation!,Yesno!,2) <> 1 then
			return
		end if
	end if
	ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', 0, 0, 0, 0, ll_import_id, ls_salesforce_id, ls_sfurl, ll_facility_id, ll_contact_id)
	if len(ls_salesforce_id) <= 0 then
		messagebox('Sync Contact', 'Failed to synchronize the contact (ID = '+String(ll_contact_id)+') to SalesForce.')
	else
		messagebox('Sync Contact', 'Succeed to sync the contact to salesforce.')
	end if					
end if
end event
...
event retrieveend;call super::retrieveend;Integer	li_contact_id,li_Cycle,li_Count,li_Value
long		ll_Row
Boolean 	lb_Flag
...
lm_dw.m_table.m_synctosf.visible = True //Added by Harry 2017-03-17 for task 749
// Popup menu
...
if dwo.name = 'contact_type2' then
	if data = 'B' then
		...
		dw_master.Modify("t_defaultto.x=1696 t_defaultto.y=176 ")
		dw_master.Modify("defaultto.x=1796 defaultto.y=248 ")
		//end
	else
		...
		//Added By Jay Chen 11-23-2015
		dw_master.Modify("t_defaultto.x=2039 t_defaultto.y=20 ")
		dw_master.Modify("defaultto.x=2153 defaultto.y=96 ")
		//end
	end if
end if
//---------------------------- APPEON END ----------------------------
...
event pfc_validation;call super::pfc_validation;if this.getrow() < 1 then return success //Added By Jay Chen 05-21-2014
...
if rowsinserted > 0 then
	...
	//Added By Ken.Guo 05/05/2014. audit for new company 
	ls_company_name = this.object.facility_name[this.getrow()]
	//Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_facility_id and table_id = 10003;
	...
	values(:ll_facility_id, -9999,:ll_seq_no,10003,0,'','INSERT',:gs_user_id,getdate(),'I', 0, null,:ll_facility_id,:ls_company_name,:ls_company_name);
end if
...
String ls_first_name,ls_last_name,ls_user_d,ls_Category, ls_tmp
Long   ll_li_contact_id
//end of add
Long li_next_id //Added By Jay Chen 01-06-2014
...
if Len(ls_user_d) > 0 then
	...
	if UPPER(ls_Category) = 'A' THEN
		Select TOP 1 ISNULL(a.first_name,''),ISNULL(a.last_name,'') Into :ls_first_name,:ls_last_name 
		from ctx_contacts a inner join app_facility b on (a.facility_id = b.facility_id) 
		where b.contact_type2 = 'A' and a.user_d = :ls_user_d and a.contact_id <> :ll_li_contact_id;
		If SQLCA.sqlcode = 0 Then
			ls_tmp = this.getitemstring(ll_Row,"user_d",Primary!	,true)
			This.SetItem( ll_Row,"user_d", ls_tmp)				//Added By Mark Lee 04/30/2015 set the default value.
...
if Not f_validstr(STRING(dw_master.object.facility_id[ll_Row])) then
	...
	if li_facility_id = 0 then
		if Not f_validstr(STRING(dw_facility_detail.object.facility_id[dw_facility_detail.getrow()])) then
			SELECT Max( facility_id ) INTO :li_next_id FROM app_facility;
			IF IsNull(li_next_id) then li_next_id = 0
			li_next_id ++
			dw_facility_detail.SetItem( 1, "facility_id", li_next_id )
			a_parent_tab.function dynamic of_set_facility_id(li_next_id,'F') 
		end if
		//save company informartion first
		if dw_facility_detail.Update() = 1 then
			COMMIT;
		else
			MessageBox('Warning','DataWindow Save failed!',Exclamation!)
			ROLLBACK USING SQLCA;
...
//Added By Jay Chen 11-23-2015
if dw_facility_detail.rowcount() > 0 then
	if dw_facility_detail.getitemstring(1, "contact_type2") = 'B' then
		dw_master.Modify("t_defaultto.x=1696 t_defaultto.y=176 ")
		dw_master.Modify("defaultto.x=1796 defaultto.y=248 ")
	else
		dw_master.Modify("t_defaultto.x=2039 t_defaultto.y=20 ")
...
if ll_Row > 0 and ll_Row <= This.RowCount() then
	...
	//Added By Ken.Guo 05/05/2014
	String ls_contact_name, ls_firstname, ls_lastname
	Long ll_seq_no
	if rowsinserted > 0 then
		ls_firstname = This.GetItemString(ll_Row,'first_name')
		ls_lastname = This.GetItemString(ll_Row,'last_name')
		If isnull(ls_firstname) Then ls_firstname = ''
		If isnull(ls_lastname) Then ls_lastname = ''
		ls_contact_name = ls_firstname + ' ' + ls_lastname
		Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_ContactId and table_id = 10004;
		insert into sys_audit (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)   
		values(:ll_ContactId, -9999,:ll_seq_no,10004,0,'','INSERT',:gs_user_id,getdate(),'I', 0, null,:ll_FacilityId,:ls_contact_name,:ls_contact_name);
	end if
...
if dwo.name = 'user_d' and not IsNull(data) then
	...
	if lb_Prompt then
		MessageBox(gnv_app.iapp_object.DisplayName,'The email address for this User ID has not been configured properly. ~r~nIf its Contact Type is selected in Work Flow, the email will not be sent successfully.')
	end if
end if
...
lstr_contact.as_facility_name = 	dw_master.object.facility_name[1]															//Added By Mark Lee 01/21/2015
