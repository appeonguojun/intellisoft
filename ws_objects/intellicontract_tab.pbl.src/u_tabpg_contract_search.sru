...
type pb_1 from picturebutton within u_tabpg_contract_search
end type
type pb_2 from picturebutton within u_tabpg_contract_search
end type
type pb_3 from picturebutton within u_tabpg_contract_search
end type
type pb_4 from picturebutton within u_tabpg_contract_search
end type
type sle_page from singlelineedit within u_tabpg_contract_search
end type
type uo_page from u_display_page within u_tabpg_contract_search
end type
end forward
...
//---------<RP> 9.21.2016 --------------------------------------------
// Used for direct query of specific contract
datastore ids_find
//-------------------------------------------------------------------------
...
//Added By Jay Chen 04-10-2015
datastore ids_contract_access_contract_type
datastore ids_contract_access_division
datastore ids_contract_access_department
//end
...
//-----------<RP> 9.13.2016------------------------------
// Test...try retrieving access rights only on initial retrieve
boolean ib_accessRightsRetrieved = FALSE
//------------------------------------------------------------
...
string is_masterchild_filter,is_masterchild_sort   //Added By Jay Chen 11-04-2014
boolean ib_get_masterchild = false
str_master_child_sort istr_master_child

n_ds ids_contract_status_noaccess  //Added By Ken.Guo 02/05/2015

//Added By Jay Chen 05-12-2015
long il_current_page = 1
long il_perpage_rows,il_orig_row 
long il_sum_pages,il_last_row 
string is_sort_field = 'ctx_id' 
string is_display_page  
string is_sql_where
string is_type = 'PB'
datastore ids_search_parent_child_ctx
boolean ib_refresh_button = false
boolean ib_refresh_web_ctx = false
boolean ib_open_with_ctx = false
//end
string is_filter_company_contact //Added By Jay Chen 10-30-2015
n_ds ids_related_ctx_tmp //Added By Ken.Guo 2016-08-11
end variables
...
public function long of_getchild_sortnum (long ll_master_contract_id, long ll_ctx_id)
public subroutine of_retreivegroupacess ()
public subroutine of_build_sql_where ()
public subroutine of_page_toolbar ()
public subroutine of_get_related_child_web (long al_ctx_id)
public subroutine of_get_pageparm ()
public subroutine of_set_dw_title (string as_page, string as_focus_sign)
public function boolean of_get_hscrollbar (u_dw adw_1)
public function string of_build_table_filter (datastore ads_value, string as_table, string as_sql)
public function integer of_add_table (string as_table_name, ref string as_where_clause, ref datastore ads_list)
public function boolean of_confirmdoctextindex ()
public function integer of_search_sql (ref string as_from, ref string as_where)
public function integer of_search_add_table (string as_table_name, ref datastore ads_table, ref string as_where)
public function string of_search_escape_sql (string as_term)
public function integer of_goto_contract (long al_ctx_id)
public function long of_retrievewithparms ()
public function boolean of_check_multi_field (string as_colname)
public function string of_multi_select_sql (string as_colname, string as_value)
end prototypes
...
if IsValid(gds_contract_screen) then
	//if IsNull(gi_contract_view_id) or gi_contract_view_id = 0 then gi_contract_view_id = 1001 //Commented By Ken.Guo 2015-09-16. fixed preload CL contract bug.
...
of_page_toolbar() //Added By Jay Chen 05-13-2015
...
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then //Added By Jay Chen 05-19-2015
//	dw_search_det1.title = 'Contract Browse ' + "(" + string(il_last_row) + ")"
	of_set_dw_title('1','0')
else
...
//		tv_ctx.getitem(ll_handle,ltvi_1)
		...
		il_tv_ctx_current_handle = ll_handle
		 ...
		 //Added By Jay Chen 05-15-2015
		if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
//		if is_display_page = '1' and is_Type = 'WEB' then 
			if inv_ctx_tv.is_tv_type = 'all' then
...
String 		ls_ColName,ls_ColType,ls_Value, ls_col_display
...
IF ll_rowcnt > 0 Then 
	...
	For i = 1 to ll_rowcnt
		...
		ls_col_display = lds_search_filter.GetItemString(i,'col_display')
...
//				dw_search.setitem( ll_row, ls_ColName, datetime(ls_Value))
			CASE 'dat'		//Added By Mark Lee 11/10/2015
				dw_search.setitem( ll_row, ls_ColName, datetime(ls_Value))
			CASE 'var','cha'				
...
event type long ue_search_2();Long ll_ret
...
// <RP> 7.25.2016 - Call new method of_retrieveWithParms()
ll_ret = of_retrieveWithParms()

Return ll_ret

// <RP> 7.25.2016
//// New wrapper to call method of_retrieveWithParms() while preserving backward-compatability with existing
//// code
////
////	Includes wrapper code copied from:
////	cb_1::clicked()
////	of_retrieve()
////	of_retrieve_ctx()
////	of_filter()
////
//
//// New method to integrate new DW objects + stored procedure
//boolean lb_prevAutoRetrieve
//
///* <RP> - Set up ids_search */
///* <RP>: consider gb_IsDocMode and "d_user_ctx_search" ?? */
//IF NOT isValid(ids_search) THEN
//	ids_search = CREATE datastore
//END IF
//IF ids_search.dataobject <> dw_search_det1.dataobject THEN
//	ids_search.dataobject = dw_search_det1.dataobject
//END IF
//ids_search.setTransObject(SQLCA)
//
//lb_prevAutoRetrieve = ib_autoretrieve
//dw_search_det1.ib_retrieve_drop_downs = false
...
//long ll_ret 
//SetPointer(Hourglass!)
//if not ib_retriev_detail then return
//if not ib_autoretrieve then return
...
//if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then //Added By Jay Chen 05-13-2015
//if is_type = 'WEB' then
...
//	else
//		ids_search.DataObject = 'd_sp_contract_search_web'
//		ids_search.SetTransObject( sqlca)
//		select count(*) into :ll_cnt from ctx_basic_info_log where session_id = :gs_session_id;
//		if isnull(ll_cnt) then ll_cnt = 0
//		if ll_cnt = 0 then
//			insert into ctx_basic_info_log (user_id,session_id,sqlwhere) values(:gs_user_id,:gs_session_id,:is_sql_where);
//		else
//			update ctx_basic_info_log set sqlwhere = :is_sql_where where session_id = :gs_session_id;
//		end if
//		if is_display_page = '1' then
//			il_current_page = 1 
//		else
//			il_current_page = -1
//		end if
//		ll_ret = ids_search.Retrieve(gs_user_id,'1',il_current_page,il_perpage_rows,gs_session_id,0)
//		select pcount,rcount into :il_sum_pages,:il_last_row from ctx_basic_info_log where session_id = :gs_session_id;
//	end if
...
//			ids_search.DataObject = "d_user_ctx_search"
//			ids_search.SetTransObject(SQLCA)
//			gnv_debug.of_output( False,'ids_search.Retrieve: start.')
//			ll_ret = ids_search.Retrieve(Upper(gs_user_id),gl_MoveToCtxID)
//			gnv_debug.of_output( False,'ids_search.Retrieve: end.')
//		else
...
////	//Added By Jay Chen 05-15-2015
////	if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
////		datastore lds_user_access
////		lds_user_access = create datastore
////		lds_user_access.dataobject = 'dw_user_access_ctx_list'
////		lds_user_access.settransobject(sqlca)
////		lds_user_access.retrieve(gs_user_id)
////		il_contract_array = lds_user_access.object.ctx_id.current
////		destroy lds_user_access
////	else  
...
//		gnv_debug.of_output( False,'dw_search_det1.event rowfocuschanged(1): start.')
//		dw_search_det1.event rowfocuschanged(1)
//		gnv_debug.of_output( False,'dw_search_det1.event rowfocuschanged(1): end.')
//	Else
//		dw_search_det1.scrolltorow(ll_find) //it will trigger rowfocuschanged event
//	End If
// Else
//	//Added By Jay Chen 05-14-2015 Locate the ctx to the specified page and specified line
//	if (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) and ((ib_refresh_button and len(is_sql_where) < 1) or ib_open_with_ctx  ) then 
//		select count(1) into :ll_cnt from ctx_basic_info where ctx_id = :il_ctx_id;
//		if isnull(ll_cnt) then ll_cnt = 0
//		if ib_open_with_ctx  then
//			if ll_cnt > 0 then gf_goto_contract_page(il_ctx_id,'designscreen')
//		else
//			if ll_cnt > 0 then gf_goto_contract_page(il_ctx_id,'other')
//		end if
//		ll_find = dw_search_det1.find("ctx_id = " + string(il_ctx_id),1,dw_search_det1.rowcount())
//		if ll_find <= 0 then dw_search_det1.event rowfocuschanged(1)
//		ib_refresh_button = false
//		ib_open_with_ctx = false
//		//end add
//	else
//		dw_search_det1.event rowfocuschanged(1)
//	end if
...
//
//
//ib_autoretrieve = lb_prevAutoRetrieve
//
...
gnv_debug.of_output( False,'iw_parent.function dynamic wf_reset_retrieved(): start.')
If iw_parent.classname() = 'w_contract' Then //Added By Ken.Guo 2016-08-09. added if statement
	iw_parent.function dynamic wf_reset_retrieved()
End If
gnv_debug.of_output( False,'iw_parent.function dynamic wf_reset_retrieved(): end.')
inv_contract_details.is_mode = inv_contract_details.edit
/*
...
if not gb_use_opmode  then
	gnv_app.ib_ref_cache = true
	gnv_debug.of_output( False,'gnv_app.of_setcontraccache( ): start.')
	gnv_app.of_setcontraccache( )
	gnv_app.of_setcontraccache_code_lookup( FALSE)
	gnv_debug.of_output( False,'gnv_app.of_setcontraccache( ): end.')
end if
...
//modified by gavins 20160805
ll_ret = of_retrieve_ctx()
// <RP> 9.16.2016 - check return code
IF ll_ret = -2 THEN
	/* User has chosen to cancel following warning message about large result set. */
	return -2
END IF
...
//
/*
if (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then  //Added By Jay Chen 05-12-2015 prepare retrieve condition
//if is_type = 'WEB' then
	gnv_debug.of_output( False,'of_build_sql_where(): start.')
	of_build_sql_where()
	gnv_debug.of_output( False,'of_build_sql_where(): end.')
end if
...
gnv_debug.of_output( False,'of_retrieve_ctx(): start.')
ll_ret = of_retrieve_ctx() //Added By Ken.Guo 2009-06-03. use function to retrieve
gnv_debug.of_output( False,'of_retrieve_ctx(): end.')
...
*/ //modifie by gavins 20160805  comment;
...
If iw_parent.classname() = 'w_contract' Then //Added By Ken.Guo 2009-06-17. Add if condition.
	...
	IF Isvalid(lw_tmp) THEN
		IF lw_tmp.ib_win_just_open THEN
			...
			IF isvalid(lw_tmp.inv_contract_codes) Then
				gnv_debug.of_output( False,'lw_tmp.inv_contract_codes.of_del_ctx_directory( ): start.')
				lw_tmp.inv_contract_codes.of_del_ctx_directory( )
				gnv_debug.of_output( False,'lw_tmp.inv_contract_codes.of_del_ctx_directory( ): end.')
			End If
...
//------------ <RP> 9.13.2016 -----------------------------------------------------------------------
// Try retrieving the access lists only once
IF NOT ib_accessRightsRetrieved THEN
...
//	ids_ctx_group_user_view.Retrieve(gs_user_id) //Added by Ken.Guo on 2009-02-12
	...
	//Added By Jay Chen 04-10-2015
	ids_contract_access_contract_type.Retrieve( gs_user_id )
	ids_contract_access_division.Retrieve( gs_user_id )
	ids_contract_access_department.Retrieve( gs_user_id )
	//end
	//---------End Modified -----------------------------
	gnv_appeondb.of_commitqueue( )
	
	ib_accessRightsRetrieved = TRUE
END IF
//------------ END <RP> 9.13.2016 -----------------------------------------------------------------------

//Added By Ken.Guo 2017-06-21. Task 796:Copied contracts display a different view than the original contract
...
//Move to of_filter 
gnv_debug.of_output( False,'of_retrieve().of_filter(): start.')
of_filter()
gnv_debug.of_output( False,'of_retrieve().of_filter(): end.')
...
//Added By Ken.Guo 2016-09-20. hide the drop down arrow for status column.
String ls_modify
If dw_search_det1.describe('custom_status1.dddw.name') = 'd_dddw_code_lookup' Then
                ls_modify = 'custom_status1.dddw.useasborder=no '
End If
If dw_search_det1.describe('custom_status2.dddw.name') = 'd_dddw_code_lookup' Then
                ls_modify += 'custom_status2.dddw.useasborder=no'
End If
If ls_modify <> '' Then
                dw_search_det1.Modify(ls_modify)
End If
...
//n_ds lds_cache
//IF dw_search_det1.getChild("app_facility", ldwc_dddw) = 1 THEN
//	IF gnv_app.inv_dwcache.of_getRegistered("dddw_contact_company", lds_cache) = 1 THEN
////		IF lds_cache.rowCount() <> ldwc_dddw.rowCount() THEN
//			lds_cache.shareData(ldwc_dddw)
////		END IF
//	END IF
//END IF
//// end <RP> 9.9.2016 -------------------------------------------------------------------------------------------------------------
...
n_ds					lds_Value, lds_value_facility
...
lds_value_facility  = create  n_ds																		//Added By Mark Lee 11/27/2015
lds_value_facility.dataobject = 'd_contract_search_criteria_ext'
			...
			choose case lower(ls_ColName)
				...
				case 'product_type'
					...
					if f_validstr(ls_Value) THEN 
						...
						ls_Value = inv_string.of_globalreplace( ls_Value, "'","~~'")
						//End change code 08.24.2007
						if len(is_filter_string) > 1 then //Modified By Jay Chen 11-12-2014
							 is_filter_string = is_filter_string +" AND (Upper(product_type) like '%" + Upper(ls_Value) + "%')"
						else 
							 is_filter_string = " (Upper(product_type) like '%" + Upper(ls_Value) + "%')"
						end if 
//						if len(is_filter_string) > 1 then 
...
//						end if 
				...
				case  'feesched_custom_n1',  'feesched_custom_n2',  'feesched_custom_n3',  'feesched_custom_n4',  'feesched_custom_n5'
					...
					lds_value.SetItem( lds_value.RowCount( ), 'types', ls_ColType )
					lds_value.SetItem( lds_value.RowCount( ), 'value', ls_value )	
				case 'app_facility_country','app_facility_corp_office_flag','app_facility_corp_office_link'					//Added By Mark Lee 11/27/2015
					lds_value_facility.insertrow( 0 )
					lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'field', ls_ColName )
					lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'types', ls_ColType )
					lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'value', ls_value )	
				case else
		...
		CASE 'dat'
			choose case lower(ls_ColName)
				...
				case 'term_date','last_revis_date','review_date','version_date','custom_date1','custom_date2' //Modified By Jay Chen 04-13-2015 add custom_date
					IF f_validstr(ls_Value) THEN 
						IF len(is_filter_string) > 1 THEN 
				...
				case 'term_date_2','last_revis_date_2','review_date_2','version_date_2','custom_date1_2','custom_date2_2' 
...
//					ls_contact = ls_Value					//Commented By Mark Lee 11/27/2015
				case 'ccci_name'
					ls_ccci_name = ls_value
					if pos(ls_ccci_name,"'") > 0 then ls_ccci_name = gnv_string.of_globalreplace( ls_ccci_name, "'", "''" ) //Added By Jay Chen 02-09-2015
				case 'ccci_association' //Commented By Jay 2016-07-04
					ls_ccci_association = ls_value //Commented By Jay 2016-07-04
//				case 'ccci_association', 'ccci_association2','ccci_association3','ccci_association4','ccci_association5',&
//					'ccci_association6','ccci_association7','ccci_association8','ccci_association9','ccci_association10' //Modified By Jay 2016-07-04
//					If f_validstr ( ls_Value ) Then
//						ls_ccci_association = ls_ccci_association + " AND " + mid ( Lower ( ls_ColName ), 6 ) + "= '" + ls_Value + "' "  //Added By Jay 2016-07-04
//					End If
				...
				case 'occi_name'
					...
					if pos(ls_occi_name,"'") > 0 then ls_occi_name = gnv_string.of_globalreplace( ls_occi_name, "'", "''" ) //Added By Jay Chen 02-09-2015
				...
				case 'feesched_custom_text4', 'feesched_custom_text5', 'feesched_custom_text6', 'feesched_custom_text7','feesched_custom_text8', 'feesched_notes'
					...
					lds_value.SetItem( lds_value.RowCount( ), 'types', ls_ColType )
					lds_value.SetItem( lds_value.RowCount( ), 'value', ls_value )	
				case 'app_facility_custom_1','app_facility_custom_2','app_facility_custom_3','app_facility_custom_4','app_facility_custom_5','app_facility_custom_6', &
						'app_facility_custom_7','app_facility_custom_8','app_facility_custom_9','app_facility_custom_10','app_facility_status','app_facility_street', &
						'app_facility_city','app_facility_state','app_facility_contact_type2','app_facility_contact','app_facility_street_2','app_facility_street_3',&
						'app_facility_zip','app_facility_phone','app_facility_fax','app_facility_web_address'						
					lds_value_facility.insertrow( 0 )												//Added By Mark Lee 11/27/2015
					lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'field', ls_ColName )
					lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'types', ls_ColType )
					lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'value', ls_value )		
					if dw_search.describe("#" + string(ll_Num) + ".edit.style") = 'dddw' then
						lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'style', 'Y' )								
					else
						lds_value_facility.SetItem( lds_value_facility.RowCount( ), 'style', '' )		
					end if 
				case else
					if Not f_validstr(ls_Value) THEN Continue
					ls_Value = inv_string.of_globalreplace( ls_Value, '"','~~"')
...
ls_SQL = of_build_table_filter(lds_value_facility,'app_facility',ls_SQL)														//Added By Mark Lee 11/27/2015  tableD ... 
...
If IsValid( lds_value_facility ) Then destroy( lds_value_facility )
...
/*
if not (is_display_page = '1'  and (AppeonGetClientType() = 'WEB' or is_type = 'WEB'))  then //Added By Jay Chen 05-13-2015
	gnv_debug.of_output( False,'of_build_filter( ): start.')
	of_build_filter( )
	gnv_debug.of_output( False,'of_build_filter( ): end.')
end if
*/ //modified by gavins 20160805 comment;
...
ib_Trigger_RowChangeEvent = False
If ib_tree_view Then
	gnv_debug.of_output( False,'of_copy_row_tv( ): start.')
	of_copy_row_tv()
	gnv_debug.of_output( False,'of_copy_row_tv( ): end.')
Else
	gnv_debug.of_output( False,'of_copy_row( ): start.')
	of_copy_row()
	gnv_debug.of_output( False,'of_copy_row( ): end.')
End If
ib_Trigger_RowChangeEvent = True
...
Else
	If iw_parent.classname() = 'w_contract' Then
		gnv_debug.of_output( False,'iw_parent.Dynamic Function wf_set_tab_enabled(True): start.')
		iw_parent.Dynamic Function wf_set_tab_enabled(True)
		gnv_debug.of_output( False,'iw_parent.Dynamic Function wf_set_tab_enabled(True): end.')
	End If
end if
...
If ll_find > 0 Then
 ...
 Else
	...
	if (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) and ((ib_refresh_button and len(is_sql_where) < 1) or ib_open_with_ctx  ) then 
		...
		If dw_search_det1.Rowcount ( ) > 0 Then  dw_search_det1.event rowfocuschanged(1) //modified by gavins 20160805
	end if
End If
//---------------------------- APPEON END ----------------------------
...
//	If ib_force_retrieve or (Not ib_retrieved_ctx) Then  //modified by gavins 20160805
		if gb_IsDocMode then
			ids_search.DataObject = "d_user_ctx_search"
			ids_search.SetTransObject(SQLCA)
			gnv_debug.of_output( False,'ids_search.Retrieve: start.')
...
//Added By Jay Chen 05-12-2015
if is_display_page = '1' then
	sle_page.text = string(il_current_page) + '/' + string(il_sum_pages)
end if
//end
...
If isvalid(iu_PowerFilter) Then
	gnv_debug.of_output( False,'iu_PowerFilter(): start.')
	iu_PowerFilter.of_reset( ) 
	iu_PowerFilter.of_Reset_OriginalFilter( )//added by gavins 20130424
	gnv_debug.of_output( False,'iu_PowerFilter(): end.')
End If

//Added By Ken.Guo 12/10/2011. 
string ls_display_contract_parent_child_tree
...
if ls_display_contract_parent_child_tree = '1' then //Added By Jay Chen 06-15-2015
	If inv_ctx_tv.is_tv_type = 'row' Then 
		If dw_search_det1.GetRow() > 0 Then
			ll_ctx_id = dw_search_det1.GetItemNumber(dw_search_det1.GetRow(), 'ctx_id')
			gnv_debug.of_output( False,'of_get_related_ctx(): start.')
			This.of_get_related_ctx( ll_ctx_id, 'row')	
			gnv_debug.of_output( False,'of_get_related_ctx(): end.')
		End If
	Else
		gnv_debug.of_output( False,'of_refresh_tv(ids_search): start.')
		of_refresh_tv(ids_search)
		gnv_debug.of_output( False,'of_refresh_tv(ids_search): end.')
	End If
end if
...
Long ll_ctx_id 
string ls_ctx_id
datastore lds_child_ctx

//Added By Jay Chen 05-15-2015
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
//if is_display_page = '1' and is_Type = 'WEB' then  //new method 
	of_get_related_child_web(al_ctx_id)
	if ids_search.Rowcount() > il_orig_row then ids_search.rowsdiscard(il_orig_row + 1,ids_search.Rowcount(), Primary!) 
	destroy ids_search_parent_child_ctx
...
gnv_debug.of_output( False,'of_refresh_tv():of_tv_get_max_len(): start.')
of_tv_get_max_len(ids_search)
gnv_debug.of_output( False,'of_refresh_tv():of_tv_get_max_len(): end.')
...
gnv_debug.of_output( False,'of_refresh_tv():of_gen_treeview(): start.')
of_gen_treeview()
gnv_debug.of_output( False,'of_refresh_tv():of_gen_treeview(): end.')
//gnv_debug.of_output('of_gen_treeview e')
gnv_debug.of_output( False,'of_refresh_tv():of_gen_treeview_other(): start.')
of_gen_treeview_other()
gnv_debug.of_output( False,'of_refresh_tv():of_gen_treeview_other(): end.')
//gnv_debug.of_output('of_add_other_ctx e')

If inv_ctx_tv.ii_displaycount = 1 Then
	gnv_debug.of_output( False,'of_refresh_tv():of_tv_set_group_sum(): start.')
	of_tv_set_group_sum()
	gnv_debug.of_output( False,'of_refresh_tv():of_tv_set_group_sum(): end.')
End If
...
if  Isvalid(ids_ctx_tv_map) then 			//Added By Mark Lee 05/11/2015
	If ids_ctx_tv_map.Find('ctx_id = ' + String(al_ctx_id),1,ids_ctx_tv_map.rowcount() ) > 0 Then
		gnv_debug.of_output( False,'This.event ue_locate_treeview(al_ctx_id): start.')
		This.event ue_locate_treeview(al_ctx_id)	
		gnv_debug.of_output( False,'This.event ue_locate_treeview(al_ctx_id): end.')
		Return
	End If
end if 
...
gnv_debug.of_output( False,'of_get_related_parent_ctx(): start.')
ll_parent_ctx_id = This.of_get_related_parent_ctx( al_ctx_id )
gnv_debug.of_output( False,'of_get_related_parent_ctx(): end.')
//Modified By Ken.Guo 12/10/2011
gnv_debug.of_output( False,'of_get_related_child_ctx(): start.')
This.of_get_related_child_ctx( ll_parent_ctx_id )
gnv_debug.of_output( False,'of_get_related_child_ctx(): end.')
//This.of_get_related_child_ctx( al_ctx_id )

//Added By Ken.Guo 2016-08-10. get the all related ctx.
Long i, ll_cnt
If as_type = 'row' and inv_ctx_tv.ii_show_data = 1 Then
	If Not isvalid(ids_related_ctx_tmp) Then
		ids_related_ctx_tmp = Create n_ds
		ids_related_ctx_tmp.dataobject = 'd_sp_contract_search_related'
		ids_related_ctx_tmp.settransobject(sqlca)		
	End If
	ids_related_ctx_tmp.retrieve(al_ctx_id,gs_user_id)
	ll_cnt = ids_related_ctx_tmp.rowcount()
	ids_search_related_ctx.reset()
	ids_related_ctx_tmp.rowscopy(1,ll_cnt, Primary!,ids_search_related_ctx,1,primary! )
End If
...
gnv_debug.of_output( False,'of_get_related_ctx():of_refresh_tv(): start.')
This.of_refresh_tv(ids_search_related_ctx)
gnv_debug.of_output( False,'of_get_related_ctx():of_refresh_tv(): end.')

//This.Post event ue_locate_treeview(al_ctx_id)
gnv_debug.of_output( False,'of_get_related_ctx():ue_locate_treeview(): start.')
event ue_locate_treeview(al_ctx_id)					//Added By Mark Lee 12/28/2012
gnv_debug.of_output( False,'of_get_related_ctx():ue_locate_treeview(): end.')
...
long ll_orig_row,ll_cnt
string ls_ctx_id

ll_find_row = ids_search.Find('ctx_id = ' + String(al_ctx_id),1,ids_search.Rowcount())
If ll_find_row <= 0 Then Return 0

//Modified By Jay Chen 05-15-2015
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
//if is_display_page = '1' and is_Type = 'WEB' then
	ls_ctx_id = string(al_ctx_id)
	il_orig_row = ids_search.Rowcount() 
	select count(*) into :ll_cnt from ctx_basic_info_log where session_id = :gs_session_id;
	if isnull(ll_cnt) then ll_cnt = 0
	if ll_cnt = 0 then
		insert into ctx_basic_info_log (user_id,session_id,treewhere) values(:gs_user_id,:gs_session_id,:ls_ctx_id);
	else
		update ctx_basic_info_log set treewhere = :ls_ctx_id where session_id = :gs_session_id;
	end if
	ids_search_parent_child_ctx = create datastore
	ids_search_parent_child_ctx.dataobject = 'd_sp_contract_search_web'
	ids_search_parent_child_ctx.settransobject(sqlca)
	ids_search_parent_child_ctx.retrieve(gs_user_id,'0',-2,il_perpage_rows,gs_session_id,0)
	ids_search_parent_child_ctx.Rowscopy(1, ids_search_parent_child_ctx.rowcount(), Primary!, ids_search, ids_search.rowcount() + 1, Primary!)
end if
//end
...
//Commented By Mark Lee 11/27/2015
//ls_where = ""		
...
If ll_Find <= 0 Then 
	...
	ll_Find = ids_related_ctx_tmp.Find('ctx_id = ' + String(al_ctx_id), 1, ids_related_ctx_tmp.RowCount()) 
	If ids_related_ctx_tmp.RowsCopy( ll_Find,ll_Find,Primary!,dw_search_det1,dw_search_det1.RowCount() + 1,Primary!) = 1 Then
		of_set_dw_title('0','0')
...
String 	ls_powerfilter,	ls_pos, ls_to_do_list, ls_Preview
...
If dw_search_det1.visible Then
	ls_powerfilter =  gnv_user_option.of_Get_option_value( gs_user_id, "Contract_Search_Tab_PowerFilter" ) 
	ls_Preview  =  dw_search_det1.describe( "datawindow.print.preview" )
	If isvalid(iu_PowerFilter) and  Len( ls_Preview ) > 0 and Not( ls_Preview = '?' or ls_Preview = '!' ) Then //Added by gavin 20160301
...
If not IsValid(ids_templates_list) Then 
	...
	ids_templates_list.retrieve( gs_user_id ) //Modified By Jay 2016-07-18
...
public function long of_getchild_sortnum (long ll_master_contract_id, long ll_ctx_id);//added by gavins 20141017  get child contract sort number
Long ll_RowNum

string		ls_Filter, ls_Sort,ls_org_filter


//Modified By Jay Chen 11-04-2014
//ls_Filter = ids_Search.Describe( "datawindow.Table.Filter" )
//ls_Sort = ids_Search.Describe( "datawindow.Table.Sort" )
ls_Filter = is_masterchild_filter
ls_Sort = is_masterchild_sort
//if ls_Filter = "?" then ls_Filter = ""
//if is_filter_string <> ls_Filter then
//	ls_org_filter = is_filter_string
//end if
...
ids_Search.SetFilter( "master_contract_id = " + string(ll_master_contract_id ) )
ids_Search.Filter( )

ids_Search.SetSort( "ctx_id" )
ids_Search.Sort( )

//Added By Jay Chen 10-17-2014 avoid dupicate ctx rownum
ids_Search.setfilter("(ctx_id <> ctx_id[-1] or getrow() = 1) and master_contract_id = " + string(ll_master_contract_id ))
ids_Search.Filter( )
//end
ll_RowNum = ids_Search.Find( "ctx_id=" +string( ll_ctx_id ),1, ids_Search.RowCount( ) ) 

If ll_RowNum<0 Then 
	MessageBox( 'Tips', 'Find Expresss is invalid.' )
	ll_RowNum = 0
End If
...
ids_Search.SetSort( ls_Sort )
ids_Search.Sort( )

//Added By Jay Chen 11-04-2014
ib_get_masterchild = false
istr_master_child.al_child_sort_index = ll_RowNum
istr_master_child.al_ctx_id = ll_ctx_id
//end
...
public subroutine of_retreivegroupacess ();//added by gavins 20150306    because  click search button ,  no retrieve them .    when contract windows open  , retrieve them.

If not ib_autoretrieve Then
...
public subroutine of_build_sql_where ();//<Modify> 11/26/2007 by: Andy
Long   ll_Num,ll_Cnt
String ls_ColType,ls_ColName,ls_Value
String	ls_MaxTime = "23:59:59"
String ls_document_name,ls_document_content
	...
	Choose Case Lower(Left ( ls_ColType , 3 ))
		CASE 'int','lon','dec','num','dou','rea','ulo', 'mon'
			choose case lower(ls_ColName)
				case 'group_multi_loc_id'
					if f_validstr(ls_Value) then
						if len(is_sql_where) > 1 then 
							 is_sql_where = is_sql_where +' AND contracted_entity = ' + ls_Value
						else 
							 is_sql_where = ' contracted_entity = ' + ls_Value
						end if
					end if
				case 'app_facility'
					ls_Value = trim(dw_search.getitemstring(dw_search.getrow(),'facility_name'))
					...
					if f_validstr(ls_Value) THEN 
						...
						if len(is_sql_where) > 1 then 
							//Modify by Jack 05/11/2007
							is_sql_where = is_sql_where + " AND (facility_name like '%" + ls_Value + "%')" //08.24.2007 Only use single quotes
						else 
							is_sql_where = " (facility_name like '%" + ls_Value + "%')"
						end if 
					end if
				case 'category'
					if f_validstr(ls_Value) THEN 
						if len(is_sql_where) > 1 then 
							 is_sql_where = is_sql_where +' AND category = ' + ls_Value
						else 
							 is_sql_where = ' category = ' + ls_Value
						end if 
					end if
				...
				case 'product_type'
					...
					if f_validstr(ls_Value) THEN 
						...
						ls_Value = inv_string.of_globalreplace( ls_Value, "'","~~'")
						//End change code 08.24.2007
						if len(is_sql_where) > 1 then //Modified By Jay Chen 11-12-2014
							 is_sql_where = is_sql_where +" AND (product_type like '%" + ls_Value + "%')"
						else 
							 is_sql_where = " (product_type like '%" + ls_Value + "%')"
						end if 
//						if len(is_sql_where) > 1 then 
//							 is_sql_where = is_sql_where +' AND product_type = ' + "'" + ls_Value + "'"
//						else 
//							 is_sql_where = ' product_type = ' + "'" + ls_Value + "'"
//						end if 
					end if
				...
				case 'ctx_id'
					if f_validstr(ls_Value) THEN 
							if len(is_sql_where) > 1 then 
								 is_sql_where = is_sql_where +' AND ctx_id = ' + ls_Value
							else 
								 is_sql_where = ' ctx_id = ' + ls_Value
							end if 
					end if
				...
				case 'group_id'
					if f_validstr(ls_Value) THEN 
						if len(is_sql_where) > 1 then 
							 is_sql_where = is_sql_where +' AND group_id = ' + ls_Value
						else 
							 is_sql_where = ' group_id = ' + ls_Value
							end if 
					end if 
				...
				case 'status'
					if f_validstr(ls_Value) THEN 
						if len(is_sql_where) > 1 then 
							 is_sql_where = is_sql_where +' AND status = ' + ls_Value
						else 
							 is_sql_where = ' status = ' + ls_Value
						end if 
					end if
				//Added By Ken.Guo 2010-06-08.					
				case 'document_id'	
				...
				case 'unread_email'
					if f_validstr(ls_Value) THEN 
						if len(is_sql_where) > 1 then 
							If ls_Value = '1' Then
								is_sql_where = is_sql_where +' AND unread_email_cnt > 0 '
							ElseIf ls_value = '0' Then
								is_sql_where = is_sql_where +' AND unread_email_cnt = 0 '
							End If
						else 
							If ls_Value = '1' Then
								is_sql_where = ' unread_email_cnt > 0 '
							ElseIf ls_value = '0' Then
								is_sql_where = ' unread_email_cnt = 0 '
							End If
						end if 
				...
				case else
					if Not f_validstr(ls_Value) THEN Continue
					if len(is_sql_where) > 1 then 
						 is_sql_where = is_sql_where +' AND ' + ls_ColName + ' = ' + ls_Value
					else 
						 is_sql_where = ls_ColName + ' = ' + ls_Value
					end if
		...
		CASE 'dat'
			choose case lower(ls_ColName)
				case 'effective_date'
					IF f_validstr(ls_Value) THEN
						IF len(is_sql_where) > 1 THEN
							 is_sql_where = is_sql_where +' AND effective_date >= ' + "convert(varchar(20),'" + ls_Value + "',111)" 
						ELSE			
							 is_sql_where = ' effective_date >= ' + "convert(varchar(20),'" + ls_Value + "',111)"
						END IF
					END IF
				...
				case 'expriation_date'	
					IF f_validstr(ls_Value) THEN 
						IF len(is_sql_where) > 1 THEN 
							 is_sql_where = is_sql_where +' AND expriation_date >= ' + "convert(varchar(20),'" + ls_Value + "',111)" 
						ELSE 
							 is_sql_where = ' expriation_date >= ' + "convert(varchar(20),'" + ls_Value + "',111)"
						END IF
					END IF 
				...
				case 'term_date','last_revis_date','review_date','version_date','custom_date1','custom_date2' //Modified By Jay Chen 04-13-2015 add custom_date
					IF f_validstr(ls_Value) THEN 
						IF len(is_sql_where) > 1 THEN 
							 is_sql_where = is_sql_where +' AND ' + ls_ColName + ' >=' + "convert(varchar(20),'" + ls_Value + "',111)" 
						ELSE 
							 is_sql_where = ls_ColName + ' >= ' + "convert(varchar(20),'" + ls_Value + "',111)"
						END IF
					END IF 
			...
			choose case lower(ls_ColName)
				...
				case 'product_name'
					...
					if f_validstr(ls_Value) THEN 
						...
						//Replace '=' to  'Like' - Modify by jervis 01.22.2010
						if len(is_sql_where) > 1 then 
							 is_sql_where = is_sql_where +" AND (product_name like '%" + ls_Value + "%')"
						else 
							 is_sql_where = " (product_name like '%" + ls_Value + "%')"
						end if 
					end if
				...
				case 'owner'
					...
					IF f_validstr(ls_Value) THEN
						...
						IF len(is_sql_where) > 1 THEN
							 is_sql_where = is_sql_where + " AND rtrim(ltrim(owner)) = '" + ls_Value + "'" 
						ELSE
							 is_sql_where = " rtrim(ltrim(owner)) = '" + ls_Value + "'" 
						END IF
					END IF
...
//				case 'app_facility_contact_id'						//Commented By Mark Lee 11/27/2015
//					ls_contact = ls_Value	
				case 'ccci_name'
					ls_ccci_name = ls_value
					if pos(ls_ccci_name,"'") > 0 then ls_ccci_name = gnv_string.of_globalreplace( ls_ccci_name, "'", "''" ) //Added By Jay Chen 02-09-2015
...
//					End If
				...
				case else
					...
					if dw_search.describe("#" + string(ll_Num) + ".edit.style") = 'dddw' then
						...
						if ls_dddwname  = 'd_dddw_lookup_code' or ls_dddwname = 'd_dddw_company' or ls_dddwname = 'd_dddw_company_list' then //Add by jervis -- 05.13.2011
							if len(is_sql_where) > 1 then 
								 is_sql_where = is_sql_where +" AND (Upper(" + ls_ColName + ") = '" + Upper(ls_Value) + "')"
							else 
								 is_sql_where = " (Upper(" + ls_ColName +  ") = '" + Upper(ls_Value) + "')"
							end if
						else
							if len(is_sql_where) > 1 then 
								 is_sql_where = is_sql_where +" AND (Upper(" + ls_ColName + ") Like '%" + Upper(ls_Value) + "%')"
							else 
								 is_sql_where = " (Upper(" + ls_ColName +  ") Like '%" + Upper(ls_Value) + "%')"
							end if
						end if
					else
							if len(is_sql_where) > 1 then 
...
If ls_doc_filter <> '' Then
	If is_sql_where = '' Then
		is_sql_where = 'ctx_id in ' + ls_doc_filter
	Else
		is_sql_where += ' And ctx_id in ' + ls_doc_filter
	End If
End If

If IsValid( lds_value ) Then destroy( lds_value )
If IsValid( lds_value_facility ) Then destroy( lds_value_facility )					//Added By Mark Lee 11/27/2015
end subroutine

public subroutine of_page_toolbar ();//====================================================================
// Function: of_page_toolbar
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 05-12-2015
//--------------------------------------------------------------------
...
is_display_page = gnv_user_option.of_Get_option_value( gs_user_id, "display_page" )
if isnull(is_display_page) or is_display_page = '' then is_display_page = '0'
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
//if is_display_page = '1' then
	pb_1.visible = true
	pb_2.visible = true
	pb_3.visible = true
	pb_4.visible = true
	sle_page.visible = true
	pb_1.y = dw_search_det1.y + dw_search_det1.height - 164
	pb_2.y = pb_1.y
	pb_3.y = pb_1.y
	pb_4.y = pb_1.y
	sle_page.y = pb_1.y + 3
	pb_4.x = dw_search_det1.x + dw_search_det1.width - 200
	pb_3.x = pb_4.x -110
	sle_page.x = pb_3.x -215
	pb_2.x = sle_page.x -110
	pb_1.x = pb_2.x -110
	pb_1.width = 110
	pb_2.width = 110
	pb_3.width = 110
	...
	pb_4.bringtotop = true
	sle_page.bringtotop = true
else
	pb_1.visible = false
	pb_2.visible = false
	pb_3.visible = false
	pb_4.visible = false
	sle_page.visible = false
end if
...
public subroutine of_get_related_child_web (long al_ctx_id);//====================================================================
// Function: of_get_related_child_web
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 05-15-2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2014 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
long ll_find_row,ll_ctx_id
string ls_filter

ll_find_row = ids_search.Find('master_contract_id = ' + String(al_ctx_id),1,ids_search.Rowcount())
Do While ll_find_row > 0 
	ll_ctx_id = ids_search.GetItemNumber(ll_find_row,'ctx_id')
	If ids_search_related_ctx.Find('ctx_id = ' + String(ll_ctx_id),1,ids_search_related_ctx.Rowcount()) = 0 Then 
		ids_search.Rowscopy( ll_find_row, ll_find_row, Primary!, ids_search_related_ctx, 1, Primary!)
		of_get_related_child_web(ll_ctx_id)
	End If
	If ll_find_row =  ids_search.Rowcount() Then
		Exit
	End If
...
string ls_line,ls_display_page
...
is_display_page = ls_display_page
if is_display_page = '0' then il_current_page = -1
end subroutine

public subroutine of_set_dw_title (string as_page, string as_focus_sign);//====================================================================
// Function: of_set_dw_title
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    string    as_page
// 	value    string    as_focus_sign
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 05-27-2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2014 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
if as_page = '1' then
	if as_focus_sign = '1' then
		dw_search_det1.Title = gs_left_title_sign + 'Contract Browse (' + string(il_last_row) + ')' + gs_right_title_sign
	else
		dw_search_det1.title = 'Contract Browse ' + "(" + string(il_last_row) + ")"
	end if
else
	if as_focus_sign = '1' then
		dw_search_det1.title = gs_left_title_sign + 'Contract Browse (' + string(dw_search_det1.rowcount()) + ')' + gs_right_title_sign
	else
		dw_search_det1.title = 'Contract Browse ' + "(" + string(dw_search_det1.rowcount()) + ")"
	end if
end if
...
public function boolean of_get_hscrollbar (u_dw adw_1);// Function: of_get_hscrollbar
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
Long ll_dw_width,i, ll_cnt, ll_x,ll_width,ll_pos
Decimal{2} ldec_Zoom
String ls_objects, ls_oject_arr[], ls_type
n_cst_string lnv_string

ldec_Zoom         = Long(adw_1.describe("DataWindow.Zoom"))
ll_dw_width = adw_1.width

ls_objects = adw_1.Describe("Datawindow.Objects")
lnv_string.of_parsetoarray( ls_objects, '~t', ls_oject_arr[])
ll_cnt = Upperbound(ls_oject_arr[])

For i = 1 To ll_cnt
	If adw_1.describe( ls_oject_arr[i] + '.visible') = '0' Then Continue
	ls_type = adw_1.describe( ls_oject_arr[i] + '.type') 
	Choose Case ls_type
		Case 'line'
			ll_pos = 40 +  Long(adw_1.describe( ls_oject_arr[i] + '.x1') ) 
			ll_pos = ll_pos * ldec_Zoom/100
			If ll_pos > ll_dw_width Then
				Return True
			End If
			ll_pos = 40 +  Long(adw_1.describe( ls_oject_arr[i] + '.x2') ) 
			ll_pos = ll_pos * ldec_Zoom/100
			If ll_pos > ll_dw_width Then
				Return True
			End If			
			
		Case Else // 'button', 'column', 'picture','computedcomumn','text'	,'bitmap','groupbox'
			If isnumber(adw_1.describe( ls_oject_arr[i] + '.x') ) and  isnumber(adw_1.describe( ls_oject_arr[i] + '.width') ) then
				ll_pos = 40 + Long(adw_1.describe( ls_oject_arr[i] + '.x') ) +  Long(adw_1.describe( ls_oject_arr[i] + '.width') )
				ll_pos = ll_pos * ldec_Zoom/100
				If ll_pos > ll_dw_width Then
					Return True
				End If
...
public function string of_build_table_filter (datastore ads_value, string as_table, string as_sql);//====================================================================
// Function: of_build_table_filter
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    datastore    ads_value
// 	value    string       as_table
// 	value    string       as_sql
//--------------------------------------------------------------------
// Returns:  string
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 11/27/2015
//--------------------------------------------------------------------
...
Integer		li_Pos, li_Pos2
Long			ll_Num
String			ls_ColType, ls_ColName, ls_Value, ls_style
double		ldb_value
Datetime		ldt_value

if isnull(as_table) or trim(as_table) = '' then return ''
...
For ll_Num = 1 to ads_value.RowCount( )
	...
	ls_ColName =  ads_value.GetItemString( ll_Num, 'field' )	
	ls_Value =  ads_value.GetItemString( ll_Num, 'value' )
	ls_style =  ads_value.GetItemString( ll_Num, 'style' )
	
	Choose Case Lower(Left ( ls_ColType , 3 ))
		CASE 'int','lon','dec','num','dou','rea','ulo', 'mon'
			ldb_value = double( ls_value )
			If Not isnull(ldb_value ) and  ldb_value <> 0 Then
				ls_where += ' And ' + Replace(ls_ColName, len(as_table)+1, 1 , '.') + ' = '	 + String(ldb_value)
			End If
		Case 'dat'
			If Not (isnull(ls_Value )  Or ls_Value = '' ) Then
				ls_where += ' And ' + Replace(ls_ColName, len(as_table)+1, 1 , '.') + " = '"	 + String(ls_Value) + "'"
			End If
		CASE 'var','cha'	//text maybe use patindex 
			If Not isnull(ls_Value ) and  ls_Value <> '' Then
				if not isnull(ls_style) and ls_style <> '' then 
					ls_where += ' And ' + Replace(ls_ColName, len(as_table)+1, 1 , '.') + " = '"	 +  ls_Value + "' "
				else
					ls_where += ' And ' + Replace(ls_ColName, len(as_table)+1, 1 , '.') + " like '%"	 +  ls_Value + "%' "
				end if 
			End If
	End Choose	
Next
...
public function integer of_add_table (string as_table_name, ref string as_where_clause, ref datastore ads_list);// of_add_table()
//
//	7.18.2016 - Initial version by Ronnie Po
//
//	Arguments:
//	as_name - name of table to add
//	reference as_where_clause - the dynamic WHERE clause
//	reference ads_list - datastore initalized from "d_ext_table" - contains the list of table names
//
//	Description
//	Called by of_build_sql() - add the table as_name to the list of table names (to be used in dynamic FROM clause)
//
//	Returns:
//	1 - as_name successfully added to list
//	0 - No action; as_name is already in list
//	-1 - Error
//
long ll_row

IF NOT isValid(ads_list) THEN
	return -1
END IF

ll_row = ads_list.find("name = '" + lower(as_table_name) + "'", 1, ads_list.rowCount())
IF ll_row > 0 THEN
	return 0
END IF

ll_row = ads_list.insertRow(0)
ads_list.object.name[ll_row] = lower(as_table_name)

/* Add the portion of the WHERE clause required to join the new table to ctx_basic_info */
as_where_clause = as_where_clause + " and " + char(13) + char(10)
...
public function boolean of_confirmdoctextindex ();//	Function of_confirmDocTextIndex()
//
//	Returns:
//	TRUE - Full text index has been created on
//	FALSE - Index has not been created
//
//	Description
//	7.19.2016 - Initial version by Ronnie Po
//
integer li_fulltext
string ls_msg

// Logic taken from of_build_doc_filter()
  SELECT
    COLUMNPROPERTY (OBJECT_ID('ctx_am_doc_image'),'image_file','IsFulltextIndexed' )
  INTO
    :li_fulltext
  FROM
    ids
...
IF SQLCA.SQLCode = -1 THEN
	messageBox("Search Documents", "Unable to determine whether the full-text catalog has been created for documents.")
	return FALSE
ELSEIF li_fulltext = 0 Then
	ls_msg = "The Full-Text Catalog for documents has not yet been created. Do you want to create it now?" &
	+ "~r~n~r~nYes - Create the text index and proceed with searching document contents" &
	+ "~r~n~r~nNo - Do not search document contents"
	IF Messagebox("Search Documents", ls_msg, Question!,YesNo!) = 1 Then
		IF Not Isvalid(gnv_logservice) THEN gnv_logservice = Create n_logservice
		n_cst_update_sql lnv_sql
		lnv_sql = Create n_cst_update_sql
		lnv_sql.of_release_fulltext( )			
...
public function integer of_search_sql (ref string as_from, ref string as_where);// of_search_sql()
//
// Arguments:
//	ref as_from - additions to the dynamic FROM clause
//	ref as_where - addiions to the dynamic WHERE clause
//
//	Returns:
//	1 - Success
//	-1 - Error
...
//
//	Description
//	The base query selects from table ctx_basic_info
//	Depending on search criteria provided by the user, determine the additional required portion(s) of the FROM clause and WHERE clause
//	and return those in as_from and as_where, respectively
//
Long   ll_ix
String ls_ColType,ls_ColName,ls_Value, ls_style
String ls_doc_name,ls_doc_content
Long ll_doc_id,ll_doc_category, ll_doc_status
string ls_facility_name
String	ls_Visible

string ls_from, ls_where
datastore lds_table
long ll_row

string ls_where_occi, ls_where_ccci //Added By Gavin.Jay 2017-02-16

IF dw_search.rowCount() < 1 THEN
	return -1
ELSEIF dw_search.acceptText() <> 1 THEN
	return -1
END IF

lds_table = CREATE datastore
lds_table.dataobject = "d_ext_table"

/*==========================================================================*/
/* Document parameters */
ll_doc_id = dw_search.object.document_id[1]
ls_doc_name = trim(dw_search.object.document_name[1])
IF isNull(ls_doc_name) THEN ls_doc_name = ""
ll_doc_category = dw_search.object.document_category[1]
ll_doc_status = dw_search.object.document_status[1]
ls_doc_content = trim(dw_search.object.document_content[1])
IF isNull(ls_doc_content) THEN ls_doc_content = ""

IF isNull(ll_doc_id) AND (ls_doc_name = "") AND isNull(ll_doc_category) AND isNull(ll_doc_status) &
	AND (ls_doc_content = "") THEN
	/* No document parms */
ELSE
	ls_from += ",~r~n  ctx_am_document"
	ls_where += " AND~r~n    ( ctx_am_document.ctx_id = ctx_basic_info.ctx_id )"
	IF NOT isNull(ll_doc_id) THEN
		ls_where += " AND~r~n    ( ctx_am_document.doc_id = " + string(ll_doc_id) + " )"
	END IF
	IF ls_doc_name <> "" THEN
		ls_doc_name = inv_string.of_globalReplace(ls_doc_name, "'", "''")
		ls_where += " AND~r~n    ( ctx_am_document.doc_name LIKE '%" + ls_doc_name + "%' )"
	END IF
	IF NOT isNull(ll_doc_category) THEN
		ls_where += " AND~r~n    ( ctx_am_document.category = " + string(ll_doc_category) + " )"
	END IF
	IF NOT isNull(ll_doc_status) THEN
		ls_where += " AND~r~n    ( ctx_am_document.status = " + string(ll_doc_status) + " )"
	END IF
	IF ls_doc_content <> "" THEN
		/* Perform a full text search if index has been created. */
		IF of_confirmDocTextIndex() THEN
			// <RP> 8.15.2016 ---------------------------------------------------------
			ls_from += ",~r~n  ctx_am_doc_image"
			ls_where += " AND~r~n    ( ctx_am_doc_image.doc_id = ctx_am_document.doc_id )" &
				+ " AND~r~n    ( ctx_am_doc_image.revision = ctx_am_document.revision )" &
				+ " AND~r~n    ( contains(ctx_am_doc_image.image_file ,'" + ls_doc_content + "' ) or contains( ctx_am_doc_image.pdftext, '" +  ls_doc_content + "' ) )"
		END IF
	END IF
END IF
...
/* facility_name */
ls_facility_name = trim(dw_search.object.facility_name[1])
IF isNull(ls_facility_name) THEN ls_facility_name = ""
IF lower(ls_facility_name) = "(none)" THEN ls_facility_name = ""
IF ls_facility_name <> "" THEN
	/* Check for reserved characters when performing a SQL LIKE comparison */
	ls_facility_name = of_search_escape_sql(ls_facility_name)
	of_search_add_table("app_facility", lds_table, ls_where)
	ls_where += " AND~r~n    ( app_facility.facility_name LIKE '%" + ls_facility_name + "%' )"
END IF

FOR ll_ix = 1 to long(dw_search.describe("DataWindow.Column.Count"))
	//if dw_search.getitemstatus( 1, ll_ix, Primary!) <> DataModified! then continue	-- Comment by Jervis 04.28.2009 Fixed BugJ042802
	ls_ColType =  dw_search.describe("#" + string(ll_ix) + ".ColType")
	ls_ColName =  dw_search.describe("#" + string(ll_ix) + ".Name")
	ls_Visible = dw_search.describe("#" + string(ll_ix) + ".visible")
	ls_Value = dw_search.inv_base.of_getitem(1,ls_ColName,false)
	ls_style = dw_search.describe("#" + string(ll_ix) + ".edit.style")	
	...
	END IF
	CHOOSE CASE lower(ls_colName)
		//Added by harry 20170509 Task 762	
		CASE "custom_1", "custom_2", "custom_3", "custom_4"
			IF of_check_multi_field(ls_colName) THEN 
				IF ls_style = 'dddw' THEN
					ls_where += " AND~r~n    ( ctx_basic_info." + trim(ls_colName) + " = '" + ls_value +  "' OR " + of_multi_select_sql(ls_colName, ls_value) + " ) "
				ELSE
					ls_value = of_search_escape_sql(ls_value)
					ls_where += " AND~r~n    ( ctx_basic_info." + trim(ls_colName) + " LIKE '%" + ls_value + "%' )"
				END IF
			ELSE
				CONTINUE
			END IF
		/*	
		CASE "ctx_id", "category", "status", "facility_name", "contract_type", "group_multi_loc_id", "product_type", &
			"group_id", "effective_date", "effective_date2", "expriation_date", "expriation_date2", "last_revis_date", &
			"last_revis_date_2", "review_date", "review_date_2", "term_date", "term_date_2", "version_date", &
			"version_date_2", "custom_date1", "custom_date1_2", "custom_date2", "custom_date2_2", "custom_1", &
			"custom_2", "custom_3", "custom_4", "custom_n1", "custom_n2", "custom_n3", "custom_n4", "custom_n5", &
			"custom_n6", "owner", "dvision", "document_id", "document_name", "document_category", "document_status", &
			"document_content", "product_name", "ccci_name", "ccci_association", "ccci_contact_email", "ccci_custom_n1", &
			"ccci_custom_1", "occi_name", "occi_association", "occi_contact_email", "occi_custom_n1", "occi_custom_1"
			*/
		CASE "ctx_id", "category", "status", "facility_name", "contract_type", "group_multi_loc_id", "product_type", &
			"group_id", "effective_date", "effective_date2", "expriation_date", "expriation_date2", "last_revis_date", &
			"last_revis_date_2", "review_date", "review_date_2", "term_date", "term_date_2", "version_date", &
			"version_date_2", "custom_date1", "custom_date1_2", "custom_date2", "custom_date2_2", &
			 "custom_n1", "custom_n2", "custom_n3", "custom_n4", "custom_n5", &
			"custom_n6", "owner", "dvision", "document_id", "document_name", "document_category", "document_status", &
			"document_content", "product_name", "ccci_name", "ccci_association", "ccci_contact_email", "ccci_custom_n1", &
			"ccci_custom_1", "occi_name", "occi_association", "occi_contact_email", "occi_custom_n1", "occi_custom_1"
			/* These are handled elsewhere, or on the back end */
			CONTINUE
		CASE ELSE
			/* Drop through */
	END CHOOSE
	...
	Choose Case Lower(Left ( ls_ColType , 3 ))
		CASE 'int','lon','dec','num','dou','rea','ulo', 'mon'
			IF match(lower(ls_colName), "^custom2_") THEN
				of_search_add_table("ctx_custom", lds_table, ls_where)
				ls_where += " AND~r~n    ( ctx_custom." + lower(mid(ls_colName, 9)) + " = " + ls_value + " )"
				CONTINUE
			END IF
			choose case lower(ls_ColName)
//				case 'unread_email'
...
//						end if 
//					end if	
				CASE 'feesched_category', 'feesched_service_item', 'feesched_item_status', 'feesched_custom_int1','feesched_custom_int2', &
					'feesched_billing_cycle', 'feesched_fee', 'feesched_custom_fee1','feesched_custom_fee2','feesched_custom_fee3', &
					'feesched_custom_n1',  'feesched_custom_n2',  'feesched_custom_n3',  'feesched_custom_n4',  'feesched_custom_n5'
					of_search_add_table("ctx_fee_sched_nm", lds_table, ls_where)
					ls_where += " AND~r~n    ( ctx_fee_sched_nm." + lower(mid(ls_colName, 10)) + " = " + ls_value + " )"
				case 'app_facility_country','app_facility_corp_office_flag','app_facility_corp_office_link'
					of_search_add_table("app_facility", lds_table, ls_where)
					ls_where += " AND~r~n    ( app_facility." + lower(mid(ls_colName, 14)) + " = " + ls_value + " )"

 				//Begin - Added By Jay 2017-02-15
				Case 'ccci_custom_n2','ccci_custom_n3'
					//of_search_add_table("ctx_contract_contacts", lds_table, ls_where)
					ls_where_ccci += " AND~r~n    ( ctx_contract_contacts." + lower(mid(ls_colName, 6)) + " = " + ls_value + " )"
				Case 'occi_custom_n2','occi_custom_n3'
					//of_search_add_table("ctx_contract_contacts", lds_table, ls_where)
					ls_where_occi += " AND~r~n    ( ctx_contract_contacts." + lower(mid(ls_colName, 6)) + " = " + ls_value + " )"
				Case 'ccci_contacts_contact_department','ccci_contacts_contact_type'
					//of_search_add_table("ctx_contacts", lds_table, ls_where)
					ls_where_ccci += " AND~r~n    ( ctx_contacts." + lower(mid(ls_colName, 15)) + " = " + ls_value + " )"						
				Case 'occi_contacts_contact_department','occi_contacts_contact_type'
					//of_search_add_table("ctx_contacts", lds_table, ls_where)					
					ls_where_occi += " AND~r~n    ( ctx_contacts." + lower(mid(ls_colName, 15)) + " = " + ls_value + "  )"		
				//End - Added By Jay 2017-02-15
				
				
				case else
					/* <RP> - Assume that the column is in the ctx_basic_info table */
					ls_where += " AND~r~n    ( ctx_basic_info." + trim(ls_colName) + " = " + ls_value + " )"
		...
		CASE 'dat'
			...
			END IF
			choose case lower(ls_ColName)
				case 'feesched_custom_date1', 'feesched_custom_date2','feesched_custom_date3','feesched_custom_date4', &
					'feesched_custom_date5', 'feesched_start_date', 'feesched_end_date'
					of_search_add_table("ctx_fee_sched_nm", lds_table, ls_where)
					ls_where += " AND~r~n    ( ctx_fee_sched_nm." + lower(mid(ls_colName, 10)) + " = '" + ls_value + "' )"
				case else
					/* <RP> - Assume that the column is in the ctx_basic_info table */
		...
		CASE 'var','cha'	
			IF match(lower(ls_colName), "^custom2_") THEN
				...
				IF ls_style = "dddw" THEN
					//ls_where += " AND~r~n    ( ctx_custom." + lower(mid(ls_colName, 9)) + " = '" + ls_value + "' )"
					IF of_check_multi_field(ls_colName) THEN //Added by harry 20170509 Task 762
						ls_where += " AND~r~n    ( ctx_custom." + lower(mid(ls_colName, 9)) + " = '" + ls_value + "' OR " + of_multi_select_sql(ls_colName, ls_value) + " ) "
					ELSE 
						ls_where += " AND~r~n    ( ctx_custom." + lower(mid(ls_colName, 9)) + " = '" + ls_value + "' )"
					END IF
				ELSE
					ls_value = of_search_escape_sql(ls_value)
					ls_where += " AND~r~n    ( ctx_custom." + lower(mid(ls_colName, 9)) + " LIKE '%" + ls_value + "%' )"
				END IF
				CONTINUE
			END IF			
			choose case lower(ls_ColName)
				case 'feesched_description', 'feesched_custom_text1', 'feesched_custom_text2', 'feesched_custom_text3', &
					'feesched_custom_text4', 'feesched_custom_text5', 'feesched_custom_text6', 'feesched_custom_text7', &
					'feesched_custom_text8', 'feesched_notes'
					of_search_add_table("ctx_fee_sched_nm", lds_table, ls_where)
					ls_where += " AND~r~n    ( ctx_fee_sched_nm." + lower(mid(ls_colName, 10)) + " LIKE '%" + ls_value + "%' )"
				case 'app_facility_custom_1','app_facility_custom_2','app_facility_custom_3','app_facility_custom_4', &
					'app_facility_custom_5','app_facility_custom_6', 'app_facility_custom_7','app_facility_custom_8', &
					'app_facility_custom_9','app_facility_custom_10','app_facility_status','app_facility_street', &
					'app_facility_city','app_facility_state','app_facility_contact_type2','app_facility_contact','app_facility_street_2', &
					'app_facility_street_3','app_facility_zip','app_facility_phone','app_facility_fax','app_facility_web_address'						
					of_search_add_table("app_facility", lds_table, ls_where)
					IF ls_style = 'dddw' THEN
						ls_where += " AND~r~n    ( app_facility." + lower(mid(ls_colName, 14)) + " = '" + ls_value + "' )"
					ELSE
						ls_value = of_search_escape_sql(ls_value)
						ls_where += " AND~r~n    ( app_facility." + lower(mid(ls_colName, 14)) + " LIKE '%" + ls_value + "%' )"
					END IF	

				//Begin - Added By Jay 2017-02-15

				Case 'ccci_association2','ccci_association3','ccci_association4','ccci_association5','ccci_association6',&
					'ccci_association7','ccci_association8','ccci_association9','ccci_association10','ccci_custom_2','ccci_custom_3'
					//of_search_add_table("ctx_contract_contacts", lds_table, ls_where)
					IF ls_style = 'dddw' THEN
						ls_value = of_search_escape_sql(ls_value)
						ls_where_ccci += " AND~r~n    ( ctx_contract_contacts." + lower(mid(ls_colName, 6)) + " = '" + ls_value + "' )"						
					ELSE
						ls_value = of_search_escape_sql(ls_value)
						ls_where_ccci += " AND~r~n    ( ctx_contract_contacts." + lower(mid(ls_colName, 6)) + " LIKE '%" + ls_value + "%' )"
					END IF
				Case 'occi_association2','occi_association3','occi_association4','occi_association5','occi_association6',&
					'occi_association7','occi_association8','occi_association9','occi_association10','occi_custom_2','occi_custom_3'
					//of_search_add_table("ctx_contract_contacts", lds_table, ls_where)
					IF ls_style = 'dddw' THEN
						ls_value = of_search_escape_sql(ls_value)
						ls_where_occi += " AND~r~n    ( ctx_contract_contacts." + lower(mid(ls_colName, 6)) + " = '" + ls_value + "'  )"						
					ELSE
						ls_value = of_search_escape_sql(ls_value)
						ls_where_occi += " AND~r~n    ( ctx_contract_contacts." + lower(mid(ls_colName, 6)) + " LIKE '%" + ls_value + "%' )"
					END IF
				Case 'ccci_contacts_contact_title','ccci_contacts_status','ccci_contacts_defaultto','ccci_contacts_custom_1',&
					'ccci_contacts_custom_2','ccci_contacts_custom_3','ccci_contacts_custom_4','ccci_contacts_custom_5',&
					'ccci_contacts_custom_6','ccci_contacts_custom_7','ccci_contacts_custom_8','ccci_contacts_custom_9','ccci_contacts_custom_10'
					//of_search_add_table("ctx_contacts", lds_table, ls_where)
					IF ls_style = 'dddw' THEN
						ls_value = of_search_escape_sql(ls_value)
						ls_where_ccci += " AND~r~n    ( ctx_contacts." + lower(mid(ls_colName, 15)) + " = '" + ls_value + "' )"						
					ELSE
						ls_value = of_search_escape_sql(ls_value)
						ls_where_ccci += " AND~r~n    ( ctx_contacts." + lower(mid(ls_colName, 15)) + " LIKE '%" + ls_value + "%' )"
					END IF
				Case 'occi_contacts_contact_title','occi_contacts_status','occi_contacts_defaultto','occi_contacts_custom_1',&
					'occi_contacts_custom_2','occi_contacts_custom_3','occi_contacts_custom_4','occi_contacts_custom_5',&
					'occi_contacts_custom_6','occi_contacts_custom_7','occi_contacts_custom_8','occi_contacts_custom_9','occi_contacts_custom_10'
					//of_search_add_table("ctx_contacts", lds_table, ls_where)
					IF ls_style = 'dddw' THEN
						ls_value = of_search_escape_sql(ls_value)
						ls_where_occi += " AND~r~n    ( ctx_contacts." + lower(mid(ls_colName, 15)) + " = '" + ls_value + "' )"						
					ELSE
						ls_value = of_search_escape_sql(ls_value)
						ls_where_occi += " AND~r~n    ( ctx_contacts." + lower(mid(ls_colName, 15)) + " LIKE '%" + ls_value + "%' )"
					END IF
					//End - Added By Jay 2017-02-15
...
FOR ll_row = 1 TO lds_table.rowCount()
	ls_from = ls_from + ",~r~n  " + lds_table.object.name[ll_row]
NEXT

DESTROY lds_table

//Added By Gavin.Jay 2017-02-16  Task 543:Add the contract_contact_type field to the Search Criteria
if Len ( ls_where_ccci ) > 0 Then
	ls_where += " AND ~r~n ctx_basic_info.ctx_id = ~r~n" + &
	" ( SELECT distinct ctx_contract_contacts.ctx_id FROM  ctx_contract_contacts, ctx_contacts ~r~n" + &
	...
	" AND ctx_contract_contacts.ic_area = 'E' ~r~n" + ls_where_ccci + ")~r~n"
End If

if Len ( ls_where_occi ) > 0 Then
	ls_where += " AND ~r~n ctx_basic_info.ctx_id = ~r~n" + &
	" ( SELECT distinct ctx_contract_contacts.ctx_id FROM  ctx_contract_contacts, ctx_contacts ~r~n" + &
	" WHERE (ctx_contract_contacts.contact_id = ctx_contacts.contact_id ~r~n" + &
	" AND ctx_contract_contacts.ctx_id = ctx_basic_info.ctx_id ~r~n" + &
	" AND ctx_contract_contacts.ic_area = 'I') ~r~n" + ls_where_occi + ")~r~n"
End If
//////////////////////////end  //Added By Gavin.Jay 2017-02-16
...
public function integer of_search_add_table (string as_table_name, ref datastore ads_table, ref string as_where);// of_search_table()
// 	Returns:
//	1 - Successfully added
//	0 - No action, table has already been added
//	-1 - Error
//
// Dynamically build list of tables plus WHERE clause
// 7.19.2016 - Initial version by Ronnie Po
//
// Called by of_search_sql()
//
// Assume that ads_table and as_where are valid
//
long ll_row

ll_row = ads_table.find("name = '" + lower(as_table_name) + "'", 1, ads_table.rowCount())
IF ll_row > 0 THEN
	/* Table has already been added */
	return 0
ELSE
	ll_row = ads_table.insertRow(0)
	ads_table.object.name[ll_row] = lower(as_table_name)
	CHOOSE CASE lower(as_table_name)
		CASE "app_facility"
			as_where += " AND~r~n    ( app_facility.facility_id = ctx_basic_info.app_facility )"
		CASE ELSE
			as_where += " AND ~r~n    ( " + lower(as_table_name) + ".ctx_id = ctx_basic_info.ctx_id )"
	END CHOOSE
	return 1
END IF
...
public function string of_search_escape_sql (string as_term);// of_search_escape_sql()
//
// Arguments:
//	as_term
//		- the search term, in which reserved characters in a LIKE comparison will be escaped so that they behave like regular characters
//
//	Returns:
//	String - the search term, with reserved characters replaced by their escaped versions
//
//	8.12.2016 - Initial version by Ronnie Po
//
//
IF isNull(as_term) THEN
	return as_term
ELSEIF trim(as_term) = "" THEN
	return as_term
END IF

/* Replace the following with their escaped versions: */
/* % (percent) */
/* _ (underscore) */
/* ^ (caret) */
/* [ */
...
as_term = inv_string.of_globalReplace(as_term, "'", "''")

/* Replace brackets with something unique first, else the escaped characters will be interpreted as brackets */
as_term = inv_string.of_globalReplace(as_term, "[", "<iembhkw4u7dk>")
as_term = inv_string.of_globalReplace(as_term, "]", "<dhbhelowkd3e>")

as_term = inv_string.of_globalReplace(as_term, "%", "[%]")
as_term = inv_string.of_globalReplace(as_term, "_", "[_]")
as_term = inv_string.of_globalReplace(as_term, "^", "[^]")
...
public function integer of_goto_contract (long al_ctx_id);// Function of_goto_contract
//
// Go to a specific contract, for example when clicking a recent contract on the Dashboard
//
// Argument:
//	al_ctx_id - identifies the specific contract to find
//
// Returns:
//	1 - success
//	-1 - error
//
// 9.21.2016  - Initial version by <RP>
//

long ll_row, ll_ret, ll_null
string ls_null
datetime ldt_null
decimal ldec_null
...
IF isNull(al_ctx_id) THEN return -1
...
IF dw_search_det1.rowCount() = 0 THEN
	/* Perform a search */
	dw_search.reset()
	dw_search.insertRow(0)
	dw_search.object.ctx_id[1] = al_ctx_id
	cb_1.event clicked()
END IF
...
IF ll_row = 0 THEN
	...
	/* Try to retrieve */
	IF NOT isValid(ids_find) THEN
		ids_find = CREATE datastore
		ids_find.dataobject = "d_sp_contract_search_base"
		ids_find.setTransObject(SQLCA)
	END IF
	lb_prevAutoCommit = SQLCA.AutoCommit
	SQLCA.AutoCommit = TRUE
	ll_ret = ids_find.retrieve( gs_user_id, al_ctx_id )

	SQLCA.AutoCommit = lb_prevAutoCommit


	IF ll_ret = 0 THEN
		return -1
	ELSE
		/* Move rows to the end of dw_search_det1 */
		ll_row = dw_search_det1.rowCount() + 1
		li_ret = ids_find.rowsCopy(1, ids_find.rowCount(), primary!, dw_search_det1, ll_row, primary!)
		dw_search_det1.resetUpdate()
		/* For backward compatability, also copy to ids_search */
		li_ret = ids_find.rowsCopy(1, ids_find.rowCount(), primary!, ids_search, ids_search.rowCount() + 1, primary!)
	END IF
...
IF ll_row > 0 THEN
	...
	dw_search_det1.scrolltorow(ll_row)
	dw_search_det1.selectrow( ll_row, true)
	inv_contract_details.of_set_ctx_id(ll_row)
	dw_search_det1.event rowfocuschanged( ll_row)
...
public function long of_retrievewithparms ();//	Function of_retrieveWithParms()
//
//	Description:
//	New retrieval functionality to replace previous retrieve + filter functionnality
//	
//	Returns:
//	Number of rows retrieved - Success
// -2 - User canceled the retrieve following warning message about large result set
//	-1 - Nonspecific Error
//
//	7.20.2016 - Initial version by Ronnie Po
// Change return integer to Long Modified by Gavin 20161018
long ll_ret
string ls_where, ls_from
integer li_ix
string ls_edit_style[]

IF of_search_sql(ls_from, ls_where) = -1 THEN
	return -1
END IF

// 8.15.2016 - send edit style to SP
FOR li_ix = 1 TO 4
	ls_edit_style[li_ix] = dw_search.describe("custom_" + string(li_ix) + ".Edit.Style")
NEXT

//ll_ret = ids_search.retrieve (gs_user_id, dw_search.object.ctx_id[1], dw_search.object.category[1], dw_search.object.status[1], dw_search.object.contract_type[1], &
//	dw_search.object.group_multi_loc_id[1], dw_search.object.group_id[1], dw_search.object.product_type[1], dw_search.object.product_name[1], dw_search.object.effective_date[1], &
//	dw_search.object.effective_date2[1], dw_search.object.expriation_date[1], dw_search.object.expriation_date2[1], dw_search.object.last_revis_date[1], dw_search.object.last_revis_date_2[1], &
//	dw_search.object.review_date[1], dw_search.object.review_date_2[1], dw_search.object.term_date[1], dw_search.object.term_date_2[1], 	dw_search.object.version_date[1], &
//	dw_search.object.version_date_2[1], dw_search.object.custom_date1[1], 	dw_search.object.custom_date1_2[1], dw_search.object.custom_date2[1], dw_search.object.custom_date2_2[1], &
//	dw_search.object.custom_1[1], dw_search.object.custom_2[1], dw_search.object.custom_3[1],	dw_search.object.custom_4[1], dw_search.object.custom_n1[1], &
//	dw_search.object.custom_n2[1], dw_search.object.custom_n3[1], dw_search.object.custom_n4[1], dw_search.object.custom_n5[1], 	dw_search.object.custom_n6[1], &
//	dw_search.object.owner[1], dw_search.object.dvision[1], dw_search.object.ccci_name[1], dw_search.object.ccci_association[1], dw_search.object.ccci_contact_email[1], &
//	dw_search.object.ccci_custom_n1[1], dw_search.object.ccci_custom_1[1], dw_search.object.occi_name[1], dw_search.object.occi_association[1], dw_search.object.occi_contact_email[1], &
//	dw_search.object.occi_custom_n1[1], dw_search.object.occi_custom_1[1], ls_from, ls_where )

// <RP> debug 9.8.2016
//ll_ret = ids_search.retrieve (gs_user_id)

//Added by Harry 20170509 Task 762:Search does not find results in multi-select fields
String ls_custom_1, ls_custom_2, ls_custom_3, ls_custom_4

If of_check_multi_field('custom_1') Then
	ls_custom_1 = ""
Else
	ls_custom_1 = of_search_escape_sql(dw_search.object.custom_1[1])
End If

If of_check_multi_field('custom_2') Then
	ls_custom_2 = ""
Else
	ls_custom_2 = of_search_escape_sql(dw_search.object.custom_2[1])
End If

If of_check_multi_field('custom_3') Then
	ls_custom_3 = ""
Else
	ls_custom_3 = of_search_escape_sql(dw_search.object.custom_3[1])
End If

If of_check_multi_field('custom_4') Then
	ls_custom_4 = ""
Else
	ls_custom_4 = of_search_escape_sql(dw_search.object.custom_4[1])
End If

//Modified by Harry 20170509 Task 762
/*
ll_ret = ids_search.retrieve (gs_user_id, dw_search.object.ctx_id[1], dw_search.object.category[1], dw_search.object.status[1], dw_search.object.contract_type[1], &
	dw_search.object.group_multi_loc_id[1], dw_search.object.group_id[1], dw_search.object.product_type[1], dw_search.object.product_name[1], &
	dw_search.object.effective_date[1], dw_search.object.effective_date2[1], dw_search.object.expriation_date[1], dw_search.object.expriation_date2[1], &
	dw_search.object.last_revis_date[1], dw_search.object.last_revis_date_2[1], dw_search.object.review_date[1], 	dw_search.object.review_date_2[1], &
	dw_search.object.term_date[1], dw_search.object.term_date_2[1], dw_search.object.version_date[1], dw_search.object.version_date_2[1], &
	dw_search.object.custom_date1[1], 	dw_search.object.custom_date1_2[1], dw_search.object.custom_date2[1], dw_search.object.custom_date2_2[1], &
	ls_edit_style[1], ls_edit_style[2], ls_edit_style[3], ls_edit_style[4], of_search_escape_sql(dw_search.object.custom_1[1]), &
	of_search_escape_sql(dw_search.object.custom_2[1]), of_search_escape_sql(dw_search.object.custom_3[1]), of_search_escape_sql(dw_search.object.custom_4[1]), &
	dw_search.object.custom_n1[1], dw_search.object.custom_n2[1], dw_search.object.custom_n3[1], dw_search.object.custom_n4[1], dw_search.object.custom_n5[1], &
	dw_search.object.custom_n6[1], dw_search.object.owner[1], dw_search.object.dvision[1], dw_search.object.ccci_name[1], dw_search.object.ccci_association[1], &
	dw_search.object.ccci_contact_email[1], dw_search.object.ccci_custom_n1[1], dw_search.object.ccci_custom_1[1], dw_search.object.occi_name[1], &
	dw_search.object.occi_association[1], dw_search.object.occi_contact_email[1], dw_search.object.occi_custom_n1[1], 	dw_search.object.occi_custom_1[1], &
	ls_from, 	ls_where, "Y" )	
	*/
ll_ret = ids_search.retrieve (gs_user_id, dw_search.object.ctx_id[1], dw_search.object.category[1], dw_search.object.status[1], dw_search.object.contract_type[1], &
	dw_search.object.group_multi_loc_id[1], dw_search.object.group_id[1], dw_search.object.product_type[1], dw_search.object.product_name[1], &
	...
	dw_search.object.term_date[1], dw_search.object.term_date_2[1], dw_search.object.version_date[1], dw_search.object.version_date_2[1], &
	dw_search.object.custom_date1[1], 	dw_search.object.custom_date1_2[1], dw_search.object.custom_date2[1], dw_search.object.custom_date2_2[1], &
	ls_edit_style[1], ls_edit_style[2], ls_edit_style[3], ls_edit_style[4], ls_custom_1, ls_custom_2, ls_custom_3, ls_custom_4, &
	dw_search.object.custom_n1[1], dw_search.object.custom_n2[1], dw_search.object.custom_n3[1], dw_search.object.custom_n4[1], dw_search.object.custom_n5[1], &
	dw_search.object.custom_n6[1], dw_search.object.owner[1], dw_search.object.dvision[1], dw_search.object.ccci_name[1], dw_search.object.ccci_association[1], &
	dw_search.object.ccci_contact_email[1], dw_search.object.ccci_custom_n1[1], dw_search.object.ccci_custom_1[1], dw_search.object.occi_name[1], &
	dw_search.object.occi_association[1], dw_search.object.occi_contact_email[1], dw_search.object.occi_custom_n1[1], 	dw_search.object.occi_custom_1[1], &
...
//
// <RP> 9.15.2016 ---------------------------------------------
// Stopgap: check if a warning message was returned because of a large result set
IF ll_ret = 1 THEN
	IF ids_search.object.app_facility[1] = -1 THEN
		IF messageBox("Warning", ids_search.getItemString(1, "product_name"), StopSign!, YesNo!, 2) = 2 THEN
			/* Indicate that user has canceled the retrieval */
			return -2
		ELSE
			/* Re-run the retrieve with check_rowcount_flag = 'N', forcing the result set. */
			//Modified by Harry 20170509 Task 762
			/*
			ll_ret = ids_search.retrieve (gs_user_id, dw_search.object.ctx_id[1], dw_search.object.category[1], dw_search.object.status[1], dw_search.object.contract_type[1], &
				dw_search.object.group_multi_loc_id[1], dw_search.object.group_id[1], dw_search.object.product_type[1], dw_search.object.product_name[1], &
...
public function boolean of_check_multi_field (string as_colname);//Added by harry 20170509 Task 762:Search does not find results in multi-select fields
...
If match(lower(as_colname), "^custom2_") Then
	ls_screen_id = '10'
	ls_field_name = lower(mid(as_colname, 9))
Else
	ls_screen_id = '9'
	ls_field_name = as_colname
End If
...
public function string of_multi_select_sql (string as_colname, string as_value);//Added by harry 20170509 Task 762:Search does not find results in multi-select fields
...
If match(lower(as_colname), "^custom2_") Then
	ls_screen_id = '10'
	ls_field_name = lower(mid(as_colname, 9))
	ls_table_name = 'ctx_custom'
Else
	ls_screen_id = '9'
	ls_field_name = as_colname
	ls_table_name = 'ctx_basic_info'
End If


ls_ret_sql = " exists ( select 1 from screen_field_select_item x where x.ctx_id = " + ls_table_name + ".ctx_id and x.field_name = '" + ls_field_name + "' and x.screen_id = " + ls_screen_id + " and x.field_data = '" + as_value + "')"
...
this.Control[iCurrent+21]=this.sle_page
this.Control[iCurrent+22]=this.uo_page
end on
...
this.inv_resize.of_unregister(uo_page) //Added By Jay Chen 03-25-2016 unresize uo_page
...
//Added By Jay Chen 04-10-2015
ids_contract_access_contract_type = create n_ds
ids_contract_access_contract_type.dataobject = 'ds_contract_access_contract_type'
ids_contract_access_contract_type.settransobject(sqlca)
ids_contract_access_division = create n_ds
ids_contract_access_division.dataobject = 'ds_contract_access_division'
ids_contract_access_division.settransobject(sqlca)
ids_contract_access_department = create n_ds
ids_contract_access_department.dataobject = 'ds_contract_access_department'
ids_contract_access_department.settransobject(sqlca)
//end

//Added By Ken.Guo 02/05/2015
ids_contract_status_noaccess = create n_ds
ids_contract_status_noaccess.dataobject = 'd_contract_status_noaccess'
ids_contract_status_noaccess.settransobject(sqlca)

//added by gavins  retrieve group access 
of_RetreiveGroupAcess()
//--------------------------- APPEON BEGIN ---------------------------
//$< Add  > 2008-05-13 By: Scofield
//$<Reason> Add the right to turn off the more button.
ll_Right = w_mdi.of_security_access(2081)
if ll_Right = 0 then
//	dw_search.Object.b_more.Visible = false //modified by gavins 20150519   because datawindow isnot create in constructor event. 
//	//dw_search.Object.b_clear.X = Long(dw_search.Object.owner.X) + Long(dw_search.Object.owner.Width) - Long(dw_search.Object.b_clear.Width)
//	dw_Search.Object.b_clear.x = dw_Search.Object.b_search.x
//	dw_Search.Object.b_search.x = dw_Search.Object.b_more.x
end if
...
if (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) and gl_MoveToCtxID > 0 and gs_MoveToTabNo <> "" then
	ib_autoretrieve = true
else
...
if IsValid(ids_related_ctx_tmp) then Destroy ids_related_ctx_tmp //Added By Ken.Guo 2016-08-11
//---------<RP> 9.21.2016 --------------------------------------------
IF isValid(ids_find) THEN DESTROY ids_find
//-------------------------------------------------------------------------
end event
...
event clicked;call super::clicked;// <RP> 7.25.2016---------------------------------------------------------------------------------
//	Redirect to ue_search_2()
//
//parent.event ue_search_2()
//return
//-----------------------------------------------------------------------------------------------------
...
//Added By Alan on 2009-1-14
ib_autoretrieve = true
gnv_debug.of_output( False,'parent.of_retrieve( ): start.')
li_ret = parent.of_retrieve( )
gnv_debug.of_output( False,'parent.of_retrieve( ): end.')
//Added By Alan on 2009-1-14
ib_autoretrieve = lb_tempautoret
parent.ib_retriev_detail = false

// <RP> 9.16.2016 - check return code. -2 means that user has canceled after receiving warning message about large result set
IF li_ret >= 0 THEN

	gnv_debug.of_output( False,'dw_search_det1.Event ue_refreshdddw( dddw_contact_company ): start.')
	dw_search_det1.Event ue_refreshdddw( 'dddw_contact_company' )//added by gavins 20121129
	gnv_debug.of_output( False,'dw_search_det1.Event ue_refreshdddw( dddw_contact_company ): end.')
	gnv_debug.of_output( False,'dw_search_det1.event ue_populatedddws( ): start.')
	dw_search_det1.event ue_populatedddws( )
	gnv_debug.of_output( False,'dw_search_det1.event ue_populatedddws( ): end.')
...
//$< Add  > 2007-08-17 By: Scofield
//$<Reason> If there is no row and isn't called by post_open event,then give use a message.
// <RP> 9.16.2016 - add " AND li_ret <> -2" */
if (dw_search_det1.RowCount() < 1) AND (li_ret <> -2) then
	iw_parent.title = 'Contract Logix'
	gnv_status_info.of_display( 'Ready', '1'  )//added by gavins 20130410	
...
ib_refresh_button = false
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then //Added By Jay Chen 05-18-2015
//if is_display_page = '1' and is_type = 'WEB' then //Added By Jay Chen 05-18-2015
	cb_1.event clicked()
	ib_retrieved_ctx = false
	ib_retriev_detail = true
else
	//
//	if ib_retrieved_ctx or ib_retriev_detail = false then
//		gnv_debug.of_output( False,'of_filter(): start.')
//		of_filter()	//08.23.2007 By Jervis
//		gnv_debug.of_output( False,'of_filter(): end.')
//		//--------------------------- APPEON BEGIN ---------------------------
//		//$< Add  > 2007-08-17 By: Scofield
//		//$<Reason> If there is no row and isn't called by post_open event,then give use a message.
//		if dw_search_det1.RowCount() < 1 then
...
//		//---------------------------- APPEON END ----------------------------
//	else
		gnv_debug.of_output( False,'cb_1.event clicked(): start.')
		cb_1.event clicked()
		gnv_debug.of_output( False,'cb_1.event clicked(): end.')
//	end if
end if
...
//Added By Ken.Guo 2015-10-22.
IF Not Isvalid(iw_parent) THEN 							
	of_getparentwindow(iw_parent)	
End If
If iw_parent.classname() <> 'w_contract' Then
	If of_get_hscrollbar(dw_search) Then
		ll_height += 52
	End If
End If
...
//	End If	
	//Modified By Jay Chen 11-21-2014
	MessageBox(gnv_app.iapp_object.DisplayName,"There are no search criteria selected.~r~nPlease select one or more search criteria values to assign a Quick Filter.")
End If
...
If dw_search.rowcount() >= 1 Then
	...
	FOR ll_Num = 1 to ll_Cnt
		...
		Choose Case Lower(Left ( ls_ColType , 3 ))
			...
			CASE 'dat'	
//				ids_search_filter.setItem( ll_row,"data_values",datetime(ls_Value))		//Added By Mark Lee 11/10/2015
				ids_search_filter.setItem( ll_row,"data_values",ls_Value)	
			CASE 'var','cha'							
				ids_search_filter.setItem( ll_row,"data_values",ls_Value)		
		End Choose	
...
Choose Case dwo.Name
	...
	Case 'b_clear'
		...
		//---------------------------- APPEON END ----------------------------
	CASE "b_search"
		// <RP> 7.20.2016 - redirect to ue_search_2()
//		parent.event ue_search_2()  //modified by gavins 20160805
//		return
//	Case 'b_search' //Added By Ken.Guo 10/30/2012. if you modified below script, you should do same changes for ue_powerfilter() event
		ib_force_retrieve = False
		gnv_debug.of_output( False,'cb_3.event clicked(): start.')
		cb_3.event clicked()
		gnv_debug.of_output( False,'cb_3.event clicked(): end.')
		ib_force_retrieve = True
		
		if IsValid(w_SearchPage_Calendar) then w_SearchPage_Calendar.of_RefreshCondition()
		//Added By Mark Lee 12/25/2012
		if IsValid(w_todolist_contract) then 
			gnv_debug.of_output( False,'w_todolist_contract.of_RefreshData(): start.')
			w_todolist_contract.of_RefreshData()
			gnv_debug.of_output( False,'w_todolist_contract.of_RefreshData(): end.')
		end if
		if IsValid(w_tabpg_contract_doc_mgr) then w_tabpg_contract_doc_mgr.of_RefreshData()
		
		gnv_debug.of_output( False,'of_refresh_menu_status(): start.')
		of_refresh_menu_status()			//Added By Mark Lee 12/18/2012	
		gnv_debug.of_output( False,'of_refresh_menu_status(): end.')
...
FOR LL_I = 1 TO LL_COLUMN_COUNT
	IF Describe("#" +STRING(LL_I) + ".Edit.Style") = 'dddw' then
		...
		IF li_value = 1 THEN
			...
			if lb_filter then //Added By Jay Chen 12-07-2015
				dwchild.setfilter("")
				dwchild.filter()
				ls_DataColumn = This.Describe(ls_colname + ".dddw.DataColumn")
				dwchild.SetFilter( "(isnull(" + ls_DataColumn + ") or " + ls_DataColumn + " > 0) and (status = 'A') " )
				dwchild.Filter( )
				lb_filter = false
			else
				ls_DataColumn = This.Describe(ls_colname + ".dddw.DataColumn")
...
//Added By Ken.Guo 2016-03-24. fixed the dddw error when category used error dddwchild
If This.Describe(  "Category.DDDW.Name") <> 'd_dddw_user_access_category' Then
	This.Modify("category.dddw.name='d_dddw_user_access_category' category.dddw.datacolumn='lookup_code' category.dddw.displaycolumn='code'" )
End If
...
dwchild.SetItem(ll_Row,'description','') //Modified By Ken.Guo 2015-05-26. Task #119

ll_Row = ldwc_doc_category.InsertRow(1)
ldwc_doc_category.SetItem(ll_Row,'description','')//Modified By Ken.Guo 2015-05-26. Task #119
...
string ls_display_contract_parent_child_tree
ls_display_contract_parent_child_tree = gnv_user_option.of_Get_option_value( gs_user_id, "Contract_Search_Tab_Display_Contract_Parent_Child_Tree" ) //Added By Jay Chen 06-15-2015
If row > 0 Then
//	Parent.Post Event ue_locate_treeview(This.GetItemNumber(This.GetRow(),'ctx_id'))
	if ls_display_contract_parent_child_tree = '1' then Parent.Event ue_locate_treeview(This.GetItemNumber(This.GetRow(),'ctx_id'))				//Added By Mark Lee 12/28/2012
End If
tv_ctx.SetRedraw(True)
end event
...
//---------- <RP> 9.22.2016 ------------------------------------------------------------
// Set pointer to indicate activity
setPointer(hourglass!)
//-------------------------------------------------------------------------------------------
//--------------------------- APPEON BEGIN ---------------------------
...
//Begin - Added By Mark Lee 03/30/2015.
...
if ll_count > 1 then 
	ls_title =  string(Rowcount())+' Items '+ string(ll_count)+' Selected ' 
else
	ls_title = Trim ( ls_id + ls_company  )
end if 
iw_parent.title = 'Contract Logix ' + ls_title
//gnv_status_info.of_display( Trim ( ls_id + ls_company  ), '1'  )//added by gavins 20130410		
gnv_status_info.of_display( ls_title, '1'  )
//End - Added By Mark Lee 03/30/2015.
//---------------------------- APPEON END ----------------------------
...
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then 
//	This.Title = gs_left_title_sign + 'Contract Browse (' + string(il_last_row) + ')' + gs_right_title_sign
	of_set_dw_title('1','1')
else
...
if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
//	this.title = 'Contract Browse (' + string(il_last_row) + ')'
	of_set_dw_title('1','0')
else
...
//	this.event ue_create_menu_item(lm_Menu)
	...
	//Modified By Ken.Guo 2015-09-18
	//gnv_app.of_modify_menu_attr( lm_Menu.m_createdoc,'enabled', m_pfe_cst_mdi_menu_contract.m_edit.m_newdocument.m_createdoc.visible)
	gnv_app.of_modify_menu_attr( lm_Menu.m_createdoc,'enabled', m_pfe_cst_mdi_menu_contract.m_edit.m_newdocument.m_createdoc.enabled)
...
//Begin - Modified By Mark Lee 09/09/2014 add batch deelet contracts funcation.
////Move to top
...
string ls_ctx_id = "", ls_msg
//Added By Jay Chen 10-22-2014
string ls_select_ctx_id[]  
...
if ll_row > 0 then 
	...
	Do While ll_row > 0
		...
		ls_ctx_id = ls_ctx_id +  string(ll_ctx_id)+ ", "
		ll_FoundRow ++
		ll_row = this.Getselectedrow(ll_row)
	Loop
	ls_ctx_id = left(ls_ctx_id, len(ls_ctx_id) -2 )
	lnv_arrstring.of_parsetoarray( ls_ctx_id, ',', ls_select_ctx_id[]) //Added By Jay Chen 10-22-2014
	if ll_FoundRow = 1  then
		ls_msg = 'Are you sure you want to delete this contract (ID = '+String(ls_ctx_id)+')?~r~n~r~nWARNING! If you proceed, you will delete all data connected to this contract. This operation cannot be undone.'
	else
		ls_msg = 'Are you sure you want to delete these contracts (ID = '+String(ls_ctx_id)+')?~r~n~r~nWARNING! If you proceed, you will delete all data associated with these contracts. This operation cannot be undone.'
	end if
	if messagebox('Delete Contract', ls_msg,Exclamation!,Yesno!,2) = 1 then
		SetPointer(HourGlass!)		
//		for i = this.rowcount() to 1 step -1 
		//Modified By Jay Chen 10-22-2014
		for i = 1 to upperbound(ls_select_ctx_id)
//			if this.IsSelected (i) = true then
//				ll_ctx_id = this.getitemnumber(i,'ctx_id')
				ll_ctx_id = long(ls_select_ctx_id[i])
				ll_find = this.find("ctx_id = " + string(ll_ctx_id) , 1, this.rowcount())
				if isnull(ll_find) then ll_find = 0
				if ll_find = 0 then continue
				If This.Describe( 'record_id.type') = 'column' Then
//					ll_record_id = this.getitemnumber(i,'record_id')  
					ll_record_id = this.getitemnumber(ll_find,'record_id')  
				else
					ll_record_id = 0
				End If
...
//				This.DeleteRow(i)
				...
				if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
					il_last_row --
					if mod(il_last_row,il_perpage_rows) = 0 then
						il_sum_pages = il_last_row / il_perpage_rows
					else
						il_sum_pages = il_last_row / il_perpage_rows + 1
					end if
					This.Title = 'Contract Browse (' + string(il_last_row) + ')'
					delete from ctx_basic_info_pagedata where ctx_id = :ll_ctx_id;
				else
					This.Title = 'Contract Browse (' + string(This.RowCount()) + ')'	
				end if
//				ll_row = i
				ll_row = ll_find
//			end if 
...
//End - Modified By Mark Lee 09/09/2014
...
//if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then
//	This.Title = gs_left_title_sign +  'Contract Browse (' + string(dw_search_det1.rowcount()) + '/'  + string(il_last_row) + ')' + gs_right_title_sign
//else
...
If handle > 0 Then
	If tv_ctx.GetItem(handle,ltvi_1) = 1 Then
		...
		If ll_find <= 0 Then
			if is_display_page = '1' and (AppeonGetClientType() = 'WEB' or is_Type = 'WEB' ) then //Added By Jay Chen 05-15-2015
//			if is_display_page = '1' and is_type = 'WEB' then
				gf_goto_contract_page(ll_ctx_id,'other')
				ll_find = dw_search_det1.Find('ctx_id = ' + String(ll_ctx_id),1,dw_search_det1.Rowcount())
			else
				...
				//Return			
				Messagebox('ContractLogix','This contract '+String(ll_ctx_id)+' is not currently displayed in the Browse screen, it may be filtered out. ~r~nThe system will display this contract in the Browse screen.' )
				ll_find = Parent.of_move_ctx_to_browsedw( ll_ctx_id )
				If ll_find > 0 Then
					This.event selectionchanged( handle, handle)
				Else
					Messagebox('ContractLogix','Failed to display this contract. Please change your filter criteria to display it first.')
					Return ll_find
				End If
			end if
		End If
...
event clicked;//Added By Jay Chen 05-13-2015
string ls_sort_s

if il_sum_pages < 1 then return
//inv_ctx_tv.is_tv_type = 'row'
ids_search.DataObject = 'd_sp_contract_search_web'
ids_search.SetTransObject( sqlca)
il_current_page = 1
dw_search_det1.reset()
ids_search.Retrieve(gs_user_id,'0',il_current_page,il_perpage_rows,gs_session_id,0)
ids_search.RowsCopy(1,ids_search.rowcount(), Primary!, dw_search_det1, 1, Primary!)
ls_sort_s = dw_search_det1.Describe( "datawindow.table.sort")	
if ls_sort_s <> "" and ls_sort_s <> "?" then 
	dw_search_det1.Setsort(  of_replace_sort(ls_sort_s )) 
...
//inv_ctx_tv.is_tv_type = 'row'
il_current_page --
if il_current_page < 1 then il_current_page = 1
dw_search_det1.reset()
ids_search.DataObject = 'd_sp_contract_search_web'
ids_search.SetTransObject( sqlca)
ids_search.Retrieve(gs_user_id,'0',il_current_page,il_perpage_rows,gs_session_id,0)
...
//inv_ctx_tv.is_tv_type = 'row'
il_current_page++
if il_current_page > il_sum_pages then il_current_page = il_sum_pages
dw_search_det1.reset()
ids_search.DataObject = 'd_sp_contract_search_web'
ids_search.SetTransObject( sqlca)
ids_search.Retrieve(gs_user_id,'0',il_current_page,il_perpage_rows,gs_session_id,0)
...
event modified;//Added By Jay Chen 05-13-2015
string ls_sort_s,ls_text,ls_pagenum
long ll_pos
...
ll_pos = pos(ls_text,'/')
if ll_pos > 0 then
	ls_pagenum = left(ls_text,ll_pos - 1)
else
	ls_pagenum = ls_text
end if
if IsNumber(ls_pagenum) then
	if long(ls_pagenum) < 1 then return
else
	If Len( ls_pagenum ) > 0  And not IsNumber( ls_pagenum ) Then 	
		messagebox('Input Error','Please enter a valid number.')
		return 
	end if
end if
il_current_page = long(ls_pagenum)
if il_current_page > il_sum_pages then il_current_page = il_sum_pages
dw_search_det1.reset()
ids_search.DataObject = 'd_sp_contract_search_web'
ids_search.SetTransObject( sqlca)
...
type uo_page from u_display_page within u_tabpg_contract_search
event lbtn_down pbm_lbuttondown
boolean visible = false
integer x = 2491
integer y = 316
integer height = 100
...
event lbtn_down;send(handle(this),274,61458,0) //Added By Jay Chen 03-28-2016
end event

on uo_page.destroy
call u_display_page::destroy
end on
