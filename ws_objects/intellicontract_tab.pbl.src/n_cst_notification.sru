...
n_ds lds_am_action_items //Added By Jay Chen 12-24-2013
string is_col_tab
...
public function integer of_insert_document_action_item (long al_doc_id, long al_lookup_code, string as_user_id, string as_letter_flag, str_alarm astr_alm)
end prototypes
...
FOR ll_i = 1 TO adw.rowcount( )  //all of the alarms	
	 ...
	 //If gnv_data.of_getitem('icred_settings', 'set_10', False) = '1' Then //Debug //Commented By Ken.Guo 2017-02-22
	...
	//End If
	 ...
	 IF ( NOT IsNull(ls_mailto) AND Len(Trim(ls_mailto)) > 0 ) & 
		...
		// determine if it's in the range to alarm	//modified LD_DATE -> Date( ld_date )  by gavins 20151028
		ll_relative_date = inv_datetime.of_relativedatetime( Datetime( Date(LD_DATE), time('00:00:00' ) ) ,  (ll_notificatin_days * SECONDS_IN_A_DAY) * -1 )
...
//						if abs(inv_datetime.of_SecondsAfter(ldt_last_notification_date, ldtServerDate)) >= (ll_frequency * SECONDS_IN_A_DAY)  then
				...
				IF IsNull(ldt_last_wf_trigger) OR date(ldt_last_wf_trigger) = date('1900-01-01') or isnull(ll_total_tries_counter) or ll_total_tries_counter = 0 THEN //Added By Ken.Guo 2017-02-22. added checking ll_total_tries_counter for Essentia Workflows bug.
...
//		select getdate() into :ldtServerDate from ids	using SQLCA;
	CASE 'SQL'
		select getdate() into :ldtServerDate from ids using SQLCA;
END CHOOSE
//Modified By Mark Lee 01/09/2014 gnv_data.of_getItem("icred_settings", "send_range", False) = 'A' and
			...
			IF ll_action_type > 0 THEN
				...
				FOR ll_h = 1 TO UpperBound(ls_users)
					...
					*/
					...
					//Added By Jay Chen 12-24-2013 Remark: insert document record to table ctx_am_action_item
					if lower(ls_table) = 'ctx_am_document'  then  
						SELECT seq_id, attempt_times, action_status INTO :ll_ctx_action_item_id, :ll_no_attempts, :ll_action_status
					  	FROM ctx_am_action_item 
					 	WHERE doc_id = :astr_alm.doc_id AND  alm_column = :ls_column AND action_user = :ls_user_id;
						 
						IF Isnull(ll_ctx_action_item_id) OR ll_ctx_action_item_id = 0 THEN							
							of_insert_document_action_item(astr_alm.doc_id,ll_action_type,ls_user_id,'N', astr_alm) 
						else
							IF Isnull(ll_no_attempts) THEN ll_no_attempts = 0
								ll_no_attempts ++
								Select alm_total_tries_counter Into :ll_alm_count From CTX_all_NOTIFICATION 
								...
								If ll_action_status = ll_complete and ll_alm_count = 0 Then
									...
									Where doc_id = :astr_alm.doc_id AND seq_id = :ll_ctx_action_item_id;														
								Else
									//Update NOTIFICATION 
									Update ctx_am_action_item
										Set attempt_times = :ll_no_attempts, last_attempt = :ldtServerDate
							 		Where doc_id = :astr_alm.doc_id AND action_user = :ls_user_id AND action_type = :ll_action_type AND seq_id = :ll_ctx_action_item_id;															
								end if
						end if
				     //end add
					else
						...
						ELSE
							...
							If ll_action_status = ll_complete and ll_alm_count = 0 Then
								...
								//Modified By Ken.Guo 2016-05-30 added action_date, inserted_at_date_time
...
//Update to DB
IF (lds_action_items.Modifiedcount( ) + lds_action_items.deletedcount( ) > 0) or (lds_am_action_items.Modifiedcount( ) + lds_am_action_items.deletedcount( ) > 0) THEN  //Modified By Jay Chen 12-24-2013
	//--------------------------- APPEON BEGIN ---------------------------
	...
	If lds_action_items.update(False,True) <> 1 or  lds_am_action_items.update(False,True) <> 1 Then
...
FOR i = 1 to ll_cnt
	IF istr_exp[i].export_id = ll_exportid THEN 
		If istr_exp[i].ds_export.find('rec_id = ' + String(astr_alm.ctx_id) , 1, istr_exp[i].ds_export.rowcount() ) > 0 Then //Added By Ken.Guo 02/06/2015. Added if  statement.
			ib_new_export = FALSE
			ll_current_row = i
			EXIT
		End If
...
IF (Upper(astr_alm.alm_table) = 'CTX_BASIC_INFO' or lower(astr_alm.alm_table) = 'ctx_custom_multi_detail' or lower(astr_alm.alm_table) = 'ctx_custom_multi_hdr' or Upper(astr_alm.alm_table) = 'CTX_CUSTOM' or lower(astr_alm.alm_table) = 'ctx_fee_sched_nm' or lower(astr_alm.alm_table) = 'ctx_am_document')  THEN  //Modified By Jay Chen 12-24-2013
...
If (lower(astr_alm.alm_table) = 'ctx_basic_info' or lower(astr_alm.alm_table) = 'ctx_custom' or lower(astr_alm.alm_table) = 'ctx_custom_multi_detail' or lower(astr_alm.alm_table) = 'ctx_custom_multi_hdr' or lower(astr_alm.alm_table) = 'ctx_fee_sched_nm' or lower(astr_alm.alm_table) = 'ctx_am_document') and ib_inserted_action_item = False and ll_total_tries = ll_counter + 1 Then //Modified By Jay Chen 12-24-2013
...
for ll_i = 1 to adw.rowcount( )  //all of the alarms
	...
	//For update action items
	 ...
	 //Added By Ken.Guo 2017-02-21
	 gnv_debug.of_output(false, 'n_cst_notification.of_send_mail(). ctx_id= '+ String( lstr_alm.ctx_id)+ &
	 						', alm_table = ' + lstr_alm.alm_table+', alm_column = '+  lstr_alm.alm_column + &
							 ', wf_id = ' + string(lstr_alm.wf_id)+', alm_times_reminded = '+  gf_String(lstr_alm.alm_times_reminded ) + &
							', wf_trigger = ' + gf_String(adw.getItemNumber(ll_i, 'wf_trigger')) + ', gb_workflow='+String(gb_workflow) )
	 ...
	 IF adw.getItemNumber(ll_i, 'wf_trigger') = 1 and gb_workflow THEN //01.08.2009 By Jervis
		...
		Else
			 //Added By Ken.Guo 2017-02-21
			 gnv_debug.of_output(false, 'n_cst_notification.of_send_mail(). of_workflow_create_action_items<>1')
		END IF
...
Else
	...
	Else
		...
		Else
			ls_err_text = 'Failed to open the email alarm message. doctype <> 1. ' 
			gnv_debug.of_output(true, ls_err_text +  ', CTX ID = ' + String(astr_alm.ctx_id) + ', Field Name = ' +  astr_alm.as_column_name + ',word Length = ' + string(Len( ablb_message )) + '.')
			ll_opened = -2
		END IF	
	End IF
End If
...
Else
	ll_Return = -1
End If
ls_Addresser	= lnv_Mail.of_replace_send_user(ls_Addresser )	//Added By Mark Lee 01/09/2014
...
//Insert to audit table //Modified By Ken.Guo 12/26/2012. Added Level ID1 and level id 2
Insert Into em_mail_audit
(user_id,mail_from,mail_to,mail_cc,mail_subject,mail_date,mail_attach_name,mail_template_id,ctx_id_list,doc_id_list,ai_id_list,level_id1_list,level_id2_list,wf_id,alarm_type,field_name,send_result,error_text,compute_info)
Values 
(:gs_user_id,:ls_Addresser,:ls_Mail_To,:ls_Mail_CC,:ls_subject,getdate(),:ls_attachfilename,:astr_alm.alm_email_template_id,:astr_alm.ctx_id,:astr_alm.doc_id,:astr_alm.ai_id, :astr_alm.level_id1,:astr_alm.level_id2, :astr_alm.wf_id,:ls_alarm_type,:ls_columnname,:li_send_result,:ls_errtext,:ls_computer_info);
...
public function string of_get_alarm_type (string as_alm_table);//Added By Ken.Guo 12/26/2012
String ls_alarm_type
...
public function integer of_insert_document_action_item (long al_doc_id, long al_lookup_code, string as_user_id, string as_letter_flag, str_alarm astr_alm);//====================================================================
// Function: of_insert_document_action_item
//--------------------------------------------------------------------
// Description:Insert records to table ctx_am_action_item
//--------------------------------------------------------------------
// Arguments:
// 	value    long         al_doc_id
// 	value    long         al_lookup_code
// 	value    string       as_user_id
// 	value    string       as_letter_flag
// 	value    str_alarm    astr_alm
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 12-24-2013
//--------------------------------------------------------------------
...
ll_row = lds_am_action_items.insertrow(1)
lds_am_action_items.setitem(ll_row,'doc_id' ,al_doc_id )
lds_am_action_items.setitem(ll_row,'action_type' ,al_lookup_code  )
if f_validstr(as_user_id) then 
   lds_am_action_items.setitem(ll_row,'action_user' ,as_user_id)
end if 
if f_validstr(as_letter_flag) then 
   lds_am_action_items.setitem(ll_row,'wf_complete_flag' ,as_letter_flag)
end if 
lds_am_action_items.setitem(ll_row,'first_attempt' ,date(ldtServerDate))
lds_am_action_items.setitem(ll_row,'last_attempt' ,date(ldtServerDate))
lds_am_action_items.setitem(ll_row,'attempt_times' ,1)
lds_am_action_items.setitem(ll_row,'alm_column', astr_alm.alm_column)
lds_am_action_items.setitem(ll_row,'notes', astr_alm.as_notes ) 
...
IF ll_i > 0 THEN
	...
	lds_am_action_items.setitem(ll_row,'due_date', LD_DATE)
END IF
/* IF ids_alm_status.Rowcount( ) = 1 THEN
	ls_column = ids_alm_status.getItemString(1, 'alm_column')
	LD_DATE = ids_alm_status.GetItemDateTime(1, ls_column)
...
Long ll_action_status
ll_action_status = Long(gnv_data.of_getitem( 'code_lookup', 'lookup_code', "upper(lookup_name) = upper('Contract Action Status') and upper(code) = upper('InComplete')"))
IF ll_action_status > 0 THEN lds_am_action_items.setitem(ll_row,'action_status' ,ll_action_status)

// Set today's date to aciton_date.
lds_am_action_items.setitem(ll_row,'create_date', Datetime(today()))
lds_am_action_items.setitem(ll_row,'status_date', Datetime(today()))
//---------------------------- APPEON END ----------------------------
...
lds_am_action_items = create n_ds
lds_am_action_items.dataobject = 'd_am_action_item_record'
lds_am_action_items.settransobject( sqlca)
