...
//	this.tab_multi.tabpage_header.dw_header.Sharedata( this.tab_multi.tabpage_detail.dw_browse)						//Added By Mark Lee 10/26/2015
	ib_retrieved = false																														
	this.tab_multi.tabpage_detail.of_retrieve( )																							//delete to data for update 
...
event pfc_updateprep;call super::pfc_updateprep;//Added By Ken.Guo 2016-02-16
If gb_workflow Then
	n_cst_workflow_triggers lnv_workflow
	lnv_workflow = create n_cst_workflow_triggers
	il_screen_id[1] = 53 //Header-Header
	is_UpdateCol[il_screen_id[1]] = lnv_workflow.of_getupdatecol( tab_multi.tabpage_header.dw_header )
	il_screen_id[2] = 55 //Detail-Detail
	is_UpdateCol[il_screen_id[2]] = lnv_workflow.of_getupdatecol( tab_multi.tabpage_detail.dw_detail )
	destroy lnv_workflow
end if
...
event selectionchanging;call super::selectionchanging;//Added By Mark Lee 10/26/2015
If  oldindex <> newindex and oldindex <> -1 Then 
	if newindex = 2 and (this.tabpage_header.dw_header.modifiedcount( )  + this.tabpage_header.dw_header.deletedcount( ) > 0 )  then 
		if MessageBox('Save','Changes have been made,  Save changes?',Question!,YesNo!) = 1 then
			this.tabpage_header.of_update( true, true)
		end if
	elseif newindex = 1 and (this.tabpage_detail.dw_browse.deletedcount( )  > 0 or &
			(this.tabpage_detail.dw_detail.modifiedcount( )  + this.tabpage_detail.dw_detail.deletedcount( ) > 0 ) ) then 
		if MessageBox('Save','Changes have been made,  Save changes?',Question!,YesNo!) = 1 then
			if this.tabpage_detail.dw_browse.deletedcount( )  > 0 and &
			(this.tabpage_detail.dw_detail.modifiedcount( )  + this.tabpage_detail.dw_detail.deletedcount( ) > 0 ) then 
				gnv_appeondb.of_startqueue( )
				this.tabpage_detail.dw_browse.event pfc_preupdate( )
				this.tabpage_detail.dw_browse.update( )
				this.tabpage_detail.dw_detail.update( )
				gnv_appeondb.of_commitqueue( )
			elseif this.tabpage_detail.dw_browse.deletedcount( )  > 0 and &
			(this.tabpage_detail.dw_detail.modifiedcount( )  + this.tabpage_detail.dw_detail.deletedcount( ) = 0 ) then  
				this.tabpage_detail.dw_browse.event pfc_preupdate( )
				this.tabpage_detail.dw_browse.update( )
			elseif this.tabpage_detail.dw_browse.deletedcount( )  = 0 and &
...
//			this.tabpage_detail.of_update( true, true)
		end if 
	end if 
end if

if oldindex =  2 and newindex = 1 then 
	this.tabpage_header.of_retrieve( )
elseif oldindex =  1 and newindex = 2 then 
	this.tabpage_detail.of_retrieve( )
end if 
...
event selectionchanged;//Added By Mark Lee 10/27/2015 	overwriter
long 	ll_row, ll_hdr_id, ll_find


if newindex = 2 then
	ll_row = this.tabpage_detail.dw_browse.GetRow( )
	if this.tabpage_header.dw_header.getrow( ) > 0 then 			//Added By Mark Lee 11/24/2015
		ll_hdr_id = this.tabpage_header.dw_header.getitemnumber( this.tabpage_header.dw_header.getrow( ) , "hdr_id")
		if not isnull(ll_hdr_id) and ll_hdr_id > 0 and  this.tabpage_detail.dw_browse.rowcount() > 0 then 
			ll_find = this.tabpage_detail.dw_browse.find( "hdr_id="+string(ll_hdr_id), 1, this.tabpage_detail.dw_browse.rowcount() )
			if not isnull(ll_find) and ll_find > 0 then 
				this.tabpage_detail.dw_browse.scrolltorow(ll_find)
				if ll_row = ll_find  then this.tabpage_detail.dw_browse.event rowfocuschanged(ll_row)
			else
	...
	if this.tabpage_detail.dw_browse.getrow( ) > 0 then 				//Added By Mark Lee 11/24/2015
		ll_hdr_id = this.tabpage_detail.dw_browse.getitemnumber( this.tabpage_detail.dw_browse.getrow( ) , "hdr_id")
		if not isnull(ll_hdr_id) and ll_hdr_id > 0 and  this.tabpage_header.dw_header.rowcount() > 0 then 
			ll_find = this.tabpage_header.dw_header.find( "hdr_id="+string(ll_hdr_id), 1, this.tabpage_header.dw_header.rowcount() )
			if not isnull(ll_find) and ll_find > 0 then 
				this.tabpage_header.dw_header.scrolltorow(ll_find)
			else
