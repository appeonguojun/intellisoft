...
public subroutine of_audit_action_items_status (long al_ctx_id, long al_doc_id, long al_table_id, long al_old_status_value, long al_new_status_value, long al_rec_id)
end prototypes
...
If ls_editstyle='dddw' or ls_editstyle='ddlb' or ls_editstyle='radiobuttons' or edit_codetable = "yes" OR editmask_codetable = "yes" or ls_editstyle = 'checkbox'		then 
				CHOOSE CASE upper(left(adw.Describe(ls_column_name +".COLType"),4))
						CASE 'CHAR','DATE'
							...
							//Added By Mark Lee 01/04/2015 
							if ls_editstyle='dddw' and isnumber(as_old_value) then 
								ls_Values = ''
								 if string(adw.describe("#" + string(al_column)+".name")) = "app_facility" and adw.dataobject = "d_contract_det_master" then
									LL_OLD_VALUE = Integer(as_old_value)
									SELECT facility_name into :ls_Values FROM app_facility where facility_id = :LL_OLD_VALUE;  
								 else
									ls_Values = gnv_data.of_getitem( "code_lookup", "code", "lookup_code = " + String(as_old_value)) 
								end if 
								if not isnull(ls_Values) and trim(ls_Values) <> ""then
									as_old_value = ls_Values    	
								end if 
							end if 
						...
						CASE ELSE  
							LL_OLD_VALUE = adw.getitemNumber(al_rows,al_column,PRIMARY!,true)
							 ...
							 if string(adw.describe("#" + string(al_column)+".name")) = "app_facility" and adw.dataobject = "d_contract_det_master" then
								...
								If LL_OLD_VALUE < 20 Then //Added By Ken.Guo 2015-06-01. for ddlb, checkbox
									as_old_value =  String(LL_OLD_VALUE)  //Added By Ken.Guo 2015-06-01 
								Else
...
Choose Case as_new_value
	CASE 'DELETED'
		...
		for ll_i = 1 to ll_deleted_count
			...
			elseif lb_ctx_am_action then	//For ctx_am_action_items - jervis 11.10.2010
				...
				if isnull(ll_record_id) then ll_record_id = 0
			Else
				if il_table_id = 1066 then  //Added By Jay Chen 06-25-2014 remark: for ctx_custom_multi_hdr
					ll_record_id = ads.GetItemNumber( ll_i, "hdr_id",Delete!,true )
					if isnull(ll_record_id) then ll_record_id = 0
				elseif il_table_id = 1067 then  //Added By Jay Chen 06-25-2014 remark: for ctx_custom_multi_detail
					ll_record_id = ads.GetItemNumber( ll_i, "detail_id",Delete!,true )
					if isnull(ll_record_id) then ll_record_id = 0
				else
					ll_record_id = 0
				end if
	...
	CASE 'INSERT'
		...
		Else
			if il_table_id = 1066 then  //Added By Jay Chen 06-25-2014 remark: for ctx_custom_multi_hdr
				ll = ads.GetItemNumber( al_current_row, "hdr_id" )
				if isnull(ll) then ll = 0
			elseif il_table_id = 1067 then  //Added By Jay Chen 06-25-2014 remark: for ctx_custom_multi_detail
				ll = ads.GetItemNumber( al_current_row, "detail_id" )
				if isnull(ll) then ll = 0
			else
				ll = 0
			end if
	...
	CASE ELSE //'EDIT'
		...
		Else
			if il_table_id = 1066 then  //Added By Jay Chen 06-25-2014 remark: for ctx_custom_multi_hdr
				ll_id2 = ads.GetItemNumber( al_current_row, "hdr_id" )
				if isnull(ll_id2) then ll_id2 = 0
			elseif il_table_id = 1067 then  //Added By Jay Chen 06-25-2014 remark: for ctx_custom_multi_detail
				ll_id2 = ads.GetItemNumber( al_current_row, "detail_id" )
				if isnull(ll_id2) then ll_id2 = 0
			else
				ll_id2 = 0
			end if
...
public subroutine of_audit_action_items_status (long al_ctx_id, long al_doc_id, long al_table_id, long al_old_status_value, long al_new_status_value, long al_rec_id);//====================================================================
// Function: of_audit_action_items_status
//--------------------------------------------------------------------
// Description: Audit Action Items Status change
//--------------------------------------------------------------------
// Arguments:
//                al_ctx_id
//                al_doc_id
...
Long ll_seq_no, ll_field_id
String ls_field_name_alias 

gnv_appeondb.of_startqueue( )
	Select code into :ls_old_status from code_lookup where lookup_code = :al_old_status_value;
	Select code into :ls_new_status from code_lookup where lookup_code = :al_new_status_value;
	Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :al_ctx_id  and table_id = :al_table_id;
gnv_appeondb.of_commitqueue( )

Choose Case al_table_id 
	Case 1021  //ctx_action_item
		ls_field_name_alias = 'Status'
		ll_field_id = 20118
		setnull(al_doc_id)
	Case 1064  //ctx_am_action_item
		ls_field_name_alias = 'action status'
		ll_field_id = 200805	
End Choose

Insert into sys_audit(rec_id, field_id, seq_no, table_id, prac_id, old_value, new_value, user_id, date_time_modified, audit_type, exported, export_date, facility_id, tfield_name, tfield_name_allias,sys_notification_pid,sql_syntax)  
Values(:al_rec_id, :ll_field_id,:ll_seq_no,:al_table_id,:al_ctx_id,:ls_old_status,:ls_new_status,:gs_user_id,getdate(),'E', 0, null,:al_doc_id,'action_status',:ls_field_name_alias,null,null);

If sqlca.sqlcode <> 0 Then
	gnv_debug.of_output(true,'Failed to audit the action item status data.' + 'CTX ID = ' + String(al_ctx_id) + ', table_id = ' + String(al_table_id) + ', rec_id = ' + String(al_rec_id))
End If
...
lds_sys_audit.DataObject = "ds_view_audit_save"
