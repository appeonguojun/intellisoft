...
string ls_col_visible,ls_bcol_visible,ls_bcol_name,ls_modify
...
ls_customscreen = {'custom_date1','custom_date2','custom_date3','custom_date4','custom_date5','start_date','end_date'} //Added By Jay Chen 12-04-2013

ll_colcount = UpperBound(ls_customscreen)

//Added By Jay Chen 06-26-2014 remark:check start_date and end_date object visible
for ll_k = 1 to ll_colCount
	if ls_customscreen[ll_k] <> 'start_date' and ls_customscreen[ll_k] <> 'end_date' then continue
	ls_bcol_name = 'b_' + ls_customscreen[ll_k]
	ls_col_visible = adw_alm.describe( ls_customscreen[ll_k] +".visible")
	ls_bcol_visible = adw_alm.describe(ls_bcol_name +".visible")
	if ls_col_visible = '0' and ls_bcol_visible <> '0' then
		ls_modify += " " + ls_bcol_name + ".visible='0' "
	end if
next
if len(trim(ls_modify)) > 0 then adw_alm.modify(ls_modify)
//end
...
public subroutine of_post_goto_row (long al_fee_nm_id, string as_column);This.Post of_goto_row( al_fee_nm_id, as_column)
end subroutine
...
//SetRowFocusIndicator(Hand!) //Commented By Jay Chen 03-18-2016 V15_Product_Fixes_Enhancements: Remove “Hand” in Fee Schedules NM Tab
