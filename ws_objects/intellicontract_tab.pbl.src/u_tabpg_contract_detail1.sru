...
string is_prior_col //Added By Ken.Guo 2015-11-26
Long il_originalY// added by gavins 20151126
end variables
...
public subroutine of_auto_add_contact_user ()
public function integer of_set_contract_status_color ()
public function integer of_display_multi_company ()
end prototypes
...
Long ll_CusRight,ll_NotesRight,ll_AuditRight
...
//$<reason> Fix a defect.
long ll_ctx_id
long ll_selecttab //Added By Jay Chen 07-21-2014
...
//ll_selecttab = tab_1.selectedtab //Added By Jay Chen 07-21-2014  //Commented By Ken.Guo 2016-03-29. move to down
...
//			gnv_appeondb.of_startqueue( )
		   	This.dw_1.event pfc_retrieve( )
			gnv_appeondb.of_startqueue( )  //Modified By Jay Chen 04-29-2015
			This.dw_ccc.event pfc_retrieve( )
			This.dw_occ.event pfc_retrieve( )
			This.dw_products.event pfc_retrieve( )
			gnv_appeondb.of_commitqueue( )
	...
	//---------------------------- APPEON END ----------------------------
Else
	ll_selecttab = tab_1.selectedtab //Added By Ken.Guo 2016-03-29 copy from top. need not remember the tabpage after retrieve the data again. it depend on the tab's default selected properties
end if
...
gnv_debug.of_output( False,'of_RefreshTab(): start.')
of_RefreshTab()					//Added by Scofield on 2008-05-13
gnv_debug.of_output( False,'of_RefreshTab(): end.')

//Added By Jay Chen 07-21-2014  remark:back setfocus to original tabpage after refresh tabpage data
if ll_selecttab > 1 then
	if isvalid(gw_contract) then
		if gw_contract.tab_contract_details.selectedtab = 2 then
			tab_1.selectedtab = ll_selecttab
		end if
	end if
end if
//end add
...
gnv_debug.of_output( False,'of_change_screen: start.') //Added By Jay Chen 12-16-2015
dw_1.accepttext()
...
If li_view_id <> gi_contract_view_id Then
	...
	if isvalid(gw_contract) then
		...
		gnv_debug.of_output( False,'ue_set_tabpage_properties: start.') 
		gw_contract.TriggerEvent("ue_set_tabpage_properties")
		gnv_debug.of_output( False,'ue_set_tabpage_properties: end.') 
	end if
	
	gnv_debug.of_output( False,'wf_tab_visible: start.') 
	iw_parent.function dynamic wf_tab_visible(gi_contract_view_id) 
	gnv_debug.of_output( False,'wf_tab_visible: end.') 
	gnv_debug.of_output( False,'wf_set_select: start.')
	iw_parent.function dynamic wf_set_select('All',false) 
	gnv_debug.of_output( False,'wf_set_select: end.') 
...
//	iw_parent.function dynamic of_select_details_tab()
	If w_contract.tab_contract_details.tabpage_details.visible = False Then 
		If ib_new = False Then
			gnv_debug.of_output( False,'of_select_details_tab: start.') 
			iw_parent.function dynamic of_select_details_tab()
			gnv_debug.of_output( False,'of_select_details_tab: end.') 
		End If
		Return
	End If
	//---------------------------- APPEON END ----------------------------
	gnv_debug.of_output( False,'of_create_dw: start.') 
	of_create_dw()
	gnv_debug.of_output( False,'of_create_dw: end.')
	gnv_debug.of_output( False,'of_create_custom_dw: start.')
	of_create_custom_dw()  //add by jervis 03.12.2011
	gnv_debug.of_output( False,'of_create_custom_dw: end.')
	dw_1.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'dw_1.event ue_populatedddws( ): start.') 
	dw_1.event ue_populatedddws( )
	gnv_debug.of_output( False,'dw_1.event ue_populatedddws( ): end.') 
	dw_contract_dates.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'dw_contract_dates.event ue_populatedddws( ): start.') 
	dw_contract_dates.event ue_populatedddws( )
	gnv_debug.of_output( False,'dw_contract_dates.event ue_populatedddws( ): end.') 
	dw_ccc.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'dw_ccc.event ue_populatedddws( ): start.') 
	dw_ccc.event ue_populatedddws( )
	gnv_debug.of_output( False,'dw_ccc.event ue_populatedddws( ): end.') 
	dw_occ.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'dw_occ.event ue_populatedddws( ): start.') 
	dw_occ.event ue_populatedddws( )
	gnv_debug.of_output( False,'dw_occ.event ue_populatedddws( ): end.') 
	dw_products.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'dw_products.event ue_populatedddws( ): start.') 
	dw_products.event ue_populatedddws( )
	gnv_debug.of_output( False,'dw_products.event ue_populatedddws( ): end.') 
	dw_products.ib_retrieve_drop_downs = false
	dw_products.event ue_populatedddws( )
	
	tab_1.tabpage_notes.dw_1.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'tab_1.tabpage_notes.dw_1.event ue_populatedddws( ): start.')
	tab_1.tabpage_notes.dw_1.event ue_populatedddws( )
	gnv_debug.of_output( False,'tab_1.tabpage_notes.dw_1.event ue_populatedddws( ): end.')
	tab_1.tabpage_custom.dw_1.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'tab_1.tabpage_custom.dw_1.event ue_populatedddws( ): start.')
	tab_1.tabpage_custom.dw_1.event ue_populatedddws( )
	gnv_debug.of_output( False,'tab_1.tabpage_custom.dw_1.event ue_populatedddws( ): end.')
	tab_1.tabpage_1.dw_1.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'tab_1.tabpage_1.dw_1.event ue_populatedddws( ): start.')
	tab_1.tabpage_1.dw_1.event ue_populatedddws( )
	gnv_debug.of_output( False,'tab_1.tabpage_1.dw_1.event ue_populatedddws( ): end.')
	//--------------------------- APPEON BEGIN ---------------------------
	//$<add> 03.14.2007 By: Jack (Contract)
	//$<reason> Fix a defect.
	tab_1.tabpage_2.dw_1.ib_retrieve_drop_downs = false
	gnv_debug.of_output( False,'tab_1.tabpage_2.dw_1.event ue_populatedddws( ): start.')
	tab_1.tabpage_2.dw_1.event ue_populatedddws( )
	gnv_debug.of_output( False,'tab_1.tabpage_2.dw_1.event ue_populatedddws( ): end.')
...
gnv_debug.of_output( False,'Event ue_set_tabpage_properties(): start.')			
Event ue_set_tabpage_properties() //Added By Ken.Guo 09/27/2011. 
gnv_debug.of_output( False,'Event ue_set_tabpage_properties(): end.')	

gnv_debug.of_output( False,'of_change_screen: end.')
end subroutine
...
// $<add> 08.22.2006 by Liang QingShi
//////////////////////////////////////////////////////////////////////
long		ll_CusRight,ll_NotesRight,ll_AuditRight,ll_TrackRight
  ...
  //Modified by Nova on 2008-08-14
	CHOOSE CASE Lower ( Left ( adw.Describe ( ls_column_name + ".ColType" ) , 5 ) )
		...
		CASE "datet"		//  DATETIME DATATYPE
			Choose Case lower(ls_str) 
				Case 'today','now'	//Added By Ken.Guo 2010-10-14.
					adw.SetItem ( 1, ls_column_name, DateTime(today(), now()))	 //Added By Ken.Guo 2010-10-14. //modified by gavins add now()
				Case Else
...
li_return = lnv_workflow.of_trigger_for_ctx( ll_ctx_id, ll_category , ll_status_id )
...
If isvalid( gw_contract) and ib_new and ab_detail Then //Modified By Ken.Guo 04/01/2014. when user added new contact, Need not check the required columns.
...
String	ls_LastName,ls_FirstName,ls_MidName,ls_Userd,ls_Phone,ls_EMail, ls_ext, ls_facility_name
...
if Not IsNull(ll_ConId) and ll_ConId > 0 then
	...
	if ll_Find <= 0 then
		...
		dw_ccc.SetItem(ll_Row,'facility_name',ls_facility_name)				//Added By Mark Lee 01/21/2015 
...
Long li_tabid //Added By Ken.Guo 2016-03-29

tab_1.Visible = true
cb_Max.Visible = true		 

if tab_1.tabpage_custom.Visible then
	//tab_1.SelectTab(1) //Modified By Ken.Guo 2016-03-29
	li_tabid = 1 
...
elseif tab_1.tabpage_2.Visible then
	//tab_1.SelectTab(4)
	li_tabid = 4 
elseif tab_1.tabpage_group_access.visible then //Added By Jay Chen 02-02-2015
	//tab_1.SelectTab(5)
	li_tabid = 5
else
	tab_1.Visible = false
...
//Added By Ken.Guo 2016-03-29
String ls_default_selected
ls_default_selected = gnv_user_option.of_get_viewid_option_value( gi_contract_view_id, 'View_Tab_Details_Default_Selected_TabID' , FALSE ) //Added By Ken.Guo 2016-03-28
If isnumber(ls_default_selected) Then
	If tab_1.control[long(ls_default_selected)].visible Then
		Tab_1.selecttab(Long(ls_default_selected))
	ElseIf li_tabid > 0 Then
		tab_1.SelectTab(li_tabid)
	End If
ElseIf li_tabid > 0 Then
	tab_1.SelectTab(li_tabid)
End If
...
if isvalid(tab_1.tabpage_1.dw_1.iuo_select) then
	tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.reset()
end if 
if isvalid(tab_1.tabpage_custom.dw_1.iuo_select) then
	tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.reset()
end if 
...
cb_max.x = tab_1.x + tab_1.width - cb_max.width - 150 //Modified By Jay Chen 01-20-2016
IF cb_max.text = "-" THEN
	cb_max.text = '+'
End IF 
...
public subroutine of_auto_add_contact_user ();//====================================================================
// Function: of_auto_add_contact_user
//--------------------------------------------------------------------
// Description: Auto add the login user as the owner when creating a new contract
//--------------------------------------------------------------------
// Arguments:
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 10-28-2013
//--------------------------------------------------------------------
...
long ll_row,ll_found,ll_cnt,i
datastore lds_contact_user
str_contact lstr_contact
boolean lb_auto_add_owner = true
string ls_default_email,ls_owner,ls_userid

//1. check our company contact is or not exist owner
for i = 1 to dw_occ.rowcount( )
	ls_default_email = dw_occ.getitemstring(i,"default_email")
	ls_owner = dw_occ.getitemstring(i,"ctx_contract_contacts_owner")
	ls_userid = dw_occ.getitemstring(i,"user_d")
	if Upper(ls_default_email) = 'Y' or Upper(ls_owner) = 'Y' or Upper(ls_userid) = Upper(gs_user_id)  then
		lb_auto_add_owner = false
		exit
	end if
next

//2.if our company contact is not exist owner, auto add the login user as the owner
if lb_auto_add_owner then
	...
	//Added By Jay Chen 08-08-2014
	lds_contact_user.setfilter("contact_type2 = 'A'")
	lds_contact_user.filter()
	//end
	ll_found = lds_contact_user.find("Upper(user_d) = '"+Upper(gs_user_id)+"'",1, lds_contact_user.rowcount())
	if ll_found <=0 then return
	lstr_contact.as_last_name = lds_contact_user.object.last_name[ll_found]
	lstr_contact.as_first_name = lds_contact_user.object.first_name[ll_found]
	lstr_contact.as_middle_name = lds_contact_user.object.middle_name[ll_found]
	...
	lstr_contact.as_phone_number = lds_contact_user.object.contact_phone[ll_found]
	lstr_contact.as_contact_title = lds_contact_user.object.contact_title[ll_found] 
	lstr_contact.al_contact_department = lds_contact_user.object.contact_department[ll_found] 
	lstr_contact.as_email = lds_contact_user.object.contact_email[ll_found] 
	ll_row = dw_occ.insertrow(0)
	dw_occ.object.last_name[ll_row] = lstr_contact.as_last_name
	dw_occ.object.first_name[ll_row] = lstr_contact.as_first_name
	dw_occ.object.middle_name[ll_row] = lstr_contact.as_middle_name
	dw_occ.object.contact_id[ll_row] = lstr_contact.al_contact_id
	dw_occ.object.contact_type[ll_row] = lstr_contact.al_contact_type
	If dw_occ.Describe("association.type") = 'column' Then 
		dw_occ.object.association[ll_row] = String(lstr_contact.al_contact_type)
	End If
	If dw_occ.Describe("contact_phone.type") = 'column' Then
		dw_occ.object.contact_phone[ll_row] = lstr_contact.as_phone_number
	End If
	If dw_occ.Describe("user_d.type") = 'column' Then	
		dw_occ.object.user_d[ll_row] = lstr_contact.as_user_id
	end if
	If dw_occ.Describe("contact_email.type") = 'column' Then	
		dw_occ.object.contact_email[ll_row] = lstr_contact.as_email
	End If
	If dw_occ.Describe("contact_title.type") = 'column' Then	
		dw_occ.object.contact_title[ll_row] = lstr_contact.as_contact_title
	end if
	If dw_occ.Describe("contact_department.type") = 'column' Then	
		dw_occ.object.contact_department[ll_row] = lstr_contact.al_contact_department
	end if
	dw_occ.object.default_email[ll_row] = 'Y'
	dw_occ.object.ctx_contract_contacts_owner[ll_row] = 'Y'
end if
...
public function integer of_set_contract_status_color ();//====================================================================
// Function: of_set_contract_status_color
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:         Ken.Guo           Date: 02/05/2015
//--------------------------------------------------------------------
...
Long ll_noaccess_status_code, ll_cnt,i
String ls_noaccess_status_list
DatawindowChild ldwc_status
n_ds ids_noaccess
dw_1.GetChild('status', ldwc_status)
If Not isvalid(ldwc_status) Then Return 0

If isvalid(gw_contract) Then
ids_noaccess = gw_contract.tab_contract_details.tabpage_search.ids_contract_status_noaccess
ids_noaccess.Retrieve(gs_user_id)
If ids_noaccess.rowcount() = 0 Then
	ldwc_status.Modify("code.Color='0'")
	ldwc_status.Modify("description.Color='0'")
Else
	For i = 1 to ids_noaccess.RowCount()
		ll_noaccess_status_code = ids_noaccess.GetItemNumber(i, 'category')
		If ll_noaccess_status_code <> -777 and Not isnull(ll_noaccess_status_code) Then
			ls_noaccess_status_list += String(ll_noaccess_status_code) + ','
		End If
	Next
	ls_noaccess_status_list = Left(ls_noaccess_status_list, Len(ls_noaccess_status_list) - 1)
	ldwc_status.Modify("code.Color='0~tIf (lookup_code in (" + ls_noaccess_status_list + "), rgb(255,0,0), rgb(0,0,0))'")
	ldwc_status.Modify("description.Color='0~tIf (lookup_code in (" + ls_noaccess_status_list + "), rgb(255,0,0), rgb(0,0,0))'")
End If

End If
end function

public function integer of_display_multi_company ();//Added By Jay Chen 01-18-2016 workaround APB BUG: cannot display company assemble
long ll_row,ll_column_count,ll_null
string ls_dddw_name,ls_colname,ls_coltype,ls_style,ls_value,ls_tag
datawindowchild dwchild
if AppeonGetClientType() = 'WEB' then
	ll_column_count = long(tab_1.tabpage_custom.dw_1.object.datawindow.column.count)
	FOR ll_row = 1 TO ll_column_count
		ls_colname = tab_1.tabpage_custom.dw_1.Describe("#" +string(ll_row) + ".Name")
		ls_coltype = tab_1.tabpage_custom.dw_1.Describe(ls_colname+".coltype")
		ls_style = tab_1.tabpage_custom.dw_1.Describe(ls_colname+".edit.style")
		if ls_style = 'dddw' and left(ls_coltype,4) = 'char' then
			ls_dddw_name = lower(tab_1.tabpage_custom.dw_1.Describe("#" +string(ll_row) + ".DDDW.Name"))
			ls_tag = tab_1.tabpage_custom.dw_1.describe(ls_colname +".tag")
			ls_value = tab_1.tabpage_custom.dw_1.getitemstring(1, ls_colname)
			if ls_dddw_name = "d_dddw_company" or ls_dddw_name = "d_dddw_company_list" or ls_dddw_name = 'dddw_contact_company_new' then 
				if long(ls_value) = 0 then //data value is not single company
					tab_1.tabpage_custom.dw_1.getchild(ls_colname, dwchild)
...
//  string ls_syntax, ls_temp,ls_multi_select
//	ll_viewid = tab_1.tabpage_custom.dw_1.il_viewid
//	ll_screenid = tab_1.tabpage_custom.dw_1.il_screenid
//	if(ll_screenid = 9 ) then
//		FOR ll_row = 1 TO ll_column_count
//			ls_colname = tab_1.tabpage_custom.dw_1.Describe("#" +string(ll_row) + ".Name")
//			ls_coltype = tab_1.tabpage_custom.dw_1.Describe(ls_colname+".coltype")
//			ls_style = tab_1.tabpage_custom.dw_1.Describe(ls_colname+".edit.style")
//			if ls_style = 'dddw' and left(lower(ls_coltype),4) = 'char' and left (lower(ls_colname),7) = 'custom_' then
//					ls_multi_select = "";
//					select object_format into :ls_multi_select from screen_field_select_item_attribute where view_id = :ll_viewid and screen_id = :ll_screenid and object_name = :ls_colname and object_type = 'multi-select'; 
...
//						ls_temp = ""
//						
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.name")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.name='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.displaycolumn")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.displaycolumn='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.datacolumn")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.datacolumn='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.allowedit")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.allowedit='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.useasborder")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.useasborder='" + ls_temp + "' "
//						
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.autohscroll")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.autohscroll='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.autoretrieve")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.autoretrieve='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.hscrollbar")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.hscrollbar='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.hsplitscroll")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.hsplitscroll='" + ls_temp + "' "
//						
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.imemode")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.imemode='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.limit")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.limit='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.nilisnull")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.nilisnull='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.percentwidth")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.percentwidth='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.required")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.required='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.showlist")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.showlist='" + ls_temp + "' "
//						ls_temp = tab_1.tabpage_custom.dw_1.Describe(ls_colname + ".dddw.vscrollbar")
//						ls_syntax = ls_syntax + ls_colname + ".dddw.vscrollbar='" + ls_temp + "' "
//						
//						If  tab_1.tabpage_custom.dw_1.of_set_multi_select(ls_colname,ll_viewid,ll_screenid, ls_syntax)= 1 then	
//							If (ll_screenid = 9) then
...
//								tab_1.tabpage_custom.dw_1.Modify ( ls_syntax )
//							End If
//						End If
//						ls_syntax= ""
//					End if
...
if dw_contract_dates.getrow() > 0 then
	...
	dw_contract_dates.setitem(dw_contract_dates.getrow(),'version_date', datetime(today(), now()))//modified by gavins 20151028  add now()
end if
ib_new = true
is_version_number = ''
...
//Added By Ken.Guo 2017-04-24. //Task 727:Default contacts are not added to new contracts
If dw_1.rowcount() = 1 Then
	dw_1.event itemchanged( 1, dw_1.object.app_facility, string(dw_1.getitemnumber(1, 'app_facility'))  )
End If
...
FOR li_index = 1 TO li_count 
   lst_temp = lwo_temp[li_index] 
	...
	//--------End Modified -------------------------------------
		...
		//Added By Jay Chen 04-02-2015
		if lst_temp = tab_1.tabpage_custom.dw_1 then
			if w_mdi.of_security_access(2091) = 1 then
				lst_temp.of_disable_columns( )
				continue
			end if
		end if
		if lst_temp = tab_1.tabpage_1.dw_1 then
		...
		//Added By Ken.Guo 2015-08-07. Control Notes and Group dw.
		if lst_temp = tab_1.tabpage_notes.dw_1 then
			if w_mdi.of_security_access(2092) = 1 then
				lst_temp.of_disable_columns( )
				continue
			end if
		end if
		if lst_temp = tab_1.tabpage_group_access.dw_1 then
...
//if ib_new_record or w_contract.ib_use_template then //Modified By Jay Chen 01-22-2014
if ib_new_record  then 
	IF of_create_action_item_by_workflow() = 1 Then
		
			//Added By Ken.Guo 2015-06-24	
			If isvalid(gw_contract) Then
//				gw_contract.tab_contract_details.Post of_OpenCtxWorkFlowStep(0,gw_contract.tab_contract_details.selectedtab) //Modified By Jay Chen 08-21-2015
				gw_contract.tab_contract_details.of_OpenCtxWorkFlowStep(0,gw_contract.tab_contract_details.selectedtab)
			End If		
	End If
...
ib_new_record = false
if isValid(w_contract) then
	w_contract.ib_use_template = false //Added By Jay Chen 01-22-2014
end if 
...
//if isvalid(iw_parent) then 			//Added By Mark Lee 05/22/2015  
//	if iw_parent.classname( ) = "w_contract" then 
////		iw_parent.tab_contract_details.FUNCTION DYNAMIC of_OpenCtxWorkFlowStep(0,2)	
//		powerobject		lpo_parent
//		//Loop getting the parent of the object until it is of type window!
//		lpo_parent = this.GetParent()
//		do while IsValid (lpo_parent)
//			//and lower(string(lpo_parent.TypeOf())) = userobject! 
//			if lpo_parent.classname( ) = "tab_contract_details" then Exit	
//			lpo_parent = lpo_parent.GetParent()
//		loop
//		
//		if IsValid (lpo_parent) then
//			lpo_parent.FUNCTION DYNAMIC of_OpenCtxWorkFlowStep(0,2)
//		end If
//	end if 
//end if 

//Added by harry 20170502 //Modified By Ken.Guo 2017-07-17. adde sync_category list
Long ll_count
String ls_sync_category //Added By Ken.Guo 2017-07-17
long ll_category

gnv_appeondb.of_startqueue( )
	select Isnull(count(*), 0) into :ll_count from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and a.auto_sync_salesforce = 1 and b.isoft_table_id = 1001;
	select Top 1 sync_category  into :ls_sync_category from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and a.auto_sync_salesforce = 1 and b.isoft_table_id = 1001;
gnv_appeondb.of_commitqueue( )

ll_category = dw_1.getitemnumber(dw_1.getrow(), 'category') //Added By Ken.Guo 2017-07-17
if ll_count > 0 then
	if isValid(w_contract) then
		If ls_sync_category = 'all' or pos(ls_sync_category + ',' ,  String(ll_category) + ',' ) > 0 Then //Added By Ken.Guo 2017-07-17. Added If statement
			w_contract.ib_auto_sync_sf = true
			w_contract.trigger event ue_sync_to_salesforce()
			w_contract.ib_auto_sync_sf = false
		End If 
	end if
end if
...
long ll_i, 	ll_ctx_id,ll_contact_id
...
if isnull(DW_1.object.ctx_id[1]) then Return FAILURE //Added By Jay Chen 08-22-2014

//Added By Jay Chen 10-30-2015
is_filter_company_contact = gnv_data.of_getitem("icred_settings", "filter_company_contact", false)
if is_filter_company_contact = '0' then
	for ll_i = 1 to this.rowcount( )
		ll_contact_id = this.object.contact_id[ll_i]		
		update ctx_contacts set status = 'A' where contact_id = :ll_contact_id;
	next
end if
//end
...
if newindex = 1 then 
	...
	//Added By Jay Chen 01-18-2016 workaround APB BUG: cannot display company assemble
	if tab_1.tabpage_custom.dw_1.rowcount() > 0 and AppeonGetClientType() = 'WEB' then
		of_display_multi_company()
	end if
	
	//Commented By Ken.Guo 2017-03-1. need not set ib_refresh_multi to true.
	//	if isvalid(tab_1.tabpage_1.dw_1.iuo_select) then
	//		tab_1.tabpage_1.dw_1.iuo_select.ib_refresh_multi = true
	//	end if
	//	if isvalid(tab_1.tabpage_custom.dw_1.iuo_select) then
	////		tab_1.tabpage_1.dw_1.iuo_select.ib_refresh_multi = true //Modified By Jay Chen 02-15-2016
	//		tab_1.tabpage_custom.dw_1.iuo_select.ib_refresh_multi = true
	//	end if
...
string ls_string,ls_required_rows_flag //Added By Jay Chen 07-29-2015
//--------------------------- APPEON BEGIN ---------------------------
...
long ll_rtn 
if this.visible then //Added By Jay Chen 11-30-2015
	ll_rtn = gf_validate_required_rows(this.il_viewid,this.il_screenid,this.rowcount())
...
if isvalid(iw_parent) then
	...
	ls_title = 'Contract Logix Product: '
end if
...
//Added By Jay Chen 07-29-2015
ls_string = string(this.il_viewid) + '-' + string(this.il_screenid)
...
for ll_i = 1 to this.rowcount()	
	if ls_required_rows_flag <> '1' then  //Added By Jay Chen 07-29-2015 need to valid product name and type when add blank row in new mode
...
//Added By Ken.Guo 2016-08-17. autofield data for sharedataed dw
 dw_contract_dates.Post of_auto_compute_field( 0,'R')
 tab_1.tabpage_custom.dw_1.Post of_auto_compute_field(0,'R') 
end event
...
if isvalid(gw_contract) then //Added By Jay Chen 11-04-2014
	gw_contract.tab_contract_details.tabpage_search.ib_get_masterchild = true
end if
...
If isnull(ldt_version_date) Then
	dw_1.setitem(1,'version_date',datetime(today(),now())) //modified by gavins 20151028  add now()
End If
//---------------------------- APPEON END ----------------------------
...
if rowcount( ) > 0 THEN
	if not f_validstr(string( dw_1.object.ctx_id[1] )) then
		...
		tab_1.tabpage_group_access.il_ctx_id = inv_contract_details.of_get_ctx_id( )	//Added By Jay Chen 02-02-2015
		//--------------------------- APPEON BEGIN ---------------------------
	...
	if il_app_facility > 0 then
		...
		is_filter_company_contact = gnv_data.of_getitem("icred_settings", "filter_company_contact", false)
		if is_filter_company_contact = '0' then
			update app_facility set status = 'A' where facility_id = :il_app_facility; 
		end if
		//end
	end if
	//--------------------------- APPEON BEGIN ---------------------------
   ...
   case 'b_contract_search'
	...
	Case 'b_app_facility_dotdd_company'
	   lstr_pass.s_string = 'Select_Company'
  	   ls_Company =  this.describe("Evaluate( 'lookupdisplay(app_facility) ', "+string(1)+" )")
	   lstr_pass.l_facility_id =Long(this.object.app_facility[1]) //Added By Jay Chen 01-07-2015
	   //Added By Jay Chen 06-30-2015 
	   string ls_company_name,ls_displaycolumn
	   ls_displaycolumn = dw_1.Describe("app_facility.dddw.displaycolumn")
	   if left(lower(ls_displaycolumn),8) = 'compute_'	then
		   select facility_name into :ls_company_name from app_facility where facility_id = :lstr_pass.l_facility_id;
	   	   if isnull(ls_company_name) then ls_company_name = ''
	   	   if ls_Company <> ls_company_name then ls_company = ls_company_name	
	   end if
	   //end
...
//				if w_contract.tab_contract_details.tabpage_search.dw_search.getchild("facility_name",ldwc_search) = 1 then
//					ls_facility_name = gnv_data.of_GetItem( "ctx_facility", "facility_name", "facility_id = " + string(il_app_facility))
//					ll_find = ldwc_search.find("facility_name = '"+ls_facility_name+"'",1,ldwc_search.rowcount())
//					if isnull(ll_find) then ll_find = 0
//					if ll_find < 1 then
//						ll_row = ldwc_search.InsertRow( 0)
//						ldwc_search.SetItem( ll_row,"facility_id", il_app_facility)
//						ldwc_search.SetItem( ll_row,"facility_name", ls_facility_name)
//					end if
//				end if
//			end if
//			//end add
			...
			of_AutoDataFill(il_ViewId,il_ScreenId,'app_facility',row,String(il_app_facility) ) //Added By Ken.Guo 2015-11-27	
...
choose case dwo.name 
	...
	case 'status' 
		...
		//Begin - Modified By Ken.Guo 02/05/2015
		//<$Reason>Check contract access right to the current contract status 
		IF Not IsNull(data) and data <> '-777' THEN
			IF IsValid(gw_contract) THEN 
				//Parent.of_set_contract_status_color( )
				//If gw_contract.tab_contract_details.tabpage_search.ids_contract_status_noaccess.Retrieve(gs_user_id)	 > 0 Then
					If gw_contract.tab_contract_details.tabpage_search.ids_contract_status_noaccess.Find("category = " + data,1, gw_contract.tab_contract_details.tabpage_search.ids_contract_status_noaccess.rowcount()) > 0 Then
						MessageBox(gnv_app.iapp_object.DisplayName,"You don't have permission for this contract status. ~r~nNo data will be saved.")
						RETURN 2 //reject changes and allow focus changed
					End If
				//End If
			end If
		End If
		//End - Modified By Ken.Guo 02/05/2015
...
//				MessageBox("Contract Logix","You don't have full access to the new category, hence the change will not be made!")
	...
	//Added By Jay Chen 04-10-2015 Check contract access right to the current division,contract_type,department
	case 'dvision'
		IF Not IsNull(data) and data <> '-777' THEN
			IF IsValid(gw_contract) THEN 
				ll_cnt = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_division.Retrieve(gs_user_id)	
				IF ll_cnt > 0 THEN ll_found = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_division.Find("category = " + data, 1, ll_cnt)
			END IF	
			IF NOT ll_found > 0 THEN
				ll_code = long(data)
				SELECT count(*) into :ll_cnt FROM code_lookup WHERE lookup_name='contract division' and lookup_code = :ll_code;
				if ll_cnt > 0 then
					MessageBox(gnv_app.iapp_object.DisplayName,"You don't have permission or full access to add records to this division. ~r~nNo data will be saved.")
					RETURN 2 //reject changes and allow focus changed
				end if
			END IF
		END IF
	...
	case 'contract_department'
		IF Not IsNull(data) and data <> '-777' THEN
			IF IsValid(gw_contract) THEN 
				ll_cnt = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_department.Retrieve(gs_user_id)	
				IF ll_cnt > 0 THEN ll_found = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_department.Find("category = " + data, 1, ll_cnt)
			END IF	
			IF NOT ll_found > 0 THEN
				ll_code = long(data)
				SELECT count(*) into :ll_cnt FROM code_lookup WHERE lookup_name='contract department' and lookup_code = :ll_code;
				if ll_cnt > 0 then
					MessageBox(gnv_app.iapp_object.DisplayName,"You don't have permission or full access to add records to this department. ~r~nNo data will be saved.")
					RETURN 2 //reject changes and allow focus changed
				end if
			END IF
		END IF
	...
	case 'contract_type'
		IF Not IsNull(data) and data <> '-777' THEN
			IF IsValid(gw_contract) THEN 
				ll_cnt = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_contract_type.Retrieve(gs_user_id)	
				IF ll_cnt > 0 THEN ll_found = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_contract_type.Find("category = " + data, 1, ll_cnt)
			END IF	
			IF NOT ll_found > 0 THEN
				ll_code = long(data)
				SELECT count(*) into :ll_cnt FROM code_lookup WHERE lookup_name='contract type' and lookup_code = :ll_code;
				if ll_cnt > 0 then
					MessageBox(gnv_app.iapp_object.DisplayName,"You don't have permission or full access to add records to this contract type. ~r~nNo data will be saved.")
					RETURN 2 //reject changes and allow focus changed
				end if
			END IF
		END IF
...
gnv_debug.of_output( False,'of_Set_Title(): start.')
of_Set_Title( )
gnv_debug.of_output( False,'of_Set_Title(): end.')
...
End If
If isnull(ldt_version_date) Then
	dw_contract_dates.object.version_date[1] = datetime(today(), now())
	ldt_version_date = datetime(today(),now())//modified by gavins 20151028  add now()
End If
...
gnv_debug.of_output( False,'ue_post_itemchanged: start.')
...
IF ls_column_name = 'category' and dw_1.getrow() > 0 THEN
	gnv_debug.of_output( False,'this.event pfc_postinsertrow( row): start.')
	this.event pfc_postinsertrow( row)					//Added By Mark Lee 05/24/2013
	gnv_debug.of_output( False,'this.event pfc_postinsertrow( row): end.')
End If
...
IF data = '-777' and ls_column_name = 'category' and dw_1.getrow() > 0 THEN
	...
	IF Not IsNull(ll_category) and ll_category <> -777 THEN
		...
		IF NOT ll_found > 0 THEN
			//Added By mark 04/13/12
//			MessageBox("Contract Logix","You don't have full access to the new category, hence the change will not be made!")
			MessageBox("Contract Logix","You don't have permission or full access to add records to this category. ~r~nNo data will be saved.")
...
gnv_debug.of_output( False,'ue_post_itemchanged: end.')
...
//Modified By Ken.Guo 08/27/2014
//Return 1
Return AncestorReturnValue
end event

event ue_populatedddws;call super::ue_populatedddws;Parent.post of_set_contract_status_color() //Added By Ken.Guo 02/05/2015

//Added By Jay Chen 05-27-2015 filter no access category
datawindowchild ldwc_search,ldwc_detail
long ll_find,ll_cnt,ll_code
dw_1.getchild("category",ldwc_detail)
if isvalid(gw_contract) then
	gw_contract.tab_contract_details.tabpage_search.dw_search.getchild("category",ldwc_search)
	if ldwc_search.rowcount() > 0 then
		for ll_cnt = ldwc_detail.rowcount() to 1 step -1
			ll_code = ldwc_detail.getitemnumber(ll_cnt, 'lookup_code')
			if isnull(ll_code) then ll_code = 0
			if ll_code <= 0 then continue
			ll_find = ldwc_search.find("lookup_code = " + string(ll_code) ,1, ldwc_search.rowcount())
			if ll_find < 1 then
				ldwc_detail.rowsdiscard(ll_cnt, ll_cnt, primary!)
			end if
		next
	end if
end if
...
//Added By Ken.Guo 2015-11-26. Filter Company data by Type
String ls_CurTag, ls_filter 
DataWindowChild ldwc_company
Long ll_facility_id
string ls_ori_filter //Added By Jay Chen 03-22-2016

If dwo.name = 'app_facility' Then
	//filter by status
	is_filter_company_contact = gnv_data.of_getitem("icred_settings", "filter_company_contact", false)
	if is_filter_company_contact = '1' then
		ls_filter = "app_facility_status = 'A' "
	else	
		ls_filter = " 1 = 1"
	end if	
	
	//filter by contact type
	This.GetChild('app_facility', ldwc_company)
	//Added By Jay Chen 03-22-2016 add depend filter for facility
	ls_ori_filter = ldwc_company.describe( "datawindow.table.filter")  
	If Len(ls_ori_filter) = 1 Then ls_ori_filter = ''
	//end
	
	ls_CurTag = This.Describe(dwo.name + ".tag")
	Choose Case Upper(ls_curTag) 
		Case '','ALL'
			//
		Case 'A'
			ls_filter = ls_filter +  ' and (contact_type2 = "A" or isnull(contact_type2)) '
		Case 'B'
			ls_filter = ls_filter + ' and (contact_type2 = "B" or isnull(contact_type2)) '
		Case 'O'
			ls_filter = ls_filter + ' and (contact_type2 = "O" or isnull(contact_type2)) '
	End Choose
	
	//Filter by current data
	If row > 0 Then 
		ll_facility_id = This.GetItemNumber(row, 'app_facility')
		If ll_facility_id > 0 Then
			ls_filter = ls_filter + ' or facility_id = ' + String(ll_facility_id)
		End If
	End If
	
	//Added By Jay Chen 03-22-2016
	if len(ls_ori_filter) > 0 then
		ls_filter = ls_filter + ' and ' + ls_ori_filter
	end if
	//end
	ldwc_company.SetFilter(ls_filter)
	ldwc_company.Filter()	
	is_prior_col = 'app_facility'
ElseIf is_prior_col = 'app_facility' Then
	This.GetChild('app_facility', ldwc_company)
	ldwc_company.SetFilter('')
	ldwc_company.Filter()	
End If
...
IF Lower(ls_datatype) = 'datetime' THEN
	...
	//Added By Ken.Guo 11/28/2013
	If Not This.of_check_isdate_format( ls_colname,true) Then
		Return
	End If
...
//Added By Jay Chen 12-26-2013
If dwo.Name = 'b_custom_status1track' or dwo.Name = 'b_custom_status2track' then
	//Modified By Ken.Guo 2010-02-25 . pass ab_readonly parm.
	Str_ctx_info lstr_ctx
	ll_ctx_id = This.GetItemNumber(This.GetRow(),'ctx_id')
	lstr_ctx.ab_readonly = of_check_read_only( )
...
//		tab_1.tabpage_2.dw_1.Modify(	" DataWindow.Detail.Height = " +  String(tab_1.tabpage_2.dw_1.Height + 30)) //Modified By Jay Chen 10-16-2014
