...
if ln_getfilename.of_getopenfilename("Open image or text file", ls_path, named, "*", "All Files (*.*),*.*") = 1 then
	is_doc_path = ls_path
else
	is_doc_path = ''
end if
	...
	case else 
			...
			*/
			If right(lower(STRING(this.object.image_version[getrow()] )),5)='.html' or right(lower(STRING(this.object.image_version[getrow()] )),4)='.htm' Then
			    ll_ret = ShellExecuteA ( Handle( This ), "open", 'IEXPLORE', inv_contract_details.is_contrtact_dir_path +'\'  + STRING(this.object.image_version[getrow()] ) + '_' +this.object.file_name[getrow()] , ls_Null, 4)
			else				
			    ll_ret = ShellExecuteA ( Handle( This ), "open", inv_contract_details.is_contrtact_dir_path +'\'  + STRING(this.object.image_version[getrow()] ) + '_' +this.object.file_name[getrow()] , ls_Null, ls_Null, 4)
			end if
			//---------------------------- APPEON END ----------------------------   
...
case 'b_open'
		...
		*/
		value = ShellExecuteA ( Handle( This ), "open", 'IEXPLORE', ls_execute , ls_Null, 4)
		//---------------------------- APPEON END ----------------------------   		
		choose case value
			CASE 0 
				parent.of_set_message( 'The operating system is out of memory or resources.')
         CASE 2 
			...
			CASE 31 
				...
				*/
				ShellExecuteA ( Handle( This ), "open", 'IEXPLORE', ls_execute , ls_Null, 4)
				//---------------------------- APPEON END ----------------------------   
			CASE 8 
				parent.of_set_message('There was not enough memory to complete the operation. ') 
			CASE 26 
