...
//ls_customscreen = {'custom_date1','custom_date2','custom_date3','custom_date4','custom_date5'} //Modified By Jay Chen 11-27-2013
ls_customscreen = {'custom_date1','custom_date2','custom_date3','custom_date4','custom_date5','custom_date6','custom_date7','custom_date8','custom_date9','custom_date10','custom_date11','custom_date12','custom_date13','custom_date14','custom_date15','custom_date16','custom_date17','custom_date18','custom_date19','custom_date20'}
...
this.of_setupdateable( true)												//Added By Mark Lee 10/26/2015
...
//	gnv_appeondb.of_startqueue( )														//Added By Mark Lee 10/26/2015
//	dw_detail.update()
//	dw_browse.update()
//	gnv_appeondb.of_commitqueue( )
	...
	if isvalid(m_pfe_cst_mdi_menu_contract) then																	//Added By Mark Lee 10/27/2015
...
event pfc_preupdate;call super::pfc_preupdate;long			ll_rowcount, i , ll_ctx_id, ll_hdr_id, ll_i
string			ls_sql[]
integer		li_return

//Added By Mark Lee 10/26/2015
...
//	ib_retrieved = false
	...
	u_tabpg_contract_multi	luo_contract
					
	//Loop getting the parent of the object until it is of type window!
	lpo_parent = this.GetParent()
	do while IsValid (lpo_parent)				
		If lpo_parent.TypeOf() = UserObject! and lpo_parent.classname( ) = "tabpage_multi" Then Exit
		lpo_parent = lpo_parent.GetParent()
	loop

	if isvalid(lpo_parent) then 
		luo_contract = lpo_parent
		luo_contract.tab_multi.tabpage_header.event ue_reset_ib_retrieve( )
	end if 
end if 
...
if ll_rowcount > 0 then 
	...
	for ll_i = 1 to ll_rowcount
		ll_hdr_id = this.GetItemNumber(ll_i,"hdr_id", Delete!, true)
		if not isnull(ll_hdr_id) and ll_hdr_id > 0 then
			i++
			ls_sql[i] = "delete ctx_custom_multi_detail where ctx_id = "+string(ll_ctx_id)+" and hdr_id = "+string(ll_hdr_id)+" ; "			
		end if 
	next
end if 

if i > 0 then 
	li_return = gnv_string.of_execute_sql( ls_sql, "Update Custom multi detail record(s)", "error", true)
end if 
...
of_enable_alarm(TRUE)//added by gavins 20121217
end event
...
ll_ctx_id = inv_contract_details.of_get_ctx_id()
if getrow() <=0 then Return 														//Added By Mark Lee 11/24/2015 bug.
...
event pfc_addrow;//Added By Jay Chen 06-24-2014
dwItemStatus l_status
if isvalid(gw_contract) then gw_contract.tab_contract_details.tabpage_multi.tab_multi.tabpage_header.dw_header.accepttext()
if dw_browse.GetRow() < 1 then return success
l_status = dw_browse.getitemstatus(dw_browse.GetRow(), 0 ,primary!)
if l_status = New! then
	messagebox('Caution','Please input custom multi header tabpage data first.')
	return success
end if
