...
Long il_ctx_id_master //Added By Ken.Guo 2016-12-21
end variables

forward prototypes
public function integer of_retrieve ()
public subroutine of_hide_page_toolbar ()
end prototypes
...
//Modified By Ken.Guo 2016-08-09
...
uo_page.visible = false //Added By Jay Chen 03-25-2016
this.setredraw(true)
end subroutine
...
//Modified By Ken.Guo 2016-12-21
//il_ctx_id = long(mid(ls_parm,1,pos(ls_parm,'-') - 1))
il_ctx_id_master = long(mid(ls_parm,1,pos(ls_parm,'-') - 1))
...
if ll_Right = 0 then
	ll_x = Long(dw_Search.describe( "b_search.x" ) )
	ll_x2 = Long(dw_Search.describe( "b_more.x" ) )
	dw_search.Modify( "b_more.visible = false b_clear.x= " + string( ll_x ) + " b_search.x=" + string ( ll_x2 )  )
...
event resize;call super::resize;of_hide_page_toolbar() //Added By Jay Chen 05-14-2015
end event
...
If ll_row > 0 Then
	...
	//Added By Jay Chen 04-03-2015
	if isnull(ls_company) or trim(ls_company) = '' or isnumber(ls_company) then 
		ll_facility_id = dw_search_det1.GetItemNumber( ll_row, "app_facility" )
		ls_company=gnv_data.of_getitem("ctx_facility","facility_name","facility_id = " + String(ll_facility_id))
	end if
	//end add
End If
...
Parent.ib_autoretrieve = true //Added by gavins 20160817
...
//		end if
		...
		//Modified By Ken.Guo 2016-12-21. change il_ctx_id to il_ctx_id_master
		If il_ctx_id_master > 0 Then
			If len(is_filter_string) = 0 Then
				is_filter_string = "ctx_id <> " + string(il_ctx_id_master)
			Else
				is_filter_string = is_filter_string + " AND ctx_id <> " + string(il_ctx_id_master)
			End If
		End If	
	End If
...
event st_vbar2::lbuttonup;call super::lbuttonup;of_hide_page_toolbar() //Added By Jay Chen 05-14-2015
end event

type st_hbar from u_tabpg_contract_search`st_hbar within u_tabpg_master_contract_search
end type

event st_hbar::lbuttonup;call super::lbuttonup;of_hide_page_toolbar() //Added By Jay Chen 05-14-2015
end event
...
event st_vbar::lbuttonup;call super::lbuttonup;of_hide_page_toolbar() //Added By Jay Chen 05-14-2015
end event

type st_hbar2 from u_tabpg_contract_search`st_hbar2 within u_tabpg_master_contract_search
end type

type pb_1 from u_tabpg_contract_search`pb_1 within u_tabpg_master_contract_search
end type

type pb_2 from u_tabpg_contract_search`pb_2 within u_tabpg_master_contract_search
end type

type pb_3 from u_tabpg_contract_search`pb_3 within u_tabpg_master_contract_search
end type

type pb_4 from u_tabpg_contract_search`pb_4 within u_tabpg_master_contract_search
end type

type sle_page from u_tabpg_contract_search`sle_page within u_tabpg_master_contract_search
end type

type uo_page from u_tabpg_contract_search`uo_page within u_tabpg_master_contract_search
end type
