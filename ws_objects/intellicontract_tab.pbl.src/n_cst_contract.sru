...
public function integer of_set_checkdoublecontract (string as_key, string as_value)
public function integer of_delete_set_checkdoublecontract ()
public function integer of_checkdoublecontract (string as_class, datawindow adw_current, integer ai_ctxid, datastore ads_current)
end prototypes
...
If FileExists(ls_FullFileName) Then
	gnv_filetime.of_getlastwritedatetime( ls_FullFileName, ls_UpdateDT)
End If
...
//Added By Ken.Guo 2015-05-04
gnv_filetime.of_setlastwritedatetime( ls_FullFileName, ldt_LastUpdate)
...
Delete From screen_field_select_item where ctx_id = :al_ctx_id; //Added by Harry 20170516
commit;
...
choose case lower(as_table)
	...
	case "ctx_am_document" //Added By Jay Chen 11-29-2013
		ll_doc_id = adw_screen.getitemnumber(adw_screen.getrow(),'doc_id')
...
IF Lower(ls_datatype) = 'datetime' and IsValid(gw_contract) THEN
	...
	if isnumber(adw_screen.Describe(ls_ColName + ".x")) then		
		...
		*/
		...
		If Not adw_screen.Dynamic of_check_isdate_format( ls_ColName,true) Then
			Return 0
		End If
		...
		if Of_Show_Missing_Msg(ls_ColName, adw_screen,ls_datatype,adw_screen.getrow()) = 1 then
			...
			lstr_pass.s_string_array[8] = string(ll_doc_id)
...
if ls_datatype = "datetime" then
	...
	choose case lower(as_table)
		...
		case 'ctx_fee_sched_nm'  //jervis - 08.02.2011
			...
			lds_wf_triggered.SetTransObject(SQLCA)
			ll_cnt = lds_wf_triggered.Retrieve(ll_ctxid,ll_doc_id,ll_hdr_id,ll_detail_id,as_table)
		case 'ctx_am_document' //Added By Jay Chen 12-24-2013
			ll_ctxid = adw_alarm.getitemnumber( ll_row, "ctx_id")
			ll_doc_id = adw_alarm.GetItemNumber(ll_row,'doc_id')
	end choose
	...
	if lb_alarm then 
		...
		choose case lower(as_table)
			...
			case 'ctx_am_document' //Added By Jay Chen 12-24-2013
				...
				delete from ctx_am_action_item where  doc_id = :ll_doc_id and alm_column = :ls_colname;
			...
			case else //add fee scheduce --jervis 08.02.2011
				delete from ctx_all_alarm_attachment where ctx_id = :ll_ctxid and doc_id = :ll_doc_id  and alm_table = :as_table and alm_column = :ls_colname;
				delete from ctx_all_notification where ctx_id = :ll_ctxid and doc_id = :ll_doc_id and alm_table = :as_table and alm_column = :ls_colname;
				delete from ctx_action_items where  ctx_id = :ll_ctxid and doc_id = :ll_doc_id  and alm_table = :as_table and alm_column = :ls_colname;
				delete from ctx_alm_snooze   where ctx_id = :ll_ctxid and doc_id = :ll_doc_id  and alm_table = :as_table and alm_column = :ls_colname;
		end choose
...
//				adw_alarm.SetItem( ll_row, ls_ColName + "_flag", f_set_ai_flag(as_table, ls_ColName,ll_ctxid,ll_doc_id, ll_hdr_id ,ll_detail_id))
			...
			case else //add fee scheduce --jervis 08.02.2011
//				adw_alarm.SetItem( ll_row, ls_ColName + "_flag", f_set_ai_flag(as_table, ls_ColName,ll_ctxid,ll_doc_id, ll_hdr_id ,ll_detail_id))
//				adw_alarm.SetItemStatus(ll_row, ls_ColName  + "_flag", Primary!, NotModified!)	//Added By Jay Chen 12-24-2013
				ls_filename = f_set_flag(as_table, ls_colname, ll_ctxid,ll_doc_id,ll_hdr_id,ll_detail_id)
				adw_alarm.modify("b_"+ ls_colname + ".FileName ='" + ls_filename + "'")
...
public function integer of_set_checkdoublecontract (string as_key, string as_value);string 	ls_Item

ls_Item = "FindDoubleContractBasicSpecificColumn"

gnv_user_option.of_Set_option_value("contractlogix-super-master",   ls_Item + as_key  , as_value )
...
public function integer of_delete_set_checkdoublecontract ();//of_System_delete
string 	ls_Item

ls_Item = "FindDoubleContractBasicSpecificColumn"

gnv_user_option.of_System_delete(    "Left( lower(Option_name), " + string ( len(ls_Item) ) + ") = '" + lower(ls_item)+"'"    )
...
public function integer of_checkdoublecontract (string as_class, datawindow adw_current, integer ai_ctxid, datastore ads_current);//get column
string			ls_Item, ls_Columns[], ls_Values[], ls_Type
Integer		li_i, li_Return, li_yn
DataStore	lds_Option
string 		ls_Sql, ls_where, ls_Err, ls_check

ls_Item = "FindDoubleContractBasicSpecificColumn"

ls_check = gnv_user_option.of_Get_option_value(  "contractlogix-super-master"   ,  ls_Item  + "-checked")
If ls_check = '0' Then Return 0

//Added By Ken.Guo 07/03/2014. Check Each type for new/save/copy/template.
String ls_item2
ls_item2 = "FindDoubleContractCheckType"
ls_check = gnv_user_option.of_Get_option_value(  "contractlogix-super-master"   ,  ls_Item2 +'_'+ lower(as_class) + "-checked")
If ls_check = '0' or ls_check = '' or isnull(ls_check) Then Return 0
  
ls_Sql = 'select top 1 ctx_id from ctx_basic_info '
ls_where = ""
...
For li_i = 1 To  lds_Option.Rowcount( )
	...
	If ls_columns[li_i] = '1' Then ls_columns[li_i] = ""
Next

If as_class = 'copy' or as_class = 'template' Then 
	If ads_Current.Rowcount( ) = 0 Then return 0
	For li_i = 1 To UpperBound( ls_Columns )
		If ls_columns[li_i] = '' Then continue
		ls_Type = ads_Current.Describe( ls_Columns[li_i] + ".coltype" )
		
		Choose case lower(Left(ls_type,3 ) )
			case 'cha'
				ls_Values[ li_i ] = ads_Current.GetItemString( ads_Current.GetRow( ), ls_columns[li_i] )
				If Isnull( ls_Values[ li_i ] ) Then 
					ls_Where += ls_columns[li_i] + " is null and "
				Else
					ls_Where += ls_columns[li_i] + "='" + ls_Values[ li_i ] + "' and "
				End If
			case 'num', 'dec'
				ls_Values[ li_i ] = string(ads_Current.GetItemnumber( ads_Current.GetRow( ), ls_columns[li_i] ) )
				If Isnull( ls_Values[ li_i ] ) Then 
					ls_Where += ls_columns[li_i] + " is null and "
				Else
					ls_Where += ls_columns[li_i] + "=" + ls_Values[ li_i ] + " and "
				End If
			case 'dat'
				ls_Values[ li_i ] = string(ads_Current.GetItemDateTime( ads_Current.GetRow( ), ls_columns[li_i] ), 'yyyy-mm-dd hh:mm:ss' )
				If Isnull( ls_Values[ li_i ] ) Or  Left( ls_Values[ li_i ], 10 ) = '1900-01-01' Then 
					ls_Where += ls_columns[li_i] + " is null and "
				Else
					ls_Where += " convert( varchar(20)," + ls_columns[li_i] + " , 120 ) = '" + ls_Values[ li_i ] + "' and "
				End If
			case else
				continue
		end choose
...
Else
	If adw_Current.Rowcount( ) = 0 Then return 0
	For li_i = 1 To UpperBound( ls_Columns )
		ls_Type = adw_Current.Describe( ls_Columns[li_i] + ".coltype" )
		
		Choose case lower(Left(ls_type,3 ) )
			case 'cha'
				ls_Values[ li_i ] = adw_Current.GetItemString( adw_Current.GetRow( ), ls_columns[li_i] )
				If Isnull( ls_Values[ li_i ] ) Then 
					ls_Where += ls_columns[li_i] + " is null and "
				Else
					ls_Where += ls_columns[li_i] + "='" + ls_Values[ li_i ] + "' and "
				End If
			case 'num', 'dec'
				ls_Values[ li_i ] = string(adw_Current.GetItemnumber( adw_Current.GetRow( ), ls_columns[li_i] ) )
				If Isnull( ls_Values[ li_i ] ) Then 
					ls_Where += ls_columns[li_i] + " is null and "
				Else
					ls_Where += ls_columns[li_i] + "=" + ls_Values[ li_i ] + " and "
				End If
			case 'dat'
				ls_Values[ li_i ] = string(adw_Current.GetItemDateTime( adw_Current.GetRow( ), ls_columns[li_i] ), 'yyyy-mm-dd hh:mm:ss' )
				If Isnull( ls_Values[ li_i ] ) Or  Left( ls_Values[ li_i ], 10 ) = '1900-01-01' Then 
					ls_Where += ls_columns[li_i] + " is null and "
				Else
					ls_Where += " convert( varchar(20)," + ls_columns[li_i] + " , 120 ) = '" + ls_Values[ li_i ] + "' and "
...
If Len( ls_Where  ) > 0 Then 
	...
	//Modified By Ken.Guo 07/08/2014. copy/save/template shoule check all contracts.
	//If as_class = 'copy' Then
	//	ls_Where += " ctx_id > 0 and ( ctx_id < " + string( ai_ctxid ) + " or ctx_id > " + string( ai_ctxid ) + ") " 
	//ElseIf  as_class = 'new' or as_class = 'template' Then
	If as_class = 'copy' or as_class = 'new' or as_class = 'template' Then
		ls_where += " ctx_id>0"
	ElseIf as_Class = 'save' Then
		ls_Where += " ctx_id > 0 and ( ctx_id < " + string( ai_ctxid ) + " or ctx_id > " + string( ai_ctxid ) + ") " 
	End If
	ls_Sql += " where " +  ls_Where 

	DECLARE doublecheck DYNAMIC CURSOR FOR SQLSA ;

	PREPARE SQLSA FROM :ls_sql ;
	
	OPEN  DYNAMIC doublecheck;
	
	fetch doublecheck into :li_Return;
	if  sqlca.sqlcode = -1 then
		ls_Err = sqlca.sqlerrtext
		close doublecheck;
		MessageBox( 'Tips', 'Check for duplicate contract fails,' + ls_Err)
		Return 0		
	end if
	close doublecheck;
	
	If li_Return > 0 Then
		li_yn = MessageBox( 'Tips', 'The contract you are trying to create is duplicated with an existing contract ID ' + string( li_Return ) + ' with regard to the comparison columns.~r~r Do you want to continue?' , Question!, yesno!, 1 )
		If li_yn = 1 then return 0
		Return li_Return
	Else
		Return 0
	End If
