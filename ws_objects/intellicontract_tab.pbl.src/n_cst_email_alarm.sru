...
public function integer of_run_email_alarm ()
end prototypes
...
String ls_field_name, ls_alarm_type, ls_errtext 
...
For i = 1 To ll_cnt
	...
	String ls_attachfilename,ls_columnname,ls_computer_info
...
////Use webapi send email alarm added By Jimmy.Lee 2013-08-28.
If gnv_data.of_getitem( 'icred_settings', 'use_web_email', False) = '1' Then
	n_cst_webapi lnv_webapi

//	string ls_password
//	n_cst_encrypt ln_encrypt
//	if len(gs_password) > 0 then
//		ls_accesstoken = lnv_webapi.of_login(gs_user_id,gs_password,ls_cachename)
//	else
//		select user_password into :ls_password from security_users where user_id = :gs_user_id;
//		ls_password = ln_encrypt.of_decrypt(ls_password)
//		ls_accesstoken = lnv_webapi.of_login(gs_user_id,ls_password,ls_cachename)
//	end if
...
//		lnv_webapi.post of_alarmemail(ls_accesstoken)
		lnv_webapi.of_alarmemail( )		//Added By Mark Lee 09/03/2013
		//lnv_webapi.post of_logout(ls_accesstoken)	
		if len( lnv_webapi.errorcode ) > 0 then return -1
	end if
	return 1
end if
