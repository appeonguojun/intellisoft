...
boolean lb_validate //Added By Jay Chen 01-05-2015
long ll_invalid_row
string ls_invalid_notes //Added By Jay Chen 08-11-2015
...
//					exit
	...
	if len(trim(this.GetItemString(ll_i,"notes"))) > 2000 then  //Added By Jay Chen 01-05-2015
		lb_validate = true 
		//Added By Jay Chen 08-11-2015
		ll_invalid_row = ll_i
		ls_invalid_notes = left(this.GetItemString(ll_i,"notes"),30) + '...'
		//end
	end if
next
...
//Added By Jay Chen 01-05-2015
if lb_validate then
	MessageBox("Message","The length of the notes [" + ls_invalid_notes + "] has more than 2000 characters.Please modify it again!")
	//Added By Jay Chen 08-11-2015
	this.scrolltorow(ll_invalid_row)
	this.setrow(ll_invalid_row)
	this.setfocus()
...
if w_mdi.of_security_access(6982) = 0 then	//Modify Other Users AI
	...
	for ll_i = 1 to RowCount
		...
		if (ls_assigned_to <> gs_user_id or isnull(ls_assigned_to)) and (ls_assigned_by <> gs_user_id or isnull(ls_assigned_by)) then
			...
			this.setitemstatus(ll_i, 'readonly_flag', primary!,notmodified!) //Added By Ken.Guo 2017-07-25.  Field Audit Log error.
		end if
	next
end if
//---------------------------- APPEON END ----------------------------
...
Boolean lb_readonly = false //Added By Jay Chen 05-16-2014
string ls_notes //Added By Jay Chen 05-16-2014

// mskinner 05 april 2006 -- begin
//if of_check_read_only( ) then return //Modified By Jay Chen 05-16-2014
if of_check_read_only( ) then lb_readonly = true
// mskinner 05 april 2006 -- end 

//Begin - Added By Mark Lee 04/30/2013
If row > 0 Then
	If this.GetItemNumber( row,"readonly_flag") = 1 Then
//		Return  //Modified By Jay Chen 05-16-2014
		lb_readonly = true
	End IF
End If 
//End - Added By Mark Lee 04/30/2013

if lb_readonly and dwo.name <> 'b_notes' then return //Added By Jay Chen 05-16-2014
...
choose case dwo.name
	...
	case 'b_due_date','b_due_date_g','b_due_date_r' //Alfee 02.26.2008
		...
		If Not This.of_check_isdate_format( 'due_date',true) Then
			Return
		End If				
...
//Added By Ken.Guo 2016-05-16. jump focus to other column. to workaround issue.
//otherwise the aciton status dddw does not filter data after get focus again.
If GetColumnName() = 'action_status' Then
	Send( Handle( This),256,9,Long(0,0))  //set to next focus.
End If
end event
