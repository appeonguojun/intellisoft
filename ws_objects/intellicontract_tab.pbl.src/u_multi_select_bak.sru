$PBExportHeader$u_multi_select_bak.sru
forward
global type u_multi_select_bak from u_base
end type
type dw_select from u_dw within u_multi_select_bak
end type
end forward
...
global u_multi_select_bak u_multi_select_bak
...
String	ls_coltype,ls_value,ls_style
Integer	li_x
Integer	li_y
long ll_screen,ll_cnt,ll_row,ll_value
...
If IsValid(idw_requestor) Then
	...
	if left(ls_coltype,4) = 'char' and left(ls_colname,7) = 'custom_' and ls_style = 'dddw' then
		...
		for ll_cnt =1 to ldwc.rowcount()
			ll_value = ldwc.getitemnumber(ll_cnt, ls_data)
			...
			dw_select.setitem(ll_row,"as_itemcode",ll_value)
			dw_select.setitem(ll_row,"as_itemname",ls_value)
		next
	else
		return 0
	end if
...
event itemchanged;call super::itemchanged;string ls_value,ls_select,ls_data,ls_itemname,ls_fieldname
long ll_row,ll_itemcode
//Added By Jay Chen 12-23-2015
if dwo.name = 'as_choose' then
	...
	for ll_row = 1 to dw_select.rowcount()
		...
		if ls_select = 'Y' then
			ll_itemcode = dw_select.getitemnumber(ll_row,"as_itemcode")
			ls_itemname = dw_select.getitemstring(ll_row,"as_itemname")
			if  ls_value = '' then
				ls_value = ls_itemname
				ls_data = string(ll_itemcode)
			else
				ls_value = ls_value + ',' + ls_itemname
				ls_data =  ls_data + ',' + string(ll_itemcode)
			end if
		end if
	next

	If IsValid(idw_requestor) Then
		ls_fieldname = idw_requestor.getcolumnname()
		ll_row = idw_requestor.getrow()
		if ll_row > 0 then
			idw_requestor.setitem(ll_row,ls_fieldname, ls_value)
		end if
	End If
