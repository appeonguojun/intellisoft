...
if rb_5.Checked then
	...
	*/
	...
	if is_filter_company_contact = '1' then ls_where += " and (status = 'A') " //Added By Jay Chen 10-29-2015
	
//	if il_facility_id > 0 then ls_where += " and (facility_id = " + String(il_facility_id) + ")" //Added By Jay Chen 01-07-2015
	...
	if dw_facility_search.RowCount() > 0 then
		...
		if il_facility_id > 0 then
			ll_find = dw_facility_search.find("facility_id = " + string(il_facility_id), 1, dw_facility_search.RowCount())
			if isnull(ll_find) then ll_find = 0
		end if
		if ll_find <= 0 then ll_find = 1
		il_facility_id = 0
		//end
		dw_facility_search.SetRow(ll_find)
		dw_facility_search.SelectRow(0,false)
		dw_facility_search.SelectRow(ll_find,true)
		dw_facility_search.Event RowFocusChanged(ll_find)
...
long ll_import_id, ll_facility_id, ll_count, ll_rtn
string ls_msg,ls_user,ls_pwd,ls_token 
string ls_salesforce_id, ls_sfurl

select a.import_id into :ll_import_id from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and b.isoft_table_id = 1006;
if isnull(ll_import_id) then ll_import_id = 0
if ll_import_id <= 0 then 
	messagebox('Contractlogix','The system can not find the mapping for the Companies synchronization. Please make sure you have configured it correctly in the Data Synchronization Painter.')
	return 
end if


ll_facility_id = this.getitemnumber(this.getrow(), 'facility_id')
ls_msg =  'Are you sure you want to sync this facility (ID = '+String(ll_facility_id)+') to SalesForce?'
...
ls_pwd += ls_token
if messagebox('Sync Facility', ls_msg,Exclamation!,Yesno!,2) = 1 then
	//check the facility_id if or not already sync to salesforce
	select count(*) into :ll_count from import_outbound_records where lower(ctx_tablename) = 'app_facility' and lower(ctx_fieldname) = 'facility_id' and ctx_record_id = :ll_facility_id and export_type = 1;
	if isnull(ll_count) then ll_count = 0
	if ll_count > 0 then 
		if messagebox('Sync Facility', 'The facility (ID = '+String(ll_facility_id)+') has already been synchronized to SalesForce.~r~nAre you sure you want to overwrite the facility information on SalesForce?',Exclamation!,Yesno!,2) <> 1 then
			return
		end if
	end if
	ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', 0, 0, 0, 0, ll_import_id, ls_salesforce_id, ls_sfurl, ll_facility_id, 0)
	if len(ls_salesforce_id) <= 0 then
		messagebox('Sync Facility', 'Failed to synchronize the facility (ID = '+String(ll_facility_id)+') to SalesForce.')
	else
		messagebox('Sync Facility', 'Succeed to sync the facility to salesforce.')
	end if					
end if
end event
...
IF IsValid(w_contract_contact) THEN
	IF w_contract_contact.istr_pass.l_facility_id = -999 THEN
		...
		ELSE
			ls_email = ""
		END IF
		//Added By Jay Chen 10-14-2015
		if len(ls_email) = 0 then
			messagebox('Tips','Please select a contact with an email address.')
			return
		end if
		//end
		w_contract_contact.of_add_contacts_email(ls_email)
...
for ll_I = 1 to dw_search.RowCount()
	if dw_search.getitemnumber(ll_I,'selected')	 = 1 then
		ll_contact_id = dw_search.object.contact_id[ll_I]  // Add 04.27.2007 By: Jack
			...
			lstr_contact.as_facility_name = dw_search.object.facility_name[ll_I] 			//Added By Mark Lee 01/21/2015
...
event ue_sync_to_salesforce();//Added by Harry 20170323 for task 749
n_cst_webapi lnv_webapi
...
String	ls_FullName
Long	li_Contact_id,li_facility_id //Added By Ken.Guo 11/12/2013 Change integer to long type
string ls_access 
...
lm_dw.m_table.m_synctosf.visible = True //Added by Harry 2017-03-23 for task 749
