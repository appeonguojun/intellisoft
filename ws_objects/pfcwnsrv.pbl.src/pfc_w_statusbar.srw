...
//Modified By Ken.Guo 2011-08-30
//Timer (0.5, this)
This.event timer( )
Timer(ii_secondstorefresh,This)
...
// Set its position.
of_SetPosition()//modify by gavins 20130510
...
string		ls_modifyexp
string		ls_rc
ulong			ll_totalmem //Added By Ken.Guo 2010-12-14.
...
If iw_parentwindow.inv_statusbar.of_GetMem() Then
	...
	//Begin Modified By Ken.Guo 2010-12-14
	/*
	...
	*/
	...
	//End Modified By Ken.Guo 2010-12-14
...
if ll_desiredstatubar_xpos < ll_frame_xpos then 
	...
	If this.width <> ( iw_parentwindow.workspacewidth() - (20 + li_xpos_extra) ) Then
	...
	End If
	//---------------------------- APPEON END ----------------------------
	If this.width <> ii_winmaxwidth Then
		this.width = ii_winmaxwidth
	End If
end if
...
idw_Statusbar = Message.PowerobjectParm //added by gavins 20130510
...
//this.of_SetPosition() //modify by gavins 20130510
...
if AppeonGetClientType() = "PB" then
	...
	*/
	...
	Else
		...
		li_pos = Pos(ls_title,'  ')
		Do While li_pos > 0 
			ls_title = Replace(ls_title,li_pos,2,' ') 
			li_pos = Pos(ls_title,'  ')
		loop
		...
		If Len(ls_title) > 95 Then
			ls_title = Left(ls_title,95)   
		End If
		//---------------------------- APPEON END ----------------------------
...
If IsValid( lw_Temp ) Then
	...
	//Modified By Mark Lee 09/30/2013 change the code "lw_temp=w_contract" to "lower(lw_temp.classname( )) = "w_contract" "
	If Not isvalid( gw_Contract ) and lower(lw_temp.classname( )) = "w_contract"  Then  ls_value = "Ready"
	
//	If is_Last_Process_value = ls_Value Then Return
	
	If left( ls_Value, 14 ) = "Contract Logix" Then ls_value = Trim( Mid( ls_value, 15 ) )
