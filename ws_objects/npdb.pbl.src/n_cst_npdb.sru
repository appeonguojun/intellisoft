...
private function integer of_create_upidlist ()
...
//Start Code Change ---- 02.23.2007 #V7 maha
 ls_ini_val =  ProfileString(gs_dir_path + gs_DefDirName + "\intellicred.ini", "npdb", "ProxyHostName", "None")
 if ls_ini_val <> "None" then
	ls_file_text_array[upperbound(ls_file_text_array) + 1] = NEW_LINE + "ProxyHostName=" +  ls_ini_val 
end if

 ls_ini_val =  ProfileString(gs_dir_path + gs_DefDirName + "\intellicred.ini", "npdb", "ProxyPort", "None")
 if ls_ini_val <> "None" then
...
FOR ll_j = 1 TO lcc
			...
			s_type = 	upper(Left ( string(a_dw.Describe("#" + string(ll_j) + ".ColType")), 5))
 			CHOOSE CASE  s_type //maha
					case 'CHAR(' // char	 
						if ( isnull(a_dw.getItemstring(1, ll_j)) or len(trim(string(a_dw.getItemstring(1, ll_j)))) < 1)   and ls_required=  "yes" then
						   if a_dw = ids_pracs_info then //practitioner
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value.')// Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
							else
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value. Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
							end if
							ll_return = failure
					...
					case 'LONG(' // long
						if isnull(a_dw.getItemNumber(1, ll_j)) or len(trim(string(a_dw.getItemNumber(1, ll_j)))) < 1 and ls_required=  "yes" then
							if a_dw = ids_pracs_info then //practitioner
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value.')// Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
							else
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value. Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
					...
					case 'DATE(' // date
						if isnull(a_dw.getItemdate(1, ll_j)) or len(trim(string(a_dw.getItemdate(1, ll_j)))) < 1  and ls_required=  "yes" then
							if a_dw = ids_pracs_info then //practitioner
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value.')// Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
							else
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value. Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
					...
					case 'DATET' // date
						if isnull(a_dw.getItemdatetime(1, ll_j)) or len(trim(string(a_dw.getItemdatetime(1, ll_j)))) < 1 and ls_required=  "yes" then
							if a_dw = ids_pracs_info then //practitioner
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value.')// Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
							else
								of_set_error_message('Missing data: ' +   ls_message + ' is a requied value. Please check the NPDB tab for facility :' + a_dw.getItemString(1,'facility_name'))
	...
	if ll_return = failure then
		if a_dw = ids_pracs_info then
			of_set_error_message(upper('NPDB request has not been submitted due to missing data. For practitioner ID number ' + string(a_dw.getItemnumber(1, "prac_id"))))
		else
			of_set_error_message(upper('NPDB request has not been submitted for facility: ' + a_dw.getItemString(1,'facility_name')+ ' due to missing data.'))
		end if
	end if
...
ls_stat = mid(ls_read_string, pos(ls_read_string, "StatusCode=" ) + 11 )
...
ls_tag_string[upperbound(ls_tag_string) + 1]  =  ids_npdb_fac_info.object.npdb_cert_phone [1] + delimiter   // cert.person phone
...
for ll_i = 1 to ids_grad.rowcount()
	if ll_i < 6 then // can only have five
		...
		if len(ls_year_thru) < 1 or isnull(ls_year_thru) then  //Start Code Change ---- 05.11.2007 #V7 maha added null check
...
if len(ls_category) = 0 then
	ls_category = "699"
elseif len(ls_category) = 1 then
	ls_category = "00" + ls_category 
elseif len(ls_category) = 2 then
...
ls_tag_string[upperbound(ls_tag_string) + 1]  =  ''                                       				     + delimiter   // SUFFIX (NR)
ls_tag_string[upperbound(ls_tag_string) + 1]  =  upper(left(string(ids_pracs_info.object.gender[1]),1))    + delimiter   // GENGER 
...
if lds_address_info.rowcount() > 0 then 
	...
	if f_validstr(string(lds_address_info.object.zip[1])) then
		ls_tag_string[upperbound(ls_tag_string) + 1]  =  lds_address_info.object.zip[1]							+ delimiter   // HOME ZIP5 (NR)
   	else
...
for ll_i = 1 to ids_pracs.rowcount()
	ls_file = is_directories[3] + '\' +  ids_pracs.object.doc_id[ll_i] + '.dat' 
...
if upper(is_internet_inuse) =  "N" then
	...
	openwithparm(w_sql_msg,ls_err_message )
else
...
for ll_i = 1 to upperbound(lnv_dirlist)
		 ...
		 //Start Code Change ----10.14.2008 #V8 maha - trap for missing CUSE
			 ...
			 for ll_l = 1 to upperbound(ls_tag_array)
				if mid(ls_tag_array[ll_l],1,4) = "CUSE" then
			...
			ids_npdb_dws[ll_hdr_position].setItem(ids_npdb_dws[ll_hdr_position].rowcount(), 'prac_id' , long(ls_array[2]))
				...
				IF of_display_error( ) =  failure THEN //function does not return failure
				   // do nothing else
					...
					// set the npdn header id fo these recors
					of_set_npdb_id()
...
//		integer qwe
//		qwe = ids_npdb_fac_info.rowcount()
		...
		FOR ll_i = 1 to ids_pracs.rowcount()
			...
			ids_pracs_info.setfilter("prac_id = " + string(ids_pracs.object.prac_id[ll_i]))
			ids_pracs_info.filter()
			IF of_check_missing_values(ids_pracs_info)       <> success THEN CONTINUE
			IF of_create_upload_dat_indv(IS_CURRENT_FILE_NAME )                   <> success THEN CONTINUE
			//Start Code Change ----06.04.2008 #V81 maha - populate array to create tracker to eliminate creration of tracker records for pracs that fail because of missing data
			ls_tracker_docs[upperbound(ls_tracker_docs) + 1] = IS_CURRENT_FILE_NAME
			//End Code Change---06.04.2008
		NEXT// prac
debugbreak()
	 IF of_create_upidlist()								        <> success THEN CONTINUE
   ///move the rows to the deleted buffer
	...
	ids_npdb_fac_info.RowsMove(1, ids_npdb_fac_info.rowcount(), Primary! , ids_pracs, 1, Delete!)
	// CLEAR PRAC FILTER
	ids_pracs.SetFilter("")  
	ids_pracs.filter()
...
inv_filesrv.of_ChangeDirectory (is_itp_path)
of_create_file(is_batch_files[3], is_batch_file_text[3] + ' ' +ids_npdb_fac_info.object.npdb_pass[1])  
...
If appeongetclienttype() = 'PB' Then
	...
	value = ShellExecuteA ( Handle( This ), "open", 'IEXPLORE', ls_execute , ls_Null, 0)
end if
...
FOR ll_i = 1 TO ids_varification.rowcount()
	...
	IF lb_failed = true then
		ls_note = " Send Error " + string(date(ldt_now))
			...
			ELSE
				ids_varification.object.notes[ll_i] = Mid(ids_varification.object.notes[ll_i] +' , '+ ls_NOTE ,1 , MAX_LENGTH )
			 END IF
	ELSE		
...
if upper(is_internet_inuse) <> "Y" then
   open(w_infodisp)
	...
	w_infodisp.title = 'ITP Interface Progress'
...
for ll_i =1 to ids_npdb_dws[ll_hdr_position].rowcount()	
	...
	if len(ls_err_cd) > 0 then  //write error messages to message
	...
	IF ll_found_row > 0 then
		if len(ls_err_cd) > 1 then  //got an error
	...
	if ids_tracker_by_doc_id.rowcount() > 0 then 
		if len(ls_err_cd) > 0 then  //got an error
...
CHOOSE CASE TRIM(UPPER(as_process))
	CASE IS_Send
		...
		datastore lds
		lds = CREATE datastore
		lds.dataobject = 'd_npdb_doc_ids'
		lds.settransobject( sqlca)
		lds.retrieve( )
		
		LONG ll_ids_row_count
		ll_ids_row_count =lds.rowcount( )
		*/
		CHOOSE CASE adw.dataobject
		   ...
		   CASE 'd_ver_summary_record_view_browse'
				long ll_found
				adw.accepttext()
...
IF NOT FileExists ( is_current_path + "\itp\itp.jar") THEN 
	...
	choose case ret
		...
		case -1 
			...
			is_messsages[upperbound(is_messsages) + 1] = 'The ITP Jar file is missing. Please contact ContractLogix Support for assistance.'
		...
		case else
			// unknown error
			is_messsages[upperbound(is_messsages) + 1] = "An unknown error has occurred while copying the jar file. Contact ContractLogix Support for assistance."
			...
			if ret <> 1 then
				string is_notify[]
				is_notify[1] = "support@contractlogix.com"
				of_mail_error( 'ITP Interface', 'The jar file could not be copied', is_notify)
				ll_return = failure
			end if
...
long ll_I
is_internet_inuse = 'N'
is_current_path = gs_dir_path + gs_DefDirName//gnv_app.is_application_path //Start Code Change ---- 04.26.2006 #422 maha
...
if of_create_folders( )      <> success then 
	messagebox("Failure in NPDB ITP","of_create_folders failed")
	return failure
end if
if of_create_batch_files( )  <> success then 
	messagebox("Failure in NPDB ITP","of_create_batch_files failed")
	return failure
end if
//debugbreak()
if of_move_jar( )            <> success then 
	messagebox("Failure in NPDB ITP","of_move_jar failed")
	return failure
end if
if of_create_classpath( ) <> success then 
	messagebox("Failure in NPDB ITP","of_create_classpath failed")
	return failure
end if
...
if not f_validstr(ls_current_version) then 
	if messagebox('Java JRE Missing','The JRE is not installed on this machine. Would you like to install it now?',question!,yesNo!,1) = 1 then 
		...
		*/
      ShellExecuteA ( Handle( This ), "open", 'IEXPLORE', "http://jdl.sun.com/webapps/download/AutoDL?BundleId=10342"  , ls_Null, 0)
	   //---------------------------- APPEON END ----------------------------   
		return failure
	end if 
end if 
			...
			If upperbound(ls_tag) > 8 then  //if failed and no err code allow it to go to the QSTA later
				for s = 9 to upperbound(ls_tag) // 9 and greater of the FSTA will be the error codes
					...
					if ll_foundrow > 0 then 
						...
						is_messsages[upperbound(is_messsages) + 1] = 'An NPDB error has occured. The Query failed. The code is :' + ls_tag[s] + ' Please inform ContractLogix that the error code is missing from the system.' 
					end if 
				next
				return failure //-1
			end if
		...
		IF ls_tag[3] = "R" THEN // rejected
			for s = 5 to upperbound(ls_tag) // 5 and greater of the QSTA will be the error codes
				...
				if ll_foundrow > 0 then 
					...
					is_messsages[upperbound(is_messsages) + 1] = 'An NPDB error has occured. The Query was rejected. The code is :' + ls_tag[s] + ' Please inform ContractLogix that the error code is missing from the system.' 
				end if 
			next
...
FOR ll_loop = 1 TO ll_source_len
	...
	if inv_string.of_IsPrintable(lch_char) then
		...
		ls_source = Replace(ls_source, ll_pos, 1, new_line)
		ll_pos = ll_pos + 2
	end if 
...
if ll_lic_count < 1 then
	...
	for i = 1 to ll_lic_count
		...
		if len(ls_lic[i]) = 0 then
			ls_lic[i] = "699"
		elseif len(ls_lic[i]) = 1 then
			ls_lic[i] = "00" + ls_lic[i]
		elseif len(ls_lic[i]) = 2 then
			ls_lic[i] = "0" + ls_lic[i]
		end if
...
if isnull(ids_npdb_fac_info.object.security_users_npdb_user_id[1]) then
	...
	if isnull(ids_npdb_fac_info.object.security_users_npdb_password[1]) then
		is_password =  ids_npdb_fac_info.object.npdb_pass[1]
	else
...
if isnull(ids_npdb_fac_info.object.security_users_user_name[1]) then
...
public function integer of_failed_message (string as_section);//Start Code Change ----04.25.2008 #V8 maha - function created for displaying error specifics for file sections
...
for ll_i = 1 to ids_dw_datatore.rowcount()
	...
	for ll_j = 1 to upperbound(ids_npdb_dws)
	  if ids_npdb_dws[ll_j].DataObject = ids_dw_datatore.object.dw_name[ll_i]  then
           ls_tag_array[upperbound(ls_tag_array) + 1] = UPPER(MID(string(ids_npdb_dws[ll_j ].DATAOBJECT),8,LEN(ids_npdb_dws[ll_j ].DATAOBJECT) ))
     end if 
    next
next
...
IF ISNULL(ls_text_string) OR LEN(TRIM(ls_text_string)) < 1 THEN 
