...
if FileExists(gs_dir_path + gs_DefDirName + "\security_mod_1101.txt") = true then
		if FileExists(gs_dir_path + gs_DefDirName + "\security_submod_1101.txt") = true then
			if FileExists(gs_dir_path + gs_DefDirName + "\security_actions_1101.txt") = true then
				
						
			dw_imp_mod.SetTransObject( SQLCA	)
			dw_imp_mod.reset()
			mc = dw_imp_mod.importfile( gs_dir_path + gs_DefDirName + "\security_mod_1101.txt" )
			
			dw_imp_sub.SetTransObject( SQLCA	)
			dw_imp_sub.reset()
			sc = dw_imp_sub.importfile( gs_dir_path + gs_DefDirName + "\security_submod_1101.txt" )
			
			dw_imp_action.SetTransObject( SQLCA	)
			dw_imp_action.reset()
			ac = dw_imp_action.importfile( gs_dir_path + gs_DefDirName + "\security_actions_1101.txt" )

			else
				MessageBox("Problem with file. ","Check to see that file security_actions_1101.txt exists in the " + gs_dir_path + "ContractLogix folder.")
				ii_mods = 0
				return
			end if
		else
		messagebox("Problem with file. ","Check to see that file security_submod_1101.txt exists in the " + gs_dir_path + "ContractLogix folder.")
		ii_mods = 0
		return
		end if
else
	messagebox("Problem with file. ","Check to see that file security_mod_1101.txt exists in the " + gs_dir_path + "ContractLogix folder.")
	ii_mods = 0
	return
end if
