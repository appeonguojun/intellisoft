$PBExportHeader$w_ai_print_letters_new.srw
$PBExportComments$Workaround a bug from PB+VSS
forward
global type w_ai_print_letters_new from w_response
end type
type cbx_app_printer from checkbox within w_ai_print_letters_new
end type
type r_bar_outside from rectangle within w_ai_print_letters_new
end type
type cb_close from u_cb within w_ai_print_letters_new
end type
type cb_print_all from u_cb within w_ai_print_letters_new
end type
type cbx_preview from checkbox within w_ai_print_letters_new
end type
type ole_print from olecustomcontrol within w_ai_print_letters_new
end type
type dw_parent_facility from u_dw within w_ai_print_letters_new
end type
type dw_address_cnt from u_dw within w_ai_print_letters_new
end type
type dw_addr_mail_cnt from u_dw within w_ai_print_letters_new
end type
type gb_1 from groupbox within w_ai_print_letters_new
end type
type r_bar from rectangle within w_ai_print_letters_new
end type
type st_bar from statictext within w_ai_print_letters_new
end type
type st_apps from statictext within w_ai_print_letters_new
end type
type ddplb_filter from dropdownpicturelistbox within w_ai_print_letters_new
end type
type dw_letters from u_dw within w_ai_print_letters_new
end type
type uo_print_app from pfc_cst_u_create_app2 within w_ai_print_letters_new
end type
type gb_corro from groupbox within w_ai_print_letters_new
end type
type dw_corro from u_dw within w_ai_print_letters_new
end type
type gb_letters from groupbox within w_ai_print_letters_new
end type
end forward
...
global w_ai_print_letters_new w_ai_print_letters_new
...
//was removed 10.08 during performance modifications - can be removed end of 2008 - maha
Integer li_error = 0
Integer li_rc
Integer li_retval
Integer i
...
on w_ai_print_letters_new.create
int iCurrent
call super::create
...
on w_ai_print_letters_new.destroy
call super::destroy
...
FOR i = 1 TO rc
	...
	IF Pos( Upper(ls_doc_name), "EXPIRING" ) > 0  and Pos( Upper(ls_doc_name), "EXPIRING_APPOINT" ) = 0 then //skip expiring appointment
		ll_exp_cnt++
	End if		
next
...
string binarykey = "w_ai_print_letters_new.win"
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
...
//note:  was removed 10.08 during performance modifications.  of_buttonclicked function can deleted by end of 2008 if no further need
...
debugbreak()
	...
	//Start Code Change ----06.13.2008 #V81 maha - combined exp cred function
	...
	*/
	...
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@	
		...
		FOR i = 1 TO li_rc
			...
			IF Pos( Upper(ls_doc_name), "EXPIRING" ) > 0  THEN
				if Pos( Upper(ls_doc_name), "EXPIRING_APPOINT" ) > 0 then continue //skip expiring appointment
		...
		Next
		//report errors
		if len(ls_error) > 16 then openwithparm(w_sql_msg,ls_error)
	...
	ELSE //individual letters
		...
		FOR i = 1 TO li_rc
			...
			IF   doctest = ls_doc_name THEN			
				...
				IF Pos( Upper(ls_doc_name), "EXPIRING" ) > 0  THEN
					//expiring cred letters addr link check
					if Pos( Upper(ls_doc_name), "EXPIRING_APPOINT" ) < 1 then //Expiring credentials
	...
	//	ls_style = "I" 
		ls_style = Upper(Left(This.GetItemString( row, "letter_style1" ),1))
	end if
	...
	IF ib_print_letter THEN
		...
		*/
		...
		elseif lb_combined = true then
			...
			li_retval = inv_printletters.of_printletters( is_doc_id[], "S", ls_let_name, 	ls_style , idw_verif_data,ls_doc_name, lb_print_preview, ls_sql,ll_pracids[],li_facility_id )
