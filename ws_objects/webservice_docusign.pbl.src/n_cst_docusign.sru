...
String is_soap_url = 'http://esign.contractlogix.com/contractlogixdocusign/ContractlogixDocusign.asmx?wsdl'			
...
If Len(is_password) > 0 Then
	is_password = lnv_Encrypt.of_decrypt_new(is_password , li_val)
End If
...
//	Messagebox('Error', E2.text +"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
//End Try
//
//If Left(Lower(ls_link),5) = 'error' Then
//	ib_isvalid_sertifi = False
//	Messagebox('Error',ls_link+"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
//	Return ''
//End If
//
//
...
//	ib_isvalid_sertifi = False
//	Messagebox('Error ','Failed to remove the signature request. ' + ls_ret+"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
//	Return -1
//End If
//
//
...
For i = 1 To ll_cnt
		...
		If lower(lstr_sertifi_file.astr_sertifi_document[j].Status) = "declined"  then
			...
			Select Top 1 seq_id into  :lstr_sertifi_doc.al_ai_seq_id From 	ctx_am_action_item 
				Where doc_id = :lstr_sertifi_doc.al_doc_id and (Action_type = :gnv_docusign.il_esign  )  
							and Action_status = :gnv_docusign.il_incomplete and (es_docid = :lstr_sertifi_doc.as_es_docid) ;
	...
	If lstr_sertifi_doc.al_ai_seq_id > 0 Then
		...
		lstr_action_declined[UpperBound(lstr_action_declined[])].wf_action_status_id = gnv_docusign.il_action_declined
		lstr_action_declined[UpperBound(lstr_action_declined[])].wf_action_type_id = gnv_docusign.il_esign
			...
			lds_waiting_sign.SetItem(i,'Status',il_doc_signed)
					...
					gnv_appeondb.of_startqueue( )
						...
						Select Top 1 seq_id into  :ll_seq_no2 From ctx_am_action_item 
							Where 	doc_id = :lstr_sertifi_doc.al_doc_id and (Action_type = :gnv_docusign.il_doccheckin  ) 	
									and Action_status = :gnv_docusign.il_incomplete and (es_docid = :lstr_sertifi_doc.as_es_docid) ;			
...
If ll_ret <> 0 Then
	...
	Else
		if gb_message = true then 
			gnv_debug.of_message("Docusign Error"+ String(ll_ret) +" : Failed to create the SOAP instance, please contact your system administrator or Contract Logix support for additional help.")
		else
			Messagebox('Docusign Error' + String(ll_ret) ,'Failed to create the SOAP instance, please contact your system administrator or Contract Logix support for additional help.')
		end if 
	End If
	Return False 
End If
...
if ib_ping_server then //Added By Jay Chen 07-24-2014
	If Not of_ping_sertifi_server( ) Then
		if gb_message = true then 
			gnv_debug.of_message("Docusign Error: Cannot connect to Docusign server. You may experience some network connection issue or failed to install the SOAP toolkit.~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
		else
			Messagebox('Docusign Error','Cannot connect to Docusign server. You may experience some network connection issue or failed to install the SOAP toolkit.~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.')
		end if 
		Return False
	End If
end if
...
If ab_ping_apicode Then
	if ib_ping_server then //Added By Jay Chen 07-24-2014
		...
		Choose Case li_ret 
			...
			Case -2
				if gb_message = true then 
					gnv_debug.of_message("Docusign Error: Cannot connect to Docusign server. You may experience some network connection issue.~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
				else
					Messagebox('Docusign Error','Cannot connect to Docusign server. You may experience some network connection issue.~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.')
				end if 
				Return False
		End Choose
	end if
...
//	If ab_showerror Then Messagebox('Error',ls_errormessage+"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
...
Catch (Throwable	E2)
	if gb_message = true then 
		gnv_debug.of_message("Docusign Error(of_ping_apicode): "+e2.text+"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
	else
		Messagebox('Error',e2.text+"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
	end if 
...
Catch (Throwable	E2)	
	ib_isvalid_sertifi = False
	if gb_message = true then 
		gnv_debug.of_message("Docusign Error(of_get_doc_status("+astr_sertifi_doc.as_es_fileid+" , "+astr_sertifi_doc.as_es_docid+")): "+E2.text +"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
	else
		Messagebox('Error', E2.text +"~r~n~r~nPlease contact your system administrator or Contract Logix support for additional help.")
	end if 
End Try
