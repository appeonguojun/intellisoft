...
st_recs.Text = String(rc)
For r = 1 To rc
	rec = dw_hosp.GetItemNumber(r,"rec_id")
	prac = dw_hosp.GetItemNumber(r,"prac_id")
	//dw_hlink.settransobject(sqlca)
	//sc = dw_hlink.retrieve(rec)
	dw_affil.SetTransObject(sqlca)
	fc = dw_affil.Retrieve(prac)
	//messagebox("fc",fc)
	For f = 1 To fc
		fac = dw_affil.GetItemNumber(f,"parent_facility_id")
		//messagebox("sc",sc)
		Choose Case is_dw
			Case "S"
				Select count(specialty_id) Into :sc From pd_spec_facility_link Where specialty_id = :rec And facility_id = :fac;
			Case "H"
				Select count(hosp_affil_id) Into :sc From pd_hosp_facility_link Where hosp_affil_id = :rec And facility_id = :fac;
			Case "A"
				Select count(address_id) Into :sc From pd_address_link Where address_id = :rec And facility_id = :fac;
		End Choose
		//messagebox("sc",sc)
		If sc = 0 Then
			If is_dw = "S" Then
			...
			dw_hlink.SetItem(nr,"rec_id",rec)
			dw_hlink.SetItem(nr,"facility_id",fac)
			dw_hlink.SetItem(nr,"p_set",pracing)
			dw_hlink.SetItem(nr,"prac_id",prac)
			If is_dw = "A" Then
