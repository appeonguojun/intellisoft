...
long textcolor = 33554432
long backcolor = 67108864
string text = "The default API code is Contract Logix, but you can obtain your own API code from https://www.docusign.com/"
...
for ll_Cycle = 1 to RowCount
	ls_Password = This.GetItemString(ll_Cycle,"docusign_password")
	...
	This.SetItem(ll_Cycle,"docusign_password",ls_Password)
	This.SetItemStatus(ll_Cycle,"docusign_password",Primary!,NotModified!)
next
...
for ll_Cycle = 1 to ll_Count
	ls_InputPass = This.GetItemString(ll_Cycle,"docusign_password")
	
	SELECT docusign_password INTO :ls_DBPass FROM icred_settings  ;
	
	if IsNull(ls_InputPass) then
		if Not IsNull(ls_DBPass) then
			UPDATE icred_settings SET docusign_password = NULL   ;
		...
		if IsNull(ls_DBPass) or ls_InputPass <> ls_DBPass then
			UPDATE icred_settings SET docusign_password = :ls_InputPass, encrypt_esign_pwd = 1  ;
