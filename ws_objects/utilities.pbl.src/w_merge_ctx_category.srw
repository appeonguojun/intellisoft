...
If Isnull(ll_source_category) Then
	Messagebox('ContractLogix','The Source Contract Category can not be empty.')
	dw_source.SetFocus()
	Return -1
End If

If Isnull(ll_target_category) Then
	Messagebox('ContractLogix','The Target Contract Category can not be empty.')
	dw_target.SetFocus()
	Return -1
End If

If ll_source_category = ll_target_category Then
	Messagebox('ContractLogix','The Target Contract Category and the Source Contract Category can not be the same value.')
	dw_target.SetFocus()
	Return -1	
End If
...
If Messagebox('Merge Contract Category','Are you sure you want to merge the contract category?',Question!,YesNo!) = 1 Then
	...
	If li_ret < 0 Then
		Messagebox('ContractLogix','Failed to merge the contract category. ~r~n'+ls_err+'~r~nPlease call support.',Exclamation!)
	Else
		Messagebox('ContractLogix','Merge successfully. Please re-login the application.')
	End If
