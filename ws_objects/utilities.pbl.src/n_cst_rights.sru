...
iS_paths[upperbound(iS_paths) + 1] = gs_dir_path + gs_DefDirName + '\' //Start Code Change ---- 04.26.2006 #423 maha
...
ls_dir_list[10] = gs_dir_path + gs_DefDirName + "\Request"					//Added By Mark Lee 11/11/2013	for requestwebform document file
...
Choose Case li_err_type
	Case -1
		Messagebox('Error','The Contract Logix system could not create the REQUIRED temporary working directory ' + ls_dir_list[i] + '.  This is typically due to a Windows OS permissions issue.  Your IT department may have not given you the appropriate rights to create directories under the ' + ls_parent_dir + ' directory and this is required to setup the Contract Logix system appropriately.' + &
	...
	Case -2
		Messagebox('Error','The Contract Logix system could not create the REQUIRED temporary file in the temporary directory ' + ls_dir_list[i] + '.  This is typically due to a Windows OS permissions issue.  Your IT department may have not given you the appropriate rights to create file under the ' + ls_dir_list[i] + ' directory and this is required to setup the Contract Logix system appropriately.' + &
			...
			'~r~nOpen menu System -> Personal Settings -> Temporary Files Settings, and then click the Browse button, select a directory that you have full permissions, and then click the Save button.',Exclamation!)
	Case -3
		Messagebox('Error','The Contract Logix system could not write to a file in the temporary directory ' + ls_dir_list[i] + '.  This is typically due to a Windows OS permissions issue.  Your IT department may have not given you the appropriate rights to write to file under the ' + ls_dir_list[i] + ' directory and this is required to setup the Contract Logix system appropriately.' + &
...
public function integer of_check_dir_right (string as_dir, boolean ab_showmessage, string as_title);Integer li_err_type = 1,li_FileNum
String ls_test_file
integer li_pos_dot, li_pos_line

as_dir = gnv_string.of_replace_env_var(as_dir) //Added By Ken.Guo 12/10/2013
...
gnv_appeondll.of_parsepath(as_dir)
//If Not DirectoryExists(as_dir) Then
If Not gf_check_programfile_dir(as_dir) Then //Added By Jay Chen 03-27-2014
	li_err_type = -1
Else
	ls_test_file = as_dir + '\' + INTELLI_TEST_FILE
	li_FileNum = FileOpen( ls_test_file ,LineMode!, Write!, LockWrite!, Append!)
...
If li_err_type < 0 and ab_showmessage Then
	Messagebox(as_title+ ' Error','The Contract Logix system could not write to a file in the directory "' +as_dir+ '".  This is typically due to a Windows OS permissions issue.  Your IT department may have not given you the appropriate rights to write to file under the "' + as_dir + '" directory and this is required to setup the Contract Logix system appropriately.'  ,Exclamation!)
End If
...
event constructor;ids_paths = create DataStore
ids_paths.dataobject = 'ds_paths'
ids_paths.settransobject(SQLCA)
ids_paths.RETRIEVE()
inv_filesrv = create n_cst_filesrvwin32

intelli_test_file  = gs_user_id +'_test'+String(now(),'hhmmss')+'.TXT'
end event
