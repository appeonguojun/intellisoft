...
string		ls_sql //Added by Harry 20170525
...
for ll_Cycle = 1 to ll_ViewCnt
	...
	if SQLCA.SQLCode < 0 then
		...
		//Added By Jay Chen 12-17-2013 Remark:import picture painter from SourceDB to TargetDB
		DECLARE lup_ImportPicture PROCEDURE FOR up_importpictures
			  	:ls_SourceDBName,
			  	:ls_TargetDBName,
				:ls_ViewID;
		EXECUTE lup_ImportPicture;
		if SQLCA.SQLCode < 0 then
			ls_Error += "Import Pictures : "+ "~n" + SQLCA.SQLErrText + "~n~n"
			ROLLBACK;
		end if
		close lup_ImportPicture;
		//end add
		
		//Added by harry for task776
		ls_sql = "INSERT INTO " + ls_TargetDBName + ".dbo.screen_field_select_item_attribute(view_id, screen_id, object_name, object_type, object_format) "  + &
					" SELECT " + String(ll_MaxViewID) + ", screen_id, object_name, object_type, object_format "  + &
					" FROM " + ls_SourceDBName + ".dbo.screen_field_select_item_attribute WHERE view_id = " + ls_ViewID + "; "
