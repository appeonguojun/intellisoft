...
if cbx_entity.checked then
	...
	ls_val = upper(ls_val)
	if len(sle_1.text) > 0 then //check for mid
		li_mid = integer(sle_1.text)
		if li_mid > 0  then //if a valid number
			if li_mid < len(ls_val) then //if the len of the value is less than mid number
				ls_sign = " like "
				if rb_start.checked then
					ls_val = mid(ls_val,1,li_mid) + "% "
...
for i = 1 to UpperBound(ls_abb)
	li_pos = pos(ls_val,ls_abb[i],1)
	if li_pos > 0 then
		ls_val = mid(ls_val,1,li_pos -1) + ls_full[i] + mid(ls_val,li_pos + li_len[i])
	end if
next
...
if len(as_string) > 0 then //check for mid
	li_mid = integer(sle_1.text)
	if li_mid > 0  then //if a valid number
		ls_val = mid(ls_val,1,li_mid)
	end if
...
if ab_new then
	...
	if is_type = "C" then
		dw_existing.setitem(al_exist,"code",upper(mid(dw_import.getitemstring(al_imp,"description"),1, 6)))
	elseif is_type = "A" then
		dw_existing.setitem(al_exist,"code",upper(mid(dw_import.getitemstring(al_imp,"entity_name"),1, 6)))
	end if
...
if i = 0 then
	ls_file = gs_dir_path + gs_DefDirName + "\caqh_tables_import.txt"
	if not fileexists(ls_file) then 
		messagebox("File not found","The CAQH table import file ( " + ls_file + " ) does not exist. Unable to continue.  Contact ContractLogix support.")
		return -1
	else
...
ls_a = gs_dir_path + gs_DefDirName + "\caqh_import_addr.txt"
ls_c = gs_dir_path + gs_DefDirName + "\caqh_import_code.txt"
...
//create parm
ls_table = mid(ls_type,1,1) + "*" + is_tname
...
if is_type = "A" then
	if dw_import.rowcount() > 0 then
		if len(dw_import.getitemstring(1,"street1")) > 0 then
			of_field_to_match("SZ")
		else
			of_field_to_match("E")
		end if
...
string title = "ContractLogix Lookups"
