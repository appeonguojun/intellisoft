...
if not fileexists(spath + gs_DefDirName + "\intellicred.exe") then
	res = messagebox(spath + gs_DefDirName + "\intellicred.exe","The entered path is not valid for the IntelliCred/App program.  Do you wish to set  the key anyway?", Question!,yesno!,2)
	if res = 2 then
		return
	end if
...
//	RegistrySet( "HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\intellicred\", "installpath", RegString!, spath )
	RegistrySet( "HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\intellicred\", "installpath", RegString!, spath )
Else
	//Modified By Mark Lee 04/18/12
//	gnv_appeondll.of_RegistrySet( "HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\intellicred\", "installpath", RegString!, spath )
	gnv_appeondll.of_RegistrySet( "HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\intellicred\", "installpath", RegString!, spath )
End If
//---------------------------- APPEON END ----------------------------
