...
//Added by Harry 20170410 for task 746
datawindowchild ldwc_child
Long ll_currow
if dw_filter.GetChild( 'category',ldwc_child) = 1 then
	ldwc_child.SetTransobject( sqlca)
	ldwc_child.retrieve()
	ll_currow = ldwc_child.insertrow(1)
	ldwc_child.setitem(ll_currow, 'code', 'ALL')
	ldwc_child.setitem(ll_currow, 'lookup_code', -999)
end if

if dw_filter.GetChild( 'contract_type',ldwc_child) = 1 then
	ldwc_child.SetTransobject( sqlca)
	ldwc_child.retrieve('contract type')
	ll_currow = ldwc_child.insertrow(1)
	ldwc_child.setitem(ll_currow, 'code', 'ALL')
	ldwc_child.setitem(ll_currow, 'lookup_code', -999)
end if
...
//Commented by Harry 20170410 for task 746
//If ll_ctx_id > 0 Then
...
ll_ctx_id2 = This.GetItemNumber(1,'ctx_id2')

If ll_ctx_id > 0 and  ll_ctx_id2 > 0 Then
	ls_filter = ' ctx_am_document_ctx_id >= ' + String(ll_ctx_id) + ' and ctx_am_document_ctx_id <= ' + String(ll_ctx_id2) 
ElseIf ll_ctx_id > 0 Then
	ls_filter = ' ctx_am_document_ctx_id = ' + String(ll_ctx_id)
ElseIf ll_ctx_id2 > 0 Then
	ls_filter = ' ctx_am_document_ctx_id = ' + String(ll_ctx_id2)	
End If

ll_category = This.GetItemNumber(1,'category')
If ll_category <> -999 and Not isnull(ll_category) Then
	If ls_filter <> '' Then
		ls_filter +=  ' and category = ' + String(ll_category)
	Else
		ls_filter +=  ' category = ' + String(ll_category)
	End If
End If

ll_contract_type = This.GetItemNumber(1,'contract_type')
If ll_contract_type <> -999 and Not isnull(ll_contract_type) Then
	If ls_filter <> '' Then
		ls_filter +=  ' and contract_type = ' + String(ll_contract_type)
	Else
		ls_filter +=  ' contract_type = ' + String(ll_contract_type)
	End If
End If
//End Added
