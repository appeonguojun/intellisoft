...
String ls_file
String ls_outpath
IF Len(Trim(sle_path.Text)) > 0 THEN
...
IF ll_return > 0 THEN
	...
	FOR li_i = 1 To ll_return
		...
		//0:no process
		//1:Successful
		ls_path = Mid(ls_encry,1,Len(ls_encry) - 1)
		ls_file = Is_str[li_i].sqlfilename
		ls_outpath = Mid(Is_str[li_i].sqlfilepath,1,Len(Is_str[li_i].sqlfilepath) - 1)
