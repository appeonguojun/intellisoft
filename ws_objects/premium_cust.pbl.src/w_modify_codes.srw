...
//		SetCapture(Handle(this)) // keeps the hourglass solid
		ll_start = Cpu()
			...
			FOR ll_i = 1  TO  upperbound(is_file_names)  //is_file_names created in lnv_string.of_parsetoarray
				IF Pos ( is_file_names[ll_i], 'Export' ) > 0 THEN CONTINUE
					IF Pos ( is_file_names[ll_i], 'dba.' ) > 0 THEN
						...
						for ll_a = 1 to ids_sys_fields.ROWCOUNT()
							if ids_sys_fields.OBJECT.sys_fields_field_name[ll_a] = ids_credantial_dws[ll_j].Describe("#" + String(ll_K) + ".name") then
								...
								for ll_H = 1 to ids_credantial_dws[ll_j].rowcount() //...go through the import data window rows
									if lower(mid(ls_col_type,1,4)) = "char" then //trap for character fields maha 082905
...
//	gotoError:
	...
	//else 
		...
		IF LEN(SQLCA.SQLErrText) < 1 THEN
...
//	if GetFileOpenName("Select File For Import",  is_docname, is_named, "CPT", "ContractLogix Export Files (*.CPT),*.CPT," ) = 1 then	//Added By Mark Lee 08/15/2014
	if  ln_getfilename.of_getopenfilename("Select File For Import",  is_docname, is_named, "CPT", "ContractLogix Export Files (*.CPT),*.CPT," ) = 1 then	
		gf_save_dir_path(is_docname) //Added by Ken.Guo on 2009-03-10
...
ls_loc = as_fpath
is_docname = mid(as_fpath,1,len(as_fpath) - 4)
ls_loc = '"' + as_fpath + '"'
if not fileexists(as_fpath) then
	messagebox("Decription","Unable to find file " + as_fpath)
	return -1
end if
runpath = gs_dir_path + gs_DefDirName + "\encrypt\ccrypt.exe --decrypt --key 70157 " + ls_loc 
//messagebox("",runpath)
i = run(runpath)
