...
If Not gb_autozoom Then Return 1 //added by gavins 20130107
...
FOR ll_count = 1 to UpperBound(adw_control_array[])
   ...
   if   TypeOf(adw_control_array[ll_count]) = datawindow! then
		   ldw = adw_control_array[ll_count]
			...
			//Modified By Ken.Guo 01/14/2013. maybe the dataobject is empty if user have no access of the registry.
			//IF len(string(ldw.dataobject)) > 1 then
			IF len(string(ldw.Describe('datawindow.Type'))) > 1 then

			//Begin - Added By Ken.Guo 11/14/2011. because it will trigger rowfocuschanged event when set zoom
			//If ldw.dataobject <> 'd_sp_contract_search' and ldw.dataobject <> 'd_agreement_template_browse' and ldw.dataobject <> 'd_ctx_letter_status' and ldw.dataobject <> 'd_dm_doc_ai_graph' Then
    			// ldw.object.datawindow.zoom = li_zoom
			//End If
			If ldw.dataobject <> 'd_ctx_letter_status' and ldw.dataobject <> 'd_dm_doc_ai_graph' Then
				...
				End If
			End If
			//End - Added By Ken.Guo 11/14/2011.
