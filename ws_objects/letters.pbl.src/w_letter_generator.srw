...
integer width = 3666
integer height = 1956
string title = "Practitioner Correspondence Generator"
...
ELSEIF ls_wizard_dw = "d_crit_pracs" THEN
	...
	tab_ltr.tabpage_criteria.cb_search.Visible = True
	tab_ltr.tabpage_criteria.dw_criteria.Title = "Select Practitioner"
...
//gs_save_path = gs_dir_path + gs_DefDirName + "\Standard\Letters\"
//debugbreak()
	...
	//End Code Change---10.03.2006
				...
				CHOOSE CASE tab_ltr.tabpage_criteria.dw_letter.DataObject
					CASE "d_ltr_general_corrospondence", "d_ltr_net_dev"
						tab_ltr.tabpage_criteria.dw_letter.of_SetTransObject( SQLCA )
						li_row_cnt = tab_ltr.tabpage_criteria.dw_letter.retrieve( il_facility_id, il_prac_ids )				 
					CASE "d_ltr_net_dev_app_rcvd_prac", "d_ltr_net_dev_app_rcvd_payer"
						tab_ltr.tabpage_criteria.dw_letter.of_SetTransObject( SQLCA )
						tab_ltr.tabpage_criteria.dw_letter.retrieve( il_prac_ids, il_facility_ids_all, ii_app_ids )				 
						li_row_cnt = tab_ltr.tabpage_criteria.dw_letter.RowCount( )
					...
					CASE "d_ltr_net_dev_app_rcvd_prac_all", "d_ltr_net_dev_app_rcvd_payer_all"
						...
						//---------------------------- APPEON END ----------------------------
						li_row_cnt = tab_ltr.tabpage_criteria.dw_letter.retrieve( il_prac_ids, il_facility_ids_all, ii_app_ids )
						IF li_row_cnt = 0 THEN
				...
				FOR i = 1 TO li_num_pracs
					...
					next
					if li_check = 0 then MessageBox("Error", "Unable to create letter for practitioner # " + string(il_prac_ids[i]) + " ("+ ls_full_name +") ~n~r1) Check to see if there is a mailing address link for the selected facility.~n~r2) Make sure that that address is active.~n~r3) Make sure the Appointment Status for this practitioner is active or pending for the selected facility.~n~r4) Be sure the practitioner has a specialty of Primary order.")
				NEXT
				//\maha
...
//---------------------------- APPEON END ----------------------------
		...
		IF li_set6 = 1 THEN
			if upper(ls_letter_path) = upper(ls_run_path ) then //maha 050602 trap against copying to same location
				messagebox("Path error","Your letter path is set to " + gs_dir_path + gs_DefDirName + "\letters\standard\ ~rand the Copy Letters setting is turned on. ~rGo to system->utilities->Application settings and set copy letters to No to run letters normally.")
				return -1
			end if
			...
			//ls_to = gs_dir_path + gs_DefDirName + "\letters\standard\" + ls_doc_name
			ls_to = ls_run_path + ls_doc_name
			//copy letter to c drive then run 
			//FileDelete( gs_dir_path + gs_DefDirName + "\letters\standard\" + ls_doc_name )
...
IF li_set6 = 1 THEN
			if upper(ls_letter_path) = upper(gs_dir_path + gs_DefDirName + "\letters\standard\") then //maha 050602 trap against copying to same location
				messagebox("Path error","Your letter path is set to " + gs_dir_path + gs_DefDirName + "\letters\standard\ ~rand the Copy Letters setting is turned on. ~rGo to system->utilities->Application settings and set copy letters to No to run letters normally.")
				RETURN  failure
			end if
			//check to make sure letter and path exists
...
CHOOSE CASE ll_qa_profile_level
	...
	CASE 1
		...
		ids_physician.retrieve(il_prac_ids[1])
	...
	CASE 2
		...
		ids_physician.of_SetTransObject( SQLCA)
 		ids_physician.retrieve(il_facility_id)
...
IF ib_from_prac_folder THEN
	li_ret = tab_ltr.tabpage_criteria.dw_select_report.SetFilter( "letter_name = 'Credentialing Application Signature Follow up (Individual)' OR letter_name = 'Payer Follow up Letter (Individual)'" )
	tab_ltr.tabpage_criteria.dw_select_report.Filter( )
ELSE
	li_ret = tab_ltr.tabpage_criteria.dw_select_report.SetFilter( "letter_name = 'Credentialing Application Signature Follow up (ALL)' OR letter_name = 'Payer Follow up Letter (All)'" )
	tab_ltr.tabpage_criteria.dw_select_report.Filter( )
END IF
...
//messagebox("ai_exp_id",ai_exp_id)


//select export_name into :ls_exp_name from export_header where export_id = :ai_exp_id;
//
//if isnull(ls_exp_name) or ls_exp_name = "" then
//	return -1
//end if

//ls_path = gs_dir_path + gs_DefDirName + "\letters\data\"// + ls_exp_name + ".txt" cahnged 022003 the file name is set in the export routine
...
//Added by Nova on 2008-08-13  .//Ken.Guo 2008-09-27
