...
type st_tip from statictext within w_export_expression
end type
...
Long il_screenid //Added By Ken.Guo 2015-08-27
...
string is_exptype //Added By Ken.Guo 2016-08-19
end variables
...
public function integer of_check_12 ()
end prototypes
...
event ue_init_function(integer ai_type);If ai_type = 12 Then//added by gavins 20160818
	...
	is_Function[7] = "CASE x WHEN a THEN b WHEN c THEN d... ELSE e END";ii_SelStart[7] = 6; ii_SelNum[7] = 1
	is_Function[8] = "CASE WHEN x = a then b WHEN x = c THEN d... ELSE e END";ii_SelStart[8] = 11; ii_SelNum[8] = 1
Elseif ai_type = 4 then
	...
	if isvalid(w_contract_data_design) then //Added By Jay Chen 10-17-2014
		if w_contract_data_design.uo_design.ii_screen_id = 3 then
			is_Function[108] = "gf_getchildcontract_number(master_contract_id, ctx_id)";ii_SelStart[108] = 56; ii_SelNum[108] = 0
			is_Function[109] = "f_get_loginuser()";ii_SelStart[109] = 19; ii_SelNum[109] = 0 //added by gavins 20150819
		Else
			is_Function[108] = "f_get_loginuser()";ii_SelStart[108] = 19; ii_SelNum[108] = 0 //added by gavins 20150819			
		end if
	end if
...
for ll_Num = 1 to ll_Cnt
	...
	if istr_pass.s_long[1] = 3 then	//Advanced Update in workflow call -- jervis 03.12.2009
		if pos(ls_Field,'[') > 0 and pos(ls_Field,']') > 0 then
			ll_pos = pos(ls_Field,'[')
...
For li_i = 1 To UpperBound( ls_Validation )
	If Pos( ls_Source, '~r~n' + ls_Validation[li_i] ) > 0 Then
...
Long    ll_Ret = 0
Long    ll_Pos,ll_LeftBracketCnts,ll_RightBracketCnts
String  ls_Expression,ls_Err,ls_Msg,ls_Systax,ls_value
n_cst_string lnvo_string
n_ds    lds
String	  ls_Validation[]
Integer li_i
...
If as_attribute = 'compute' Then	
	...
	lds.modify(ls_Modify)
	 ...
	 If ls_Err = '' Then
		...
		If len(ls_coltype_target) < 3 Then ls_Err = 'Invalid compute expression.' //Workaround APB bug. APB cannot verify the compute expression. so check the datatype.		
		...
		End Choose				
		If istr_pass.s_long[1] = 7 and ls_coltype_source2 <> ls_coltype_target2 and ls_Err = '' Then			
	 ...
	 End If
ElseIf  as_attribute = 'compute_condition' Then	 //Added By Ken.Guo 2016-08-18
	ls_Modify = 'create compute(band=detail alignment="0" expression="" border="5" color="0" x="0" y="0" height="52" width="520"  name=compute_field_temp font.face="MS Sans Serif" font.height="-8" font.weight="400"  font.family="2" font.pitch="2" font.charset="0" background.mode="1" background.color="553648127" )'
	lds.modify(ls_Modify)
	 ls_Err = lds.modify('compute_field_temp.expression="' +ls_Expression+ '"')
	 If Len(lds.Describe("compute_field_temp.coltype")) < 3 Then  ls_Err = 'Invalid compute expression.' //Workaround APB bug. APB cannot verify the compute expression. so check the datatype.
	 If ls_Err = '' Then
		lds.insertrow(0)
		ls_value = trim(lower(lds.describe('Evaluate("'+ls_Expression+'",1)')))
		If (ls_value <> 'false' and ls_value <> 'true') and appeongetclienttype() = 'PB' Then
			Messagebox('Condition Expression ' + ls_value,"Invalid conditional expression. The expression must return True or False, such as 'custom_1 = custom_2', 'ctx_id > 100' or '1 <> 1'")
			if IsValid(lds) then destroy lds
			Return -1		
		End If
		If Left(lds.Describe("compute_field_temp.coltype"),4) <> 'char' and appeongetclienttype() = 'WEB' Then //workaround APB bug. ls_value maybe return '' for web. so only check the data type.
			Messagebox('Condition Expression ' + ls_value,"Invalid conditional expression. The expression must return True or False, such as 'custom_1 = custom_2', 'ctx_id > 100' or '1 <> 1'")
			if IsValid(lds) then destroy lds
			Return -1		
		End If		
		If (ls_value <> '' and ls_value <> 'false' and ls_value <> 'true') and appeongetclienttype() = 'WEB' Then //workaround APB bug. if ls_value return data, check the data.
			Messagebox('Condition Expression ' + ls_value,"Invalid conditional expression. The expression must return True or False, such as 'custom_1 = custom_2', 'ctx_id > 100' or '1 <> 1'")
			if IsValid(lds) then destroy lds
			Return -1		
		End If		
		
		is_exptype =  lds.Describe("compute_field_temp.coltype")
	End If
Else
...
public function integer of_check_12 ();String 	ls_Expression, ls_lower_exp
Boolean	lb_Succ, lb_Multi_Check, lb_Col_Regular
String		ls_Multi_Rows[], ls_Full_Name, ls_Col, ls_Table
String		ls_SQL, ls_From
Integer	li_i, li_Pos
...
ls_Multi_Rows = { "ctx_products", "ctx_fee_sched_nm", "ctx_custom_multi_hdr", "ctx_custom_multi_detail" }

For li_i =1 To UpperBound( ls_Multi_Rows )
	If  Pos( Lower( ls_Expression ), ls_Multi_Rows[li_i] ) > 0 Then
		lb_Multi_Check = True
		lb_Succ = False
	End If
Next
...
For li_i = 1 To dw_1.RowCount( )
	ls_Full_Name = 	dw_1.GetItemString(li_i,'export_field_name')	
	ls_Col = Mid( ls_Full_Name , Pos( ls_Full_Name , '.' ) + 1 )
	ls_Table = Left( ls_Full_Name, Pos( ls_Full_Name , '.' ) - 1 )
	If Pos (ls_From, ls_Table ) = 0 Then  ls_From =  ls_From + ls_Table + ','
	
	li_Pos = Pos(  Lower(ls_Expression), Lower( ls_Col ) )
	If li_Pos > 0 Then
		If Pos( Lower(ls_Expression), Lower(ls_Full_Name) ) <=0 Then
			lb_Col_Regular = True
			lb_Succ = false
		End If
	End If	
...
If lb_Succ Then
	ls_SQL = 'Select top 1 ' + ls_Expression + '  FROM ' + Left( ls_From , Len( ls_From ) - 1 )
	...
	End If
Else
	If lb_Multi_Check Then
		MessageBox( gnv_app.iapp_object.DisplayName, 'The expression cannot contain multi-row fields from the following tables "ctx_products", "ctx_fee_sched_nm", "ctx_custom_multi_hdr" and "ctx_custom_multi_detail" ', StopSign! )
	ElseIf lb_Col_Regular Then
		MessageBox( gnv_app.iapp_object.DisplayName, 'Please use [table name].[field name] to reference a field in the expression. For example: ctx_basic_info.custom_1.' ,StopSign!)
	Else
		Messagebox(gnv_app.iapp_object.DisplayName,"Expression is not valid.",StopSign!)
	End If
	mle_1.setfocus( )
...
this.Control[iCurrent+2]=this.st_expression
this.Control[iCurrent+3]=this.sle_labeltext
this.Control[iCurrent+4]=this.st_labeltext
this.Control[iCurrent+5]=this.lb_functions
...
//12 used by gavins 20160818
//if istr_pass.l_facility_id = -1 then//screen call
Choose Case istr_pass.s_long[1]
	Case 2, 6,7,10,11,21  //2-Screen Painter Call    Added by gavins 20120504   add 6  //Added By Ken.Guo 2016-08-16.added 7,10,11,21 
		...
		SetRedraw(True)
		//end of add
		st_tip.text = "Please input a PowerBuilder Expression." //Added By Jay Chen 03-28-2014
		...
		If  istr_pass.s_long[1] = 7 or istr_pass.s_long[1] =10 or istr_pass.s_long[1] = 11 or istr_pass.s_long[1] = 21  Then //Added By Ken.Guo 2016-08-16
			//
		Else			
			...
			if IsNull(istr_pass.s_string) or Trim(istr_pass.s_string) = "" then
				...
				if ls_LabelName = "!" then
					...
					st_tip.y = st_tip.y + 45 //Added By Jay Chen 03-28-2014
	...
	Case 3 //3-Advanced update in workflow Call	- jervis 03.12.2009
		...
		dw_1.Width = gb_2.Width - 64 //124
		SetRedraw(True)
		st_tip.text = "Please input a T-SQL Expression." //Added By Jay Chen 03-28-2014
		dw_1.DataObject = 'd_export_fields'
...
//				dw_1.SetItem(ll_Row,"export_field_name",ldwc_child.GetItemString(ll_num,'field_dbname'))
	...
	Case 5 //added by gavins 20120503   compute
		...
		cb_1.visible = true
		SetRedraw(True)
		if lower(is_expression) = 'visible' or lower(is_expression) = 'x' or lower(is_expression) = 'y' or lower(is_expression) = 'width' or lower(is_expression) = 'height' or pos(lower(is_expression),'color') > 0 then //Added By Jay Chen 05-24-2014
			st_tip.text = "Please input a PowerBuilder Expression."
		else
			st_tip.text = "Please input a T-SQL or PowerBuilder Expression." //Added By Jay Chen 03-28-2014
		end if
		...
		If dw_1.RowCount( ) > 0 Then
			dw_1.SelectRow( 0, False)
		End If
	Case 12 //request form painter  //added by gavins 20160818
		...
		lds_Source.DataObject = 'd_request_form_detail'
		lds_Source.SetTransObject( SQLCA )
		lds_Source.Retrieve( istr_pass.s_long[2]  )
		lds_Source.SetFilter( "design = '" + istr_pass.s_string_array[4] + "'" )
		lds_Source.Filter( )
		...
		For ll_Num = 1 To ll_Cnt
			ll_Row = dw_1.InsertRow(0)
			ls_Table_Name = 	lds_Source.GetItemString(ll_num,'table_name')	
			If Len( ls_Table_Name ) = 0 Or Len( lds_Source.GetItemString(ll_num,'col_name') ) = 0 Or Isnull( ls_Table_Name) Or IsNull( lds_Source.GetItemString(ll_num,'col_name') )   Then Continue
			dw_1.SetItem(ll_Row,"export_field_name", ls_Table_Name + "." + lds_Source.GetItemString(ll_num,'col_name'))
			
			dw_1.SetItem(ll_Row,"export_field_label",lds_Source.GetItemString(ll_num,'export_field_name'))
		Next
		
		st_tip.Text = "Supported operators:	= , > , < , >=, <=" + "~r~n" + "Examples:			CASE WHEN ctx_basic_info.custom_1 = ‘A’ THEN 1 ELSE 0 END "
...
//Added By Ken.Guo 2015-08-27
If Upperbound( istr_pass.s_long[]) > 1 Then 
	If istr_pass.s_long[2] > 0 Then
		il_screenid = istr_pass.s_long[2]
	End if
End If
...
//Added By Ken.Guo 2015-08-27. need added table name for T-SQL
If  Pos(st_tip.text, 'T-SQL') > 0 and  Pos(st_tip.text, 'PowerBuilder') = 0 Then
	Choose Case Parent.il_screenid
		Case 10 // Custom 2
			is_CurrColumn = 'ctx_custom.' + is_CurrColumn
	End Choose
End If
...
Choose Case istr_pass.s_long[1]
	...
	Case 7 //Added By Ken.Guo 2016-08-16
		If of_check_screen( 'compute' ) = -1 Then Return
	Case 10 ////Added By Ken.Guo 2016-08-18
		If of_check_screen( 'compute_condition' ) = -1 Then Return
	Case 11,21 //Added By Ken.Guo 2016-08-18
		If of_check_screen('compute') = -1 Then Return
		lstr_pass.s_string_array[2] = is_exptype
	Case 12//Added by gavins 20160818
		If of_Check_12( ) < 0 Then Return
End Choose
...
Choose Case istr_pass.s_long[1]
	...
	Case 5  
		...
		End If
	Case 7 //Added By Ken.Guo 2016-08-16
		If of_check_screen( 'compute' ) = 0 Then
			MessageBox(gnv_app.iapp_object.DisplayName,"Expression is OK.")
		End If
	Case 10  //Added By Ken.Guo 2016-08-18 condition
		If of_check_screen( 'compute_condition' ) = 0 Then
			MessageBox(gnv_app.iapp_object.DisplayName,"Expression is OK.")
		End If		
	Case 11,21 //Added By Ken.Guo 2016-08-16 field expression
		If of_check_screen('compute') = 0 Then
			MessageBox(gnv_app.iapp_object.DisplayName,"Expression is OK.")
		End If		
	Case 12  //added by gavins 20160818
		If of_Check_12( ) > 0 Then
			MessageBox(gnv_app.iapp_object.DisplayName,"Expression is OK.")
		End If
End Choose
