...
type cb_hide from commandbutton within w_auto_export
end type
type dw_autoexport from datawindow within w_auto_export
end type
...
string title = "Contract Logix Automatic Export"
...
yield()																		//Added By Mark Lee 03/12/2015
...
If gb_contract_version Then
	this.title = "Contract Logix Automatic Export"
end if
//end of add
...
if gb_AutoExport then Halt close
end event
...
//Added By Mark Lee 09/23/2015
ls_msg = "The Auto-Export function will stop working after you close this window.~r~n"+&
		"If you do not want to terminate the Auto-Export function, please minimize the window instead.~r~n"+&
...
integer height = 120
integer taborder = 40
string dataobject = "d_user_auto_export"
...
value = ln_getfilename.of_getopenfilename("Select Directory and enter 'Path' as file name",&
+ docname, named, "DOC", &
	+ "text Files (*.txt),*.txt,Query Files(*.sql),*.sql")
...
//	this.setitem(1,"save_to_path",path) 
	...
	if len(dw_detail.getitemstring(1,"sql_syntax")) > 0 then
...
//	+ docname, named, "DOC", &
//	+ "text Files (*.txt),*.txt")
	value = ln_getfilename.of_getsavefilename("Save As File",&
	+ docname, named, "DOC", &
	+ "text Files (*.txt),*.txt")
...
MessageBox("The Auto Export program: ","See Auto Export Manual~r~r~rCopyright "+gs_copyright+" Contract Logix Inc~r~r"+gs_user_version)			//Modified By Mark Lee 09/07/2015 add the gs_user_version 
