...
type cb_1 from commandbutton within w_email_account_edit
end type
...
String is_test_receive_errtext,is_test_send_errtext,is_test_send_rtn //Added By Jay Chen 04-22-2014
boolean ib_show_sendmsg 
end variables
...
public function integer of_createusertree (string as_userid);long i, ll_Root,ll_Root2
...
for i = 1 to ids_User.RowCount()
	...
	ll_Root2 = tv_user.InsertItemLast(ll_Root, ltvi_Item)
	//Added By Jay Chen 09-02-2015
	ls_username = trim(ids_User.GetItemString(i, "user_name"))
	if isnull(ls_username) then ls_username = ''
	if ls_username <> '' then
		ltvi_Item.Label = ls_username
		ltvi_Item.Data = ids_User.GetItemString(i, "user_id")
		ltvi_Item.PictureIndex = 4
		ltvi_Item.SelectedPictureIndex = 4
		tv_user.InsertItemLast(ll_Root2, ltvi_Item)
	end if
	//end
next
//tv_user.ExpandAll(ll_Root)
tv_user.collapseitem( ll_Root2)
...
public subroutine of_set_used_saved_pwd ();//Added By Ken.Guo 2016-06-29
...
If dw_detail.getrow() > 0 Then
	...
	If gnv_user_option.of_get_option_value('allow_ad_function') = '1' and li_ad_status = 1 and li_ad_save_pwd = 1 and Len(ls_ad_user) > 0 Then 		
		...
		//dw_browse.modify('use_ad_password.visible = "1"')
	Else
		dw_detail.modify('use_ad_password.visible = "0"')
		//dw_browse.modify('use_ad_password.visible = "0"')
	End If
End If
end subroutine
...
If gnv_data.of_getitem('icred_settings', 'set_56', False) = '1' Then
	dw_detail.modify("server_t.visible = '0' server.visible = '0' port_t.visible = '0' port.visible = '0' authmode.visible = '0' gb_1.Visible = '0' smtp_ssl.visible='0' sendprotocol.visible='0' ")
...
event clicked;tv_user.SetRedraw(False)

if this.text = '+ ALL' then
	tv_user.expandall(1)
	this.text = '- ALL'
else
	tv_user.collapseitem(1)
	this.text = '+ ALL'
end if

tv_user.SetRedraw(True)
end event
...
string 	ls_is_default
string ls_test_message //Added By Jay Chen 06-26-2014

ls_test_message = "This is a test e-mail to ensure your access to Contract Logix is properly established. Please contact your Contract Logix Administrator if you have questions about this e-mail." //Added By Jay Chen 06-26-2014
...
ls_is_default = dw_detail.GetItemString(ll_row,"is_default")			//Added By Mark 05/07/2014


//Added By Ken.Guo 2016-06-27
Integer li_ad_status,li_ad_save_pwd
String ls_ad_user,ls_userid, ls_ad_pwd
...
If gnv_data.of_getitem( 'icred_settings', 'use_web_email', False) = '1' Then
	if isnull(ls_is_default) or ls_is_default <> "Y" then 
		MessageBox(gnv_app.iapp_object.DisplayName,"The system is current set to send emails using the web service. ~r~nYou have to check the 'Set as Default Account' checkbox in order to send an account setting test email. ")
		dw_detail.SetColumn('is_default')
		dw_detail.SetFocus()
		return
	end if 
end if 

//Added By Mark 06/05/2014
If dw_detail.ModifiedCount ( )  > 0 Then
	MessageBox(gnv_app.iapp_object.DisplayName,"The data need to be saved before you can run the current step. ")
	return 
End if

//Added By Jay Chen 04-22-2014
is_test_receive_errtext = ''
is_test_send_errtext = ''
ib_show_sendmsg = false
//end add
...
If gnv_data.of_getitem('icred_settings', 'receive_email', False) = '1' Then
	If ls_Protocol = 'POP3' Then
		lnv_Mail_pop3 = Create n_cst_easymail_pop3 	 //Added By Ken.Guo 2015-07-27
		If lnv_Mail_pop3.of_createobject( ) = -1 Then
			f_show_message('create POP3 object','','','','')
			dw_detail.SetColumn('POP3_Server')
			dw_detail.SetFocus()	
			If isvalid(lnv_Mail_pop3) Then Destroy lnv_Mail_pop3 //Added By Ken.Guo 2015-07-27
			Return
		End If
		lnv_Mail_pop3.of_Setpop3Info(ls_pop3_Server, ll_pop3_Port, li_pop3ssl)
		lnv_Mail_pop3.of_SetAccount(ls_Account, ls_Password)
		lnv_Mail_pop3.ib_show_pingmsg = false //Added By Jay Chen 04-22-2014
		If lnv_Mail_pop3.of_PingServer(True) = False Then 
//			Return //Comment By Jay Chen 04-22-2014
		End If
		lnv_Mail_pop3.of_disconnect_pop3( )
		If isvalid(lnv_Mail_pop3) Then Destroy lnv_Mail_pop3 //Added By Ken.Guo 2015-07-27
	ElseIf ls_Protocol = 'IMAP4' Then//added by gavins 20121127
		lnv_Mail_imap4 = Create n_cst_easymail_imap4 //Added By Ken.Guo 2015-07-27
		If lnv_Mail_imap4.of_createobject( ) = -1 Then
			f_show_message('create imap4 object','','','','')
			dw_detail.SetColumn('POP3_Server')
			dw_detail.SetFocus()	
			If isvalid(lnv_Mail_imap4) Then Destroy lnv_Mail_imap4 //Added By Ken.Guo 2015-07-27
			Return
		End If
		lnv_Mail_imap4.of_setimap4info(ls_pop3_Server,ll_pop3_Port,li_pop3ssl)
		lnv_Mail_imap4.ib_show_pingmsg = false  //Added By Jay Chen 04-22-2014
...
//			Return //Comment By Jay Chen 04-22-2014
	...
	Else //Exchange2003/2007 //Added By Ken.Guo 07/27/2013
		lnv_Mail_eamail = Create n_cst_easymail_eamail //Added By Ken.Guo 2015-07-27
		If lnv_Mail_eamail.of_createobject( ) = -1 Then
			f_show_message('create eamail object','','','','')
			dw_detail.SetColumn('POP3_Server')
			dw_detail.SetFocus()	
			If isvalid(lnv_Mail_eamail) Then Destroy lnv_Mail_eamail //Added By Ken.Guo 2015-07-27
			Return			
		End If
		lnv_Mail_eamail.of_seteaserverinfo( lstr_email_account )
		lnv_Mail_eamail.of_setlog(True)
		lnv_Mail_eamail.ib_show_pingmsg = false  //Added By Jay Chen 04-22-2014
...
lnv_Mail.is_user_id = gs_CurAddresser 		//Added By Mark 05/07/2014 move it
If dw_detail.describe( 'server.visible' ) = '1'   Then
...
//	lnv_Mail.is_user_id = gs_CurAddresser //Added By Ken.Guo 2010-10-18.
	//Modified By Jay Chen 06-26-2014
//	ll_Return = lnv_Mail.of_SendText(ls_Email, '', '','Test Mail', 'Hello,This is a test Mail', '')
	ll_Return = lnv_Mail.of_SendText(ls_Email, '', '','Test Mail', ls_test_message , '')
	//end
...
if ll_Return = 0 then
	li_send_result = 0
//	MessageBox("Congratulation", "Email test is Successfull!") //Comment By Jay Chen 04-22-2014
else
	li_send_result = -1
	//Added by Nova.zhang on 2008-11-07
	If lstr_email_account.as_sendprotocol = 'SMTP' Then
//		f_show_message('error_code_'+string(ll_Return),'%1S%',String(ll_Return),'','')  //Comment By Jay Chen 04-22-2014
	Else
		//Added By Ken.Guo 07/04/2013
		ls_err_desc = lnv_Mail.of_GeteMailError(ll_Return)
//		f_show_message('error_code_'+string(ll_Return),'ALL',ls_err_desc,'','') //Comment By Jay Chen 04-22-2014
	End If
end if
...
if ls_errtext <> '' then is_test_send_errtext = ls_errtext
if isnull(is_test_send_errtext) then is_test_send_errtext = ''
if isnull(is_test_receive_errtext) then is_test_receive_errtext = ''
is_test_send_rtn = string(ll_Return)
if (is_test_send_errtext = '' and is_test_send_rtn = '0') and is_test_receive_errtext = '' then
	ls_pass_flag = '1'
else
	ls_pass_flag = '0'
end if
OpenWithParm(w_cust_test_account_message,ls_pass_flag)
//end add
...
If dw_detail.describe( 'server.visible' ) = '1'   Then
	...
	Values 
	(:gs_user_id,:gs_CurAddresser,:ls_Email,'','Test Mail',getdate(),'',null,null,null,null,null,'Test Email','',:li_send_result,:ls_errtext,:ls_computer_info);
End IF
...
String ls_ad_user
integer li_ad_save_pwd, li_ad_status
...
choose case dwo.name
	...
	Case "protocol" //added by gavins 20121120
		...
		End If	
//	Case 'use_ad_password' //Added By Ken.Guo 2016-06-27
//		If data = '1' Then
//			ls_userid = this.object.user_id[row]
//			select ad_status, ad_user, ad_save_pwd into :li_ad_status, :ls_ad_user, :li_ad_save_pwd  from security_users where user_id = :ls_userid;
//			If ls_ad_user = '' or isnull(ls_ad_user) Then
//				Messagebox('Use Saved AD Password','Cannot set this option. ~r~nThe current user '+ls_userid+' does not have AD login information. Please go to User Painter -> AD User to set the AD information for the user.')
//				Post SetItem(row, 'use_ad_password',0)
//			ElseIf li_ad_status = 0 or isnull(li_ad_status) Then
//				Messagebox('Use Saved AD Password','Cannot set this option. ~r~nThe current user '+ls_userid+' does not have AD login information. Please go to User Painter -> AD User to enable Status.')
//				Post SetItem(row, 'use_ad_password',0)
//			ElseIf li_ad_save_pwd = 0 or isnull(li_ad_save_pwd) Then
//				Messagebox('Use Saved AD Password','Cannot set this option. ~r~nThe current user '+ls_userid+' does not have AD Auto Login information. Please check the Remember me option on the login window or go to User Painter -> AD User to enable Save AD Password.')
//				Post SetItem(row, 'use_ad_password',0)
//			End If			
//		End If
end choose
...
//Added By Ken.Guo 2016-06-29
of_set_used_saved_pwd()
...
if dw_sysaddr.ModifiedCount() > 0 then
	dw_sysaddr.Update()
	gnv_data.of_retrieve( "icred_settings") //Added By Jay Chen 08-04-2014 remark:refresh cache data
end if
...
gnv_echosign.ib_isvalid_sertifi = false //Added By Jay Chen 10-16-2013
gnv_docusign.ib_isvalid_sertifi = false //Added By Jay Chen 04-15-2014
end event
...
dw_browse.event rowfocuschanged( ll_InsertRow) //Added By Ken.Guo 2016-06-29
...
string picturename[] = {"Library!","Custom076!","Custom026!","Tab!"}
