...
global n_cst_easymail_eamail n_cst_easymail_eamail
...
Catch (Runtimeerror err1)
	...
	if isvalid(w_email_account_edit) then
		w_email_account_edit.is_test_receive_errtext = 'Failed to connect to incoming mail Server. '+err1.text + '~r~n' +String( iole_EAClient.LastErrorDescription )
	end if
	//end add
...
If ab_show_msg Then
	//Added By Jay Chen 04-22-2014
	if isvalid(w_email_account_edit) then
		w_email_account_edit.is_test_receive_errtext = 'Failed to connect to incoming mail Server. '+ls_errtext
	end if
	//end add
	if ib_show_pingmsg then
...
//Commented By Ken.Guo 03/30/2015. for APB Bug. 
////Added By Jay Chen 01-22-2015
//boolean lb_read
//lb_read = aole_mail_info.read
//
...
li_keyword_mode = Integer(gnv_data.of_getItem("icred_settings", "Email_tag_mode", False))  //Added by harry 2017-02-04

//Commented By Ken.Guo 03/30/2015. for APB bug
////Make as unread. Only pop3 does not Support
//If iole_EAServer.Protocol > 0 and not lb_read Then
...
//Get Custom Fields
If String(lole_Mail.Headers.GetValueOfKey('X-FromContractLogix')) = 'Yes' Then
...
End If
 ...
 //Added by harry 2017-02-04
If li_keyword_mode = 1 Then //Subject Mode
	If inv_easy_smtp.of_get_keyword(lole_Mail.Subject,  ls_ctx_id, ls_doc_id, ls_ai_id) > 0 Then  lb_needdownload = True
End If
