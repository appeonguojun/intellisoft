...
global n_cst_easymail_pop3 n_cst_easymail_pop3
...
String is_checkin_filename = ''   //added by Harry 20170106
end variables
...
If ab_show_msg Then
	...
	if isvalid(w_email_account_edit) then
		w_email_account_edit.is_test_receive_errtext = 'Failed to connect to POP3 Server. '+ls_errtext
	end if
	//end add
...
IF appeongetclienttype() = 'PB' AND gs_dbtype = 'SQL' THEN
		...
		IF ib_show_msg THEN  //Add by Harry 12-28-2016 for task711
	...
	If iole_POP3Mail.Messages.Item(ll_msg_num).LoadMessage(as_eml_file,0,2,0) = 0 Then
		...
		If ls_html_dir <> '' Then
			...
			Catch ( oleruntimeerror e2)
				...
				If iole_word.documents.count = 0 then //Added By Ken.Guo 2015-07-22 Added if statement.
					iole_word.Quit(0)
				End if
...
If li_fromclx = 0 Then
		...
		ls_temp = '(' + Mid(ls_body_text,ll_start + 4,ll_end - ll_start - 4) + ')'  //modify by harry 2016-12-30
...
If as_emlfile <> '' Then //EML's attachments
	...
	If Not FileExists(as_emlfile) Then
		...
		Else
			is_checkin_msg = 'Failed to download the EML file from the database.'
	...
	If ll_msg_num  > 0 Then
		...
		For i = 1 To iole_POP3Mail.Messages.Item(ll_msg_num).attachments.count
			...
			If not ib_show_msg and len(is_checkin_filename) > 0 Then  //Modify by Harry 01-06-2017 for task711
				ls_filename_temp = gnv_string.of_globalreplace( ls_filename, '.', ' ')   //Added By Harry 2017-01-11.
				If pos(ls_filename_temp, is_checkin_filename) > 0 Then
					al_index = i
				Else
					al_index = 0
				End If
			...
			If j = al_index Then
				...
				If FileExists(ls_pathfilename) Then
					Do While 1 = 1 
						...
						If k = 100 Then
							...
							Else
								is_checkin_msg = 'Failed to create the attachment name in the folder "' + ls_attachdir + '"'
								gnv_debug.of_output(true, is_checkin_msg )
							End If
							Return -1
						End If
				...
				If ll_ret <> 0 Then
					...
					Else
						is_checkin_msg = 'Failed to export the attachment to '+ls_pathfilename+'. Error code is ' + String(ll_ret) + '.'
						gnv_debug.of_output(true, is_checkin_msg )
					End If
					Return -2
				End If	
		...
		Else
			is_checkin_msg = 'Failed to open the attachment, please call support.'
			gnv_debug.of_output(true, is_checkin_msg )
		End If
		Return -3	
	Else
		...
		Else
			is_checkin_msg = 'Failed to load the EML file, please call support.'
			gnv_debug.of_output(true, is_checkin_msg )
		End If
		Return -4
	End If
Else //Word's attachments
	...
	If ids_mail_attach.Rowcount() = 0 or al_index > ids_mail_attach.Rowcount() Then
		...
		Else
			is_checkin_msg = 'This Email item has no attachment in the database.'
			gnv_debug.of_output(true, is_checkin_msg )
		End If
		Return -1
	End If
	
	If not ib_show_msg and len(is_checkin_filename) > 0 and al_index < ids_mail_attach.Rowcount() Then  //Modify by Harry 01-06-2017 for task711
		For i = 1 to  ids_mail_attach.Rowcount()
			ls_attach_name = ids_mail_attach.GetItemString(i,'attachment_name')
			ls_filename_temp = gnv_string.of_globalreplace( ls_attach_name, '.', ' ')   //Added By Harry 2017-01-11.
			If pos(ls_filename_temp, is_checkin_filename) > 0 Then
				al_index = i
				exit
			End If
		...
		If FileExists(ls_attach_file_temp) Then
			Do While 1 = 1 
				...
				If k = 100 Then
					...
					Else
						is_checkin_msg = 'Failed to create the file with the same name of attachment in the folder "' + This.of_get_Email_attachdir( al_Emailid, 'eml') + '"'
						gnv_debug.of_output(true, is_checkin_msg )
					End If
					Return -1
				End If
...
Post SetPointer(Arrow!)
Messagebox('Import','Email file successfully imported.')			//Added By Mark Lee 07/12/2013
Return 1
	...
	If ll_msg_num  > 0 Then
		If iole_POP3Mail.Messages.Item(ll_msg_num).Attachments.Count > 0 Then
			...
			For i = 1 To iole_POP3Mail.Messages.Item(ll_msg_num).Attachments.Count
				...
				ll_size = inv_cst_filesrv.of_genfilesize(ll_size,ls_unit)
				//If ls_ref_docname <> '' and Trim(Lower(lnv_string.of_globalreplace( ls_ref_docname, '.', ' ') )) = Trim(Lower( lnv_string.of_globalreplace( ls_attachname, '.', ' ') )) Then  //Harry 2017-01-06
				If ls_ref_docname <> '' and Trim(lnv_string.of_globalreplace( lnv_string.of_globalreplace(Lower(ls_ref_docname), '.docx', '.doc'), '.', ' ')) = Trim(lnv_string.of_globalreplace( lnv_string.of_globalreplace(Lower(ls_attachname), '.docx', '.doc'), '.', ' ')) Then  //Harry 2017-01-06 
