...
type cb_refresh from commandbutton within w_security_license_user
end type
...
String ls_client_id,ls_client_secret,ls_url,ls_redirect,ls_null,ls_user_id, ls_website
long ll_row
...
ls_website = dw_1.getitemstring(ll_row,"esign_website") //Added By Ken.Guo 2017-06-20
...
if ls_client_id = '' then
	...
	dw_1.setcolumn("esign_client_id")  //Modified by Harry 2017-01-21
	return -1
end if
if ls_client_secret = '' then
	...
	dw_1.setcolumn("esign_client_secret")  //Modified by Harry 2017-01-21
	return -1
end if

//Modified By Ken.Guo 2017-06-20
If isnull(ls_website) or ls_website = '' Then
	ls_url = 'https://secure.echosign.com/public/oauth'
Else
	ls_url = ls_website
End If
...
if isnull(ls_redirect) or ls_redirect = '' then ls_redirect = 'https://web.contractlogix.com/ContractlogixWeb/api/AccessToken'  //Modified by Harry 2017-01-18
...
choose case gs_esigntype
	case 'docusign'
		...
		cb_save.x = 2779 
		cb_close.x = 3141 
		cb_refresh.x = 2417  //Added by Harry 2017-02-03
	case 'echosign'
		...
		if gs_echosign_method = 'http' then //Added By Jay Chen 02-04-2016
			...
			cb_save.x = 1721
			cb_close.x = 2088
			cb_refresh.x = 1354  //Added by Harry 2017-02-03
		else
			...
			cb_save.x = 871
			cb_close.x = 1233
			cb_refresh.x = 509  //Added by Harry 2017-02-03
		end if
	case 'sertifi'
...
event clicked;dw_1.Retrieve(gs_esigntype)  //Added by Harry 2017-02-03
end event
...
If  gs_esigntype = 'docusign' Then
	...
	for ll_Cycle = 1 to RowCount
		ls_Password = This.GetItemString(ll_Cycle,"esign_password")
		...
		This.SetItem(ll_Cycle,"esign_password",ls_Password)
		This.SetItemStatus(ll_Cycle,"esign_password",Primary!,NotModified!)
	next
	SetPointer(Arrow!)
End If
...
String ls_userid, ls_type

SetPointer(HourGlass!)
If  gs_esigntype = 'docusign' Then
	ll_Count = This.RowCount()
	for ll_Cycle = 1 to ll_Count
		ls_userid = This.GetItemString(ll_Cycle,"user_Id")
		ls_type = This.GetItemString(ll_Cycle,"esign_type")
		ls_InputPass = This.GetItemString(ll_Cycle,"esign_password")
		
		if IsNull(ls_userid) or ls_userid = '' then CONTINUE
		
		SELECT esign_password INTO :ls_DBPass FROM personal_license_setting WHERE user_Id = :ls_userid and esign_type = :ls_type  ;
		
		if IsNull(ls_InputPass) then
			if Not IsNull(ls_DBPass) then
				UPDATE personal_license_setting SET esign_password = NULL WHERE  user_Id = :ls_userid and esign_type = :ls_type  ;
			...
			if IsNull(ls_DBPass) or ls_InputPass <> ls_DBPass then
				UPDATE personal_license_setting SET esign_password = :ls_InputPass, encrypt_esign_pwd = 1 WHERE  user_Id = :ls_userid and esign_type = :ls_type  ;
