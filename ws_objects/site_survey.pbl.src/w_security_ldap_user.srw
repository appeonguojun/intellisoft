...
string title = "Auto-Login User Mapping Painter"
...
Else
	If Messagebox('Domain info',"Local Computer's Domain Name is: '" + ls_domainname + "'." + '~r~n' + &
								  ...
								  'Do you want to set the Auto-Login information to the current login user ~'' + gs_user_id +'~'?' , Question!, YesNo! 	) = 1 Then
...
For i = 1 to ll_rowcount
	...
	If ll_findRow > 0 Then
		...
		Messagebox('Save Error','User "' +ls_user1+ '" and "'+ ls_user2 +'"'+  ' have been linked to the same Auto-Login user. Please modify either of them before saving.')
		//SetRow
		dw_ldap_user.SetFocus()
