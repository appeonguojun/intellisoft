...
string 			is_html_docname
blob				iblb_doccontent
...
public function string of_replace_send_user (string as_user)
public function string of_createappointment (string as_subject, string as_body, datetime adt_date, string as_type, string as_fromuser, string as_touser[])
public function boolean of_pingserver (string as_userid, integer ai_sendauthmode)
end prototypes

public function integer of_createobject ();long ll_Result

//Added By Jay Chen 07-17-2014
if of_ignoreocx_check("emsmtp.dll") then
	Messagebox('Create OLEObject','Cannot create oleobject as the emsmtp.dll is ignored.')
	gnv_debug.of_output(True, 'Cannot create oleobject as the emsmtp.dll is ignored.')
	Return -1
End If
//end 
...
If ib_Appointment Then
	...
	ls_Appointmentfile = of_CreateAppointment( as_Subject, as_Message, idt_AppointmentDate, is_AppointmentType, is_user_id, ls_mailto )
...
If gnv_data.of_getitem( 'icred_settings', 'use_web_email', False) <> '1' Then
	...
	//Added By Mark 05/07/2014 add is_user_id param
	lnv_webapi.of_setprotocol( 	ii_Protocol )
	ll_Return = lnv_webapi.of_Send(gs_accesstoken, is_user_id, ls_MailTo[], ls_MailCc[], ls_MailBcc[], as_Subject, as_message, "", ls_Attchment[], 1 ,0,0,0) 
...
IF gi_email_type = 0 Then  //only for Easymail //Added If statement by Ken.Guo 06/04/2013
	as_UserID = of_replace_send_user(as_UserID)
End IF
//end

//Commented By Mark 05/07/2014 The special user donot use replace.
//as_UserID = of_replace_send_user(as_UserID)		//Added By Mark Lee 01/08/2014
//Added By Ken.Guo 07/03/2013
...
//Commented By Ken.Guo 2016-04-19
...
//lnv_String.of_ParseToArray(as_attchment, ",", ls_attchment_temp[]) //Commented By Ken.Guo 2016-04-19
lnv_String.of_ParseToArray(as_attchment, ";", ls_attchment_temp[]) //Added By Ken.Guo 2016-04-19
...
public function boolean of_pingserver (string as_userid);return of_pingserver(as_userid, 0)			//Added By Mark Lee 11/21/2014

////Modified By Ken.Guo 07/03/2013
//Long ll_Result
//str_email_account lstr_email_account
//as_UserID = of_replace_send_user(as_UserID)		//Added By Mark Lee 01/09/2014
...
//lstr_email_account.ai_SendAuthMode = 0
...
//	if not of_ignoreocx_check("emsmtp.dll") then //Added By Jay Chen 07-21-2014
...
Else
	is_html_docname = au_email_edit.of_gettempfile()
	au_email_edit.of_getblob(ref iblb_doccontent)
	ls_HtmlFile = au_email_edit.of_GetHtmlFile()
...
If ib_Appointment Then
	...
	ls_Appointmentfile = of_CreateAppointment( as_Subject, is_AppointmentBody, idt_AppointmentDate, is_AppointmentType,is_user_id, ls_mailto )
	If Len( ls_AppointmentFile ) > 0 Then
		ls_Attchment2 = ls_Attchment
		ls_Attchment2[ UpperBound( ls_Attchment ) + 1  ] = ls_AppointmentFile
	Else
...
If gnv_data.of_getitem( 'icred_settings', 'use_web_email', False) <> '1' Then
	...
	End If
else   // Add by Jimmy Lee 09/04/2013 Use webapi send email

	n_cst_webapi lnv_webapi		//Added By Mark Lee 12/04/2013
	
	if isnull(gs_accesstoken) or trim(gs_accesstoken) = '' then gs_accesstoken = lnv_webapi.of_convert_json_token( )
	If  isnull(gs_accesstoken) or trim(gs_accesstoken) = '' then return -1	
	lnv_webapi.of_setprotocol( 	ii_Protocol )
//	If of_get_app_setting("set_56","I") = 1 then//modified by gavins 20140716
//		ll_Return = lnv_webapi.of_Send(gs_accesstoken, ls_MailTo[], ls_MailCc[], ls_MailBcc[], as_Subject, "", is_html_docname, ref iblb_doccontent, ls_Attchment[], 1,al_ctx_id,al_doc_id,al_action_item_id) 
//	Else
//		ll_Return = lnv_webapi.of_Send(gs_accesstoken, ls_MailTo[], ls_MailCc[], ls_MailBcc[], as_Subject, "", is_html_docname, ref iblb_doccontent, ls_Attchment[], 2,al_ctx_id,al_doc_id,al_action_item_id) 
//	End If
	ll_Return = lnv_webapi.of_Send(gs_accesstoken, ls_MailTo[], ls_MailCc[], ls_MailBcc[], as_Subject, "", as_HtmlFile, ref iblb_MailContent, ls_Attchment[], 2,al_ctx_id,al_doc_id,al_action_item_id) 
end if
//End - Added By Mark Lee 10/15/12
...
Decimal ldec_attach_size_all //Added By Ken.Guo 04/22/2014
Long ll_timeout //Added By Ken.Guo 04/22/2014
...
for i = 1 to UpperBound(as_Attchment[])
	...
	//Added By Ken.Guo 04/22/2014. get attachments total size.
	If Upper(Trim(ls_unit)) = 'MB' Then
		ldec_attach_size_all += ldec_filesize 
	Else
		ldec_attach_size_all += ldec_filesize/1000 
	End If
...
If gnv_data.of_getItem("icred_settings", "email_tag_mode", False) = '1' And &
   gnv_data.of_getItem("icred_settings", "receive_email", False) = '1' Then	//Added By Ken.Guo 2011-03-03. If No Receive Email Module, then needn't add keyword.
		//Added By Jay Chen 09-01-2015
		boolean lb_add_subject = true
		if isvalid(w_clause_painter) then
			if w_clause_painter.tab_1.ib_send_notification then
				lb_add_subject = false
			else
				lb_add_subject = true
			end if
		end if
		//end
		if lb_add_subject then as_subject = This.of_add_keyword(as_subject, gstr_email_info.ss_ctx_id_list , gstr_email_info.ss_doc_id_list, gstr_email_info.ss_ai_id_list) 
End If

//Added By Ken.Guo 04/22/2014. 60s/MB
ll_timeout = Ceiling(ldec_attach_size_all) * 60
If isnull(ll_timeout) or ll_timeout < 60 Then ll_timeout = 60
If ll_timeout > 600 Then ll_timeout = 600

// Send email
If is_SendProtocol = 'SMTP' Then 
	iole_SmtpMail.Subject = as_Subject
	iole_SmtpMail.TimeOut = ll_timeout //Added By Ken.Guo 04/22/2014
Else
	iole_EAMail.Subject = as_Subject
	iole_EAMail.TimeOut = ll_timeout //Added By Ken.Guo 04/22/2014
End If
...
if ll_Result = 0 then
	...
	If Len(ls_Attachment) > 990 Then
		ls_Attachment = Left(ls_Attachment, 990) + '...'
	End If
...
Else
	//Modified By Ken.Guo 01/14/2014
	//as_subject = as_subject + ' (CTX)'
	as_subject = as_subject + ' (CTX_ID = 0)'
	Return as_subject
End IF
...
If Pos(as_subject,'(CTX_ID =') <= 0 Then Return -1
...
public subroutine of_set_global_var (long al_ctx_id, long al_doc_id, long al_ai_id);If gstr_email_info.ss_ctx_id_list = '' or gstr_email_info.ss_ctx_id_list = '0'  Then
	gstr_email_info.ss_ctx_id_list = String(al_ctx_id)
End If

If gstr_email_info.ss_doc_id_list = '' or  gstr_email_info.ss_doc_id_list = '0'  Then
	gstr_email_info.ss_doc_id_list = String(al_doc_id)
End If

If gstr_email_info.ss_ai_id_list = '' or gstr_email_info.ss_ai_id_list = '0'  Then
...
If gnv_data.of_getitem( 'icred_settings', 'use_web_email', False) = '1' Then
	Return ls_computer_name + ' (' + ls_computer_ip + ')' + " By WebServer"
else
...
public function integer of_getdefaultaccount (string as_userid, ref string as_server, ref long al_port, ref long al_authmode, ref string as_account, ref string as_password, ref string as_email, ref integer ai_smtpssl, ref integer ai_pop3ssl);gs_CurAddresser = as_UserID	

//Added By Ken.Guo 2016-06-27
Integer li_ad_status,li_ad_save_pwd, li_use_ad_pwd
String ls_ad_user, ls_ad_password
Boolean lb_ad_used_config
...
//End - Added By Mark Lee 10/12/12
	...
	ids_sent_msg.SetItem(1,'mail_cc',ls_MailCc)
	If Len(ls_Attachment) > 990 Then ls_Attachment = Left(ls_Attachment, 990) + '...' //Added By Ken.Guo 12/18/2013
	ids_sent_msg.SetItem(1,'mail_attach_name',ls_Attachment)
	ids_sent_msg.SetItem(1,'mail_fromclx',1)
...
Integer li_smtpssl,li_pop3ssl, li_authtype
Integer li_use_ad_pwd //Added By Ken.Guo 2016-06-27
...
if SQLCA.SQLCode = 0 then
	...
	astr_email_account.ai_SendAuthMode_Exchange = li_authtype
	Return 1
end if
...
Choose Case astr_email_account.as_SendProtocol
	Case 'SMTP'
		...
		If Not IsValid(iole_SmtpMail) Then This.of_createobject( )						//Added By Mark Lee 03/12/2015
	...
	Case Else //EASendMail: Exchange2003 | Exchange 2007
		...
		Else
			...
			ii_Protocol = 2 //added by gavins 20140623
		End If
...
For i = 1 To ll_max
	...
	If i <> 1 Then gnv_debug.of_output(False, 'Failed to delete the ' + ls_filepath + '\' + as_filename + "*." + as_fileext + ' file. Now using ' + as_filename + String(i) +  "." + as_fileext + '.') //Modified By Jay Chen 08-04-2014
	Return ls_FileName
...
// Returns:  string
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 01/08/2014
//--------------------------------------------------------------------
...
//
//====================================================================
string 	ls_user, ls_send_range

ls_user = as_user
 
If gnv_data.of_getItem("icred_settings", "send_range", False) = 'A' and gnv_data.of_getItem("icred_settings", "set_56", False) = '0' then
	ls_user = gnv_data.of_getItem("icred_settings", "set_addresser", False)
...
public function string of_createappointment (string as_subject, string as_body, datetime adt_date, string as_type, string as_fromuser, string as_touser[]);//
String			ls_vcal, ls_attach_fullname, ls_Html, ls_keyword, ls_touser
...
as_fromuser = of_replace_send_user(as_fromuser)		//Added By Mark Lee 01/09/2014
of_GetDefaultAccount(as_fromuser,lstr_email_account) 
...
ls_vcal += "VERSION:2.0"
ls_vcal += "~r~n"
ls_vcal += "METHOD:REQUEST"
ls_vcal += "~r~n"
ls_vcal += "BEGIN:VEVENT"
ls_vcal += "~r~n"
For li_i = 1 To Upperbound( as_touser[] )
	ls_touser = left( as_touser[li_i], pos(  as_touser[li_i], "@") - 1 )
	ls_vcal += "ATTENDEE;CN="+ ls_touser  +";RSVP=TRUE:mailto:" + as_touser[li_i] 
	ls_vcal += "~r~n"
Next
If Upperbound( as_touser ) = 0 Then ls_vcal += "~r~n"
...
End If
ls_vcal += "~r~n"
ls_vcal += "ORGANIZER;CN=" + lstr_email_account.as_account +  ":mailto:" + lstr_email_account.as_emailaddress
ls_vcal += "~r~n"
ls_vcal += "TRANSP:TRANSPARENT"
ls_vcal += "~r~n"
ls_vcal += "SEQUENCE:0"
...
public function boolean of_pingserver (string as_userid, integer ai_sendauthmode);//====================================================================
// Function: of_pingserver
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    string     as_userid
// 	value    integer    ai_SendAuthMode
//--------------------------------------------------------------------
// Returns:  boolean
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 11/21/2014
//--------------------------------------------------------------------
...
//if of_SetSmtpInfo(lstr_email_account.as_sendserver ,lstr_email_account.al_sendport ,lstr_email_account.ai_sendauthmode ,lstr_email_account.ai_sendssl ) < 0 then return false
//if of_SetAccountAddress(lstr_email_account.as_account , lstr_email_account.as_password , lstr_email_account.as_emailaddress ) < 0 then return false
lstr_email_account.ai_SendAuthMode = ai_SendAuthMode
if of_SetSmtpInfo(lstr_email_account ) < 0 then return false
if of_SetAccountAddress(lstr_email_account) < 0 then return false
