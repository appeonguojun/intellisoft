...
type contract_status from pfc_cst_u_security_contract_status_role within tab_security
end type
type contract_status from pfc_cst_u_security_contract_status_role within tab_security
end type
type tab_security from tab within w_security_roles_painter
...
public subroutine of_ini_contract_status ()
public subroutine of_refresh_contract_status (long al_role_id)
end prototypes
...
//	tab_security.tabpage_contract.uo_contract.dw_2.retrieve(al_role_id) //Modified By Jay Chen 04-02-2015
...
// $<description>
//////////////////////////////////////////////////////////////////////
// $<add> 11.01.2006 by Jack (Contract Logix Agreement Manager Painters)
//////////////////////////////////////////////////////////////////////
...
public subroutine of_ini_contract_status ();//====================================================================
// Function: of_ini_contract_status
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
n_cst_update_role_right  lnv_role_right

lnv_role_right = Create n_cst_update_role_right
lnv_role_right.of_update_contract_status()
Destroy lnv_role_right
end subroutine

public subroutine of_refresh_contract_status (long al_role_id);//====================================================================
// Function: of_refresh_contract_status
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
tab_security.contract_status.dw_1.Reset()
tab_security.contract_status.dw_1.Settransobject(SQLCA)
tab_security.contract_status.dw_1.retrieve(al_role_id)
Return
end subroutine
...
this.Control[iCurrent+4]=this.dw_select_role
this.Control[iCurrent+5]=this.tab_security
this.Control[iCurrent+6]=this.gb_select
end on
...
event open;call super::open;String 		ls_data
...
//		tab_security.tabpage_contract.uo_contract.dw_4.visible = False	
		...
		long ll_type,ll_row
		for ll_row = tab_security.tabpage_contract.uo_contract.dw_2.rowcount() to 1 step -1
			ll_type = tab_security.tabpage_contract.uo_contract.dw_2.getitemnumber(ll_row,"types")
			if ll_type = 2 or ll_type = 3 or ll_type = 4 then
				tab_security.tabpage_contract.uo_contract.dw_2.deleterow(ll_row)
			end if
		next
...
tab_security.contract_status.of_Set_Parent_Window( This )
of_ini_contract_status()
...
//--------------------------- APPEON BEGIN ---------------------------
//$<add> 11.28.2006 By: Jack (Contract Logix Agreement Manager Painters)
//$<reason> Contract Management Rights.
If gb_agreement_manager And gnv_data.of_getitem('icred_settings' ,'set_cont' , False ) = '1' Then
	of_contract_management_rights_tab()
Else
...
if IsValid(w_Mdi) then w_Mdi.of_menu_security(w_Mdi.MenuName)		//Added by Scofield on 2010-01-13
...
//$<reason> 
//IF tab_security.tabpage_roles.uo_roles.ii_id < 1 or&				//Added By Mark Lee 08/18/2014
IF (tab_security.tabpage_roles.uo_roles.ii_id = 0  or tab_security.tabpage_roles.uo_roles.ii_id = -1 ) or &
...
//tab_security.tabpage_contract.uo_contract.dw_2.reset() //Modified By Jay Chen 03-19-2015
...
tab_security.contract_status.dw_1.reset() //Added By Ken.Guo 02/05/2015
long ll_role_id 
...
//Added By Jay Chen 03-24-2015
if tab_security.tabpage_contract.uo_contract.dw_1.modifiedcount( ) > 0 then
	if messagebox(gnv_app.iapp_object.DisplayName,'Do you want to save changes?',question!,yesno!) = 1 then
		tab_security.tabpage_contract.uo_contract.dw_1.update()
	end if
end if
//end
...
ElseIf tab_security.SelectedTab = 5 Then //Contract Status //Added By Ken.Guo 02/04/2015
	Parent.of_refresh_contract_status( tab_security.tabpage_roles.uo_roles.ii_id)	
END IF
//---------------------------- APPEON END ----------------------------
...
event dropdown;call super::dropdown;//Added By Jay Chen 05-28-2015
if tab_security.SelectedTab = 3 then
	if tab_security.tabpage_contract.uo_contract.dw_1.modifiedcount( ) > 0 then
		if tab_security.tabpage_contract.uo_contract.of_validate_right( ) < 1 then return -1
	end if
end if

if tab_security.SelectedTab = 1 then
	if tab_security.tabpage_roles.uo_roles.dw_actions.modifiedcount( ) > 0 then
		if tab_security.tabpage_roles.uo_roles.of_validate_right( ) < 1 then return -1
	end if
end if
end event
...
this.contract_status=create contract_status
...
ElseIf newindex = 5 Then	//Contract Status //Added By Ken.Guo 02/04/2015
	Parent.of_refresh_contract_status( ll_role_id)	
End If
//---------------------------- APPEON END ----------------------------
