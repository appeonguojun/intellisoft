...
type dw_occ from u_dw within w_email_folder
end type
...
integer width = 4645
integer height = 1772
string title = "Contract Logix Email "
...
public subroutine of_auto_modify_owner (integer al_ctx_id)
public subroutine of_goto_contract (long al_ctx_id)
public subroutine of_after_checkin_email (string as_protocol, string as_tip)
end prototypes
...
If  ls_email_checkin = '1' Then
	ls_tip = st_tip.text // //Added By Harry 2017-01-12
	st_tip.text = "Auto checking in document..."
	
	If ls_protocol = 'POP3' Then
		inv_em_pop3.is_status = inv_em_pop3.IS_PROCESSING
	ElseIf ls_protocol = 'IMAP4' Then
		inv_em_imap4.is_status = inv_em_imap4.IS_PROCESSING
	Else
		inv_em_eamail.is_status = inv_em_eamail.IS_PROCESSING
	End If
	...
	lnv_dm_utils.of_check_in_from_email()
	This.Post of_after_checkin_email(ls_protocol, ls_tip)  //Added By Harry 2017-01-12.
...
If idw_folder_browse[ii_current_main_folder_id] <> adw_item Then Return 0

If al_currentrow > adw_item.RowCount() Then Return 0

if al_currentrow <= 0 or adw_item.RowCount()  = 0  then //Added By Ken.Guo 2016-05-23 added  adw_item.RowCount()  = 0 
...
Choose Case ls_email_type
	...
	//	"EasyMail.Message.6"
		...
		gnv_debug.of_output( True, 'inv_em_pop3.of_eml2html start...' ) //Added By Jay Chen 02-13-2015
		ls_html_file = inv_em_pop3.of_eml2html(ll_id,ls_email_file)
		gnv_debug.of_output( True, 'inv_em_pop3.of_eml2html end...' ) 
		...
		If FileExists(ls_html_file) Then
			...
			gnv_debug.of_output( True, 'inv_em_pop3.of_setattachment start...' ) 
			inv_em_pop3.of_setattachment(ll_id, ll_doc_id,ls_email_file,ls_attach_list,lv_attachment)
			gnv_debug.of_output( True, 'inv_em_pop3.of_setattachment end...' ) 
			
			//Added By Ken.Guo 07/31/2013
 			If adw_item.rowcount() = 0 Then Return 0 //Added By Ken.Guo 2016-05-24
	...
	Case 'doc'  //Word format
		...
		gnv_debug.of_output( True, 'inv_em_pop3.of_doc2html start...' ) 
		inv_em_pop3.of_doc2html( ls_email_file, ls_html_file)
		gnv_debug.of_output( True, 'inv_em_pop3.of_doc2html end...' ) 
		ole_1.object.Navigate(ls_html_file)
		is_current_eml_file = '' //EML file N/A
		is_current_doc_file = ls_email_file
		if adw_item.rowcount() > 0 then 		//Added By Mark Lee 09/02/2014
...
//
Post of_reset_attachUI()  //Modified By Ken.Guo 2017-01-05. added Post key to workaround APB 6.8 crash issue.

il_current_disp_mail = ll_id

gnv_debug.of_output( True, 'of_set_header start...' ) 
of_set_header(adw_item,al_currentrow)
gnv_debug.of_output( True, 'of_set_header end...' ) 
...
if lb_auto_modify_owner then
	...
	If  ls_auto_modify_owner = '1'  Then
		of_auto_modify_owner(ll_ctx_id)
	end if
end if
...
//		IF isvalid(w_contract_email_folder) and il_ctx_id > 0 then	  
		IF il_ctx_id > 0 then //Modified By Jay Chen 09-10-2015 set default ctx when open import email from selected contract
...
If isvalid(m_pfe_cst_mail) Then
	...
	End If
	If isvalid(inv_em_pop3) Then //Added By Ken.Guo 2015-07-27 Added if statement.
...
//		m_pfe_cst_mail.m_email.m_stopreceive.enabled = True
	...
	if w_mdi.of_security_access( 7021 ) = 0 then //email log
		m_pfe_cst_mail.m_email.m_emaillog.enabled = False
	end if
...
If Isvalid(m_pfe_cst_mdi_menu) Then
	If ll_unread_cnt > 0 Then
		gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu.m_system.m_emailsignature.m_emailsentitems,'toolbarItemname', 'home_fill.gif')
	Else
		gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu.m_system.m_emailsignature.m_emailsentitems,'toolbarItemname', 'home.gif')
	End If
End If
...
long ll_contact_id,ll_record_id
...
dw_occ.settransobject(sqlca)
dw_occ.retrieve(al_ctx_id,'I')
...
ll_rowcount = dw_occ.rowcount()
ll_find = dw_occ.find("Upper(user_d) = '"+Upper(lstr_contact.as_user_id)+"'",1, ll_rowcount)
if ll_find > 0 then 
	ll_contact_id = dw_occ.getitemnumber(ll_find,"contact_id")
	ls_default_email = dw_occ.getitemstring(ll_find,"default_email")
	ls_owner = dw_occ.getitemstring(ll_find,"ctx_contract_contacts_owner")
	if (ls_default_email <> 'Y' or ls_owner <> 'Y')  then
		...
		if ls_default_email <> 'Y' then
			dw_occ.SetItem(ll_find,"default_email", 'Y')
		end if
		if ls_owner <> 'Y' then
			dw_occ.SetItem(ll_find,"ctx_contract_contacts_owner", 'Y')
		end if
		lb_edit = true
	else
		return
	...
	dw_occ.SetItem(ll_row,"default_email", 'Y')
	dw_occ.SetItem(ll_row,"ctx_contract_contacts_owner", 'Y')
	dw_occ.SetItem(ll_row,"ctx_id", al_ctx_id)
	dw_occ.SetItem(ll_row,"record_id", ll_record_id)
	lb_edit = false
end if
commit;
//write audit log
lnv_audit.of_set_prac_id(al_ctx_id)
lnv_audit.of_audit_fields(dw_occ)
...
if lb_edit then
	dw_occ.SetItemStatus(ll_find,"default_email", Primary!, NotModified!)
	dw_occ.SetItemStatus(ll_find,"ctx_contract_contacts_owner", Primary!, NotModified!)
else
	dw_occ.SetItemStatus(ll_find,0, Primary!, NotModified!)
end if

end subroutine

public subroutine of_goto_contract (long al_ctx_id);//====================================================================
// Function: of_goto_contract
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 05-14-2015
//--------------------------------------------------------------------
...
if isvalid(gw_contract) then
	...
	if ll_RowPos <= 0 then
		...
		gf_goto_contract_page(al_ctx_id,'other')
		ll_RowPos = gw_contract.tab_contract_details.tabpage_search.ids_search.Find("ctx_id = " + String(al_ctx_id), 1, gw_contract.tab_contract_details.tabpage_search.ids_search.RowCount())
		if ll_RowPos <= 0 then //end add
...
public subroutine of_after_checkin_email (string as_protocol, string as_tip);//====================================================================
// Function: of_after_checkin_email
//--------------------------------------------------------------------
// Description: After check in document from email, refresh the icon of attachment
//--------------------------------------------------------------------
// Arguments:
//				string		as_protocol
//				string		as_tip
...
ll_currow = idw_folder_browse[ii_current_main_folder_id].getrow()
If ll_currow > 0 Then  
	il_current_disp_mail = -1
	This.Event ue_rowfocuschanged(idw_folder_browse[ii_current_main_folder_id], 1)
End If	


If as_protocol = 'POP3' Then
	inv_em_pop3.is_status = inv_em_pop3.IS_COMPLETE
ElseIf as_protocol = 'IMAP4' Then
	inv_em_imap4.is_status = inv_em_imap4.IS_COMPLETE
Else
	inv_em_eamail.is_status = inv_em_eamail.IS_COMPLETE
End If
...
this.Control[iCurrent+19]=this.ole_mail_view
this.Control[iCurrent+20]=this.dw_sent_items
this.Control[iCurrent+21]=this.rte_header
...
//Added By Ken.Guo 2015-07-27
If isvalid(inv_em_pop3) Then Destroy inv_em_pop3
If isvalid(inv_em_imap4) Then Destroy inv_em_imap4 
...
event timer;call super::timer;String ls_protocol
ii_timer_cnt = ii_timer_cnt + ii_timer
If ii_timer_cnt >= 3600 Then ii_timer_cnt = 0

//Add by Harry 2017-01-10
Select  protocol Into  :ls_protocol
From em_smtp_accounts
Where user_id = :gs_user_id and is_default = 'Y';
...
If gnv_data.of_getitem('icred_settings', 'receive_email', False) = '1' Then
	IF il_autoreceive_timer <> 0 Then
		If Mod(ii_timer_cnt,il_autoreceive_timer) = 0 Then
				//If inv_em_pop3.is_status = inv_em_pop3.IS_COMPLETE Then  //Modify by Harry  2017-01-10
				If (ls_protocol = 'POP3' and inv_em_pop3.is_status = inv_em_pop3.IS_COMPLETE) or &
				(ls_protocol = 'IMAP4' and inv_em_imap4.is_status = inv_em_imap4.IS_COMPLETE) or &
				(ls_protocol <> 'IMAP4' and ls_protocol <> 'POP3' and inv_em_eamail.is_status = inv_em_eamail.IS_COMPLETE) Then 
...
event pfc_preopen;call super::pfc_preopen;//Added By Ken.Guo 2015-07-27
inv_em_pop3 = Create n_cst_easymail_pop3  
inv_em_imap4 = Create n_cst_easymail_imap4

inv_em_eamail = Create n_cst_easymail_eamail  //Added By Ken.Guo 2015-07-30
end event
...
integer height = 100
integer taborder = 50
string dataobject = "d_contract_det_contact_external"
end type
...
If IsValid( w_contract ) Then
	...
	of_goto_contract(il_ctx_id) //Added By Jay Chen 05-14-2015
...

