...
Choose Case inv_cachelist[li_id].s_method
	...
	Case RETRIEVE, SQL		
		...
		if as_id = 'dddw_contact_company' or as_id = 'dddw_contact_company_search' then
			inv_cachelist[li_cnt].ds_obj.setfilter("")
			inv_cachelist[li_cnt].ds_obj.filter()
		end if
		//end 
		ll_rows = inv_cachelist[li_cnt].ds_obj.Retrieve(la_args[1], la_args[2], &
			...
			if as_id = 'dddw_contact_company' or as_id = 'dddw_contact_company_search' then
				string ls_filter_company_contact
				ls_filter_company_contact = gnv_data.of_getitem("icred_settings", "filter_company_contact", false)
				if ls_filter_company_contact = '1' then
					if as_id = 'dddw_contact_company_search' then
						inv_cachelist[li_cnt].ds_obj.setfilter("app_facility_status = 'A' ")
						inv_cachelist[li_cnt].ds_obj.filter()
					else
						inv_cachelist[li_cnt].ds_obj.setfilter("status = 'A' ")
						inv_cachelist[li_cnt].ds_obj.filter()
					end if
					ll_rows = inv_cachelist[li_cnt].ds_obj.rowcount()
				end if
			end if
			//end
		Return ll_rows
