...
if RegistryGet( "HKEY_LOCAL_MACHINE\IIM\" ,"PathBasic", RegString!, is_macro_path)  = -1 then
	is_macro_path = "C:\Program Files\iMacros\"
end if

is_macro_path = of_check_path(is_macro_path,1) //check for slash at end

IF NOT FileExists(is_macro_path + "imacros.exe") THEN
	MessageBox("Missing Software", "Unable to find the Imacros software necessary to run this function.  This can be downloaded from the ContractLogix Group Download website. Please contact Customer Support if you need assistance with this." )
	Close( This )
	Return -1	
END IF
...
IF Len( StrIng( ll_response_code  ) )  = 0 THEN
...
ls_savefile = gs_dir_path + gs_DefDirName + "\snapshot.bmp"
...
ls_data = "SAVEAS TYPE=BMP FOLDER=" + gs_dir_path + gs_DefDirName + "\" + " FILE=snapshot.bmp"
...
li_retval = myoleobject.ConnectToNewObject(  "InternetMacros.iim")
IF li_retval < 0 THEN
