...
if mid(ls_path,len(ls_path),1) <> "\" then ls_path += "\"
	...
	debugbreak()
		for i = 1 to ic //list of imported records
			...
			elseif f < 0 then //failure of find
				messagebox("Web Crawler Import","Copy Header failure; Contact ContractLogix Support.")
...
long backcolor = 67108864
boolean enabled = false
string text = "2) Copy the files into your Intellcred folder."
boolean focusrectangle = false
end type
...
long backcolor = 67108864
boolean enabled = false
string text = "1) Contact ContractLogix Support to get import files"
boolean focusrectangle = false
end type
...
long backcolor = 67108864
boolean enabled = false
string text = "To view a complete list of Web crawler Websites that are currently available from ContractLogix Group Inc, click on the Go To Web Site button below."
boolean focusrectangle = false
end type
