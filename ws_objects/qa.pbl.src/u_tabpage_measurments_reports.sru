...
if ll_count < 1 then 
	If gs_cust_type <> 'I' Then	//Added By Ken.Guo 05.27.2008.IntelliAPP should not show tabpage_apptmnt_status.
		if messagebox('NPDB Reports', 'This report cannot be run because the practitiner is not associated with a PRIMARY department.' + &
	...
	Else
		...
		Messagebox('NPDB Reports', 'This report cannot be run because the practitiner is not associated with a PRIMARY department.')
	End If					
	Return 
end if
