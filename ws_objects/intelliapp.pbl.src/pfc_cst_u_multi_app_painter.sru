...
type gb_1 from groupbox within pfc_cst_u_multi_app_painter
end type
end forward
...
For i = 1 to rc -1
	...
	for r = i + 1 to rc
		if ll_facid = this.getitemnumber(r,"parent_facility_id") and ll_appid = this.getitemnumber(r,"app_id") then
			messagebox("Duplicate Application","There is a duplcate Application selected for the Facility/Group on line " + string(r) + ".~rYou must correct before saving. ")
			return -1
		end if
	next
next
...
event itemchanged;call super::itemchanged;dw_app_data.Retrieve( Integer( data ) )
of_enable_disable_objects( 1 )
