...
/*
	...
	*/
	...
	///Start Code Change ---- 12.05.2005 #160 maha	
			...
			if res = -1 then
				messagebox("Rowscopy error in of_place_template of w_section_templates", "Contact ContractLogix Group support.")
				return -1 
			end if
...
ELSE
	ls_field = dwo.Name
	IF Right( ls_field, 2 ) = "_t" THEN
		This.SetSort( Mid( ls_field, 1, Len( ls_field )-2))
		This.Sort( )
	END IF
END IF
end event
