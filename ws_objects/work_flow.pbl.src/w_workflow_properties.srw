...
public subroutine wf_trigger_category (string as_trigger_by)
end prototypes
...
public function integer of_redraw_dataobject (string data);wf_trigger_category(data) //Added By Jay Chen 08-07-2014
		...
		*/
		...
		if data = '105' then 
			if dw_parameter.Getchild("screen_id",ldwc) = 1 then
				...
				ldwc.reset() //Added By Ken.Guo 2016-02-18. workkaround PB's bug. auto retrieved DDDW data.
				ll_Row = ldwc.insertrow(0)
				ldwc.setitem(ll_Row,"screen_id",-100)
				ldwc.setitem(ll_Row,"screen_name","Document Manager->Document")		
				
				//Added By Ken.Guo 2016-02-17
				ll_Row = ldwc.insertrow(0)
				ldwc.setitem(ll_Row,"screen_id",51)
				ldwc.setitem(ll_Row,"screen_name","Documents->Add Document Properties")		
				If gnv_data.of_getitem( "icred_settings", "set_51", FALSE) = '1' Then
					ll_Row = ldwc.insertrow(0)
					ldwc.setitem(ll_Row,"screen_id",52)
					ldwc.setitem(ll_Row,"screen_name","Documents->Create Document Properties")	
				End If				
...
gnv_win_backcolor.of_ChangeWindowBackColor(this)  //Added By Ken.Guo 2016-03-23
...
public subroutine wf_trigger_category (string as_trigger_by);//====================================================================
// Function: wf_trigger_category
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    string    as_trigger_by
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 08-07-2014
//--------------------------------------------------------------------
...
long ll_wf_id,ll_count
datastore lds_category

if as_trigger_by = '002' then
	lds_category = create datastore
	lds_category.dataobject = 'd_code_lookup_category'
	lds_category.settransobject(SQLCA)
	lds_category.retrieve()
	ll_count = lds_category.rowcount()
	ll_wf_id = istr_parm.l_wf_id
	lds_category.setfilter("custom_5 = '"+string(ll_wf_id)+"' ")
	lds_category.filter()
	if lds_category.rowcount() = 0 then
		dw_workflow.Modify( 'compute_category.expression="~'Null~' " ')
	elseif lds_category.rowcount() < ll_count then
		dw_workflow.Modify( 'compute_category.expression="~'Specific~' " ')
	elseif lds_category.rowcount() = ll_count then
		dw_workflow.Modify( 'compute_category.expression="~'All~' " ')
	end if
end if
...
choose case istr_parm.s_type
	case 'new'
		this.title = 'Work Flow New' //Added By Jay Chen 08-07-2014
		this.event ue_new()
	case 'copy'
		this.title = 'Work Flow New' //Added By Jay Chen 08-07-2014
		this.event ue_copy()
...
If ls_wf_triggered_by = '105' Then
	If dw_parameter.find('screen_id = 51', 1, dw_parameter.rowcount()) > 0 and dw_parameter.find('screen_id = 52', 1, dw_parameter.rowcount()) > 0  Then
		messagebox('Information','You cannot use both Add Document Properties Screen and Create Document Properties Screen in the same time as the trigger parameters.')
		dw_parameter.setfocus()
		dw_parameter.setcolumn('screen_id')
		Return
	End If
End If
...
string ls_visible //Added By Jay Chen 02-01-2016
...
if ls_wf_triggered_by = '105' and al_screen_id < 0  then //Modified By Ken.Guo 2016-02-18. added al_screen_id < 0 
...
If li_count > 0  Then
	...
	For li_i = 1 to li_count
		...
		//Modified By Jay Chen 02-01-2016
		ls_visible = dw_1.describe(ls_name + ".visible")
		if Pos( ls_visible, '~t' ) > 0 then 
			ls_visible = '1'
		end if
		if long(ls_visible) = 0 then continue
		//end
//		if long(dw_1.describe(ls_name + ".visible")) = 0 then continue
		...
		//Added By Jay Chen 02-25-2014 Remark: remove the table name of column
		if ls_name = 'ctx_contract_contacts_owner' and ls_dbname = 'owner' then
			ls_name = 'owner'
		end if
//		if Pos(ls_name,ls_tablename) > 0 then
//			ls_name = right(ls_name,len(ls_name) - len(ls_tablename) - 1)
//		end if
		//end add
...
if ls_trigger_by = "008" and this.RowCount() > 0 then
	...
	if ll_count > 0 then
		of_MessageBox ("pfc_preupdate", gnv_app.iapp_object.DisplayName, &
				"The same record already exists in the current module.  Please re-enter in the Trigger Parameters field. ", information!, Ok!, 1)
		return -1
	end if
end if
...
if dwo.name = "b_dropdown" then
	...
	elseif ls_dddw_type = 'P' then	//Company dddw-jervis 12.9.2009
		string ls_value, ls_value_old
		string ls_operator
		//Select company
		//Modified By Ken.Guo 2017-05-19
		//OpenWithParm(w_facility_name_select,"code&" + ls_lookup_name)
		ls_value_old = this.getitemstring(row, 'value')
		If isnull(ls_value_old) Then ls_value_old = ''
		OpenWithParm(w_facility_name_select,"code&" + ls_value_old)
...
elseif dwo.name = 'b_expression' then
	...
	lstr_pass.s_long[2] = ll_screen_id //Added By Ken.Guo 2015-08-27
...
string ls_null
datawindowchild ldwc //Added By Jay Chen 07-25-2014
long ll_row
...
//		if ls_triggered_by = "100" or ls_triggered_by = "105" then //Modified By Jay Chen 07-25-2014
		if ls_triggered_by = "100"  then
			dw_parameter.Reset()
			dw_parameter.event pfc_insertrow()
			dw_parameter.Post Event pfc_retrievedddw("screen_id")
		end if
		//Added By Jay Chen 07-25-2014
		if ls_triggered_by = "105" then
			if dw_parameter.Getchild("screen_id",ldwc) = 1 then
				ldwc.reset()
				ldwc.settransobject( SQLCA )
				ll_Row = ldwc.insertrow(0)
...
If dwo.name = 'b_retrigger' Then
	Messagebox('Allow Multiple Instances Help',+&
		"Multiple instances of the same Work Flow are only allowed for completed workflows.~r~n" + & 
		"However, only one instance of the same Work Flow is allowed for uncompleted workflows.~r~n")
End If

//Added By Jay Chen 08-06-2014
long ll_wfid
If dwo.name = 'b_category' and Row > 0 Then
	ll_wfid = this.getitemnumber( row, "wf_id")
	if isnull(ll_wfid) then ll_wfid = 0
	if ll_wfid = 0 or ll_wfid = -1 then
		Messagebox('Information','Please save the current work flow before proceeding to set the associated category.')
		return 
	end if
	OpenWithParm(w_wf_category, ll_wfid)
	If message.Stringparm = 'cancel' Then
		Return failure
	End If
