...
ll_wf_id = Long(Left(is_Module, Len(is_Module) - 2))
is_Module = Right(is_Module, 2)
...
IF dw_1.Retrieve(ll_wf_id) > 0 THEN
	...
	for ll_Cycle = 1 to ll_Cnts
		...
		elseif Left(ls_User,2) = "$$" and Right(ls_User,2) = "$$" then
			dw_1.SetItem(ll_Row,ls_EMailUser[ll_Cycle],Mid(ls_User,3,Len(ls_User) - 4))
		end if
	next
