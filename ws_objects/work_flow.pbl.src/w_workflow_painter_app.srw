...
type gb_3 from groupbox within w_workflow_painter_app
end type
...
type st_1 from statictext within w_workflow_painter_app
end type
end forward
...
if ab_Send then//modified by gavins 20121203
//	SELECT Count(*)
//	  INTO :ll_Cnt
...
//Added By Jay Chen 08-06-2014 remark:check workflow trigger param
...
li_rtn = f_ctx_find(1,lds_data_comparision,ls_updatecol[],'2',ls_where)
destroy lds_data_comparision
if li_rtn = -1 then
	Messagebox('Information','The work flow trigger parameters are invalid. Please go to Properties to correct them. ')
	return -1
end if
//end
...
String ls_filter
long	 ll_workflow_id , ll_wf_id , ll_ins,ll_find, ll_Rtn, ll_RowCount
string ls_action_type
DataWindowChild ldwc_child
...
choose case dwo.name
	...
	case 'wf_id'
		...
		ll_RowCount = tab_1.tabpage_types.dw_action_type_detail.RowCount()
		tab_1.tabpage_types.dw_action_type_detail.retrieve(ll_workflow_id)
		
		If ll_RowCount > 0 Then //Added By Jay 2016-05-10
			tab_1.tabpage_types.dw_action_type_detail.Event RowFocusChanged ( tab_1.tabpage_types.dw_action_type_detail.GetRow() )
		End If
...
long textcolor = 128
long backcolor = 67108864
string text = "Status (Status of Action Type)"
end type
