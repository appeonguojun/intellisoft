...
string text = "none"
integer unitsperline = 100
integer unitspercolumn = 50
...
type prototypes
Function ulong GetScrollRange( ulong hwnd, ulong nBar, ref ulong nMinPos, ref ulong nMaxPos ) library "user32.dll"
...
//Added By Jay Chen 12-25-2014
long il_step
ulong il_minpos,il_maxpos,il_max_height
end variables
...
////Added By Jay Chen 12-25-2014
//GetScrollRange(handle(parent), 1, il_minpos, il_maxpos)
//if AppeonGetClientType() = 'WEB' then il_maxpos = PixelsToUnits(il_maxpos, YPixelsToUnits!)
//il_max_height = il_maxpos * 3400 / 1928
//if dw_detail.height + dw_Summary.height + dw_Footer.height + ll_moved  > il_max_height and ll_moved > 0 then 
//	messagebox('Contractlogix','The maximum height for the detail band, summary band and footer band is ' + string(il_max_height) + '.')
//	this.move(this.x,this.y - ll_moved)
//	return
//end if
////end
...
//		//Added By Jay Chen 12-25-2014 max height only supprot 11000 
//		GetScrollRange(handle(parent), 1, il_minpos, il_maxpos)
//		ll_steppos = (dw_detail.height + dw_Summary.height + dw_Footer.height) * (il_maxpos/11000)
//		ll_step = ll_steppos * (100/il_maxpos)
//		if ll_step > il_step then
//			for k = 1 to ll_step - il_step
//				Send(Handle(parent), 277, 1, 0)
//			next
//		else
//			for k = 1 to il_step - ll_step
//				Send(Handle(parent), 277, 0, 0)
//			next
//		end if
//		il_step = ll_step
//		//end add
		...
		if isvalid(w_contract_data_design) then
			w_contract_data_design.uo_design.vsb_1.maxposition = w_contract_data_design.uo_design.vsb_1.maxposition + ll_moved
		end if
		//Added By Jay Chen 12-30-2014
		if isvalid(w_contract_report_design) then
			w_contract_report_design.uo_design.vsb_1.maxposition = w_contract_report_design.uo_design.vsb_1.maxposition + ll_moved
		end if
	end if
End If
...
Else	 
	if ib_moving then
		...
		il_step = ll_step
		****************************************/
//		gnv_status_info.of_display("height: " + string(dw_detail.height) + ' maxpos' +string(il_maxpos) )
		//end add
