...
type cb_2 from commandbutton within w_contract_screen_history
end type
...
this.Control[iCurrent+7]=this.dw_history
...
event clicked;open(w_screenbatchupdate)
end event
...
If Isvalid( lds_Create ) Then destroy( lds_Create )//added by gavins 20140714
...
Else
	...
	if IsValid(w_contract_data_design) then 
		w_contract_data_design.uo_design.il_seqno = ll_Seq
	end if
	//end add
