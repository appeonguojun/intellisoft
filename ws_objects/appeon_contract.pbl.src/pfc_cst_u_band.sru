...
long il_record_resize = 0 //Added By Jay Chen 03-02-2016
long il_record_move = 0
end variables
...
public function integer of_get_object_attribute (datawindow ids_terget, ref datastore ids_properties, string as_objname)
public function string of_modify_object_attribute (datastore ads_properties)
public subroutine of_set_button_dotdd_company (string as_column, string as_value)
end prototypes

event ue_regenerate();//--------------------------- APPEON BEGIN ---------------------------
//$<add> 2007-11-05 By: Andy
string  ls_modify,ls_Array[]
string  ls_Modify1,ls_BandType,ls_modify_taborder
Long    ll_row
...
If Upper(is_BandType) <> 'DETAIL' Then
	...
	For i = 1 to ll_cnt
		//Modified By Ken.Guo 2015-05-13
		//This.Modify("#"+String(i)+".TabSequence = '0'")
		ls_modify_taborder += "#"+String(i)+".TabSequence = '0' "
	Next
	This.Modify(ls_modify_taborder)
End If
...
this.modify(ls_Modify + ' ' + ls_Modify1)
...
string ls_dwObjSyntax,ls_dwSyntax,ls_action
long ll_find
string	ls_X1, ls_X2, ls_Y1, ls_Y2
string ls_x,ls_y,ls_h,ls_w
...
if Not iuo_parent.uo_band.ib_MouseDown then
	...
	ELSE
		...
		il_record_resize = 0 //Added By Jay Chen 03-02-2016
		il_record_move = 0
	END IF
	
	if ls_Modify = "" then il_record_resize = 0  //Added By Jay Chen 03-02-2016
	if ls_Modify <> "" then
		This.SetRedraw( False )
		//Added By Jay Chen 02-26-2016 
		ls_action = 'Moveresize ' + ls_ObjName
//		ll_find = iuo_Parent.ids_step_list.find( "step_name = '"+ls_action+"'" ,1, iuo_Parent.ids_step_list.rowcount())
//		if isnull(ll_find) then ll_find = 0
//		if ll_find = 0 then
		if il_record_resize = 1 then //Added By Jay Chen 03-02-2016
			if this.describe(ls_ObjName + ".type") <> 'line' then
				ls_x = This.Describe(ls_ObjName + ".X")
				ls_y = This.Describe(ls_ObjName + ".Y")
				ls_w = This.Describe(ls_ObjName + ".Width")
				ls_h = This.Describe(ls_ObjName + ".Height")
				ls_dwObjSyntax =	ls_ObjName + ".x = " + ls_x
				ls_dwObjSyntax += " " + ls_ObjName + ".y = " + ls_y
				ls_dwObjSyntax += " " + ls_ObjName + ".width = " + ls_w
				ls_dwObjSyntax += " " + ls_ObjName + ".height = " + ls_h
			else
				ls_x1 = This.Describe(ls_ObjName + ".x1")
				ls_x2 = This.Describe(ls_ObjName + ".x2")
				ls_y1 = This.Describe(ls_ObjName + ".y1")
				ls_y2 = This.Describe(ls_ObjName + ".y2")
				ls_dwObjSyntax =	ls_ObjName + ".x1 = " + ls_x1
				ls_dwObjSyntax += " " + ls_ObjName + ".x2 = " + ls_x2
				ls_dwObjSyntax += " " + ls_ObjName + ".y1 = " + ls_y1
				ls_dwObjSyntax += " " + ls_ObjName + ".y2 = " + ls_y2
			end if
			iuo_Parent.of_record_undolist(ls_dwObjSyntax,ls_ObjName,'Moveresize') 
		end if
		//end
		This.Modify(ls_Modify)
		This.SetRedraw( True )
...
FOR LL_I = 1 TO ll_count
	IF lower(Describe(is_objects[ll_i] + ".Band")) =  lower(ls_band) then
		ls_type = lower(Describe(is_objects[ll_i] + ".type"))
		ls_colname = Describe(is_objects[ll_i] + ".Name")
		ls_Visible =  this.Describe(is_objects[ll_i] + ".visible")

		/**********************************************************///added by gavins 20120508
		If  ls_Visible <> '0' And ls_Visible <> '1'  And ls_Visible <> '-1'   Then
			this.Modify(is_objects[ll_i] + ".visible = 1")
		End If
				
		If Len( ls_DataObject ) > 0 Then
			ls_DefaultExpression = lds_Default.Describe(is_objects[ll_i] + ".visible")
		...
		//Added By Ken.Guo 2015-05-14. for performance.
		...
		if ls_type = "column" then
			...
			//dddw
			IF Describe(is_objects[ll_i] + ".Edit.Style") = 'dddw' then				
				ls_dddw_name = lower(Describe(is_objects[ll_i] + ".DDDW.Name"))							
				IF ls_dddw_name = "d_dddw_code_lookup" OR ls_dddw_name = "d_dddw_code_lookup_action_type" THEN
					ls_coltag = Lower(Describe(is_objects[ll_i] + ".TAG"))
...
long		ll_Pos,ll_NextBlank,ll_find
string ls_tmp,ls_expression //Added By Jay Chen 12-10-2013
boolean lb_modify = true //Added By Jay Chen 12-20-2013
string ls_undo_modify //Added By Jay Chen 02-29-2016
...
choose case as_Property
	case "name"
		...
		if ls_Error <> "" then
			...
			if ll_Pos > 0 and ll_NextBlank > 0 then
				...
				iuo_Parent.of_record_undolist(ls_Modify,ls_OriTextName,'Name') //Added By Jay Chen 02-24-2016
				This.Modify(ls_Modify)
				iuo_parent.Event ue_SetModify(ls_Modify)
	...
	case "text"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Text') //Added By Jay Chen 02-24-2016
	...
	case "visible"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Visible') //Added By Jay Chen 02-24-2016
		//Added By Ken.Guo 07/04/2012
		If ls_FieldName <> '' Then
	...
	case "text_color"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Color') //Added By Jay Chen 02-24-2016
	...
	case "background_color"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'BackgroundColor') //Added By Jay Chen 02-24-2016
	...
	case "alignment"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Alignment') //Added By Jay Chen 02-24-2016
	...
	case "border"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Border') //Added By Jay Chen 02-24-2016
	...
	case 'font_face'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Fontface') //Added By Jay Chen 02-24-2016
	...
	case 'font_height'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Fontheight') //Added By Jay Chen 02-24-2016
	...
	case 'font_weight'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Fontweight') //Added By Jay Chen 02-24-2016
	...
	case 'font_italic'
		...
		if ldw_prop.GetItemString(1, as_Property) = '1' then
			ls_undo_modify = ls_CurTextName + ".font.italic = '0' "
		else
			ls_undo_modify = ls_CurTextName + ".font.italic = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_CurTextName,'italic') //Added By Jay Chen 02-24-2016
	...
	case 'font_underline'
		...
		if ldw_prop.GetItemString(1, as_Property) = '1' then
			ls_undo_modify = ls_CurTextName + ".font.underline = '0' "
		else
			ls_undo_modify = ls_CurTextName + ".font.underline = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_CurTextName,'underline') //Added By Jay Chen 02-24-2016
	...
	case 'font_strikethrough'
		...
		if ldw_prop.GetItemString(1, as_Property) = '1' then
			ls_undo_modify = ls_CurTextName + ".font.strikethrough = '0' "
		else
			ls_undo_modify = ls_CurTextName + ".font.strikethrough = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_CurTextName,'strikethrough') //Added By Jay Chen 02-24-2016
	...
	case 'visibleexpress' 
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'visible expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)	
		Return 1
	Case 'fontcolorexpress'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'fontcolorexpress') //Added By Jay Chen 02-24-2016
	...
	Case 'backgroundcolorexpress'
		...
		ls_Modify  += " " + ls_CurTextName + ".background.mode='2' "
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'backgroundcolorexpress') //Added By Jay Chen 02-24-2016
	...
	case 'slideup'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'slideup') //Added By Jay Chen 02-24-2016
	//Added By Jay Chen 12-10-2013
	case 'text_x'
		ls_Modify = ls_CurTextName + ".x = '" + string(ldw_prop.GetItemNumber(1, as_Property)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'X') //Added By Jay Chen 02-24-2016
	case 'text_y'
		ls_Modify = ls_CurTextName + ".y = '" + string(ldw_prop.GetItemNumber(1, as_Property)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Y') //Added By Jay Chen 02-24-2016
	case 'text_width'
		ls_Modify = ls_CurTextName + ".width = '" + string(ldw_prop.GetItemNumber(1, as_Property)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Width') //Added By Jay Chen 02-24-2016
	case 'text_height'
		ls_Modify = ls_CurTextName + ".height = '" + string(ldw_prop.GetItemNumber(1, as_Property)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Height') //Added By Jay Chen 02-24-2016
	Case 'xexpress'
		lb_modify = false
		ls_tmp =  string(ldw_prop.GetItemNumber(1, "text_x"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression =  ldw_prop.GetItemString(1, as_Property)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_CurTextName + '.x = "' + ls_tmp +'"'
		else
			ls_Modify = ls_CurTextName + '.x = "' + ls_tmp + "~t" + ls_expression +'"'
		end if	
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Xexpress') //Added By Jay Chen 02-24-2016
		//Added By Jay Chen 01-20-2014
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_CurTextName + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"xexp",ls_expression)
		end IF
	Case 'yexpress'
		lb_modify = false
		ls_tmp =  string(ldw_prop.GetItemNumber(1, "text_y"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression =  ldw_prop.GetItemString(1, as_Property)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_CurTextName + '.y = "' + ls_tmp +'"'
		else
			ls_Modify = ls_CurTextName + '.y = "' + ls_tmp + "~t" + ls_expression +'"'
		end if	
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Yexpress') //Added By Jay Chen 02-24-2016
		//Added By Jay Chen 01-20-2014
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_CurTextName + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"yexp",ls_expression)
		end IF
	Case 'widthexpress'
		lb_modify = false
		ls_tmp =  string(ldw_prop.GetItemNumber(1, "text_width"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression =  ldw_prop.GetItemString(1, as_Property)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_CurTextName + '.width = "' + ls_tmp +'"'
		else
			ls_Modify = ls_CurTextName + '.width = "' + ls_tmp + "~t" + ls_expression +'"'
		end if	
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Widthexpress') //Added By Jay Chen 02-24-2016
		//Added By Jay Chen 01-20-2014
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_CurTextName + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"wexp",ls_expression)
		end IF
	Case 'heightexpress'
		lb_modify = false
		ls_tmp =  string(ldw_prop.GetItemNumber(1, "text_height"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression =  ldw_prop.GetItemString(1, as_Property)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_CurTextName + '.height = "' + ls_tmp +'"'
		else
			ls_Modify = ls_CurTextName + '.height = "' + ls_tmp + "~t" + ls_expression +'"'
		end if	
		iuo_Parent.of_record_undolist(ls_Modify,ls_CurTextName,'Heightexpress') //Added By Jay Chen 02-24-2016
		//Added By Jay Chen 01-20-2014
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_CurTextName + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"hexp",ls_expression)
		end IF
		//End add
	case else
		Return 0
...
if lb_modify then 
	...
	if as_Property = 'text_x' or as_Property = 'text_y' or as_Property = 'text_width' or as_Property = 'text_height' then
		if isvalid(w_contract_data_design) then
			w_contract_data_design.uo_design.of_refreshmarkpos()
		elseif isvalid(w_contract_report_design) then
			w_contract_report_design.uo_design.of_refreshmarkpos()
		end if
	end if
end if
//this.Modify(ls_Modify) //Modified By Jay Chen 12-20-2013
...
CHOOSE CASE as_Property
	CASE "visible"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'visible') //Added By Jay Chen 02-24-2016
	CASE "background_color"
		if Pos(ls_PropExp,"~t") <= 0 then ls_PropExp = "0" + TABCHAR + ls_PropExp
		ls_Modify = ls_Object + ".Background.Color = ~"" + ls_PropExp + '"'
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'background color') //Added By Jay Chen 02-24-2016
	CASE "brush_color"
		if Pos(ls_PropExp,"~t") <= 0 then ls_PropExp = "0" + TABCHAR + ls_PropExp
		ls_Modify = ls_Object + ".Brush.Color = ~"" + ls_PropExp + '"'
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'brush color') //Added By Jay Chen 02-24-2016
	CASE "brush_hatch"
		if Pos(ls_PropExp,"~t") <= 0 then ls_PropExp = "6" + TABCHAR + ls_PropExp
		ls_Modify = ls_Object + ".Brush.Hatch = ~"" + ls_PropExp + '"'
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'brush hatch') //Added By Jay Chen 02-24-2016
	CASE "pen_color"
		if Pos(ls_PropExp,"~t") <= 0 then ls_PropExp = "0" + TABCHAR + ls_PropExp
		ls_Modify = ls_Object + ".Pen.Color = ~"" + ls_PropExp + '"'
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'pen color') //Added By Jay Chen 02-24-2016
	CASE "pen_style"
		if Pos(ls_PropExp,"~t") <= 0 then ls_PropExp = "0" + TABCHAR + ls_PropExp
		ls_Modify = ls_Object + ".Pen.Style = ~"" + ls_PropExp + '"'
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'pen style') //Added By Jay Chen 02-24-2016
	CASE "pen_width"
		if Pos(ls_PropExp,"~t") <= 0 then ls_PropExp = "1" + TABCHAR + ls_PropExp
		ls_Modify = ls_Object + ".Pen.Width = ~"" + ls_PropExp + '"'
		iuo_Parent.of_record_undolist(ls_Modify,ls_Object,'pen width') //Added By Jay Chen 02-24-2016
	CASE else
		Return 0
end CHOOSE
...
string ls_action,ls_dwSyntax,ls_dwObjSyntax //Added By Jay Chen 02-26-2016
...
//Added By Jay Chen 12-19-2013
String ls_x,ls_y,ls_Modify2,ls_Expression,ls_x1,ls_x2,ls_y1,ls_y2,ls_Expression1,ls_Expression2
long ll_find
if This.Describe(as_ObjName + ".type") <> 'line' THEN
	if al_Diff_X <> 0 then	
		//Added By Jay Chen 12-19-2013
		ls_expression = ''
		ls_x = This.Describe(as_ObjName + ".X")
		if Pos( ls_x, '~t' ) > 0 then
			ls_Expression = Mid( ls_x,  Pos( ls_x, '~t' ) + 1 ) 	
			ls_Expression = Left( ls_Expression, Len( ls_Expression ) - 1 )
			ls_x = Mid( ls_x, 2, Pos( ls_x, '~t' ) - 2 )
		else 
			ll_find = iuo_parent.ids_object.find("colname = '" + as_ObjName + "'",1,iuo_parent.ids_object.rowcount())
			if ll_find > 0 then
				ls_Expression = iuo_parent.ids_object.getitemstring(ll_find,"xexp")
			end if
		end if	
		ll_X1 = long(ls_x) + al_Diff_X
//		ll_X1 = long(This.Describe(as_ObjName + ".X")) + al_Diff_X 
		...
		if isnull(ls_expression) or ls_expression = '' then
			...
			ls_Modify2 += as_ObjName + '.X = "' + String(ll_X1) + "~t" + ls_expression + '"'
		end if
	end if
	if al_Diff_Y <> 0 then
		//Added By Jay Chen 12-19-2013
		ls_expression = ''
		ls_y = This.Describe(as_ObjName + ".Y")
		if Pos( ls_y, '~t' ) > 0 then
			ls_Expression = Mid( ls_y,  Pos( ls_y, '~t' ) + 1 ) 	
			ls_Expression = Left( ls_Expression, Len( ls_Expression ) - 1 )
			ls_y = Mid( ls_y, 2, Pos( ls_y, '~t' ) - 2 )
		else 
			ll_find = iuo_parent.ids_object.find("colname = '" + as_ObjName + "'",1,iuo_parent.ids_object.rowcount())
			if ll_find > 0 then
				ls_Expression = iuo_parent.ids_object.getitemstring(ll_find,"yexp")
			end if
		end if
		ll_Y1 = long(ls_y) + al_Diff_Y
//		ll_Y1 = long(This.Describe(as_ObjName + ".Y")) + al_Diff_Y
		...
		if isnull(ls_expression) or ls_expression = '' then
			...
			ls_Modify2 += as_ObjName + '.Y = "' + String(ll_Y1) + "~t" + ls_expression + '"'
		end if
	end if
else
	if al_Diff_X <> 0 then
...
//		ll_X2 = long(This.Describe(as_ObjName + ".X2")) + al_Diff_X
		ls_Expression1 = ''
		ls_Expression2 = ''
		ls_x1 = This.Describe(as_ObjName + ".X1")
		ls_x2 = This.Describe(as_ObjName + ".X2")
		if Pos( ls_x1, '~t' ) > 0 then
			ls_Expression1 = Mid( ls_x1,  Pos( ls_x1, '~t' ) + 1 ) 	
			ls_Expression1 = Left( ls_Expression1, Len( ls_Expression1 ) - 1 )
			ls_x1 = Mid( ls_x1, 2, Pos( ls_x1, '~t' ) - 2 )
		end if
		if Pos( ls_x2, '~t' ) > 0 then
			ls_Expression2 = Mid( ls_x2,  Pos( ls_x2, '~t' ) + 1 ) 	
			ls_Expression2 = Left( ls_Expression2, Len( ls_Expression2 ) - 1 )
			ls_x2 = Mid( ls_x2, 2, Pos( ls_x2, '~t' ) - 2 )
		end if
		ll_find = iuo_parent.ids_object.find("colname = '" + as_ObjName + "'",1,iuo_parent.ids_object.rowcount())
		if ll_find > 0 then
			ls_Expression1 = iuo_parent.ids_object.getitemstring(ll_find,"xexp")
			ls_Expression2 = iuo_parent.ids_object.getitemstring(ll_find,"wexp")
		end if
			
		ll_X1 = long(ls_x1) + al_Diff_X
		ll_X2 = long(ls_x2) + al_Diff_X
		if ll_X1 < 0 then ll_X1 = 0
		...
		if isnull(ls_expression1) or ls_expression1 = '' then
			ls_Modify2 =ls_Modify
		else
			ls_Modify2 += as_ObjName + '.X1 = "' + String(ll_X1) + "~t" + ls_expression1 + '"'
		end if
		
		ls_Modify += " " + as_ObjName + ".X2 = " + String(ll_X2)
		if isnull(ls_expression2) or ls_expression2 = '' then
			ls_Modify2 =ls_Modify
		else
			ls_Modify2 += as_ObjName + '.X2 = "' + String(ll_X2) + "~t" + ls_expression2 + '"'
		end if
	END IF
...
//		ll_Y2 = long(This.Describe(as_ObjName + ".Y2")) + al_Diff_Y
		ls_Expression1 = ''
		ls_Expression2 = ''
		ls_y1 = This.Describe(as_ObjName + ".Y1")
		ls_y2 = This.Describe(as_ObjName + ".Y2")
		if Pos( ls_y1, '~t' ) > 0 then
			ls_Expression1 = Mid( ls_y1,  Pos( ls_y1, '~t' ) + 1 ) 	
			ls_y1 = Mid( ls_y1, 2, Pos( ls_y1, '~t' ) - 2 )
		else 
			ll_find = iuo_parent.ids_object.find("colname = '" + as_ObjName + "'",1,iuo_parent.ids_object.rowcount())
			if ll_find > 0 then
				ls_Expression1 = iuo_parent.ids_object.getitemstring(ll_find,"yexp")
			end if
		end if
		if Pos( ls_y2, '~t' ) > 0 then
			ls_Expression2 = Mid( ls_y2,  Pos( ls_y2, '~t' ) + 1 ) 	
			ls_x2 = Mid( ls_y2, 2, Pos( ls_y2, '~t' ) - 2 )
		else 
			ll_find = iuo_parent.ids_object.find("colname = '" + as_ObjName + "'",1,iuo_parent.ids_object.rowcount())
			if ll_find > 0 then
				ls_Expression2 = iuo_parent.ids_object.getitemstring(ll_find,"hexp")
			end if
		end if
		ll_Y1 = long(ls_y1) + al_Diff_Y
		ll_Y2 = long(ls_y2) + al_Diff_Y
		...
		if isnull(ls_expression1) or ls_expression1 = '' then
			...
			ls_Modify2 += as_ObjName + '.Y1 = "' + String(ll_Y1) + "~t" + ls_expression1 + '"'
		end if
		...
		if isnull(ls_expression1) or ls_expression1 = '' then
			...
			ls_Modify2 += as_ObjName + '.Y2 = "' + String(ll_Y2) + "~t" + ls_expression2 + '"'
		end if
	end if
end if

il_record_move++ //Added By Jay Chen 03-02-2016

//Added By Jay Chen 02-26-2016 
string ls_w,ls_h
ls_action = 'Move ' + as_ObjName
//ll_find = iuo_Parent.ids_step_list.find( "step_name = '"+ls_action+"'" ,1, iuo_Parent.ids_step_list.rowcount())
//if isnull(ll_find) then ll_find = 0
//if ll_find = 0 then
//if il_record_move = 1 then //Added By Jay Chen 03-02-2016
if il_record_move <= iuo_parent.of_GetSelectedObjectCnts() then //Modified By Ken.Guo 2016-04-13. support move multi objects.
	if this.describe(as_ObjName + ".type") <> 'line' then
		ls_x = This.Describe(as_ObjName + ".X")
		ls_y = This.Describe(as_ObjName + ".Y")
		ls_w = This.Describe(as_ObjName + ".Width")
		ls_h = This.Describe(as_ObjName + ".Height")
		ls_dwObjSyntax =	as_ObjName + ".x = " + ls_x
		ls_dwObjSyntax += " " + as_ObjName + ".y = " + ls_y
		ls_dwObjSyntax += " " + as_ObjName + ".width = " + ls_w
		ls_dwObjSyntax += " " + as_ObjName + ".height = " + ls_h
	else
		ls_x1 = This.Describe(as_ObjName + ".x1")
		ls_x2 = This.Describe(as_ObjName + ".x2")
		ls_y1 = This.Describe(as_ObjName + ".y1")
		ls_y2 = This.Describe(as_ObjName + ".y2")
		ls_dwObjSyntax =	as_ObjName + ".x1 = " + ls_x1
		ls_dwObjSyntax += " " + as_ObjName + ".x2 = " + ls_x2
		ls_dwObjSyntax += " " + as_ObjName + ".y1 = " + ls_y1
		ls_dwObjSyntax += " " + as_ObjName + ".y2 = " + ls_y2
	end if
	iuo_Parent.of_record_undolist(ls_dwObjSyntax,as_ObjName,'Move' + String(il_record_move)) 
end if
//end

This.Modify(ls_Modify)
//iuo_Parent.Event ue_SetModify(ls_Modify)
iuo_Parent.Event ue_SetModify(ls_Modify2)
...
//
//====================================================================
String ls_x,ls_y,ls_width,ls_height //Added By Jay Chen 12-10-2013
...
//	al_Height = long(This.Describe(as_ObjName + ".Y2"))
	ls_x = This.Describe(as_ObjName + ".X1")
	If Pos( ls_x, '~t' ) > 0 Then ls_x = Mid( ls_x, 2, Pos( ls_x, '~t' ) - 2 )
	ls_y = This.Describe(as_ObjName + ".Y1")
	If Pos( ls_y, '~t' ) > 0 Then ls_y = Mid( ls_y, 2, Pos( ls_y, '~t' ) - 2 )
	ls_width  = This.Describe(as_ObjName + ".X2")
	If Pos( ls_width, '~t' ) > 0 Then ls_width = Mid( ls_width, 2, Pos( ls_width, '~t' ) - 2 )
	ls_height = This.Describe(as_ObjName + ".Y2")
	If Pos( ls_height, '~t' ) > 0 Then ls_height = Mid( ls_height, 2, Pos( ls_height, '~t' ) - 2 )
	al_X = long(ls_x)
	al_Y = long(ls_y)
	al_Width  = long(ls_width)
	al_Height = long(ls_height)
else
	//Modified By Jay Chen 12-10-2013
...
//	al_Height = long(This.Describe(as_ObjName + ".Height"))
	...
	ls_y = This.Describe(as_ObjName + ".Y")
	If Pos( ls_y, '~t' ) > 0 Then ls_y = Mid( ls_y, 2, Pos( ls_y, '~t' ) - 2 )
	ls_width  = This.Describe(as_ObjName + ".Width")
	If Pos( ls_width, '~t' ) > 0 Then ls_width = Mid( ls_width, 2, Pos( ls_width, '~t' ) - 2 )
	ls_height = This.Describe(as_ObjName + ".Height")
	If Pos( ls_height, '~t' ) > 0 Then ls_height = Mid( ls_height, 2, Pos( ls_height, '~t' ) - 2 )
	al_X = long(ls_x)
	al_Y = long(ls_y)
	al_Width  = long(ls_width)
...
CHOOSE CASE as_Property_Name
	CASE "fields_label_border"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'LabelBorder') //Added By Jay Chen 02-24-2016
	...
	CASE "fields_border"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Border') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "data_view_fields_visible"
		...
		if ls_Visible = '0' then
			ls_undo_modify = ls_Column_Name + ".visible = '1' "
		else
			ls_undo_modify = ls_Column_Name + ".visible = '0' "
		end if
//		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Visible') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)
		...
		if Left(ls_Column_Name,Len("object")) <> "object" then
			if iuo_Parent.uo_band.dw_Syntax.Describe(ls_LabelName + ".Name") = ls_LabelName then
				ls_Modify = ls_LabelName + ".visible = '" + ls_Visible + "' "
				 //Added By Jay Chen 03-01-2016
				if ls_Visible = '0' then
					ls_undo_modify = ls_undo_modify + ' ' + ls_LabelName + ".visible = '1' "
				else
					ls_undo_modify = ls_undo_modify + ' ' + ls_LabelName + ".visible = '0' "
				end if
				if ls_Visible = '0' then
				...
				//Added By Ken.Guo 2016-04-29
				If ldw_band.describe(ls_LabelName + ".text ") <> ls_FieldLabel Then
					iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'labeltext')
				End If
		...
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'Visible') //Modified By Jay Chen 03-01-2016
	...
	CASE "data_view_fields_field_label"
		ls_FieldLabel = iuo_parent.dw_properties.GetItemString(1, "data_view_fields_field_label")
		ls_Modify = ls_LabelName + ".text = ~"" + ls_FieldLabel + "~""
		If ldw_band.describe(ls_LabelName + ".text ") <> ls_FieldLabel Then //Modified By Ken.Guo 2016-04-29. added if
			iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'labeltext') //Added By Jay Chen 02-24-2016
		End If
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	...
	CASE "data_view_fields_label_justification"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'Alignment') //Added By Jay Chen 02-24-2016
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "data_view_fields_field_order"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'FieldOrder') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
		
		//Added By Ken.Guo 2016-02-24
		If iuo_parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".edit.style") = 'dddw' Then 
			if (iuo_parent.il_dw_style = 1 and ( iuo_parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".tabsequence") = '0' or iuo_parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".Protect") = '1' )) OR iuo_parent.ii_screen_id = 2 then  //Added By Ken.Guo 2016-02-24
				If Lower(iuo_parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".DDDW.UseAsBorder")) <> 'no' Then  
					iuo_parent.uo_band.dw_detail.Modify( ls_Column_Name + ".DDDW.UseAsBorder = No" )
					iuo_parent.event ue_setmodify(  ls_Column_Name + ".DDDW.UseAsBorder = No"  ) 
				End If
			else
				If Lower(iuo_parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".DDDW.UseAsBorder")) <> 'yes' Then  
					iuo_parent.uo_band.dw_detail.Modify( ls_Column_Name + ".DDDW.UseAsBorder = Yes" )
					iuo_parent.event ue_setmodify(  ls_Column_Name + ".DDDW.UseAsBorder = Yes"  ) 
				End If
			end if			
		End If		
	...
	CASE "data_view_fields_required"
		...
		if iuo_Parent.dw_properties.GetItemString(1, "data_view_fields_required") = "Y" then
			ls_undo_band_syntax = ls_Column_Name + "." + ls_Edit_Style + ".Required = 'No'"
			ls_undo_band_syntax += ls_LabelName + ".font.underline = '0' "
		else
			ls_undo_band_syntax = ls_Column_Name + "." + ls_Edit_Style + ".Required = 'Yes'"
			ls_undo_band_syntax += ls_LabelName + ".font.underline = '1' "
		end if
		gf_record_screen_step(ls_Column_Name,'Required',ls_undo_band_syntax,ls_undo_band_syntax) 
		//end add 
		// Required
		if iuo_Parent.dw_properties.GetItemString(1, "data_view_fields_required") = "Y" then
	...
	CASE "data_view_fields_justification"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Alignment') //Added By Jay Chen 02-24-2016
		//added by gavins 20120510
	...
	CASE "data_view_fields_field_x"
		ls_Modify = ls_Column_Name + ".x = '" + string(iuo_Parent.dw_properties.GetItemNumber(1, as_Property_Name)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'X') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		//Added By Jay Chen 12-31-2013
		if isvalid(w_contract_data_design) then
			w_contract_data_design.uo_design.of_refreshmarkpos()
		...
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "data_view_fields_field_y"
		ls_Modify = ls_Column_Name + ".y = '" + string(iuo_Parent.dw_properties.GetItemNumber(1, as_Property_Name)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Y') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		if isvalid(w_contract_data_design) then
			w_contract_data_design.uo_design.of_refreshmarkpos()
		elseif isvalid(w_contract_report_design) then
		...
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "data_view_fields_field_width"
		ls_Modify = ls_Column_Name + ".width = '" + string(iuo_Parent.dw_properties.GetItemNumber(1, as_Property_Name)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Width') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		if isvalid(w_contract_data_design) then
			w_contract_data_design.uo_design.of_refreshmarkpos()
		elseif isvalid(w_contract_report_design) then
		...
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "data_view_fields_field_height"
		ls_Modify = ls_Column_Name + ".height = '" + string(iuo_Parent.dw_properties.GetItemNumber(1, as_Property_Name)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Height') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		if isvalid(w_contract_data_design) then
			w_contract_data_design.uo_design.of_refreshmarkpos()
		elseif isvalid(w_contract_report_design) then
	...
	CASE "sys_fields_drop_down_width"
		...
		if ls_Edit_Style = "dddw" then
			...
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Dropdown width') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	CASE "sys_fields_field_case"
		...
		if ls_Edit_Style = "edit" OR ls_Edit_Style = "dddw" OR ls_Edit_Style = "ddlb" then
			...
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Case') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	CASE "sys_fields_field_mask"
		...
		ls_column_type = iuo_Parent.dw_properties.GetItemString(1, "sys_fields_field_type" ) 
		if IsNull(ls_EditMask) or Trim(ls_EditMask) = "" or Trim(ls_EditMask) = "null" then ls_EditMask = ""
		if IsNull(ls_column_type) or Trim(ls_column_type) = "" or Trim(ls_column_type) = "null" then ls_column_type = ""
		...
		Else
			...
			*/
			ls_Modify = ls_Column_Name + ".EditMask.Mask = '" + ls_EditMask + "' "
			if lower(ls_column_type) = "datetime" then 				//Added By Mark Lee 09/26/2014
//				ls_Modify = ls_Column_Name + ".Edit.Format = '" + ls_EditMask + "' "	//Modified By Jay Chen 04-17-2015
				if left(lower(ls_EditMask),3) <> 'hh:' then //hh:mm set edit style to editmask
					ls_Modify = ls_Column_Name + ".Edit.Format = '" + ls_EditMask + "' "
				end if
			end if 
		End If
		...
		//Added By Jay Chen 06-29-2015 fix the bug: can not set default value to 00/00/0000 from time format to datetime format.
		if lower(ls_column_type) = "datetime" then 
			if left(lower(ls_EditMask),3) <> 'hh:' then
				ls_Modify += ls_Column_Name + ".EditMask.Mask = '" + ls_EditMask + "' "
			end if
		end if
		//end
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Mask') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "data_view_fields_display_only"
		...
		if ls_Edit_Style = "edit" then
			...
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Displayonly') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	CASE "sys_fields_lookup_field"
		...
		elseif ll_find = 0  and ls_str = 'Y' then
			...
			iuo_parent.ids_field_ex.SetItem(ll_row,"field_Add",'N') //Modified By Jay Chen 04-22-2015 Modify default value to N
		end if
		...
		choose case ls_str
			...
			case 'C'	//Company
				ls_Modify = ''
			case 'U'	//User
				//if iuo_parent.il_dw_style = 1 OR iuo_parent.ii_screen_id = 2 then  //Commented By Ken.Guo 2016-02-24
				if (iuo_parent.il_dw_style = 1 and ( iuo_Parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".tabsequence") = '0' or iuo_Parent.uo_band.dw_Syntax.Describe(ls_Column_Name + ".Protect") = '1' )) OR iuo_parent.ii_screen_id = 2 then  //Added By Ken.Guo 2016-02-24
					ls_UseAsBorder = 'No'
				else
					ls_UseAsBorder = 'Yes'
		...
		//Added By Jay Chen 02-29-2016
		of_get_object_attribute(iuo_Parent.uo_band.dw_Syntax,lds_properties,ls_Column_Name)
		ls_undo_modify = of_modify_object_attribute(lds_properties)
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'Field Style') 
		//end
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	...
	case 'fields_cbx_on'
		...
		if  iuo_parent.dw_properties.GetItemString(1, "sys_fields_lookup_field") = 'B' then 
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Checkbox On') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	case 'fields_cbx_off'
		...
		if  iuo_parent.dw_properties.GetItemString(1, "sys_fields_lookup_field") = 'B' then 
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Checkbox Off') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	case 'fields_cbx_other'
		...
		if  iuo_parent.dw_properties.GetItemString(1, "sys_fields_lookup_field") = 'B' then 
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Checkbox Other') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	case 'fields_3dlock'
		...
		if  iuo_parent.dw_properties.GetItemString(1, "sys_fields_lookup_field") = 'B' then 
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'3D Lock') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	case 'fields_3dstate'
	case 'fields_cbx_text'
		...
		if  iuo_parent.dw_properties.GetItemString(1, "sys_fields_lookup_field") = 'B' then 
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Checkbox Text') //Added By Jay Chen 02-24-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	CASE "sys_fields_lookup_code"
		...
		choose case ls_str
			case 'Y'	// Lookup Table
				...
				iuo_parent.dw_properties.SetItem( 1, "fields_add_lookup", 'N' ) //Jervis 01.26.2011 //Modified By Jay Chen 04-21-2015 Modify default value to N
				
			case 'C'	//Company
				//change 'd_dddw_company' to 'd_dddw_company_list' -- jervis 04.15.2011
				//Only add tag properties for app_facility field for screen #3 //Added By Ken.Guo 2015-11-26
				If  iuo_parent.ii_screen_id = 3 and ls_Column_Name = 'app_facility' Then
					ls_Modify = ls_Column_Name + ".tag = '" + String(iuo_parent.dw_properties.GetItemString(1, "sys_fields_lookup_code")) + "'"					
				Else
					ls_Modify = ls_Column_Name + ".dddw.VscrollBar = Yes " + ls_Column_Name + ".dddw.AutoHScroll = Yes " + ls_Column_Name + ".dddw.useasborder = " + ls_UseAsBorder + " " + &
		...
		of_get_object_attribute(iuo_Parent.uo_band.dw_Syntax,lds_properties,ls_Column_Name)
		ls_undo_modify = of_modify_object_attribute(lds_properties)
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'Lookup Table Name') 
		//end
//		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Lookup Table Name') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "sys_fields_lookup_field_name"
		...
		of_get_object_attribute(iuo_Parent.uo_band.dw_Syntax,lds_properties,ls_Column_Name)
		ls_undo_modify = of_modify_object_attribute(lds_properties)
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'Display Name') 
//		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Display Name') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "sys_fields_default_value"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Default Vaule') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "fields_label_textcolor"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'Color') //Added By Jay Chen 02-24-2016
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE "fields_label_textbkcolor"
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'BackgroundColor') //Added By Jay Chen 02-24-2016
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'fields_vscrollbar'
		if ls_Edit_Style = "edit" OR ls_Edit_Style = "dddw" OR ls_Edit_Style = "ddlb" then
			...
			iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Word Wrap') //Added By Jay Chen 03-01-2016
			This.Modify(ls_Modify)
			iuo_Parent.Event ue_SetModify(ls_Modify)
		end if
	case 'fields_textcolor'
		ls_Modify = ls_Column_Name + ".color = " + String(iuo_Parent.dw_properties.GetItemNumber(1, "fields_textcolor"))
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Color') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'fields_textbkcolor'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'BackgroundColor') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	...
	Case 'fields_protect'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Protect') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)
	...
	Case 'fields_slideup'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'SlideUp') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	Case 'fields_autoheight'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'AutoSize') //Added By Jay Chen 02-24-2016
		//This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	...
	case 'font_face'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Font face') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'font_height'
		ls_Modify = ls_Column_Name + ".font.height = '-" + string(iuo_Parent.dw_properties.GetItemNumber(1, as_Property_Name)) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Font height') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'font_weight'
		ls_Modify = ls_Column_Name + ".font.weight = '" + iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'Font weight') //Added By Jay Chen 02-24-2016
		//added by gavins 20120510
		iuo_Parent.of_synchronization_toolbar( "bold" )
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'font_italic'
		...
		if iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) = '1' then
			ls_undo_modify = ls_Column_Name + ".font.italic = '0' "
		else
			ls_undo_modify = ls_Column_Name + ".font.italic = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'Italic') //Added By Jay Chen 02-24-2016
		//added by gavins 20120510
		iuo_Parent.of_synchronization_toolbar( "italic" )
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'font_underline'
		...
		if iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) = '1' then
			ls_undo_modify = ls_Column_Name + ".font.underline = '0' "
		else
			ls_undo_modify = ls_Column_Name + ".font.underline = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'underline') //Added By Jay Chen 02-24-2016
		//added by gavins 20120510
		iuo_Parent.of_synchronization_toolbar( "underline" )
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'font_strikethrough'
		...
		if iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) = '1' then
			ls_undo_modify = ls_Column_Name + ".font.strikethrough = '0' "
		else
			ls_undo_modify = ls_Column_Name + ".font.strikethrough = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_Column_Name,'strikethrough') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	...
	CASE 'label_font_face' 
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'font face') //Added By Jay Chen 02-24-2016
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	CASE 'label_font_height' 
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'font height') //Added By Jay Chen 02-24-2016
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	case 'label_font_weight'
		ls_Modify = ls_LabelName + ".font.weight = '" + iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) + "'"
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'font weight') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'label_font_italic'
		ls_Modify = ls_LabelName + ".font.italic = '" + iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) + "'"
		if iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) = '1' then
			ls_undo_modify = ls_LabelName + ".font.italic = '0' "
		else
			ls_undo_modify = ls_LabelName + ".font.italic = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_LabelName,'italic') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'label_font_underline'
		ls_Modify = ls_LabelName + ".font.underline = '" + iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) + "'"
		if iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) = '1' then
			ls_undo_modify = ls_LabelName + ".font.underline = '0' "
		else
			ls_undo_modify = ls_LabelName + ".font.underline = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_LabelName,'underline') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	case 'label_font_strikethrough'
		...
		if iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) = '1' then
			ls_undo_modify = ls_LabelName + ".font.strikethrough = '0' "
		else
			ls_undo_modify = ls_LabelName + ".font.strikethrough = '1' "
		end if	
		iuo_Parent.of_record_undolist(ls_undo_modify,ls_LabelName,'strikethrough') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)	
...
//		This.Modify(ls_Modify)
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'visible expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	Case 'fields_protectexpress' //Added By Jay Chen 09-10-2015
		if len(iuo_Parent.dw_properties.GetItemString(1, as_Property_Name)) > 0 then
			ls_Modify = ls_Column_Name + '.protect = "' + "0~t" + iuo_Parent.dw_properties.GetItemString(1, as_Property_Name) + '"'
		else
			ls_Modify = ls_Column_Name + '.protect = 0 '
		end if
//		This.Modify(ls_Modify)
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'protect expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)
	Case 'fields_backgroundcolorexpress'
		...
		ls_Modify  += " " + ls_Column_Name + ".background.mode='2' "
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'backgroundcolor expression') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	Case 'fields_fontcolorexpress'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'fontcolor expression') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	Case 'label_fontcolorexpress'
		...
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'fontcolor expression') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)
	Case 'label_backgroundcolorexpress'
		...
		ls_Modify  += " " + ls_LabelName + ".background.mode='2' "
		iuo_Parent.of_record_undolist(ls_Modify,ls_LabelName,'backgroundcolor expression') //Added By Jay Chen 02-24-2016
		This.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)		
	//Added By Jay Chen 12-09-2013
	Case 'fields_xexpress'
		ls_tmp =  string(iuo_Parent.dw_properties.GetItemNumber(1, "data_view_fields_field_x"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression = iuo_Parent.dw_properties.GetItemString(1, as_Property_Name)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_Column_Name + '.x = "' + ls_tmp +'"'
		else
			ls_Modify = ls_Column_Name + '.x = "' + ls_tmp + "~t" + ls_expression +'"'
		end if
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_Column_Name + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"xexp",ls_expression)
		end IF
//		This.Modify(ls_Modify)
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'x expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	Case 'fields_yexpress'
		ls_tmp =  string(iuo_Parent.dw_properties.GetItemNumber(1, "data_view_fields_field_y"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression = iuo_Parent.dw_properties.GetItemString(1, as_Property_Name)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_Column_Name + '.y = "' + ls_tmp +'"'
		else
			ls_Modify = ls_Column_Name + '.y = "' + ls_tmp + "~t" + ls_expression + '"'
		end if
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_Column_Name + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"yexp",ls_expression)
...
//		This.Modify(ls_Modify)
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'y expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	Case 'fields_widthexpress'
		ls_tmp =  string(iuo_Parent.dw_properties.GetItemNumber(1, "data_view_fields_field_width"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression = iuo_Parent.dw_properties.GetItemString(1, as_Property_Name)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_Column_Name + '.width = "' + ls_tmp +'"'
		else
			ls_Modify = ls_Column_Name + '.width = "' + ls_tmp + "~t" + ls_expression + '"'
		end if
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_Column_Name + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"wexp",ls_expression)
...
//		This.Modify(ls_Modify)
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'width expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)
	Case 'fields_heightexpress'
		ls_tmp =  string(iuo_Parent.dw_properties.GetItemNumber(1, "data_view_fields_field_height"))
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = '0'
		ls_expression = iuo_Parent.dw_properties.GetItemString(1, as_Property_Name)
		if isnull(ls_expression) or ls_expression = '' then
			ls_Modify = ls_Column_Name + '.height = "' + ls_tmp +'"'
		else
			ls_Modify = ls_Column_Name + '.height = "' + ls_tmp + "~t" + ls_expression + '"'
		end if
		ll_find = iuo_Parent.ids_object.find("colname = '" + ls_Column_Name + "'",1,iuo_Parent.ids_object.rowcount())
		if ll_find > 0 then
			iuo_Parent.ids_object.setitem(ll_find,"hexp",ls_expression)
...
//		This.Modify(ls_Modify)
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'height expression') //Added By Jay Chen 02-24-2016
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	//end add
	Case 'fields_validation'						//Added By Mark Lee 07/28/2015
		ls_tmp = iuo_parent.dw_properties.GetItemString(1, "fields_validation")
		If Isnull( ls_tmp ) Or Trim( ls_tmp ) = '' Then ls_tmp = ''
		if ls_tmp ='' then 
			ls_Modify = ls_Column_Name + '.validation ="" '+ls_Column_Name+'.validationmsg="" '
		elseif (lower(ls_tmp) = lower("Email Address")) then 
			ls_Modify = ls_Column_Name +'.validation="trim(gettext())= ~'~' or isnull(gettext()) " '+ls_Column_Name+'.validationmsg="Please enter a valid email address such as sample@domain.com or multiple ones separated by ~';~'" ' 			
		else
			ls_Modify = ls_Column_Name + '.validation ="" '+ls_Column_Name+'.validationmsg="" '
		end if 
		iuo_Parent.of_record_undolist(ls_Modify,ls_Column_Name,'validation') //Added By Jay Chen 02-24-2016
		if IsValid(ldw_band) then ldw_band.Modify(ls_Modify)
		iuo_Parent.Event ue_SetModify(ls_Modify)	
	Case 'field_use_compute' //Added By Ken.Guo 2016-08-16
		ll_Find = iuo_parent.ids_field_ex.find( "field_name='" + ls_Column_Name + "'" , 1, iuo_parent.ids_field_ex.RowCount())
		if ll_find = 0 then 
			ll_find = iuo_parent.ids_field_ex.InsertRow( 0)
			iuo_parent.ids_field_ex.SetItem(ll_find,"data_view_id",iuo_parent.il_data_view_id )
			iuo_parent.ids_field_ex.SetItem(ll_find,"screen_id",iuo_parent.ii_screen_id )
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_name",ls_Column_Name)
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_use_compute",0)
		end if
		if ll_find > 0 then 
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_use_compute",iuo_Parent.dw_properties.GetItemNumber(1,"field_use_compute"))		
		end if
		if IsValid(m_pfe_cst_contract_data_design) then
			gnv_app.of_modify_menu_attr( m_pfe_cst_contract_data_design.m_file.m_save,'Enabled', true)
		end if		
	Case 'field_compute_expression' //Added By Ken.Guo 2016-08-16
		ll_Find = iuo_parent.ids_field_ex.find( "field_name='" + ls_Column_Name + "'" , 1, iuo_parent.ids_field_ex.RowCount())
		if ll_find = 0 then 
			ll_find = iuo_parent.ids_field_ex.InsertRow( 0)
			iuo_parent.ids_field_ex.SetItem(ll_find,"data_view_id",iuo_parent.il_data_view_id )
			iuo_parent.ids_field_ex.SetItem(ll_find,"screen_id",iuo_parent.ii_screen_id )
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_name",ls_Column_Name)
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_compute_expression","")
		end if
		if ll_find > 0 then 
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_compute_expression",iuo_Parent.dw_properties.GetItemString(1,"field_compute_expression"))		
		end if
		if IsValid(m_pfe_cst_contract_data_design) then
			gnv_app.of_modify_menu_attr( m_pfe_cst_contract_data_design.m_file.m_save,'Enabled', true)
		end if	
	Case 'sort_expression', 'filter_expression' //Added By Ken.Guo 2017-03-02 task 722
		ll_Find = iuo_parent.ids_field_ex.find( "field_name='" + ls_Column_Name + "'" , 1, iuo_parent.ids_field_ex.RowCount())
		if ll_find = 0 then 
			ll_find = iuo_parent.ids_field_ex.InsertRow( 0)
			iuo_parent.ids_field_ex.SetItem(ll_find,"data_view_id",iuo_parent.il_data_view_id )
			iuo_parent.ids_field_ex.SetItem(ll_find,"screen_id",iuo_parent.ii_screen_id )
			iuo_parent.ids_field_ex.SetItem(ll_find,"field_name",ls_Column_Name)
			iuo_parent.ids_field_ex.SetItem(ll_find,as_property_name,"")
		end if
		if ll_find > 0 then 
			iuo_parent.ids_field_ex.SetItem(ll_find,as_property_name,iuo_Parent.dw_properties.GetItemString(1,as_property_name))		
		end if
		if IsValid(m_pfe_cst_contract_data_design) then
			gnv_app.of_modify_menu_attr( m_pfe_cst_contract_data_design.m_file.m_save,'Enabled', true)
		end if			
...
String 	ls_optionname, ls_option_value, ls_user_id
...
Choose Case as_type
	Case "Destroy"
		...
		Else
			...
			End IF
		End If 
		//Begin - Added By Jay 2017-01-18
		of_set_button_dotdd_company(ls_Column_Name,'N')		
		//End - Added By Jay 2017-01-18
		...
		If iuo_Parent.dw_properties.GetItemString(1, "fields_add_dotdd_company") = "Y" Then
			...
			Else
				ls_Modify = " create button(band=detail text='...' enabled=yes action='0' border='0' color='0' x='"+string(ll_x)+"' y='"+string(ll_y)+"' height='76' width='69' " +&
								" vtextalign='0' htextalign='0'  name=b_"+ls_Column_Name+"_dotdd_company visible='1'  font.face='MS Sans Serif' font.height='-8' font.weight='400'  " + &
			...
			End If
			//Begin - Added By Jay 2017-01-18
			of_set_button_dotdd_company(ls_Column_Name,'Y')		
			//End - Added By Jay 2017-01-18
...
public function integer of_get_object_attribute (datawindow ids_terget, ref datastore ids_properties, string as_objname);//====================================================================
// Function: of_get_object_attribute
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value        datastore    ids_terget
// 	reference    datastore    ids_properties
// 	value        string       as_objname
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
...
public function string of_modify_object_attribute (datastore ads_properties);//====================================================================
// Function: of_modify_object_attribute
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    datastore    ids_properties
//--------------------------------------------------------------------
// Returns:  string
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 02-29-2016
...
for i = 1 to li_count
	...
	if ls_provalue = '!' or ls_provalue = '?' then continue
	if isnull(ls_provalue) then continue
	
	if len(ls_modify) = 0 then
		if Match(ls_provalue,'^\".*\"$') or Match(ls_provalue,"^\'.*\'$") then	//04/27/2007 By jervis Fixed a defect for APB
			ls_modify = ls_objname + "." + ls_proname +" = " + ls_provalue
		else
			ls_modify = ls_objname + "." + ls_proname +" = '" + ls_provalue + "'"
		end if
	else
		if Match(ls_provalue,'^\".*\"$') or Match(ls_provalue,"^\'.*\'$") then	//04/27/2007 By jervis Fixed a defect for APB
			ls_modify = ls_modify + ' ' + ls_objname + "." + ls_proname +" = " + ls_provalue
		else
			ls_modify = ls_modify + ' ' + ls_objname + "." + ls_proname +" = '" + ls_provalue + "'"
		end if
	end if
...
public subroutine of_set_button_dotdd_company (string as_column, string as_value);//====================================================================
// Function: of_set_button_dotdd_company
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
//
//====================================================================
Long 		ll_View_ID, ll_Screen_ID, ll_FRow
String 	ls_optionname, ls_option_value, ls_user_id


ll_View_ID = iuo_parent.il_data_view_id
ll_Screen_ID = iuo_parent.ii_screen_id 
ls_optionname = as_column + '@' + string(  ll_Screen_ID,'000000') + 'dotadd_company_column' 
ls_option_value= as_value
ls_user_id = 'contractlogix-super-master'

ll_FRow = iuo_parent.ids_button_dotadd_company.Find( "view_id = " + string(ll_View_ID) + " and option_name = '" + ls_optionname + "'", 1, iuo_parent.ids_button_dotadd_company.RowCount( ) )
If ll_FRow = 0 Then
	iuo_parent.ids_button_dotadd_company.InsertRow( 0 )
	iuo_parent.ids_button_dotadd_company.SetItem( iuo_parent.ids_button_dotadd_company.RowCount( ), 'view_id', ll_View_ID )
	iuo_parent.ids_button_dotadd_company.SetItem( iuo_parent.ids_button_dotadd_company.RowCount( ), 'option_name', ls_optionname )
	iuo_parent.ids_button_dotadd_company.SetItem( iuo_parent.ids_button_dotadd_company.RowCount( ), 'option_value', ls_option_value )
	iuo_parent.ids_button_dotadd_company.SetItem( iuo_parent.ids_button_dotadd_company.RowCount( ), 'user_id', ls_user_id )	
	iuo_parent.ids_button_dotadd_company.SetItem( iuo_parent.ids_button_dotadd_company.RowCount( ), 'screen_id', ll_Screen_ID )
Else
	iuo_parent.ids_button_dotadd_company.SetItem( ll_FRow, 'option_value', ls_option_value )
End If
end subroutine
...
IF iuo_parent.is_AddObjectType <>'' THEN
	...
	Lower( iuo_parent.is_AddObjectType) <> Lower('picture')  and Lower( iuo_parent.is_AddObjectType) <> Lower('line') and  lower( iuo_parent.is_AddObjectType ) <> lower( 'rectangle' )  Then		
		...
		If iuo_Parent.is_curband = 'DETAIL' Then
			...
			//Added By Ken.Guo 2015-05-18
			iuo_Parent.Dynamic Function of_generate_grid_columns(iuo_Parent.is_Fields)
...
IF Pos(iuo_parent.is_last_object[iuo_parent.ii_Obj_Select_Cnt], "obj" ) = 0 THEN
	...
	IF IsValid(w_contract_field_properties) THEN
		w_contract_field_properties.of_set_tip(ls_ObjName)										//Added By Harry 11-08-2016
		...
		//Added By Ken.Guo 2017-05-25. Get multi-select properties
		string ls_multi_select
		long ll_find
		If isvalid(w_contract_field_properties) Then
			w_contract_field_properties.is_field_name = ls_ObjName
			ls_coltype = lower(w_contract_field_properties.dw_properties.GetItemString( ll_row, "sys_fields_field_type" ))
			if (iuo_parent.ii_screen_id = 9 or iuo_parent.ii_screen_id = 10) and left(ls_ObjName,7) = 'custom_' and left(ls_coltype,4) = 'char' then
					if isvalid(iuo_parent.ids_select_item_attribute) then 
						//Get value from local cache first
						ll_find = iuo_parent.ids_select_item_attribute.find("field_name = '"+ls_ObjName+"'",1,iuo_parent.ids_select_item_attribute.rowcount())
						if isnull(ll_find) then ll_find = 0
						if ll_find > 0 then
							ls_multi_select = iuo_parent.ids_select_item_attribute.getitemstring(ll_find, 'field_value')
						Else
							//Get value from global cache.
							ls_multi_select = gnv_data.of_getitem( 'multi_select_fields', 'object_format', 'view_id = ' + String(iuo_parent.il_data_view_id ) + ' and screen_id = ' + String(iuo_parent.ii_screen_id) + ' and object_name = "' +ls_ObjName+ '"' + ' and object_type = "multi-select"' )
						End If
					Else
						//Get value from global cache.
						ls_multi_select = gnv_data.of_getitem( 'multi_select_fields', 'object_format', 'view_id = ' + String(iuo_parent.il_data_view_id ) + ' and screen_id = ' + String(iuo_parent.ii_screen_id) + ' and object_name = "' +ls_ObjName+ '"' + ' and object_type = "multi-select"' )
					...
					w_contract_field_properties.tab_properties.tabpage_edit.dw_3.setitem(1, "fields_multi_select", ls_multi_select)
					iuo_parent.is_multi_select_old = ls_multi_select			
			end if
		end if
...
event destructor;call super::destructor;this.dataobject = ''
end event
