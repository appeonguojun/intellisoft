...
boolean ib_refresh = false //Added By Jay Chen 07-24-2015

long il_pointerx,il_pointery //Added By Jay Chen 02-26-2016
...
il_pointerx = PointerX()
il_pointery = PointerY()

//Modified By Ken.Guo 2016-04-11
//open(w_steplist) 
If isvalid(w_steplist) Then
	Close(w_steplist)
Else
	Open(w_steplist,w_mdi)
End If
end event
...
uo_design.TriggerEvent("ue_field_compute")
return
...
IF NOT gb_se_version THEN
	...
	if li_Return = -1 then
		...
		if not isnull(ls_tmp) and trim(ls_tmp) = '1' then 
			uo_design.of_show_more_bands(true)
			uo_design.Dynamic Trigger Event ue_showmoreband()
		end if 
	end if
	//---------------------------- APPEON END ----------------------------
END IF
...
//Added By Jay Chen 07-20-2015 save field tips
long ll_cnt
string ls_tip
string ls_multi_select,ls_field,ls_value
long k

//Modified by harry 2016-11-08  remove to uo_design.of_save()
/*
if uo_design.is_field_tip_new <> uo_design.is_field_tip_old then
	ls_tip = uo_design.is_field_tip_new
	select count(*) into :ll_cnt from ctx_screen_field_tips where view_id = :uo_design.il_data_view and screen_id = :uo_design.ii_screen_id and field_name = :uo_design.is_CurSelectObj; 
	if isnull(ll_cnt) then ll_cnt = 0
	if ll_cnt = 0 then
		insert into ctx_screen_field_tips(view_id,screen_id,field_name,field_tip) values(:uo_design.il_data_view,:uo_design.ii_screen_id,:uo_design.is_CurSelectObj,:ls_tip);
	else
		update ctx_screen_field_tips set field_tip = :ls_tip where view_id = :uo_design.il_data_view and screen_id = :uo_design.ii_screen_id and field_name = :uo_design.is_CurSelectObj; 
	end if
...
//Added By Jay Chen 12-22-2015 save multi select item
//string ls_multi_select,ls_field,ls_value
//long k
//if uo_design.is_multi_select_new <> uo_design.is_multi_select_old then
if isvalid(uo_design.ids_select_item_attribute) then 
	for k = 1 to uo_Design.ids_select_item_attribute.rowcount()
		ls_field = uo_Design.ids_select_item_attribute.getitemstring(k, "field_name")
		ls_value = uo_Design.ids_select_item_attribute.getitemstring(k, "field_value")
		if uo_Design.ii_screen_id = 9 or uo_Design.ii_screen_id = 10 then
			ll_cnt = 0
			ls_multi_select = uo_design.is_multi_select_new
			select count(*) into :ll_cnt from screen_field_select_item_attribute where view_id = :uo_design.il_data_view and screen_id = :uo_design.ii_screen_id and object_name = :ls_field and object_type = 'multi-select'; 
			if isnull(ll_cnt) then ll_cnt = 0
			if ll_cnt = 0 then
				insert into screen_field_select_item_attribute(view_id,screen_id,object_type,object_name,object_format) values(:uo_design.il_data_view,:uo_design.ii_screen_id,'multi-select',:ls_field,:ls_value);
			else
				update screen_field_select_item_attribute set object_format = :ls_value where view_id = :uo_design.il_data_view and screen_id = :uo_design.ii_screen_id and object_name = :ls_field and object_type = 'multi-select'; 
			end if
			commit;
		end if
	next
	destroy uo_design.ids_select_item_attribute
end if
//end if
//end
...
if IsValid(w_documents_browse) or IsValid(w_request_webform_approve) then il_SaveResult = ll_Rtn //Modified By Jay Chen 06-09-2014
...
if uo_design.ib_screen_document then //Added By Jay Chen 04-03-2014
	if IsValid(w_documents_browse) then
		if il_SaveResult = 1 then
			...
			if ib_refresh then
				if isvalid(gw_contract) then
					if isvalid(gw_contract.ids_screen_prop) then 
						gw_contract.ids_screen_prop.Retrieve()
					end if
				end if
			end if
			//end
...
elseif uo_design.ib_screen_others then //Added By Jay Chen 06-09-2014
	if IsValid(w_request_webform_approve) then
		if il_SaveResult = 1 then
			uo_design.TriggerEvent("ue_Refresh_otherwindow")
		end if
	end if
else
	if IsValid(gw_Contract)  then 
...
If isvalid(w_steplist) Then Close(w_steplist) //Added By Ken.Guo 2016-04-11
...
int  li
long	ll_pos,ll_pos2,ll_find,ll_row
...
if isnull(uo_design.is_new_dw_syntax) or uo_design.is_new_dw_syntax = '' then
	ls_Syntax =  uo_design.uo_band.dw_syntax.describe("datawindow.syntax")
else
	ls_Syntax = uo_design.is_new_dw_syntax
end if
...
IF ls_Syntax <> uo_design.is_OrgSyntax or uo_design.dw_properties_dw.ModifiedCount() > 0 or uo_design.ib_field_tip THEN //Modify by jervis 03.25.2009
	...
	if li = 1 or li = 2 then
		for ll_row = 1 to uo_design.ids_undo_object_pic.rowcount()
			ls_fieldname = uo_design.ids_undo_object_pic.getitemstring(ll_row, "as_filedname")
			ls_filename = uo_design.ids_undo_object_pic.getitemstring(ll_row, "as_filename")
			ls_step = 'delete ' + ls_fieldname 
			ll_find = uo_design.ids_step_list.find("lower(field_name) = '"+lower(ls_fieldname)+"' and lower(step_name) = '"+lower(ls_step)+"'", 1, uo_design.ids_step_list.rowcount())
		...
		next
		if uo_design.ib_field_tip then uo_design.ib_field_tip = false //Added by Harry 2016-10-08
	end if
    //end
	...
	ELSEIF li = 3 THEN
		return 1
	Else
		//Begin - Added By Jay 2017-01-19
		if IsValid (uo_design.ids_button_dotadd_company) Then
			If uo_design.ids_button_dotadd_company.RowCount () > 0 Then
				uo_design.ids_button_dotadd_company.ReSet() 
			End If
		End If
		//End - Added By Jay 2017-01-19
	END IF
		
	
END IF

uo_design.of_deletepicfile( )//added by gavins 200131014
