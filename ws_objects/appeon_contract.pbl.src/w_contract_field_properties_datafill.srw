...
ls_lookup_field = Upper(Trim(  w_contract_field_properties.dw_properties.GetItemString(1,'sys_fields_lookup_field') ))
Choose Case ls_lookup_field
	Case 'Y' //code_lookup
		dw_datafill.SetItem(ll_Row,'data_from', 'code_lookup' )
	Case 'C' //company
		dw_datafill.SetItem(ll_Row,'data_from', 'app_facility' )
	Case 'E' //code_lookup   category field and ( il_ScreenId = 51 Or il_ScreenId = 52 )  //Added By Jay 2016-07-05
		If Upper ( is_FieldName )  =  Upper ( "category" ) And ( il_ScreenId = 51 Or il_ScreenId = 52 ) Then 
			dw_datafill.SetItem(ll_Row,'data_from', 'code_lookup' )
		End If
End Choose
...
Choose Case ls_lookup_field
	Case 'Y' //code lookup
		...
		for ll_Cycle = 1 to ll_ColCnt
			...
			loop
		next
	Case 'C' //company //Added By Ken.Guo 2015-11-26 company field support data fill function.
		n_ds lds_company, lds_company_custom
		lds_company = Create n_ds
		lds_company.dataobject = 'd_facility_detail'
		lds_company_custom = Create n_ds
		lds_company_custom.dataobject = 'd_contact_custom_configlist'		
		lds_company_custom.settransobject(sqlca)
		lds_company_custom.retrieve()
		lds_company_custom.setfilter('field_falg = "C"')
		lds_company_custom.filter()
		dw_datafill.GetChild('field_name_map',ldwc_Child)
		
		ll_ColCnt = Long(lds_company.describe( "datawindow.column.count"))
		For ll_Cycle = 1 to ll_ColCnt
			ls_ColName = lds_company.Describe("#" + String(ll_Cycle) + ".Name")
			ls_ColType = lds_company.Describe("#" + String(ll_Cycle) + ".Coltype")
			
			//only show visible fields and custom fields
			If Left(ls_ColName,7) <> 'custom_'  and ls_ColName <> 'facility_id' and lds_company.Describe("#" + String(ll_Cycle) + ".visible") = '0' Then
				continue
			End If
			...
			If Left(ls_ColName,7) = 'custom_' Then //for custom label of  custom fields
				ll_find = lds_company_custom.find("field_name = '" +ls_ColName+ "'", 1, lds_company_custom.rowcount())  
				If ll_find > 0 Then
					ldwc_Child.SetItem(ll_Row,'discolumn', lds_company_custom.getitemstring(ll_find, 'field_label') )
				End If
			Else  //for fixed label for basic fields
				If Len(lds_company.describe(ls_ColName + '_t.name') ) > 1 Then //if exists.
					ldwc_Child.SetItem(ll_Row,'discolumn',lds_company.describe(ls_ColName + '_t.text'))
				End If
			End If
	...
	Case 'E'
		...
		if Upper ( is_FieldName )  =  Upper ( "category" ) And ( il_ScreenId = 51 Or il_ScreenId = 52 ) Then 	
			...
			if  ( isNull ( is_LookupName ) or is_LookupName = '' ) Then 
				is_LookupName = 'Contract Category'
			End If
