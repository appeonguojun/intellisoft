...
event ue_synchronizechildren ( long handle,  integer state )
event ue_synchronizeparent ( long handle,  integer state )
event ue_statechanged ( long handle,  integer prevstate )
event ue_check_action ( integer ai_state )
end type
...
public function integer of_resetselectedpicture (boolean ab_status)
public function boolean of_checkedselectedpicture (long al_handle)
public subroutine of_getselectedindexes (long al_handle, ref string as_data[], ref string as_parentdata[], ref string as_label[])
public subroutine of_getselectedindex (long al_handle, ref string as_data[], ref string as_parentdata[], ref string as_label[])
end prototypes

event ue_synchronizechildren(long handle, integer state);//====================================================================
// Event: ue_synchronizechildren
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 04/14/2015
//--------------------------------------------------------------------
...
if this.getitem(handle, tvitem) = -1 then return 
...
do while(childitem<>-1)
	...
	this.Event ue_synchronizechildren(childitem, state) 
...
event ue_synchronizeparent(long handle, integer state);long parentitem
...
event ue_statechanged(long handle, integer prevstate);//====================================================================
// Event: ue_statechanged
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    long       handle
// 	value    integer    prevstate
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 04/14/2015
...
if getitem(handle, tvitem) = -1 then return 
...
//			if ntvitem.statepictureindex=1 then return //????,???!
...
event ue_check_action(integer ai_state);if isnull(ai_state ) or (ai_state <>1 and ai_state <> 2) then return 
if this.checkboxes = false then return

long		ll_RootHandle 
ll_RootHandle = this.of_GetRootHandle(il_CurrentHandle)
this.event ue_synchronizechildren(ll_RootHandle, ai_state)

long nextitem 
treeviewitem ntvitem
nextitem=this.finditem(nexttreeItem!, ll_RootHandle) 
do while(nextitem<>-1) 
	this.getitem(nextitem, ntvitem) 
//	if ntvitem.statepictureindex=1 then return 
	nextitem=this.finditem(NextTreeItem!, nextitem) 
	this.event ue_synchronizechildren(nextitem, ai_state)
loop

nextitem=this.finditem(previoustreeItem!, ll_RootHandle) 
do while(nextitem<>-1) 
	this.getitem(nextitem, ntvitem) 
//	if ntvitem.statepictureindex=1 then return 
	nextitem=this.finditem(previoustreeItem!,nextitem) 
...
if au_available.ids_data.RowCount() > 0 then 		//Added By Mark Lee 04/16/2015
	If au_available.ids_data.RowsCopy(1, au_available.ids_data.RowCount(), Primary!, ids_data, 1, Primary!) <> 1 Then
		MessageBox( 'Screen ('+as_tab_name+')', 'Failure to Rowscopy!' )
	End If
end if 
...
public function integer of_resetselectedpicture (boolean ab_status);//====================================================================
// Function: of_resetselectedpicture
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    boolean    ab_status
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
...
do while ll_FindHandle <> -1	
	of_SetSelectedPicture(ll_FindHandle, ab_status)
	ll_FindHandle = this.FindItem(NextTreeItem!, ll_FindHandle)
loop
...
public function boolean of_checkedselectedpicture (long al_handle);long ll_FindHandle
TreeViewItem ltvi_Item
boolean		lbn_checked = false
string 		ls_check = ''

// Set picture according to flag
this.GetItem(al_Handle, ltvi_Item)
if ltvi_Item.PictureIndex <> 1 then
	if ltvi_Item.PictureIndex = 5 and ltvi_Item.SelectedPictureIndex = 5 then 
		lbn_checked = true
	else
		lbn_checked = false
	end if 
...
ll_FindHandle = this.FindItem(ChildTreeItem!, al_Handle)
do while ll_FindHandle <> -1
	lbn_checked = of_checkedSelectedPicture(ll_FindHandle) 
	if ls_check = ''  then 
		ls_check = lower(string(lbn_checked))
	elseif  ls_check <> string(lbn_checked) then 
		return false 
//	else
//
	end if 
...
public subroutine of_getselectedindexes (long al_handle, ref string as_data[], ref string as_parentdata[], ref string as_label[]);//====================================================================
// Function: of_getselectedindexes
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// 	reference    string    as_parentdata[]
// 	reference    string    as_label[]
//--------------------------------------------------------------------
// Returns:  (none)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 04/16/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
do while ll_FindHandle <> -1	
	this.of_getselectedindex(ll_FindHandle, as_data, as_parentdata, as_label)
	ll_FindHandle = this.FindItem(NextTreeItem!, ll_FindHandle)
loop
...
//
//====================================================================
long 						ll_FindHandle, ll_count
TreeViewItem 			ltvi_Item, ltvi_parentItem
...
if this.GetItem(al_Handle, ltvi_Item) = -1 then return 
ll_FindHandle = al_Handle 

if ltvi_Item.PictureIndex = 5 and ltvi_Item.SelectedPictureIndex = 5 then
	ll_count ++
	as_data[ll_count] = string(ltvi_Item.data)
	as_label[ll_count] = string(ltvi_Item.label)
	ll_FindHandle = this.FindItem(ParentTreeItem!, ll_FindHandle)
	if ll_FindHandle <> -1 then 
			this.GetItem(ll_FindHandle, ltvi_parentItem)
			as_parentdata[ll_count] = string(ltvi_parentItem.data)
//			as_label[ll_count] = string(ltvi_parentItem.label)
	end if 
	return // the end 
end if 
...
If isvalid(ids_pic) Then Destroy ids_pic //Added By Ken.Guo 2015-05-27
end event

event itempopulate;long ll_Row
long ll_RowCount
long ll_Pos
long ll_Handle,ll_window_handle
string ls_tab_name
...
//		of_InsertItem(ll_Handle, 58, "Email Manager")
	...
	case "tabpage_windows" //Added By Jay Chen 06-09-2014
		// Request WebForm Window
		ll_window_handle = of_InsertFolder(Handle, "tabpage_windows", "Request WebForm Window")
		//Request WebForm Window - Browse
		ll_Handle = of_InsertFolder(ll_window_handle, "tabpage_windows", "Browse")
		of_InsertItem(ll_Handle, 61, "Search Criteria")
		of_InsertItem(ll_Handle, 62, "Request Form Browse")
		ll_Handle = of_InsertFolder(ll_window_handle, "tabpage_windows", "Details")
		of_InsertItem(ll_Handle, 63, "Request Webform Header")
		of_InsertItem(ll_Handle, 64, "Request Webform Detail")
		of_InsertItem(ll_Handle, 65, "Request Webform Log")
	...
	case else
		...
		ids_data.Filter()
		ll_RowCount = ids_data.RowCount()
		if ll_RowCount > 0 then 				//Added By Mark Lee 04/16/2015
