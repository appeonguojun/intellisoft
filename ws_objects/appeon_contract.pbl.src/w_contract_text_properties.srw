...
string ls_express //Added By Jay Chen 12-10-2013
...
If  tab_1.tabpage_general.dw_1.GetRow( ) > 0  Then
	...
	ls_express = tab_1.tabpage_general.dw_1.GetItemString(tab_1.tabpage_general.dw_1.GetRow( ), "xexpress")
	If Not(trim( ls_express)= '' or isnull( ls_express ) ) Then
		tab_1.tabpage_general.dw_1.Modify( "b_x.color = '16711935'" )	
	Else
		tab_1.tabpage_general.dw_1.Modify( "b_x.color = '0'" )	
	End If
	ls_express = tab_1.tabpage_general.dw_1.GetItemString(tab_1.tabpage_general.dw_1.GetRow( ), "yexpress")
	If Not(trim( ls_express)= '' or isnull( ls_express ) ) Then
		tab_1.tabpage_general.dw_1.Modify( "b_y.color = '16711935'" )	
	Else
		tab_1.tabpage_general.dw_1.Modify( "b_y.color = '0'" )	
	End If
	ls_express = tab_1.tabpage_general.dw_1.GetItemString(tab_1.tabpage_general.dw_1.GetRow( ), "widthexpress")
	If Not(trim( ls_express)= '' or isnull( ls_express ) ) Then
		tab_1.tabpage_general.dw_1.Modify( "b_width.color = '16711935'" )	
	Else
		tab_1.tabpage_general.dw_1.Modify( "b_width.color = '0'" )	
	End If
	ls_express = tab_1.tabpage_general.dw_1.GetItemString(tab_1.tabpage_general.dw_1.GetRow( ), "heightexpress")
	If Not(trim( ls_express)= '' or isnull( ls_express ) ) Then
		tab_1.tabpage_general.dw_1.Modify( "b_height.color = '16711935'" )	
	Else
		tab_1.tabpage_general.dw_1.Modify( "b_height.color = '0'" )	
	End If
...
String			ls_visible, ls_color, ls_default, ls_expression, ls_column,ls_field
str_pass		lstr_pass
...
event editchanged;call super::editchanged;If row> 0 Then
	...
	choose case dwo.name 
		...
		case  'text_x','text_y','text_width','text_height' //Added By Jay Chen 12-12-2013
			AcceptText( )
	end choose
End If
end event
...
if inv_data_design.is_open_style = "screen" and inv_data_design.ib_HasNewModify and inv_data_design.ii_Screen_id >= 3 and inv_data_design.ii_Screen_id <= 10 Then
	...
	ElseIf Not( inv_data_design.ii_Screen_id >=6 and inv_data_design.ii_Screen_id  <=7 ) Then
		if inv_data_design.ii_Screen_id = 10 then //Added By Jay Chen 07-03-2014
			ls_TObj = 'custom2_' + ls_ObjName
			inv_data_design.Post of_AutoUpdateSearchScreen(ls_ObjName, ls_TObj )	
			inv_data_design.Post of_AutoUpdateDetailScreen(ls_ObjName)
		else
...
elseif (inv_data_design.is_open_style = "screen" and inv_data_design.ib_HasNewModify and (inv_data_design.ii_Screen_id = 53 or inv_data_design.ii_Screen_id  =54)) Then
	inv_data_design.Post of_AutoUpdateScreen("Custom Multi",ls_ObjName)
end if 
...
if inv_data_design.is_open_style = "screen" and inv_data_design.ib_HasNewModify and ( inv_data_design.ii_Screen_id = 6 Or inv_data_design.ii_Screen_id = 7 or inv_data_design.ii_Screen_id = 1) then 
	...
	if ls_sync_screen_contact = '1' then
		if left(lower(ls_ObjName),20) = 'ctx_contacts_custom_' or left(lower(ls_ObjName),20) = 'app_facility_custom_' then
			//Added By Jay Chen 12-15-2015
...
//			if Messagebox("Confirm Sync","Do you want to sync the labels and style to the Contact Window?",Question!,YesNo!) = 1 then
				inv_data_design.of_auto_update_contact_custom(ls_ObjName)
				//Added By Jay Chen 11-30-2015 sync search screen and contract contact screen
				if left(lower(ls_ObjName),20) = 'app_facility_custom_' then
					if inv_data_design.ii_Screen_id = 1 then
						inv_data_design.of_auto_update_contactsyntax(6, ls_ObjName)
						inv_data_design.of_auto_update_contactsyntax(7, ls_ObjName)
					elseif inv_data_design.ii_Screen_id = 6 then
						inv_data_design.of_auto_update_contactsyntax(1, ls_ObjName)
						inv_data_design.of_auto_update_contactsyntax(7, ls_ObjName)
