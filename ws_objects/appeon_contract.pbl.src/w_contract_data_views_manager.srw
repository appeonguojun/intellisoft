...
type cb_unselected from commandbutton within w_contract_data_views_manager
end type
type cb_selected from commandbutton within w_contract_data_views_manager
end type
...
/*isnert field extend property*/ //Modified By Ken.Guo 2016-08-16 added field_use_compute
//Modified By Ken.Guo 2017-03-02 Task 722 added sort_expression,filter_expression
insert into dbo.ctx_screen_field_ex (data_view_id
      ,screen_id
      ,field_name
      ,field_add
      ,field_use_compute
      ,field_compute_expression
	,sort_expression
	,filter_expression)
...
//Added By Ken.Guo 2016-08-19
delete from ctx_screen_field_compute where view_id = :al_new_view_id and screen_id = :al_screen_id;
insert into dbo.ctx_screen_field_compute (view_id
      ,screen_id
      ,turnon
      ,field_compute_conditions
      ,field_compute_expression_true
      ,field_compute_fields_true
	  ,field_compute_expression_false
	  ,field_compute_fields_false
		)
select :al_new_view_id
      ,screen_id
      ,turnon
	  ...
	  ,field_compute_expression_false
	  ,field_compute_fields_false
from ctx_screen_field_compute where view_id = :al_old_view_id and screen_id = :al_screen_id;

//Added by Harry 2070508 for Task 776:Multi-Select Option gets disabled when copied to new views and screens
DELETE FROM screen_field_select_item_attribute WHERE  view_id = :al_new_view_id and screen_id = :al_screen_id;
INSERT INTO screen_field_select_item_attribute(view_id, 
			screen_id, 
			object_name, 
			object_type, 
			object_format)
...
WHERE view_id = :al_old_view_id and screen_id = :al_screen_id;
...
gnv_data.of_retrieve('multi_select_fields') //Added By Ken.Guo 2017-05-26
end event
...
//isnert field extend property --jervis 05.13.2011 //Modified By Ken.Guo 2016-08-16 added field_use_compute
//Modified By Ken.Guo 2017-03-02. Task 722 Added sort_expression,filter_expression
insert into dbo.ctx_screen_field_ex (data_view_id
      ,screen_id
      ,field_name
...
//Added By Ken.Guo 2016-08-19
//delete from ctx_screen_field_compute where view_id = :ll_new_view_id;
insert into dbo.ctx_screen_field_compute (view_id
      ,screen_id
      ,turnon
      ,field_compute_conditions
...
select :ll_new_view_id
	  ...
	  ,field_compute_expression_false
	  ,field_compute_fields_false
from ctx_screen_field_compute where view_id = :il_data_view_id ;
...
//Added by Harry 2070508 for Task 776:Multi-Select Option gets disabled when copied to new views and screens
INSERT INTO screen_field_select_item_attribute(view_id, screen_id, object_name, object_type, object_format)
SELECT :ll_new_view_id,
			screen_id,
			object_name, 
			object_type, 
...
delete from screen_field_select_item_attribute where view_id = :il_data_view_id ; //Added by Harry 20170508
commit using sqlca;
gnv_appeondb.of_commitqueue()
