	...
	if lb_file Then
		IF ll_flen > 32765 THEN
			IF Mod(ll_flen, 32765) = 0 THEN
				 //ll_flen = ll_flen/32765 //Commented By Jay 2016-04-21
				 ll_loops = ll_flen/32765 //Modified By Jay 2016-04-21
			ELSE
...
il_screen_id = ll_Screen_ID //Added By Jay Chen 06-04-2015
...
If ll_Screen_ID = 1 Or ll_Screen_ID = 2 Or ll_Screen_ID = 3 Or ll_Screen_ID = 5 Or ll_Screen_ID = 9 Or ll_Screen_ID = 8  Then
	...
	If ll_Screen_ID <=2 Then
		...
		ddlb_1.addItem("All" )
	else //Added By Jay Chen 06-04-2015
		cbx_filter.Text = cbx_filter.Text + '(including Contract Detail;Contract Dates;Notes;Custom Data screens.)'
	End If
	...
	if ls_FieldName <> "" then
		...
		If lb_Category Then
			If left( ls_FieldName, 14) = 'custom2_custom' Then
				//Modified By Jay Chen 06-03-2015
				ls_visible = lds_Temp[1].Describe( Mid(ls_FieldName, 9 ) + ".visible" )
				if ls_visible = '-1' or ls_visible = 'Y' then ls_visible = '1'
//				If  lds_Temp[1].Describe( Mid(ls_FieldName, 9 ) + ".visible" ) = '1' Then
				If  ls_visible = '1' or Pos( ls_visible, '~t' ) > 0 Then
			...
			ElseIf left( ls_FieldName, 5) = 'ccci_' Then
				//Modified By Jay Chen 06-03-2015
				ls_visible =  lds_Temp[2].Describe( Mid(ls_FieldName,6 ) + ".visible" ) 
				if ls_visible = '-1' or ls_visible = 'Y' then ls_visible = '1'
//				If lds_Temp[2].Describe( Mid(ls_FieldName,6 ) + ".visible" ) = '1'  Or  lower(Mid(ls_FieldName,6 ) ) = 'name'  Then
				If ls_visible = '1' or Pos( ls_visible, '~t' ) > 0 Or  lower(Mid(ls_FieldName,6 ) ) = 'name'  or  lower(Left(ls_FieldName,14 ) ) = 'ccci_contacts_'  Then  //Added by ken.guo 2017/02/15 added 'ccci_contacts_'
			...
			ElseIf left( ls_FieldName, 5) = 'occi_' Then
				ls_visible =  lds_Temp[3].Describe( Mid(ls_FieldName, 6 ) + ".visible" )
				if ls_visible = '-1' or ls_visible = 'Y' then ls_visible = '1'
//				If  lds_Temp[3].Describe( Mid(ls_FieldName, 6 ) + ".visible" ) = '1' Or  lower(Mid(ls_FieldName,6 ) ) = 'name'  Then
				If  ls_visible = '1' or Pos( ls_visible, '~t' ) > 0 Or  lower(Mid(ls_FieldName,6 ) ) = 'name'  or  lower(Left(ls_FieldName,14 ) ) = 'occi_contacts_'  Then //Added by ken.guo 2017/02/15 added 'occi_contacts_'
					...
					End If 	
				End If
			ElseIf Pos( ls_FieldName, 'app_facility_' ) > 0 Then						//Modified By Mark Lee 11/27/2015
			...
			ElseIf Left( Lower( ls_FieldName), 9 ) = 'feesched_' Then
				ls_visible =  lds_Temp[4].Describe( Mid(ls_FieldName, 10 ) + ".visible" )
				if ls_visible = '-1' or ls_visible = 'Y' then ls_visible = '1'
...
//					If ls_visible = '1' Then //Modified By Jay Chen 06-03-2015 add expression
					if ls_visible = '-1' or ls_visible = 'Y' then ls_visible = '1'
					If ls_visible = '1' or Pos( ls_visible, '~t' ) > 0 Then
...
For li_i = 1 To UpperBound( lds_Screen )//added by gavins 20140714
	If isvalid( lds_screen[li_i] ) then destroy ( lds_Screen[li_i] )
Next
For li_i = 1 To UpperBound( lds_Temp )
	If isvalid( lds_Temp[li_i] ) then destroy ( lds_Temp[li_i] )
Next

//Added By Jay Chen 06-04-2015
If ll_Screen_ID = 1 Or ll_Screen_ID = 2  Then
	If Pos(lower( ddlb_1.text), 'all' ) > 0 Then
		cbx_filter.enabled = true
	else
		cbx_filter.enabled = false
	end if
	ddlb_1.addItem("Others" )
end if
if ll_Screen_ID = 8 Or ll_Screen_ID = 9  Then
	cbx_filter.visible = false
	dw_fieldlist.Height = dw_fieldlist.Height + 80
else
...
string text = "Display all unused fields "
end type
...
If Len( ls_Text ) > 0 Then 
	If Pos( lower( ls_Text), 'all' ) > 0 Then
		...
		dw_fieldlist.Setfilter( " Not( isnull(screenname ) or trim(screenname ) = '' )" )
		dw_fieldlist.filter( )
		cbx_filter.enabled = true //Added By Jay Chen 06-04-2015
	Else
		if lower( ls_Text) = 'others' then //Added By Jay Chen 06-04-2015
			dw_fieldlist.Setfilter( " (isnull(screenname )) or (trim(screenname ) = '') " )
			dw_fieldlist.filter( )
		else
