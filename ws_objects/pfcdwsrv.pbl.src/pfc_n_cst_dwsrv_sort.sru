...
DO WHILE (1=1)
	ll_Pos  = Pos(ls_Sort,",",ll_Start)
	if ll_Pos < 1 then exit
	
	ls_Sort = left(ls_Sort,ll_Pos) + Trim(mid(ls_Sort,ll_Pos + 1))
	ll_Start = ll_Pos + 1
LOOP
...
FOR li_i = 1 to li_num_cols
	...
	If Pos(Lower(ls_parse), Lower("LookUpDisplay(")) > 0 Then
		...
		//added by nova 2008.03.24 begin
		//reason: fixed web error.
	elseif Pos(Lower(ls_parse), Lower("LookUpDisplay (")) > 0 Then
		ls_parse = lnv_string.of_GlobalReplace ( ls_parse, "LookUpDisplay (", "" ) 
		ls_parse = lnv_string.of_GlobalReplace ( ls_parse, " )", "" ) 
		ls_parse = Trim ( ls_parse )
		//added by nova 2008.03.24 end
	End If
