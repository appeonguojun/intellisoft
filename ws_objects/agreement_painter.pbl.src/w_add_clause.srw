...
string is_code_lookup_code //Added By Jay Chen 09-10-2014
boolean ib_remove_template_clause = false //Added By Jay Chen 09-10-2014

n_cst_other_audit  inv_audit
end variables
...
//		Select count(*) Into :ll_cnt1 From ctx_acp_template_clause Where ctx_acp_clause_id = :ll_clauseid;				//Added By Mark Lee 02/19/2016
		Select  count(*)  Into :ll_cnt1 From ctx_acp_template_clause , ctx_acp_template  
				Where ctx_acp_template_clause.ctx_acp_template_id =ctx_acp_template.ctx_acp_template_id 
				and (ctx_acp_template.deleted is null or ctx_acp_template.deleted <> 'Y' )
				and   ctx_acp_template_clause.ctx_acp_clause_id = :ll_clauseid;
...
long		ll_Cycle,ll_RowCnts,ll_TotalCnts,ll_Row,ll_Checked, ll_seq_no
long		ll_clause_id,ll_attribute_id,ll_Value_id
String	ls_Alias , ls_p_attri_name,ls_attri_name, ls_clause_name, ls_name
...
ls_clause_name =  ldw_Clause.GetItemString(ll_Row,"clause_name") //Added By Ken.Guo 2015-06-02
...
Select isnull(max(seq_no),0)  Into :ll_seq_no From sys_audit Where rec_id = :ll_clause_id and table_id = 10001;
...
for ll_Cycle = 1 to ll_RowCnts
	...
	if ldw_ItemStatus = DataModified! then
		...
		ls_p_attri_name =  ldw_AttriValue.GetItemString(ll_Cycle,"lookup_name") //Added By Ken.Guo 2015-06-02
		ls_attri_name =  ldw_AttriValue.GetItemString(ll_Cycle,"code") //Added By Ken.Guo 2015-06-02
		...
		if ll_TotalCnts <= 0 then
			if ll_Checked = 1 then
				...
				//Added By Ken.Guo 2015-06-02	  
				ll_seq_no++
				ls_name = ls_clause_name + ' (' + String(ll_clause_id) + ') - ' + ls_p_attri_name + ' - '+ ls_attri_name
				...
				values(:ll_clause_id, -9999,:ll_seq_no,10001,0,'False','True',:gs_user_id,getdate(),'E', 0, null,null,:ls_name,:ls_name);	  
			...
			if ll_Checked = 0 then
				...
				ls_name = ls_clause_name + ' (' + String(ll_clause_id) + ') - ' + ls_p_attri_name + ' - '+ ls_attri_name
				insert into sys_audit  (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)  
				values(:ll_clause_id, -9999,:ll_seq_no,10001,0,'True','False',:gs_user_id,getdate(),'E', 0, null,null,:ls_name,:ls_name);
...
If tab_Attribute.tabpage_General.dw_AddClause.update() = 1 Then //Added By Ken.Guo 05/04/2014 Added If statement
	tab_Attribute.tabpage_General.dw_AddClause.event pfc_postupdate() //Added By Ken.Guo 05/04/2014
End If
...
//	
	...
	inv_clause.of_registry_set( il_ctx_acp_clause_id,ldec_revision , ldt_updatedate)
...
//		ll_approval_status = tab_Attribute.tabpage_General.dw_AddClause.GetItemNumber(tab_Attribute.tabpage_General.dw_AddClause.getrow(), "approval_status")	//Modified By Jay Chen 02-15-2016
		//Modify by jervis - 02.09.2010
		//adjusting Access at Template or Access at Doc Manager property in Clause Approval Status lookup code
		//ll_clause_approved = long(gnv_data.of_getitem('code_lookup','lookup_code',"lookup_name = 'Clause Approval Status' and code = 'Approved'"))	
...
//	Select count(*) Into :ll_cnt1 From ctx_acp_template_clause Where ctx_acp_clause_id = :ll_clauseid;		//Added By Mark Lee 02/19/2016
	Select  count(*)  Into :ll_cnt1 From ctx_acp_template_clause , ctx_acp_template  
		...
		and   ctx_acp_template_clause.ctx_acp_clause_id = :ll_clauseid;
	Select count(*) Into :ll_cnt2 From ctx_am_document_clause Where ctx_acp_clause_id = :ll_clauseid;
	select code into :is_code_lookup_code from code_lookup where lookup_name = 'Clause Approval Status' and lookup_code = :ll_approval_status; //Added By Jay Chen 09-10-2014
	gnv_appeondb.of_commitqueue()
//	If ll_cnt1 > 0 or ll_cnt2 > 0 Then 
	If (ll_cnt1 > 0 or ll_cnt2 > 0) and (lower(is_code_lookup_code) <> 'obsolete') Then //Modified By Jay Chen 09-10-2014
...
//Added By Ken.Guo 2015-12-04
Long ll_export_id_org,ll_export_id
Choose Case dwo.name
	Case 'export_id' 
		If lower(is_parm) = 'update' or  lower(is_parm) = 'copy'  Then
			...
			If ll_export_id_org <> ll_export_id Then
				If Messagebox('Warning','It is not recommended to change the export value because changing the export could cause the document merge error if a field from the original export has already been inserted into the document. ~r~n~r~nAre you sure you want to change it?', question!, yesno!,2) = 2 Then
...
//		li_rtn = GetFileOpenName("Select File", ls_FilePath,ls_FileName,"DOC","Doc Files (*.DOC),*.DOC,Docx Files (*.DOCX),*.DOCX")
		li_rtn = ln_getfilename.of_getopenfilename("Select File", ls_FilePath,ls_FileName,"DOC","Doc Files (*.DOC),*.DOC,Docx Files (*.DOCX),*.DOCX")
...
//--------------------------------------------------------------------
// Description: Add audit
//--------------------------------------------------------------------
// Arguments:
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
// Author:         Ken.Guo           Date: 05/04/2014
//--------------------------------------------------------------------
...
long ll_ctx_acp_clause_id,ll_seq_no
String ls_clause_name

If is_parm = 'Add' or is_parm = 'Copy'  Then	
	ll_ctx_acp_clause_id = tab_Attribute.tabpage_General.dw_AddClause.GetItemNumber(tab_Attribute.tabpage_General.dw_AddClause.getrow(),'ctx_acp_clause_id')	
	ls_clause_name = tab_Attribute.tabpage_General.dw_AddClause.GetItemString(tab_Attribute.tabpage_General.dw_AddClause.getrow(),'clause_name') + ' (' + String(ll_ctx_acp_clause_id) + ')'	 
	Select isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_ctx_acp_clause_id and table_id = 10001;
	insert into sys_audit  (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)  
	values(:ll_ctx_acp_clause_id, -9999,:ll_seq_no,10001,0,'','INSERT',:gs_user_id,getdate(),'I', 0, null,null,:ls_clause_name,:ls_clause_name);
End If

Return 1
end event

event updateend;call super::updateend;//Added By Ken.Guo 2015-06-01
If lower(is_parm) = 'update' then
	inv_audit.of_update()
End If

end event

event updatestart;call super::updatestart;//Added By Ken.Guo 2015-06-01
inv_audit.of_reset()
inv_audit.of_audit( this , 10001, 'ctx_acp_clause_id', 'clause_name')
end event
...
If isnull(ls_clause_name) or ls_clause_name = '' Then
	messagebox('ContractLogix','Required value missing for Clause Name on row 1. Please enter a value.')
	tab_Attribute.tabpage_General.dw_AddClause.setfocus()
	tab_Attribute.tabpage_General.dw_AddClause.setcolumn('clause_name')
	Return
End If
...
If isnull(ll_folderid) or ll_folderid < 0 Then
	messagebox('ContractLogix','Required value missing for Folder Name on row 1. Please enter a value.')
	tab_Attribute.tabpage_General.dw_AddClause.setfocus()
...
If is_parm = 'Add' or is_parm = 'Copy'  Then	
	...
	if li_count = 1 then
		if messagebox('ContractLogix','The name of the Clause already exists, do you want to continue?' ,Question!,YesNo!) = 2 then
			tab_Attribute.tabpage_General.dw_AddClause.setfocus()
			tab_Attribute.tabpage_General.dw_AddClause.setcolumn('clause_name')
			Return
		end if
...
//Added By Jay Chen 09-10-2014 only remove template clause when convert status to obsolete.
long ll_status_ori,ll_status_new
ll_status_ori   = tab_Attribute.tabpage_General.dw_AddClause.GetItemNumber(tab_Attribute.tabpage_General.dw_AddClause.getrow(), "approval_status",primary!,true)
ll_status_new = tab_Attribute.tabpage_General.dw_AddClause.GetItemNumber(tab_Attribute.tabpage_General.dw_AddClause.getrow(), "approval_status",primary!,false)
if (ll_status_new <> ll_status_ori) and (lower(is_code_lookup_code) = 'obsolete') then
	ib_remove_template_clause = true 
end if
//end
...
If tab_Attribute.tabpage_General.dw_AddClause.modIfiedcount() > 0 Then
	...
	*/
	...
	If lower(is_parm) = 'update' and &
		...
		ldw_clause.GetItemNumber(ldw_clause.getrow(),'page_break',Primary!,False) <> ldw_clause.GetItemNumber(ldw_clause.getrow(),'page_break',Primary!,True) or &
		 ldw_clause.GetItemNumber(ldw_clause.getrow(),'comment_disp',Primary!,False) <> ldw_clause.GetItemNumber(ldw_clause.getrow(),'comment_disp',Primary!,True) ) Then //Modified By Jay Chen 11-06-2013
		 	lb_update_documents = True
	End If
	
	//Added By Jay Chen 11-06-2013
	If lower(is_parm) = 'update' then
		if  ldw_clause.GetItemNumber(ldw_clause.getrow(),'comment_disp') = 1 then
			if ldw_clause.GetItemString(ldw_clause.getrow(),'information_message',Primary!,False) <> ldw_clause.GetItemString(ldw_clause.getrow(),'information_message',Primary!,True)  Then
		 		lb_update_documents = True
			end if
		end if
	end if
