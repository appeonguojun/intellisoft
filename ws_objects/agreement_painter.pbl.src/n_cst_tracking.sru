...
If ll_rowcount > 0 Then
	ls_finder = " sys_tracking_log_modules ='"+as_modules+"' and sys_tracking_log_session_id <> '" +gs_session_id+ "' " //Modified By Ken.Guo 2016-06-07
	...
	DO
		...
		If ll_find > 0 then 
			...
			astr_sys_tracking_log_arr_s.astr_sys_tracking_log_arr[j].as_user			=ids_sys_tracking_log.GetItemString(ll_find,"session_info_user_id") + ' (IP:' + ids_sys_tracking_log.GetItemString(ll_find,"session_info_client_ip") + ')' //Modified By Jay Chen 01-15-2015 Add ip info.
		End If 
...
ids_sys_tracking_log = Create n_ds
...
Choose Case as_operation
	Case 'add_doc_tracking'
		//Added By Jay Chen 01-05-2015 avoid insert statement conflicted with the FOREIGN KEY error
		select count(*) into :ll_cnt from session_info where session_id = :gs_session_id;
		...
		If ll_rowcnt > 0 Then
			...
			If ll_find > 0 Then 
				...
				If ids_sys_tracking_log.update() = 1 Then
					...
					Return 
				Else
					//Modified By Ken.Guo 2016-04-05
					//Rollback;
					//MessageBox("SQL Error", sqlca.SQLErrText )
					gnv_debug.of_output(True, 'Tracking doc add. SessionID:' + gs_session_id + ', Module: ' + as_modules + ', Argument 1: ' + as_value1)
					Return 
				End IF
			Else
	...
	Case 'delete_doc_tracking'
		If abln_deleteallflag = False Then
			...
			If Sqlca.Sqlcode < 0 Then 
				...
				//MessageBox("SQL Error", sqlca.SQLErrText )
				//Rollback;
				gnv_debug.of_output(True, 'Tracking doc delete. SQL:' + ls_delete + '. Error: ' + sqlca.SQLErrText)
				Return 
			Else
				//Commit; //Commented By Ken.Guo 2016-04-05
				Return 
			End If 
		End If 
End Choose 
