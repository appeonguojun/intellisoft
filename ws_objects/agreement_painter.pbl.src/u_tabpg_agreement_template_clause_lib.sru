...
long il_num = 0,il_last_handle,il_handle[] //Added By Jay Chen 01-28-2014
long il_draghandle //Added By Jay Chen 03-14-2014
end variables
...
do while ll_find_clause > 0 
	...
	l_tvi_clause.selectedpictureindex = 3 //Modified By Jay Chen 01-28-2014
...
type tv_1 from u_tvs within u_tabpg_agreement_template_clause_lib
event ue_statechanged ( long handle,  integer prevstate )
integer x = 23
integer y = 292
integer width = 1527
integer height = 2188
...
string statepicturename[] = {"unselect.gif","select.gif"}
...
/***********Commented By Jay Chen 01-28-2014
...
il_draghandle = NewHandle //Added By Jay Chen 03-14-2014
...
if classname(source) = "tv_1" then
	if tv_1.GetItem(il_draghandle,l_tvi) = 1 then
		if l_tvi.PictureIndex = 1 then
			if l_tvi.data <> -1 then
				ll_find = iw_atac.dw_pend_clause.find("folder_id ="+string(l_tvi.data),1,iw_atac.dw_pend_clause.rowcount())
				if isnull(ll_find) then ll_find = 0
				if ll_find > 0 then
					iw_atac.dw_pend_clause.deleterow(ll_find)
				end if
			end if
		end if
	end if
...
Treeviewitem l_tvi,l_tvi2,l_tvi3
Long ll_clause_id,ll_null,li_row,ll_row,ll_find,i,ll_null2[],ll_child,ll_child2
string ls_null,ls_clause_name
datetime ldt_modify_date
...
//Added By Jay Chen 02-18-2014 Remark:unselect folder and clause property record
if tv_1.Getitem(handle,l_tvi) = 1 then
	if l_tvi.PictureIndex = 1 or l_tvi.PictureIndex = 6 then return
end if

//Added By Jay Chen 02-12-2014
//if tv_1.Getitem(handle,l_tvi) = 1 then
//	if l_tvi.data = -1 and l_tvi.statepictureindex = 1 then
//		if messagebox(gnv_app.iapp_object.DisplayName,'It will be very slow to select all clauses to the template,do you want to continue to assign?',question!,yesno!) = 2 then 
//			return
//		end if
//	end if
//end if

//Added By Jay Chen 01-28-2014 Remark:set value for each item's checkbox 
IF tv_1.Getitem(handle,l_tvi) = 1 THEN
	il_num++
	if KeyDown(KeyControl!) or il_num = 1 then
		if l_tvi.StatePictureIndex = 1 then
			l_tvi.StatePictureIndex = 2
			il_handle[upperbound(il_handle) + 1] = handle
		else
			l_tvi.StatePictureIndex = 1
		end if
	else
		if il_last_handle <> handle then
			for i = 1 to upperbound(il_handle)
				if tv_1.Getitem(il_handle[i],l_tvi2) = 1 then
					if l_tvi2.PictureIndex <> 1 then
						ll_find = iw_atac.dw_pend_clause.find("clause_id ="+string(l_tvi2.data),1,iw_atac.dw_pend_clause.rowcount())
					else
						ll_find = iw_atac.dw_pend_clause.find("folder_id ="+string(l_tvi2.data),1,iw_atac.dw_pend_clause.rowcount())
					end if
					if ll_find > 0 then
						iw_atac.dw_pend_clause.deleterow(ll_find)
					end if
					l_tvi2.StatePictureIndex = 1
					tv_1.setitem(il_handle[i],l_tvi2)
					//cancel already select folder's clause
					if l_tvi2.PictureIndex = 1 then
						ll_child = tv_1.finditem(ChildTreeItem!,il_handle[i])
						do while ll_child <> -1
							if tv_1.Getitem(ll_child,l_tvi2) = 1 then
								ll_find = iw_atac.dw_pend_clause.find("folder_id ="+string(l_tvi2.data),1,iw_atac.dw_pend_clause.rowcount())
								if ll_find > 0 then iw_atac.dw_pend_clause.deleterow(ll_find)
								if l_tvi2.pictureindex = 1 and l_tvi2.level = 2 then
									ll_child2 = tv_1.finditem(ChildTreeItem!,ll_child)
									do while ll_child2 <> -1
										if tv_1.Getitem(ll_child2,l_tvi3) = 1 then
											l_tvi3.StatePictureIndex = 1
											tv_1.setitem(ll_child2,l_tvi3)
										end if
										ll_child2 = tv_1.finditem(NextTreeItem!,ll_child2)
									loop
								end if
								l_tvi2.StatePictureIndex = 1
								tv_1.setitem(ll_child,l_tvi2)
							end if
	...
	tv_1.setitem(handle,l_tvi)
	//Added By Jay Chen 01-28-2014 Remark:insert pending clause data to parent window
	IF l_tvi.PictureIndex <> 1 THEN //clause
		...
		if l_tvi.StatePictureIndex = 2 then  
			li_row = iw_atac.dw_pend_clause.insertrow(0)
			ll_row = ds_clause.find("ctx_acp_clause_id ="+string(l_tvi.data),1,ds_clause.rowcount())
			if ll_row > 0 then 
				ldt_modify_date = ds_clause.getitemdatetime(ll_row,"modify_date")
				ls_clause_name = ds_clause.getitemstring(ll_row,"clause_name")		
			end if
			iw_atac.dw_pend_clause.setitem(li_row,"folder_id",ll_null)
			iw_atac.dw_pend_clause.setitem(li_row,"clause_id",l_tvi.data)
			iw_atac.dw_pend_clause.setitem(li_row,"clause_name",ls_clause_name)
			iw_atac.dw_pend_clause.setitem(li_row,"modify_date",ldt_modify_date)
		elseif l_tvi.StatePictureIndex = 1 then
			ll_find = iw_atac.dw_pend_clause.find("clause_id ="+string(l_tvi.data),1,iw_atac.dw_pend_clause.rowcount())
			if ll_find > 0 then
				iw_atac.dw_pend_clause.deleterow(ll_find)
			end if
		end if
	ELSE //folder
		if l_tvi.data <> -1 then  //second level folder
			if l_tvi.StatePictureIndex = 2 then  
				li_row = iw_atac.dw_pend_clause.insertrow(0)
				iw_atac.dw_pend_clause.setitem(li_row,"folder_id",l_tvi.data)
				iw_atac.dw_pend_clause.setitem(li_row,"clause_id",ll_null)
				iw_atac.dw_pend_clause.setitem(li_row,"clause_name",ls_null)
			elseif l_tvi.StatePictureIndex = 1 then
				ll_find = iw_atac.dw_pend_clause.find("folder_id ="+string(l_tvi.data),1,iw_atac.dw_pend_clause.rowcount())
				if ll_find > 0 then
					iw_atac.dw_pend_clause.deleterow(ll_find)
			...
			ll_child = tv_1.finditem(ChildTreeItem!,handle)
			do while ll_child <> -1
				if tv_1.Getitem(ll_child,l_tvi2) = 1 then
					if l_tvi.StatePictureIndex = 1 then
						l_tvi2.StatePictureIndex = 1
					else
						l_tvi2.StatePictureIndex = 2
					end if
					tv_1.setitem(ll_child,l_tvi2)
				end if
				ll_child = tv_1.finditem(NextTreeItem!,ll_child)
			loop
		else //one level folder
			ll_child = tv_1.finditem(ChildTreeItem!,handle)
			do while ll_child <> -1
				if tv_1.Getitem(ll_child,l_tvi2) = 1 then
					if l_tvi.StatePictureIndex = 1 then
					...
					tv_1.setitem(ll_child,l_tvi2)
					//find second level folder clause
					ll_child2 = tv_1.finditem(ChildTreeItem!,ll_child) 
					do while ll_child2 <> -1
						if tv_1.Getitem(ll_child2,l_tvi3) = 1 then
							if l_tvi.StatePictureIndex = 1 then
								...
								l_tvi3.StatePictureIndex = 2
							end if
							tv_1.setitem(ll_child2,l_tvi3)
							ll_child2 = tv_1.finditem(NextTreeItem!,ll_child2)
						end if
					loop
					//insert pending folder data to parent window
					if l_tvi.StatePictureIndex = 2 then  
						li_row = iw_atac.dw_pend_clause.insertrow(0)
						iw_atac.dw_pend_clause.setitem(li_row,"folder_id",l_tvi2.data)
						iw_atac.dw_pend_clause.setitem(li_row,"clause_id",ll_null)
						iw_atac.dw_pend_clause.setitem(li_row,"clause_name",ls_null)
					elseif l_tvi.StatePictureIndex = 1 then
						ll_find = iw_atac.dw_pend_clause.find("folder_id ="+string(l_tvi2.data),1,iw_atac.dw_pend_clause.rowcount())
...
event dragleave;call super::dragleave;//Added By Jay Chen 03-14-2014
long ll_clause_id,li_row,ll_null,ll_row,ll_child,ll_find
datetime ldt_modify_date
string ls_clause_name,ls_null
treeviewitem l_tvi,l_tvi2
setnull(ll_null)
setnull(ls_null)
...
if tv_1.GetItem(il_draghandle,l_tvi) = 1 then
	if l_tvi.PictureIndex <> 1 then
		ll_clause_id = l_tvi.data 
		inv_clause_comparison.of_select_clause(il_draghandle, ll_clause_id)
		iw_atac.dw_pend_clause.reset( )
		li_row = iw_atac.dw_pend_clause.insertrow(0)
		ll_row = ds_clause.find("ctx_acp_clause_id ="+string(l_tvi.data),1,ds_clause.rowcount())
		if ll_row > 0 then 
			ldt_modify_date = ds_clause.getitemdatetime(ll_row,"modify_date")
		...
		if l_tvi.StatePictureIndex = 1 then
			l_tvi.StatePictureIndex = 2
//			tv_1.setitem(il_draghandle,l_tvi)
		end if
	else //Added By Jay Chen 04-16-2014
		if l_tvi.data <> -1 then  //second level folder
//			if l_tvi.StatePictureIndex = 1 then
//				l_tvi.StatePictureIndex = 2
//				tv_1.setitem(il_draghandle,l_tvi)
...
//			if l_tvi.StatePictureIndex = 2 then  
				...
				iw_atac.dw_pend_clause.setitem(li_row,"clause_name",ls_null)
//			end if
//			ll_child = tv_1.finditem(ChildTreeItem!,il_draghandle)
//			do while ll_child <> -1
//				if tv_1.Getitem(ll_child,l_tvi2) = 1 then
//					if l_tvi.StatePictureIndex = 1 then
//						l_tvi2.StatePictureIndex = 1
...
//Added By Mark Lee 02/14/2015 add isvalid() adujst 
if isvalid(lstr_ClauseFilter) and lstr_ClauseFilter.Clause_Exp <> "Cancel" then
