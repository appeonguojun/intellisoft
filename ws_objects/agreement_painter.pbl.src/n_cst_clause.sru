...
public function integer of_registry_set_destroy (long al_clause_id, decimal adec_revision, string as_updatedate)
end prototypes
...
//Modified By Ken.Guo 2015-04-29. Use last modified datetime
...
Datetime ldt_lastupdate
 ls_filename = of_generate_name(al_clause_id,adec_revision)
 If FileExists(ls_filename) Then
	 gnv_filetime.of_getlastwritedatetime( ls_filename, ldt_lastupdate)
	 as_lastupdate = String(ldt_lastupdate, 'mm/dd/yyyy hh:mm:ss')
Else
	Return -1
End If
...
Datetime ldt_lastupdate
 ls_filename = of_generate_name(al_clause_id,adec_revision)
 If FileExists(ls_filename) Then
	 gnv_filetime.of_setlastwritedatetime( ls_filename, adt_updatedate)
Else
	Return -1
End If
end function
...
as_file_name = of_generate_name(al_clause_id,adec_revision)
	...
	If lb_select = True Then
		...
		FileClose(li_FileNum)		
		//gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Agreement", 'clause_' + string(al_clause_id) + '.' + String(adec_revision), regstring!, ls_updatedate)			
		of_registry_set(al_clause_id,adec_revision,adt_lastmodify) //Modified By Ken.Guo 2015-04-29
	End If	
Return 1
end function
...
string 		ls_prompt, ls_template_prompt																									//Added By Mark Lee 07/23/2015
...
ls_prompt = gnv_data.of_getitem( 'icred_settings', 'clause_update_prompt', False)											//Added By Mark Lee 07/23/2015
ls_template_prompt = gnv_data.of_getitem( 'icred_settings', 'clause_update_template', False)								//Added By Mark Lee 08/27/2015
if isnull(ls_prompt) then ls_prompt = '1'
if isnull(ls_template_prompt) then ls_template_prompt = '1'

//Added By Jay Chen 09-10-2014 only remove template clause when set clause status to Obsolete
if isvalid(w_add_clause) then
	if w_add_clause.ib_remove_template_clause then
		If lds_clause_template_ref.RowCount() > 0 Then
			ll_mes = messagebox('Updating Existing Templates','Do you want to remove this clause from the templates which reference this clause?',question!,YesNo!)
			If ll_mes = 1 Then
				astr_clause.as_update_action = 'remove_clause'
				openwithparm(w_clause_template,astr_clause)
			End If
		End If
		w_add_clause.ib_remove_template_clause = false
		return 1
	end if
end if
//end
...
//Update Template
If lds_clause_template_ref.RowCount() > 0 Then
	if ls_template_prompt = '1' then 																											//Added By Mark Lee 08/27/2015
		...
		If ll_mes = 1 Then
			openwithparm(w_clause_template,astr_clause)
		End If
//	else																																	//Added By Mark Lee 11/18/2015
//		openwithparm(w_clause_template,astr_clause)
	end if
End If
...
public function integer of_registry_set_destroy (long al_clause_id, decimal adec_revision, string as_updatedate);//Modified By Mark Lee 04/18/12
//Return gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Agreement", 'clause_' + string(al_clause_id) + '.' + String(adec_revision) , regstring!, as_updatedate)	
Return gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\Agreement", 'clause_' + string(al_clause_id) + '.' + String(adec_revision) , regstring!, as_updatedate)	
