...
string is_full_text_search
Boolean 	ib_mouse_down	= false													//Added By Mark Lee 08/27/2015
long		il_move_x = 0				
string 	is_move_position	

boolean ib_send_notification = false //Added By Jay Chen 09-01-2015
string is_locktext //Added By Jay Chen 10-24-2015
end variables
...
public function integer of_remove_template_clause (long al_template_id[], datetime adt_lastupdate[], long al_clause_id[])
public function integer of_build_template_tree (long al_clause_id)
public subroutine of_restore ()
public function integer of_set_move_pointer (long al_xpos, long al_ypos)
public function integer of_set_move (long al_xpos)
public subroutine of_getemailmessage (long al_email_id, ref blob ablb_message)
public function integer of_send_clause_notification (long al_clause_id, string as_action)
end prototypes
...
//ls_new_sql = is_old_sql + " where 1 = 1 "																//Added By Mark Lee 07/16/2015
//ls_new_sql = is_old_sql + " where (deleted is null or deleted <> 'Y') "							// change it 
ls_new_sql = is_old_sql 
...
//		ids_search_clause_text.retrieve(ls_clause_name)		
		...
		//End - Added By Mark Lee 07/13/12
		is_full_text_search = ls_fulltext //Added By Jay Chen 08-27-2015
	End If
...
If is_types = 'Add' Then	
	...
	If tabpage_clause.ole_1.object.doctype > 0 Then tabpage_clause.ole_1.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
...
long i,ll_find,ll_Handle, ll_count
...
//ids_clause_rely.setfilter('')												//Commented By Mark Lee 07/15/2015
//ids_clause_rely.filter()													//Commented By Mark Lee 07/15/2015	
...
If is_type = 'F' Then
	...
	If ll_find > 0 Then
		//		ids_folder.deleterow(ll_find)													//Added By Mark Lee 07/16/2015
		ll_count = 0 
		Select count(1) 
		Into :ll_count
		From ctx_acp_clause 
		Where ctx_acp_folder_id = :il_ctx_acp_folder_id ; 
		
		if ll_count = 0 then 
			ids_folder.deleterow(ll_find)	
		else
			ids_folder.setitem( ll_find, "deleted", "Y")
			ids_folder.setitem( ll_find, "deleted_by", gs_user_id)
			ids_folder.setitem( ll_find, "deleted_date", DateTime(today(), now()) )		
		end if 
	End If	
ElseIf is_type = 'C' Then
	...
	Else
		...
		If ll_find > 0 Then
			...
			Return 0
		End If
		//Begin - Modified By Mark Lee 07/15/2015			don't delete records.
...
//			ids_clause.deleterow(ll_find)
			ids_clause.setitem( ll_find, "deleted", "Y")
			ids_clause.setitem( ll_find, "deleted_by", gs_user_id)
			ids_clause.setitem( ll_find, "deleted_date", DateTime(today(), now()) )			
		End If
		//End - Modified By Mark Lee 07/15/2015
		
		gnv_appeondb.of_startqueue( )
		Select clause_name Into :ls_clause_name From  ctx_acp_clause Where ctx_acp_clause_id = :il_ctx_acp_clause_id;
		Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :il_ctx_acp_clause_id and table_id = 10001;
		gnv_appeondb.of_commitqueue( )
		
		
		ls_name = ls_clause_name + ' (' + String(il_ctx_acp_clause_id) + ')' //Added By Ken.Guo 2015-06-01
...
//		DELETE FROM ctx_acp_clause_image WHERE clause_id = :il_ctx_acp_clause_id; 								//move it 
		//End - Modified By Mark Lee 07/15/2015

		//Modified By Jay Chen 01-06-2014 Remark: insert a log to ctx_acp_clause_audit when delete clause
//		DELETE FROM ctx_acp_clause_audit WHERE clause_id = :il_ctx_acp_clause_id; 
		Insert into ctx_acp_clause_audit(clause_id,revision,actions,action_by,action_date,notes)
		Values(:il_ctx_acp_clause_id,:idec_revision,'Deleted',:gs_user_id,getdate(),:ls_clause_name);
		...
		values(:il_ctx_acp_clause_id, -9999,:ll_seq_no,10001,0,'','DELETE',:gs_user_id,getdate(),'D', 0, null,null,:ls_name,:ls_name);
...
gnv_appeondb.of_startqueue( )
//ids_clause_rely.Update()							 //Commented By Mark Lee 07/15/2015
ids_clause.Update()
ids_folder.Update()
...
If il_handle <> 0 Or il_handle1 <> 0 Then
	...
	Else
		If isvalid(m_pfe_cst_clause_painter) Then
			...
			gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_restorerow,'enabled', False)						//Added By Mark Lee 07/16/2015			
...
//Added By Ken.Guo 05/04/2014
String ls_clause_name
Long ll_seq_no,ll_count_clause_audit,ll_count_sys_audit, ll_count
...
ElseIf ls_type = 'C' Then			
	...
	//Added By Ken.Guo 05/04/2014
	gnv_appeondb.of_startqueue( )
	Select clause_name Into :ls_clause_name From  ctx_acp_clause Where ctx_acp_clause_id = :ll_id;
	Select isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_id and table_id = 10001;
	Select Count(1) Into :ll_count_clause_audit From ctx_acp_clause_audit Where clause_id = :ll_id and actions = 'Deleted' and action_by = :gs_user_id;
	Select Count(1) Into :ll_count_sys_audit From sys_audit Where rec_id = :ll_id and table_id = 10001 and audit_type = 'D' and user_id = :gs_user_id;	
	gnv_appeondb.of_commitqueue( )

	gnv_appeondb.of_startqueue( )
	//Added By Mark Lee 07/15/2015			don't delete records.
...
//		DELETE FROM ctx_acp_clause_image WHERE clause_id = :ll_id; 							
		...
		//Added By Ken.Guo 05/04/2014
		String ls_name
		ls_name = ls_clause_name + ' (' + String(ll_id) + ')'
		If ll_count_clause_audit = 0 Then
			Insert into ctx_acp_clause_audit(clause_id,revision,actions,action_by,action_date,notes)
			Values(:ll_id,:idec_revision,'Deleted',:gs_user_id,getdate(),:ls_clause_name);
		End If
		If ll_count_sys_audit = 0 Then
			insert into sys_audit (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)   
			values(:ll_id, -9999,:ll_seq_no,10001,0,'','DELETE',:gs_user_id,getdate(),'D', 0, null,null,:ls_name,:ls_name);
		End If
	gnv_appeondb.of_commitqueue( )
...
//Added By Jay Chen 08-26-2015 set full text search results color highlighted
string ls_findword
ls_findword = trim(tabpage_browse.dw_filter.getitemstring(1,'textsearch'))
if isnull(ls_findword) then ls_findword = ''
if len(ls_findword) > 0 then
	oleobject lole_word
	lole_word = Create oleobject
	lole_word = tabpage_clause.ole_1.object.Activedocument
	lole_word.ActiveWindow.Selection.Find.ClearHitHighlight
	lole_word.ActiveWindow.Selection.Find.HitHighlight(ls_findword,65535)
	Destroy lole_word
end if
...
//	m_pfe_cst_clause_painter.m_file.m_autoretrieve.visible = True
	...
	If w_mdi.of_security_access(6961) = 0 Then //Delete
		...
		m_pfe_cst_clause_painter.m_edit.m_restorerow.visible = False																	//Added By Mark Lee 07/15/2015
		gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_restorerow,'toolbaritemvisible', False)
	Else
		...
		gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_restorerow,'enabled', True)									//Added By Mark Lee 07/15/2015
		m_pfe_cst_clause_painter.m_edit.m_restorerow.visible = True
		gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_restorerow,'toolbaritemvisible', True)
	End If	
	...
	//Added By Jay Chen 09-11-2014
	m_pfe_cst_clause_painter.m_file.m_map.visible = False
	gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_file.m_map,'toolbaritemvisible', False)
	//end
...
ElseIf ai_tabpage = 2 Then
	...
	gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_delete1,'toolbaritemvisible', False)	
	m_pfe_cst_clause_painter.m_edit.m_restorerow.visible = False																					//Added By Mark Lee 07/15/2015
	gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_restorerow,'toolbaritemvisible', False)							//Added By Mark Lee 07/15/2015		
	...
	//Added By Jay Chen 09-11-2014
	m_pfe_cst_clause_painter.m_file.m_map.visible = true
	gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_file.m_map,'toolbaritemvisible', true)
	//end
...
IF ll_row > 0 Then
	...
	If tabpage_browse.dw_browse.GetItemNumber(ll_row,'level') = 2 or tabpage_browse.dw_browse.GetItemString(ll_row,'checkout_status') <> '0' Then
		...
		gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_delete1,'enabled', False)
		gnv_app.of_modify_menu_attr( m_pfe_cst_clause_painter.m_edit.m_restorerow,'enabled', False)							//Added By Mark Lee 07/15/2015		
...
If tabpage_browse.dw_browse.rowcount() > 0 Then
	ll_new_clause_id = tabpage_browse.dw_browse.getitemnumber(tabpage_browse.dw_browse.getrow(),'ctx_acp_clause_id')
//	of_build_template_tree(ll_new_clause_id)   //Added By Jay Chen 09-11-2014
...
IF tabpage_browse.dw_browse.object.checkout_status[tabpage_browse.dw_browse.getrow()] <> "0" THEN //Modified by Ken.Guo on 2009-03-27
	...
	IF Message.Doubleparm = 1 THEN
		...
		//Added By Jay Chen 09-01-2015 send email notification when a clause is checked out.
		string ls_enable
		long ll_rtn
		select email_enable into :ls_enable from ctx_clause_email where email_id = 1;
		if isnull(ls_enable) or ls_enable = '' then ls_enable = 'N'
		if ls_enable = 'Y' then 
			gnv_status_info.of_display("Start check in clause notification, please wait...")
			ll_rtn = of_send_clause_notification(il_ctx_acp_clause_id,'checkin')
			gnv_status_info.of_display("")
			if ll_rtn = -1 then
				gnv_debug.of_output(true,"Send clause notification failed when check in clause id:" + string(lstr_add_doc.doc_id))
			end if
		end if
		//end
...
ldw_browse.SetItem(ll_row,'modify_date',ids_clause_info.GetItemDatetime(1,'modify_date'))				//Added By Mark Lee 07/14/2015
...
IF SQLCA.SQLCODE = 0 THEN
	...
	if isnull(ls_enable) or ls_enable = '' then ls_enable = 'N'
	if ls_enable = 'Y' then 
		gnv_status_info.of_display("Start check out clause notification, please wait...")
		ll_rtn = of_send_clause_notification(lstr_add_doc.doc_id,'checkout')
		gnv_status_info.of_display("")
		if ll_rtn = -1 then
			gnv_debug.of_output(true,"Send clause notification failed when check out clause id:" + string(lstr_add_doc.doc_id))
		end if
	end if
	//end
...
tabpage_browse.dw_browse.SetItem(tabpage_browse.dw_browse.GetRow(),'modify_date',datetime(today(),now()))				//Added By Mark Lee 07/14/2015
...
IF lb_revision_control THEN
	...
	tabpage_browse.dw_browse.SetItem(ll_row,'modify_date',datetime(today(),now()))						//Added By Mark Lee 07/14/2015
...
//Added By Ken.Guo 2015-06-01
Long ll_seq_no1
String ls_clause_name1
If  lstr_add_audit.action = 'Modified' Then
	ls_clause_name1 = tabpage_browse.dw_browse.GetItemString(ll_row, "clause_name") + ' (' + String(il_ctx_acp_clause_id) +')'
	Select isnull(max(seq_no),0) + 1 Into :ll_seq_no1 From sys_audit Where rec_id = :ll_ctx_acp_clause_id and table_id = 10001;
	insert into sys_audit  (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)  
	values(:il_ctx_acp_clause_id, -9999,:ll_seq_no1,10001,0,'','Modified Content',:gs_user_id,getdate(),'E', 0, null,null,:ls_clause_name1,:ls_clause_name1);	
End If
...
if as_Filter <> "" then
	...
	do while ll_Found > 0
		...
		Tabpage_Browse.dw_Browse.SetItem(ll_Found,"modify_date",datetime(today(),now()))				//Added By Mark Lee 07/14/2015
...
public function integer of_remove_template_clause (long al_template_id[], datetime adt_lastupdate[], long al_clause_id[]);//====================================================================
// Function: of_remove_template_clause
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 09-10-2014
//--------------------------------------------------------------------
...
Long ll_cnt, i, j, ll_start_ori, ll_len_ori, ll_pagebreak, ll_end_new,ll_AppendParagraph,ll_ret,ll_end_ori
String ls_bookmark, ls_clausename, ls_template_name[]
String ls_lastupdate
Boolean lb_select = FALSE
n_cst_word_utility lnv_word
...
//remove clause in each template file
TRY 
	lole_word.Application.NormalTemplate.Saved = TRUE 
	IF IsValid(w_appeon_gifofwait) THEN	w_appeon_gifofwait.st_1.Text = "Removing template clause files..."		
	FOR j = 1 TO ll_cnt 
		IF IsValid(w_appeon_gifofwait) THEN w_appeon_gifofwait.hpb_1.StepIt()		
		lole_template = lole_word.Application.Documents.Open(ls_template_name[j])
		gnv_word_utility.of_modify_word_property( lole_word.Activedocument)	
		gnv_shell.of_delete_recent( ls_template_name[j], true) 
		IF lole_word.Application.Documents.Count < 1 THEN 			
			MessageBox("Warning","Open the template file of " + ls_template_name[j] +" failed!", exclamation!)			
			EXIT
		END IF
		For k = 1 To UpperBound(al_clause_id[])
			ls_bookmark = "clause_" + string(al_clause_id[k])
			If Not lole_template.content.bookmarks.Exists(ls_bookmark) Then Continue
			IF IsValid(w_appeon_gifofwait) THEN	w_appeon_gifofwait.st_1.Text = "Removing template files with clause #"+String(al_clause_id[k])+"..."
				lole_template.content.bookmarks.item(ls_bookmark).select()
				lole_template.activewindow.selection.delete()
		Next
		...
		If ll_ret < 0 Then
			...
			Return ll_ret
		Else
			delete from ctx_acp_template_clause WHERE ctx_acp_template_id = :al_template_id[j] and ctx_acp_clause_id = :al_clause_id[1];
		END IF		
	NEXT
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 09-11-2014
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
if al_clause_id = il_old_clause_id then return 1
...
//clear the treeview
ll_handle = tabpage_clause.tv_clause.finditem(roottreeitem!, 0)
do while ll_handle > 0 
   	tabpage_clause.tv_clause.deleteitem(ll_handle)
	ll_handle = tabpage_clause.tv_clause.finditem(roottreeitem!, 0)
loop
...
ll_root = tabpage_clause.tv_clause.insertitemlast(0,litv_item)
ll_handle = ll_root
...
for i = 1 to ll_cnt 
	...
	if ll_category <> ll_category_temp then 
		...
		litv_item.overlaypictureIndex = 1
		litv_item.selectedpictureindex = 2	
		ll_handle = tabpage_clause.tv_clause.insertitemlast(ll_root,litv_item)
	...
	if not isnull(ll_template_id) then 
		...
		tabpage_clause.tv_clause.insertitemlast(ll_handle,litv_item)		
	end if
next

ll_handle = tabpage_clause.tv_clause.finditem(roottreeitem!, 0)
if ll_handle > 0 then tabpage_clause.tv_clause.expanditem(ll_handle)

tabpage_clause.tv_clause.selectitem(ll_root)
if tabpage_clause.il_map_clause <> al_clause_id then 
	ll_height = tabpage_clause.ole_1.height
	tabpage_clause.of_template_maponoff(false)
	tabpage_clause.ole_1.height = ll_height
	if isvalid(w_clause_painter) then
		w_clause_painter.ib_map = false
	end if
end if
tabpage_clause.il_map_clause = al_clause_id
...
public subroutine of_restore ();string			ls_msg

//of_restore

OpenWithParm(w_agreement_restore, "d_agreement_clause_restore")

ls_msg = Message.StringParm

if not isnull(ls_msg) and ls_msg = 'success' then 
	this.tabpage_browse.cb_go.event clicked()
end if 
end subroutine

public function integer of_set_move_pointer (long al_xpos, long al_ypos);//====================================================================
// Function: of_set_move_pointer
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 08/28/2015
//--------------------------------------------------------------------
...
//
//====================================================================
long					ll_category_x , ll_category_y , ll_category_width, ll_category_height , ll_revision_x
long					ll_clause_full_name_x , ll_clause_full_name_width, ll_y
long					ll_audit_status_x,  ll_audit_user_x

ll_category_x = long(this.tabpage_browse.dw_browse.Describe("category_t.x"))
ll_category_width  = long(this.tabpage_browse.dw_browse.Describe("category_t.width"))
ll_category_y = long(this.tabpage_browse.dw_browse.Describe("category_t.y"))
ll_category_height  = long(this.tabpage_browse.dw_browse.Describe("category_t.height"))

ll_clause_full_name_x = long(this.tabpage_browse.dw_browse.Describe("clause_full_name_t.x"))
ll_clause_full_name_width  = long(this.tabpage_browse.dw_browse.Describe("clause_full_name_t.width"))

ll_revision_x  = long(this.tabpage_browse.dw_browse.Describe("revision_t.x"))

ll_audit_status_x  = long(this.tabpage_browse.dw_browse.Describe("audit_status.x"))
ll_audit_user_x  = long(this.tabpage_browse.dw_browse.Describe("audit_user.x"))

//gnv_debug.of_output( true, "dwo.name ="+string(dwo.name )+" xpos="+string(xpos)+" ypos="+string(ypos) )
//gnv_debug.of_output( true, "ll_category_x="+string(ll_category_x)+" ll_category_width="+string(ll_category_width)+ &
//			" ll_category_height="+string(ll_category_height)+" ll_category_y="+string(ll_category_y) + &
//			" ll_clause_full_name_x="+string(ll_clause_full_name_x)+" ll_clause_full_name_width="+string(ll_clause_full_name_width))

if al_xpos >= ll_category_x + ll_category_width -5 and al_xpos <= ll_clause_full_name_x + 5  and al_ypos >= ll_category_y  and al_ypos <= ll_category_y + ll_category_height  then
//	if al_xpos <= ll_audit_status_x + 10 then return 0					//min value
//	SetPointer(SizeWE! )		
	return 1
elseif al_xpos >= ll_clause_full_name_x + ll_clause_full_name_width -5  and al_xpos <= ll_revision_x + 5 and al_ypos >= ll_category_y and al_ypos <= ll_category_y + ll_category_height  then
//	if al_xpos <= ll_audit_user_x + 10 then return 0					//min value
//	SetPointer(SizeWE! )		
	return 2
else
//	SetPointer(Arrow!)						// maybe don't need it 	
end if 
...
//
//====================================================================
long						ll_width, ll_move, ll_x, ll_upperbound1,  i , ll_xpos
string 					ls_headerlabel[], ls_modify, ls_modify2
long						ll_audit_status_x, ll_audit_date_x, ll_audit_user_x, ll_audit_note_x

ls_headerlabel[1] = "revision"
ls_headerlabel[2] = "approved_by"
ls_headerlabel[3] = "date_approved"
ls_headerlabel[4] = "approval_status"
ls_headerlabel[5] = "checkout_by"
ls_headerlabel[6] = "checkout_date"
ls_headerlabel[7] = "temp_usage"
ls_headerlabel[8] = "dm_usage"
ls_headerlabel[9] = "negotiated"
ls_headerlabel[10] = "negotiated_precent"
ls_headerlabel[11] = "negotiated_last_date"
ls_headerlabel[12] = "attribute_1_value"
ls_headerlabel[13] = "attribute_2_value"
ls_headerlabel[14] = "attribute_3_value"
ls_headerlabel[15] = "attribute_4_value"
ls_headerlabel[16] = "attribute_5_value"
ls_headerlabel[17] = "attribute_6_value"
ls_headerlabel[18] = "modify_date"

ll_upperbound1 = upperbound(ls_headerlabel)

//move 
if il_move_x  > 0 and trim(is_move_position) <> "" then 
	if is_move_position = "category" then 			
		ll_x = long(this.tabpage_browse.dw_browse.describe( "category_t.x"))
		ll_audit_status_x = long(this.tabpage_browse.dw_browse.describe( "audit_status.x"))

		ll_xpos = al_xpos
		if al_xpos <= ll_audit_status_x + 10 then ll_xpos = ll_audit_status_x + 10			//min value
		if ll_xpos >  ll_x then 
			ll_width = ll_xpos - ll_x
//		else
//			ll_width = ll_category_x - ll_xpos
		end if 
		this.tabpage_browse.dw_browse.modify( "category_t.width="+string(ll_width)+  " category.width="+string(ll_width)+" audit_status.width="+string(ll_xpos - ll_audit_status_x))
		
		if ll_xpos > il_move_x then 
			ll_move = ll_xpos -  il_move_x
		else
			ll_move = ll_xpos -  il_move_x
		end if 
		
		ll_x  = long(this.tabpage_browse.dw_browse.describe( "clause_full_name_t.x"))
		ll_audit_date_x = long(this.tabpage_browse.dw_browse.describe( "audit_date.x"))
		ll_audit_user_x = long(this.tabpage_browse.dw_browse.describe( "audit_user.x"))
		ll_audit_note_x = long(this.tabpage_browse.dw_browse.describe( "audit_note.x"))
		this.tabpage_browse.dw_browse.modify( "clause_full_name_t.x ="+string(ll_x + ll_move )+  " clause_full_name.x ="+string(ll_x + ll_move ) )
		this.tabpage_browse.dw_browse.modify( "audit_date.x ="+string(ll_audit_date_x + ll_move )+" audit_user.x="+string(ll_audit_user_x + ll_move )+" audit_note.x="+string(ll_audit_note_x + ll_move ))
		
	elseif is_move_position = 'name' then
		ll_x  = long(this.tabpage_browse.dw_browse.describe( "clause_full_name_t.x"))
		ll_audit_user_x = long(this.tabpage_browse.dw_browse.describe( "audit_user.x"))
		ll_audit_note_x = long(this.tabpage_browse.dw_browse.describe( "audit_note.x"))
		
		ll_xpos = al_xpos
		if al_xpos <= ll_audit_user_x + 10 then ll_xpos = ll_audit_user_x + 10			//min value
		if ll_xpos >  ll_x then
			ll_width = ll_xpos - ll_x
//		else
//			ll_width = ll_x - ll_xpos
		end if 
		...
		this.tabpage_browse.dw_browse.modify( "clause_full_name_t.width="+string(ll_width)+  " clause_full_name.width="+string(ll_width))		
		this.tabpage_browse.dw_browse.modify( "audit_user.width="+string(ll_xpos - ll_audit_user_x )+  " audit_note.x="+string(ll_audit_note_x + ll_move))		
	...
	ls_modify = " "
	ls_modify2 = " "
	for i =1 to ll_upperbound1
		ll_x  = long(this.tabpage_browse.dw_browse.describe( ls_headerlabel[i]+"_t.x"))
		ls_modify += ls_headerlabel[i]+"_t.x ="+string(ll_x + ll_move) + " "
		ll_x  = long(this.tabpage_browse.dw_browse.describe( ls_headerlabel[i]+".x"))
		ls_modify += ls_headerlabel[i]+".x ="+string(ll_x + ll_move) + " "
	next 
	this.tabpage_browse.dw_browse.modify( ls_modify )
End if 
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 09-01-2015
//--------------------------------------------------------------------
...
public function integer of_send_clause_notification (long al_clause_id, string as_action);//====================================================================
// Function: of_send_clause_notification
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
string ls_to,ls_cc,ls_subject,ls_message,ls_errtext,ls_email_type,ls_computer_info
long li_send_result,ll_Return,ll_find,ll_null,ll_email_id
string ls_folder,ls_category,ls_clause,ls_auto_sign,ls_err_text
long ll_folder,ll_category,ll_ret
Blob		lblb_Message
...
select subject,email_to,email_cc,auto_sign into :ls_subject,:ls_to,:ls_cc,:ls_auto_sign from ctx_clause_email where email_id = :ll_email_id;	
if isnull(ls_to) then ls_to = ''
if ls_to = '' then
	messagebox('Clause notification','Please set the email clause notification settings first!')
	return -1
end if

if isvalid(w_clause_painter) then
	ll_find = w_clause_painter.tab_1.tabpage_browse.dw_browse.find("ctx_acp_clause_id = " + string(al_clause_id),1, w_clause_painter.tab_1.tabpage_browse.dw_browse.rowcount())
	if isnull(ll_find) then ll_find = 0
	if ll_find > 0 then
		ll_folder = w_clause_painter.tab_1.tabpage_browse.dw_browse.getitemnumber(ll_find, "ctx_acp_folder_id")
		ls_clause = w_clause_painter.tab_1.tabpage_browse.dw_browse.getitemstring(ll_find, "clause_name")
		ll_category = w_clause_painter.tab_1.tabpage_browse.dw_browse.getitemnumber(ll_find, "category")
		SELECT folder_name into :ls_folder FROM ctx_acp_folder where ctx_acp_folder_id = :ll_folder;
		SELECT description into :ls_category FROM code_lookup WHERE lookup_code = :ll_category;
		if as_action = 'checkout' then
			ls_subject = '[' + ls_folder + '] ' + '- [' + ls_category + '] ' + '- [' + ls_clause + '] ' + '- [' + gs_user_id + '] ' + '- [check out]'
		elseif as_action = 'checkin' then
			ls_subject = '[' + ls_folder + '] ' + '- [' + ls_category + '] ' + '- [' + ls_clause + '] ' + '- [' + gs_user_id + '] ' + '- [check in]'
		end if
	end if
end if

of_GetEMailMessage(ll_email_id,lblb_Message)
If Not IsValid( w_email_edit ) Then Open( w_email_edit )
ll_ret = w_email_edit.ole_word.of_Open_Buffer(lblb_Message)
If ll_ret < 0 Then 
	ls_err_text = 'Failed to open the clause notification message. ll_err = ' + String(ll_ret)
	gnv_debug.of_output(true, ls_err_text +  ', Clause ID = ' + String(al_clause_id) + '.')
	w_email_edit.ole_word.of_Close()
...
//ll_Return = lnv_Mail.of_sendtext(gs_user_id, ls_to,ls_cc, '',ls_subject,ls_message ,'')
ib_send_notification = true
ll_Return = lnv_Mail.of_SendHtml(gs_user_id, ls_to, ls_cc, '',ls_subject , w_email_edit.ole_Word, '', lb_auto_sign,ll_null,ll_null,ll_null  ) 
ib_send_notification = false	
...
End If
if as_action = 'checkout' then
	ls_email_type = 'Clause check out notification'
else
	ls_email_type = 'Clause check in notification'
end if
ls_computer_info = lnv_Mail.of_get_compute_info( )
Insert Into em_mail_audit(user_id,mail_from,mail_to,mail_cc,mail_bcc,mail_subject,mail_date,mail_attach_name,mail_template_id,ctx_id_list,alarm_type,send_result,error_text,compute_info)
Values (:gs_user_id,:gs_user_id,:ls_to,:ls_cc,'',:ls_subject,getdate(),'', null,null,:ls_email_type,:li_send_result,:ls_errtext,:ls_computer_info);

If  IsValid( w_email_edit ) Then w_email_edit.ole_word.of_Close()
If  IsValid( w_email_edit ) Then Close(w_email_edit)		
lnv_Mail.of_DeleteObject()
if ll_Return <> 0 then
	return -1
...
//Begin - Added By Mark Lee 03/25/2015.
if newindex =1 and oldindex =2 then 
	If w_mdi.of_security_access(6962) <> 0 and w_mdi.of_security_access(6800) = 2 Then //Added By Ken.Guo 2016-02-18 added if
		if  of_ismodified( )  then 
			if MessageBox("Save Clause", "The current Clause has been modified. ~r~n~r~nDo you want to save the changes?",Information! , YesNo!) = 1 then 
				of_save(False)
			end if 
		end if 
	End If
end if 
//End - Added By Mark Lee 03/25/2015.
...
string ls_search_latest
ls_search_latest = gnv_user_option.of_get_option_value("contractlogix-super-master", "search_latest_clause")
If  ls_search_latest  = 'Y'  or isnull(ls_search_latest) or trim(ls_search_latest ) = '' Then
	tabpage_browse.dw_filter.setitem(1,"search_latest",'Y')
Else
	tabpage_browse.dw_filter.setitem(1,"search_latest",'N')
End If
//end
end event
...
//Added By Jay Chen 08-27-2015
string ls_search_latest
if dwo.name = 'search_latest' then
	gnv_user_option.of_Set_option_value("contractlogix-super-master", "search_latest_clause",data)
	gnv_user_option.of_save()
	ls_search_latest = gnv_user_option.of_get_option_value("contractlogix-super-master", "search_latest_clause")
	if isnull(ls_search_latest) then ls_search_latest = ''
	if  ls_search_latest  = 'Y'  or trim(ls_search_latest ) = '' then
		ids_search_clause_text.dataobject = 'd_search_clause_text'
	else
		ids_search_clause_text.dataobject = 'd_search_clause_text2'
	end if
...
lm_dw.m_table.m_restorerow.enabled = true					//Added By Mark Lee 07/15/2015
lm_dw.m_table.m_addrow.visible = True
If This.rowcount() < 1 Then
	lm_dw.m_table.m_copy.visible = False //Copy Clause
	lm_dw.m_table.m_delete.visible = False
	lm_dw.m_table.m_restorerow.visible = False 						//Added By Mark Lee 07/15/2015
	lm_dw.m_table.m_save.visible = False
	lm_dw.m_table.m_rules.visible = False
	lm_dw.m_table.m_properties.visible = False
Else
...
//	lm_dw.m_table.m_properties.visible = True	
	...
	If isvalid(m_pfe_cst_clause_painter) Then  //Added By Jay Chen 11-06-2013
		...
		//Added By Mark Lee 07/16/2015
		If m_pfe_cst_clause_painter.m_edit.m_restorerow.enabled and m_pfe_cst_clause_painter.m_edit.m_restorerow.toolbaritemvisible Then
		...
		End If
	else   //Added By Jay Chen 11-06-2013
		lm_dw.m_table.m_addrow.visible = False
		lm_dw.m_table.m_copy.visible = False
		lm_dw.m_table.m_delete.visible = False
		lm_dw.m_table.m_restorerow.visible = False 						//Added By Mark Lee 07/15/2015
...
if dwo.name='p_1' then
	...
	//Added By Jay Chen 08-27-2015
	n_ds ds_clause_image
	long ll_find
	if len(is_full_text_search) > 0 then
		ds_clause_image = create n_ds 
		ds_clause_image.dataobject = 'd_search_clause_image_file'
		ds_clause_image.settransobject( sqlca)
		ds_clause_image.retrieve(ll_clause_id,is_full_text_search)
	end if
	//end
	li_new = row+1
	for li = 1 to li_rows
		...
		//Added By Jay Chen 08-27-2015
		if len(is_full_text_search) > 0 then
			ll_find = ds_clause_image.find("revision = " + string(ds_audit.getitemdecimal(li,'revision')), 1, ds_clause_image.rowcount())
			if isnull(ll_find) then ll_find = 0
			if ll_find > 0 then this.setitem(li_new,'include_full_text','1')	
		end if
		//end
		li_new = li_new+1
	next
...
if ib_mouse_down =true  then 								//Added By Mark Lee 08/28/2015
	SetPointer(SizeWE! )	
else
	SetPointer(Arrow! )	
end if 
...
event pfc_restorerow;//Added By Mark Lee 07/15/2015
iw_clause_painter.event pfc_restorerow( )
Return 1

end event

event lbuttondown;call super::lbuttondown;//Added By Mark Lee 08/27/2015    
...
li_return = of_set_move_pointer( xpos, ypos) 
if li_return > 0 then
	oldpointer = SetPointer(SizeWE! )	
	il_move_x =  xpos
	if li_return = 1 then 
		is_move_position = "category"
	else
		is_move_position = 'name'
	end if 
...
event lbuttonup;call super::lbuttonup;//Added By Mark Lee 08/27/2015    
integer li_return
long			ll_category_x

if ib_mouse_down = true then 
	//move 
	of_set_move(xpos)
end if 	
...
event dwnmousemove;call super::dwnmousemove;//Added By Mark Lee 08/27/2015   
integer 					li_return
li_return = 0 
if ib_mouse_down = false then
//	li_return = of_set_move_pointer( UnitsToPixels(xpos, XUnitsToPixels!), UnitsToPixels(ypos, YUnitsToPixels!)) 
	li_return = of_set_move_pointer( PixelsToUnits(xpos, XPixelsToUnits!), PixelsToUnits(ypos, YPixelsToUnits!)) 
//	li_return = of_set_move_pointer(xpos , ypos ) 
	if li_return > 0 then
		SetPointer(SizeWE! )	
	end if 
else
	if il_move_x > 0 then 	SetPointer(SizeWE! )		
end if 
