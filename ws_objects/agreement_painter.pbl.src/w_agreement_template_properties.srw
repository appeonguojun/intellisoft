...
n_cst_other_audit inv_audit //Added By Ken.Guo 2015-06-02
end variables
...
choose case lower(is_msg)
	case "add"		
		...
		if dw_1.getchild("reviewed_status",dwc_status) = 1 then
			...
			if ll_find > 0 then 
				...
				dw_1.setitem(ll_row,'date_reviewed',datetime(today(),now()))
				dw_1.setitem(ll_row,'modify_date',datetime(today(),now()))						//Added By Mark Lee 07/14/2015
	...
	case "edit"
		...
		ldt_modify_date = idw_browse.getitemdatetime(ll_currentrow,"modify_date")					//Added By Mark Lee 07/14/2015
		...
		dw_1.setitem(ll_row,"reviewed_status",ll_reviewed_status)	
		dw_1.setitem(ll_row,"modify_date",ldt_modify_date)											//Added By Mark Lee 07/14/2015
		dw_1.resetupdate() //Added By Ken.Guo 2015-06-02
...
if len(ls_column_name) > 0 then
	...
	messagebox( "ContractLogix", "Required value missing for " + ls_label_text + " on row 1. Please enter a value." )	
	dw_1.setfocus()
...
datetime ldt_date_reviewed, ldt_lastupdate,ldt_new_lastupdate, ldt_modify_date
blob lblb_file
...
//ldt_modify_date = dw_1.getitemdatetime(ll_row,"modify_date")									//Added By Mark Lee 07/14/2015
ldt_modify_date = datetime(today(),now())																//Added By Mark Lee 07/14/2015	
...
choose case lower(is_msg)
	case "add"
		...
		*/
		...
		//Added By Mark Lee 07/14/2015 add column modify_date
		//--------------------------- APPEON BEGIN ---------------------------
		...
		INSERT INTO ctx_acp_template(ctx_acp_template_id,category,template_name,export_id,
						wf_id,description,reviewed_by,date_reviewed,reviewed_status, doctype,modify_date)
 		     VALUES(:ll_template_id,:ll_category,:ls_template_name,:li_export_id,
						:li_wf_id,:ls_description,:ls_reviewed_by,:ldt_date_reviewed,:ll_reviewed_status,'doc', :ldt_modify_date);
		/*
		INSERT INTO ctx_acp_template(ctx_acp_template_id,category,template_name,export_id,
						wf_id,description,reviewed_by,date_reviewed,reviewed_status)
 		     VALUES(:ll_template_id,:ll_category,:ls_template_name,:li_export_id,
	...
	case "edit"
		...
		//update DB						//Added By Mark Lee 07/14/2015 add column modify_date
		update ctx_acp_template 
			set category=:ll_category, template_name=:ls_template_name,export_id=:li_export_id,
				 ...
				 date_reviewed=:ldt_date_reviewed,reviewed_status=:ll_reviewed_status, modify_date=:ldt_modify_date
		...
		inv_audit.of_reset()
		inv_audit.of_audit(dw_1, 10002, 'ctx_acp_template_id','template_name')
		If inv_audit.of_update() > 0 Then
			dw_1.resetupdate( )
		End If
...
//		ldt_lastupdate = idw_browse.getitemdatetime(ll_currentrow,"modify_date")							//Added By Mark Lee 07/14/2015 
		
		//gnv_appeondb.of_startqueue() //for web performance tuning	
		//get or gen current local template file
		if isvalid(w_agreement_template_painter) then
//			w_agreement_template_painter.of_get_template_file(ll_template_id,ldt_lastupdate)
			w_agreement_template_painter.of_get_template_file(ll_template_id,ldt_modify_date)						//Added By Mark Lee 07/14/2015
		end if
		...
		*/
		insert into ctx_acp_template(ctx_acp_template_id,category,template_name,export_id,
						wf_id,description,reviewed_by,date_reviewed,reviewed_status, doctype, modify_date)
 		     values(:ll_new_template_id,:ll_category,:ls_template_name,:li_export_id,:li_wf_id,
				  	   :ls_description,:ls_reviewed_by,:ldt_date_reviewed,:ll_reviewed_status,'doc', :ldt_modify_date);
		//-------------End Modified ----------------------------------------------				  
		...
		INSERT INTO ctx_acp_template_clause_rules
           (ctx_acp_template_id,ctx_acp_clause_id,rule_row_id,rule_name,screen_id,left_bracket,field_name,field_label,operator
           , value,right_bracket,logical,field_type,table_name,value_display,lookup_code,lookup_flag,data_view_id,used_master_ctx)
		select :ll_new_template_id,ctx_acp_clause_id,rule_row_id,rule_name,screen_id,left_bracket,field_name,field_label
           , operator,value,right_bracket,logical,field_type,table_name ,value_display,lookup_code,lookup_flag,data_view_id
           , used_master_ctx
...
If ib_need_update and not ib_disableclosequery Then
	...
	Integer li_return
	li_return = Messagebox( "ContractLogix", "Do you want to save changes?",exclamation!,YesNoCancel!,1 )	
