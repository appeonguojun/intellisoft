...
//		gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Agreement", 'Document-'+string( ll_doc_id), regstring!, string( ldt_lastmodify, "mm/dd/yyyy hh:mm:ss" ))
	...
	IF ll_length > 0 THEN
		...
		If li_FileNo < 0 Then
   		...
   		Messagebox( "ContractLogix", "Failed to create temporary file." )
			continue
		End If
		...
		For li_Cnt = 1 To li_Writes
			...
			If FileWrite(li_FileNo, lblb_Data) = -1 Then
   			...
   			Messagebox( "ContractLogix", "Failed to write temporary file." )
				continue
			End if
		Next
		...
		as_filename[ll_j] = ls_name
		as_pathname[ll_j] = ls_fullname
		gnv_filetime.of_setlastwritedatetime( ls_fullname, ldt_lastmodify) //Added By Ken.Guo 2015-04-29
	ELSE
...
event resize;call super::resize;tab_1.x = 15
tab_1.y = 0
