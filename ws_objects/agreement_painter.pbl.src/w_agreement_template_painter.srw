...
Long ll_seq_no,ll_template_id
String ls_template_name, ls_name
ll_template_id = Long(ls_rtn)

ls_name = ls_template_name + '(' + String(ll_template_id) + ')'  //Added By Ken.Guo 2015-06-01
gnv_appeondb.of_startqueue( )
Select template_name Into :ls_template_name From  ctx_acp_template Where ctx_acp_template_id = :ll_template_id;
Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_template_id and table_id = 10002;
...
values(:ll_template_id, -9999,:ll_seq_no,10002,0,'','INSERT',:gs_user_id,getdate(),'I', 0, null,null,:ls_name,:ls_name);
...
update ctx_acp_template set deleted = 'Y', deleted_by =:gs_user_id, deleted_date= getdate() where ctx_acp_template_id = :ll_template_id;
//End - Modified By Mark Lee 07/15/2015

//Added By Ken.Guo 05/04/2014
insert into sys_audit  (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)  
values(:ll_template_id, -9999,:ll_seq_no,10002,0,'','DELETE',:gs_user_id,getdate(),'D', 0, null,null,:ls_name,:ls_name);
...
if tab_1.selectedtab = 2 then 
	If tab_1.tabpage_2.ole_1.object.doctype > 0 Then tab_1.tabpage_2.ole_1.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
...
lstr_docinfo.doc_name = ls_template_name
//end
...
If ll_ctx_id[1] <= 0 Then
	Return 
End If
//ls_source_file = inv_word.of_run_export(li_export_id,ll_ctx_id[]) //Modified By Jay Chen 04-17-2014
//---------------------------- APPEON END ----------------------------

//if len(trim(ls_source_file)) < 1 then return //Modified By Jay Chen 04-17-2014
...
//Added By Jay Chen 11-27-2014
if ls_use_web_merge = '1' then  //use webservice merge
 	ll_rtn = lnv_webapi.of_mergedocument(gs_accesstoken,'T',il_template_id,li_export_id,ll_ctx_id[1],true,lstr_docinfo,ls_merge_fileid)
	 if len(ls_merge_fileid) > 0 then
		li_rtn = lnv_dm_utils.of_download_merge_file(ls_merge_fileid,is_agreement_path + "tem_1_merged.doc")
...
li_rtn = inv_word.of_get_document_export_field(lole_word.ActiveDocument,ll_ctx_id[1],li_export_id) //Modified By Jay Chen 08-01-2014
//Added By Jay Chen 08-01-2014
if li_rtn <> 1 then 
	IF isvalid( w_export_progress) THEN CLOSE(w_export_progress)
	f_word_destroy(lole_word,"")	
	RETURN 
...
IF not isvalid(w_export_progress) then Open(w_export_progress)			
w_export_progress.hpb_bar.position = 100
w_export_progress.hpb_1.position = 50
w_export_progress.st_progress.text = "Merging Data"
w_export_progress.st_export.text = ""
w_export_progress.st_export1.text = ""
//end add
...
OpenWithParm(w_agreement_restore, "d_agreement_template_restore")
...
TRY
	...
	IF not FileExists(as_file) THEN
		If tab_1.tabpage_2.ole_1.object.doctype > 0 Then tab_1.tabpage_2.ole_1.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-30. workaround Task 524
...
string ls_findword,ls_text
ls_text = lower(tab_1.tabpage_1.dw_filter.describe( "template_t.text"))
if trim(ls_text) = "full text search" then
	ls_findword = trim(tab_1.tabpage_1.dw_filter.getitemstring(1,'template'))
	if isnull(ls_findword) then ls_findword = ''
	if len(ls_findword) > 0 then
		iole_word.ActiveWindow.Selection.Find.ClearHitHighlight
		iole_word.ActiveWindow.Selection.Find.HitHighlight(ls_findword,65535)
	end if
end if
//end
...
//--------End Modified ----------------------------------------------
//------------------End Added------------------------
If tab_1.tabpage_2.ole_1.object.doctype > 0 Then tab_1.tabpage_2.ole_1.object.ActiveDocument.Saved = True //Added by Harry 2017-07-10 Task 846:No prompt to save changes in Template Library Painter
return 1
end function
...
//			m_pfe_cst_agreement_template.m_file.m_print.visible = true						//Added By Mark Lee 11/15/2012
			...
			m_pfe_cst_agreement_template.m_edit.m_restorerow.visible = true				//Added By Mark Lee 07/15/2015
...
//			m_pfe_cst_agreement_template.m_file.m_print.enabled = true					//Added By Mark Lee 11/15/2012	
			...
			m_pfe_cst_agreement_template.m_edit.	m_restorerow.enabled	 = true								//Added By Mark Lee 07/15/2015
...
//			m_pfe_cst_agreement_template.m_file.m_print.toolbaritemvisible = true					//Added By Mark Lee 11/14/2012
			...
			gnv_app.of_modify_menu_attr( m_pfe_cst_agreement_template.m_edit.m_restorerow,'toolbaritemvisible', true)					//Added By Mark Lee 07/15/2015
	...
	case "detail"
			...
			m_pfe_cst_agreement_template.m_edit.m_insert.visible	 = true
			m_pfe_cst_agreement_template.m_edit.m_delete1.visible	 = true
			m_pfe_cst_agreement_template.m_edit.m_restorerow.visible = false				//Added By Mark Lee 07/15/2015			
			m_pfe_cst_agreement_template.m_file.m_properties.visible	 = true
			m_pfe_cst_agreement_template.m_edit.m_copy.visible	 = true	
			m_pfe_cst_agreement_template.m_file.m_save.visible	 = true	
			m_pfe_cst_agreement_template.m_file.m_close.visible	 = true		
			...
			m_pfe_cst_agreement_template.m_edit.m_insert.enabled	 = true
			m_pfe_cst_agreement_template.m_edit.m_delete1.enabled	 = true
			m_pfe_cst_agreement_template.m_edit.m_restorerow.enabled	 = true							//Added By Mark Lee 07/16/2015
			m_pfe_cst_agreement_template.m_file.m_properties.enabled	 = true
			m_pfe_cst_agreement_template.m_edit.m_copy.enabled	 = true	
			m_pfe_cst_agreement_template.m_file.m_save.enabled	 = true	
			m_pfe_cst_agreement_template.m_file.m_close.enabled	 = true		
			...
			m_pfe_cst_agreement_template.m_edit.m_restorerow.toolbaritemvisible	 = true				//Added By Mark Lee 07/16/2015
...
IF w_mdi.of_security_access(6801) < 2 THEN
			...
			m_pfe_cst_agreement_template.m_edit.m_insert.enabled	 = false
			m_pfe_cst_agreement_template.m_edit.m_delete1.enabled	 = false
			m_pfe_cst_agreement_template.m_edit.m_restorerow.enabled	 = false						//Added By Mark Lee 07/15/2015
			m_pfe_cst_agreement_template.m_edit.m_copy.enabled	 = false	
			m_pfe_cst_agreement_template.m_file.m_save.enabled	 = false	
...
Long ll_seq_no
String ls_name
ls_name =  tab_1.tabpage_1.dw_browse.GetItemString(ll_row, 'template_name') + ' ('+ String(ll_template_id) + ')'
Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_template_id and table_id = 10002;
insert into sys_audit  (rec_id,field_id,seq_no,table_id,prac_id,old_value,new_value,user_id,date_time_modified,audit_type,exported,export_date,facility_id,tfield_name,tfield_name_allias)  
values(:ll_template_id, -9999,:ll_seq_no,10002,0,'','Modified Content',:gs_user_id,getdate(),'E', 0, null,null,:ls_name,:ls_name);
...
//	gnv_appeondll.of_registryget("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Agreement", "template_" + string(al_templateid), regstring!, ls_regupdate)	
	...
	gnv_filetime.of_getlastwritedatetime( ls_filename, ref ls_regupdate)
	...
	elseif i > ll_bmcnt then
   	if not fileexists(ls_filename) then return -1	
		...
		if ll_bmcnt = 0 then 
			...
			//Modified By Ken.Guo 02/14/2015
			//ll_start_ori = iole_word.content.end - 1 // Jervis 06.08.2009
			//iole_word.content.select()
			ll_start_ori = lole_Selection.end   //use curent cursor position
...
//Added by harry 2017-07-10 Task 846:No prompt to save changes in Template Library Painter
IF NOT of_issaved() THEN
	IF MessageBox("Save Document","The document has been modified, do you want to save changes?",Question!,YesNo!)  = 1 THEN 
		of_savefile()
	END IF
END IF		
