...
long ll_row, ll_clause_id,ll_rtn
...
string ls_merge_fileid, ls_use_web_merge,ls_clause_name
...
If ll_ctx_id[1] <= 0 Then
	...
	Return 
End If
//ls_source_file = lnv_word.of_run_export(li_export_id,ll_ctx_id[]) //Commented By Jay Chen 04-16-2014
//---------------------------- APPEON END ----------------------------

//if len(trim(ls_source_file)) < 1 then return //Commented By Jay Chen 04-16-2014

setpointer(HourGlass!)
IF not isvalid(w_export_progress) then Open(w_export_progress)			//Added By Mark Lee 05/29/2013
w_export_progress.hpb_bar.position = 0  //Modified By Jay Chen 04-17-2014 convert hpb_bar.position = 100 to hpb_bar.position = 0
...
if ls_use_web_merge = '1' then  //use webservice merge
 	ll_rtn = lnv_webapi.of_mergedocument(gs_accesstoken,'C',ll_clause_id,li_export_id,ll_ctx_id[1],true,lstr_docinfo,ls_merge_fileid)
	 if len(ls_merge_fileid) > 0 then
		li_rtn = lnv_dm_utils.of_download_merge_file(ls_merge_fileid,is_agreement_path + "tem_2_merged.doc")
...
li_rtn = lnv_word.of_get_document_export_field(lole_word.ActiveDocument,ll_ctx_id[1],li_export_id)  //Modified By Jay Chen 08-01-2014
...
//	Messagebox(gnv_app.iapp_object.DisplayName,'Clause analysis complete.')
	If lstr_option.as_template_usage = 'Y' Then
		gnv_user_option.of_Set_option_value( "contractlogix-super-master", "Last_Full_Clause_Analysis_date_for_temp_usage",string(date(now()),'MM/dd/yyyy')	 )
	End If 
	If lstr_option.as_dm_usage = 'Y' Then
		gnv_user_option.of_Set_option_value( "contractlogix-super-master", "Last_Full_Clause_Analysis_date_for_dm_usage",string(date(now()),'MM/dd/yyyy') )
	End If
	If lstr_option.as_negotiated = 'Y' Then 
		gnv_user_option.of_Set_option_value( "contractlogix-super-master", "Last_Full_Clause_Analysis_date_for_negotiated",string(date(now()),'MM/dd/yyyy') )
	End If 
...
//Added By Jay Chen 09-19-2014 security for template map
if w_mdi.of_security_access(6801) = 0 then
	messagebox('ContractLogix','You do not have permission to access this module.')
	return
end if
...
openwithparm(w_clause_template_list,tab_1.il_old_clause_id)


//tab_1.tabpage_clause.of_template_maponoff(ib_map)
//
//if not ib_map then
//	tab_1.il_old_clause_id = 0
//	tab_1.of_open_word_file("")
//	tab_1.tabpage_clause.is_current_file = ''
//end if
//
end event
...
boolean lb_trackchange_org //Added By Ken.Guo 2015-11-06
...
//	End If
	...
	lb_trackchange_org = lole_word.ActiveDocument.TrackRevisions //Added By Ken.Guo 2015-11-06
...
//	lole_word.ActiveDocument.MailMerge.MainDocumentType = wdNotAMergeDocument
	...
	lole_word.ActiveDocument.TrackRevisions = lb_trackchange_org //Added By Ken.Guo 2015-11-06
