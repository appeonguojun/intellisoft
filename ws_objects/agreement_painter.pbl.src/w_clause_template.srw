...
//Added By Jay Chen 07-21-2015 filter delete template 
if istr_clause.as_update_action <> 'remove_clause' then
	dw_template.setfilter("deleted <> 'Y'")
	dw_template.filter()
end if
...
Integer li_rtn
Long ll_rowcount, i, j, ll_template_id[],ll_seq_no
...
String 		ls_user_name,	ls_template_name, ls_msg	, ls_name		//Added By Mark Lee 07/20/12
...
//			Currently <User Name> is editing the <Template name> Template.  You will have to wait until <User> has finished editing that Template and closed the Template before continuing. 
		...
		//Modified By Jay Chen 09-11-2014
		if istr_clause.as_update_action = 'remove_clause' then
			li_rtn = w_clause_painter.tab_1.of_remove_template_clause(ll_template_id, ldt_modify_date,istr_clause.al_clause_id_arr[] )
		else
...
//		li_rtn = w_clause_painter.tab_1.of_update_template(ll_template_id, ldt_modify_date,istr_clause.ai_type,istr_clause.al_clause_id_arr[] )
		Choose Case li_rtn 
			Case 1
				...
				ls_name =  string(dw_template.describe("evaluate('LookUpDisplay ( ctx_acp_template_id )', "+string(i)+")" )) + ' (' + String(ll_template_id[1]) + ')'
				Select Isnull(max(seq_no),0) + 1 Into :ll_seq_no From sys_audit Where rec_id = :ll_template_id[1] and table_id = 10002;
				...
				values(:ll_template_id[1], -9999,:ll_seq_no,10002,0,'','Modified Content',:gs_user_id,getdate(),'E', 0, null,null,:ls_name,:ls_name);

			Case -2
				dw_template.SetItem(i,'update_result','Failure. The template is locked for update by another user. Please try again later.')
				lb_failed = True
				
			Case -4 //Added By Jay Chen 10-24-2015
				dw_template.SetItem(i,'update_result',w_clause_painter.tab_1.is_locktext)
				lb_failed = True
