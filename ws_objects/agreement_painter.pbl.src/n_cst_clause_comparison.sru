...
IF Lower(is_step) = 'target' THEN
	...
	If idw_browse.getitemstring(al_row , 'compare_status') <> '2' Then
		...
		ls_find = 'ctx_acp_clause_id = ' + String(ll_clause_id) + ' And revision = ' + String(ldec_revision)
		ls_find = gnv_string.of_globalreplace( ls_find, ',', '.') //Added By Ken.Guo 2016-04-19.  in french, the decimal symbol ',' should change to '.'

		ll_found = ids_target_clauses.Find(ls_find, 1, ids_target_clauses.RowCount())
		If ll_found < 1 Then 
...
ls_find = "ctx_acp_clause_id = " + String(al_clause_id) + " And revision = " + String(adec_revision)
ls_find = gnv_string.of_globalreplace( ls_find, ',', '.') //Added By Ken.Guo 2016-04-19.  in french, the decimal symbol ',' should change to '.'
IF ids_target_clauses.Find(ls_find, 1, ll_cnt) > 0 THEN
	RETURN "2"
END IF
