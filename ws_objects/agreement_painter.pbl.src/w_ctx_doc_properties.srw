...
if IsNull(ablb_image_file) then ablb_image_file = blob('')
...
IF isnull( ll_category ) THEN
	...
	messagebox( "ContractLogix", "Required value missing for Document Category on row 1. Please enter a value." )
...
IF isnull( ls_name ) or ls_name = '' THEN
	...
	messagebox( "ContractLogix", "Required value missing for Document Name on row 1. Please enter a value." )	
...
IF isnull( ll_status ) THEN
	...
	messagebox( "ContractLogix", "Required value missing for Review Status on row 1. Please enter a value." )		
	dw_main.setcolumn( "reviewed_status" )
	dw_main.setfocus()
	return -1
END IF
	...
	IF lb_uploadfile THEN
		...
		IF IsValid(w_sheet_ctx_doc_painter) THEN 
			IF il_doc_id = w_sheet_ctx_doc_painter.il_currentopened THEN
				...
				Try 
					If w_sheet_ctx_doc_painter.tab_1.tabpage_document.ole_word.object.doctype > 0 Then w_sheet_ctx_doc_painter.tab_1.tabpage_document.ole_word.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
