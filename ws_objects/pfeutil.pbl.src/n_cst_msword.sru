...
if mid(as_doc_path,len(as_doc_path) , 1) = "\" then 
	messagebox('Missing Document name','The letter path has no document.')
	return -1
end if
if mid(as_merge,len(as_merge) , 1) = "\" then  
...
w_infodisp_50.st_1.text = 'Opening . Please stand by'
*/
...
////w_infodisp_50.st_1.text = 'Opening ' +ls_array[upperbound(ls_array)] + ' Please stand by.'
...
is_letter_path =  as_doc_path //is_directory + ls_array[upperbound(ls_array)]
	...
	*/
	IF Isvalid(w_appeon_gifofwait) THEN Close(w_appeon_gifofwait)
	//---------------------------- APPEON END ----------------------------
iole_word.ActiveDocument.ActiveWindow.WindowState = 1// wdWindowStateMaximize //Added By Ken.Guo 06/13/2012
...
IF appeongetclienttype() = 'WEB' THEN
	...
	IF Isnull(lb_ole_visible) OR NOT lb_ole_visible THEN
      messagebox("Information",'The word document is no longer active. Please reopen the document and keep it open until the faxing process has completed.' )
		return -1
	END IF
END IF
//---------------------------- APPEON END ----------------------------
...
w_infodisp_50.st_3.visible = false
w_infodisp_50.st_information.visible = false
w_infodisp_50.st_1.text = 'Creating MicroSoft Word dodument. Please stand by'
...
Next
ll_filelen = Len(ls_filetext)
filedelete(ls_filename)
Do 
	ll_pos = pos(ls_filetext,'~r~n',ll_pos + 1)
...
For ll_i = 1 To li_merge_doc_pages
	   ...
	   iole_word.ActiveDocument.MailMerge.DataSource.FirstRecord = ll_i
		...
		iole_word.ActiveDocument.SaveAs( LS_DOC_NAME + '_' +string(ll_i) + '.doc',0,false,'',false)
...
ls_currdir = gs_dir_path + gs_DefDirName + "\Fax\*.*"
li_entries = inv_filesrv.of_DirList(ls_currdir, 0, lnv_dirlist)

for ll_i = 1 to upperbound(lnv_dirlist)
    FileDelete ( gs_dir_path + gs_DefDirName + "\Fax\" + lnv_dirlist[ll_i].is_filename )
next
...
event constructor;is_directory = gs_dir_path + gs_DefDirName + "\fax\"
