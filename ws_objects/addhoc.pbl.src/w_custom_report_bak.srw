$PBExportHeader$w_custom_report_bak.srw
forward
global type w_custom_report_bak from pfc_w_main
end type
type tab_1 from tab within w_custom_report_bak
end type
...
type cb_24 from commandbutton within tabpage_run
end type
...
global w_custom_report_bak w_custom_report_bak
...
IF li_rc > 0 THEN
	...
	FOR i = 1 TO li_rc	
		...
		ll_Found   = Tab_1.Tabpage_Columns.dw_view_fields.Find("table_name = '" + ls_TabName + "' and field_name = '" + ls_FieldName + "'",1,ll_RowCnts)
...
long ll_i
string 	ls_face ,ls_strikeout ,ls_font_height, ls_err
...
on w_custom_report_bak.destroy
call super::destroy
destroy(this.tab_1)
end on
...
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.em_zoom then continue
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.vsb_1 then continue
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.st_8 then continue
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.st_9 then continue
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.st_step then continue
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_next_run then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_suit then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_sql then continue
//    if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_showsql then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_13 then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_sort then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_filter then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_print then continue
//	if tab_1.tabpage_run.Control[ll_i] = tab_1.tabpage_run.cb_saveas then continue
   this.inv_resize.of_Register (tab_1.tabpage_run.Control[ll_i],inv_resize.scale)
next
//Added By chenjian 2013-09-13
//inv_resize.of_Register(tab_1.tabpage_run.em_zoom, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.vsb_1, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.st_8, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.st_9, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.st_step,inv_resize.fixedbottom )
//inv_resize.of_Register(tab_1.tabpage_run.cb_next_run, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_suit, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_sql,inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_showsql, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_13, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_sort,inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_filter, inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_print,inv_resize.fixedbottom)
//inv_resize.of_Register(tab_1.tabpage_run.cb_saveas,inv_resize.fixedbottom)
//
...
ELSEIF oldindex = 3 THEN
	...
	do while ll_find > 0
		...
		ll_Max_Order++
		ll_sort = tab_1.tabpage_sort.dw_view_sort.Find( "table_name = '" + ls_v_name + "' and field_name='" + ls_c_name + "'", 0, tab_1.tabpage_sort.dw_view_sort.RowCount()) 
		if ll_sort = 0 then 
...
event clicked;Open(w_auto_report)
end event
...
*/
	...
	if ck = 1 then
		...
		INSERT INTO ctx_report_syntax(view_id,dw_sql,dw_syntax,modify_date)
			  SELECT :il_view_id,dw_sql,dw_syntax,modify_date
				 from ctx_report_syntax
...
event rowfocuschanged;long	ll_TableID
...
CHOOSE CASE lower(dwo.name)
	CASE 'table_field_name'
		...
		ll_Pos = Pos(data,'.')		
		ls_v_name = Left(data,ll_Pos - 1)
		ls_c_name = Mid(data ,ll_Pos + 1)
		IF of_get_lookup_data(ls_v_name,ls_c_name,ls_Display,ls_lookup_type,ls_lookup_name) = 1 THEN
			if isNull(ls_lu_field_name) or ls_lu_field_name = "" then
				IF lower(ls_Display) = 'code' then ls_Display = 'Code' else ls_Display = 'Description'
				this.SetItem( row, "lu_field_name", ls_Display )
...
// Event: clicked
//--------------------------------------------------------------------
// Description:利用该PageCountAcross()函数可以让数据窗口的内容自动适应纸张的大小 
...
if k=0 and j=5 then 
	messagebox("Infomation","Your report is to wide,current print can not adapt!")
end if 
...
string ls_objects,ls_colname,ls_objname,ls_modify,ls_x,ls_width,ls_papersize,ls_paperorientation
long ll_seppos,ll_maxpos,ll_paperpbunit,ll_rate,ll_dwwidth
dec{2} ld_paperwidth

dw_run.setredraw(false)
...
ll_paperpbunit = ld_paperwidth * 3380/21  
...
////3.modify datawindow control's position
//ls_objects = dw_run.Describe("DataWindow.Objects") + "~t"
//ll_seppos = Pos(ls_Objects,"~t")
//
//Do While ll_seppos > 0
//	ls_objname = Left(ls_objects,ll_seppos - 1)
//	ls_objects = Mid(ls_objects,ll_seppos + 1)
//	ll_seppos  = Pos(ls_objects,"~t")
//	li_x = Long( dw_run.Describe( ls_objname + ".x") )
//	li_width = Long( dw_run.Describe( ls_objname + ".width") )
//	if ll_paperwidth > 0 then
//		ls_x = string(li_x * ll_dwwidth / ll_paperwidth)
//		ls_modify = ls_objname+".x='"+ls_x+"'"
//		dw_run.modify(ls_Modify)
//		ls_width = string(li_width * ll_dwwidth / ll_paperwidth)
//		ls_modify = ls_objname+".width='"+ls_width+"'"
//		dw_run.modify(ls_Modify)
//	end if
//loop
//
dw_run.setredraw(true)
end event
