...
type cb_hide from commandbutton within w_auto_report
end type
...
string title = "ContractLogix Automated Report"
...
If ll_ret > 0 Then
	...
	If ll_ret = 0 Then
		gnv_debug.of_output(false, 'Auto Report Success. Success to send the email. Report name = ' + ls_report_name)						//Added By Mark Lee 09/07/2015 for debug
		dw_browse.SetItem(al_row, 'error_code', 1)
		dw_browse.Update()	
		ll_ret = 1
	Else
	...
	End If
Else
	gnv_debug.of_output(false, 'Error Originated from Contractlogix Schedule Report. Report name = ' + ls_report_name)						//Added By Mark Lee 09/07/2015 for debug
	ls_subject = 'Error Originated from Contractlogix Schedule Report'
...
if gb_AutoReport then Halt close
end event
...
If dw_detail.ModifiedCount() > 0 Then
	li_return = Messagebox('Save','Do you want to save the data?', Question!, Yesnocancel!) 
	If li_return = 1 Then
...
ls_msg = "The Auto-Report function will stop working after you close this window.~r~n"+&
		"If you do not want to terminate the Auto-Report function, please minimize the window instead.~r~n"+&
