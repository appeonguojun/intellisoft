...
type cb_back_2 from commandbutton within tabpage_tables
end type
...
type cb_back_3 from commandbutton within tabpage_columns
end type
...
type cb_back_4 from commandbutton within tabpage_filter
end type
...
type cb_back_5 from commandbutton within tabpage_sort
end type
...
type cb_back_6 from commandbutton within tabpage_preview
end type
type cb_next_6 from commandbutton within tabpage_preview
end type
type dw_display from u_dw within tabpage_preview
end type
type cb_23 from commandbutton within tabpage_preview
end type
...
type cbx_1 from checkbox within tabpage_run
end type
type cb_suit from commandbutton within tabpage_run
end type
type st_9 from statictext within tabpage_run
end type
type st_8 from statictext within tabpage_run
end type
type em_zoom from editmask within tabpage_run
end type
...
String is_PreObject_Name,is_PreObject_Color,is_CurObject_Name
...
datetime	idt_priordate 

String is_ctx_access_inner_sql

boolean ib_access_filter//Ken.Guo 2009-04-21
integer ii_prior_step
char ic_newpage_group //jervis 08.09.2010
//String		is_role_filter										//Added By Mark Lee 03/19/2015
n_ds		ids_role_filter
string						is_find_all_row	= ""					//Added By Mark Lee 07/26/2016		
end variables
...
public function string of_build_alias (long al_row, string as_label)
...
public function integer of_run_report (long al_report_id, string as_output_file, ref string as_error)
public function decimal of_get_original_size (string size_type, string size_orientation)
public subroutine of_check_report_name (long al_view_id)
public subroutine of_restrict_dynamic_filter ()
public subroutine of_setfilter_role_data ()
public subroutine of_destory_ds ()
public subroutine of_filter_role_data (string as_category, ref string as_return, ref long al_arr[])
public subroutine of_checked_action_right (string as_action)
public subroutine of_generate_lookup_column (string as_type)
private function long of_get_lookup_data (string as_v_name, string as_c_name, ref string as_display, ref string as_lookup_type, ref string as_lookup_name)
public function integer of_multi_select_sql (string as_field_name, string as_values, ref string as_multi_sql)
public function string of_get_company_name (string as_company_id)
end prototypes
...
//comment 12.11.2007 by: Andy 
//Reason:cust_table_name is no use now
/*
...
//	END IF
//END FOR
//$<del> 07.08.2008 by Andy
//if of_check_filter() = -1 then return -1
...
lbn_retrieve = wf_check_link_column()
...
FOR i = 1 TO li_rc
	...
	//Added by Nova on 2008-07-28  begin
			//Added by Nova on 2008-07-28  begin
			if ll_sel_table_id = 0 then 
				ll_FieldCnt ++
				//ls_table_fields[ll_FieldCnt] = ls_tbl_nm + "." + ls_fld_nm	//modified by nova 2008-09-22
				ls_table_fields[ll_FieldCnt] =  ls_fld_nm
				continue
			end if	
  //Added by Nova on 2008-07-28 end
	//--------------------------- APPEON BEGIN ---------------------------
	//$< Add  > 2007-08-23 By: Scofield
...
FOR i = 1 TO li_rc
	...
	lb_Flag = false
	//$<modify> 05.21.2008 by Andy 
	//$<Reason>:Fixed BugG051401
//	for ll_Cycle = 1 to ll_Count
//		if Pos(ls_field,ls_table_ids[ll_Cycle]) > 0 then
//			lb_Flag = true
//			Exit
//		end if
//	next
...
tab_1.tabpage_tables.dw_view_links.AcceptText( )			//Added By Mark Lee 06/04/12
...
string ls_operator //jervis 02032010
string ls_value
string ls_start,ls_end,ls_LeftBracket,ls_RightBracket,ls_Logical,ls_sort_field

long		ll_Found,ll_RowCnts
String	ls_FieldAlias,ls_TabName,ls_FieldName,ls_SortSynt


String ls_multi_select_sql, ls_value_code,ls_value_new,ls_value_code_new, ls_value_code_arr[] , ls_empty_arr[]//Added By Ken.Guo 2017-05-12
Long ll_i //Added By Ken.Guo 2017-05-12
...
FOR i = 1 TO li_rc
	...
	if tab_1.tabpage_columns.dw_view_fields.GetItemNumber( i, "field_visible" ) = 2 then continue //Added By jay 2016-04-05
	...
	ELSE
		...
		if IsNull(ls_lu_field) or Trim(ls_lu_field) = "" then
			...
			if ls_lookup_type = "C" or ls_lookup_type = "A" then
				...
				//Added By Mark Lee 03/20/2015		add the lower() function
				ll_Find = ids_fields.Find("lower(c_name)='" + lower(ls_c_name) + "' and lower(table_name)='" + lower(ls_table_name) + "'",0,ids_fields.RowCount())
				if ll_Find > 0 then
				...
				//end modify
			elseif ls_lookup_type = "Y" or ls_lookup_type = "U" or ls_lookup_type = "B" or ls_lookup_type = "O" then					//Added By Mark Lee 06/12/2016
				//get field width
				ll_field_width = 0
				ls_c_name = tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "table_field" )	
				ll_Find = ids_fields.Find("lower(c_name)='" + lower(ls_c_name) + "' and lower(table_name)='" + lower(ls_table_name) + "'",0,ids_fields.RowCount())
				...
				if isnull(ll_field_width) then ll_field_width = 150
				ls_str = "app_facility_"
				if isnull(ls_lu_field) or trim(ls_lu_field) = '' then ls_lu_field = 'facility_name'
				ls_str += tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_alias" ) + "." + ls_lu_field
				ls_sql = ls_sql + "~t" + "( CASE WHEN " + ls_str + " IS NULL THEN CONVERT(VARCHAR(" + String(ll_field_width) + ")," + ls_table_name + "." + ls_c_name + ") ELSE " + ls_str + " END ) " + tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_alias" ) + " ,  " + "~n"				
			end if	
		end if
...
IF li_rc > 0 THEN
	IF lb_multi_table = True THEN
		ls_sql = ls_sql + " AND ~n( ~n" //Modified By Ken.Guo 2015-11-03 added '('
	ELSE
		ls_sql = ls_sql + " WHERE ~n(" + "~n"	  //Modified By Ken.Guo 2015-11-03 added '('
	END IF
	FOR i = 1 TO li_rc
		ls_LeftBracket = Tab_1.TabPage_Filter.dw_view_where.GetItemString(i,"left_bracket")
		if IsNull(ls_LeftBracket) then ls_LeftBracket = ""
		...
		if pos(ls_tname,"v_",1) > pos(ls_tname,".",1) then //would designate a computed field //maha 020304
			...
			*/
			if gs_dbtype = "ASA" then
				ls_sql = ls_sql + "~t" + ls_LeftBracket + "(" + mid(ls_tname,1,pos(ls_tname,".",1) - 1) //get computed field name
			else
				ls_sql = ls_sql + "~t" + ls_LeftBracket + "(" + mid(ls_tname,pos(ls_tname,".",1) + 1)
			end if
			//---------------------------- APPEON END ----------------------------
			
		else //all other fields
			ls_sql = ls_sql + "~t" + ls_LeftBracket + "(" + ls_tname 
		end if
		ls_operator = tab_1.tabpage_filter.dw_view_where.GetItemString( i, "operator" ) //Jervis 02.03.2010
		ls_sql = ls_sql + " " + ls_operator + " "
		IF ls_operator = "In" THEN
			
			//Modified By Ken.Guo 2017-05-12. fix bug. Conversion failed when converting the varchar value to data type int.
			...
			If ls_field_type  = 'C' Then
				ls_value_code_new = tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value" ) 
				ls_value_code_arr[] = ls_empty_arr[]
				gnv_string.of_parsetoarray( ls_value_code_new,", ",ls_value_code_arr[] )
				For ll_i = 1 To UpperBound(ls_value_code_arr[])
					If left(ls_value_code_arr[ll_i],1) <> "'" and right(ls_value_code_arr[ll_i],1) <> "'" Then
						ls_value_code_arr[ll_i] = "'" + ls_value_code_arr[ll_i] + "'"		
					End If
				Next
				gnv_string.of_arraytostring(ls_value_code_arr[], ", ", ls_value_code_new )
				ls_sql = ls_sql + "(" + ls_value_code_new + ")"
			Else
				ls_sql = ls_sql + "(" +  tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value" )  + ")"
			End If
			
			
		elseif ls_operator = 'Like' then //Add by jervis - 02.03.2010
			ls_value = tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value" )
			if len(ls_value) > 2 then
				ls_start = Left(ls_value,1)
				ls_end = Right(ls_value,1)
				if (ls_start = "'" and ls_end = "'") or (ls_start = '"' and ls_end = '"') then
					ls_value = Mid(ls_value,2,len(ls_value) - 2)
				end if
			end if
			ls_sql = ls_sql + "'%" + ls_value + "%'"
		else
			ls_sql = ls_sql + tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value" )
		END IF	
			//li_remove_len = 6
		...
		if (Upper(ls_lookup_type) = 'C' or Upper(ls_lookup_type) = 'A') and ls_field_type = 'C' then
			If ls_operator = '<>' Then //Added By Ken.Guo 06/19/2012
				ls_sql = ls_sql + " AND " + ls_tname + " " + ls_operator + " "
			Else
				ls_sql = ls_sql + " OR " + ls_tname + " " + ls_operator + " "
			End If
			IF ls_operator = "In" THEN
				ls_sql = ls_sql + "(" + tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value_display" ) + ")"
			elseif ls_operator = 'Like' then	//Add by jervis - 02.03.2010
				ls_value = tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value_display" )
				if len(ls_value) > 2 then
					ls_start = Left(ls_value,1)
					ls_end = Right(ls_value,1)
					if (ls_start = "'" and ls_end = "'") or (ls_start = '"' and ls_end = '"') then
			...
			If ls_operator = '<>' Then
				//Modified By Ken.Guo 2017-05-12 fix bug
				//ls_sql = ls_sql + " or " + ls_tname + " IS NULL "
				ls_sql = ls_sql + " and " + ls_tname + " IS Not NULL "
			End If
		...
		//Added By Ken.Guo 2017-05-12 Task 779:Dynamic clauses based on multi-select fields do not work
		ls_multi_select_sql = ''
		If ( (Upper(ls_lookup_type) = 'C' or Upper(ls_lookup_type) = 'A' or Upper(ls_lookup_type) = 'Y' or Upper(ls_lookup_type) = 'U' or Upper(ls_lookup_type) = 'B' or Upper(ls_lookup_type) = 'O' ) 	and ls_field_type = 'C') and &
		   ( (Left (ls_tname, 21 ) = 'v_ctx_custom2.custom_'   and isnumber(mid(ls_tname,22)))  or &
			 (Left (ls_tname, 20 ) = 'v_ctx_custom.custom_'   and isnumber(mid(ls_tname,21)))  )   Then
	
			ls_value_code = tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value_display" )
			ls_value = tab_1.tabpage_filter.dw_view_where.GetItemString( i, "value" )
			This.of_multi_select_sql( ls_tname, ls_value + ' ', ls_multi_select_sql)
			If ls_multi_select_sql <> '' Then
				Choose Case lower(ls_operator) 
					Case 'like'
						If left(ls_value_code , 1 ) = "'" Then ls_value_code = mid(ls_value_code, 2)
						If right(ls_value_code, 1) = "'" Then ls_value_code = mid(ls_value_code, 1, len(ls_value_code) - 1 )					
						If ls_value_code <> '' Then
							ls_multi_select_sql = " and " +  ls_tname + " like '%"  + ls_value_code + "%' "
						End If
					Case '<>'
						ls_sql = ls_sql + ' and not  '  +  ls_multi_select_sql
					Case '=' , 'in'
						ls_sql = ls_sql + ' or '  +  ls_multi_select_sql
				End Choose
			End If
		End If		
		
		
		//BEGIN---Modify by Scofield on 2010-04-15
		ls_RightBracket = Tab_1.TabPage_Filter.dw_view_where.GetItemString(i,"right_bracket")
		if IsNull(ls_RightBracket) then ls_RightBracket = ""
		ls_sql = ls_sql + ls_RightBracket
		
		ls_sql = ls_sql + ") "
		
		if i < li_rc then
			ls_Logical = Tab_1.TabPage_Filter.dw_view_where.GetItemString(i,"logical")
			if IsNull(ls_Logical) then ls_Logical = "and"
			ls_sql = ls_sql + ls_Logical
		end if
		
		ls_sql = ls_sql +  " ~n"
		//END---Modify by Scofield on 2010-04-15
	END FOR
	ls_sql = ls_sql +  " )" //Added By Ken.Guo 2015-11-03
END IF
...
IF li_rc > 0 THEN
	...
	FOR i = 1 TO li_rc	
		...
		//Added By Ken.Guo 10/24/2012. Support SQL Server 2005 or above. ORDER BY items must appear in the select list if SELECT DISTINCT is specified
		If tab_1.tabpage_tables.cbx_distinct.checked Then 
			...
			//Added By Mark Lee 03/20/2015		add the lower() function
			ll_Find = tab_1.tabpage_columns.dw_view_fields.find("lower(field_name)='"+lower(ls_sort_field)+"'",1, tab_1.tabpage_columns.dw_view_fields.rowcount() )
			if ll_Find = 0 then
				Continue
			end if
		End If
		
		//BEGIN---Modify by Scofield on 2010-04-27
		ls_TabName   = Tab_1.Tabpage_Sort.dw_view_sort.GetItemString(i,"table_name")
		ls_FieldName = Tab_1.Tabpage_Sort.dw_view_sort.GetItemString(i,"field_name")
		
		ll_RowCnts = Tab_1.Tabpage_Columns.dw_view_fields.RowCount()
		//Added By Mark Lee 03/20/2015		add the lower() function
		ll_Found   = Tab_1.Tabpage_Columns.dw_view_fields.Find("lower(table_name) = '" + lower(ls_TabName) + "' and lower(field_name) = '" + lower(ls_FieldName )+ "'",1,ll_RowCnts)
		
		if ll_Found > 0 then
			ls_FieldAlias = Tab_1.Tabpage_Columns.dw_view_fields.GetItemString(ll_Found,"field_alias")
			ls_SortSynt   = ls_FieldAlias
		end if
		
		if ll_Found <= 0 or IsNull(ls_FieldAlias) or Trim(ls_FieldAlias) = "" then
			ls_FieldAlias = ls_TabName + "." + ls_FieldName
			ls_SortSynt   = ls_FieldName
		end if
		
		ls_sql += ls_FieldAlias + " "
		is_sort_synt += ls_SortSynt + " "
		//END---Modify by Scofield on 2010-04-27
...
Integer li_previous_col //Added By Ken.Guo 2010-10-13.
...
//Add New page on group break - jervis 08.09.2010
if ic_newpage_group = '1' then
...
For i = 1 To li_rc
	...
	If tab_1.tabpage_columns.dw_view_fields.GetItemNumber( i, "field_visible" ) = 0 Then 
		ll_width = 0
		//Added By Mark Lee 09/15/2014  when you create a report use the unvisible column at the frist time, the column will need to display in the design screen.
		adw_preview.Modify( ls_label + ".Visible='0' " +  ls_column + ".Visible='0' ")
	end if
	...
	If IsNull( tab_1.tabpage_columns.dw_view_fields.GetItemNumber( i, "field_x" ) ) Or +&
		...
		Else
			...
			if i > 1 then //Add by jervis - 01.28.2010
				...
				//Begin Modified By Ken.Guo 2010-10-13
				//ls_x = string(long(adw_preview.Describe( "#" +string(i - 1) + ".x")) + long(adw_preview.Describe( "#" +string(i - 1) + ".width")) + 20)
				If li_previous_col > 0 Then
					ls_x = string(long(adw_preview.Describe( "#" +string(li_previous_col) + ".x")) + long(adw_preview.Describe( "#" +string(li_previous_col) + ".width")) + 20)										
				Else
					ls_x = string(long(adw_preview.Describe( "#" +string(i - 1) + ".x")) + long(adw_preview.Describe( "#" +string(i - 1) + ".width")) + 20)
				End If
				li_previous_col = i 
				//End Modified By Ken.Guo 2010-10-13
	...
	Else
		...
		tab_1.tabpage_columns.dw_view_fields.SetItem( i, "field_x", Long(ls_x)  )
	End If
	If i = 1 Then li_previous_col = i  //Added By Ken.Guo 2010-10-13.
...
If Len(Trim(is_filter)) > 0 Then
	adw_preview.Modify("create text(band=header text='Filter: " + is_filter + "' border='0' color='0' x='10' y='150' height='150' width='3400'  font.face='MS Sans Serif' font.height='-8' font.weight='600'  font.family='2' font.pitch='2' font.charset='0' background.mode='1' background.color='553648127' name=title3_t)" )
End If
//END IF

/*
//Create Default Picture object -- Jervis 11.19.2009
datastore lds_picture
long ll_i,ll_x,ll_y,ll_height
string ls_band ,ls_name,ls_filename
lds_picture = create datastore
lds_picture.dataobject = 'd_conv_view_pic'
lds_picture.Settransobject( sqlca)
lds_picture.Retrieve( 0,0)
li_cnt = lds_picture.Rowcount( )
for ll_i = 1 to li_cnt
	//Download picture to local
	//of_download_pic(view_id,pic_id)
	
	ll_x = lds_picture.GetItemNumber( ll_i,"Pic_x")
	ll_y = lds_picture.GetItemNumber( ll_i,"Pic_y")
	ll_width = lds_picture.GetItemNumber( ll_i,"Pic_width")
	ll_height = lds_picture.GetItemNumber( ll_i,"Pic_height")
	ls_band = lds_picture.GetItemString( ll_i,"Pic_band")
	ls_name = lds_picture.GetItemString( ll_i,"Pic_name")
	adw_preview.Modify("create picture(band =" + ls_band + " name = " + ls_name + " filename = '" + ls_filename + "' X='" + string(ll_x) + "' Y = '" + string(ll_y) + "' width = '" + string(ls_width) + "' height = '" + string(ll_height) + "')")
next
destroy lds_picture
*/
...
ib_new = False //Added By Ken.Guo 2010-10-13.
Return 0
...
FOR i = 1 TO li_rc
	IF Len( tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_wizard_name" ) ) > 0 THEN
		...
		//Added by Nova on 2008-07-28 begin
		if pos(is_wizard_field_name[ li_cnt ],"v_",1) > pos(is_wizard_field_name[ li_cnt ],".",1) then 
				if gs_dbtype = "ASA" then
				is_wizard_field_name[ li_cnt ]= mid(is_wizard_field_name[ li_cnt ],1,pos(is_wizard_field_name[ li_cnt ],".",1) - 1) //get computed field name
			else
				is_wizard_field_name[ li_cnt ]=mid(is_wizard_field_name[ li_cnt ],pos(is_wizard_field_name[ li_cnt ],".",1) + 1)
			end if
		end if 	
		//Added by Nova on 2008-07-28 end
		...
		//Added By Mark Lee 03/20/2015		add the lower() function
		ll_Find = ids_fields.Find("lower(c_name)='" + lower(tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_name" )) + "' and lower(table_name)='" + lower(tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "table_name" ) )+ "'",0,ids_fields.RowCount())
		if ll_Find > 0 then
...
//is_role_filter		= ""					//Added By Mark Lee 03/19/2015 
of_next_wizard()
...
string ls_app_name
string		ls_arr[], ls_sql
string 				ls_tmp
long					ll_all_row, ll_find					//Added By Mark Lee 07/26/2016
String ls_field_values, ls_multi_sql	//Added by harry 20170508 for Task 750
...
IF ii_current_wizard_step > 0 THEN 
	...
	IF tab_1.tabpage_run.dw_wizard.DataObject = "d_crit_date_range_custom" THEN
		...
		END IF
		*/
		//Set default when value is null  - jervis 03.02.2011 
		if isnull(ls_from_date) then ls_from_date = ""
		if isnull(ls_to_date) then ls_to_date = ""
		
		//Commented By Ken.Guo 2011-03-23
		//if isnull(ls_from_date) then ls_from_date = string(today(),'yyyy/mm/dd')
		//if isnull(ls_to_date) then ls_to_date = string(today(),'yyyy/mm/dd')
		
		is_filter = is_filter + "Date Range=" + ls_from_date + " - " + ls_to_date
		IF ls_from_date + ls_to_date <> "" THEN
			if ls_from_date <> "" then
				ls_from_date = "Convert(datetime, " + is_wizard_field_name[ ii_current_wizard_step ] + ",102 )  >= Convert(datetime,'" + ls_from_date + "',102) "
			end if
			if ls_to_date <> "" then
				If ls_from_date <> "" Then //Added By Ken.Guo 2011-03-23. Added If statement
					ls_to_date = " And Convert(datetime, " + is_wizard_field_name[ ii_current_wizard_step ] + ",102 )  <= Convert(datetime,'" + ls_to_date + "' ,102 )"
				Else
					//Added By Ken.Guo 2011-03-23.
					ls_to_date = " Convert(datetime, " + is_wizard_field_name[ ii_current_wizard_step ] + ",102 )  <= Convert(datetime,'" + ls_to_date + "' ,102 )"
				End If
			end if
			is_sql_where = is_sql_where + "(" + ls_from_date + ls_to_date + ")"
			ii_total_where_values++								
		END IF
	...
	ELSEIF tab_1.tabpage_run.dw_wizard.DataObject = "d_crit_net_dev_type_ir" then //maha 071604
			IF tab_1.tabpage_run.dw_wizard.rowcount( ) >0 then 							//Added By Mark Lee 03/18/2015 sometimes, the dw_wizard doesn't have any row.
				IF Len( is_filter ) > 1 THEN
					is_filter = is_filter + " : "
				END IF		
				//Added By Mark Lee 07/26/2016 apb bug, the first row maybe not is the 'all' row in the web.
				ll_all_row = 0 			// 0, nuselect ; 1 , select
				if trim(is_find_all_row) <> "" then 
					ll_find = tab_1.tabpage_run.dw_wizard.Find(is_find_all_row,1,tab_1.tabpage_run.dw_wizard.RowCount())
					if not isnull(ll_find) and ll_find >0 then 
						ll_all_row = tab_1.tabpage_run.dw_wizard.GetItemNumber( ll_find, "selected" ) 
					end if 
				end if 	
//				IF tab_1.tabpage_run.dw_wizard.GetItemNumber( 1, "selected" ) = 1 THEN
				IF ll_all_row = 1 THEN
...
//				tab_1.tabpage_run.dw_wizard.Find("selected=1",1,tab_1.tabpage_run.dw_wizard.RowCount()) > 0 THEN  //if all is not selected and exists selected item
			...
			//end modify 07.11.2008
				...
				FOR i = 1 TO li_rc
					...
					IF tab_1.tabpage_run.dw_wizard.GetItemNumber( i, "selected" ) = 1 THEN
						...
						IF is_wizard_lookup_type[ ii_current_wizard_step ] = "C" THEN  //code lookup
							...
							END IF
							ls_field_values = ls_field_values + "'" + String(tab_1.tabpage_run.dw_wizard.GetItemNumber( i, "lookup_code" )) + "'," //Added by harry 20170508 for Task 750:Dynamic Filters in reports not working for multi-select fileds
						...
						ELSEIF  (is_wizard_lookup_type[ ii_current_wizard_step ] = "Y" or is_wizard_lookup_type[ ii_current_wizard_step ] = "B" or +&
							is_wizard_lookup_type[ ii_current_wizard_step ] = "U" or is_wizard_lookup_type[ ii_current_wizard_step ] = "O" ) THEN 			//facility			//Added By Mark Lee 06/07/2016
								ls_facility_name =  tab_1.tabpage_run.dw_wizard.GetItemString( i, "facility_name" )
								ls_facility_name =  of_replace(ls_facility_name,"'","''")
								is_facility_name = is_facility_name + ", " + ls_facility_name 	
								is_sql_where = is_sql_where + String(tab_1.tabpage_run.dw_wizard.GetItemNumber( i, "facility_id" )) + "' OR " 	
								//167:varchar,175:char
								//If column type is char or varchar,maybe it will return lookup code or description,
								//so the where clause should also use the code or description values.
								if is_wizard_coltype[ ii_current_wizard_step ] = '167' OR is_wizard_coltype[ ii_current_wizard_step ] = '175' OR &
									is_wizard_coltype[ ii_current_wizard_step ] = 'varchar' OR is_wizard_coltype[ ii_current_wizard_step ] = 'char' THEN
									is_sql_where = is_sql_where + is_wizard_field_name[ ii_current_wizard_step ] + " = '" + ls_facility_name + "' OR " 	
								END IF
								is_filter = is_filter + " " +ls_facility_name + ","			
								IF is_wizard_lookup_type[ ii_current_wizard_step ] = "U" THEN ls_field_values = ls_field_values + "'" + String(tab_1.tabpage_run.dw_wizard.GetItemNumber( i, "facility_id" )) + "'," //Added by harry 20170508 for Task 750
						END IF
					ELSE
				...
				//Added by harry 20170508 for Task 750:Dynamic Filters in reports not working for multi-select fileds
				IF of_multi_select_sql(is_wizard_field_name[ ii_current_wizard_step ], ls_field_values, ls_multi_sql) = 1 THEN
					is_sql_where = is_sql_where + ls_multi_sql + " OR "
				END IF
				
				//$<Modify> 02.22.2008 by Andy
				//$Reason:fixed a bug from Robert Goemans
				//is_filter = Mid( is_filter, 1, Len( is_filter ) -2 )
				is_filter = Mid( is_filter, 1, Len( is_filter ) -1 )
				//end of modify 02.22.2008
				is_address_types = Mid( is_address_types, 1, Len( is_address_types ) -3 ) + ")"
	
				if is_wizard_lookup_type[ ii_current_wizard_step ] = "F" or  is_wizard_lookup_type[ ii_current_wizard_step ] = "Y"  or  &
					is_wizard_lookup_type[ ii_current_wizard_step ] = "B" or  is_wizard_lookup_type[ ii_current_wizard_step ] = "U" or &
					is_wizard_lookup_type[ ii_current_wizard_step ] = "O" THEN //facility				//Modified By Mark Lee 06/13/2016 add Y U B O
			...
			ELSE 
				if is_wizard_lookup_type[ ii_current_wizard_step ] = "F"  or  is_wizard_lookup_type[ ii_current_wizard_step ] = "Y"  or  &
					is_wizard_lookup_type[ ii_current_wizard_step ] = "B" or  is_wizard_lookup_type[ ii_current_wizard_step ] = "U" or &
					is_wizard_lookup_type[ ii_current_wizard_step ] = "O" THEN //maha 101602		//Modified By Mark Lee 06/13/2016 add Y U B O
...
IF ii_current_wizard_step > ii_total_wizard_steps THEN 
	///*************run report********************
//	//Begin - Added By Mark Lee 03/19/2015.		check role filter data.			//Modified By Mark Lee 04/10/2015 It donot need.
//	of_setfilter_role_data( )
//	if trim(is_role_filter) <> "" then 
//		IF Right( is_role_filter, 5 ) = " AND " THEN
//			is_role_filter = Mid( is_role_filter, 1, Len( is_role_filter ) -5 )
//		END IF
//
//		IF Right( is_sql_where, 5 ) = " AND " or Right( is_sql_where, 7 ) = " WHERE " THEN 
//		else
//			is_sql_where = is_sql_where + " AND "
//		end if
//		is_sql_where = is_sql_where + is_role_filter
//	end if 
//	//End - Added By Mark Lee 03/19/2015.
...
ELSE //create next wizard
	...
	elseif is_wizard_lookup_type[ ii_current_wizard_step ] = "Y" or is_wizard_lookup_type[ ii_current_wizard_step ] = "B" or &
		is_wizard_lookup_type[ ii_current_wizard_step ] = "U" or is_wizard_lookup_type[ ii_current_wizard_step ] = "O" THEN		//Added By Mark Lee 06/07/2016
		tab_1.tabpage_run.dw_wizard.DataObject = "d_crit_company"				
	ELSE
	...
	elseif tab_1.tabpage_run.dw_wizard.DataObject = "d_crit_company" then			//Added By Mark Lee 06/07/2016
		if is_wizard_lookup_type[ ii_current_wizard_step ] = "B" or is_wizard_lookup_type[ ii_current_wizard_step ] = "O" then 			
			tab_1.tabpage_run.dw_wizard.Retrieve( is_wizard_lookup_type[ ii_current_wizard_step ])
		elseif is_wizard_lookup_type[ ii_current_wizard_step ] = "U" then 				
			tab_1.tabpage_run.dw_wizard.Retrieve("A")
		elseif is_wizard_lookup_type[ ii_current_wizard_step ] = "Y" then 		
			tab_1.tabpage_run.dw_wizard.Retrieve("All")
		else
			tab_1.tabpage_run.dw_wizard.Retrieve("All")
		end if 
	else
	...
	End If
	//Added By Mark Lee 03/17/2015 Restrict Dynamic Filter Options in Report Writer by Static Filter definition of the same Lookup in the same report.
	if ib_access_filter then
		of_restrict_dynamic_filter()
	end if 
...
of_checked_action_right('tabpage') 							//Added By Mark Lee 04/08/2015
...
FOR i = 1 TO li_rc
	//Added By Mark Lee 03/20/2015		add the lower() function
	ls_table_ids[i] = string(tab_1.tabpage_tables.dw_view_tables.GetItemString( i, "conv_view_table_table_name" ) )
END FOR
//--------------------------- APPEON BEGIN ---------------------------
//$<ID> PT-05
//$<modify> 02.08.2006 By: Rodger Wu
...
ic_newpage_group = tab_1.tabpage_browse.dw_detail.getitemstring(1,"newpage_group")	//jervis 08.09.2010
this.title = "Custom Reports ( " + ls_name + " )"  //maha 093004
...
of_checked_action_right( "modify")
...
String ls_ctx_id_table,ls_ctx_id_col //Added by Ken.Guo on 2009-02-23
String ls_table1_arr[],ls_table2_arr[],ls_Relate_arr[] //Added by Ken.Guo on 2009-02-23
boolean lb_right_ctx_table_exist = false //Added By Jay Chen 03-13-2015
string ls_temp_join_sql
...
If lds.RowCount() = 0 then
	...
	For ll_Cycle = 1 To ll_rc
		ls_table1_arr[ll_Cycle] = tab_1.tabpage_tables.dw_view_tables.GetItemString(ll_Cycle, "conv_view_table_table_name" ) //Added by Ken.Guo on 2009-02-23
		ls_from += "~t" + tab_1.tabpage_tables.dw_view_tables.GetItemString(ll_Cycle, "conv_view_table_table_name" ) + ", "  + "~r~n"
	Next
	
	ls_from = Left( ls_from, Len( ls_from ) - Len("~r~n") - 2) + " "
else
//---------------------------- APPEON END ----------------------------
	For ll_Row = 1 To lds.RowCount()
		...
		ls_Table1 = Left(ls_Col1, Pos(ls_Col1, ".") - 1)
		ls_Table2 = Left(ls_Col2, Pos(ls_Col2, ".") - 1)
		...
		//Added By Jay Chen 03-13-2015
		if Upper(ls_Table2) = Upper('v_ctx_basic_info') And ls_Relate = 'LEFT outer JOIN' then
			lb_right_ctx_table_exist = True
		end if
		//end
...
DO While UpperBound(lstr_Join[]) > 0
	If UpperBound(ls_TableList[]) = 0 Then
		...
		Next
//		ls_From += ls_TableList[1]+" "+lstr_Join[1].key+" "+ls_TableList[2]+" on ("+ls_Tmp+") " //Modified By Jay Chen 03-13-2015		//Added By Mark Lee 04/10/2015 add the ib_access_filter adjuage
		if  ib_access_filter and lb_right_ctx_table_exist and Upper(ls_TableList[2]) = Upper('v_ctx_basic_info') then 
			ls_From += ls_TableList[1]+" "+lstr_Join[1].key+" "+'('+ls_TableList[2] + of_build_ctx_access_sql('v_ctx_basic_info.ctx_id')+')'+" on ("+ls_Tmp+") "
		else
	...
	For ll_Pos = 1 To UpperBound(ls_TableList[])
		...
		For ll_Cnt = 1 To UpperBound(lstr_Join[])
			...
			If ls_TableList[ll_Pos] = lstr_Join[ll_Cnt].table1 Then
				...
				ll_pos1 = pos(lower(ls_From),lower(lstr_Join[ll_Cnt].key+" "+lstr_Join[ll_Cnt].table2+" on (")) 
				if ll_pos1 > 0 then
					ls_From = Left(ls_From,ll_pos1 + len(lstr_Join[ll_Cnt].key+" "+lstr_Join[ll_Cnt].table2+" on (") - 1) + ls_Tmp + " and " + Mid(ls_From,ll_pos1 + len(lstr_Join[ll_Cnt].key+" "+lstr_Join[ll_Cnt].table2+" on ("),len(ls_From))
				else
			...
			ElseIf ls_TableList[ll_Pos] = lstr_Join[ll_Cnt].table2 Then
				...
				ll_pos1 = pos(lower(ls_From),lower(lstr_Join[ll_Cnt].key+" "+lstr_Join[ll_Cnt].table1+" on (")) 
				if ll_pos1 > 0 then
					ls_From = Left(ls_From,ll_pos1 + len(lstr_Join[ll_Cnt].key+" "+lstr_Join[ll_Cnt].table1+" on (") - 1) + ls_Tmp + " and " + Mid(ls_From,ll_pos1 + len(lstr_Join[ll_Cnt].key+" "+lstr_Join[ll_Cnt].table1+" on ("),len(ls_From))
				else
	...
	For ll_Pos = 1 To UpperBound(ls_Temp[])
		IF Pos(ls_Deal, "/" + ls_Temp[ll_Pos] + "/") < 1 Then
...
//Added by Nova on 2008-07-28 begin
	//get field type
	//Added by Nova on 2008-08-06 begin
	ls_field_type = ''
	ls_c_name = tab_1.tabpage_columns.dw_view_fields.GetItemString( ll_Find, "table_field" )
	//Added By Mark Lee 03/20/2015		add the lower() function
	ll_Find_type = ids_fields.Find("lower(c_name)='" + lower(ls_c_name) + "' and lower(table_name)='" + lower(ls_table_name )+ "'",0,ids_fields.RowCount())
	...
	IF ls_lookup_type = "C" THEN
		...
		END IF
//	ELSE
	ELSEIF ls_lookup_type = "A" THEN		//Modified By Mark Lee 06/12/2016
		IF  ls_field_type = '106' Or ls_field_type = '108' Or ls_field_type = '60' Or ls_field_type = '122' THEN
	...
	ELSEIF ls_lookup_type = "Y" or  ls_lookup_type = "U"  or  ls_lookup_type = "B" or  ls_lookup_type = "O" THEN		//Added By Mark Lee 06/12/2016
		IF  ls_field_type = '106' Or ls_field_type = '108' Or ls_field_type = '60' Or ls_field_type = '122' THEN
			ls_from = ls_from + '~r~n' + "Left outer Join app_facility AS app_facility_" + ls_field_alias + " ON ( CONVERT ( VARCHAR, CONVERT  (decimal(18,0)," + ls_table_name + "." + ls_field_name + ") ) = CONVERT(VARCHAR,app_facility_" + ls_field_alias + ".facility_id" + " ))"
		ELSE
			ls_from = ls_from + '~r~n' + "Left outer Join app_facility AS app_facility_" + ls_field_alias + " ON ( CONVERT ( VARCHAR," + ls_table_name + "." + ls_field_name + ") = CONVERT(VARCHAR,app_facility_" + ls_field_alias + ".facility_id" + " ))"
		END IF
	END IF
	//Added by Nova on 2008-07-28 begin
...
Else	
	...
	If lds.RowCount() = 0 then
		...
		For ll_Num = 1 to UpperBound(ls_table1_arr[])
			ll_Find = lds_view_columns.Find('Upper(v_name) = Upper("' + ls_table1_arr[ll_Num] +'") And '+ 'Upper(c_name) = "CTX_ID"',1,lds_view_columns.RowCount())
			If ll_find > 0 Then
	...
	Else
		...
		If ls_ctx_id_col = '' Then
			...
			For ll_Num = 1 to UpperBound(ls_table2_arr[])
				...
				ll_Find = lds_view_columns.Find('Upper(v_name) = Upper("' + ls_table2_arr[ll_Num] +'") And '+ 'Upper(c_name) = "CTX_ID"',1,lds_view_columns.RowCount())
				If ll_find > 0 Then
...
String  ls_v_name,ls_c_name,ls_c_alias,ls_c_label
...
CHOOSE CASE ll_index
	CASE 2
		if tab_1.tabpage_tables.tv_tables.getItem(al_handle,ltvi_item) = 1 then
			...
			//Added By Mark Lee 03/20/2015		add the lower() function
			ll_Find = tab_1.tabpage_tables.dw_view_tables.Find( " lower(conv_view_table_table_name)='" + lower(ls_v_name) + "'", 0, ll_Cnt)
			...
			if ll_Cnt >= li_level then
				...
				//Added By Mark Lee 03/20/2015		add the lower() function
				ll_Find = ids_tables.Find("lower(v_name)='" + lower(ls_v_name) + "'",0,ids_tables.RowCount())
				if ll_Find > 0 then
	...
	CASE 3
		if tab_1.tabpage_columns.tv_fields.getItem(al_handle,ltvi_item) = 1 then
			...
			ls_c_label= String(ltvi_item.label)
			li_level  = ltvi_item.level
			...
			//Add 'Lower' convert - jervis 10/12/2011
			ll_Cnt  = tab_1.tabpage_columns.dw_view_fields.RowCount()
			ll_Find = tab_1.tabpage_columns.dw_view_fields.Find( "lower(table_name) = '" + lower(ls_v_name) + "' and lower(field_name)='" + lower(ls_c_name) + "'", 0, ll_Cnt)
			if ll_Find > 0 then return
			
			ll_Find = ids_fields.Find("lower(c_name)='" + lower(ls_c_name) + "' and lower(table_name)='" + lower(ls_v_name) + "'",0,ids_fields.RowCount())
			...
			//Begin - Added By Mark Lee 12/25/2015.
			if lower(ls_v_name) = 'v_ctx_basic_info' and (lower(ls_c_name)='dvision' or lower(ls_c_name)='dvision_code' or lower(ls_c_name)='dvision_description' ) then 
				ls_c_label = gnv_string.of_globalreplace(ls_c_label, 'Dvision', 'Division')		
			end if 
			//End - Added By Mark Lee 12/25/2015.

			li_nr = tab_1.tabpage_columns.dw_view_fields.InsertRow( 0 )
			ls_c_alias = of_build_alias(li_nr,ls_c_label) //Add by jervis 01.09.2010
			il_next_field_id++
			...
			tab_1.tabpage_columns.dw_view_fields.SetItem( li_nr, "field_alias", ls_c_alias )  //Modify by jervis 01.09.2010
			tab_1.tabpage_columns.dw_view_fields.SetItem( li_nr, "label_text", ls_c_label )
...
//			tab_1.tabpage_columns.dw_view_fields.SetItem( li_nr, "sel_table_id", ll_field_id )
			tab_1.tabpage_columns.dw_view_fields.object.sel_field_id[li_nr] = 	ll_table_id
			tab_1.tabpage_columns.dw_view_fields.object.sel_table_id[li_nr] = 	ll_field_id				
			...
			IF of_get_lookup_data(ls_v_name,ls_c_name,ls_Display,ls_lookup_type,ls_lookup_name) = 1 THEN
				//Added By Mark Lee 06/12/2016
//				IF lower(ls_Display) = 'code' then ls_Display = 'Code' else ls_Display = 'Description'
				if not isnull(ls_lookup_type) and ( upper(trim(ls_lookup_type)) = 'Y' or upper(trim(ls_lookup_type)) = 'U' &
					or upper(trim(ls_lookup_type)) = 'B' or upper(trim(ls_lookup_type)) = 'O'  ) then 
					//
				else
					IF lower(ls_Display) = 'code' then ls_Display = 'Code' else ls_Display = 'Description'
				end if 
				tab_1.tabpage_columns.dw_view_fields.SetItem( li_nr, "lu_field_name", ls_Display )
				tab_1.tabpage_columns.dw_view_fields.SetItem( li_nr, "lookup_type", ls_lookup_type )								
			END IF
			//end modify 03.28.2008
	...
	CASE 5
		if tab_1.tabpage_sort.tv_sort.getItem(al_handle,ltvi_item) = 1 then
			...
			ll_Find = tab_1.tabpage_sort.dw_view_sort.Find( "lower(table_name) = '" + lower(ls_v_name) + "' and lower(field_name)='" + lower(ls_c_name) + "'", 0, ll_Cnt)
			if ll_Find > 0 then return
...
CHOOSE CASE ll_index
	...
	CASE 5
		...
		//Added By Mark Lee 03/20/2015		add the lower() function
		ll_Find = ids_sort.Find("lower(c_name)='" + lower(ls_c_name) + "' and lower(table_name)='" + lower(ls_v_name) + "'",0,ids_sort.RowCount())
		if ll_Find > 0 then
...
String ls_dwsyntax,ls_screenID,ls_ERRORS, ls_Screen_style
Long   ll_Find
...
FOR i = 1 TO li_rc
	...
	IF Not IsNull( tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_wizard_name" ) ) and &
		tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_wizard_name" ) <> 'Date Range Comparison' and &
		tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "field_wizard_name" ) <> 'Single Date Comparison' THEN
	...
	IF Len( Trim(tab_1.tabpage_columns.dw_view_fields.GetItemString( i, "aggregate_type" )) ) > 1 THEN
		//li_total_agg = tab_1.tabpage_columns.dw_view_fields.GetItemNumber( i, "count_fld" ) + tab_1.tabpage_columns.dw_view_fields.GetItemNumber( i, "sum_fld" ) + tab_1.tabpage_columns.dw_view_fields.GetItemNumber( i, "avg_fld" ) 
		IF li_total_agg = 0 THEN
...
IF Not tab_1.tabpage_tables.cbx_advanced.checked THEN
	IF tab_1.tabpage_tables.dw_view_tables.RowCount() > 1 AND tab_1.tabpage_tables.dw_view_links.RowCount() = 0  THEN
		...
		IF  tab_1.tabpage_tables.dw_view_links.RowCount() = 0 OR tab_1.tabpage_tables.dw_view_tables.RowCount() - 1   > tab_1.tabpage_tables.dw_view_links.RowCount() THEN
			MessageBox( "Linking Error", "Report Writer is having trouble figuring out the appropriate Report Table links and needs your help.")
			tab_1.tabpage_tables.cbx_advanced.checked = True
			tab_1.tabpage_tables.cbx_advanced.Event clicked()
			RETURN -1
		END IF
...
//		if  pos(ls_def,"pd_affil_dept",1) > 0 then
	...
	if a = 0 then
		//$<modify> 04.03.2008 by Andy
		//messagebox("Table Select error","Department Security is turned on so the v_affil_dept must be used in all reports.  If you do not have this view contact ContractLogix support.")
		messagebox("Table Select error","Department Security is turned on so the Affil Department must be used in all reports.  If you do not have this view contact ContractLogix support.")
		//end modify 04.03.2008
		return -1
	end if
end if
...
start_pos = Pos(arg_str, arg_old, start_pos)
...
DO WHILE start_pos > 0
	...
	arg_str = Replace(arg_str, start_pos, Len(arg_old), arg_new)
	
	// Find the next occurrence of old_str.
	
	start_pos = Pos(arg_str, arg_old, start_pos+Len(arg_new))
...
long ll_i
string 	ls_face ,ls_strikeout ,ls_font_height, ls_err, ls_field_name
...
Long			ll_SepPos,ll_Pos,ll_RowStart,ll_RowEnd, ll_find
...
If ls_error <> '' Then
	MessageBox('','Create DataStore faile!')
	Return
End If
...
Do While ll_SepPos > 0
	...
	If adw_Preview.Describe(ls_ObjName + ".Name") <> ls_ObjName Then
		...
		ElseIf lds_dw.Describe( ls_ObjName + ".type") = 'text' Then
			If lds_dw.Describe( Left(ls_ObjName,Len(ls_ObjName) - 2) + ".type") = 'column' Then Continue
		End If
		
		//Igonre all object when name <> object - jervis 03.09.2010
		if left(ls_ObjName,6) <> 'object' then continue
		
		ls_Syntax = Mid(ls_dw_Syntax,LastPos(Left(ls_dw_Syntax,Pos(ls_dw_Syntax,"band=")),ENTERCHAR) + Len(ENTERCHAR))
		ls_Syntax = ENTERCHAR + ls_Syntax
		ll_Pos = Pos(ls_Syntax,"name=" + Lower(ls_ObjName) + " ")
		
		If ll_Pos > 0 Then
			ll_RowStart = LastPos(Left(ls_Syntax,ll_Pos),ENTERCHAR) + Len(ENTERCHAR)
			ll_RowEnd   = Pos(ls_Syntax,ENTERCHAR,ll_RowStart)
			ls_Modify   = 'Create ' + Mid(ls_Syntax,ll_RowStart,ll_RowEnd - ll_RowStart)
			ls_err = adw_Preview.Modify(ls_Modify)
	...
	Else
		...
		//END---Modify by Scofield on 2009-12-30
			...
			//Add font.face,strikeout - jervis 09.06.2011
			ls_face = lds_dw.Describe(ls_ObjName+".font.face")
			ls_strikeout = lds_dw.Describe(ls_ObjName+".font.strikethrough")
			ls_font_height = lds_dw.Describe(ls_ObjName+".font.height")
			...
			If Left(ls_bgcolor,1) = '"' And Right(ls_bgcolor,1) = '"' Then ls_bgcolor = Mid(ls_bgcolor,2,Len(ls_bgcolor) - 2)
			...
			//Added By Jay Chen 08-14-2015 remark: the column display is checked but the column syntax visible is 0
			long ll_visible
			ll_find = 0
			if ls_visible = '0' then
				ll_find = tab_1.tabpage_columns.dw_view_fields.find( " lower(field_alias)='"+lower(ls_ObjName)+"' ", 1, tab_1.tabpage_columns.dw_view_fields.rowcount())
				if ll_find > 0 then
					 ll_visible = tab_1.tabpage_columns.dw_view_fields.getitemnumber(ll_find, "field_visible")
					 if ll_visible = 1 then ls_visible = '1'
				end if
			end if
			//end
			...
			//Begin - Added By Mark Lee 01/23/2015. sync the field value.  
			if isvalid( tab_1.tabpage_columns.dw_view_fields) then
				if tab_1.tabpage_columns.dw_view_fields.rowcount() > 0 then 
					if ls_type= 'text'  and  lds_dw.Describe( Left(ls_ObjName,Len(ls_ObjName) - 2) + ".type") = 'column' then 
						ls_field_name = Left(ls_ObjName,Len(ls_ObjName) - 2)
						ll_find = tab_1.tabpage_columns.dw_view_fields.find( "lower(field_alias)='"+lower(ls_field_name)+"'", 1, tab_1.tabpage_columns.dw_view_fields.rowcount())
						if not isnull(ll_find) and ll_find  > 0 then 
							if lower(tab_1.tabpage_columns.dw_view_fields.GetItemString(ll_find,"label_text")) <> lower(ls_text) then 
								 tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"label_text", ls_text)
							End if
						end if 
					elseif (ls_type = 'column') then 
						if ls_visible = '0' then ls_x = '-1' //Added By Jay Chen 10-24-2015
						ls_field_name = ls_ObjName		
						ll_find = tab_1.tabpage_columns.dw_view_fields.find( "lower(field_alias)='"+lower(ls_field_name)+"'", 1, tab_1.tabpage_columns.dw_view_fields.rowcount())
						if not isnull(ll_find) and ll_find  > 0 then 
							if (tab_1.tabpage_columns.dw_view_fields.GetItemNumber(ll_find,"field_visible")) <> integer(ls_visible)  then 
								if ls_visible = '0' then 
									if  tab_1.tabpage_columns.dw_view_fields.GetItemNumber( ll_find,"field_x") > 0 then 
										tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"field_x", -1)
										tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"field_visible", 0)
									end if 
								else
									tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"field_visible", 1)
									if tab_1.tabpage_columns.dw_view_fields.GetItemNumber( ll_find,"field_x") <> integer(ls_x) then 
//										tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"field_x", ls_x)
										tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"field_x", long(ls_x)) //Added By Jay Chen 10-24-2015
									end if 
								end if 
							else
								if tab_1.tabpage_columns.dw_view_fields.GetItemNumber( ll_find,"field_x") <> integer(ls_x) then 
//									tab_1.tabpage_columns.dw_view_fields.SetItem( ll_find,"field_x", ls_x)
			...
			End if 
			//End - Added By Mark Lee 01/23/2015.
			...
			//Add font.face,strikeout - jervis 09.06.2011
			ls_Modify = ls_ObjName+".font.height='"+ls_font_height+"'"
			adw_Preview.Modify(ls_Modify)
			ls_Modify = ls_ObjName+".font.face='"+ls_face+"'"
			adw_Preview.Modify(ls_Modify)
			ls_Modify = ls_ObjName+".font.strikethrough='"+ls_strikeout+"'"
			adw_Preview.Modify(ls_Modify)
...
ls_Modify = 'Create text(band=header text="text" border="5"  '&
	...
	+' name=obj_temp  font.face="MS Sans Serif" font.height="-8"  )'
	
//Refresh create date - jervis 12.13.2010
ls_modify += ' title2_t.text = "Date: ' + string(today()) + '" '
adw_Preview.Modify(ls_Modify)
...
for ll_i = 1 to lds_bitmap.Rowcount( )
	lb_file = lb_filenull //Added By Jay Chen 12-18-2013 
...
//		select datalength(pic_bitmap) into :ll_datalen from conv_view_pic where pic_id = :ll_pic_id; //Modified By Jay Chen 12-18-2013
...
//				SELECTBLOB SubString(pic_bitmap,:ll_Start,8000) INTO :lb_Temp	from conv_view_pic where pic_id = :ll_pic_id; //Modified By Jay Chen 12-18-2013
				lb_file += lb_Temp
			next
		else
			if ls_pictype = 'system' then
...
//			selectblob pic_bitmap into :lb_file from conv_view_pic where pic_id = :ll_pic_id; //Modified By Jay Chen 12-18-2013
		end if
...
adw_Preview.height = adw_Preview.height + 4
//Added By Jay Chen 09-16-2013
tab_1.tabpage_run.dw_run.modify("datawindow.print.preview = 'yes'") //Modified By Ken.Guo 02/27/2014
//tab_1.tabpage_run.dw_run.object.datawindow.print.preview = 'yes'
tab_1.tabpage_run.dw_run.modify("datawindow.zoom="+string(100))
tab_1.tabpage_run.em_zoom.text = '100'
...
//PB9 upgrade to PB11.5, the convert between string and blob  Bug- jervis 09.29.2011
ls_dwsyntax = string(lb_all_data)
if pos(ls_dwsyntax,'release') = 0 then ls_dwsyntax = string(lb_all_data,EncodingANSI!)
return ls_dwsyntax
...
"            	coltype = (CASE ctx_acp_contract_management_right.types~r~n" + &
"            				    WHEN '1' THEN 'category' ~r~n" + &
"            				    WHEN '6' THEN 'division'~r~n" + &
"            				    WHEN '7' THEN 'department'~r~n" + &
"            				    WHEN '8' THEN 'contract_type'END),~r~n" + &
"            	access = (CASE WHEN ctx_acp_contract_management_right.rights IN('11','61','71','81') THEN 'F' ~r~n" + &
"            			   	   WHEN ctx_acp_contract_management_right.rights IN('12','62','72','82') THEN 'R' ~r~n" + &
"            	     		   ELSE 'N' END) ~r~n" + &
...
"      		AND ctx_acp_contract_management_right.rights In ('11','12','13')) OR ~r~n" + &
"      		(ctx_acp_contract_management_right.types=6 ~r~n" + &
"      		AND ctx_acp_contract_management_right.category = ctx_basic_info.dvision  ~r~n" + &
"      		AND ctx_acp_contract_management_right.rights In ('61','62','63')) OR  ~r~n" + &
"      		(ctx_acp_contract_management_right.types=7 ~r~n" + &
"      		AND ctx_acp_contract_management_right.category = ctx_basic_info.contract_department ~r~n" + &
"      		AND ctx_acp_contract_management_right.rights In ('71','72','73')) OR ~r~n" + &
"      		(ctx_acp_contract_management_right.types=8 ~r~n" + &
...
public function string of_get_lookupdesc_multi (string as_lookup_code);Long 		ll_Cycle, ll_cnt
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Scofield		Date: 2009-06-03
//--------------------------------------------------------------------
//	Copyright (c) 1999-2009 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
for ll_Cycle = 1 to ll_ColCnts
	...
	if ii_with_data = 0 then
		...
		ls_Modify  = 'create text(band=' + ls_Band + ' alignment="' + ls_Align + '" text="' + ls_ColName + '" border="0" color="' + ls_Color + '"' &
						 ...
						 + '" font.face="MS Sans Serif" font.height="-8" font.weight="' + ls_Weight &
...
public function string of_build_alias (long al_row, string as_label);//Add by jervis -- 11.20.2009
...
//Column name Repeat 
For li_i = 1 To tab_1.tabpage_columns.dw_view_fields.rowcount() 
	if li_i = al_row then continue
	ls_other_alias = tab_1.tabpage_columns.dw_view_fields.getitemstring(li_i,'field_alias')
...
public function integer of_checkselectcontent (string as_content, string as_codename, string as_operator);//check  return select content  if multi  it is in   , one is equal 
//
Integer			li_Pos = 0
...
If ll_rowcount > 0 Then
	FOR i = 1 TO ll_rowcount
		...
		If lbn_notice = True and  lbn_retrieve = False Then
			...
			li_cnt = MessageBox("ContractLogix",ls_msg, Question!, YesNo!)			
			If li_cnt = 1 THen 
...
public function integer of_run_report (long al_report_id, string as_output_file, ref string as_error);u_dw ldw_report_list
...
If ll_row <= 0 Then
	as_error = 'Auto Report Error -1. The report is not exists. report_id = ' + String(al_report_id) + '.'
...
If ll_cnt > 0 Then
	as_error = 'Auto Report Error -2. The report has Filter By field(s), it is not support. report_id = ' + String(al_report_id) + ', report_name = '+ls_report_name+'.'
	gnv_debug.of_output(True, as_error)
	Return -2
End If
...
public function decimal of_get_original_size (string size_type, string size_orientation);//====================================================================
// Function: of_get_original_size
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    string    size_type
// 	value    string    size_orientation
//--------------------------------------------------------------------
// Returns:  decimal
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 09-16-2013
//--------------------------------------------------------------------
...
string ls_papertype,ls_orientation
dec{2} ld_width
/* remark 1in = 2.54cm
    0 = "Default"
...
ls_papertype = size_type
ls_orientation = size_orientation

choose case ls_papertype
	case '0'
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 03/09/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
//
//====================================================================
string 		ls_dw_Syntax, ls_report_name, ls_error, ls_text, ls_tmp, ls_dw_select
DataStore 	lds_dw
Blob			lb_Syntax
...
Into :ls_report_name
From conv_view 
Where view_id = :al_view_id  Using sqlca;

ls_dw_Syntax = of_GetSyntax(al_view_id)

lds_dw = Create DataStore
lds_dw.Create(ls_dw_Syntax,ls_error)

If ls_error <> '' Then
	MessageBox('Check Report Name Error','Create DataStore faile!')
	Return
End If

ls_text = lds_dw.Describe("title1_t.text")
if not isnull(ls_text) and Upper(left(ls_text,8)) = Upper("Report: ")  then 
	ls_tmp = mid(ls_text, 9)
	if isnull(ls_tmp) or upper(ls_tmp) <> upper(ls_report_name ) then 
		ls_error = lds_dw.Modify("title1_t.text='Report: "+ls_report_name+"' ")
		ls_dw_Syntax = lds_dw.Describe("DataWindow.Syntax")
		ls_dw_select = lds_dw.Describe("DataWindow.Table.Select")	
		lb_Syntax = Blob(ls_dw_Syntax)		
		
		gnv_appeondb.of_startqueue( )
		UPDATE ctx_report_syntax 
		SET dw_sql = :ls_dw_Syntax,dw_select=:ls_dw_select 
		WHERE view_id = :al_view_id;
		
		UPDATEBLOB ctx_report_syntax SET dw_syntax = :lb_Syntax 
		WHERE view_id=:al_view_id;
		gnv_appeondb.of_commitqueue( )
		Commit;
	end if 
...
public subroutine of_restrict_dynamic_filter ();//====================================================================
// Function: of_restrict_dynamic_filter
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 03/17/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
// Restrict Dynamic Filter Options in Report Writer by Static Filter definition of the same Lookup in the same report.
//====================================================================
long				i, j,  ll_count, ll_more, ll_less, ll_lookup_code, ll_cnt, ll_find, ll_add[], ll_reduce[], ll_role_arr[]
string				ls_col, ls_more[], ls_less[], ls_operator, ls_value, ls_value_display, ls_arr[], ls_code, ls_dataobject
string 			ls_sql, ls_role_arr[], ls_lookup_code

ll_count = tab_1.tabpage_filter.dw_view_where.rowcount( )
if tab_1.tabpage_run.dw_wizard.rowcount( ) > 0  then
	if ll_count >0 then
//is_wizard_lookup_name[ ii_current_wizard_step ]							Contract Category
//is_wizard_field_name[ ii_current_wizard_step ]								v_ctx_basic_info.category
//is_wizard_compare_field[ ii_current_wizard_step ]							C
//is_screen_lookup_type[ii_current_wizard_step]								C
//is_wizard_coltype[ii_current_wizard_step]										108
//is_wizard_field_label[ii_current_wizard_step]									Category

		ls_dataobject = tab_1.tabpage_run.dw_wizard.dataobject
		if  ls_dataobject = 'd_crit_custom' or   ls_dataobject = 'd_crit_net_dev_type_ir' or &
				ls_dataobject = 'd_crit_custom_address' then 
			For i = 1 to ll_count 
				ls_col  = tab_1.tabpage_filter.dw_view_where.getitemString(i,'table_field_name')
				ls_operator = tab_1.tabpage_filter.dw_view_where.getitemString(i,'operator')
				ls_value_display = tab_1.tabpage_filter.dw_view_where.getitemString(i,'value_display')
				ls_value = tab_1.tabpage_filter.dw_view_where.getitemString(i,'value')		
				ls_value = gnv_string.of_globalreplace( ls_value, "'", "")					//// replace '17959'
				if lower(ls_col) = lower(is_wizard_field_name[ ii_current_wizard_step ]	) then
					//	operator 	= \ 	> \ < \ In \ Is \ Is Not \ <> \ Like 
					if lower(ls_operator) = lower('=') then 
						ll_more ++
//						ls_more[ll_more] = ls_value
						ll_add[ll_more] = long(ls_value)
					elseif lower(ls_operator) = lower('<>') then 
						ll_less++
//						ls_less[ll_less] = ls_value
						ll_reduce[ll_less] = long(ls_value)
					elseif lower(ls_operator) = lower('In') then 	
						if pos(ls_value,',') > 0 then
							gnv_string.of_parsetoarray(ls_value, ',', ls_arr)
							for j = 1 to Upperbound(ls_arr)
								ll_more ++
//								ls_more[ll_more] = trim(ls_arr[j])
								ll_add[ll_more] =long(trim(ls_arr[j]))
							next
						else
							ll_more ++
//							ls_more[ll_more] = ls_value
			...
			//exists
//			if upperbound(ls_more) > 0 then
			if upperbound(ll_add) > 0 then
				ll_cnt = tab_1.tabpage_run.dw_wizard.rowcount( )
				if ll_cnt > 0 then
					for j = ll_cnt to 1 step -1
						ll_lookup_code =  tab_1.tabpage_run.dw_wizard.GetItemnumber( j, 'lookup_code')
//						if gnv_string.of_isexists( ls_more,string(ll_lookup_code)) = false then		
						if gnv_string.of_isexists( ll_add,ll_lookup_code) = false then		
							ls_code = ''
							IF ls_dataobject <> "d_crit_date_range_custom" AND ls_dataobject <> "d_crit_single_date_custom" AND &
								ls_dataobject <> "d_crit_facility" AND ls_dataobject <> "d_crit_address_type_ir" AND &
								ls_dataobject <> "d_crit_net_dev_type_ir" AND ls_dataobject <> "d_crit_app_payor" and ls_dataobject <> "d_crit_company" THEN
								ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( j, 'code')							
							ELSEIF ls_dataobject = "d_crit_facility" or  ls_dataobject = "d_crit_company"  THEN		//Added By Mark Lee 06/07/2016 add d_crit_company 
								ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( j, 'facility_name')
							ELSEIF ls_dataobject = "d_crit_net_dev_type_ir"  THEN
								ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( j, 'code')
							ELSEIF ls_dataobject= "d_crit_app_payor"  THEN
								ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( j, 'application_name')
							ELSEIF ls_dataobject = "d_crit_date_range_custom"	then  
								//
							ELSEIF ls_dataobject = "d_crit_single_date_custom" then
								//
							END IF
							if lower(ls_code) = lower("All") then continue
							tab_1.tabpage_run.dw_wizard.deleterow( j)
						end if 
					next 
				end if 
			end if 
			...
			//not exists
//			if upperbound(ls_less) > 0 then
			if upperbound(ll_reduce) > 0 then
				ll_cnt = tab_1.tabpage_run.dw_wizard.rowcount( )			
				if ll_cnt > 0 then
					for j = ll_cnt to 1 step -1
						ll_lookup_code =  tab_1.tabpage_run.dw_wizard.GetItemnumber( j, 'lookup_code')
//						if gnv_string.of_isexists( ls_less,string(ll_lookup_code)) = true then
						if gnv_string.of_isexists( ll_reduce,ll_lookup_code) = true then
							tab_1.tabpage_run.dw_wizard.deleterow( j)
						end if
					next 
				end if 
	...
	//accroding to the role settings
	ls_sql = ""
	of_filter_role_data( lower(is_wizard_field_name[ ii_current_wizard_step ]), ls_sql,ll_role_arr) 
	if upperbound(ll_role_arr) > 0 then
		ll_cnt = tab_1.tabpage_run.dw_wizard.rowcount( )			
		if ll_cnt > 0 then
			for j = ll_cnt to 1 step -1
				ll_lookup_code =  tab_1.tabpage_run.dw_wizard.GetItemnumber( j, 'lookup_code')
//				if gnv_string.of_isexists( ls_role_arr,string(ll_lookup_code)) = true then
				if gnv_string.of_isexists( ll_role_arr,ll_lookup_code) = true then					
					tab_1.tabpage_run.dw_wizard.deleterow( j)
				end if
			next 
		end if 
	...
	if ll_cnt =1 then 
		...
		IF ls_dataobject <> "d_crit_date_range_custom" AND ls_dataobject <> "d_crit_single_date_custom" AND &
			ls_dataobject <> "d_crit_facility" AND ls_dataobject <> "d_crit_address_type_ir" AND &
			ls_dataobject <> "d_crit_net_dev_type_ir" AND ls_dataobject <> "d_crit_app_payor" and ls_dataobject <> "d_crit_company" THEN
			ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( ll_cnt, 'code')							
		ELSEIF ls_dataobject = "d_crit_facility" or  ls_dataobject = "d_crit_company"  THEN			//Added By Mark Lee 06/07/2016 add d_crit_company
			ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( ll_cnt, 'facility_name')
		ELSEIF ls_dataobject = "d_crit_net_dev_type_ir"  THEN
			ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( ll_cnt, 'code')
		ELSEIF ls_dataobject= "d_crit_app_payor"  THEN
			ls_code = tab_1.tabpage_run.dw_wizard.getitemstring( ll_cnt, 'application_name')
		ELSEIF ls_dataobject = "d_crit_date_range_custom"	then  
			//
		ELSEIF ls_dataobject = "d_crit_single_date_custom" then
			//
		END IF
		if lower(ls_code) = lower("All") then 
			tab_1.tabpage_run.dw_wizard.deleterow( ll_cnt)	
		end if 
	end if 
...
public subroutine of_setfilter_role_data ();//====================================================================
// Function: of_setfilter_role_data
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 03/19/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
//
//====================================================================
string 			ls_sql, ls_arr[], ls_tmp
long				ll_rold_id, ll_count, ll_val, arr[]

ls_sql = " "
ll_count = 0

ll_rold_id = integer(gnv_data.of_getItem("security_users", "role_id", " lower(user_id) ='"+lower(gs_user_id)+"' ") 	)
if not isvalid(ids_role_filter) then 
	ids_role_filter = create n_ds
	ids_role_filter.dataobject  = 'ds_ctx_acp_contract_management_right_report'
	ids_role_filter.settransobject( sqlca)
	ll_count = ids_role_filter.retrieve(ll_rold_id )
else
	ll_count = ids_role_filter.rowcount( )
	if ll_count > 0 then   // dynamic change the role id
		ll_val = ids_role_filter.GetItemNumber(1,'role_id')
		if isnull(ll_val) or ll_val <> ll_rold_id then
			ll_count = ids_role_filter.retrieve(ll_rold_id )
		end if 
	end if 
end if
...
if ll_count > 0 then 
	//  'v_ctx_basic_info.category' 		1
	of_filter_role_data( 'v_ctx_basic_info.category', ls_sql, arr)		
	//  'v_ctx_basic_info.status' 	5
	of_filter_role_data( 'v_ctx_basic_info.status', ls_sql, arr)			
	//'v_ctx_basic_info.dvision' 	6
	of_filter_role_data( 'v_ctx_basic_info.dvision', ls_sql, arr)				
	//'v_ctx_basic_info.contract_type' 	8
	of_filter_role_data( 'v_ctx_basic_info.contract_type', ls_sql, arr)				
	//'v_ctx_basic_info.contract_department ' 	7
	of_filter_role_data( 'v_ctx_basic_info.contract_department', ls_sql, arr)	
end if 
	

end subroutine

public subroutine of_destory_ds ();if IsValid(ids_role_filter) then destroy ids_role_filter		//Added By Mark Lee 03/19/2015 refresh
end subroutine

public subroutine of_filter_role_data (string as_category, ref string as_return, ref long al_arr[]);//====================================================================
// Function: of_filter_role_data
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value        string    as_category
// 	reference    string    as_return
// 	reference    long      al_arr[]
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 03/24/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
string 			ls_field_name, ls_rights, ls_sql, ls_arr[], ls_tmp, ls_setfilter, ls_setfind
long				ll_rold_id, i, ll_count, ll_val, ll_find, ll_cnt, arr[]
string 			null_str
...
ls_sql = " "
ll_cnt = 0 
if isnull(as_category) or trim(as_category) = "" then return 

choose case as_category
	case 'v_ctx_basic_info.category'
		ls_setfind = " lower(table_name)='v_ctx_basic_info' and lower(field_name) = 'category' "
		ls_setfilter =  " types = 1 and  rights = '13' "
	case 'v_ctx_basic_info.status'
		ls_setfind = " lower(table_name)='v_ctx_basic_info' and lower(field_name) = 'status' "
		ls_setfilter =  " types = 5 and  rights = '53' "		
	case 'v_ctx_basic_info.dvision'
		ls_setfind = " lower(table_name)='v_ctx_basic_info' and lower(field_name) = 'dvision' "		
		ls_setfilter =  " types = 6 and  rights = '63' "
	case 'v_ctx_basic_info.contract_type'
		ls_setfind = " lower(table_name)='v_ctx_basic_info' and lower(field_name) = 'contract_type' "		
		ls_setfilter =  " types = 8 and  rights = '83' "		
	case 'v_ctx_basic_info.contract_department'		
		ls_setfind = " lower(table_name)='v_ctx_basic_info' and lower(field_name) = 'contract_department' "		
		ls_setfilter =  " types = 7 and  rights = '73' "			
	case else
		return
end choose 
...
if tab_1.tabpage_columns.dw_view_fields.rowcount( ) >0 then 
	if not isvalid(ids_role_filter) then 
		...
		ids_role_filter.dataobject  = 'ds_ctx_acp_contract_management_right_report'
		ids_role_filter.settransobject( sqlca)
		ids_role_filter.retrieve(ll_rold_id )
	end if 
	
	ll_find = tab_1.tabpage_columns.dw_view_fields.find(ls_setfind , 1, tab_1.tabpage_columns.dw_view_fields.rowcount( ))
	if ll_find > 0 then 
		SetNull(null_str)
//		ids_role_filter.SetFilter(null_str)
		ids_role_filter.SetFilter(" ")
		ids_role_filter.Filter()
		ids_role_filter.GroupCalc()
		ids_role_filter.setfilter(ls_setfilter)
		ids_role_filter.filter( )
		ids_role_filter.GroupCalc()		
		ll_count  = ids_role_filter.rowcount( )
		if ll_count > 0 then 
			for i = 1 to ll_count
				ll_cnt ++ 
				ll_val = ids_role_filter.GetItemNumber(i, 'category')			// all column 
				al_arr[ll_cnt] = long(ll_val)
			next 
			if upperbound(al_arr) > 0 then
				gnv_string.of_arraytostring( al_arr, ' , ', ls_tmp)
				as_return =  " "+as_category+" not in ("+ls_tmp+") "
//				is_role_filter = is_role_filter +  " "+as_category+" not in ("+ls_tmp+")  AND "	
			end if 
		end if  
	end if 
end if 
...
public subroutine of_checked_action_right (string as_action);if isnull(as_action) then return 

choose case as_action
	case 'modify'
		if w_mdi.of_security_access( 7013 ) = 0 or w_mdi.of_security_access( 1470 ) <> 2  then //Modified By Ken.Guo 2015-09-18. added check 1470.
			tab_1.tabpage_browse.cb_delete.enabled = false			
			tab_1.tabpage_browse.cb_new.enabled = false
			tab_1.tabpage_browse.cb_copy.enabled = false			
			tab_1.tabpage_browse.cb_25.enabled = false						//Auto Report	
			tab_1.tabpage_preview.cb_14.enabled = false
			tab_1.tabpage_preview.cb_23.enabled = false
			
			//Added By Ken.Guo 2015-09-18. for readonly 
			tab_1.tabpage_browse.cb_properties.enabled = False
			tab_1.tabpage_browse.cbx_goto.checked = True
			tab_1.tabpage_browse.cbx_goto.enabled = False			
			tab_1.tabpage_columns.Enabled = false
			tab_1.tabpage_filter.Enabled = false
			tab_1.tabpage_sort.Enabled = false
			tab_1.tabpage_tables.Enabled = false
	...
	case 'output'
		if w_mdi.of_security_access( 7014 ) = 0 then
			tab_1.tabpage_run.cb_saveas.enabled = false
			tab_1.tabpage_run.cb_print.enabled = false		
		end if
	case 'tabpage'
		if w_mdi.of_security_access( 7013 ) = 0 or w_mdi.of_security_access( 1470 ) <> 2 then //Modified By Ken.Guo 2015-09-18. added check 1470.
			tab_1.tabpage_columns.Enabled = false
...
public subroutine of_generate_lookup_column (string as_type);string 			ls_default, ls_default1, ls_company, ls_err, ls_field_name1, ls_field_disply1
long				i,	ll_find
n_ds 				lds_cc_custom_fields_falg


ls_default	= " 	 /Code	C/Description	D/"
ls_default1	= " 	 /Code	Code/Description	Description/"
ls_company =  " 	 /Facility Id	facility_id/Company Name	facility_name/Company Category	contact_type2/Status	status/Street 1	street/" +&
					"Street 2	street_2/Street 3	street_3/City	city/State	state/Zip	zip/Country	country/Phone	phone/"+&
					"Ext	ext/Fax	fax/Corporate Office	corp_office_flag/ID #	contact/Corp Office Link	corp_office_link/Web Address	web_address/"+&
					"Email Address	email_address/"

IF tab_1.tabpage_columns.dw_view_fields.RowCount() = 0 OR tab_1.tabpage_columns.dw_view_fields_edit.rowcount( ) = 0 THEN
	Return 
END IF
	
if not isnull(as_type) and (as_type = 'Y' OR as_type = 'U' OR as_type = 'B' OR as_type = 'O') then
	...
	//		ls_field_disply1 = ls_field_disply1
		else
			ls_field_disply1 =  "Custom "+string(i)				
		end if 
		ls_company =ls_company+ ls_field_disply1+"~t"+ls_field_name1+"/"
	Next 
	destroy lds_cc_custom_fields_falg	

//	ls_err = tab_1.tabpage_columns.dw_view_fields.modify("field_options.values='"+ls_company+"'")
	ls_err= tab_1.tabpage_columns.dw_view_fields_edit.modify("field_options.values='"+ls_company+"' lu_field_name.values='"+ls_company+"'")
	// set all data in filldata
	ls_err = tab_1.tabpage_columns.dw_view_fields.modify("field_options.values='"+ls_default+ls_company+"'")
else
//	ls_err =tab_1.tabpage_columns.dw_view_fields.modify("field_options.values='"+ls_default+"'")
	ls_err= tab_1.tabpage_columns.dw_view_fields_edit.modify("field_options.values='"+ls_default+"' lu_field_name.values='"+ls_default1+"'")
end if 
...
if lb_Flag then
	...
	FOR ll_j = 1 to ll_dsCnt
		...
		//Added By Mark Lee 06/08/2016			
		if Lower(ls_DddwName) = "dddw_contact_company_new" then
			as_Display = istr_ds[ll_i].lds[ll_j].describe(as_c_name + ".dddw.DisplayColumn")
			as_lookup_type = "Y"		 
			as_lookup_name = istr_ds[ll_i].lds[ll_j].describe(as_c_name + ".tag")
			if isnull(as_lookup_name) or trim(as_lookup_name) = "" or as_lookup_name = "?" or as_lookup_name = "!" then as_lookup_name = "COMPANY NAME" 
			Exit
		elseif Lower(ls_DddwName) = "d_dddw_company" or Lower(ls_DddwName) = "d_dddw_company_list" then
			as_Display = istr_ds[ll_i].lds[ll_j].describe(as_c_name + ".dddw.DisplayColumn")
			as_lookup_name = istr_ds[ll_i].lds[ll_j].describe(as_c_name + ".tag")
			if isnull(as_lookup_name) or trim(as_lookup_name)='' or as_lookup_name = '!' or as_lookup_name = '?' or lower(as_lookup_name) = 'all' then 
				as_lookup_type = "Y"		 
				as_lookup_name = "COMPANY NAME"
			elseif lower(as_lookup_name)= lower('A') then
				as_lookup_type = "U"		 
				as_lookup_name = "OUR COMPANY"
			elseif lower(as_lookup_name)= lower('B') then
				as_lookup_type = upper(as_lookup_name)			
				as_lookup_name = "CONTRACTED COMPANY"
			elseif lower(as_lookup_name)= lower('O') then
				as_lookup_type = upper(as_lookup_name)			
				as_lookup_name = "OTHER COMPANY"				
			end if 
			Exit
		end if					
	NEXT				
	if il_module = 2 and IsNumber(istr_ds[ll_i].ls_screen) then
		...
		//Added By Mark Lee 03/20/2015		add the lower() function
		ll_Find = inv_data_entry.ids_screen_fields.find( "lower(db_field_name)='" + lower(as_c_name) + "'", 0, ll_fCnt)
		if ll_Find > 0 then
...
public function integer of_multi_select_sql (string as_field_name, string as_values, ref string as_multi_sql);//Added by harry 20170508 for Task 750:Dynamic Filters in reports not working for multi-select fileds
String ls_table_name
String ls_field_name
String ls_screen_id
String ls_values
...
If Len(as_values) > 0 Then
	ls_values = Left(as_values, Len(as_values) - 1)
Else
	Return 0
End If

If Len(as_field_name) <= 0 Then
	Return 0
Else
	ll_pos = Pos(as_field_name, '.')
	If ll_pos > 0 Then
		ls_table_name = Mid(as_field_name, 1, ll_pos - 1)
		ls_field_name = Mid (as_field_name, ll_pos + 1, Len(as_field_name) - ll_pos)
	End If
End If

If ls_table_name <> 'v_ctx_custom' and  ls_table_name <> 'v_ctx_custom2' Then
	Return 0
Else
	If ls_table_name = 'v_ctx_custom' Then 
		ls_screen_id = '9'
	ElseIf ls_table_name = 'v_ctx_custom2' Then 
		ls_screen_id = '10'
	End If
End If
...
INTO :ll_count
FROM screen_field_select_item_attribute 
WHERE screen_id = :ls_screen_id
AND object_name = :ls_field_name
AND object_type = 'multi-select'
AND object_format = 'Y';

If ll_count <= 0 Then Return 0

as_multi_sql = " exists ( select 1 from screen_field_select_item x where x.ctx_id = " + ls_table_name + ".ctx_id and x.field_name = '" + ls_field_name + "' and x.screen_id = " + ls_screen_id + " and x.field_data in (" + ls_values + ") ) "

Return 1
end function

public function string of_get_company_name (string as_company_id);//====================================================================
// Function: of_get_company_name
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
gnv_appeonDb.of_StartQueue()
tab_1.tabpage_browse.dw_report_list.Retrieve(gs_user_id) //Added By Ken.Guo 2015-08-05. Add role filter.
//comment 12.11.2007 by: Andy
...
//Added By Mark Lee 04/07/2015
of_checked_action_right( "modify")
of_checked_action_right("output")
end event
...
if IsValid(ids_role_filter) then destroy ids_role_filter		//Added By Mark Lee 03/19/2015
...
event selectionchanging;//Added By Alan on 2009-2-11
int		i,li_rc,li_cnt
long		ll_Rtn
String	ls_Syntax
long ll_find,ll_sort
long ll_rowcount
long ll_newrow
long ll_Max_Order
string ls_v_name
...
ii_prior_step = oldindex 	//jervis 01.26.2010
...
ELSEIF oldindex = 3 THEN
	...
	//Auto Set Sort - Modify by jervis 10.15.2009
	//$<add> 04.15.2008 by Andy Reason:Optimize speed.
	//of_save( )
	//tab_1.tabpage_columns.dw_view_fields.Update()
	//end 04.15.2008
	ll_rowcount = tab_1.tabpage_columns.dw_view_fields.Rowcount( )
	ll_find = tab_1.tabpage_columns.dw_view_fields.Find( 'group_by_field = 1',1,ll_rowcount)
	SELECT Max(conv_view_sort.sort_order)
			  INTO :ll_Max_Order
			  FROM conv_view_sort  
			 WHERE conv_view_sort.view_id = :il_view_id ;
	...
	do while ll_find > 0
		...
		//Auto insert sort
		ls_v_name = tab_1.tabpage_columns.dw_view_fields.GetItemString( ll_find, 'table_name')
		ls_c_name = tab_1.tabpage_columns.dw_view_fields.GetItemString( ll_find, 'field_name')
		ll_Max_Order++
			//Added By Mark Lee 03/20/2015		add the lower() function
		ll_sort = tab_1.tabpage_sort.dw_view_sort.Find( " lower(table_name) = '" + lower(ls_v_name) + "' and lower(field_name)='" + lower(ls_c_name) + "'", 0, tab_1.tabpage_sort.dw_view_sort.RowCount()) 
		if ll_sort = 0 then 
			ll_newrow = tab_1.tabpage_sort.dw_view_sort.Insertrow( 1 ) //insert to frist row - jervis 11.02.2010
	
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "project_id", ii_project_id )
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "view_id", il_view_id )
		
			il_next_sort_id ++
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "sort_id", il_next_sort_id )
			
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "field_name", ls_c_name )
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "table_name", ls_v_name )
			//$<add> 03.31.2008 by Andy
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "dbname", ls_v_name + '.' + ls_c_name )
			//end add 03.31.2008
			
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "sort_dir", "A" )
			tab_1.tabpage_sort.dw_view_sort.SetItem( ll_newrow, "sort_order",ll_Max_Order)
		elseif ll_sort > 0 then
			//tab_1.tabpage_sort.dw_view_sort.SetItem( ll_sort, "sort_order",ll_Max_Order)
		end if

      // Prevent endless loop
		ll_find++
      IF ll_find > ll_rowcount THEN EXIT
		ll_find = tab_1.tabpage_columns.dw_view_fields.Find( 'group_by_field = 1',ll_find,ll_rowcount)
	loop
	tab_1.tabpage_columns.dw_view_fields.Update()
	tab_1.tabpage_sort.dw_view_sort.AcceptText()
	tab_1.tabpage_sort.dw_view_sort.Update( )	
...
ELSEIF oldindex = 4 THEN
	if of_check_filter() = 1 then
		tab_1.tabpage_filter.dw_view_where.update( )
	else //Don't allow move to next tab -- jervis 11.10.2010
		return 1 
	end if
ELSEIF oldindex = 5 THEN
	tab_1.tabpage_sort.dw_view_sort.update( )
ELSEIF oldindex = 6 THEN
	Tabpage_Preview.dw_label.setfocus( )
	tabpage_browse.dw_detail.update( )
	tabpage_columns.dw_view_fields.update( )
	
	ls_Syntax = Tabpage_Preview.dw_Preview.Describe("DataWindow.Syntax")
	if ls_Syntax <> is_Origin_Syntax then
		ll_Rtn = MessageBox(gnv_app.iapp_object.DisplayName,'Do you want to save changes?',Question!,YesNoCancel!)
		if ll_Rtn = 1 then
			Tabpage_Preview.cb_14.TriggerEvent(Clicked!)
		elseif ll_Rtn = 3 then
			Return 1
		end if
	end if
...
ELSEIF newindex = 6 THEN
	...
	ll_Rtn = of_preview( tab_1.Tabpage_Preview.dw_preview )
	if ll_Rtn < 0 then
...
ELSEIF newindex = 7 THEN
	...
	//---------------------------- APPEON END ----------------------------
	//Added By chenjian 2013-09-13
	if not tab_1.tabpage_run.dw_run.bringtotop then
		tab_1.tabpage_run.dw_run.object.datawindow.print.preview = 'yes'
	//    tab_1.tabpage_run.dw_run.object.datawindow.print.preview.rulers='Yes'
    		tab_1.tabpage_run.dw_run.modify("datawindow.zoom="+string(100))
    		tab_1.tabpage_run.em_zoom.text = '100'
	//    tab_1.tabpage_run.cb_suit.triggerevent(clicked!)
    end if
	 //Added By Jay Chen 11-27-2014
	 if tab_1.tabpage_run.dw_run.describe("datawindow.print.preview") = 'yes' then
		tab_1.tabpage_run.cbx_1.checked = true
	end if
END IF
...
CHOOSE CASE newindex
	...
	CASE 3,5
		...
		FOR ll_N = 1 to ll_Cnt
			//Added By Mark Lee 03/20/2015		add the lower() function
			ls_viewList[ll_N] = lower(tab_1.tabpage_tables.dw_view_tables.GetItemString(ll_N,"conv_view_table_table_name"))
		NEXT
		if newindex = 3 then
...
event clicked;
IF IsValid(w_auto_report) THEN 				//Added By Mark Lee 09/07/2015
	if w_auto_report.WindowState =Minimized! then 
		 w_auto_report.WindowState=Normal!
	else 
		w_auto_report.show()
	end if 
else
...
//ll_view_id = dw_detail.GetItemNumber( 1, "view_id" ) -- comment by jervis 10.19.2009
...
if  lstr_parm.s_type = 'ok' and not isnull(lstr_parm.s_name ) and trim(lstr_parm.s_name) <> ""  then 
	of_check_report_name(ll_view_id)
end if 
...
if upper(is_mod_user) = upper(gs_user_id) and tab_1.tabpage_browse.cb_new.enabled then //maha 111902 //Modified By Ken.Guo 2015-09-18. added check cb_new(for readonly function)
	tab_1.tabpage_columns.Enabled = True
	tab_1.tabpage_filter.Enabled = True
	tab_1.tabpage_preview.Enabled = True
	tab_1.tabpage_sort.Enabled = True
...
of_checked_action_right('tabpage') //Added By Ken.Guo 2015-09-18		
...
dw_detail.Update()
dw_report_list.retrieve(gs_user_id) //Added By Ken.Guo 2015-08-05. Add role filter.
//comment 12.11.2007 by: Andy
//IF ib_new THEN
//	tab_1.tabpage_tables.dw_select_table.Retrieve()
//END IF
...
IF dw_report_list.RowCount() = 0 THEN	Return
...
gnv_appeondb.of_StartQueue()
Delete From conv_view_where Where view_id = :li_view_id;
Delete From conv_view_table Where view_id = :li_view_id;
Delete From conv_view_fields Where view_id = :li_view_id;
Delete From conv_view_links Where view_id = :li_view_id;
Delete From conv_view_sort Where view_id = :li_view_id;
Delete From conv_view Where view_id = :li_view_id;
Delete From ctx_report_syntax Where view_id = :li_view_id;

//BEGIN---Modify by Scofield on 2009-08-07
DELETE FROM dashboard_gadgets_favorite
      WHERE UPPER(favorite_type) = 'INTELLIREP' AND data_id = :li_view_id;
//END---Modify by Scofield on 2009-08-07
commit;
dw_report_list.Retrieve(gs_user_id) //Added By Ken.Guo 2015-08-05. Add role filter.
gnv_appeondb.of_CommitQueue()
...
res = MessageBox("Copy report","Are you sure you want to copy this report?",question!,yesno!,1)
...
IF dw_report_list.rowcount() > 0 THEN	
	ll_view_id = dw_report_list.GetItemNumber(dw_report_list.GetRow(),'view_id')
	//ls_name = dw_detail.getitemstring(1,"view_name")  -- comment by jervis 10.19.2009
	ls_name = dw_report_list.getitemstring(dw_report_list.GetRow(),"view_name")
	...
	//of_retrieve_report(il_view_id) -- comment by jervis 10.19.2009
	of_retrieve_report(ll_view_id)
...
*/
	...
	if ck = 1 then
		//BEGIN---Modify by Scofield on 2010-03-12
		INSERT INTO ctx_report_syntax(view_id,dw_sql,dw_syntax,modify_date,dw_select)
			  SELECT :il_view_id,dw_sql,dw_syntax,modify_date,dw_select
				 from ctx_report_syntax
				WHERE view_id = :ll_view_id;
		//END---Modify by Scofield on 2010-03-12
		//Added By Jay Chen 12-27-2013
		insert into conv_view_pic(view_id,screen_id,pic_type,pic_band,pic_name,pic_filename,pic_bitmap,pic_x,pic_y,pic_width,pic_height,pic_tag,pic_painter_id)
		select :il_view_id,
				screen_id,
				pic_type,
				pic_band,
				...
				pic_painter_id
		from conv_view_pic
		where view_id = :ll_view_id and screen_id = 0;

		of_check_report_name(il_view_id)							//Added By Mark Lee 03/09/2015
		//end add
		gnv_appeondb.of_StartQueue()
		dw_report_list.retrieve(gs_user_id) //Added By Ken.Guo 2015-08-05. Add role filter.
		of_retrieve_detail(il_view_id)
...
this.cb_back_2=create cb_back_2
...
event clicked;if ii_prior_step < tab_1.selectedtab and ii_prior_step > 0 then
	tab_1.selecttab( ii_prior_step)
...
if lower(dwo.name) = "p_1" or lower(dwo.name) = "t_1" or lower(dwo.name) = "t_3" then
	...
	if of_get_image(ls_v_name,ll_dataLength,lblb_data) = 1 then 
		...
		ls_path = gs_dir_path + gs_DefDirName + "\temp"
		//check path
		...
		lstr_parm.b_flag = lb_flag //Jervis 12.15.2009
...
if this.GetItem( al_parent, ltvi ) = 1 then
	...
	if ll_module = 1 OR ll_module = 2 then
		...
		For ll_i = ll_Cnt to 1 step -1
			...
			ls_Screen = Left(ls_Screen,Pos(ls_Screen,',') - 1)
...
//Added by Nova on 2008-08-20
String ls_tmp_table_name
String ls_tmp_first_table_name
Boolean lb_macthed_multi_tables //Added By Ken.Guo 2016-03-15
...
FOR i = 1 TO li_rc //if an affil_stat child add the affil_stat_view
	//Added By Mark Lee 03/20/2015		add the lower() function
	ls_table_name = lower(dw_view_tables.GetItemString( i, "table_name" ))
//Start Code Change ---- 10.05.2007 #V7 maha added v_affil_dept to list
	IF ls_table_name = "v_dept" OR ls_table_name = "v_affil_dept" OR ls_table_name = "v_staff_cat" OR ls_table_name = "v_staff_leave"  OR ls_table_name = "v_committee_review" OR +&
		...
		IF dw_view_tables.Find( "table_name = 'v_affil_stat'", 1, 1000 ) = 0 THEN
			...
			select v_alias into :ls_table_alias from view_alias where lower(v_name) = :ls_table_name ; 
			if Len(ls_table_alias) = 0 then ls_table_alias = ls_table_name
...
//Start Code Change ---- 10.05.2007 #V7 maha added v_affil_dept to list
	...
	//$<reason> Modification for Contract Logix.

	IF Upper( Left( ls_first_table_name, 5 ) ) = "V_CTX" AND &
		Upper( Left( ls_table_name, 5 ) ) = "V_CTX" THEN
		...
		If lower(ls_first_table_name) = 'v_ctx_multi_header' and lower(ls_table_name) = 'v_ctx_multi_detail' then
			...
			lb_macthed_multi_tables = True //Added By Ken.Guo 2016-03-15
...
//Modify 07/19/2007 by: Andy
//FOR i = 1 TO li_rc
FOR i = 1 TO li_rc//Added by Nova on 2008-08-20
//end of modify
	//Added By Mark Lee 03/20/2015
	ls_table_name = lower(dw_view_tables.GetItemString( i, "table_name" )	)
	CHOOSE CASE ls_table_name
		CASE "v_affil_stat"
			IF NOT lb_affil_stat_allready_linked THEN
				...
				IF dw_view_tables.Find( "lower(table_name) = 'v_basic'", 1, 1000 ) > 0 THEN				
					dw_view_links.SetItem( li_link_row, "link_from_table_nm", "v_basic" )				
				ELSEIF dw_view_tables.Find( "lower(table_name) = 'v_basic_info'", 1, 1000 ) > 0 THEN			
					dw_view_links.SetItem( li_link_row, "link_from_table_nm", "v_basic_info" )								
				ELSEIF dw_view_tables.Find( "lower(table_name) = 'v_full_name'", 1, 1000 ) > 0 THEN			
...
//				ls_tmp_first_table_name=ls_first_table_name
			//Added by Nova on 2008-08-20
			if (lower(ls_tmp_table_name)<>lower(ls_table_name))  and  (lower(ls_tmp_first_table_name)<>lower(ls_tmp_first_table_name)) then
				//modify 07/19/2007 by: Andy
			...
			IF dw_view_tables.Find( "table_name = 'v_affil_stat'", 1, 1000 ) > 0 THEN
				//IF NOT lb_affil_stat_allready_linked THEN	//Added by Nova on 2008-08-20			
					cb_view_add_link.TriggerEvent( Clicked! )
					li_link_row = dw_view_links.RowCount( )
					dw_view_links.SetItem( li_link_row, "link_type", "=" )				
...
//Added By Mark Lee 03/20/2015
IF dw_view_tables.Find( "lower(table_name) = 'v_net_dev_actions'", 1, 1000 ) > 0  and dw_view_tables.Find( "lower(table_name) = 'v_net_dev_ids'", 1, 1000 ) > 0 THEN
	cb_view_add_link.TriggerEvent( Clicked! )
	li_link_row = dw_view_links.RowCount( )
	dw_view_links.SetItem( li_link_row, "link_type", "=" )
...
//Added By Ken.Guo 2016-03-15. auto set table link for multi header and multi detail tables.
If not lb_macthed_multi_tables and dw_view_tables.Find( "lower(table_name) = 'v_ctx_multi_header'", 1, 1000 ) > 0 and dw_view_tables.Find( "lower(table_name) = 'v_ctx_multi_detail'", 1, 1000 ) > 0 Then
	cb_view_add_link.TriggerEvent( Clicked! )
	li_link_row = dw_view_links.RowCount( )
	dw_view_links.SetItem( li_link_row, "link_type", "=" )
	dw_view_links.SetItem( li_link_row, "link_from_table_nm", "v_ctx_multi_header" )				
	dw_view_links.SetItem( li_link_row, "link_from_field_nm", "hdr_id" )			
	dw_view_links.SetItem( li_link_row, "link_to_table_nm", "v_ctx_multi_detail" )				
	dw_view_links.SetItem( li_link_row, "link_to_field_nm", "hdr_id" )	
End If
...
event doubleclicked;//Add by Jervis 02.02.2010
...
this.cb_back_3=create cb_back_3
...
event constructor;//Begin - Added By Mark Lee 06/07/2016.
// field_wizard_name type value:
//C
//S
//''
//NULL
...
//P
//
//in the design of company lookup code value:
//All								All
//A								Our company	
//B									Contracted Company		
//O								Other							
//but this company lookup code value:
//Y									Company Name
//U								OUR COMPANY
//B									CONTRACTED COMPANY
//O								OTHER COMPANY
//End - Added By Mark Lee 06/07/2016.
//--------------------------- APPEON BEGIN ---------------------------
//$<add> 02.22.2008 by Andy
...
if This.GetChild( "field_wizard_name", dwchild ) = 1 then 
	...
	//Begin - Added By Mark Lee 06/07/2016.
	dwchild.InsertRow( 1 ) 
	dwchild.SetItem( 1, "lookup_name", "COMPANY NAME" )
	dwchild.SetItem( 1, "type", "Y" )
	
	dwchild.InsertRow( 1 ) 
	dwchild.SetItem( 1, "lookup_name", "OUR COMPANY" )
	dwchild.SetItem( 1, "type", "U" )
	
	dwchild.InsertRow( 1 ) 
	dwchild.SetItem( 1, "lookup_name", "CONTRACTED COMPANY" )
	dwchild.SetItem( 1, "type", "B" )
	
	dwchild.InsertRow( 1 ) 
	dwchild.SetItem( 1, "lookup_name", "OTHER COMPANY" )
	dwchild.SetItem( 1, "type", "O" )
	//End - Added By Mark Lee 06/07/2016.
...
CHOOSE CASE String(dwo.name)
	CASE "field_wizard_name"
		This.GetChild( "field_wizard_name", dwchild )
		//Added By Mark Lee 03/20/2015		add the lower() function
		li_found = dwchild.Find( "lower(lookup_name) = '" + lower(data) + "'", 1, dwchild.RowCount() )//GetRow()
		IF li_found > 0 THEN
			This.SetItem( row, "field_wizard_type", dwchild.GetItemString( li_found, "type" ) )
			This.SetItem( row, "lookup_type", dwchild.GetItemString( li_found, "type" ) )  //Added by Harry 2017-08-11 When set a filter by,the lookup_type field 's value is null.
		END IF
		...
		of_generate_lookup_column(dwchild.GetItemString( li_found, "type" ))										//Added By Mark Lee 06/08/2016
	CASE "field_visible"
		IF data = "0" THEN
			This.SetItem( row, "field_x", -1 )
		ELSEIF data = "2" THEN //Added By jay 2016-04-05
			This.SetItem( row, "field_x", -99999 )
		ELSE
	...
	CASE "aggregate_type"
		IF TRIM(data) = "" then 
			...
			/*
			temp comment by jervis 03.08.2010
			//Add by jervis 02.02.2010
			//Don't allow the Aggregate Type = Group to be selected 
			//unless the Group by this Column check box is checked
			ls_temp = String(this.GetItemNumber(row,"group_by_field"))
			if ls_temp = '0' or isnull(ls_temp) then
				if Data = 'GL' or data = 'GLSL' then
					MessageBox('Infomation',"Don't allow the Group Level to be selected unless the Group by this Column check box is checked")
					return 2
				end if
			end if
			*/
...
//		END IF
	...
	CASE "filter_by_field"
		...
		ELSE
			...
			//Added By Mark Lee 07/22/2016
			if lower(ls_v_name) = "v_ctx_basic_info" and lower(ls_c_name) = lower("Company_Name") then  
				ls_c_name = "app_facility"
//				if isnull(this.getitemstring(row,"lu_field_name")) or trim(this.getitemstring(row,"lu_field_name")) = "" then This.setitem(row,"lu_field_name","facility_name")
			end if 
			IF of_get_lookup_data(ls_v_name,ls_c_name,ls_Display,ls_lookup_type,ls_lookup_name) = 1 THEN
...
//				IF lower(ls_Display) = 'code' then ls_Display = 'C' else ls_Display = 'D'
				...
				of_generate_lookup_column(ls_lookup_type)	
			...
			if ll_Find > 0 then
				...
				if ls_ColType = '61' or ls_ColType = 'timestamp' then
					...
					return 
				elseif  ls_ColType = '167' or ls_ColType = 'timestamp' then 
					//Added By Mark Lee 07/22/2016
				end if
			end if
			
			//If a user clicks on the Filter by this Column checkbox and the Filter Wizard and Compare To can't be figured out then popup a message - jervis 02.02.2010
			MessageBox("Filtering","The Report Writer does not have enough information to configure/Select the Filter Settings.  You need to manually set the Filter Wizard and Compare To fields.")
		END IF
	CASE "label_text"
		//$<add> 03.31.2008 by Andy
	...
	CASE "lu_field_name"
		...
		ls_c_name = GetItemString(row,"field_name")
		//Added By Mark Lee 03/20/2015		add the lower() function
		ll_Find   = tab_1.tabpage_filter.dw_view_where.Find( "lower(table_field_name)='" + lower(ls_v_name) + "." + lower(ls_c_name) + "'", 1, tab_1.tabpage_filter.dw_view_where.RowCount())
		if ll_Find > 0 then 
...
FOR i = 1 TO rowcount
	...
	ELSE
		if Long( This.GetItemNumber( i, "field_x" ) ) = -99999 then //Added By jay 2016-04-05
			This.SetItem( i, "field_visible", 2 )
		else
...
event rowfocuschanged;long							ll_TableID, ll_find
DatawindowChild			dwchild
...
//Begin - Added By Mark Lee 06/08/2016.
this.setredraw( false)
string 	ls_v_name, ls_c_name, ls_Display, ls_lookup_type, ls_lookup_name
string 	ls_temp
ls_v_name = GetItemString(CurrentRow,"table_name")
ls_c_name = GetItemString(CurrentRow,"field_name")
ls_temp = this.getItemString(CurrentRow,"field_wizard_name")
if lower(ls_v_name) = "v_ctx_basic_info" and lower(ls_c_name) = lower("Company_Name") then  
	ls_c_name = "app_facility"
//	if isnull(this.getitemstring(CurrentRow,"lu_field_name")) or trim(this.getitemstring(CurrentRow,"lu_field_name")) = "" then This.setitem(CurrentRow,"lu_field_name","facility_name")
end if 
of_get_lookup_data(ls_v_name,ls_c_name,ls_Display,ls_lookup_type,ls_lookup_name)
if not isnull(ls_lookup_name) and trim(ls_lookup_name) <> "" then this.SetItem( CurrentRow, "field_wizard_name", ls_lookup_name )			//!!
this.GetChild( "field_wizard_name", dwchild )
ll_find = dwchild.Find( "lower(lookup_name) = '" + lower(this.getItemString(CurrentRow,"field_wizard_name") ) + "'", 1, dwchild.RowCount() ) 
IF isnull(ll_find) or ll_find <= 0 then 
	of_generate_lookup_column("")										
else 
	of_generate_lookup_column(dwchild.GetItemString( ll_find, "type" ))										
END IF
this.SetItem( CurrentRow, "field_wizard_name", ls_temp )		//reset 
this.setredraw( true)
//End - Added By Mark Lee 06/08/2016.
...
dw_view_fields.SetItem( li_nr, "label_text", Mid(ls_field,1, 38) )
...
this.cb_back_4=create cb_back_4
...
Long   ll_Pos, ll_Return
String ls_lookup_name,ls_desc,ls_lookup_type,ls_Return,ls_Temp
String ls_field_type,ls_table_field
String ls_v_name,ls_c_name,ls_Display
String ls_Null,ls_Val,ls_lu_field_name,ls_operator,ls_value,ls_rqmnt
String ls_facilityname
String ls_value_arr[], ls_value_temp
...
elseif dwo.Name = "b_dropdown" then
	...
	ls_operator = this.GetItemString(row, "operator")
	IF Lower(ls_operator) = "in"  and NOT (Upper(ls_lookup_type) = 'Y' Or Upper(ls_lookup_type) = 'U'  or Upper(ls_lookup_type) = 'B' Or Upper(ls_lookup_type) = 'O') THEN //Modified By Ken.Guo 2017-05-18
	...
	w_custom_report.tab_1.tabpage_filter.sle_1.Text = ls_lu_field_name
	...
	IF Upper(ls_lookup_type) = 'C' Or Upper(ls_lookup_type) = 'A' THEN
		...
		IF ls_lookup_name = "" THEN
			...
			ll_Pos = Pos(ls_table_field,'.')
			ls_v_name = Left(ls_table_field,ll_Pos - 1)
			ls_c_name = Mid(ls_table_field ,ll_Pos + 1)
			IF of_get_lookup_data(ls_v_name,ls_c_name,ls_Display,ls_lookup_type,ls_lookup_name) = 1 THEN
...
//					END IF
					//Begin - Added By Mark Lee 06/12/2016.
					if not isnull(ls_lookup_type) and ( upper(trim(ls_lookup_type)) = 'Y' or upper(trim(ls_lookup_type)) = 'U' &
						or upper(trim(ls_lookup_type)) = 'B' or upper(trim(ls_lookup_type)) = 'O'  ) then 
						ls_lu_field_name = ls_Display
					else
					...
					of_generate_lookup_column(ls_lookup_type)	
					//End - Added By Mark Lee 06/12/2016.
		...
		//Added By Ken.Guo 2017-05-18
		IF Upper(ls_lookup_type) = 'Y' Or Upper(ls_lookup_type) = 'U'  or Upper(ls_lookup_type) = 'B' Or Upper(ls_lookup_type) = 'O'  THEN
			ls_Value = this.getitemstring(row, 'value')
			
			//delete single quote 
			If left(ls_value,1) = "'" and right(ls_value, 1) = "'" Then
				ls_value_temp = Mid(ls_value, 2, len(ls_value) - 2)
				ls_value_temp = gnv_string.of_globalreplace(ls_value_temp, "','",  ",")
			End If

			OpenWithParm(w_facility_name_select,"code&" + ls_value_temp)
			ls_value = ''
			ls_Value = Message.StringParm
			if IsNull(ls_Value) or Trim(ls_Value) = "" then Return
			...
			//Add sigle quote to each value.
			ls_value_temp = gnv_string.of_globalreplace(ls_Value, ',',  '|||')
			ls_value_temp = "'" + gnv_string.of_globalreplace(ls_value_temp, '|||',  "','") + "'"

			This.SetItem(row,"value", ls_value_temp)
			if Lower(ls_operator) <> "in" then
				ls_desc = gnv_data.of_getItem("ctx_facility","facility_name","facility_id = " + string(ls_value))	
			else
				ls_desc = of_get_company_name(ls_Value)
			end if
...
CHOOSE CASE lower(dwo.name)
	CASE 'table_field_name'
		...
		if this.getchild('table_field_name',ldwc) = 1 then
			//Added By Mark Lee 03/20/2015		add the lower() function
			ll_Find = ldwc.find( "lower(dbname)='" + lower(data) + "'", 1, ldwc.rowcount())
			if ll_Find > 0 then
		...
		ll_Pos = Pos(data,'.')		
		//Added By Mark Lee 03/20/2015
		ls_v_name = lower(Left(data,ll_Pos - 1))
		ls_c_name = lower(Mid(data ,ll_Pos + 1))
		IF of_get_lookup_data(ls_v_name,ls_c_name,ls_Display,ls_lookup_type,ls_lookup_name) = 1 THEN
			if isNull(ls_lu_field_name) or ls_lu_field_name = "" then
//				IF lower(ls_Display) = 'code' then ls_Display = 'Code' else ls_Display = 'Description'
				//Added By Mark Lee 06/12/2016
	...
	CASE 'operator'
		If this.GetItemString( row, 'operator' , primary!, true ) = 'In' And data <> 'In' Then
			this.SetItem( row, "value"  , '' )	
			this.SetItem( row, "value_display"  , '' )
		End If
...
if dwo.name = "left_bracket" then
	...
	if IsNull(ls_data) or Trim(ls_data) = "" then
		...
		ll_Limit = long(This.Describe("left_bracket.Edit.Limit"))
		if Len(ls_data) < ll_Limit then
			This.SetItem(Row,"left_bracket",ls_data + "(")
		end if
	end if
elseif dwo.name = "right_bracket" then
	...
	if IsNull(ls_data) or Trim(ls_data) = "" then
		...
		ll_Limit = long(This.Describe("right_bracket.Edit.Limit"))
		if Len(ls_data) < ll_Limit then
			This.SetItem(Row,"right_bracket",ls_data + ")")
		end if
	end if
end if
...
event rbuttondown;String ls_data
...
if dwo.name = "left_bracket" then
	...
	if IsNull(ls_data) or Trim(ls_data) = "" then
		...
		This.SetItem(Row,"left_bracket",Left(ls_data,Len(ls_data) - 1))
	end if
...
elseif dwo.name = "right_bracket" then
	...
	if IsNull(ls_data) or Trim(ls_data) = "" then
		...
		This.SetItem(Row,"right_bracket",Left(ls_data,Len(ls_data) - 1))
	end if
...
this.cb_back_5=create cb_back_5
...
this.of_register( 1, "module", "name", ":al_view_id", "d_view_module_fields", SQLCA, "")
this.of_register( 2, "v_name", "v_alias", ":al_view_id,:al_module", "d_view_names_fields", SQLCA, "")
this.of_register( 3, "c_name", "c_alias", ":as_table,:al_data_view_id,:al_view_id", "d_view_fields_alias_sort", SQLCA, "flag")
//---------------------------- APPEON END ----------------------------
...
this.cb_back_6=create cb_back_6
this.cb_next_6=create cb_next_6
...
destroy(this.cb_back_6)
destroy(this.cb_next_6)
...
event clicked;//Jervis 01.27.2010
of_selecttab(7)
end event
...
if Pos(is_CurObject_Name, "_label" ) > 0 then
	ls_Type = "column"
	is_object_type = ls_Type
	is_CurObject_Name = Left(is_CurObject_Name,Len(is_CurObject_Name) - Len("_label"))
end if
...
IF ls_Type = "text"  And Pos( is_CurObject_Name, "title" ) = 0 THEN
...
fontfamily fontfamily = swiss!
string facename = "MS Sans Serif"
string text = "Advanced Design..."
end type

event clicked;datetime		ldt_modifydate
...
OpenWithParm(w_contract_report_design,lstr)
...
//Begin - Added By Mark Lee 01/22/2015.
tab_1.tabpage_preview.cb_14.TriggerEvent(Clicked!)
//End - Added By Mark Lee 01/22/2015.
end event
...
event clicked;//tab_1.tabpage_preview.cb_14.TriggerEvent(Clicked!)		move it 
...
// Returns:  long
//--------------------------------------------------------------------
// Author:	Scofield		Date: 2009-05-21
//--------------------------------------------------------------------
//	Copyright (c) 1999-2009 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
String		ls_Syntax,ls_dw_select
long			ll_Count
blob			lb_Syntax
DateTime		ldt_Today

ls_Syntax = tab_1.tabpage_Preview.dw_Preview.Describe("DataWindow.Syntax")
ls_dw_select = tab_1.tabpage_Preview.dw_Preview.Describe("DataWindow.Table.Select")				//Added By Mark Lee 01/19/2015
...
if ll_Count = 0 then
	INSERT ctx_report_syntax(view_id,dw_sql,modify_date,dw_select) VALUES(:il_view_id,:ls_Syntax,:ldt_Today,:ls_dw_select);
else
	UPDATE ctx_report_syntax SET dw_sql = :ls_Syntax,modify_date = :ldt_Today,dw_select=:ls_dw_select WHERE view_id = :il_view_id;
end if
...
//Begin - Added By Mark Lee 01/22/2015.
//Added By Mark Lee 01/22/2015 need sync the data for the web export data. (  04.15.2008 by Andy Reason:Optimize speed. )

//of_dw_syntax_update()
if tab_1.tabpage_columns.dw_view_fields.modifiedcount( ) > 0  or  tab_1.tabpage_columns.dw_view_fields.deletedcount( ) > 0   then 
	tab_1.tabpage_preview.dw_label.setfocus( )
	tab_1.tabpage_columns.dw_view_fields.update( )	
end if 
//End - Added By Mark Lee 01/22/2015.
...
FOR i = 1 TO ii_col_count
	...
	dw_Display.Modify(ls_column_name[i] + ".Font.Face='MS Sans Serif'")
...
IF ii_tag > 0 THEN
	...
	//Comment by jervis 01.06.2010
	/*
	//Change alias to label -- jervis 11.20.2009
	ls_alias = of_build_alias(ii_tag,data)
	if ls_alias <> '' then tab_1.tabpage_columns.dw_view_fields.SetItem(ii_tag,"field_alias",ls_alias)
	*/
END IF
//---------------------------- APPEON END ----------------------------
...
event clicked;if this.checked then
	tab_1.tabpage_run.dw_run.modify("datawindow.print.preview = 'yes'")
else
	tab_1.tabpage_run.dw_run.modify("datawindow.print.preview = 'no'")
end if
end event
...
fontfamily fontfamily = swiss!
string facename = "MS Sans Serif"
string text = "Fit Size"
end type

event clicked;//====================================================================
// Event: clicked
//--------------------------------------------------------------------
// Description:With this PageCountAcross () function allows the contents of the data window automatically adapt to the size of paper 
//--------------------------------------------------------------------
// Arguments:
//--------------------------------------------------------------------
// Returns:  long
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 09-17-2013
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
//
//====================================================================
int li_count,li_index,li_x,li_width,li_pos[]
string ls_objects,ls_colname,ls_objname,ls_modify,ls_x,ls_width,ls_papersize,ls_paperorientation,ls_zoom,ls_across
long ll_seppos,ll_maxpos,ll_paperpbunit,ll_rate,ll_dwwidth,j,k
dec{2} ld_paperwidth

if tab_1.tabpage_run.dw_run.describe("datawindow.print.preview") <> 'yes' then return

tab_1.tabpage_run.dw_run.SetRedraw(FALSE) 

IF AppeonGetClientType() = 'PB' THEN //PB run mode use PageCountAcross() function
    k = 0
    for j=5 to 500 
//		if j = 117 then debugbreak()
		ls_zoom=string(j) 
		tab_1.tabpage_run.dw_run.modify("datawindow.zoom="+ls_zoom) 
		ls_across = tab_1.tabpage_run.dw_run.describe("evaluate('PageCountAcross()',"+string(tab_1.tabpage_run.dw_run.object.datawindow.firstrowonpage)+")") 
		if long(ls_across)=2 then 
			ls_zoom=string(j - 2) 
			tab_1.tabpage_run.dw_run.modify("datawindow.zoom="+ls_zoom) 
			k=1 
			exit 
	...
	next
	em_zoom.text = string(ls_zoom)
	if k=0 and j=5 then 
		messagebox("Infomation","Your report is to wide,current printer can not adapt!")
	end if 

ELSE                                                 //WEB run mode manual compute zoom rate
	li_count = integer(dw_run.Object.DataWindow.Column.Count)
	if li_count <= 0 then return
	//1. get datawindow actual width
	for li_index = 1 to li_count
		li_x = integer(dw_run.Describe('#' + String(li_index) + '.x'))
		li_width = integer(dw_run.Describe('#' + String(li_index) + '.width'))
		li_pos[UpperBound(li_pos) + 1] = li_x + li_width 
		if UpperBound(li_pos) > 1 then
			if li_x + li_width > ll_maxpos then
				ll_maxpos = li_x + li_width
			end if
		else
			ll_maxpos = li_x + li_width
		end if
	next
	ll_dwwidth = ll_maxpos + 50
	//2. compute best zoom rate
	ls_papersize = dw_run.object.datawindow.print.paper.size
	ls_paperorientation = dw_run.object.datawindow.print.orientation
	ld_paperwidth = of_get_original_size(ls_papersize,ls_paperorientation)
	if isnull(ld_paperwidth) or ld_paperwidth = 0 then
		if ls_paperorientation <> '1' then
			ld_paperwidth = 21
		else
			ld_paperwidth = 29.7
		end if
	end if
	ll_paperpbunit = ld_paperwidth * 3385/21  
	if ll_dwwidth > 0 then
		ll_rate = long(100* ll_paperpbunit / ll_dwwidth)
	end if
	em_zoom.text = string(ll_rate)
	dw_run.modify("datawindow.zoom="+string(ll_rate))
...
event ue_changed;dw_run.modify("datawindow.zoom="+this.text)
end event

event modified;dw_run.modify("datawindow.zoom="+this.text)
end event
...
//EVENT ue_mail('Report Writer','' )
//---------------------------- APPEON END ----------------------------

if of_get_app_setting("set_56","I") = 1 then
	Super::EVENT ue_mail_bak('Report Writer','' )
else
	Super::EVENT ue_mail('Report Writer','' )
end if
...
event pfc_pagesetup;call super::pfc_pagesetup; tab_1.tabpage_run.dw_run.modify("datawindow.zoom="+string(100))
 tab_1.tabpage_run.em_zoom.text = '100'
// tab_1.tabpage_run.cb_suit.triggerevent(clicked!)
...
event clicked;String	ls_CurDir

//BEGIN---Modify by Scofield on 2009-04-24
...
newmenu.m_options.m_saveasexcel.visible = true	//01.05.2009 by jervis
...
dw_run.Filter()
//Added by Nova.zhang on 2008-10-22
dw_run.groupcalc()
...
event retrieveend;call super::retrieveend;string 		ls_tmp
This.InsertRow( 1 )
is_find_all_row = "" 								//Added By Mark Lee 07/26/2016
IF This.DataObject <> "d_crit_date_range_custom" AND This.DataObject <> "d_crit_single_date_custom" AND &
	This.DataObject <> "d_crit_facility" AND This.DataObject <> "d_crit_address_type_ir" AND This.DataObject <> "d_crit_net_dev_type_ir" AND &
	This.DataObject <> "d_crit_app_payor" and This.DataObject <> "d_crit_company" THEN			//Added By Mark Lee 06/07/2016
	...
	END IF
	is_find_all_row ="lookup_code=0 and code='All' "							
ELSEIF This.DataObject = "d_crit_facility"    THEN			
	This.SetItem( 1, "selected", 1 )
	This.SetItem( 1, "facility_id", 0 )
	This.SetItem( 1, "facility_name", "All" )	
	is_find_all_row ="facility_id=0 and facility_name='All' "								
ELSEIF This.DataObject = "d_crit_company"    THEN			//Added By Mark Lee 06/07/2016 add d_crit_company
	This.SetItem( 1, "selected", 1 )
	This.SetItem( 1, "facility_id", 0 )
//	This.SetItem( 1, "facility_name", "All, Include All selections" )		
	This.SetItem( 1, "facility_name", "All" )		
	This.SetItem( 1, "status", "A" )									//mode data for setfilter() funcation.
	ls_tmp = gnv_data.of_getitem("icred_settings", "filter_company_contact", false)
	if not isnull(ls_tmp) and trim(ls_tmp)= '1' then 
		this.setfilter("status = 'A' ")
		this.filter( )
	end if 		
	is_find_all_row ="facility_id=0 and facility_name='All' "	
ELSEIF This.DataObject = "d_crit_net_dev_type_ir"  THEN
	This.SetItem( 1, "selected", 1 )
	//This.SetItem( 1, "facility_id", 0 )
	This.SetItem( 1, "lookup_code", 0 )				//Added By Mark Lee 07/26/2016 test
	This.SetItem( 1, "code", "All" )	
	is_find_all_row ="lookup_code=0 and code='All' "					
ELSEIF This.DataObject = "d_crit_app_payor"  THEN
	This.SetItem( 1, "selected", 1 )
	//This.SetItem( 1, "facility_id", 0 )
	This.SetItem( 1, "app_id", 0 )				//Added By Mark Lee 07/26/2016 test	
	This.SetItem( 1, "application_name", "All" )	
	is_find_all_row ="app_id=0 and application_name='All' "					
...
event itemchanged;call super::itemchanged;//IF data = "1" AND row = 1 THEN							//Added By Mark Lee 07/22/2016  apb bug, the frist row is not all 
//	Integer li_rc
//	Integer i
//	li_rc = This.RowCount()
//	FOR i = 2 TO li_rc
...
long	 ll_rc, i , ll_id 
string 	ls_code, ls_code_id, ls_code_name

IF data = "1" then 
	ls_code_id = ""
	ll_id = 0 
	IF This.DataObject = "d_crit_facility" or This.DataObject = "d_crit_company"    THEN
		ls_code_id = "facility_id"
		ls_code_name = "facility_name"
	ELSEIF This.DataObject = "d_crit_net_dev_type_ir"    THEN
		ls_code_name = "code"
//		ls_code_id = ""
	ELSEIF This.DataObject = "d_crit_app_payor"    THEN
		ls_code_name = "application_name"
//		ls_code_id = ""
	else
		ls_code_id = "lookup_code"
		ls_code_name = "code"
	end if 
	
	ls_code = this.getItemstring( row, ls_code_name)
	if ls_code_id <> "" then ll_id = this.getItemNumber( row, ls_code_id)
	ll_rc = This.RowCount()
	if not isnull(ls_code) and (ls_code = 'All' ) and ( isnull(ll_id) or ll_id = 0) then 
		for i = 1 to ll_rc 
			if i <> row then This.SetItem( i, "selected", 0 )
		next
	else 
		This.SetItem( row, "selected", 0 )
	end if 
end if 
end event
