...
if Not (IsNull(ls_Value) or Trim(ls_Value) = "") then
	if is_type = 'code&' then
		...
		loop while ls_Value <> ""
		*/
		gnv_string.of_parsetoarray( ls_value,',', ls_code_arr[])
		For i = 1 To UpperBound(ls_code_arr[])
			If isnumber(Trim(ls_code_arr[i])) Then
				ll_Find = dw_facility_select.Find("facility_id = " + Trim(ls_code_arr[i])  ,1,dw_facility_select.RowCount())
				if ll_Find > 0 then
					dw_facility_select.SetItem(ll_Find,"selected",1)
				end if			
			End If
