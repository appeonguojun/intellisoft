...
type st_2 from statictext within w_help_window
end type
...
long backcolor = 67108864
boolean enabled = false
string text = "Select Subcategory to view"
boolean focusrectangle = false
end type
...
elseif rb_exact.checked = true then //exact
	...
	//		messagebox("only one word",ls_words[1])
			...
			w = 1
			ls_words[w] = mid(ls_search,1,p1 - 1)
			do while p1 >0 //for each word put into array
				p2 = p1
				p1 = Pos(ls_search, " " ,p2 + 1)
				w++
				if p1 = 0 then
					ls_words[w] = mid(ls_search,p2 + 1,len(ls_search))
				else
					ls_words[w] = mid(ls_search,p2 + 1,p1 - p2 - 1)
				end if
			loop
		end if
