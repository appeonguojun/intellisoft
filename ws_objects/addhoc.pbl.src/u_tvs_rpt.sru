...
if Len(inv_attrib[li_Index].is_FilterCols) > 0 then
...
FOR ll_row = 1 to ll_rowcount
	FOR ll_i= 1 TO ll_Cnt
		...
		Choose Case Left(ls_ColType[ll_i], 7)
...
//		ltvi_item.Selectedpictureindex = 5//li_Index
	...
	//Added By jay 2016-04-06
	ls_field_name = string(la_val[1])
	if isvalid(w_custom_report) then
		ll_find = w_custom_report.tab_1.tabpage_columns.dw_view_fields.find("field_name = '"+ls_field_name+"'",1,w_custom_report.tab_1.tabpage_columns.dw_view_fields.rowcount())
		if isnull(ll_find) then ll_find = 0
		if ll_find > 0 then
			if w_custom_report.tab_1.tabpage_columns.dw_view_fields.getitemnumber(ll_find, "field_visible") = 2 then continue
		end if
	end if
	//end
...
if IsNull (ai_level) or ai_level <= 0 or &
	Len (as_datacolumn) = 0 or IsNull (as_datacolumn) or &
	Len (as_labelcolumn) = 0 or IsNull (as_labelcolumn) or &
	Len (as_DWobjectname) = 0 or IsNull (as_DWobjectname) then
	return -1
end if
