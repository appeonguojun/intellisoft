...
forward prototypes
public function boolean of_field_check (string as_fieldname)
end prototypes

public function boolean of_field_check (string as_fieldname);//Add by Jervis 11.01.2010
...
event itemchanged;mle_cal_fld.Text = mle_cal_fld.Text + data
This.Reset()
This.InsertRow( 0 )
...
long backcolor = 67108864
boolean enabled = false
string text = "Formula: please input a T-SQL Expression"
boolean focusrectangle = false
end type
...
event clicked;
//Check field name - jervis 11.01.2010
if Not of_Field_Check(sle_fld_nm.Text) then
...
event doubleclicked;IF row > 0 THEN
	...
	ls_calc_data = Mid( ls_calc_data, 1, li_pos -1 ) + ls_calc_fld + Mid( ls_calc_data, li_pos+li_length, 1000 )
