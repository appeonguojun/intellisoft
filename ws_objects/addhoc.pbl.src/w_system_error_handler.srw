...
//dw_error.object.database[1] =  sqlca.dbparm			//Commented by Scofield on 2009-03-31
...
for ll_i = 1 to long(dw_error.Object.DataWindow.Column.Count)
		...
		if long(dw_error.Describe("#" + string(ll_i) + ".Visible ")) = 1 then 
			...
			ll_col_header_name_len = len(trim(ls_col_header_name))
			// add some spaces
...
ls_address[1] =  'support@contractlogix.com'  //Start Code Change ----11.21.2008 #V85 maha
...
ls_msg += 'The System Error window helps you communicate to ContractLogix the nature of program failures. Those failures may be due to problems with your data, may be programming errors, and so on. ContractLogix will be able to diagnose and fix the problem more quickly, if you can pass along the System Error information to the ContractLogix support team.'
ls_msg += ' ~r~n'
ls_msg += ' ~r~nThere are three ways for you to capture the System Error message:'
ls_msg += ' ~r~n'
ls_msg += ' ~r~n1) [Print] will print the information, so you can fax it to ContractLogix;'
ls_msg += ' ~r~n'
ls_msg += ' ~r~n2) [To ClipBoard] puts the information into the Windows clipboard, so you can paste it into a message that you e-mail to ContractLogix.'
ls_msg += ' ~r~n'
