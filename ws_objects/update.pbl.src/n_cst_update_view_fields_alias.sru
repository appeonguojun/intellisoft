...
public function integer of_update_view_fields_alias (long al_module, long al_data_view_id, boolean ab_force_update, string as_filter)
public function integer of_update_view_fields_visible (long al_module, long al_view_id, long al_screen_id, string as_field_name)
end prototypes
...
Do While ll_Start > 0
	ls_Next = UPPER(mid(as_Source,ll_Start + ll_OldLen,1))
	// replace as_Old with as_New and replace the next letter of the as_old with uppercase
	as_Source = Replace(as_Source, ll_Start, ll_OldLen + 1, as_New + ls_Next)
...
DO WHILE (1=1)
	...
	ls_First = Upper(Left(ls_cName,1))
	if ls_First = "_" then ls_First = ""
	ls_cName = ls_First + mid(ls_cName,2)
	ls_cName = of_globalreplace( ls_cName, "_", " ", true)
	
	ads_view_fields_alias.setitem(ll_R,"c_alias",left(ls_cName,255))
...
FOR ll_i = 1 To ll_Cnt
	...
	IF AppeonGetClientType() = "WEB" THEN
		...
		ls_downurl+= "pictures/"
		ls_FilePath = gs_dir_path + gs_DefDirName + "\pictures\" + ls_fileName
...
END IF
//Added by Nova on 2008-07-29 begin
IF lnvo_filesrv.of_fileread(ls_FilePath , lblb_data) = -1 THEN
	lb_Flag = True
	CONTINUE
END IF
//Added by Nova on 2008-07-29 end 
...
public function long of_update_view_fields_alias (long al_module, long al_data_view_id);Return of_update_view_fields_alias(al_module,al_data_view_id, False )
end function
...
//Modified by Ken.Guo 06/11/2014
Return of_update_view_fields_alias(al_module,al_data_view_id,ab_force_update,'')
...
public function integer of_update_view_fields_alias (long al_module, long al_data_view_id, boolean ab_force_update, string as_filter);//====================================================================
// Function: of_update_view_fields_alias
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:         Ken.Guo           Date: 06/11/2014
//--------------------------------------------------------------------
...
//Copy from of_update_view_fields_alias(al_module,al_data_view_id,ab_force_update)
...
n_cst_datastore lds_screen_all
String ls_Filter,ls_Error
Long ll_R,ll_ScreenAll,ll_data_view_id,ll_aCnt, ll_fields_alias_count
boolean  lb_Flag
...
lds_view_alias.Setfilter(as_filter) //Added By Ken.Guo 06/11/2014. to filter the View. empty string means is all.
...
FOR ll_n = 1 to ll_Cnt
	...
	FOR ll_i = 1 to ll_ScreenCnt
		...
		if Not IsNumber(ls_screenID) then
			...
			ll_aCnt = 0
//			select TOP 1 1 INTO :ll_aCnt FROM view_fields_alias 
//				where data_view_id = 0 AND v_name = :ls_v_name;									//Commented By Mark Lee 05/26/2015 if it is not use the ll_aCnt value, so no need the select command.
			//No fields alias
	//		IF ll_aCnt = 0 THEN	
				of_get_view_fields_alias(ll_module,0,ls_screenID,ls_v_name,lds_view_fields_alias)
	//		END IF
			...
			DO WHILE (1=1)
				...
				ll_data_view_id = lds_screen_all.GetItemNumber(ll_R,"data_view_id")
				ll_aCnt = 0
				w_infodisp.st_1.text = 'Update ' + string(ll_n) + ' of ' + string(ll_Cnt)+ ', (View ID =' + String(ll_data_view_id) +'), Please stand by!'				
//				select TOP 1 1 INTO :ll_aCnt FROM view_fields_alias 
//					where data_view_id = :ll_data_view_id AND v_name = :ls_v_name;				//Commented By Mark Lee 05/26/2015 if it is not use the ll_aCnt value, so no need the select command.
				//No fields alias
//				IF ll_aCnt = 0 THEN
					of_get_view_fields_alias(ll_module,ll_data_view_id,ls_screenID,ls_v_name,lds_view_fields_alias)
//				end if
		...
		//Added By Ken.Guo 01/19/2012. 
		of_after_getaliasformscreen(lds_view_fields_alias)
		ll_fields_alias_count = lds_view_fields_alias.rowcount()
		for i = 1 to lds_view_fields_alias.rowcount()
			ls_c_name = lds_view_fields_alias.GetItemString(i,"c_name")
			ll_data_view_id = lds_view_fields_alias.GetItemNumber(i,'data_view_id')
			ll_Find   = lds_view_fields_alias_update.Find("lower(c_name) ='" + lower(ls_c_name) + "' and data_view_id = " + String(ll_data_view_id) + " and lower(v_name) = '" +Lower(ls_v_name)+ "'" ,1,lds_view_fields_alias_update.rowcount())
			
			w_infodisp.st_1.text = 'Update ' + string(ll_n) + ' of ' + string(ll_Cnt)+ ', (Fields ' + String(i)+'/' + String(ll_fields_alias_count) +'), Please stand by!'				
			if ll_Find < 1 then
				//if field alias is not exist, then insert it.
				lds_view_fields_alias.Rowscopy(i,i,Primary!,lds_view_fields_alias_update,lds_view_fields_alias_update.RowCount() + 1,Primary!)
			else
...
public function integer of_update_view_fields_visible (long al_module, long al_view_id, long al_screen_id, string as_field_name);//====================================================================
// Function: of_update_view_fields_visible
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 05/27/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2015 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
//	al_module =1  :contractlogix  ; al_module=2  intelliCred/App
////Modified By Mark Lee 01/25/2016 syntax bug
//====================================================================
string						ls_dataobject, ls_grid_dataobject, ls_screen
integer					li_visible

if isnull(al_module) or isnull(al_view_id) or isnull(al_screen_id) then 
	return 0
end if

if isnull(as_field_name) or trim(as_field_name) = "" then 
	return 0
end if 

SELECT dataobject,grid_dataobject
Into :ls_dataobject, :ls_grid_dataobject
FROM ctx_screen  
Where screen_id = :al_screen_id; 

if isnull(ls_dataobject) then ls_dataobject = " "
if isnull(ls_grid_dataobject) then ls_grid_dataobject = " "

setnull(li_visible)
ls_screen = string(al_screen_id)

Select view_fields_alias.visible
Into :li_visible
From view_alias, view_fields_alias
Where view_alias.v_name = view_fields_alias.v_name 
And view_fields_alias.data_view_id = :al_view_id 
And lower(view_fields_alias.c_name)= :as_field_name and view_alias.module = :al_module
And ( CHARINDEX(:ls_screen+',', view_alias.screen+',') > 0 or  CHARINDEX(:ls_dataobject+',', view_alias.screen+',') > 0 
	or  CHARINDEX(:ls_grid_dataobject+',', view_alias.screen+',') > 0  )  ;
	
commit;

if not isnull(li_visible) and li_visible=0 then 
	...
	if sqlca.sqlcode <> 0 then 
		gnv_debug.of_output( true, " Exceute the of_update_view_fields_visible() function failture. sqlerr: "+sqlca.sqlerrtext  )
	end if 
end if 
