...
ls_table = Mid( ls_table, 5, 100 ) 


ls_screen_name = dw_fields.GetItemString( 1, "screen_alias" )
li_screen_len = Len( ls_screen_name )
FOR l = 1 TO li_screen_len
	CHOOSE CASE Mid( ls_screen_name, l, 1 )
	CASE " ", "!","@","#","$","%","^","&","*","(",")","-","=","+","[","]","{","}","\","/","//","|","/","?",",","<",">",".",":",";"
		ls_screen_name = Mid( ls_screen_name, 1, l -1 ) + Mid( ls_screen_name, l+1, 500 )
	END CHOOSE
END FOR
...
FOR i = 1 TO li_rc	
	...
	CHOOSE CASE Mid( ls_field_labels[i], 1, 1 )
		CASE "1","2","3","4","5","6","7","8","9","0"
			ls_field_labels[i] = Mid( ls_field_labels[i], 2, 500 )
		CASE "32234234"		
	END CHOOSE		

	li_fld_len = Len( ls_field_labels[i] )
	IF li_fld_len = 0 THEN
		ls_field_labels[i] = ls_field_orig	
	END IF
		
	FOR fl = 1 TO li_fld_len
		IF Len( ls_field_labels[i] ) = 0 THEN
			ls_field_labels[i] = ls_field_orig
			CONTINUE
		END IF
		ls_char = Upper( Mid( ls_field_labels[i], fl, 1 ) )
		CHOOSE CASE ls_char
			CASE " ", "!","@","#","$","%","^","&","*","(",")","-","=","+","[","]","{","}","\","/","|","?",",","<",">",".",":",";"
				ls_field_labels[i] = Mid( ls_field_labels[i], 1, fl -1 ) + Mid( ls_field_labels[i], fl + 1 , 500 )
				fl --
			CASE "42423423"
		END CHOOSE
	END FOR
	...
	CHOOSE CASE  Upper( ls_field_labels[i] )
		...
		CASE "342342342342"	
	END CHOOSE		
	ls_field_labels[i] = Mid( ls_field_labels[i], 1, 30 )
...
IF li_code_lookup_cnt + li_address_lookup_cnt = 0 THEN 
	ls_view_where_sql = "" 
	ls_field_sql = Mid( ls_field_sql, 1, Len( ls_field_sql ) -3 )
...
ELSE
	...
	ls_view_field_sql = Mid( ls_view_field_sql + ls_field_sql, 1, Len( ls_view_field_sql + ls_field_sql )-3 )
	ls_view_from_sql =  Mid( ls_view_from_sql +  ls_from_sql, 1, Len( ls_view_from_sql +  ls_from_sql )-3 )
	ls_view_where_sql = Mid( ls_view_where_sql + ls_where_sql, 1, Len( ls_view_where_sql + ls_where_sql )-6 )
