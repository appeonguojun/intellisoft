...
FOR li_module = 1 to 3
	...
	FOR i = 1 to ll_cnt_mailto
		...
		DO WHILE ll_found > 0 
			...
			IF Not IsNull(ls_mailid) AND Len(Trim(ls_mailid)) > 0 THEN 
				IF Len(Trim(ls_mailto)) > 0 THEN ls_mailto += "; "
				ls_mailto += ls_mailid
			END IF
			//Fixed By Jervis 04.23.2009
		...
		IF Len(Trim(ls_mailto)) > 0 THEN lds_alm_mailto.SetItem(i, 'alm_mailto', ls_mailto)
	NEXT
