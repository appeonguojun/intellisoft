...
string title = "Program Database Updates "
...
public function integer of_ver_91_updates ()
...
if cnt < 1 then 
	if not FileExists(gs_dir_path + gs_DefDirName + "\sys_fields_for_addr_det_2.txt") = true then return -1
	...
	dw_whatever.reset()
	cnt = dw_whatever.importfile( gs_dir_path + gs_DefDirName + "\sys_fields_for_addr_det_2.txt" )
...
if cnt < 1 then 
	if not FileExists(gs_dir_path + gs_DefDirName + "\data_fields_for_addr_det_2.txt") = true then return -1
	...
	dw_whatever.SetTransObject( SQLCA	)
	dw_whatever.reset()
	cnt = dw_whatever.importfile( gs_dir_path + gs_DefDirName + "\data_fields_for_addr_det_2.txt" )
	for n = 1 to cnt
		recid++
...
execute immediate :ls_sql using sqlca;//071502 maha
commit using sqlca;
ls_sql = "INSERT INTO sys_fields ALUES (	43,	100015,	150,	'date_app_sent',	'D',	20,	'App Sent Date',	NULL,	NULL,	NULL,	NULL,	NULL,	NULL,	NULL,	NULL,	NULL,	NULL,	'Y',	NULL,	NULL,	NULL,	NULL,	NULL,	NULL,	NULL);"
execute immediate :ls_sql using sqlca;//071502 maha
commit using sqlca;
...
ls_file = gs_dir_path + gs_DefDirName + "\question_lookup_506.txt"
...
if FileExists( gs_dir_path + gs_DefDirName + "\gpid dataview.txt" ) = false then
...
if FileExists( gs_dir_path + gs_DefDirName + "\priv status lookup.txt" ) = false then
...
li_rc = dw_2.ImportFile( gs_dir_path + gs_DefDirName + "\priv status lookup.txt" )
...
if vi < 1 then
	...
	for i = 1 to ic
		...
		dw_fields.ImportFile( gs_dir_path + gs_DefDirName + "\gpid dataview.txt" )
...
if cnt < 1 then
	li_rc = dw_2.ImportFile( gs_dir_path + gs_DefDirName + "\affil_stat_lookup.txt" )
...
cb_update.text = "Report Writer Security"
st_update.text  = "Report Writer Security"
...
public function integer of_update_reports ();//Obsolete - Alfee 01.30.2008
...
rpt_path = gs_dir_path + gs_DefDirName + "\reports_v95.txt"				//Modified by Scofield on 2008-06-11
wiz_path = gs_dir_path + gs_DefDirName + "\report_wiz_v95.txt"			//Modified by Scofield on 2008-06-11
...
elseif li_ver <= 80 then     //80
	of_ver80_updates()
elseif li_ver <= 85 then     //85 - alfee 07.11.2008
	of_ver85_updates()	
elseif li_ver <= 90 then     //90  Ken.Guo 01-06-2009
	of_ver90_updates()
elseif li_ver <= 95 then     //95  Scofield 2009-08-12
	of_ver95_updates()
elseif li_ver <= 100 then     //100  //Added By Ken.Guo 2010-03-25.
	of_ver100_updates()		
elseif li_ver <= 105 then     //105  //Added By jervis 05.13.2011
	of_ver105_updates()		
elseif li_ver <= 110 then     //105  //Added By jervis 05.13.2011
	of_ver110_updates()			
elseif li_ver <= 115 then     //115  //Added By Mark Lee 07/10/12
	of_ver115_updates()				
elseif li_ver <= 120 then     //120  //Added By Mark Lee 10/19/12
	of_ver120_updates()		
elseif li_ver <= 130 then     //130  //Added By Mark Lee 08/14/2013
	of_ver130_updates()		
elseif li_ver <= 140 then		//140  //Added By Mark Lee 03/03/2014
	of_ver140_updates()
elseif li_ver <= 150 then		//150  //Added By Mark Lee 09/16/2014
	of_ver150_updates()
end if
...
string s //maha050504
s = gs_dir_path + gs_DefDirName + "\Fax"
...
commit using sqlca;
ls_sql = "update ids set citrix_npdb_path = '" + gs_dir_path + gs_DefDirName + "\' where citrix_npdb_path is null;"
st_sql.text  = ls_sql
execute immediate :ls_sql using sqlca;//042705 maha 
commit using sqlca;
...
//Call update for ver85 - Alfee 07.18.2008
of_ver85_updates() 

Return 1
end function

public function integer of_ver85_updates ();//Added by Alfee on 07.11.2008
...
//1.Update sql for 8.5
IF gs_dbtype = 'ASA' THEN
	lnv_asa = Create n_cst_update_asa
	lnv_asa.of_ver85_updates()
ELSE
	lnv_sql = Create n_cst_update_sql	
	lnv_sql.of_ver85_updates()
END IF
lnv_data = Create n_cst_update_data 
lnv_data.of_ver85_updates()

lnvo_role_right.of_update_version_85() //Added By Ken.Guo 2008-11-20

//4.1 Update alarm notification users for 8.5 - 08.20.2008
...
of_ver90_updates() //Added by Ken.Guo on 2009-01-05

Return 1
end function

public function integer of_ver_91_updates ();//Start Code Change ----11.06.2008 #V85 maha
...
public function integer of_ver90_updates ();//////////////////////////////////////////////////////////////////////
// $<function>w_priv_updateof_ver90_updates()
// $<arguments>(None)
// $<returns> integer
// $<description>
// $<description>
...
//1.Update sql for 9.0
IF gs_dbtype = 'ASA' THEN
	lnv_asa = Create n_cst_update_asa
	lnv_asa.of_ver90_updates()
ELSE
	lnv_sql = Create n_cst_update_sql	
	lnv_sql.of_ver90_updates()
END IF

//2.Update data for 9.0
lnv_data.of_ver90_updates()

//3.Update role rights for 9.0
lnvo_role_right.of_update_version_90( ) 
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Scofield		Date: 2009-08-12
//--------------------------------------------------------------------
...
//1.Update sql for 9.5
IF gs_dbtype = 'ASA' THEN
	lnv_asa = Create n_cst_update_asa
	lnv_asa.of_ver95_updates()
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver95_updates()
END IF

//2.Update data for 9.5
lnv_data.of_ver95_updates()

//3.Update role rights for 9.5
lnvo_role_right.of_update_version_95( ) 
...
public function integer of_ver100_updates ();//////////////////////////////////////////////////////////////////////
// $<function>w_priv_updateof_ver100_updates()
// $<arguments>(None)
// $<returns> integer
// $<description>
// $<description>
...
//1.Update sql for 10.0
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver100_updates()
END IF

//2.Update data for 10
lnv_data.of_ver100_updates()

//3.Update role rights for 10
lnvo_role_right.of_update_version_100( ) 
...
public function integer of_ver105_updates ();//////////////////////////////////////////////////////////////////////
// $<function>w_priv_updateof_ver105_updates()
// $<arguments>(None)
// $<returns> integer
// $<description>
// $<description>
//////////////////////////////////////////////////////////////////////
// $<add> 2011-05-13 Jervis
//////////////////////////////////////////////////////////////////////
...
//1.Update sql for 10.5
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver105_updates()
END IF

//2.Update data for 10.5
lnv_data.of_ver105_updates()

//3.Update role rights for 10.5
lnvo_role_right.of_update_version_105( ) 
...
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver110_updates()
END IF

//2.Update data for 10
lnv_data.of_ver110_updates()

//3.Update role rights for 10
lnvo_role_right.of_update_version_110( ) 
...
//1.Update sql for 11.5
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver115_updates()
END IF

//2.Update data for 115
lnv_data.of_ver115_updates()

//3.Update role rights for 115
lnvo_role_right.of_update_version_115( ) 
...
//1.Update sql for 12.0
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver120_updates()
END IF

//2.Update data for 120
lnv_data.of_ver120_updates()

//3.Update role rights for 120
lnvo_role_right.of_update_version_120( ) 
...
//1.Update sql for 13.0
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver130_updates()
END IF

//2.Update data for 130
lnv_data.of_ver130_updates()

//3.Update role rights for 130
lnvo_role_right.of_update_version_130( ) 
...
//1.Update sql for 14.0
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver140_updates()
END IF

//2.Update data for 140
lnv_data.of_ver140_updates()

//3.Update role rights for 140
lnvo_role_right.of_update_version_140( ) 
...
of_ver150_updates()		//Added By Mark Lee 09/15/2014
...
//1.Update sql for 15.0
IF gs_dbtype = 'ASA' THEN
ELSE
	lnv_sql = Create n_cst_update_sql
	lnv_sql.of_ver150_updates()
END IF

//2.Update data for 150
lnv_data.of_ver150_updates()

//3.Update role rights for 150
lnvo_role_right.of_update_version_150( ) 
...
//SQLCA.DBParm = "Driver='com.sybase.jdbc2.jdbc.SybDriver',URL='jdbc:sybase:Tds:192.0.3.102:2510/IntelliCred_MtSinai'"
//connect;
//gs_dbtype = "ASA"
//
//
...
event clicked;//---------Modified by Alfee 01.30.2008 ----------------------------------------
...
//--------------------------- APPEON BEGIN ---------------------------
//$<add> 11.24.2008 By: Ken.Guo. Need Reopen IE.
Integer li_maxscript = 1
Select ie_maxscript Into :li_maxscript  From icred_settings;
If li_maxscript = 0 Then
	RegistrySet( "HKEY_CURRENT_USER\Software\Microsoft\Internet Explorer\Styles", "MaxScriptStatements", RegULong!, 5000000)
Else
	RegistrySet( "HKEY_CURRENT_USER\Software\Microsoft\Internet Explorer\Styles", "MaxScriptStatements", RegULong!, 500000000)	
End If
//---------------------------- APPEON END ----------------------------
...
//4.5 Refresh data
app_filler.ids_screen_fields_cache.settransobject(sqlca) //Added By Ken.Guo 2015-08-04. for multi db update
app_filler.ids_screen_objects_cache.settransobject(sqlca)//Added By Ken.Guo 2015-08-04 for multi db update
gnv_appeondb.of_startqueue( )
...
//Added by Nova on 2008-08-29 refresh profile
//Modified by Alan on 2008-10-16 Moved to pfc_cst_u_data_entry_report_painter/pfc_cst_u_reports
...
lnvo_screen.of_appeon2_changes(false)
Destroy lnvo_screen

//$<add> 04.24.2008 by nova
//fixed issue 5 from IntelliReport Issues 4-22-08.doc
n_cst_update_data lnv_data
lnv_data = Create n_cst_update_data

lnv_data.of_patch_screen( )//added by gavins 20120712
...
//Added By Ken.Guo 2009-10-12. get old version
String ls_current_version,ls_current_version_patch
Select set_18,set_patch_ver into :ls_current_version,:ls_current_version_patch From icred_settings;

//$<add> 02.19.2008 by Andy
//7.Update view fields alias,Screen Shot 
...
lnv_data.of_ver80_fix_view_alias( )
//04.24.2008 by nova

//Added By Mark Lee 03/20/2013 for update the view_fields_alias data in the auto upgrade.
//If Long(ls_current_version) < 110 Then //Added By Ken.Guo 08/22/2012. Added if statement. run only once, it spend too long time.
If ( Long(ls_current_version) < 110 ) Or ( Long(ls_current_version) <= 120 and long(ls_current_version_patch) <= 37 )  Then
	lnvo_update.of_update_view_fields_alias( 0, 0)
End If
	
//$<add> 04.24.2008 by nova
//fixed issue 5 from IntelliReport Issues 4-22-08.doc
lnv_data.of_ver80_fix_view_fields_alias( )

//Check Update IDs -- jervis 06.26.2009
lnv_data.of_check_ids( )

//Added By Mark Lee 07/11/12
lnv_data.inv_clause.of_ini_data( )
...
IF sqlca.SQLCode <> 0 THEN
	...
	IF IsValid(gnv_debug) THEN gnv_debug.of_output(true,"cb_update.clicked(): " + ls_sql + " Sql error text: " +  Sqlca.SQLErrText)
	gb_upgrade_failed = True
END IF
...
End If
ls_old_db_version = ls_current_version + '.' + ls_current_version_patch
...
Else
	...
	End If
End If
Insert Into db_upgrade_log (upgrade_by,upgrade_date,old_db_version,new_db_version,app_version,from_cache_name,upgrade_result,comment) 
...
if IsValid(Parent) then Close(Parent)			//Modified by Scofield on 2009-10-09
...
IF ls_development = '1' THEN //develop mode
	IF gb_upgrade_failed THEN
		ls_logfile = gs_dir_path + gs_DefDirName + "\dbupdate.log"
...
IF ls_if_show <> 'M' THEN//when is_parm is 'M',don't show message for custerm
	...
	if i = 1 then
		...
		f_EmailSettingRemind(False) //Add by Evan on 04/29/2008
	end if
