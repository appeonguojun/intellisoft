...
//		Messagebox('OCR Failure','Failed to delete the temp file ' + ls_temp_file + '.')
		if gb_recordlog = true then 
			gs_errmsg = 'OCR Failure ->Failed to delete the temp file ' + ls_temp_file + '.Possible cause:the file is read-only.'
		else 
			Messagebox('OCR Failure','Failed to delete the temp file ' + ls_temp_file + '.Possible cause:the file is read-only.')
		end if 
		Return ''
	End If
End If
