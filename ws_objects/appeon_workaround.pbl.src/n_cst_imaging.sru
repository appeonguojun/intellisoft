...
public subroutine of_refresh_path_regedit ()
...
of_refresh_path_regedit() //Added By Ken.Guo 2009-08-06.
...
if pos(appeonGetOSType(),"5.0") > 0 then return true
...
if of_get_flag() then 
	...
	if lb_ret then
		...
		of_registry_imaging() //Moved By Ken.Guo 2009-08-03. for workarond original bug for path.
...
string ls_version,ls_path,ls_ocx_path
integer li_pos

//Modified By Ken.Guo 2009-08-06
lnv_appeon.of_registryget("HKEY_LOCAL_MACHINE\SOFTWARE\eiStream\Imaging for Windows", "Version", RegString!, ls_version) 
lnv_appeon.of_registryget("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\imaging.exe","Path",RegString!,ls_path) 
lnv_appeon.of_registryget("HKEY_LOCAL_MACHINE\SOFTWARE\Classes\CLSID\{6D940280-9F11-11CE-83FD-02608C3EC08A}\InprocServer32","",RegString!,ls_ocx_path) 
If ls_ocx_path <> '' Then
	li_pos = lastpos(ls_ocx_path,'\')
	ls_ocx_path = Mid(ls_ocx_path,1,li_pos)
End If

if ls_version = "2.80.0.5510w" and lower(ls_path) = lower(ls_ocx_path) and ls_path <> '' then 
	return true
else
	return false
end if
...
public function boolean of_get_flag ();IF gs_image_flag = '1' then return true
return false
...
if lnv_appeondll.of_registryget("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\KodakImgV1", "DisplayName", RegString!, ls_version) = 1 Then
	...
	If	Pos(Upper(ls_version),Upper('Imaging for Windows')) > 0 Then 
...
public subroutine of_get_imageocx ();
gi_imageocx = 2 //Added By Ken.Guo 2009-11-09. Not use Image 360.
Return

/*
//Select an Image OCX - Added by Alfee on 05.19.2008 
IF appeongetclienttype() = 'PB' THEN //For Window version
	//IF of_check_windows_imaging() OR Pos(appeonGetOSType(),"5.0") > 0 THEN //06.10.2008	//Delete Pos(appeonGetOSType(),"5.0"),because windows 2000 No OCR. Ken.Guo 2009-03-25
	IF of_check_windows_imaging() THEN //06.10.2008	
   	gi_imageocx = 1 //Imaging 360
	ELSE   
		gi_imageocx = 2 //New image control
	END IF
ELSE 	//For Web version
	//IF of_check_windows_imaging() OR Pos(appeonGetOSType(),"5.0") > 0 OR &  //Delete Pos(appeonGetOSType(),"5.0"),because windows 2000 No OCR.  Ken.Guo 2009-03-25
	IF of_check_windows_imaging()  OR & 
...
IF lb_prompt THEN 
	...
	ELSE
		...
		IF SQLCA.SQLCode <> 0 THEN
			MessageBox("ContractLogix","Update license flag of Image for Windows Failed!")
			RETURN 0
		END IF
	END IF
END IF
...
public subroutine of_refresh_path_regedit ();n_cst_appeondll lnv_appeondll
String ls_path
Long ll_pos

//Get imgedit.ocx regist path
lnv_appeondll.of_registryget("HKEY_LOCAL_MACHINE\SOFTWARE\Classes\CLSID\{6D940280-9F11-11CE-83FD-02608C3EC08A}\InprocServer32","",RegString!,ls_path) 
If ls_path <> '' Then
	ll_pos = lastpos(ls_path,'\')
	ls_path = Mid(ls_path,1,ll_pos)
	lnv_appeondll.of_registrySet("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\imaging.exe","Path",RegString!,ls_path)
Else
	lnv_appeondll.of_registrySet("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\imaging.exe","Path",RegString!,LOCAL_PATH)
End If
end subroutine
...
//$'C:\Documents and Settings\Administrator\Application Data\appeon\192.0.3.37_Dotnet_Contract Logix8\plugin'

//LOCAL_PATH = gs_dir_path + gs_DefDirName + "\Imaging\" 
string ls_Path
ls_Path = Space(255)
