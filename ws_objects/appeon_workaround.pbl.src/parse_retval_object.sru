...
IF li_type = EAG_CHAR THEN
	retval = Char(BlobMid(bl, ll_CurrPos, 1))
ELSE
...
IF li_arrTag = EAG_ARRAY AND EAG_CHAR = li_type THEN
	...
	FOR i = 1 TO li_dimension*li_upperbound
		retval[i] = Char(BlobMid(bl, ll_currPos, 1))
		ll_currPos += 1;
	NEXT
...
// Struct Len And Name
BlobEdit(lbl_Long, 1, Long(Len(as_name)))
lbl_head += lbl_Long
...
FOR i = 1 TO li_Num
	...
	BlobEdit(lbl_Long, 1, Long(Len(ls_varName)))
	blobRet += lbl_Long
