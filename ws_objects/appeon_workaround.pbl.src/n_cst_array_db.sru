...
If ai_array_type = 1 Then //Numeric Type
	...
	For i = 1 To ll_arr_count
		...
		If Len(ls_text[j]) > 2000 Then //vchar need less than 8000 in DB, and JS run slow performance when large string var add other string, so set total is 2000.
			ls_text[j] = Mid( ls_text[j],1,(Len(ls_text[j]) - 1) ) //Delete last char ';'
			J++
		End If		
	Next
	ls_text[j] = Mid( ls_text[j],1,(Len(ls_text[j]) - 1) ) //Delete last char ';'
