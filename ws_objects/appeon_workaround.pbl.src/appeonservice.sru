...
nPos = Pos(strPath, 'repository')

IF nPos > 0 THEN
	strPath = Left(strPath, nPos - 2)
ELSE
	strPath = ''
END IF
...
astr_obj.s_type = Mid(as_xml,ll_Start[1],ll_len[1])
astr_obj.s_dataobject = Mid(as_xml,ll_Start[2],ll_len[2])
astr_obj.s_pbsyntax = Mid(as_xml,ll_Start[3],ll_len[3])
astr_obj.s_matchtype = Mid(as_xml,ll_Start[4],ll_len[4])
astr_obj.l_SetCount = long(Mid(as_xml,ll_Start[5],ll_len[5])) - 1
astr_obj.s_rowset = Mid(as_xml,ll_Start[6],ll_len[6])

//primary
astr_obj.s_pdata = Mid(as_xml,ll_Start[7],ll_len[7])
astr_obj.s_pstate = Mid(as_xml,ll_Start[8],ll_len[8])
astr_obj.s_pflag = Mid(as_xml,ll_Start[9],ll_len[9])

//filter
astr_obj.s_fdata = Mid(as_xml,ll_Start[10],ll_len[10])
astr_obj.s_fstate = Mid(as_xml,ll_Start[11],ll_len[11])
astr_obj.s_fflag = Mid(as_xml,ll_Start[12],ll_len[12])

//delete
astr_obj.s_ddata = Mid(as_xml,ll_Start[13],ll_len[13])
astr_obj.s_dstate = Mid(as_xml,ll_Start[14],ll_len[14])
astr_obj.s_dflag = Mid(as_xml,ll_Start[15],ll_len[15])

//web syntax
astr_obj.s_fullstate = Mid(as_xml,ll_Start[16],ll_len[16])
...
astr_obj.s_type = Mid(as_xml,ll_Start[1],ll_len[1])
astr_obj.s_dataobject = Mid(as_xml,ll_Start[2],ll_len[2])
astr_obj.s_fullstate = Mid(as_xml,ll_Start[3],ll_len[3])

if lb_c then
	astr_obj.s_url = Mid(as_xml,ll_Start[4],ll_len[4])
	astr_obj.i_count = long(Mid(as_xml,ll_Start[5],ll_len[5]))
	astr_obj.s_sql = Mid(as_xml,ll_Start[6],ll_len[6])
	astr_obj.s_filter = Mid(as_xml,ll_Start[7],ll_len[7])
	astr_obj.s_sort = Mid(as_xml,ll_Start[8],ll_len[8])
end if
