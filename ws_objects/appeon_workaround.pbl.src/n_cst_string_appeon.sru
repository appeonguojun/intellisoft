...
public function integer of_arraytostring (string as_source[], string as_mark, string as_delimiter, ref string as_ref_string)
...
If ll_pos > 0 Then
	Do While ll_pos > 0
		ls_filepath	= ls_filepath + left(as_pathname , ll_pos)
		as_pathname = mid(as_pathname , ll_pos + 1)
		ll_pos = pos(as_pathname , '\')
	Loop
	If len(as_pathname) > 0 Then as_filename = as_pathname
...
CHOOSE CASE Upper(as_type)
	CASE 'C', 'D' //for char, datetime type
		...
		ELSE
			...
			DO WHILE ll_pos > 0
				ls_criteria = Replace(ls_criteria, ll_pos, 1, "','")
				ls_criteria = Left(ls_criteria, ll_Pos + 2) + Trim(Mid(ls_criteria, ll_Pos + 3)) 
				ll_pos = Pos(ls_criteria, ",", ll_Pos + 3)
			LOOP
		END IF		
	CASE ELSE
		//
...
CHOOSE CASE Upper(as_type)
	CASE 'C', 'S'  //for char, datetime type only //add 'S', move 'D' - jervis 10.19.2010
		
		//Check and Add singe quotes if needed		
		IF IsNUll(as_criteria) THEN as_criteria = ""
		IF Len(Trim(as_criteria)) <= 1 THEN RETURN "'" + as_criteria + "'"
		
		ls_criteria = Trim(as_criteria)	
		IF Left(ls_criteria, 1) = "'" AND Right(ls_criteria, 1) = "'" THEN 	  //sigle quotes
			//Do nothing
		ELSEIF Left(ls_criteria, 1) = '"' AND Right(ls_criteria, 1) = '"' THEN //double quotes
			IF Len(ls_criteria) = 2 THEN ls_criteria = "''"
			IF Len(ls_criteria) > 2 THEN ls_criteria = "'" + Mid(ls_criteria, 2, Len(ls_criteria) - 2) + "'"
		ELSE 																						  //No quotes
			ls_criteria = "'" + as_criteria + "'"
		END IF	
