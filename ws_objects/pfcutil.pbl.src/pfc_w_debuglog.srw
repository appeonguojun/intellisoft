...
boolean		ib_message	= false			//Added By Mark Lee 04/03/2015
end variables
...
public subroutine of_sharedata ()
public function integer of_setmessage (boolean ab_switch)
end prototypes
...
public subroutine of_sharedata ();integer 			li_rc

if isvalid(gnv_debug) then
	// If appropriate force this window to be on top.
	of_SetAlwaysOnTop(gnv_debug.of_GetAlwaysOnTop())
	...
	li_rc = gnv_debug.ids_debuglog.ShareData (dw_debuglog)
	If li_rc <> 1 Then
		of_MessageBox ("pfc_debuglog_failedsharedata", "PowerBuilder Class Library", &
			"Open on pfc_w_debuglog event has received a of_sharedata() fail code.  "+ &
...
if isvalid(gnv_app.inv_debug) and ib_message	= false then 			//Added By Mark Lee 03/30/2015
	// If appropriate force this window to be on top.
...
if isvalid(gnv_app.inv_debug) and ib_message	= false then 
	gnv_app.inv_debug.of_PrintLog()
else
	gnv_debug.of_Printlog( )
end if 
