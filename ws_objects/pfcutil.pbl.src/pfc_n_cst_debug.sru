...
public function string of_title (boolean abln_change, string as_title)
...
//--------------------------------------------------------------------
// Arguments:
// 	value    boolean    abln_change
// 	value    string    as_title
//--------------------------------------------------------------------
// Returns:  string
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 03/30/2015
//--------------------------------------------------------------------
...
if isvalid(w_debugLog) then
	if abln_change then 
		if not isnull(as_title) then 
			w_debugLog.title = string(as_title)
		end if 
	else
		ls_return = w_debugLog.title
	end if 
end if 
return ls_return
end function
...
If IsValid(w_debuglog) Then
	w_debuglog.of_setmessage(ab_switch)
End If
