$PBExportHeader$w_contract_data_views_manager1.srw
forward
global type w_contract_data_views_manager1 from w_sheet
end type
type cb_unselected from commandbutton within w_contract_data_views_manager1
end type
type cb_selected from commandbutton within w_contract_data_views_manager1
end type
type cb_renameview from u_cb within w_contract_data_views_manager1
end type
type cb_tabprop from u_cb within w_contract_data_views_manager1
end type
type cb_8 from commandbutton within w_contract_data_views_manager1
end type
type cb_7 from commandbutton within w_contract_data_views_manager1
end type
type cb_6 from commandbutton within w_contract_data_views_manager1
end type
type cb_5 from commandbutton within w_contract_data_views_manager1
end type
type cb_4 from commandbutton within w_contract_data_views_manager1
end type
type cb_3 from commandbutton within w_contract_data_views_manager1
end type
type cb_2 from commandbutton within w_contract_data_views_manager1
end type
type cb_1 from commandbutton within w_contract_data_views_manager1
end type
type tv_selected from u_contract_screen within w_contract_data_views_manager1
end type
type tv_available from u_contract_screen within w_contract_data_views_manager1
end type
type cb_copyscreen from u_cb within w_contract_data_views_manager1
end type
type cb_add from u_cb within w_contract_data_views_manager1
end type
type cb_createview from u_cb within w_contract_data_views_manager1
end type
type cb_copyview from u_cb within w_contract_data_views_manager1
end type
type cb_save from u_cb within w_contract_data_views_manager1
end type
type cb_close from u_cb within w_contract_data_views_manager1
end type
type cb_remove from u_cb within w_contract_data_views_manager1
end type
type cb_deleteview from u_cb within w_contract_data_views_manager1
end type
type dw_view from u_dw within w_contract_data_views_manager1
end type
type gb_view from groupbox within w_contract_data_views_manager1
end type
type gb_available from groupbox within w_contract_data_views_manager1
end type
type gb_selected from groupbox within w_contract_data_views_manager1
end type
end forward
...
global w_contract_data_views_manager1 w_contract_data_views_manager1
...
public function integer of_copy_screen (long al_new_view_id, long al_screen_id, long al_old_view_id)
end prototypes
...
public function integer of_copy_screen (long al_new_view_id, long al_screen_id, long al_old_view_id);//====================================================================
// Function: of_copy_screen
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    long    al_new_view_id
// 	value    long    al_screen_id
// 	value    long    al_old_view_id
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Mark Lee		Date: 04/15/2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 Contractlogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
// move it form cb_copyscreen clicked event.
//====================================================================

datetime ldt_modify_date 

if isnull(al_new_view_id) or isnull(al_screen_id) or isnull(al_old_view_id)  or & 
	 al_new_view_id = 0 or al_screen_id = 0 or al_old_view_id = 0  then return -1 
...
delete ctx_screen_field_ex where data_view_id = :al_new_view_id and screen_id = :al_screen_id;
delete ctx_screen_field_depend where view_id = :al_new_view_id and base_screen_id = :al_screen_id;
delete ctx_screen_datafill where data_view_id = :al_new_view_id and screen_id = :al_screen_id;
DELETE ctx_screen WHERE data_view_id = :al_new_view_id and screen_id = :al_screen_id;

//added by gavins 20120417
delete  from conv_view_pic where view_id = :al_new_view_id and pic_type = 'screen' and screen_id = :al_screen_id;
...
insert into conv_view_pic ( view_id
      ...
      ,pic_tag
	  ,pic_painter_id)
       select :al_new_view_id,
			 screen_id, 
			 pic_type,
			 pic_band, 
			 pic_name, 
			  substring( pic_filename,1,2 ) + cast( :al_new_view_id as varchar(10))  + '_' + cast( screen_id as varchar(10)) +
		   substring( pic_filename, charindex( '_',pic_filename, charindex( '_',pic_filename, 3 ) + 1 ),len(pic_filename) - charindex( '_',pic_filename, charindex( '_',pic_filename, 3 ) ) - 5 )+ convert(varchar(10),DATEPART( ms,getdate())) + right(pic_filename,4 )
		     ,pic_bitmap
			  ...
			  ,pic_tag
			  ,pic_painter_id
			  from conv_view_pic where view_id = :al_old_view_id and screen_id = :al_screen_id and pic_type = 'screen';
//---------------------------------------------------------//
INSERT INTO ctx_screen(data_view_id,screen_id,tab_name,screen_name,dw_name,dataobject,description,dw_sql,create_date,modify_date,force_update,titlebar_visible,titlebar_text,boarder_style,screen_visible, screen_style, grid_sql, grid_dataobject, grid_modify)
	SELECT :al_new_view_id,:al_screen_id,tab_name,screen_name,dw_name,dataobject,description,dw_sql,create_date,:ldt_modify_date,force_update,titlebar_visible,titlebar_text,boarder_style,screen_visible, screen_style, grid_sql, grid_dataobject, grid_modify 
	FROM ctx_screen
	WHERE data_view_id = :al_old_view_id and screen_id = :al_screen_id;
	
//Update the Data Fill settings  - jervis 09.19.2010
//Modified By Jay Chen 12-07-2015 add column data_from
insert into ctx_screen_datafill
SELECT :al_new_view_id, :al_screen_id,   
       field_name,   
        field_name_fill,   
        field_name_map,   
        :gs_user_id,   
        :ldt_modify_date,   
        relative_days,data_from  
    FROM ctx_screen_datafill  
   WHERE ( ctx_screen_datafill.data_view_id = :al_old_view_id ) AND  
         ( ctx_screen_datafill.screen_id = :al_screen_id );   
...
insert into ctx_screen_field_depend (view_id
      ...
      ,base_table
      ,depend_table)
select :al_new_view_id, base_field
      ,depend_field
      ,:al_screen_id   
      ,depend_screen_id
      ,base_field_dispaly
      ...
      ,base_table
      ,depend_table 
from ctx_screen_field_depend where view_id = :al_old_view_id and base_screen_id =:al_screen_id ;

//Added By Jay Chen 10-29-2014 add depend lookup data
delete from ctx_lookup_depend where view_id = :al_new_view_id and screen_id = :al_screen_id;
insert into ctx_lookup_depend (base_lookup_code,depend_lookup_code,depend_type,view_id,screen_id)
select base_lookup_code,depend_lookup_code,depend_type,:al_new_view_id,:al_screen_id
from ctx_lookup_depend
where view_id = :al_old_view_id
...
insert into dbo.ctx_screen_field_ex (data_view_id
      ...
      ,field_name
      ,field_add)
select :al_new_view_id,screen_id
      ,field_name
      ,field_add 
from ctx_screen_field_ex where data_view_id = :al_old_view_id and screen_id = :al_screen_id;
...
on w_contract_data_views_manager1.create
int iCurrent
call super::create
this.cb_unselected=create cb_unselected
...
this.Control[iCurrent+1]=this.cb_unselected
this.Control[iCurrent+2]=this.cb_selected
this.Control[iCurrent+3]=this.cb_renameview
this.Control[iCurrent+4]=this.cb_tabprop
...
this.Control[iCurrent+13]=this.tv_selected
this.Control[iCurrent+14]=this.tv_available
this.Control[iCurrent+15]=this.cb_copyscreen
this.Control[iCurrent+16]=this.cb_add
this.Control[iCurrent+17]=this.cb_createview
this.Control[iCurrent+18]=this.cb_copyview
this.Control[iCurrent+19]=this.cb_save
this.Control[iCurrent+20]=this.cb_close
this.Control[iCurrent+21]=this.cb_remove
this.Control[iCurrent+22]=this.cb_deleteview
this.Control[iCurrent+23]=this.dw_view
this.Control[iCurrent+24]=this.gb_view
this.Control[iCurrent+25]=this.gb_available
this.Control[iCurrent+26]=this.gb_selected
end on

on w_contract_data_views_manager1.destroy
call super::destroy
destroy(this.cb_unselected)
...
//tv_selected.checkboxes = true 				//Added By Mark Lee 04/14/2015
...
inv_resize.of_Register(cb_selected, 50, 100, 0, 0)			//Added By Mark Lee 04/15/2015
inv_resize.of_Register(cb_unselected, 50, 100, 0, 0)
end event
...
event clicked;//Added By Mark Lee 04/14/2015
//tv_selected.event ue_check_action(1)

tv_selected.of_resetselectedpicture( )
end event
...
event clicked;//Added By Mark Lee 04/14/2015
//tv_selected.event ue_check_action(2)

tv_selected.of_resetselectedpicture(true )
end event
...
event clicked;call super::clicked;//Added By Mark Lee 04/14/2015
treeviewitem tvitem

if handle <= 0 or isnull(handle) then return 0

if this.getitem(handle, tvitem) = -1 then return 0

boolean		lbn_select 

if tvitem.PictureIndex <> 1 then	
	if tvitem.pictureindex  = 5 then 
		lbn_select =false 
	else
		lbn_select =true 	
	end if 
else
	lbn_select = of_checkedSelectedPicture(handle)
	lbn_select = not lbn_select
end if 

this.of_SetSelectedPicture(handle, lbn_select)
...
//Modified By Mark Lee 04/16/2015
...
//	Return
//end if
string			ls_data[], ls_parentdata[], ls_label[], ls_text
long			ll_count, i,j
tv_selected.of_getselectedindexes( 0, ls_data, ls_parentdata, ls_label)
ll_count = upperbound(ls_data)
if isnull(ll_count) or ll_count <=0 then 
	MessageBox("Copy Screen", "No screen can be copy.")
	Return
end if 

gnv_string.of_arraytostring( ls_label, " , ",ls_text)
ls_text = "("+string(ll_count)+") " + ls_text

// Open window and get data view id
//OpenWithParm(w_contract_copyscreen, ltvi_item.Label)
OpenWithParm(w_contract_copyscreen, ls_text)

if Message.StringParm = "cancel!" then Return

//Modified By Ken.Guo 2016-03-03. support multi views
//ll_new_view_id = Long(Message.StringParm)
String ls_data_view
Long ll_new_view_id_arr[]
ls_data_view = Message.StringParm
gnv_string.of_parsetoarray( ls_data_view, ',',ll_new_view_id_arr[])
If Upperbound(ll_new_view_id_arr[]) = 0 Then
	Messagebox('Error','Faild to get the view list.')
	Return
End If
...
//dw_view.Event ue_SetItem(ll_new_view_id)				//Modified By Mark Lee 04/15/2015 no need it
...
//	if tv_selected.of_Add(ls_tab_name, ls_folder_name, tv_available) = 1 then
	if tv_selected.of_Add(ls_parentdata[i], ls_label[i], tv_available) = 1 then
		ib_Alias = true
...
//Added By Mark Lee 04/15/2015
for i =1  to ll_count
	//Modified By Ken.Guo 2016-03-03
	//	of_copy_screen(ll_new_view_id,ll_screen_id,ll_old_view_id)
	for j = 1 to Upperbound(ll_new_view_id_arr[])
		of_copy_screen(ll_new_view_id_arr[j],long(ls_data[i]),ll_old_view_id)
	next
next 
...
//Modified By Ken.Guo 2016-03-03. fixed a bug: failed to reset the alias after copy screen.
//lnvo_update.of_update_view_fields_alias(1, il_data_view_id)
for j = 1 to Upperbound(ll_new_view_id_arr[])
	lnvo_update.of_update_view_fields_alias(1, ll_new_view_id_arr[j],true)
next
destroy lnvo_update
...
dw_view.Event ue_SetItem(ll_new_view_id_arr[1])
...
//Added By Jay Chen 10-29-2014 add depend lookup data
insert into ctx_lookup_depend (base_lookup_code,depend_lookup_code,depend_type,view_id,screen_id)
select base_lookup_code,depend_lookup_code,depend_type,:ll_new_view_id,screen_id
from ctx_lookup_depend
...
delete from ctx_lookup_depend where  view_id = :il_data_view_id; //Added By Jay Chen 10-29-2014
...
delete from security_viewid_options where view_id = :il_data_view_id ; //Added By Jay Chen 02-13-2014 delete tabpage detail layout property
commit using sqlca;
gnv_appeondb.of_commitqueue()
