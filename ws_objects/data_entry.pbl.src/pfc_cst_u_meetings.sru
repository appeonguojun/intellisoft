...
public function integer of_retrieve_detail ();tab_view.tabpage_browse.dw_browse.Retrieve( il_prac_id )
of_setup_dddw()
...
IF li_retval = 1 and tab_view.tabpage_detail.dw_detail.RowCount() > 0 THEN
	dwchild.SetTransObject( SQLCA )
	dwchild.Retrieve( tab_view.tabpage_detail.dw_detail.GetItemNumber( 1, "facility")  )
END IF
...
ELSE
	IF tab_view.tabpage_detail.dw_detail.RowCount( ) > 0 THEN
		ls_data = String( tab_view.tabpage_detail.dw_detail.GetItemNumber( 1, "meeting_type" ) )
	END IF
END IF
...
//lds_report.SetSQLSelect( ls_sql )
lds_report.modify('datawindow.table.select="'+ ls_sql + '"')
//---------------------------- APPEON END ----------------------------
...
IF newindex = 2 THEN
	...
	IF li_row > 0 THEN
		...
		IF tab_view.tabpage_browse.dw_browse.GetItemNumber( tab_view.tabpage_browse.dw_browse.GetRow(), "meetings_location" ) = -77 THEN
			tab_view.tabpage_detail.dw_detail.DataObject = "d_add_cme_meeting_detail"
		ELSE
...
CHOOSE CASE data
	...
	CASE "0"
		...
		dw_facility.Modify( "facility_id.background.color = '12632256' " )		
	//cme
	CASE "608"
