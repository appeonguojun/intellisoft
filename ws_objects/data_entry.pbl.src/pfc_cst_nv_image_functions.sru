...
public function integer of_retrieve_image_from_db (long al_rec_id, long al_prac_id, integer ai_facility_id, integer ai_screen_id, integer ai_seq_no)
public function integer of_save_image_to_db (long al_rec_id, long al_prac_id, long ai_facility_id, integer ai_screen_id, integer ai_seq_no)
...
public function integer of_retrieve_image_from_db (long al_rec_id, long al_prac_id, integer ai_facility_id, integer ai_screen_id, integer ai_seq_no);Long ll_rec_id
Long ll_prac_id
Long li_seq_no
...
ls_file = gs_dir_path + gs_DefDirName + "\images\"   //Start Code Change ----07.01.2008 #V81 maha - changed back.  the correction added an extra \
...
//messagebox("",ls_file)
// Get prac image info from server
//--------------------------- APPEON BEGIN ---------------------------
...
//	gnv_appeondll.of_RegistryGet("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Prac Images", ls_filename, RegString!, ls_filedate_local)
	gnv_appeondll.of_RegistryGet("HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\Prac Images", ls_filename, RegString!, ls_filedate_local)
...
IF appeongetclienttype() = 'PB' AND gs_dbtype = 'SQL' THEN
	of_get_image_pd_images(al_rec_id, al_prac_id, ai_facility_id, ai_seq_no, ai_screen_id, lb_image_data)
ELSE
	//--------------------------- APPEON BEGIN ---------------------------
	//$<Modify> 08.24.2007 By: Evan
	//$<Reason> The facility_id argument is not requirement.
...
gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\Prac Images", ls_filename, RegString!, ls_filedate_server)	
...
public function integer of_save_image_to_db (long al_rec_id, long al_prac_id, long ai_facility_id, integer ai_screen_id, integer ai_seq_no);ulong l_handle
...
//$<reason> Performance tuning.
/*
ll_filelen = FileLength( gs_dir_path + gs_DefDirName + "\temp.tif" ) 
*/
SetPointer(HourGlass!)

String ls_file
ls_file = gs_dir_path + gs_DefDirName + "\images\" + String(al_rec_id) + "_" + String(al_prac_id) + "_" +&
								String(ai_facility_id) + "_" + String(ai_screen_id) + "_" + String(ai_seq_no) + ".tif"
...
//$<reason> Fix a defect.
/*
li_filenum = FileOpen( gs_dir_path + gs_DefDirName + "\temp.tif", StreamMode!,READ!,LOCKREAD!)
*/
