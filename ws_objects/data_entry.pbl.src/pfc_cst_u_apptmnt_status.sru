...
if ic > 0 then
	for i = 1 to ic
		...
		if li_val > 0 then
			...
			if isnull(ls_val) or len(ls_val) < 1 then
...
if ib_new_record then
	if of_get_app_setting("set_55","I") =  1 then
		//debugbreak()
		iw_win.inv_printletters.of_find_missing_verifs(  This.GetItemNumber( This.GetRow(), "prac_id" ), This.GetItemNumber( This.GetRow(), "verifying_facility" ), 0,  This.GetItemstring( This.GetRow(), "apptmnt_type" ))
	end if
...
long textcolor = 128
long backcolor = 67108864
string text = "Committee Approval"
end type
...
inv_data_entry.of_add_questions("APPT",this,ii_parent_facility_id,gl_prac_id,ll_appt_id)	
//--------------------------- APPEON BEGIN ---------------------------
