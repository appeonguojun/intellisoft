...
string title = "App Audit Auto Generation"
...
is_vis = mid(message.stringparm,1,1)
ll_res = long (mid(message.stringparm,3))

if is_vis = "N" then //from DE
	st_app.text = "Do you want to complete the App Audit item now?"
	...
	if is_vis = "A" then //Start Code Change ----04.08.2008 #V8 maha - from app audit comp
		...
		st_app.text = "Do you want to complete the App Audit item and Verification now?"
	end if
