	...
	FOR i = 1 TO ll_row_cnt
		IF dw_verif.GetItemNumber( i, "selected" ) = 1 THEN
			...
			li_found = lds_ver_sno.Find( "rec_id = " + string( ll_recid ) + " AND facility_id = " + string( ll_ver_fac ) + &
												  " AND print_flag = " + string(li_printflag), 1, lds_ver_sno.Rowcount() )
			IF li_found > 0 THEN
...
il_prac_id = Long( Left( ls_parm, li_pos - 1 ) )
il_current_fac = Long( Right( ls_parm, Len( ls_parm ) - li_pos ) )
...
If this.text = "Select All" THEN
	...
	For i = 1 to dw_verif.Rowcount()
		IF dw_verif.GetItemNumber( i, "selected" ) = 0 THEN dw_verif.SetItem( i, "selected", 1 )
	Next
ELSE
	this.text = "Select All"
	For i = 1 to dw_verif.Rowcount()
		IF dw_verif.GetItemNumber( i, "selected" ) = 1 THEN dw_verif.SetItem( i, "selected", 0 )
	Next
END IF
end event
...
long textcolor = 33554432
long backcolor = 67108864
string text = "Copy with completion information"
end type
