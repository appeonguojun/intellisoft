...
//Added By Ken.Guo 2015-12-03
If li_ret <> 0 Then 
	gnv_debug.of_output(True, 'Failed to connect to Wscript.Shell.')
	Return -1
End If

Try //Added By Ken.Guo 2015-12-03. Added try... catch statement.
	lole_folder = lole_shell.SpecialFolders
	ls_recent_path = String(lole_folder.item(13)) //Recent
Catch (OLERuntimeError e1)
	gnv_debug.of_output(True, 'Failed to get Recent Path by shell object. ' + e1.text)
	Return -1
End Try
