...
IF Isnull(fullname) OR LEN(fullname) = 0 THEN RETURN -1
...
//		gnv_appeondll.of_RegistryGet("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Application List", String(ll_app_id), RegString!, ls_filedate_local)
		...
		//gnv_appeondll.of_RegistryGet("HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\Application List", String(ll_app_id), RegString!, ls_filedate_local)
		gnv_filetime.of_getlastwritedatetime( as_filename, ls_filedate_local)
...
//		gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\IntelliSoft Group\Application List", String(ll_app_id), RegString!, ls_filedate_server)	
		...
		//Modified By Ken.Guo 2015-05-04
		//gnv_appeondll.of_RegistrySet("HKEY_LOCAL_MACHINE\SOFTWARE\"+gs_DefDirName+"\Application List", String(ll_app_id), RegString!, ls_filedate_server)	
		gnv_filetime.of_setlastwritedatetime( as_filename, ldt_filedate_server)
...
IF Isnull(as_filename) OR LEN(as_filename) = 0 THEN RETURN 0
...
AND Convert(varchar(10), sys_storage_docs.area_id) = facility.letter_path
AND facility.facility_id = :ll_facility_id
...
String ls_file
ls_file = as_file
IF Right(ls_file, 4) = ".doc" THEN
	ls_file = Left(ls_file, Len(ls_file) - 4)
ELSE
	RETURN -1
END IF
