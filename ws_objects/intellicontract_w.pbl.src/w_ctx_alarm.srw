...
boolean ib_calendar_readonly = false //Added By Jay Chen 09-16-2015
...
//			dw_alm_det.retrieve(long(istr_pass.s_string_array[1]), istr_pass.s_string_array[2], istr_pass.s_string_array[3],0, long(istr_pass.s_string_array[6]),long(istr_pass.s_string_array[7]))	
      		dw_alm_det.retrieve(long(istr_pass.s_string_array[1]), istr_pass.s_string_array[2], istr_pass.s_string_array[3],long(istr_pass.s_string_array[8]), long(istr_pass.s_string_array[6]),long(istr_pass.s_string_array[7]))	 //Modified By Jay Chen 11-29-2013
	End Choose
...
If ib_new_mode Then // modified by gavin 2011-12-28
	...
	gnv_appeondb.of_commitqueue( )
	of_get_default_message(gw_contract.il_data_view_id, is_table, is_column, lblb_Message) //Modified By Ken.Guo 12/13/2012. //Modified By Ken.Guo 06/19/2013
End If
...
ELSE
	...
	If isnull(lblb_Message) or len(lblb_Message) = 0 Then
		...
		Messagebox('Error','Failed to get the email message content or email message content is empty. please quit Contractlogix and then try agian or reset the message content.')
	Else
		ole_message.of_open(lblb_Message)		
	End If
END IF
...
CHOOSE CASE Upper(istr_pass.s_string_array[2])
	...
	case "CTX_AM_DOCUMENT" //Added By Jay Chen 11-29-2013
		il_ctx_id = long(istr_pass.s_string_array[1])
		il_doc_id = long(istr_pass.s_string_array[8])
		il_level_id1 =  0
		il_level_id2 =  0
	case else //add fee scheduce - jervis 08.02.2011
		il_ctx_id = long(istr_pass.s_string_array[1])
...
If isnull(lblb_message) or Len(lblb_message) = 0 Then
	MessageBox("Save Message", "Failed to get the message content. Please quit Contractlogix then try again.")
	Return -2
End If
...
IF IsNull(ll_export_id ) or ll_export_id < 1 THEN
	cb_field.Enabled = FALSE
ELSE
	If w_mdi.of_security_access( 6957 ) = 1 Then //Added By Jay Chen 08-25-2015 add calendar readonly
		cb_field.Enabled = FALSE
	else
...
Choose case lower(is_table)
	...
	case 'ctx_custom_multi_hdr','ctx_custom_multi_detail','ctx_fee_sched_nm','ctx_am_document' //Modified By Jay Chen 11-29-2013
		ll_ctxid = dw_alm_det.getitemnumber( 1, "ctx_id")
...
this.Control[iCurrent+1]=this.cb_field
...
this.Control[iCurrent+7]=this.dw_alm_det
...
//Added By Jay Chen 09-16-2015
if ib_calendar_readonly then 
	dw_alm_det.object.datawindow.readonly = true
	dw_alm_msg.object.datawindow.readonly = true
	dw_alm_det.modify("b_reset.enabled = false")
	dw_alm_det.modify("b_log.enabled = false")
	dw_alm_det.modify("b_snooze.enabled = false")
	dw_alm_msg.modify("b_sendto.enabled = false")
	dw_alm_msg.modify("b_attach.enabled = false")
	dw_alm_msg.modify("b_attach2.enabled = false") //Added By Jay Chen 09-16-2015
	dw_alm_msg.modify("b_attach3.enabled = false")
	dw_alm_msg.modify("b_clear.enabled = false")
end if
//end add
...
//Added By Jay Chen 08-25-2015  alarm items can be viewed but not changed when calendar is read only
Window	lw_activewindow
string ls_activewindow_name
lw_activewindow = w_mdi.GetActiveSheet( )
if isvalid(lw_activewindow) then
	ls_activewindow_name = lw_activewindow.classname( )
end if
If w_mdi.of_security_access( 6957 ) = 1 and lower(ls_activewindow_name) = 'w_calendarview' Then
	ib_calendar_readonly = true
	cb_ok.enabled = false
	cb_field.enabled = false
	cb_default.enabled = false
	ole_message.enabled = false
End If
...
IF IsNull(ll_email_id) OR ll_email_id < 1 THEN 
	MessageBox("ContractLogix","There isn't an Email template found, please check it first!") //Modified By Jay Chen 04-17-2014
	RETURN
END IF
...
IF IsNull(ll_export_id) OR ll_export_id < 1 THEN 
	MessageBox("ContractLogix","There isn't a related export id found, please check it first!")
	RETURN
END IF
...
//Added By Ken.Guo 11/01/2012. Support Owner for Alarms //Modified By Ken.Guo 2016-03-17 added owner and ctx_id clause.
If Pos(lstr_alarm_email.as_to, Lower('$<OwnerEmail>$' )) > 0 Then
	...
	And ctx_contract_contacts.owner = 'Y' 
	AND  ctx_contract_contacts.ic_area = 'I'
	And ctx_basic_info.ctx_id = :il_ctx_id;
...
If (lower(is_table) = 'ctx_fee_sched_nm' or Lower(is_table) = 'ctx_basic_info' or Lower(is_table) = 'ctx_custom'  or Lower(is_table) = 'ctx_custom_multi_hdr'  or Lower(is_table) = 'ctx_custom_multi_detail' or Lower(is_table) = 'ctx_am_document') Then  //Modified By Jay Chen 11-29-2013
	of_update_snooze() 
end if
...
CHOOSE CASE Upper(istr_pass.s_string_array[2])
	...
	//case 'CTX_CUSTOM_MULTI_HDR','CTX_CUSTOM_MULTI_DETAIL'  //ADD CUSTOM MULTI - jervis 06.30.2011
	case else //add fee scheduce - jervis 08.02.2011
		//Modified By Jay Chen 11-29-2013
		if Upper(istr_pass.s_string_array[2]) = 'CTX_AM_DOCUMENT' then
			If ib_DW_Retrieve Then this.retrieve(long(istr_pass.s_string_array[1]), istr_pass.s_string_array[2], istr_pass.s_string_array[3],long(istr_pass.s_string_array[8]), long(istr_pass.s_string_array[6]),long(istr_pass.s_string_array[7]))
		else
		...
		if rowcount( ) < 1 then 
			event pfc_addrow( )
			this.object.ctx_id[1] = long(istr_pass.s_string_array[1])
			this.object.doc_id[1] = long(istr_pass.s_string_array[8])
			this.object.alm_table[1] = istr_pass.s_string_array[2]
			this.object.alm_column[1] = istr_pass.s_string_array[3]
			this.object.alm_action_title[1] = istr_pass.s_string_array[4] 
...
//		ll_Rtn = GetFileOpenName("Select File", ls_PathName, ls_FileName, "DOC", "All Files (*.*), *.*")					//Added By Mark Lee 08/15/2014
		...
		//Added By Ken.Guo 12/18/2013
		If Len(ls_attachlist) > 990 Then 
			ls_AttachList = Left(ls_attachlist, 990) + '...'
		End If
	...
	CASE 'b_attach2' //Added By Jay Chen 09-11-2015
		...
		IF lstr_pass.l_count > 0 THEN
			...
			Inv_string.of_arraytostring( lstr_pass.s_filename ,";",false,ls_attachment)
			ls_AttachList = Trim(dw_alm_msg.GetItemString(1,"alm_attachment_name"))
			if IsNull(ls_AttachList) then ls_AttachList = ""
			IF Len(ls_AttachList) > 0 Then
				ls_AttachList += ";" + ls_attachment
			ELSE
				ls_AttachList = ls_attachment
			END IF
			//Set fullname variable
			inv_string.of_arraytostring( lstr_pass.s_pathname ,";",false,ls_attachment)
			IF Len(Trim(is_attachment)) > 0 Then
	...
	CASE 'b_attach3' //Added By Jay Chen 09-11-2015
...
08w_ctx_alarm.bin 

18w_ctx_alarm.bin 
