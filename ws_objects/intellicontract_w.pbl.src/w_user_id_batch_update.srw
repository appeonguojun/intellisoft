...
type st_5 from statictext within w_user_id_batch_update
end type
type em_to from editmask within w_user_id_batch_update
end type
type em_from from editmask within w_user_id_batch_update
end type
type st_duedate2 from statictext within w_user_id_batch_update
end type
type st_duedate from statictext within w_user_id_batch_update
end type
type cbx_duedate from checkbox within w_user_id_batch_update
end type
type cbx_request_webform from checkbox within w_user_id_batch_update
end type
type cbx_request_form from checkbox within w_user_id_batch_update
end type
...
type gb_4 from groupbox within w_user_id_batch_update
end type
end forward
...
em_to em_to
em_from em_from
st_duedate2 st_duedate2
st_duedate st_duedate
cbx_duedate cbx_duedate
cbx_request_webform cbx_request_webform
cbx_request_form cbx_request_form
rb_2 rb_2
rb_1 rb_1
cbx_8 cbx_8
cbx_7 cbx_7
...
string ls_to_user,ls_by_user,ls_send_to_user,ls_cc_user, ls_sql[]
...
If cbx_request_form.checked = True Then lb_select = True								//Added By Mark Lee 11/04/2014
If cbx_request_webform.checked = True Then lb_select = True						//Added By Mark Lee 11/04/2014	
...
//Added By Ken.Guo 2017-03-21 task 756
date ld_from, ld_to
string ls_from,ls_to, ls_filter
if cbx_duedate.checked then
	ld_from = date(em_from.text)
	ld_to = date(em_to.text )
	ls_from = string(ld_from, 'yyyymmdd000000')
	ls_to = string(ld_to,'yyyymmdd235959')
	if (em_from.text) <> '00/00/0000' and  (em_from.text) <> '' then
		ls_filter = " string( due_date,'yyyymmddhhmmss')  >= '" + ls_from + "'"
		if  (em_to.text) <> '00/00/0000' and  (em_to.text) <> '' then
			ls_filter += " and string( due_date,'yyyymmddhhmmss')  <= '" + ls_to + "'"
		end if
	else
		if  (em_to.text) <> '00/00/0000'  and  (em_to.text) <> '' then
			ls_filter = " string( due_date,'yyyymmddhhmmss')  <= '" + ls_to + "'"
		end if		
	end if
	if ls_filter <> '' then
		lds_ctx_action_items.setfilter(ls_filter)
		lds_ctx_action_items.filter()
		lds_ctx_am_action_item.setfilter(ls_filter)
		lds_ctx_am_action_item.filter()		
	end if
end if
...
//Added By Mark Lee 11/05/2014
i = 0 
If cbx_request_form.checked = True and  cbx_request_form.enabled = true  Then 
	i ++ 
	ls_sql[i] = " Update request_form  set owner_user = '"+ls_set_to_user_id+"' WHERE ( lower(owner_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_form  set user_id = '"+ls_set_to_user_id+"' WHERE ( lower(user_id) = lower('"+ls_original_user_id+"') ) ;"
	i ++ 
	ls_sql[i] = " Update request_form  set cc_owner_user = '"+ls_set_to_user_id+"' WHERE ( lower(cc_owner_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_form  set cc_reject_user = '"+ls_set_to_user_id+"' WHERE ( lower(cc_reject_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_form  set cc_approve_user = '"+ls_set_to_user_id+"' WHERE ( lower(cc_approve_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_form  set deleted_by = '"+ls_set_to_user_id+"' WHERE ( lower(deleted_by) = lower('"+ls_original_user_id+"') ) ; "	
	i ++ 
	ls_sql[i] = " Update request_form  set admin = '"+ls_set_to_user_id+"' WHERE ( lower(admin) = lower('"+ls_original_user_id+"') ) ; "		
	i ++ 		
	//Added By Mark Lee 05/10/2016
//	ls_sql[i] = " Update request_form  set submit_request_to = left(submit_request_to, charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to ))  -1 ) + '"+ls_set_to_user_id+";' +" + &
//					" right(submit_request_to, len(submit_request_to) - charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) - len('"+ls_original_user_id+"') ) from request_form " + &
//					" WHERE ( submit_request_to is not null AND  (charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) > 0 ) )  ; "
	ls_sql[i] = " Update request_form  set submit_request_to = ( case when ( charindex ( lower('"+ls_set_to_user_id+";'), lower(submit_request_to ) )  > 0 or (right(lower(submit_request_to ), len('"+ls_set_to_user_id+"') ) = lower('"+ls_set_to_user_id+"') ) ) then " + &
			" left(submit_request_to, charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to ))  -1 ) + right(submit_request_to, len(submit_request_to) - charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) - len('"+ls_original_user_id+"') ) " + &		
			" else   left(submit_request_to, charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to ))  -1 ) + '"+ls_set_to_user_id+";' + right(submit_request_to, len(submit_request_to) - charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) - len('"+ls_original_user_id+"') ) " + & 
		 " end ) from request_form WHERE ( submit_request_to is not null AND  (charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) > 0 ) ) ; " 
	i ++ 				
//	ls_sql[i] = " Update request_form  set submit_request_to = left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 ) + '"+ls_set_to_user_id+"' from request_form " + &
// 					" WHERE ( submit_request_to is not null AND  right(lower(submit_request_to ), len('"+ls_original_user_id+"') ) = lower('"+ls_original_user_id+"') )  ; "					
	ls_sql[i] = " Update request_form  set submit_request_to = ( case when charindex ( lower('"+ls_set_to_user_id+";'), lower(submit_request_to ) )  > 0 then " + &
			" left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 -1 ) " + &
			" else left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 ) + '"+ls_set_to_user_id+"' end ) from request_form " + &
 					" WHERE ( submit_request_to is not null AND  right(lower(submit_request_to ), len('"+ls_original_user_id+"') ) = lower('"+ls_original_user_id+"') )  ; "					
	i ++ 
	ls_sql[i] = " Update request_form_detail  set user_id = '"+ls_set_to_user_id+"' WHERE ( lower(user_id) = lower('"+ls_original_user_id+"') ) ; "				 
	i ++ 
	ls_sql[i] = " Update request_form_document  set user_id = '"+ls_set_to_user_id+"' WHERE ( lower(user_id) = lower('"+ls_original_user_id+"') ) ; "				 
	
end if 

If cbx_request_webform.checked = True and  cbx_request_webform.enabled = true  Then 
	i ++ 
	ls_sql[i] = " Update request_webform  set owner_user ='"+ls_set_to_user_id+"' WHERE ( lower(owner_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_webform  set user_id = '"+ls_set_to_user_id+"' WHERE ( lower(user_id) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_webform  set cc_owner_user = '"+ls_set_to_user_id+"' WHERE ( lower(cc_owner_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_webform  set cc_reject_user = '"+ls_set_to_user_id+"' WHERE ( lower(cc_reject_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_webform  set cc_approve_user = '"+ls_set_to_user_id+"' WHERE ( lower(cc_approve_user) = lower('"+ls_original_user_id+"') ) ; "
	i ++ 
	ls_sql[i] = " Update request_webform  set create_user = '"+ls_set_to_user_id+"' WHERE ( lower(create_user) = lower('"+ls_original_user_id+"') ) ; "	
	i ++ 
//	ls_sql[i] = " Update request_webform  set submit_request_to = left(submit_request_to, charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to ))  -1 ) + '"+ls_set_to_user_id+";' +" + &
//					" right(submit_request_to, len(submit_request_to) - charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) - len('"+ls_original_user_id+"') ) from request_webform " + &
//					" WHERE ( submit_request_to is not null AND  (charindex( lower('"+ls_original_user_id+"')+';', lower(submit_request_to )) > 0 ) )  ; "
	ls_sql[i] = " Update request_webform  set submit_request_to = ( case when charindex ( lower('"+ls_set_to_user_id+";'), lower(submit_request_to ) )  > 0 then " + &
			" left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 -1 ) " + &
			" else left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 ) + '"+ls_set_to_user_id+"' end ) from request_webform " + &
					" WHERE ( submit_request_to is not null AND  right(lower(submit_request_to ), len('"+ls_original_user_id+"') ) = lower('"+ls_original_user_id+"') )  ; "					
	i ++ 				
//	ls_sql[i] = " Update request_webform  set submit_request_to = left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 ) + '"+ls_set_to_user_id+"' from request_webform " + &
// 					" WHERE ( submit_request_to is not null AND  right(lower(submit_request_to ), len('"+ls_original_user_id+"') ) = lower('"+ls_original_user_id+"') )  ; " 					
	ls_sql[i] = " Update request_webform  set submit_request_to = ( case when charindex ( lower('"+ls_set_to_user_id+";'), lower(submit_request_to ) )  > 0 then " + &
			" left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 -1 ) " + &
			" else left(submit_request_to, charindex( lower('"+ls_original_user_id+"'), lower(submit_request_to )) -1 ) + '"+ls_set_to_user_id+"' end ) from request_webform " + &
 					" WHERE ( submit_request_to is not null AND  right(lower(submit_request_to ), len('"+ls_original_user_id+"') ) = lower('"+ls_original_user_id+"') )  ; "					
	i ++ 
	ls_sql[i] = " Update request_webform_detail  set user_id = '"+ls_set_to_user_id+"' WHERE ( lower(user_id) = lower('"+ls_original_user_id+"') ) ; "				 
	i ++ 
	ls_sql[i] = " Update request_webform_document  set user_id = '"+ls_set_to_user_id+"' WHERE ( lower(user_id) = lower('"+ls_original_user_id+"') ) ; "				 
	
end if 

if i > 0 then 
	gnv_string.of_execute_sql( ls_sql, "User ID Batch Update", "User ID Batch Update", false)
end if 
...
this.st_duedate2=create st_duedate2
this.st_duedate=create st_duedate
this.cbx_duedate=create cbx_duedate
this.cbx_request_webform=create cbx_request_webform
this.cbx_request_form=create cbx_request_form
...
this.Control[iCurrent+4]=this.st_duedate2
this.Control[iCurrent+5]=this.st_duedate
this.Control[iCurrent+6]=this.cbx_duedate
this.Control[iCurrent+7]=this.cbx_request_webform
this.Control[iCurrent+8]=this.cbx_request_form
this.Control[iCurrent+9]=this.rb_2
this.Control[iCurrent+10]=this.rb_1
this.Control[iCurrent+11]=this.cbx_8
this.Control[iCurrent+12]=this.cbx_7
this.Control[iCurrent+13]=this.st_2
this.Control[iCurrent+14]=this.st_1
this.Control[iCurrent+15]=this.cb_filter
this.Control[iCurrent+16]=this.dw_1
this.Control[iCurrent+17]=this.cb_select
this.Control[iCurrent+18]=this.cb_cancel
this.Control[iCurrent+19]=this.cb_ok
this.Control[iCurrent+20]=this.cbx_update
...
destroy(this.st_duedate2)
destroy(this.st_duedate)
destroy(this.cbx_duedate)
destroy(this.cbx_request_webform)
destroy(this.cbx_request_form)
destroy(this.rb_2)
destroy(this.rb_1)
destroy(this.cbx_8)
destroy(this.cbx_7)
...
event rbuttondown;//Added By Ken.Guo 2017-03-21
If this.text <> ''  Then
	If messagebox('Set Empty','Would you like to set this date to empty?', question!, yesno! ) = 1 Then
		this.text = ''
	End If
End If
end event
...
event clicked;//Added By Ken.Guo 2017-03-21 Task 756
If this.checked Then
	messagebox('',"By selecting Temporary Reassignment only action items with a due date specified date range will be reassigned. ")
...
long textcolor = 33554432
long backcolor = 67108864
string text = "All References in Request Webform Approve "
...
long textcolor = 33554432
long backcolor = 67108864
string text = "All References in Request Form Painter "
boolean checked = true
end type
...
event clicked;If This.Checked Then
	...
	if AppeonGetClientType() = 'PB' then	
		dw_1.Modify( "original_user_id.dddw.AutoHScroll	= 'yes' " )		//Added By Mark Lee 03/06/2015 some reason cause the redraw in APB 6.8, and not in APB6.5.
	end if 
	...
	if AppeonGetClientType() = 'PB' then	
		dw_1.Modify( "set_to_user_id.dddw.AutoHScroll	= 'yes' " )	//Added By Mark Lee 03/06/2015 some reason cause the redraw in APB 6.8, and not in APB6.5.
	end if 
	...
	cbx_5.enabled = false
	cbx_8.enabled = false
	cbx_request_form.enabled = False		//Added By Mark Lee 11/04/2014
	cbx_request_webform.enabled = False	//Added By Mark Lee 11/04/2014
			
	cbx_6.enabled = true 
	cbx_7.enabled = true 

	//Added By Ken.Guo 2017-03-21  Task 756
	cbx_duedate.enabled = False
	em_from.enabled = False
	em_to.enabled = False
	st_duedate.textcolor = rgb(192,192,192)
	st_duedate2.textcolor = rgb(192,192,192)
End If
end event
...
//	dw_1.Modify( "original_user_id.dddw.name=''dddw_security_users_batch_update" )		//Added By Mark Lee 12/01/2014   only change the choose //dddw_security_users
	dw_1.Modify( "original_user_id.dddw.name='dddw_security_users_batch_update'" )	 //Modified By Jay Chen 03-05-2015
	...
	If Len(  ls_original ) > 0 or ( ls_original = '' and ls_set = ''   )	Then
		...
		cbx_request_form.enabled = true		//Added By Mark Lee 11/04/2014
		cbx_request_webform.enabled = true	//Added By Mark Lee 11/04/2014		
	End If
	
	//Added By Ken.Guo 2017-03-21  Task 756
	cbx_duedate.enabled = true
	st_duedate.textcolor = rgb(0,0,0)
	st_duedate2.textcolor = rgb(0,0,0)
	If  cbx_duedate.checked Then
		em_from.enabled = true
		em_to.enabled = true
	Else
...
If rb_1.Checked Then
	If Len(  ls_original ) > 0  or (   trim( ls_original ) = '' and trim( ls_Set ) = ''  )  Then
		...
		If this.text = '&Select All' Then
			...
			cbx_request_form.checked = True			//Added By Mark Lee 11/04/2014
			cbx_request_webform.checked = True	//Added By Mark Lee 11/04/2014
		...
		Else
			...
			cbx_request_form.checked = False		//Added By Mark Lee 11/04/2014
			cbx_request_webform.checked = False	//Added By Mark Lee 11/04/2014
	...
	Case '03'
		If rb_1.Checked Then
			If wf_ok3() = 1 Then
				...
				//Added By Jay Chen 03-10-2015 refresh contract browse owner data
				if isvalid(gw_contract) then
					if gw_contract.tab_contract_details.selectedtab = 1 then
						gw_contract.tab_contract_details.tabpage_search.cb_1.event clicked()
					end if
				end if
				//end
			End If
