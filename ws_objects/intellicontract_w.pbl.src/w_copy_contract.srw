...
type tab_1 from tab within w_copy_contract
end type
...
type cb_tab_ok from commandbutton within tabpage_tab
end type
...
public subroutine of_create_copy (boolean abn_select)
...
lnv_copy_ctx.of_generate_select_screen_fields( il_view_id, ls_screen, tab_1.tabpage_field_list.dw_1,"all", abn_exists)
...
//Closewithreturn(Parent,ll_ctx_id)																			//Added By Mark Lee 06/30/2015
of_close(ll_ctx_id)
...
this.cb_tab_ok=create cb_tab_ok
...
string 	ls_box,	ls_screen, ls_column, ls_table_name
PowerObject 	luo_PowerObject
Checkbox 		luo_checkbox
				...
				If luo_PowerObject.classname( ) = 'cbx_' + String(ls_box) Then
					If luo_PowerObject.TypeOf() = CheckBox! Then
						...
						if isnull(ll_find) or ll_find <= 0 then 
							...
							luo_checkbox.checked = false 								//reset
						else
				...
				If luo_PowerObject.classname( ) = 'cbx_' + String(ls_box) Then
					If luo_PowerObject.TypeOf() = CheckBox! Then
						...
						if isnull(ll_find) or ll_find <= 0 then 
							...
							luo_checkbox.checked = true 												//Added By Mark Lee 08/18/2015 keep the order no
							luo_checkbox.thirdstate = true 
							luo_checkbox.threestate = true			
						end if 						
						Exit						
...
//lstr_copy_ctx.sl_workflow = long(gnv_data.of_getitem("code_lookup","custom_5","lookup_code =" + string(ll_category)))
//
////if lbn_select = true then 		//Added By Mark Lee 07/13/2015			no need it 
////	
////end if 
//lnv_copy_ctx.of_get_filter_ds(tab_1.tabpage_field_list.dw_1 )										//Added By Mark Lee 06/10/2015
//ll_ctx_id = lnv_copy_ctx.of_copy_ctx_data( lstr_copy_ctx)
