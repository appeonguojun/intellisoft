...
boolean ib_use_template = false //Added By Jay Chen 01-22-2014

Boolean ib_auto_sync_sf = false //Added by harry 20170502
...
public subroutine of_auto_create_dw_occ ()
public function datawindow of_getactivedatawindow ()
public subroutine of_compute_special_field_sum ()
public function boolean of_check_mulselect_ctx_id (string as_tabpage)
public subroutine of_save_pagebar_position ()
end prototypes
...
this.tab_contract_details.tabpage_search.ib_open_with_ctx = true

//Added By Jay Chen 01-26-2016 save screen field multi selelct item lists
if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select) then
	if of_check_mulselect_ctx_id('tabpage_1')  then //Added By Jay Chen 03-09-2016
		If this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.update() = 1 Then
			this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.il_ctx_old = This.tab_contract_details.tabpage_details.inv_contract_details.of_get_ctx_id( )
		End If
	end if
end if
if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select) then
	if of_check_mulselect_ctx_id('tabpage_custom') then  //Added By Jay Chen 03-09-2016
		If this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.update() = 1 Then
			this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.il_ctx_old = This.tab_contract_details.tabpage_details.inv_contract_details.of_get_ctx_id( )
		End If
	end if
end if
//end
		...
		*/
		...
		/* If li_rc < 0 Then 
			If tab_contract_details.selectedtab = 2 And ib_validation_1 = False Then
  				MESSAGEBOX('Contract Logix' ,'There is no DEFAULT TO assigned to this contract. Please add one before continuing.')			
			End If
			Return FAILURE
		End If */
		If li_rc <= 0 Then 
...
long ll_RowCnt,ll_RowPos //Added By Jay Chen 05-14-2015
...
//	End If
	...
	if AppeonGetClientType() = 'WEB' then  //Added By Jay Chen 05-14-2015 Locate the new ctx to the specified page and specified line
		tab_contract_details.tabpage_search.ids_search.SetFilter('')
		tab_contract_details.tabpage_search.ids_search.Filter()
		ll_RowCnt = tab_contract_details.tabpage_search.ids_search.RowCount()
		ll_RowPos = tab_contract_details.tabpage_search.ids_search.Find("ctx_id = " + String(ll_ctx_id), 1, ll_RowCnt)
		if ll_RowPos <= 0 then
...
of_save_pagebar_position() //Added By Jay Chen 03-29-2016
...
gnv_data.of_retrieve('field_compute_expression_mul') //Added By Ken.Guo 2016-08-19
...
//Added By Jay Chen 02-10-2014 Remark: refresh dddw and code lookup data
string ls_refresh_contract_cache,ls_cachelist[],ls_message
long ll_cnt, i
ls_refresh_contract_cache = gnv_user_option.of_Get_option_value( gs_user_id, "refresh_contract_cache" )
if isnull(ls_refresh_contract_cache) then ls_refresh_contract_cache = ''
if ls_refresh_contract_cache = '1' then 
	If Not isvalid(w_appeon_gifofwait) Then
		OpenWithParm(w_appeon_gifofwait,"Refreshing drop down list data caches, Please wait...")
	Else
		ls_message = w_appeon_gifofwait.st_1.text
		w_appeon_gifofwait.of_settext("Refreshing drop down list data caches, Please wait...")
	End If
	...
	w_appeon_gifofwait.of_settext(ls_message)
end if
...
//Added By Jay Chen 05-14-2015
if this.tab_contract_details.tabpage_search.dw_search_det1.rowcount() < 1 then
	this.tab_contract_details.tabpage_search.sle_page.text = '' 
end if
end event
...
If isvalid(This.tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.tabpage_preview.ole_doc.object) Then
	If This.tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.tabpage_preview.ole_doc.object.doctype > 0 Then 
		This.tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.tabpage_preview.ole_doc.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
	End If
	...
	If This.tab_contract_details.tabpage_images.tab_1.tabpage_data.il_prior_notify_doc_id > 0 Then
		gnv_tracking.of_tracking_doc( gnv_tracking.document_management,string(This.tab_contract_details.tabpage_images.tab_1.tabpage_data.il_prior_notify_doc_id),'','','','delete_doc_tracking',false )
	End If	
...
long ll_ctx_id
If Not inv_contract_codes.of_allow_copyctx() Then 
	MessageBox("ContractLogix","You don't have rights to save the contract as a template!")
	Return
End If
...
If Not inv_contract_codes.of_allow_newctx() Then 
	MessageBox("ContractLogix","You don't have rights to create a new contract!")
	Return
End If
...
ib_use_template = true //Added By Jay Chen 01-22-2014
...
event ue_refresh();string ls_browse_screen_results_style

If tab_contract_details.selectedtab = 1 Then
	tab_contract_details.tabpage_search.ib_refresh_button = true
	tab_contract_details.tabpage_search.cb_1.event clicked( )
	//Added By Jay Chen 05-26-2015 refresh calendar data
	if isvalid(w_SearchPage_Calendar) then
		ls_browse_screen_results_style = gnv_user_option.of_Get_option_value( gs_user_id, "Contract_Search_Tab_Browse_Screen_Results_Style" )
		if ls_browse_screen_results_style = '0' then
			w_SearchPage_Calendar.uo_Calendar.of_Refresh_Disp_List()
			w_SearchPage_Calendar.uo_Calendar.of_RefreshSelectScope(3)
			w_SearchPage_Calendar.uo_Calendar.of_RefreshFilterName()
		end if
	end if
	//end add
End If
...
Long		ll_DOCID
String ls_auto_modify_owner //Added By Jay Chen 01-24-2014
...
If ll_row <= 0 Then 
	Messagebox('ContractLogix','Please select contract(s) first.')
	Return 
End If
...
Do While ll_row > 0
	...
	If UpperBound(lstr_ctx_info[]) > 0 Then
		If ll_view_id <> lstr_ctx_info[1].al_view_id Then
			Messagebox('ContractLogix',"Batch Set Alarms requires all contracts to be from the same view.~r~n" +  &
...
If ls_readonly_ctx <> '' Then
	Messagebox('ContractLogix', "You only have read-only permission to contract "+ls_readonly_ctx+" in your selection, they will be ignored. " )
End If

If UpperBound(lstr_ctx_info[]) = 0 Then
	Messagebox('Contractlogix','No contract is suitable for Batch Set Alarms.')
End If
...
long 					ll_new_ctx_id,ll_row,ll_template_id, ll_find, ll_rowcount, ll_rtn, ll_template_ctx_id, ll_cnt, ll_Found
...
If ll_find > 0 Then
	...
	ll_template_ctx_id = tab_contract_details.tabpage_search.ids_templates_list.getitemNumber(ll_find, 'ctx_id') //Added by gavins 20161130

	select category into:ldec_Category from ctx_basic_info where ctx_id = :ll_template_ctx_id;
	
	IF IsValid(gw_contract) THEN 
		ll_cnt = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_category.Retrieve(gs_user_id)	
		IF ll_cnt > 0 THEN ll_found = gw_contract.tab_contract_details.tabpage_search.ids_contract_access_category.Find("category = " + string(ldec_Category), 1, ll_cnt)
...
//====================================================================
string 	ls_Copy_Contract
long 		ll_ctx_id,ll_RowCnt,ll_Rowpos
Integer li_ret1, li_ret2, li_ret3
...
ll_row = tab_contract_details.tabpage_search.dw_search_det1.getrow() //Modified By Jay Chen 07-02-2015
ll_ctx_id_source = tab_contract_details.tabpage_search.il_ctx_id
...
If Not inv_contract_codes.of_allow_newctx() Then 
	MessageBox("Contract Logix","You don't have rights to create a new contract!")
	Return
End If
...
This.Title = 'Contract Logix '  + '(' + 'New Mode' + ')'
...
// Event: ue_sync_to_salesforce
//--------------------------------------------------------------------
// Description: Sync contract to salesforce
//--------------------------------------------------------------------
// Arguments:
//--------------------------------------------------------------------
// Returns:  (None)
...
long ll_import_id,ll_row,ll_ctx_id,ll_foundrow,ll_cnt,ll_rtn,ll_count,ll_valid_count,ll_import_product_id
string ls_select_ctx_id[],ls_ctx_id ,ls_msg,ls_user,ls_pwd,ls_token 
...
boolean lb_success = true,lb_product_success = true
Boolean lb_sync_sf = false //Added by Harry 20170502
datastore lds_ctx_product

//If tab_contract_details.selectedtab = 1 Then
If tab_contract_details.selectedtab = 1 or ib_auto_sync_sf Then //Added by Harry 20170502
	lds_ctx_product = create datastore
	lds_ctx_product.dataobject = 'd_sync_ctx_product'
	lds_ctx_product.settransobject(sqlca)
	gnv_appeondb.of_startqueue( )
		lds_ctx_product.retrieve()
		if ib_auto_sync_sf then //Added by Harry 20170502  1.	Auto – Syncing Contract information
			select a.import_id into :ll_import_id from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and a.auto_sync_salesforce = 1 and b.isoft_table_id = 1001;
		else
			select a.import_id into :ll_import_id from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and b.isoft_table_id = 1001;
		end if
		select a.import_id into :ll_import_product_id from import_hdr a,import_table_records b where a.import_id = b.import_id and a.import_type = '2' and a.import_flag = '1' and a.sync_salesforce = 1 and b.isoft_table_id = 1002;
	gnv_appeondb.of_commitqueue( )
	if isnull(ll_import_id) then ll_import_id = 0
	if isnull(ll_import_product_id) then ll_import_product_id = 0
	//Commented by Harry 20170330 for task749
	/*
	if ll_import_product_id <= 0 then
		messagebox('Contractlogix','The system can not find the mapping for the Products synchronization. Please make sure you have configured it correctly in the Data Synchronization Painter.')
		return 
	end if
	*/
	if not ib_auto_sync_sf then //Added by Harry 20170502
		if ll_import_id <= 0 then 
			messagebox('Contractlogix','The system can not find the mapping for the Basic Information synchronization. Please make sure you have configured it correctly in the Data Synchronization Painter.')
			return 
		end if
		
		//get the select ctx_id
		ll_foundrow = 0
		ll_row = this.tab_contract_details.tabpage_search.dw_search_det1.getselectedrow(0)
		Do While ll_row > 0
			ll_ctx_id = this.tab_contract_details.tabpage_search.dw_search_det1.getitemnumber(ll_row,'ctx_id')
			ls_ctx_id = ls_ctx_id +  string(ll_ctx_id)+ ", "
			ll_foundrow ++
			ll_row = this.tab_contract_details.tabpage_search.dw_search_det1.getselectedrow(ll_row)
		Loop
		ls_ctx_id = left(ls_ctx_id, len(ls_ctx_id) -2 )
		lnv_arrstring.of_parsetoarray( ls_ctx_id, ',', ls_select_ctx_id[]) 
			
		if ll_foundrow = 1  then
			ls_msg = 'Are you sure you want to sync this contract (ID = '+String(ls_ctx_id)+') to SalesForce?'
		else
			if ll_foundrow > 0 then
				ls_msg = 'Are you sure you want to sync these contracts (ID = '+String(ls_ctx_id)+') to SalesForce?'
			else
				return
			end if
		end if
	else
		//ls_select_ctx_id[1] = String(this.tab_contract_details.tabpage_details.uo_1.il_ctx_id)
		ls_select_ctx_id[1] = String(this.tab_contract_details.tabpage_details.uo_1.inv_contract_details.of_get_ctx_id( ) )
	end if		
	...
	if ib_auto_sync_sf then
		lb_sync_sf = true 
	elseif messagebox('Sync Contract', ls_msg,Exclamation!,Yesno!,2) = 1 then 
		lb_sync_sf = true 
	end if
	
	//if messagebox('Sync Contract', ls_msg,Exclamation!,Yesno!,2) = 1 then
	if lb_sync_sf then //Added by Harry 20170503
		for ll_cnt = 1 to upperbound(ls_select_ctx_id)
			...
			if not ib_auto_sync_sf then //Added by Harry 20170503
				if ll_count > 0 then 
					if messagebox('Sync Contract', 'The contract (ID = '+String(ll_ctx_id)+') has already been synchronized to SalesForce.~r~nAre you sure you want to overwrite the contract information on SalesForce?',Exclamation!,Yesno!,2) <> 1 then
						continue
					end if
				end if
			end if
			//begin to sync the ctx_id
			//ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', ll_ctx_id, 0, 0, 0, ll_import_id, ls_salesforce_id, ls_sfurl)
			ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', ll_ctx_id, 0, 0, 0, ll_import_id, ls_salesforce_id, ls_sfurl, 0, 0)  //modified by harr 20170320 for task749
			if len(ls_salesforce_id) <= 0 then
				lb_success = false
				//messagebox('Sync Contract', 'Failed to synchronize the contract (ID = '+String(ll_ctx_id)+') to SalesForce.')
				if not ib_auto_sync_sf then messagebox('Sync Contract', 'Failed to synchronize the contract (ID = '+String(ll_ctx_id)+') to SalesForce.')  //Added by Harry 20170503
			else
				//sync the product of the ctx_id
				if ll_import_product_id > 0 then  //Added by Harry 20170330 task749
					ls_salesforce_id = ''
					lds_ctx_product.setfilter("ctx_id = " + string(ll_ctx_id) )
					lds_ctx_product.filter()
					for ll_row = 1 to lds_ctx_product.rowcount()
						ll_product_id = lds_ctx_product.getitemnumber(ll_row, "product_id")
						ll_product_type = lds_ctx_product.getitemnumber(ll_row, "product_type")
						//ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', ll_ctx_id, 0, ll_product_id, ll_product_type, ll_import_product_id, ls_salesforce_id, ls_sfurl)
						ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', ll_ctx_id, 0, ll_product_id, ll_product_type, ll_import_product_id, ls_salesforce_id, ls_sfurl, 0, 0) //modified by harr 20170320 for task749
						if len(ls_salesforce_id) <= 0  then
							lb_product_success = false
							//messagebox('Sync Contract Product', 'Failed to synchronize the contract product (Product ID = '+String(ll_product_id)+') to SalesForce.')
							if not ib_auto_sync_sf then messagebox('Sync Contract Product', 'Failed to synchronize the contract product (Product ID = '+String(ll_product_id)+') to SalesForce.') //Added by Harry 20170503
						end if
					next
				end if
				if lb_product_success then ll_valid_count++
			end if
		next
	end if
	...
	if not ib_auto_sync_sf then //Added by Harry 20170503
		if lb_success and ll_valid_count > 0 then
			messagebox('Sync Contract', 'Succeed to sync the contract to salesforce.')
		end if
	end if
...
public subroutine of_select_details_tab ();//---------- <RP> 9.22.2016 ------------------------------------------------------------
...
public subroutine of_dashboard (long ai_ctx_id);//--------------------------------------------------------------------------------------------------------------
//--------<RP> 9.21.2016 use new method to load the selected contract
pointer lptr_prev

lptr_prev = setPointer(hourglass!)
IF tab_contract_details.tabpage_search.of_goto_contract(ai_ctx_id) = 1 THEN
...
ELSE
	messageBox(this.className() + ".of_dashboard()", "Error - unable to find Contract No " + string(ai_ctx_id))
END IF
...
////Added By Jay Chen 06-04-2015
//if ll_find = 0 then
//	gf_goto_contract_page(ai_ctx_id,'other')
//	ll_find=tab_contract_details.tabpage_search.dw_search_det1.find("ctx_id="+string(ai_ctx_id), 1, tab_contract_details.tabpage_search.dw_search_det1.rowcount())
//end if
////end
//if ll_find>0 then
...
//end if
//end if
//--------end <RP> 9.21.2016 use new method to load the selected contract
//--------------------------------------------------------------------------------------------------------------
end subroutine
...
//			luo_tabpage = gw_contract.tab_contract_details.tabpage_qrg.tab_1.tabpage_selection
		...
		Case Else
			Continue		
	End Choose
	if isnull(luo_tabpage.picturename) then luo_tabpage.picturename = '' //Added By Jay Chen 03-21-2016
	If ls_tabicon_show = 'T' Then
		If ls_icon <> '' Then
			...
			If luo_tabpage.picturename <> ls_icon Then
...
If ls_fontname = '' then ls_fontname = 'MS Sans Serif'
...
If Len( ls_fontname ) > 0 Then
	...
	tab_contract_details.tabpage_images.tab_1.facename = ls_fontname //Added By Jay Chen 03-21-2016
	
	//Begin - Added By jay 2016-04-14.
	tab_contract_details.tabpage_details.uo_1.tab_1.facename = ls_fontname
	tab_contract_details.tabpage_action_items.tab_1.facename = ls_fontname
	tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.facename = ls_fontname
	tab_contract_details.tabpage_images.tab_1.tabpage_action.tab_1.facename = ls_fontname
	tab_contract_details.tabpage_multi.tab_multi.facename = ls_fontname
	tab_contract_details.tabpage_fee_schedules.tab_1.facename = ls_fontname
...
If Len( ls_italic ) > 0 Then
	...
	tab_contract_details.tabpage_images.tab_1.italic = ( ls_italic = 'T' )
	
	//Begin - Added By jay 2016-04-14.
	tab_contract_details.tabpage_details.uo_1.tab_1.italic = ( ls_italic = 'T' )
	tab_contract_details.tabpage_action_items.tab_1.italic = ( ls_italic = 'T' )
	tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.italic = ( ls_italic = 'T' )
	tab_contract_details.tabpage_images.tab_1.tabpage_action.tab_1.italic = ( ls_italic = 'T' )
	tab_contract_details.tabpage_multi.tab_multi.italic = ( ls_italic = 'T' )
	tab_contract_details.tabpage_fee_schedules.tab_1.italic = ( ls_italic = 'T' )
	//End - Added By jay 2016-04-14.
...
If Len( ls_underline ) > 0 Then
	...
	tab_contract_details.tabpage_images.tab_1.underline = ( ls_underline = 'T' )
	
	//Begin - Added By jay 2016-04-14.
	tab_contract_details.tabpage_details.uo_1.tab_1.underline = ( ls_underline = 'T' )
	tab_contract_details.tabpage_action_items.tab_1.underline = ( ls_underline = 'T' )
	tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.underline = ( ls_underline = 'T' )
	tab_contract_details.tabpage_images.tab_1.tabpage_action.tab_1.underline = ( ls_underline = 'T' )
	tab_contract_details.tabpage_multi.tab_multi.underline = ( ls_underline = 'T' )
	tab_contract_details.tabpage_fee_schedules.tab_1.underline = ( ls_underline = 'T' )
	//End - Added By jay 2016-04-14.

End If
If Len( ls_fixedwidth ) > 0 Then
	tab_contract_details.Fixedwidth= ( ls_fixedwidth = 'T' )
	tab_contract_details.tabpage_images.tab_1.fixedwidth = ( ls_fixedwidth = 'T' )
	
	//Begin - Added By jay 2016-04-14.
	tab_contract_details.tabpage_details.uo_1.tab_1.fixedwidth = ( ls_fixedwidth = 'T' )
	tab_contract_details.tabpage_action_items.tab_1.fixedwidth = ( ls_fixedwidth = 'T' )
	tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.fixedwidth = ( ls_fixedwidth = 'T' )
	tab_contract_details.tabpage_images.tab_1.tabpage_action.tab_1.fixedwidth = ( ls_fixedwidth = 'T' )
	tab_contract_details.tabpage_multi.tab_multi.fixedwidth = ( ls_fixedwidth = 'T' )
	tab_contract_details.tabpage_fee_schedules.tab_1.fixedwidth = ( ls_fixedwidth = 'T' )
	//End - Added By jay 2016-04-14.
	
End If
If ll_Font_height > 0 Then
	tab_contract_details.TextSize	= - ll_Font_height
	tab_contract_details.tabpage_images.tab_1.textsize = - ll_Font_height
	
	//Begin - Added By jay 2016-04-14.
	tab_contract_details.tabpage_details.uo_1.tab_1.TextSize = - ll_Font_height
	tab_contract_details.tabpage_action_items.tab_1.TextSize = - ll_Font_height
	tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.TextSize = - ll_Font_height
	tab_contract_details.tabpage_images.tab_1.tabpage_action.tab_1.TextSize = - ll_Font_height
	tab_contract_details.tabpage_multi.tab_multi.TextSize = - ll_Font_height
	tab_contract_details.tabpage_fee_schedules.tab_1.TextSize = - ll_Font_height
	//End - Added By jay 2016-04-14.
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 01-24-2014
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
ls_dataobject = tab_contract_details.tabpage_details.uo_1.dw_occ.dataobject
if isnull(ls_dataobject) or ls_dataobject = '' then
	of_auto_create_dw_occ()
	tab_contract_details.tabpage_details.uo_1.dw_occ.settransobject(sqlca)
end if
...
if lb_updateflag then
	for i = 1 to UpperBound(al_ctx_id[]) 
		ll_ctx_id = al_ctx_id[i]
		tab_contract_details.tabpage_details.uo_1.dw_occ.retrieve(ll_ctx_id,'I')
		ll_rowcount = tab_contract_details.tabpage_details.uo_1.dw_occ.rowcount()
		ll_find = tab_contract_details.tabpage_details.uo_1.dw_occ.find("Upper(user_d) = '"+Upper(lstr_contact.as_user_id)+"'",1, ll_rowcount)
		if ll_find > 0 then 
			ll_contact_id = tab_contract_details.tabpage_details.uo_1.dw_occ.getitemnumber(ll_find,"contact_id")
			ls_default_email = tab_contract_details.tabpage_details.uo_1.dw_occ.getitemstring(ll_find,"default_email")
			ls_owner = tab_contract_details.tabpage_details.uo_1.dw_occ.getitemstring(ll_find,"ctx_contract_contacts_owner")
			if (ls_default_email <> 'Y' or ls_owner <> 'Y')  then
				...
				if ls_default_email <> 'Y' then
					tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_find,"default_email", 'Y')
				end if
				if ls_owner <> 'Y' then
					tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_find,"ctx_contract_contacts_owner", 'Y')
				end if
				lb_edit = true
			else
				continue
			...
			ll_row = tab_contract_details.tabpage_details.uo_1.dw_occ.insertrow(0)
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"default_email", 'Y')
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"ctx_contract_contacts_owner", 'Y')
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"ctx_id", ll_ctx_id)
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"record_id", ll_record_id)
			lb_edit = false
		end if
		commit;
		//write audit log
		inv_audit.of_set_prac_id(ll_ctx_id)
		inv_audit.of_audit_fields(tab_contract_details.tabpage_details.uo_1.dw_occ)
		//refresh screen data
		tab_contract_details.tabpage_details.uo_1.dw_occ.retrieve(ll_ctx_id,'I')
		...
		if lb_edit then
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItemStatus(ll_find,"default_email", Primary!, NotModified!)
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItemStatus(ll_find,"ctx_contract_contacts_owner", Primary!, NotModified!)
		else
			tab_contract_details.tabpage_details.uo_1.dw_occ.SetItemStatus(ll_find,0, Primary!, NotModified!)
		end if
		lb_edit = false
	next
end if
...
public subroutine of_auto_create_dw_occ ();string ls_tab_name
string ls_dw_name[]
...
ld_dw[1] = tab_contract_details.tabpage_details.uo_1.dw_occ
ls_dw_name[1] = 'dw_occ'
ls_dataobject[1] = 'd_contract_det_contact_external'
...
public function datawindow of_getactivedatawindow ();return idw_Active
end function

public subroutine of_compute_special_field_sum ();//====================================================================
// Function: of_compute_special_field_sum
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 11-18-2014
//--------------------------------------------------------------------
...
long ll_view,ll_screen,ll_row,ll_ctx_id,ll_master_ctx_id,ll_find,ll_update_ctx_id,ll_data,ll_update_cnt,ll_index,ll_value
string ls_source_field,ls_store_field,ls_coltype,ls_sql
boolean lb_has_modify = false
datastore lds_ctx_basic_temp
decimal ldec_data,ldec_value
dwitemstatus ldw_status

if isvalid(tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1) then
	ll_row = tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getrow()
	if ll_row < 1 then return
	ll_view = tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.il_viewid
	ll_screen = tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.il_screenid
	select object_name into :ls_source_field from screen_field_special_attribute
	where view_id = :ll_view and screen_id = :ll_screen and object_type = 'sum_source_field';
	if isnull(ls_source_field) or ls_source_field = '' then return
	ls_coltype = lower(left(tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.Describe(ls_source_field +".coltype"),3))
	ldw_status= tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemstatus(ll_row, ls_source_field,Primary!)
	if ls_coltype = 'dec' then
		if tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemdecimal(ll_row, ls_source_field,Primary!,true) <> tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemdecimal(ll_row, ls_source_field,Primary!,false) then
			lb_has_modify = true
		end if
	else
		if tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemnumber(ll_row, ls_source_field,Primary!,true) <> tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemnumber(ll_row, ls_source_field,Primary!,false) then
			lb_has_modify = true
		end if
	end if
	if ldw_status = DataModified! then lb_has_modify = true	
	if lb_has_modify then
		select object_name into :ls_store_field from screen_field_special_attribute
		where view_id = :ll_view and screen_id = :ll_screen and object_type = 'sum_store_field';
		ll_ctx_id = tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemnumber(ll_row,"ctx_id")
		ll_master_ctx_id = tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemnumber(ll_row,"master_contract_id")
		...
		lds_ctx_basic_temp.retrieve(ll_master_ctx_id)
		ll_find = lds_ctx_basic_temp.find("ctx_id = " + string(ll_ctx_id), 1, lds_ctx_basic_temp.rowcount())
		if isnull(ll_find) then ll_find = 0
		if ll_find > 0 then
			if ls_coltype = 'dec' then
				lds_ctx_basic_temp.setitem(ll_find, ls_source_field, tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemdecimal(ll_row, ls_source_field,Primary!, false))
			else
				lds_ctx_basic_temp.setitem(ll_find, ls_source_field, tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.getitemnumber(ll_row, ls_source_field,Primary!, false))
			end if
			for ll_update_cnt = ll_find + 1 to lds_ctx_basic_temp.rowcount()
				ldec_value = 0
				ll_value = 0
...
public function boolean of_check_mulselect_ctx_id (string as_tabpage);//====================================================================
// Function: of_check_mulselect_ctx_id
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 03-09-2016
//--------------------------------------------------------------------
//	Copyright (c) 2008-2014 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
long ll_ctx_id,ll_cnt,ll_ctx
String ls_filter_org //Added By Ken.Guo 2017-05-08

if this.tab_contract_details.tabpage_details.uo_1.dw_1.rowcount() > 0 then
	ll_ctx = this.tab_contract_details.tabpage_details.uo_1.dw_1.getitemnumber(1,'ctx_id')
	if isnull(ll_ctx) then ll_ctx = 0
	if ll_ctx = 0 then 
		gnv_debug.of_output(true, 'Get the ctx_id failed when check multi select item list value.')
		return false
	end if
end if

if as_tabpage = 'tabpage_custom' then
	//Added By Ken.Guo 2017-05-08. Task 770. When a new contract is created, all Multi-select fields have values already selected
	ls_filter_org = this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.describe('DataWindow.Table.Filter')
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.setfilter('')
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.filter()
	
	for ll_cnt = 1 to this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.rowcount()
		ll_ctx_id = this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.getitemnumber(ll_cnt, 'ctx_id')
		if isnull(ll_ctx_id) then ll_ctx_id = 0
		if ll_ctx_id = 0 then
			this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.setitem(ll_cnt,'ctx_id',ll_ctx)
		end if
	next
	
	//Added By Ken.Guo 2017-05-08. Task 770. When a new contract is created, all Multi-select fields have values already selected
	If len(ls_filter_org) > 1 Then
		this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.setfilter(ls_filter_org)
		this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.filter()
	End If
	
else
	//Added By Ken.Guo 2017-05-08. Task 770. When a new contract is created, all Multi-select fields have values already selected
	ls_filter_org = this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.describe('DataWindow.Table.Filter')	
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.setfilter('')
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.filter()
	  
	for ll_cnt = 1 to this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.rowcount()
		ll_ctx_id = this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.getitemnumber(ll_cnt, 'ctx_id')
		if isnull(ll_ctx_id) then ll_ctx_id = 0
		if ll_ctx_id = 0 then
			this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.setitem(ll_cnt,'ctx_id',ll_ctx)
		end if
	next

	//Added By Ken.Guo 2017-05-08. Task 770. When a new contract is created, all Multi-select fields have values already selected
	If len(ls_filter_org) > 1 Then
		this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.setfilter(ls_filter_org)
		this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.filter()
	End If
...
public subroutine of_save_pagebar_position ();//Added By Jay Chen 03-29-2016 save pagetoolbar position
//Modified by gavins 08032016   cancel 
//long ll_x,ll_y
//
//if isvalid(this.tab_contract_details.tabpage_search.uo_page) then
//	ll_x = this.tab_contract_details.tabpage_search.uo_page.x
//	ll_y = this.tab_contract_details.tabpage_search.uo_page.y
//	If Not isnull(ll_x) and ll_x > 0 Then
//		gnv_user_option.of_set_option_value( "page_toolbar_x", String(ll_x) )
//	End If
//	If Not isnull(ll_y) and ll_y > 0 Then
...
tab_contract_details.tabpage_search.ib_readonly_current = False   //Added by Harry for Task 730
tab_contract_details.tabpage_search.inv_contract_details.is_mode = tab_contract_details.tabpage_search.inv_contract_details.new
inv_contract_codes.of_set_ctx_id(0)
...
//gnv_tracking.of_tracking_doc_delete('Document Management')	//Added By Mark Lee 07/20/12 //Commented By Ken.Guo 2016-04-05

//Added By Jay Chen 01-26-2016 
if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select) then
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.il_ctx_old = 0
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.il_screen_old = 0
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.il_view_old = 0
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.is_fieldname_old = ''
end if
if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select) then
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.il_ctx_old = 0
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.il_screen_old = 0
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.il_view_old = 0
	this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.is_fieldname_old = ''
end if
//end
...
if this.Event ue_show_closequery() = 1 then
	if ib_copy then ib_copy = false //Added By Jay Chen 07-23-2015
...
////Added By Ken.Guo 2011-05-05. Workaround Office OCX's bug  //Commented By Gavin.Jay 2017-04-19
...
If tab_contract_details.tabpage_images.tab_1.tabpage_data.is_show_activate_app <> "1" Or &
	tab_contract_details.tabpage_images.tab_1.tabpage_data.is_show_activate_per <> "1"  Then
	If gs_imageview_version = '5.0' and isvalid(tab_contract_details.tabpage_images.tab_1.tabpage_data.inv_dm_comparison)  Then
		tab_contract_details.tabpage_images.tab_1.tabpage_data.tab_1.tabpage_preview.ole_doc.object.activate(true)
	End If	
End If
...
//Commented By Ken.Guo 2017-05-16 move to pfc_update()
////Added By Jay Chen 01-26-2016 save screen field multi selelct item lists
//if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select) then
//	if of_check_mulselect_ctx_id('tabpage_1') then //Added By Jay Chen 03-09-2016
//		this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.update()
//	end if
//end if
//if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select) then
//	if of_check_mulselect_ctx_id('tabpage_custom') then
//		this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_custom.dw_1.iuo_select.ids_select_item_list.update()
//	end if
//end if
//end
...
//If IsValid (tab_contract_details.tabpage_images.tab_1.tabpage_data.inv_ole_utils_doc ) Then //Added By Jay 2017-01-18
//	tab_contract_details.tabpage_images.tab_1.tabpage_data.inv_ole_utils_doc.Of_Activate(false)
//End If
end event
...
event pfc_updateprep;call super::pfc_updateprep;If IsValid( tab_contract_details.tabpage_details.uo_1.dw_1 ) Then //added by gavins 20140211
	n_cst_contract   ln_cst_contract
	datastore		lds_null
	Long				ll_double_ctxid

	ln_cst_contract = create n_cst_contract
//	tab_contract_details.tabpage_details.uo_1.dw_1.AcceptText( )
	
	//Begin - Modified By Ken.Guo 07/08/2014
	//ll_double_ctxid = ln_cst_contract.of_checkdoublecontract( "save",tab_contract_details.tabpage_details.uo_1.dw_1,tab_contract_details.tabpage_search.il_ctx_id , lds_null )
	If tab_contract_details.tabpage_details.uo_1.dw_1.rowcount() > 0 Then 
		If isnull(tab_contract_details.tabpage_details.uo_1.dw_1.GetItemNumber(1, 'ctx_id')) Then
			ll_double_ctxid = ln_cst_contract.of_checkdoublecontract( "new",tab_contract_details.tabpage_details.uo_1.dw_1,tab_contract_details.tabpage_search.il_ctx_id , lds_null )
		Else
			ll_double_ctxid = ln_cst_contract.of_checkdoublecontract( "save",tab_contract_details.tabpage_details.uo_1.dw_1,tab_contract_details.tabpage_search.il_ctx_id , lds_null )
		End If
	End If
	//End - Modified By Ken.Guo 07/08/2014
	
	If ll_double_ctxid > 0 Then Return -1
End If



return 1 		//Added By Mark Lee 02/28/2014
end event

event pfc_update;call super::pfc_update;//Added By Jay Chen 11-18-2014 auto compute contract custom sepecial attribute data
string ls_enable_special_attribute
...
if ls_enable_special_attribute = '1' then
	of_compute_special_field_sum() 
end if

//Added By Ken.Guo 2017-05-16 Copy from pfc_save(). fixed a bug with compare WF.
//Added By Jay Chen 01-26-2016 save screen field multi selelct item lists
If AncestorReturnValue = 1 Then //Added By Ken.Guo 2017-05-16
	if isvalid(this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select) then
		if of_check_mulselect_ctx_id('tabpage_1') then //Added By Jay Chen 03-09-2016
			If this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.ids_select_item_list.update() = 1 Then
				this.tab_contract_details.tabpage_details.uo_1.tab_1.tabpage_1.dw_1.iuo_select.il_ctx_old = This.tab_contract_details.tabpage_details.inv_contract_details.of_get_ctx_id( )
...
//return 1 //Modified By Ken.Guo 2017-05-16
Return AncestorReturnValue
