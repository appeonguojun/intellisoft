...
string is_parent_window //Added By Jay Chen 04-01-2014

string is_mode,is_license_user_id //Added By Jay Chen 09-04-2014
datastore ids_screen_prop //Added By Jay Chen 07-24-2015
str_doc_compare istr_dm_compare //Added By Jay Chen 09-17-2015
long il_first_doc_row
boolean ib_compare = false 
long il_prior_notify_doc_id //Added By Ken.Guo 2016-04-05
string is_show_activate_app = "0", is_show_activate_per = "0" //Added By Gavin.Jay 2017-04-19
end variables
...
public subroutine of_auto_modify_owner (long al_ctx_id)
public subroutine of_show_approve_status (integer ai_detail)
public subroutine of_show_sertifi_status (long al_row, boolean ab_detail)
public function integer of_set_dw_properties (long al_data_view_id, long al_screen_id, string as_titlebar_visible_def)
public function integer of_removesignaturerequest_http (string as_agreementid)
end prototypes
...
boolean lb_auto_modify_owner = false //Added By Jay Chen 01-23-2014
String ls_auto_modify_owner //Added By Jay Chen 01-23-2014

as_action =  lower(as_action)
//--------Begin Modified by Alfee 09.30.2007-----------------------
//If of_check_read_only( ) Then Return   //Add by Jack 07.13.2007 //Added By Ken.Guo 2015-05-29. added  "and  as_action <> 'refresh_document_browse' "
IF of_check_read_only( ) THEN
	//IF as_action<>'output_email' and as_action<>'output_saveas' and as_action<>'output_print' and & //Alfee 07.23.2008	//Added By Mark Lee 03/03/2015 add ue_document_history adjust
	IF as_action<>'output_email' and as_action<>'output_saveas' and as_action<>'output_print' and as_action <> 'compare' and &
		as_action<>'select' and as_action<>'clausemap' and as_action<>'report' and as_action<>'properties'  and as_action<>'ue_document_history'  and  as_action <> 'refresh_document_browse' THEN
...
//IF as_action<>'add_buy' and as_action<>'add_sell' and as_action<>'add_other' and as_action<>'output_email' and as_action <> 'ue_document_history' and as_action <> 'refresh_document_browse' THEN		//Added By Mark Lee 09/11/2015 add 
IF as_action<>'add_buy' and as_action<>'add_sell' and as_action<>'add_other' and as_action<>'output_email' and &
	as_action <> 'ue_document_history' and as_action <> 'refresh_document_browse' and as_action <> 'batch_add'  THEN	
//---------------------------- APPEON END ---------------------------------------
...
CHOOSE CASE as_action
	CASE 'save'
		//This.Event ue_save() 
		li_ret = This.Event ue_save(FALSE)  
		if li_ret > 0 then lb_auto_modify_owner = true //Added By Jay Chen 01-23-2014
...
//			tab_1.Post SelectTab( 2 )			//Added By Mark Lee 05/07/2013
			...
			lb_auto_modify_owner = true //Added By Jay Chen 01-23-2014
		END IF
...
//			tab_1.Post SelectTab( 2 )				//Added By Mark Lee 05/07/2013
	...
	Case 'refresh_document_browse' //Added By Jay Chen 09-25-2014
		if  inv_ole_utils_doc.of_ismodified( ) then
			if MessageBox("Save Doucment", "The document is modified, do you want to save first?", Question!, YesNo!, 1) = 1 then
				This.Event ue_save(FALSE)  
			end if
		end if
		THIS.event ue_retrieve( )
	Case 'check_compare' //Added By Jay Chen 09-17-2015
		THIS.event ue_checkcompare( )
	case 'batch_add'					//Added By Mark Lee 09/08/2015
		lstr_add_doc.new = TRUE
		lstr_add_doc.ctx_id = inv_contract_details.of_get_ctx_id( )
		lstr_add_doc.doc_id = 0
		OpenWithParm(w_dm_batchadd_document, lstr_add_doc)
		
		IF Message.Doubleparm > 0 THEN
			il_doc_id = Message.Doubleparm

			If il_Add_Doc_ID > 0 Then
				THIS.event ue_retrieve( )
				post of_open_document( il_Add_Doc_ID )
				il_Add_Doc_ID = -1
			End If
			
			lb_auto_modify_owner = true 
		END IF
		
		If Message.StringParm = 'design' Then 
			SetPointer(HourGlass!)
			if IsValid(gw_Contract) then gw_Contract.of_SetCurrentScreenID(   51   )
			m_pfe_cst_mdi_menu.m_system.m_painters.m_contractdatascreenpainter.Post Event Clicked()
			SetPointer(Arrow!)
...
if inv_ole_utils_doc.of_ismodified( ) then
	if ls_use_web_merge = '1' and not lb_merged then
		if MessageBox("Save Doucment", "The document is modified, do you want to save first?", Question!, YesNo!, 1) = 1 then
			This.Event ue_save(FALSE)  
		end if
	end if
...
Next*/
//---------End Modified--------------------------------------------------------
//IF i > li_cnt Then Return //Modified By Jay Chen 08-19-2015 add header,footer if condition
IF i > li_cnt Then 
	OLEObject lole_header_fields,lole_footer_fields
	lole_header_fields = tab_1.tabpage_preview.ole_doc.object.activedocument.Sections.Item(1).Headers.Item(1).Range.Fields
	lole_footer_fields =   tab_1.tabpage_preview.ole_doc.object.activedocument.Sections.Item(1).Footers.Item(1).Range.Fields
	li_cnt = lole_header_fields.count 
	For i = 1 to li_cnt
		If lole_header_fields.Item[i].Type = wdFieldMergeField or lole_header_fields.Item[i].Type = wdFieldDocVariable Then Exit
	Next	
	if  i > li_cnt Then
		li_cnt = lole_footer_fields.count 
		For i = 1 to li_cnt
			If lole_footer_fields.Item[i].Type = wdFieldMergeField or lole_footer_fields.Item[i].Type = wdFieldDocVariable Then Exit
		Next	
	end if
...
//Added By Jay Chen 12-30-2015 for add document 
 if isnull(ll_export_id) then ll_export_id = 0
 if ll_export_id = 0 then
	Select export_id Into :ll_export_id From ctx_am_document Where doc_id = :il_preview_doc_id;
	if isnull(ll_export_id) then ll_export_id = 0
end if
//end
...
If tab_1.tabpage_preview.ole_doc.object.doctype > 0 Then tab_1.tabpage_preview.ole_doc.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
...
tab_1.tabpage_preview.ole_doc.of_set_trigger_event( False ) //Added By Ken.Guo 2016-10-27. workaround crash bug
...
If lower(as_mergemode) = 'auto' Then 
	...
	//li_rtn = inv_ole_utils_doc.of_mergedata_auto(ll_export_id, inv_contract_details.of_get_ctx_id( ),ls_merge_tmp_file,is_opened_file,true)	
		...
		li_rtn = inv_ole_utils_doc.of_useweb_mergedata_auto(ll_export_id, ll_ctx_id, ls_merge_tmp_file, is_opened_file, false, lstr_docinfo, il_preview_doc_id) //Added By Jay Chen 11-27-2014
	end if
...
Else  
	...
	if ls_use_web_merge = '0' then 
		...
		li_rtn = inv_ole_utils_doc.of_useweb_mergedata(ll_export_id, ll_ctx_id,ls_merge_tmp_file,is_opened_file,lb_merged,lstr_docinfo,il_preview_doc_id)  //Added By Jay Chen 11-27-2014
	end if
...
tab_1.tabpage_preview.ole_doc.Post of_set_trigger_event( True ) //Added By Ken.Guo 2016-10-27. workaround crash bug
end event
...
//				RETURN -1 
		...
		select modify_date into :ldt_modify_date from Ctx_am_doc_image where doc_id = :il_doc_id and revision = :ldec_new_revision;
...
long ll_export_id,ll_row,ll_template_id,ll_null
string ls_merge_field
//Added By Jay Chen 12-30-2015
string ls_add_doc_merge
ls_add_doc_merge = gnv_data.of_getitem("icred_settings","add_doc_merge",false)
if isnull(ls_add_doc_merge) or ls_add_doc_merge = '' then ls_add_doc_merge = '0'
setnull(ll_null)

IF IsNull(il_doc_id) THEN RETURN
If tab_1.tabpage_preview.ole_doc.object.DocType <> 1 Then return //no word document opened

Select export_id,From_templete_id into :ll_export_id,:ll_template_id
  From ctx_am_document
 Where doc_id = :il_doc_id;

if isnull(ll_template_id) then ll_template_id = 0
IF isnull(ll_export_id) THEN 
	if lower(is_doc_ext) <> 'doc' and lower(is_doc_ext) <> 'docx' then //Added By Jay Chen 07-06-2015
		messagebox("ContractLogix","There isn't a related export id found, please check it firstly!")
		RETURN
	end if
	//Added By Jay Chen 01-21-2016
	if (lower(is_doc_ext) = 'doc' or lower(is_doc_ext) = 'docx') and gs_esigntype <> 'docusign' then 
		messagebox("ContractLogix","If you want to insert a field into the document, please go to the Document Properties window to set the Export Name first. If this property is not on the Document Properties window, please add it in Screen Painter first.")
		RETURN
	end if
	//end
END IF
...
if ll_template_id = 0 then //Added By Jay Chen 12-30-2015 for add document
	if ls_add_doc_merge = '1' then
		lstr_pass.s_long[1] = ll_export_id
	else
		lstr_pass.s_long[1] = ll_null
	end if
else
	lstr_pass.s_long[1] = ll_export_id
end if
...
Do While ll_current_row > 0 
	...
	//Added By Ken.Guo 2016-10-18
	lstr_dm_email.fullfilename_list[i] = gf_merge_file_name(idw_LastDW.getitemstring(ll_current_row,'doc_name'),idw_LastDW.getitemstring(ll_current_row,'doc_ext'),idw_LastDW.getitemnumber(ll_current_row,'doc_id'),idw_LastDW.getitemdecimal(ll_current_row,'revision'))
...
//					Return
//				end if
				//Commented by harry 20170607 for task752:Allow the selection and check out multiple documents for E-Signature
//				if i > 1 and gs_echosign_method = 'http' then //Added By Jay Chen 02-17-2016
//					Messagebox('Check Out','You can only select one document to check out with Echosign HTTP mode.')
//					Return
//				end if
				if i > 6 then
					Messagebox('Check Out','You can at most select six documents for signature with Echosign Signature mode.')
					Return
				end if
...
//			RETURN			
//		End If
//	Case '7' //Added By Jay Chen 09-29-2013
//		//ls_checkout_status = '5'  //5: checked out; 6: all client users signed.
//		Event ue_checkout_echosign()
//		Return
END CHOOSE
...
Long ll_find_row1,ll_find_row2, ll_count = 0,ll_RtnVal,i, ll_doc_id, ls_err_cnt, ll_ctx_id
String ls_CurDir, ls_company_name, ls_doc_ext,ls_doc_name, ls_fileame
Decimal ldec_revision
u_dw_contract ldw_browse
String ls_include_ctxid, ls_include_company, ls_include_revision
...
If ll_count > 1 Then
	...
	//Added By Ken.Guo 12/17/2014. Support file name options.
	Open(w_saveas_file_options)
	If message.stringparm = '' Then Return
	If Len(message.stringparm) <> 3 Then Return
	ls_include_ctxid = Mid(message.stringparm,1,1)
	ls_include_company = Mid(message.stringparm,2,1)
	ls_include_revision = Mid(message.stringparm,3,1)
	...
	Do While ll_find_row1 > 0 
		...
		//Added By Ken.Guo 12/17/2014
		If iw_parent.classname() = 'w_contract' Then
			ll_ctx_id = inv_contract_details.of_get_ctx_id()
		Else
			ll_ctx_id = ldw_browse.getitemnumber(ll_find_row1,'ctx_id')
		End If
		...
		End If
		If ls_company_Name = '' Or IsNull( ls_company_name ) Then //added by gaivns 20150928
			ls_company_name = gnv_data.of_getitem( 'ctx_facility', 'facility_name',  'facility_id = ' + string(  gl_Company_id )  )
			
		End If
	
	
		//Begin - Modified By Ken.Guo 12/17/2014
		/*
		...
		End If
		*/
		ls_fileame = ls_CurDir + ''
		If ls_include_ctxid = '1' Then
			ls_fileame += 'CTX_' + String(ll_ctx_id) + '_'
		End If
		If ls_include_company = '1' and ls_company_name <> '' Then
			ls_fileame += ls_company_name + '_'
		End If
		ls_fileame += ls_doc_name
		If ls_include_revision = '1' Then
			ls_fileame += '_'+ String(ll_doc_id) + '_' + String(ldec_revision) +'.'+ String(ls_doc_ext)
		Else
			ls_fileame += '.'+ String(ls_doc_ext)
		End If
		//End - Modified By Ken.Guo 12/17/2014		
...
IF MessageBox("ContractLogix",ls_msg, Question!, YesNo!, 2 ) <> 1 THEN
	RETURN 0
END IF
...
//Added By Jay Chen 07-24-2015 set document search and browse screen properties when contract window is not open.
boolean lb_valid_contract = true
if not isvalid(gw_contract) then
	...
	lb_valid_contract = false
end if
//end

// Set Search Criteria screen properties
ldw_filter = tab_1.tabpage_browse.dw_filter
if lb_valid_contract then //Added By Jay Chen 07-24-2015 add if condition
	ldw_filter.of_set_properties(ldw_filter.il_ViewID, ldw_filter.il_ScreenID, "Y")
else
	 of_set_dw_properties(ldw_filter.il_ViewID, 59, "Y") //Added By Jay Chen 07-24-2015
	 ldw_filter.is_title_bar_text = ldw_filter.Title
end if 
...
If ii_ResizeWay = 1 Then
	...
	if lb_valid_contract then
		...
		ldw_screen.is_title_bar_text = ldw_screen.Title
		 ...
		 of_set_dw_properties(ldw_screen.il_ViewID, 60, "Y") //Added By Jay Chen 07-24-2015
		 ldw_screen.is_title_bar_text = ldw_screen.Title
	end if 
...
Else
	...
	//	ldw_browse.of_set_properties(ldw_filter.il_ViewID, ldw_filter.il_ScreenID, "Y")
		ldw_browse.of_set_properties(ldw_browse.il_ViewID, ldw_browse.il_ScreenID, "Y") //Modified By Jay Chen 07-24-2015
		...
		ldw_browse.is_title_bar_text = ldw_browse.Title
		 ...
		 of_set_dw_properties(ldw_browse.il_ViewID, 60, "Y") //Added By Jay Chen 07-24-2015
		 ldw_browse.Title = "Document Browse"
		 ldw_browse.is_title_bar_text = ldw_browse.Title
	end if
End If
...
//Added By Jay Chen 07-17-2014 remark: ocr can not use when ImageViewer2.OCX is ignored
if of_ignoreocx_check("ImageViewer2.OCX") then
	Messagebox("OCR","Can't OCR the document as the ImageViewer2.OCX is ignored.")
	gnv_debug.of_output(True, "Can't OCR the document as the ImageViewer2.OCX is ignored.")
	Return 
End If
//end add
...
If Upper(ls_doc_ext) = 'PDF' Then
	...
	//Added By Jay Chen 11-07-2014
	tab_1.tabpage_preview.uo_2.ib_have_got_PDF_Searchabled = True
	If Len(ls_ocr_text) > 0 Then
		tab_1.tabpage_preview.uo_2.ib_PDF_searchabled = True
	Else
		tab_1.tabpage_preview.uo_2.ib_PDF_searchabled = False
	End If
	//end
End If
...
inv_em_pop3 = Create n_cst_easymail_pop3  //Added By Ken.Guo 2015-07-27
...
If isvalid(inv_em_pop3) Then Destroy inv_em_pop3 //Added By Ken.Guo 2015-07-27
...
If ll_find_row1 > 0 Then
	...
	//	gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_output0.m_saveas,'enabled', False)		
		...
		gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_compare0,'enabled', true) //Added By Jay Chen 07-21-2015
	End If
End If
...
string ls_es_docid,ls_esign_type,ls_es_fileid,ls_license_user_id //Added By Jay Chen 09-05-2014
SetPointer(HourGlass!)
//if tab_1.tabpage_browse.dw_browse.object.checkout_status[tab_1.tabpage_browse.dw_browse.getrow()] = '1' then
if tab_1.tabpage_browse.dw_browse.object.checkout_status[tab_1.tabpage_browse.dw_browse.getrow()] <> '0' then	//1-Check out ,2- check out weblink --jervis 11.24.2010
	If Messagebox('ContractLogix','Do you want to undo check out for the document?', Question!, yesno!) = 2 Then Return
	Select es_docid,esign_type,es_fileid,es_license_user_id into :ls_es_docid,:ls_esign_type,:ls_es_fileid,:ls_license_user_id From Ctx_am_document Where doc_id = :il_doc_id;
	Update Ctx_am_document Set checkout_status = '0',checkout_by = '', checkout_date = null, 
			 ...
			 ,comments_temp = ''  //Modified by harry 2017-07-14
	Where doc_id = :il_doc_id;
	
	if sqlca.sqlcode = 0 then
		//Added By Jay Chen 04-11-2014  Remove request from docusign server
...
//		//Insert Action Item for E-signer
	...
	Else
		...
		lstr_sertifi_ref.as_sertifi_fileid = lstr_ctx_email.as_sertifi_fileid
		lstr_sertifi_ref.as_sertifi_docid[UpperBound(lstr_sertifi_ref.as_sertifi_docid[]) + 1] = lstr_ctx_email.as_sertifi_docid[i]		
...
This.post of_show_sertifi_status( ll_row,false ) //Added By Jay Chen 09-09-2014
...
if gs_echosign_method = 'http' and len(gnv_echosign.is_apicode) > 0 then //Modified by Harry 20170412
...
For i = 1 To UpperBound(lstr_ctx_email.doc_id_list[])
	If Len(lstr_ctx_email.as_sertifi_docid[i]) > 0 Then
		...
		If ll_find > 0 Then
			...
			lds_sertifi_update.SetItem(ll_find, 'esign_type',gs_esigntype)
			lds_sertifi_update.SetItem(ll_find, 'es_license_user_id',is_license_user_id) //Added By Jay Chen 09-04-2014
			lds_sertifi_update.SetItem(ll_find, 'echosign_method',gs_echosign_method) //Added By Jay Chen 02-17-2016
		Else
			//gnv_echosign.of_removesignaturerequest(lstr_sertifi_doc)
			if gs_echosign_method <> 'http' then //Modified by Harry 20170322
				gnv_echosign.of_removesignaturerequest(lstr_sertifi_doc)
			else
				of_removesignaturerequest_http(lstr_ctx_email.as_sertifi_fileid)
			end if
			Messagebox('Error','Failed to check out the document ' + lstr_ctx_email.fullfilename_list[i] + '.' )
			If isvalid(lds_sertifi_update) Then Destroy lds_sertifi_update
			Return
...
// Event: ue_sync_to_salesforce
//--------------------------------------------------------------------
// Description: sync document to salesforce
//--------------------------------------------------------------------
// Arguments:
//--------------------------------------------------------------------
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 12-16-2014
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
datawindow ldw_browse
long ll_import_id,ll_row,ll_ctx_id,ll_foundrow,ll_cnt,ll_rtn,ll_count,ll_valid_count
string ls_select_ctx_id[],ls_ctx_id ,ls_msg,ls_user,ls_pwd,ls_token,ls_doc_id,ls_select_doc_id[] 
n_cst_string lnv_arrstring
n_cst_webapi lnv_webapi
long ll_find,ll_doc_id,ll_product_id,ll_product_type
string ls_salesforce_id,ls_value, ls_sfurl
boolean lb_success = true

SELECT a.import_id into :ll_import_id FROM import_hdr a,import_table_records b
WHERE a.import_id = b.import_id
AND a.import_type = '2' AND a.import_flag = '1' 
AND a.sync_salesforce = 1 AND b.isoft_table_id = 1013;

if isnull(ll_import_id) then ll_import_id = 0
if ll_import_id <= 0 then 
	messagebox('Contractlogix','The system can not find the mapping for the Document Browse synchronization. Please make sure you have configured it correctly in the Data Synchronization Painter.')
	return 
else
	ldw_browse = tab_1.tabpage_browse.dw_browse
	If tab_1.selectedtab = 1 Then
		//get the select ctx_id,doc_id
		ll_foundrow = 0
		ll_row = ldw_browse.getselectedrow(0)
		Do While ll_row > 0
			If iw_parent.classname() <> 'w_contract' Then
				ll_ctx_id = ldw_browse.getitemnumber(ll_row,'ctx_id')
				ls_ctx_id = ls_ctx_id +  string(ll_ctx_id)+ ", "
			End If
			ll_doc_id = ldw_browse.getitemnumber(ll_row,'doc_id')
			ls_doc_id = ls_doc_id +  string(ll_doc_id)+ ", "
			ll_foundrow ++
			ll_row = ldw_browse.getselectedrow(ll_row)
		Loop
		If iw_parent.classname() <> 'w_contract' Then
			ls_ctx_id = left(ls_ctx_id, len(ls_ctx_id) -2 )
			lnv_arrstring.of_parsetoarray( ls_ctx_id, ',', ls_select_ctx_id[])
		end if
		ls_doc_id = left(ls_doc_id, len(ls_doc_id) -2 )
		lnv_arrstring.of_parsetoarray( ls_doc_id, ',', ls_select_doc_id[]) 
		
		if ll_foundrow = 1  then
			ls_msg = 'Are you sure you want to sync this document (Document ID = '+String(ls_doc_id)+') to SalesForce?'
		else
			if ll_foundrow > 0 then 
				ls_msg = 'Are you sure you want to sync these documents (Document ID = '+String(ls_doc_id)+') to SalesForce?'
			else
				return 
			end if
		end if
		...
		ls_pwd += ls_token
		if messagebox('Sync Document', ls_msg,Exclamation!,Yesno!,2) = 1 then
			for ll_cnt = 1 to upperbound(ls_select_doc_id)
				If iw_parent.classname() <> 'w_contract' Then
					ll_ctx_id = long(ls_select_ctx_id[ll_cnt])
				else
					ll_ctx_id = inv_contract_details.of_get_ctx_id()
				end if
				ll_doc_id = long(ls_select_doc_id[ll_cnt])
				//check the ctx_id if or not already sync to salesforce//Modified by gavins 20170330
//				select count(*) into :ll_count from import_outbound_records where lower(ctx_tablename) = 'ctx_basic_info' and lower(ctx_fieldname) = 'ctx_id' and ctx_record_id = :ll_ctx_id and export_type = 1;
//				if isnull(ll_count) then ll_count = 0
//				if ll_count = 0 then 
//					messagebox('Sync Document', 'The contract (ID = '+String(ll_ctx_id)+') has not been synchronized to salesforce.~r~nPlease go to the Contract Browse to synchronize the contract first.')
//					return
//				end if
				
				//check the doc_id if or not already sync to salesforce
				ls_value = string(ll_ctx_id) + ';' + string(ll_doc_id)
				select count(*) into :ll_count from import_outbound_records where lower(ctx_tablename) = 'ctx_am_document' and lower(ctx_fieldname) = 'ctx_id;doc_id' and ctx_record_id = :ls_value and export_type = 1;
				if isnull(ll_count) then ll_count = 0
				if ll_count > 0 then 
					if messagebox('Sync Document', 'The document (Document ID = '+String(ll_doc_id)+') has already been synchronized to SalesForce.~r~nAre you sure you want to overwrite the document information on SalesForce?',Exclamation!,Yesno!,2) <> 1 then
						continue
					end if
				end if
				
				//begin to sync the document_id
				//ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', ll_ctx_id, ll_doc_id, ll_product_id, ll_product_type, ll_import_id, ls_salesforce_id, ls_sfurl)
				ll_rtn = lnv_webapi.of_outboundexternaldata(ls_user,ls_pwd, '1', ll_ctx_id, ll_doc_id, ll_product_id, ll_product_type, ll_import_id, ls_salesforce_id, ls_sfurl, 0, 0)  //modified by harry 20170320 for task749
				if len(ls_salesforce_id) <= 0 then
					lb_success = false
					messagebox('Sync Document', 'Failed to synchronize the document (Document ID = '+String(ll_doc_id)+') to SalesForce.')
				else
					ll_valid_count++
				end if
			next
		end if
		if lb_success and ll_valid_count > 0 then
			messagebox('Sync Document', 'Succeed to synchronize the document to SalesForce.')
		end if
	end if
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 09-17-2015
//--------------------------------------------------------------------
...
//====================================================================
Integer li_rtn,i //03.28.2008
String ls_notes,ls_fullname
str_add_doc lstr_add_doc_arr[],lstr_add_doc
str_checkout lstr_checkout
str_ctx_email lstr_dm_email
str_add_audit lstr_add_audit[]
//n_cst_dm_utils lnv_dm_utils
Long ll_current_row,ll_cnt,ll_row,ll_foundrow,ll_foundrow1,ll_foundrow2,ll_audit_id,ll_doc_id
str_add_doc_multi lstr_add_doc_multi
string ls_docext,ls_filetype,ls_action_name,ls_checkout_status
decimal{1} ld_rev1,ld_rev2,ld_revision
long ll_select[],ll_selectrow,ll_count
...
Do While ll_row > 0
	ll_foundrow1 ++
	ls_docext = tab_1.tabpage_browse.dw_browse.getitemstring(ll_row, "doc_ext")
	if isnull(ls_docext) then ls_docext = ''
	if ls_docext = '' or (lower(ls_docext) <> 'doc' and lower(ls_docext) <> 'docx') then
		MessageBox("Tips","The selected document is not a Word document!") 
		istr_dm_compare = lstr_dm_compare
		return
	end if
	ll_row = tab_1.tabpage_browse.dw_browse.Getselectedrow(ll_row)
Loop

ll_row = tab_1.tabpage_browse.dw_detail.Getselectedrow(0)
Do While ll_row > 0
	ll_foundrow2 ++
	ll_select[upperbound(ll_select) + 1] = ll_row
	ll_row = tab_1.tabpage_browse.dw_detail.Getselectedrow(ll_row)
Loop

//temperature open. modified by gavins 20150928 //Modified By Jay Chen 11-05-2015
if ll_foundrow1 > 2 then
	MessageBox("Tips","Please select two copies of the document comparison!") 
	istr_dm_compare = lstr_dm_compare
	for ll_cnt = 1 to tab_1.tabpage_browse.dw_browse.rowcount()
		tab_1.tabpage_browse.dw_browse.selectrow(ll_cnt, false)
	next
	return
//elseif ll_foundrow1 = 1 then
//	if ll_foundrow2 <> 2 then
//		MessageBox("Tips","You must only choose two copies of the document!") 
//		for ll_cnt = 1 to tab_1.tabpage_browse.dw_detail.rowcount()
//			tab_1.tabpage_browse.dw_detail.selectrow(ll_cnt, false)
//			tab_1.tabpage_browse.dw_detail.setitem(ll_cnt, "compare_status","")
//		next
//		return
//	end if
end if

//Added By Jay Chen 11-24-2015  //only press KeyControl for history
if ll_foundrow2 = 2 then
	for ll_cnt = 1 to upperbound(ll_select)
		ll_selectrow = ll_select[ll_cnt]
		ll_doc_id = tab_1.tabpage_browse.dw_detail.getitemnumber(ll_selectrow, "doc_id")
		ld_revision = tab_1.tabpage_browse.dw_detail.getitemdecimal(ll_selectrow,'audit_version')
		if ll_doc_id = istr_dm_compare.doc_id2 and ld_revision = istr_dm_compare.revision2 then
			ll_count++
		else
			if istr_dm_compare.doc_id1 = 0 then
				istr_dm_compare.doc_id1 =  ll_doc_id
				istr_dm_compare.revision1 = ld_revision
				istr_dm_compare.filename1 = tab_1.tabpage_browse.dw_detail.getitemstring(ll_selectrow,'doc_name')
				ls_fullname = tab_1.tabpage_browse.dw_detail.getitemstring(ll_selectrow,'doc_name') + ' ' + String( ll_doc_id )+ '.' + String( ld_revision ,'##0.0')
				istr_dm_compare.fullname1 = ls_fullname
				istr_dm_compare.doc_ext1 = tab_1.tabpage_browse.dw_detail.getitemstring(ll_selectrow,'doc_ext')
				istr_dm_compare.revision_control1 = (tab_1.tabpage_browse.dw_detail.GetItemString(ll_selectrow, "revision_control") = "1")
				istr_dm_compare.ctx_id1 = istr_dm_compare.ctx_id2
			end if
		end if
	next
end if
...
if ll_foundrow1 = 1 and ll_foundrow2 <= 1 then
	if istr_dm_compare.doc_id1 = 0 then
		ll_selectrow = tab_1.tabpage_browse.dw_browse.Getselectedrow(0)
		ll_doc_id = tab_1.tabpage_browse.dw_browse.getitemnumber(ll_selectrow, "doc_id")
		ld_revision = tab_1.tabpage_browse.dw_browse.getitemdecimal(ll_selectrow,'revision')
		istr_dm_compare.doc_id1 =  ll_doc_id
		istr_dm_compare.revision1 = ld_revision
		istr_dm_compare.filename1 = tab_1.tabpage_browse.dw_browse.getitemstring(ll_selectrow,'doc_name')
		ls_fullname = tab_1.tabpage_browse.dw_browse.getitemstring(ll_selectrow,'doc_name') + ' ' + String( ll_doc_id )+ '.' + String( ld_revision ,'##0.0')
		istr_dm_compare.fullname1 = ls_fullname
		istr_dm_compare.doc_ext1 = tab_1.tabpage_browse.dw_browse.getitemstring(ll_selectrow,'doc_ext')
		istr_dm_compare.revision_control1 = (tab_1.tabpage_browse.dw_browse.GetItemString(ll_selectrow, "revision_control") = "1")
		if iw_parent.classname() = 'w_contract' Then
			istr_dm_compare.ctx_id1 = inv_contract_details.of_get_ctx_id()
		Else
			istr_dm_compare.ctx_id1 = tab_1.tabpage_browse.dw_browse.getitemnumber(ll_selectrow,'ctx_id')
		End If
	end if
end if

//Added By Jay Chen 11-27-2015
if ll_foundrow1 = 2 and ll_foundrow2 = 0 then //only press KeyControl for browse
	if istr_dm_compare.doc_id1 = 0 then
		ll_selectrow = tab_1.tabpage_browse.dw_browse.Getselectedrow(0)
		ll_doc_id = tab_1.tabpage_browse.dw_browse.getitemnumber(ll_selectrow, "doc_id")
		if ll_doc_id = istr_dm_compare.doc_id2 then
			ll_selectrow = tab_1.tabpage_browse.dw_browse.Getselectedrow(ll_selectrow)
			ll_doc_id = tab_1.tabpage_browse.dw_browse.getitemnumber(ll_selectrow, "doc_id")
		end if
		ld_revision = tab_1.tabpage_browse.dw_browse.getitemdecimal(ll_selectrow,'revision')
		istr_dm_compare.doc_id1 =  ll_doc_id
...
if ll_count = 2  then
	MessageBox("Tips","Please select two different history versions.") 
	istr_dm_compare = lstr_dm_compare
	for ll_cnt = 1 to tab_1.tabpage_browse.dw_detail.rowcount()
		tab_1.tabpage_browse.dw_detail.selectrow(ll_cnt, false)
		tab_1.tabpage_browse.dw_detail.setitem(ll_cnt, "compare_status","")
...
if istr_dm_compare.doc_id1 > 0 then
	ls_docext = istr_dm_compare.doc_ext1 
	if isnull(ls_docext) then ls_docext = ''
	if ls_docext = '' or (lower(ls_docext) <> 'doc' and lower(ls_docext) <> 'docx') then
		MessageBox("Tips","The selected document is not a Word document!") 
		istr_dm_compare = lstr_dm_compare
...
if istr_dm_compare.doc_id2 > 0 then
	ls_docext = istr_dm_compare.doc_ext2 
	if isnull(ls_docext) then ls_docext = ''
	if ls_docext = '' or (lower(ls_docext) <> 'doc' and lower(ls_docext) <> 'docx') then
		MessageBox("Tips","The selected document is not a Word document!") 
		istr_dm_compare = lstr_dm_compare
...
if istr_dm_compare.doc_id1 > 0 and istr_dm_compare.doc_id2 > 0 then
	if istr_dm_compare.doc_id1 = istr_dm_compare.doc_id2 then
		if istr_dm_compare.revision1 = istr_dm_compare.revision2 then
			MessageBox("Tips","Please select two different history versions.") 
			istr_dm_compare = lstr_dm_compare
			for ll_cnt = 1 to tab_1.tabpage_browse.dw_detail.rowcount()
				tab_1.tabpage_browse.dw_detail.selectrow(ll_cnt, false)
...
select max(audit_id) into :ll_audit_id from ctx_am_doc_audit where doc_id = :istr_dm_compare.doc_id1;
...
values(:ll_audit_id,:istr_dm_compare.doc_id1,:istr_dm_compare.revision1,:istr_dm_compare.doc_ext1,'Viewed in comparison window',:gs_user_id,getdate());

if istr_dm_compare.doc_id2 > 0 then
	ll_audit_id ++
	insert into ctx_am_doc_audit(audit_id,doc_id,revision,doc_ext,action,action_by,action_date)
	values(:ll_audit_id,:istr_dm_compare.doc_id2,:istr_dm_compare.revision2,:istr_dm_compare.doc_ext2,'Viewed in comparison window',:gs_user_id,getdate());
end if

openwithparm(w_dm_check_compare, istr_dm_compare)	
...
string ls_revision //Added By Ken.Guo 2016-04-19
...
if not isnull(ls_doc_type) and ls_doc_type<>'A'  and ls_doc_type<>' ' and ls_doc_type<>''  then 
...
if not isnull(ldc_revision) then 
	//Modified By Ken.Guo 2016-04-19
	//ls_filter += "and revision = " + string(ldc_revision) + " "
	ls_revision = string(ldc_revision)
	ls_revision = gnv_string.of_globalreplace( ls_revision, ',', '.')
	ls_filter += "and revision = " + ls_revision + " "
...
//Added By Jay Chen 03-06-2014 Remark: add custom fields
...
//ll_level = tab_1.tabpage_browse.dw_browse.getitemnumber(ll_row,'level')
If idw_LastDW.Getrow( ) > 0 Then 
	ll_level = idw_LastDW.GetItemNumber( idw_LastDW.GetRow( ), 'level' )
	if idw_LastDW.classname() = 'dw_browse' then ll_export_id = idw_LastDW.GetItemNumber( idw_LastDW.GetRow( ), 'export_id' ) //Added By Jay Chen 12-30-2015
end if
...
ELSE
	...
	//Added By Mark Lee 07/20/12		add sys_tracking_log record. //Modified By Ken.Guo 2016-04-05: move here from top.
	Boolean  lb_fullright = True
	If isvalid(m_pfe_cst_mdi_menu_contract_am) Then
		If Not m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_save0.toolbaritemvisible Then lb_fullright = False
		If Not m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_save0.enabled  Then lb_fullright = False
	End If
	If il_prior_notify_doc_id > 0 Then //Added By Ken.Guo 2016-04-05
		gnv_tracking.of_tracking_doc( gnv_tracking.document_management,string(il_prior_notify_doc_id),'','','','delete_doc_tracking',false )
	End If
	If lb_fullright Then
		gnv_tracking.of_tracking_doc(gnv_tracking.document_management,string(il_doc_id),'','','','add_doc_tracking',false)
		il_prior_notify_doc_id = il_doc_id 
	End If
	...
	IF Upper(tab_1.tabpage_browse.dw_browse.object.doc_type[tab_1.tabpage_browse.dw_browse.getrow()]) = "SELL" THEN
		...
		END IF
	elseif ls_add_doc_merge = '1' and ll_export_id > 0 then //Added By Jay Chen 12-30-2015 for add document
		ib_allowtrack = FALSE
		THIS.Event ue_mergedata('auto') //check merging data
		inv_ole_utils_doc.of_auto_numbering() //numbering after merged		
		ib_allowtrack = TRUE
	...
	//Modified By Ken.Guo 2015-08-05.  Keep tracks
	//inv_ole_utils_doc.of_trackchange_ini() //Initialize the review setting //05.09.2007 Alfee
	inv_ole_utils_doc.of_modify_word_property()
...
//Load image document
//Added By Ken.Guo 07/17/2014. From Jay
if of_ignoreocx_check("ImageViewer2.OCX") then
	Messagebox('Open Document','Cannot open the document as the ImageViewer2.OCX is ignored.')
	gnv_debug.of_output(True, 'Cannot open the document as the ImageViewer2.OCX is ignored.')
	Return 
End If
//end add
//-----------Begin Modified by Alfee on 05.19.2008 ----------------	
...
//	st_docinfo.text = tab_1.tabpage_browse.dw_browse.GetItemString(tab_1.tabpage_browse.dw_browse.getrow(),"Doc_name_full") 
	...
	If is_doc_ext = 'tif' or is_doc_ext = 'tiff' Then
		Boolean  lb_fullright = True
		If isvalid(m_pfe_cst_mdi_menu_contract_am) Then
			If Not m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_save0.toolbaritemvisible Then lb_fullright = False
			If Not m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_save0.enabled  Then lb_fullright = False
		End If			
		If il_prior_notify_doc_id > 0 Then
			gnv_tracking.of_tracking_doc( gnv_tracking.document_management,string(il_prior_notify_doc_id),'','','','delete_doc_tracking',false )
		End If	
		If lb_fullright Then
			gnv_tracking.of_tracking_doc(gnv_tracking.document_management,string(il_doc_id),'','','','add_doc_tracking',false) 
...
If lw_email_edit.ole_word.object.doctype > 0 Then lw_email_edit.ole_word.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
...
public subroutine of_show_approve_status ();//Modified By Ken.Guo 04/15/2014
of_show_approve_status(0)
	...
	if IsValid(m_pfe_cst_mdi_menu_contract_am) then
		...
		If ll_Right<>0  Then // add by gavins 20120201
			...
			If iw_Parent.ClassName() = 'w_contract' Then
				...
				If w_mdi.of_security_access(6995) = 0 Then			
					ls_Option_Value = 'false'
				End If
...
//Modified By Jay Chen 04-02-2014
...
If li_Y > 0 Then
	...
	If tab_1.tabpage_browse.st_vsplitbar.Y >  li_Y + 4 Or  tab_1.tabpage_browse.st_vsplitbar.Y <  li_Y - 4 Then
		if is_parent_window <> 'w_documents_browse' then //Modified By Jay Chen 04-02-2014
			If tab_1.tabpage_browse.dw_detail.Visible  Then 
...
elseif ii_ResizeWay = 2 and is_parent_window = 'w_documents_browse' then //Added By Jay Chen 04-01-2014
...
Long				ll_cnt, ll_doc_id, ll_Find,ll_i
boolean			lb_comparison
String				ls_current_flag, ls_flag, ls_Doc_ID
decimal			ldc_revision
DateTime		ldt_LastDateTime
String ls_Edit_Style,ls_objects,ls_type  //Added By Jay Chen 09-02-2014
string ls_name[]
...
If li_Rows > 0   Then//and ib_controalinsert
	...
	Else
		...
		for li = 1 to li_Rec
			...
			adw_detail.setitem(li_New,'esign_type',ds_audit.getitemstring(li,'esign_type'))	
			adw_detail.setitem(li_New,'esign_license_user_id',ds_audit.getitemstring(li,'es_license_user_id'))	//Added By Jay Chen 09-09-2014
			adw_detail.setitem(li_New,'echosign_method',ds_audit.getitemstring(li,'echosign_method')) //Added By Jay Chen 02-17-2016
...
//Added By Jay Chen 09-02-2014 retrieve dddw data
if adw_detail.RowCount( ) > 0 then 
	ls_objects = adw_detail.Describe( "datawindow.objects")
	lvn_string.of_parsetoarray( ls_objects, "~t", ls_name)
	ll_cnt = UpperBound(ls_name)
	for ll_i = 1 to ll_cnt
		ls_type = adw_detail.describe( ls_name[ll_i] +".type")
		if ls_type = 'column'  then
			ls_Edit_Style = adw_detail.describe(ls_name[ll_i] + ".Edit.Style")
			if ls_Edit_Style = "dddw" then
				adw_detail.GetChild( ls_name[ll_i] , dwchild )
...
// Returns:  (None)
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 01-23-2014
//--------------------------------------------------------------------
//	Copyright (c) 2008-2013 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
long ll_row,ll_found,ll_cnt,ll_find,ll_rowcount
datastore lds_contact_user
str_contact lstr_contact
boolean lb_update_owner = false,lb_edit = false
string ls_userid,ls_default_email,ls_owner
long ll_contact_id,ll_record_id,ll_old_tab
...
//check if the owner is exists
SELECT count(*) into :ll_cnt FROM ctx_contract_contacts 
WHERE ctx_id=:al_ctx_id and contact_id = :lstr_contact.al_contact_id and ic_area = 'I' and default_email='Y' and owner='Y';
if isnull(ll_cnt) then ll_cnt = 0
if ll_cnt = 0 then
	if messagebox(gnv_app.iapp_object.DisplayName,'Do you want to set yourself as the the Owner for this contract in Our Company Contact?',question!,yesnocancel!) <> 1 then 
		return
	end if
else
	return
...
if isvalid(w_contract) then
	ll_rowcount = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.rowcount()
	ll_old_tab = w_contract.tab_contract_details.selectedtab
	if ll_rowcount < 1 then
		w_contract.setredraw(false)
		w_contract.of_select_tabpage("tabpage_details")
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.event pfc_retrieve()
		ll_rowcount = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.rowcount()
	end if
	ll_find = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.find("Upper(user_d) = '"+Upper(lstr_contact.as_user_id)+"'",1, ll_rowcount)
	if ll_find > 0 then 
		ll_contact_id = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.getitemnumber(ll_find,"contact_id")
		ls_default_email = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.getitemstring(ll_find,"default_email")
		ls_owner = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.getitemstring(ll_find,"ctx_contract_contacts_owner")
		if (ls_default_email <> 'Y' or ls_owner <> 'Y')  then
			update ctx_contract_contacts set default_email='Y',owner='Y' where ctx_id = :al_ctx_id and contact_id = :ll_contact_id and ic_area = 'I';
			update ctx_contract_contacts set default_email='N',owner='N' where ctx_id = :al_ctx_id and contact_id <> :ll_contact_id and ic_area = 'I';
			if ls_default_email <> 'Y' then
				w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_find,"default_email", 'Y')
			end if
			if ls_owner <> 'Y' then
				w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_find,"ctx_contract_contacts_owner", 'Y')
			end if
			lb_edit = true
		else
			return
		...
		values(:al_ctx_id,:lstr_contact.al_contact_id,'I',:lstr_contact.al_contact_type,'Y',:ll_record_id,'Y','F');
		update ctx_contract_contacts set default_email='N',owner='N' where ctx_id = :al_ctx_id and contact_id <> :lstr_contact.al_contact_id and ic_area = 'I';
		ll_row = w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.insertrow(0)
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"default_email", 'Y')
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"ctx_contract_contacts_owner", 'Y')
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"ctx_id", al_ctx_id)
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItem(ll_row,"record_id", ll_record_id)
		lb_edit = false
	end if
	commit;
	//write audit log
	w_contract.inv_audit.of_set_prac_id(al_ctx_id)
	w_contract.inv_audit.of_audit_fields(w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ)
	//refresh screen data
	w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.event pfc_retrieve()
end if
...
//	w_contract.of_select_tabpage("tabpage_details")
	w_contract.tab_contract_details.selectedtab = ll_old_tab
	w_contract.setredraw(true)
	if lb_edit then
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItemStatus(ll_find,"default_email", Primary!, NotModified!)
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItemStatus(ll_find,"ctx_contract_contacts_owner", Primary!, NotModified!)
	else
		w_contract.tab_contract_details.tabpage_details.uo_1.dw_occ.SetItemStatus(ll_find,0, Primary!, NotModified!)
	end if
end if

end subroutine

public subroutine of_show_approve_status (integer ai_detail);str_ctx_email lstr_ctx_email
...
//Added By Ken.Guo 04/15/2014
Long ll_approve_id
If ai_detail = 1 Then
	ll_approve_id = This.tab_1.tabpage_browse.dw_detail.GetItemNumber(This.tab_1.tabpage_browse.dw_detail.GetRow(), 'approve_id') 
	If ll_approve_id > 0 and ll_approve_id <> lstr_ctx_email.al_approve_id Then
		lstr_ctx_email.al_approve_id = ll_approve_id
...
public subroutine of_show_sertifi_status (long al_row, boolean ab_detail);str_sertifi_doc lstr_sertifi_doc,lstr_sertifi_doc_ret

If al_row <= 0 Then Return

if not ab_detail then //Added By Jay Chen 09-09-2014 browse electronic status
	...
	If (lstr_sertifi_doc.as_es_apicode = '' or isnull(lstr_sertifi_doc.as_es_apicode)) and not (gs_echosign_method = 'http' and gs_esigntype= 'echosign') Then Return  //Modified by Harry 2017-01-18
...
else //history electronic status
	lstr_sertifi_doc.as_es_apicode = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'es_apicode')
	//If lstr_sertifi_doc.as_es_apicode = '' or isnull(lstr_sertifi_doc.as_es_apicode) Then Return
	If (lstr_sertifi_doc.as_es_apicode = '' or isnull(lstr_sertifi_doc.as_es_apicode)) and not (gs_echosign_method = 'http' and gs_esigntype= 'echosign') Then Return  //Modified by Harry 2017-01-18

	lstr_sertifi_doc.al_doc_id = tab_1.tabpage_browse.dw_detail.GetItemNumber(al_row, 'doc_id')
	lstr_sertifi_doc.as_es_fileid  = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'es_fileid')
	lstr_sertifi_doc.as_es_docid = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'es_docid')
	lstr_sertifi_doc.as_es_sender = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'es_sender')
	lstr_sertifi_doc.as_es_email = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'es_sender_email')
	lstr_sertifi_doc.as_checkout_by = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'checkout_by')
	lstr_sertifi_doc.as_checkout_status = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'checkout_status')
	lstr_sertifi_doc.as_filename = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'doc_name')
	lstr_sertifi_doc.as_type = 'dm'
	lstr_sertifi_doc.as_esign_type = tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'esign_type')
	lstr_sertifi_doc.as_esign_license_user_id =  tab_1.tabpage_browse.dw_detail.GetItemString(al_row, 'esign_license_user_id')
	lstr_sertifi_doc.ab_history = true
end if
...
if iw_parent.classname() = 'w_contract' Then
	ll_ctx_id = inv_contract_details.of_get_ctx_id()
Else
	ll_ctx_id = tab_1.tabpage_browse.dw_browse.getitemnumber(al_row,'ctx_id')
End If
lstr_sertifi_doc.al_ctx_id = ll_ctx_id
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 07-24-2015
//--------------------------------------------------------------------
//	Copyright (c) 2008-2014 ContractLogix, All rights reserved.
//--------------------------------------------------------------------
// Modify History:
...
choose case al_screen_id
	case 59
		ldw_properties = tab_1.tabpage_browse.dw_filter
	case 60
		ldw_properties = tab_1.tabpage_browse.dw_browse
end choose
...
public function integer of_removesignaturerequest_http (string as_agreementid);//Added by Harry 20170322
//when set echosign to http,  remove esignature request.
n_cst_webapi lnv_webapi
string ls_mode, ls_esign_token
Long ll_ret
...
if ls_mode = "global_license" then
	...
	SELECT esign_access_token into :ls_esign_token FROM personal_license_setting WHERE esign_type='echosign' and user_id = :gs_user_id;
end if

if isnull(gs_accesstoken) or trim(gs_accesstoken) = '' then gs_accesstoken = lnv_webapi.of_convert_json_token()

ll_ret = lnv_webapi.of_esign_delete_document(gs_accesstoken, ls_esign_token, as_agreementid)
...
if iw_parent.classname() = 'w_contract' Then
	tab_1.tabpage_browse.st_vsplitbar.Visible = true
End If
is_parent_window = iw_parent.classname()  //Added By Jay Chen 04-01-2014
...
long ll_export_id //Added By Jay Chen 12-30-2015
string ls_add_doc_merge
...
//Added By Jay Chen 12-30-2015
If idw_current.rowcount() > 0 and ib_detailrow Then
	ll_export_id = idw_current.getitemnumber(idw_current.getrow(),'export_id') 
	if isnull(ll_export_id) then ll_export_id = 0
End If
//end
...
If iw_parent.classname() = 'w_contract' Then
	...
	CHOOSE CASE li_setting
		...
		CASE 1	//Buy/Sell
			...
			If w_mdi.of_security_access( 6901 ) = 0 Then
				...
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd,'toolbaritemvisible', False)			//Added By Mark Lee 09/18/2015
			Else
				...
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd,'toolbaritemvisible', True)			//Added By Mark Lee 09/18/2015
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd,'enabled', True)
		...
		CASE 3	//Buy
			...
			Else
				...
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd,'toolbaritemvisible', True)					//Added By Mark Lee 09/18/2015				
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd,'enabled', True)					//Added By Mark Lee 09/18/2015
				//Set Current Menu Item  by Jervis 02.12.2009
				if li_temp = 1 then m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.CurrentItem =m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_addbuy
			End If
			If w_mdi.of_security_access( 6926 ) = 0 Then
	...
	If w_mdi.of_security_access( 7018 ) = 0 Then 
		gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_checkcompare,'toolbaritemvisible', False)
	Else
		gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_checkcompare,'toolbaritemvisible', True)
		gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_checkcompare,'enabled', True)
	End If
	//end
...
Else
	...
	gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_refreshdocumentbrowse,'toolbaritemvisible', False) //Added By Jay Chen 09-25-2014
//	gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_system.m_painters.m_contractdatascreenpainter,'toolbaritemvisible', False) //Modified By Jay Chen 04-02-2014
	gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_checkcompare,'toolbaritemvisible', False) //Added By Jay Chen 09-21-2015
End If
...
If idw_current.rowcount() > 0 Then
	...
	CHOOSE CASE tab_1.Selectedtab
		...
		CASE 2	// PREVIEW TAB		
			...
			End If
			//Added By Jay Chen 01-22-2015
			If w_mdi.of_security_access( 6906 ) = 1 Then
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_output0.m_print0,'enabled', False)
			end if
			//end
			...
			End If
			//Added By Jay Chen 12-30-2015
			if ls_add_doc_merge = '1' and (ls_type = "buy" or ls_type = "other") and ll_export_id > 0 then
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergefield,'enabled', true)	
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergedata,'enabled', true)
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_refresh,'enabled', true)
			end if
			...
			If idw_current.getitemnumber(idw_current.getrow(),'level') = 1 THEN
				If idw_current.GetitemString(idw_current.getrow(),'base_type') <> '1' Then
					gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge,'enabled', False)	
					//Modified By Jay Chen 07-06-2015
//					gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergefield,'enabled', False)	
					if lower(is_doc_ext) = 'doc' or lower(is_doc_ext) = 'docx' then //Added By Jay Chen 07-06-2015
						gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergefield,'enabled', true)	
					else
						gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergefield,'enabled', False)
					end if
					...
					gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_refresh,'enabled', False)	//Alfee 11.12.2007	
					//Added By Jay Chen 12-30-2015 for add document
					if ls_add_doc_merge = '1' and ll_export_id > 0 then
						gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergefield,'enabled', true)	
						gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_merge.m_mergedata,'enabled', true)
						gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_refresh,'enabled', true)
					end if
	...
	Else
		If Len(idw_current.GetItemString(idw_current.getrow(),'es_docid')) > 0  Then
			...
			If idw_current.GetItemString(idw_current.getrow(),'esign_type') = 'sertifi'  Then 
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_sertify,'toolbaritemname', 'sertify.jpg') 
			else
				gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_sertify,'toolbaritemname', 'echosign.jpg')
			end if
			//end add
		Else
			gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_sertify,'toolbaritemvisible', False) 
...
//Added By Jay Chen 03-04-2015
boolean lb_paste,lb_ocr,lb_pdf,lb_copy
lb_paste = tab_1.tabpage_preview.uo_2.uo_toolbar.pb_paste.visible
lb_ocr = tab_1.tabpage_preview.uo_2.uo_toolbar.pb_ocr.visible
lb_pdf = tab_1.tabpage_preview.uo_2.uo_toolbar.pb_pdf.visible
lb_copy = tab_1.tabpage_preview.uo_2.uo_toolbar.pb_copy.visible
//end
...
IF lb_protect THEN
	...
	gnv_app.of_modify_menu_attr( m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd,'enabled', False)					//Added By Mark Lee 09/18/2015
...
if not lb_protect then
	tab_1.tabpage_preview.uo_2.uo_toolbar.pb_paste.visible = lb_paste
	tab_1.tabpage_preview.uo_2.uo_toolbar.pb_ocr.visible = lb_ocr
	tab_1.tabpage_preview.uo_2.uo_toolbar.pb_pdf.visible = lb_pdf
	tab_1.tabpage_preview.uo_2.uo_toolbar.pb_copy.visible = lb_copy
end if
//end
...
if NewIndex = 2 then
	...
	is_show_activate_app = gnv_user_option.of_get_option_value('contractlogix-super-master','office_show_activate_buttons' )
	is_show_activate_per = "1"//gnv_user_option.of_get_option_value(gs_user_id,'office_show_activate_buttons' )  Modified by gavins 21070615 hide
	IF is_show_activate_app = "1" And is_show_activate_per = "1" Then
		tab_1.tabpage_preview.ole_doc.object.AddCustomButtonOnMenu(1,"Activate",true, 1)
		tab_1.tabpage_preview.ole_doc.object.AddCustomButtonOnMenu(2,"Deactivate",true, 2)
	Else
		tab_1.tabpage_preview.ole_doc.object.RemoveCustomButtonOnMenu(1)
		tab_1.tabpage_preview.ole_doc.object.RemoveCustomButtonOnMenu(2)
	End If
...
event selectionchanging;//Added By Ken.Guo 2016-04-05
Long ll_Row, ll_RowCnts
str_sys_tracking_log_arr lstr_sys_tracking_log_arr
String ls_user,ls_checkout_status
Boolean lb_fullright = True
If gnv_data.of_getitem( 'icred_settings', 'notify_user_dm', False) = '1'  and newindex = 2 Then
	ll_Row = tabpage_browse.dw_browse.GetRow()
	If ll_Row <= 0 Then Return 0
	ll_RowCnts = tabpage_browse.dw_browse.RowCount()
	if ll_Row <= ll_RowCnts and il_doc_id <= 0 then //for first row not trigger RowFocusChanged event.
		tabpage_browse.dw_browse.Event RowFocusChanged(ll_Row)
	end if	
	ls_checkout_status = tabpage_browse.dw_browse.getitemstring(ll_Row, 'checkout_status')
	If isvalid(m_pfe_cst_mdi_menu_contract_am) Then
		If Not m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_delete0.toolbaritemvisible Then lb_fullright = False
		If Not m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_delete0.enabled  Then lb_fullright = False
	End If
	If isnull(ls_checkout_status) then ls_checkout_status = '0'
	If ls_checkout_status = '0' and il_doc_id > 0 and lb_fullright and (is_doc_ext = 'doc' or is_doc_ext = 'docx' or is_doc_ext = 'xls' or is_doc_ext = 'xlsx' or is_doc_ext = 'tif' or is_doc_ext = 'tiff') Then	
		If gnv_tracking.of_find_recorde( gnv_tracking.document_management,string(il_doc_id), '', '', ref lstr_sys_tracking_log_arr) = 1 Then
			If UpperBound(lstr_sys_tracking_log_arr.astr_sys_tracking_log_arr[]) > 0 Then
				ls_user = lstr_sys_tracking_log_arr.astr_sys_tracking_log_arr[1].as_user
				If Messagebox('Warning','This document is being used by another user '+ls_user+'. If you open it and modify it now, then the modifications made by the other user may get lost. ~r~nDo you want to continue to open it?',question!,Yesno!,2) = 1 Then				
					//Allow the selection to change
				Else
...
//Modified By Jay Chen 04-01-2014
if is_parent_window = 'w_documents_browse' Then
...
event mousemove;call super::mousemove;if is_parent_window = 'w_documents_browse' then //Added By Jay Chen 04-02-2014
...
End If
if KeyDown(KeyControl!) or KeyDown(KeyShift!) then //second document
	istr_dm_compare.doc_id2 =  this.getitemnumber(row,'doc_id')
	istr_dm_compare.revision2 = this.getitemdecimal(row,'revision')
	istr_dm_compare.filename2 = this.getitemstring(row,'doc_name')
	istr_dm_compare.fullname2 = this.getitemstring(row,'doc_name_full')
	istr_dm_compare.doc_ext2 = this.getitemstring(row,'doc_ext')
	istr_dm_compare.revision_control2 = (this.GetItemString(row, "revision_control") = "1")
	istr_dm_compare.ctx_id2 = ll_ctx_id
else								 //first document
	istr_dm_compare.doc_id1 =  this.getitemnumber(row,'doc_id')
	istr_dm_compare.revision1 = this.getitemdecimal(row,'revision')
	istr_dm_compare.filename1 = this.getitemstring(row,'doc_name')
	istr_dm_compare.fullname1 = this.getitemstring(row,'doc_name_full')
	istr_dm_compare.doc_ext1 = this.getitemstring(row,'doc_ext')
	istr_dm_compare.revision_control1 = (this.GetItemString(row, "revision_control") = "1")
	istr_dm_compare.ctx_id1 = ll_ctx_id
	if dw_detail.rowcount() > 0 then il_first_doc_row = 1
end if
//end
...
//			of_show_sertifi_status(row)
			of_show_sertifi_status(row,false)
			Return			
	End Choose
...
if IsValid(m_pfe_cst_mdi_menu_contract_am) then
	...
	gnv_app.of_modify_menu_attr( lm_Menu.m_batchadd,'Enabled', m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd.Enabled)
	lm_Menu.m_batchadd.Visible = m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd.ToolbarItemVisible
	
	//Added By Jay Chen 09-25-2014
	if This.iw_parent.ClassName() = "w_contract" then
		gnv_app.of_modify_menu_attr( lm_Menu.m_refreshdocumentbrowse,'Enabled', m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_refreshdocumentbrowse.Enabled)
		lm_Menu.m_refreshdocumentbrowse.Visible = m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_refreshdocumentbrowse.ToolbarItemVisible
		gnv_app.of_modify_menu_attr( lm_Menu.m_checkcompare,'Enabled', m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_checkcompare.Enabled) //Added By Jay Chen 09-21-2015
		lm_Menu.m_checkcompare.Visible = m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_option.m_checkcompare.ToolbarItemVisible
	else
		gnv_app.of_modify_menu_attr( lm_Menu.m_refreshdocumentbrowse,'visible',false)
		lm_Menu.m_refreshdocumentbrowse.Visible = false
		gnv_app.of_modify_menu_attr( lm_Menu.m_checkcompare,'visible',false) //Added By Jay Chen 09-21-2015
		lm_Menu.m_checkcompare.Visible = false
	end if
	//end
	...
	//Added By Mark Lee 11/18/2014
	gnv_app.of_modify_menu_attr(lm_Menu.m_history,'enabled',m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_History.Enabled)
	lm_Menu.m_history.Visible = m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_History.ToolbarItemVisible
...
//		End If
	...
	If This.iw_parent.ClassName() = "w_contract" Then 
		...
		If  this.RowCount() > 0 then 
			ls_primary_doc =	this.GetItemString(tab_1.tabpage_browse.dw_browse.GetRow(),'primary_doc')
...
//	End Choose
	...
	lm_Menu.idw = This //Added By Ken.Guo 04/15/2014
end if
...
//	if dw_detail.rowcount() > 0 then
//		dw_detail.setitem(1,'compare_status', '3')
//	end if
	//end
else
	tab_1.tabpage_preview.enabled = FALSE
...
//	choose case gs_esigntype  //Modified By Jay Chen 10-10-2013
//		case 'sertifi'
//			gnv_sertifi.of_refresh_sertifi_status(ll_ctx_id , 0)
//		case 'echosign'
//			gnv_echosign.of_refresh_sertifi_status(ll_ctx_id , 0)
//	end choose
   gnv_sertifi.of_refresh_sertifi_status(ll_ctx_id , 0)
   gnv_echosign.of_refresh_sertifi_status(ll_ctx_id , 0)
...
for li = 1 to li_rows
	...
	this.setitem(li_New,'export_id',lds_doc.getitemnumber(li,'export_id')) //Added By Jay Chen 12-30-2015
...
this.sort() //Modified By Ken.Guo 2016-12-28. paste from top.
This.Setredraw( True )
...
event ue_populatedddws;call super::ue_populatedddws;//Added By Ken.Guo 2016-03-25
datawindowchild ldwc_received_by
long ll_ctxid
If This.Describe(  "received_by.DDDW.Name") <> 'dddw_doc_from_name' Then
	This.Modify("received_by.dddw.name='dddw_doc_from_name' received_by.dddw.datacolumn='id' received_by.dddw.displaycolumn='name'" )
End If
If This.getchild('received_by',ldwc_received_by) = 1 Then
	ldwc_received_by.settransobject(sqlca)
	ll_ctxid = inv_contract_details.of_get_ctx_id()
	ldwc_received_by.retrieve(ll_ctxid)
End if
...
if IsValid(m_pfe_cst_mdi_menu_contract_am) then
	...
	gnv_app.of_modify_menu_attr( lm_Menu.m_batchadd,'Enabled', m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd.Enabled)						//Added By Mark Lee 09/18/2015
	lm_Menu.m_batchadd.Visible = m_pfe_cst_mdi_menu_contract_am.m_documentmanager.m_add.m_batchadd.ToolbarItemVisible
...
int  li,li_new,li_rows,li_Find
//---Alfee 07.24.2008-----
Long ll_cnt, ll_found,ll_row,ll_select 
Decimal ldc_revision
String ls_find, ls_flag, ls_current_flag
Boolean lb_comparison = FALSE 
...
if not ib_compare  then
ll_row = tab_1.tabpage_browse.dw_browse.getrow()
if ll_row < 1 then return
if iw_parent.classname() = 'w_contract' Then
	ll_ctx_id = inv_contract_details.of_get_ctx_id()
Else
	ll_ctx_id = tab_1.tabpage_browse.dw_browse.getitemnumber(ll_row,'ctx_id')
End If
ls_docname = this.getitemstring(row,'doc_name')
ll_doc_id = this.getitemnumber(row,'doc_id')
ld_rev = this.getitemdecimal(row,'audit_version')
ls_fullname = ls_docname + ' ' + String( ll_doc_id )+ '.' + String( ld_rev ,'##0.0')
if KeyDown(KeyControl!) or KeyDown(KeyShift!) then //second document
	istr_dm_compare.doc_id2 =  this.getitemnumber(row,'doc_id')
	istr_dm_compare.revision2 = this.getitemdecimal(row,'audit_version')
	istr_dm_compare.filename2 = this.getitemstring(row,'doc_name')
	istr_dm_compare.fullname2 = ls_fullname
	istr_dm_compare.doc_ext2 = this.getitemstring(row,'doc_ext')
	istr_dm_compare.revision_control2 = (tab_1.tabpage_browse.dw_browse.GetItemString(ll_row, "revision_control") = "1")
	istr_dm_compare.ctx_id2 = ll_ctx_id
	if this.isselected(row) then
		for ll_row = 1 to this.rowcount()
			if this.getitemstring(ll_row, 'compare_status') = '4' then
				this.setitem(ll_row,'compare_status', '')
				this.selectrow(ll_row,false)
			end if
		next
		this.setitem(row,'compare_status', '4')
	else
		this.setitem(row,'compare_status', '')
	end if
	if this.isselected(il_first_doc_row) then
		this.setitem(il_first_doc_row,'compare_status', '3')
	end if
else								 //first document
	istr_dm_compare.doc_id1 =  this.getitemnumber(row,'doc_id')
	istr_dm_compare.revision1 = this.getitemdecimal(row,'audit_version')
	istr_dm_compare.filename1 = this.getitemstring(row,'doc_name')
	istr_dm_compare.fullname1 = ls_fullname
	istr_dm_compare.doc_ext1 = this.getitemstring(row,'doc_ext')
	istr_dm_compare.revision_control1 = (tab_1.tabpage_browse.dw_browse.GetItemString(ll_row, "revision_control") = "1")
	istr_dm_compare.ctx_id1 = ll_ctx_id
	il_first_doc_row = row
	for ll_row = 1 to this.rowcount()
		if this.isselected(ll_row) then
//			this.setitem(ll_row,'compare_status', '3')
		else
			this.setitem(ll_row,'compare_status', '')
		end if
	next
...
event doubleclicked;call super::doubleclicked;if row > 0 then
	...
	Choose Case dwo.name 
		Case 'p_9','p_10'
//			of_show_sertifi_status(row)
			of_show_sertifi_status(row,true)
			Return			
	End Choose
...
event onstatuschange;call super::onstatuschange;If Not this.ib_trigger_event Then Return //Added By Ken.Guo 2016-10-27. workaround crash bug.
...
event onwordwpsselchange;call super::onwordwpsselchange;If Not this.ib_trigger_event Then Return //Added By Ken.Guo 2016-10-27. workaround crash bug.
...
//IF lower(is_doc_right) <> "no editing" and lower(is_doc_right) <> "no access"THEN  
	...
	//of_setreviewbutton()  
	if Integer(Left(gs_word_version_num,2)) < 16 then of_setreviewbutton()  //Added by Harry 2016-11-11 for Task 680
...
event oncustombuttononmenucmd;call super::oncustombuttononmenucmd;//Added By Gavin.Jay 2017-04-19
If btncaption = "Activate" Then
	ole_doc.object.Activate(true)	
End If
If btncaption = "Deactivate" Then
	ole_doc.object.Activate(false)	
End If
end event

event onwordbeforedoubleclick;call super::onwordbeforedoubleclick;//Added By Gavin.Jay 2017-04-19
IF is_show_activate_app = "1" And is_show_activate_per = "1" Then
	ole_doc.object.Activate(true)	
End If
end event
...

