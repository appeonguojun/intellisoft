...
type mle_1 from multilineedit within w_dm_echosign_request
end type
...
If Not gnv_echosign.ib_isvalid_sertifi Then 
...
if gs_echosign_method = 'http' then  //Added by Harry 2017-01-20
	if  ls_mode = "global_license" then
		 gnv_echosign.of_refresh_access_token(ls_mode)
	else
		 gnv_echosign.of_refresh_access_token(gs_user_id)
	end if
end if
if ls_mode = "global_license" then
	if gs_echosign_method <> 'http' then //Added By Jay Chen 02-17-2016
		If Not gnv_echosign.of_check_sertifi( ) Then Return -1  
	else
		is_esign_token = gnv_user_option.of_get_option_value('contractlogix-super-master', "esign_access_token" )
		if isnull(is_esign_token) then is_esign_token = ''
		if is_esign_token = '' then 
			Messagebox('Esign Access Token Error','Please set the esign access token first!')
			return -1
		end if
	end if
else
	if gs_echosign_method <> 'http' then //Added By Jay Chen 02-17-2016
		If Not gnv_echosign.of_check_sertifi(false,'',true) Then Return -1 
	else
		SELECT esign_access_token into :is_esign_token FROM personal_license_setting WHERE esign_type='echosign' and user_id = :gs_user_id;
		if isnull(is_esign_token) then is_esign_token = ''
		if is_esign_token = '' then 
			Messagebox('Esign Access Token Error','Please set the esign access token first!')
			return -1
...
if isnull(gs_echosign_method) then gs_echosign_method = ''
if gs_echosign_method = '' then gs_echosign_method = 'webservice'
//end
...
event pfc_postopen;call super::pfc_postopen;//Added by Harry 20170612
if dw_doc.rowcount( ) > 1 then
	mle_1.visible = True
end if
end event
...
string text = "Notes: EchoSign will merge multiple documents into one PDF document for signing. When it is checked into Contractlogix, each document will get a copy of the same merged PDF document. "
...
Long							ll_ctx_id_list[],ll_doc_id,ll_rtn
...
string ls_agreementid,ls_documentid,ls_result
Blob 	lb_file1,lb_file2,lb_file3,lb_file4,lb_file5,lb_file6   //Added by Harry 20170613 
...
//Added by harry 20170609 for task752:Allow the selection and check out multiple documents for E-Signature
tns__documents ltns_files
string ls_doc_id_list
...
if gs_echosign_method <> 'http' then
	...
	//	ls_docname =  istr_ctx_email.pathfilename_list[i]
		...
		//Modified by harry 20170613 for APB bug
		/*
		lb_file = lb_null
		...
		*/
		choose case i
			case 1
				If inv_filesrv.of_fileread( istr_ctx_email.pathfilename_list[i] , lb_file1) < 0 Then 
					If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
					Messagebox('Error','Failed to open the document ' + istr_ctx_email.pathfilename_list[i] + '.')
					Return -1
				End If
			case 2
				If inv_filesrv.of_fileread( istr_ctx_email.pathfilename_list[i] , lb_file2) < 0 Then 
					If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
					Messagebox('Error','Failed to open the document ' + istr_ctx_email.pathfilename_list[i] + '.')
					Return -1
				End If
			case 3
				If inv_filesrv.of_fileread( istr_ctx_email.pathfilename_list[i] , lb_file3) < 0 Then 
					If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
					Messagebox('Error','Failed to open the document ' + istr_ctx_email.pathfilename_list[i] + '.')
					Return -1
				End If
			case 4
				If inv_filesrv.of_fileread( istr_ctx_email.pathfilename_list[i] , lb_file4) < 0 Then 
					If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
					Messagebox('Error','Failed to open the document ' + istr_ctx_email.pathfilename_list[i] + '.')
					Return -1
				End If
			case 5
				If inv_filesrv.of_fileread( istr_ctx_email.pathfilename_list[i] , lb_file5) < 0 Then 
					If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
					Messagebox('Error','Failed to open the document ' + istr_ctx_email.pathfilename_list[i] + '.')
					Return -1
				End If
			case 6
				If inv_filesrv.of_fileread( istr_ctx_email.pathfilename_list[i] , lb_file6) < 0 Then 
					If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
					Messagebox('Error','Failed to open the document ' + istr_ctx_email.pathfilename_list[i] + '.')
					Return -1
				End If
		...
		//Added by harry 20170609 for task752:Allow the selection and check out multiple documents for E-Signature
		ltns_files.docname[i] = ls_docname
		choose case i
			case 1
				ltns_files.bfile0 = lb_file1
			case 2
				ltns_files.bfile1 = lb_file2
			case 3
				ltns_files.bfile2 = lb_file3
			case 4
				ltns_files.bfile3 = lb_file4
			case 5
				ltns_files.bfile4 = lb_file5
			case 6
				ltns_files.bfile5 = lb_file6
		end choose
	/*	//Commented by harry 20170609
	...
	//		ls_dockeystemp = gnv_echosign.isoap_echosign.senddocument(gnv_echosign.is_apicode,ls_filename,ls_first_signer,ls_sign_type,ls_sign_flow,ls_message,ltns_files)
			...
			If lower(Left(ls_dockeystemp,5)) = 'error'  or ls_dockeystemp = '-1'  Then
	...
	Try
		//Create Request 
		//ls_dockeystemp = gnv_echosign.isoap_echosign.senddocument(gnv_echosign.is_apicode,ls_filename,ls_first_signer,ls_sign_type,ls_sign_flow,ls_message,ls_docname,lb_file,ls_cc)
		ls_dockeystemp = gnv_echosign.isoap_echosign.senddocuments(gnv_echosign.is_apicode,ls_filename,ls_first_signer,ls_sign_type,ls_sign_flow,ls_message,ls_docname,ltns_files,ls_cc)
		If lower(Left(ls_dockeystemp,5)) = 'error'  or ls_dockeystemp = '-1'  Then
			ls_error = ls_dockeystemp
			if len(ls_error) > 197 then ls_error = left(ls_error,197) + '...' //Added By Jay Chen 11-07-2013
			If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
	...
	For i = 1 to dw_doc.rowcount()
		ls_docname =  dw_doc.getitemstring(i,"docname")
		ll_doc_id = dw_doc.getitemnumber(i, "doc_id")
		 //Commented by harry 20170609
		//ll_rtn = lnv_webapi.of_esign_create_agreement(gs_accesstoken,is_esign_token,ls_first_signer,ls_sign_type,ls_sign_flow,ls_message,string(ll_doc_id),ls_cc,ls_agreementid,ls_documentid,ls_result,ls_filename)
		...
		Do 
			...
			End If
		Loop While  ll_rtn = -8 
		ls_doc_id_list = ls_doc_id_list +  ls_documentid + ',' //harrytest 
		*/
		ls_doc_id_list = ls_doc_id_list + string(ll_doc_id) + ',' //Added by harry 20170609 for task752:Allow the selection and check out multiple documents for E-Signature
	Next
	//Added by harry 20170609 for task752:Allow the selection and check out multiple documents for E-Signature
	if right(ls_doc_id_list, 1) = ',' then ls_doc_id_list = mid(ls_doc_id_list, 1, len(ls_doc_id_list) - 1)
	Do 
		ll_rtn = lnv_webapi.of_esign_create_agreement(gs_accesstoken,is_esign_token,ls_first_signer,ls_sign_type,ls_sign_flow,ls_message,ls_doc_id_list,ls_cc,ls_agreementid,ls_documentid,ls_result,ls_filename)
		If ll_rtn = -8 Then
			 gnv_echosign.of_refresh_access_token(True, '', is_esign_token)
			ib_auto_ref_token = False
		End If
	...
	if ll_rtn < 0 then
		ls_error = ls_result 
		If isvalid(lw_appeon_gifofwait) Then Close(lw_appeon_gifofwait)
		gnv_debug.of_output(true, 'send document: [' + ls_docname +'] error:'+ ls_error)
		Messagebox('Error','Failed to send the documents request.~r~n~r~n'+ls_error)
		Return 
	end if
	//ls_dockeys = ls_documentid
	ls_dockeys = ls_doc_id_list
end if

//inv_string.of_parsetoarray( ls_dockeys, '  ', istr_ctx_email.as_sertifi_docid[] )
//inv_string.of_parsetoarray( ls_dockeys, ',', istr_ctx_email.as_sertifi_docid[] )   //Added by harry 20170609 for task752:Allow the selection and check out multiple documents for E-Signature
//Added by harry 20170612 for task752:Allow the selection and check out multiple documents for E-Signature
For i = 1 to dw_doc.rowcount()
	istr_ctx_email.as_sertifi_docid[i]  = ls_dockeys
Next
...
if gs_echosign_method <> 'http' then
	...
	istr_ctx_email.as_sertifi_fileid = ls_agreementid //Added By Jay Chen 02-17-2016
end if
...

