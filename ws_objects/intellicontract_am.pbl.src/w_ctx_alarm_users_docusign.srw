$PBExportHeader$w_ctx_alarm_users_docusign.srw
forward
global type w_ctx_alarm_users_docusign from w_popup
end type
type cbx_appointment from checkbox within w_ctx_alarm_users_docusign
end type
type st_tips from statictext within w_ctx_alarm_users_docusign
end type
type cb_ic from commandbutton within w_ctx_alarm_users_docusign
end type
type cb_cancel from commandbutton within w_ctx_alarm_users_docusign
end type
type cb_ok from commandbutton within w_ctx_alarm_users_docusign
end type
type dw_1 from u_dw_contract within w_ctx_alarm_users_docusign
end type
end forward
...
global w_ctx_alarm_users_docusign w_ctx_alarm_users_docusign
...
public subroutine of_init_selected ();String ls_to_users[],ls_create_users[], ls_create_appoint[],ls_email,ls_string
n_cst_string lnv_string
long ll_i,ll_count,ll_find,ll_pos,ll_order
...
for ll_i = 1 to ll_count
	...
	ls_string = trim(ls_to_users[ll_i])
	ll_pos = pos(ls_string,'[')
	if ll_pos > 0 then
		ls_email = left(ls_string, ll_pos - 1)
		ll_order = long(mid(ls_string,ll_pos + 1,len(ls_string) - ll_pos - 1))
	else
		ls_email = ls_string
		ll_order = 1
	end if
	ll_find = dw_1.Find( "lower(trim(email_id)) ='" + lower(ls_email) + "'", 1, dw_1.RowCount())
	Do While ll_find > 0
		dw_1.SetItem( ll_find,'selected', 1)
		dw_1.SetItem( ll_find,'routingorder', ll_order)
...
on w_ctx_alarm_users_docusign.create
int iCurrent
call super::create
...
on w_ctx_alarm_users_docusign.destroy
call super::destroy
...
Long ll_row, ll_selected,li_create, ll_Appointment,ll_routingorder
...
FOR ll_row = 1 to dw_1.RowCount()
	...
	ll_routingorder = dw_1.getitemnumber(ll_row,"routingorder")
	if isnull(ll_routingorder) then ll_routingorder = 1
	if ll_routingorder < 1 then ll_routingorder = 1
	ls_mail = dw_1.GetItemString(ll_row, "email_id") + "[" + string(ll_routingorder) + "]" //Modified By Jay Chen 04-09-2014
...
If lstr_alarm.s_type <> 'Review/Approve' and lstr_alarm.s_type <> 'Esign'  Then //Added By Ken.Guo 10/31/2011.  Added if statement
	...
	if istr_alarm_user.s_flag = 'Y' then
		if len(ls_maillist) > 0 and len(ls_createlist) < 1 then
			if MessageBox('ContractLogix','If you want to create Action Item, at least one Create Action Item checkbox must be selected, Click Yes will return to re-select, Click No will not create the Action Item.',Information! ,YesNo!) = 1 then
				return
			else
				lstr_alarm.s_flag = 'N'
			end if
	...
	if istr_alarm_user.s_flag = 'N' then
		if len(ls_maillist) > 0 and len(ls_createlist) > 1 then
			if MessageBox('ContractLogix','Are you sure you want to create the Action Item?',Question!,YesNo!) = 1 then
				lstr_alarm.s_flag = 'Y'
			else
				lstr_alarm.s_flag = 'N'
			end if
...
integer height = 1168
integer taborder = 10
string dataobject = "d_notification_users_select_docusign"
