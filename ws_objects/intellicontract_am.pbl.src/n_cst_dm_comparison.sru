...
Decimal ldc_revid
String ls_compare_status, ls_docname, ls_docext, ls_find,ls_find2
...
ls_find = gnv_string.of_globalreplace( ls_find, ',', '.') //Added By Ken.Guo 2016-04-19.  in french, the decimal symbol ',' should change to '.'
...
IF Lower(is_step) = 'source' THEN	
	...
	IF ls_compare_status <> '1' THEN 
		...
		IF ls_compare_status = '2' THEN
			ls_find2 = 'doc_id = ' + String(ll_docid) + ' AND revision = ' + String(ldc_revid)
			ls_find2 = gnv_string.of_globalreplace( ls_find2, ',', '.') //Added By Ken.Guo 2016-04-19.  in french, the decimal symbol ',' should change to '.'

			ll_found = ids_target_docs.Find(ls_find2, 1, ids_target_docs.RowCount())
...
ls_find = "doc_id = " + String(al_docid) + " And revision = " + String(adc_revid)
ls_find = gnv_string.of_globalreplace( ls_find, ',', '.') //Added By Ken.Guo 2016-04-19.  in french, the decimal symbol ',' should change to '.'

IF ids_target_docs.Find(ls_find, 1, ll_cnt) > 0 THEN
	RETURN "2"
END IF
