...
type cb_settings from commandbutton within w_dm_email_document_send
end type
...
str_ctx_email istr_ctx_email,istr_ctx_email_weblink
...
public function decimal of_get_file_size (string as_filename, ref string as_unit)
end prototypes
...
ls_send_user = lnv_Mail.of_replace_send_user(gs_user_id)				//Added By Mark Lee 01/08/2014
...
//If gnv_data.of_getitem( 'icred_settings', 'use_web_email', False) = '1' Then //Comment By Jay Chen 04-22-2014
...
Values 
(:gs_user_id,:ls_send_user,:ls_to,:ls_cc,:ls_bcc,:ls_subject,getdate(),:ls_attachfilename,:il_email_template_id,:is_ctx_list,:is_doc_list,'',null,:ls_email_type,'',:li_send_result,:ls_errtext,:ls_notes,:ls_computer_info);
...
string ls_subject,ls_attach_file, ls_send_user
...
For i = 1 To UpperBound(istr_ctx_email.ctx_id_list[])
	...
	Else
		...
		li_send_result = 0
	End If
	//Modified By Ken.Guo 2016-04-19 . use ';'
...
//	end if
	...
	Values 
	(:gs_user_id,:ls_send_user,:ls_email_all,:ls_cc,:ls_bcc,:ls_subject,getdate(),:ls_attachfilename,:il_email_template_id,:il_ctx_id,'','',null,:ls_email_type,'',:li_send_result,:ls_errtext,:ls_computer_info);
...
public subroutine of_convert_pdf (boolean ab_convert);Integer 	li_file,li_rtn
...
//	END IF
	...
	FOR i = 1 to ll_cnt		
		...
		//Added By Mark 07/18/2014
//		IF lower(ls_ext) = 'doc' THEN lb_doc = TRUE //alfee 11.21.2008
		IF lower(ls_ext) = 'doc' or  lower(ls_ext) = 'docx'  THEN lb_doc = TRUE 
	...
	FOR i = 1 to ll_cnt	
		...
		IF lower(ls_ext) = 'doc' or lower(ls_ext) = 'docx'  THEN			//Added By Mark 07/18/2014
			...
			// Copy to pdf file
//			ls_pdf_path = Left(is_fullname[i], Len(is_fullname[i]) - 3) + "pdf"				//Added By Mark 07/18/2014
			ls_pdf_path =	Left(is_fullname[i], LastPos(is_fullname[i], ".") ) + "pdf"	
...
Integer 	li_ctx_id
string 	docpath, docname[], ls_attachment, ls_filename, ls_paths[]
String ls_filename_temp,ls_fullname_temp
String ls_fullnamelist[]
long ll_rtn //Added By Jay Chen 09-21-2015
...
//		li_rtn = GetFileOpenName("Select File", docpath, docname, "DOC", "All Files (*.*), *.*")	//modified by gavins 20140814
	...
	CASE "document", "attachment" //from document manager or current attachement
		...
		if Lower(as_from) = 'document' Then
			...
			If UpperBound(istr_ctx_email.doc_id_list[]) = 0 Then 
				IF Isvalid(w_appeon_gifofwait) THEN Close(w_appeon_gifofwait)	
//				Return
				if not isvalid(w_dm_check_compare) then Return //Added By Jay Chen 11-16-2015
			End If
			Openwithparm(w_appeon_gifofwait, "Attaching document...")
//			If inv_dm_utils.of_download_multi_files( istr_ctx_email.doc_id_list[] ,istr_ctx_email.revision_list[] , ls_fullnamelist[]) < 0 Then
			...
			If UpperBound(istr_ctx_email.doc_id_list[]) > 0 then
				if istr_ctx_email.ab_checkcompare then
					ll_rtn = inv_dm_utils.of_download_multi_files( istr_ctx_email.doc_id_list[] ,istr_ctx_email.revision_list[] , true, '',ls_fullnamelist[])
				else
					ll_rtn = inv_dm_utils.of_download_multi_files( istr_ctx_email.doc_id_list[] ,istr_ctx_email.revision_list[] , ls_fullnamelist[])
				end if
			end if
			//end
			if ll_rtn < 0 then
			...
			//Added By Jay Chen 11-16-2015 add compare file
			if isvalid(w_dm_check_compare) and Lower(as_from) = 'attachment' then
				if w_dm_check_compare.ib_compare_doc then
					lstr_pass.l_count++
					istr_ctx_email.doc_id_list[upperbound(istr_ctx_email.doc_id_list) + 1] = 0
					istr_ctx_email.ctx_id_list[upperbound(istr_ctx_email.ctx_id_list) + 1] = 0
					is_filename[upperbound(is_filename) + 1] = '_docs_compare.doc'
					ls_fullnamelist[upperbound(ls_fullnamelist) + 1] = gs_dir_path + gs_DefDirName + "\Agreement\" + '_docs_compare.doc'
				end if
				if w_dm_check_compare.ib_compare_pdf then
					lstr_pass.l_count++
					istr_ctx_email.doc_id_list[upperbound(istr_ctx_email.doc_id_list) + 1] = 0
					istr_ctx_email.ctx_id_list[upperbound(istr_ctx_email.ctx_id_list) + 1] = 0
					is_filename[upperbound(is_filename) + 1] = '_docs_compare.pdf'
					ls_fullnamelist[upperbound(ls_fullnamelist) + 1] = gs_dir_path + gs_DefDirName + "\Agreement\" + '_docs_compare.pdf'
				end if
			end if
			lstr_pass.s_pathname[] = ls_fullnamelist[]
			//end
...
public subroutine of_convert_pdf_bk (boolean ab_convert);Integer 	li_file
Long 		ll_cnt, i, j, ll_pos,ll_Len
String 	ls_ext, ls_temp, ls_SourceFile, ls_pdf_path,ls_prntrs,ls_DefPrinter
Oleobject lole_word
n_cst_filesrvwin32 lnv_filesvr
...
If cbx_clean.checked Then
	...
	IF lb_noclean Then //Added By Ken.Guo 2010-05-25. Add the if statement.
		if li_file_nct > 1 then
			MessageBox("ContractLogix","Some of the documents you have attached have no tracked changes or have not been modified, which will not be converted to clean copy.")
		elseif li_file_nct = 1 then
			MessageBox("ContractLogix","The document you have attached has no tracked changes.  Nothing to convert to clean copy.")
		end if
	End If
...
If AppeonGetClientType() = 'WEB' Then
	...
	If ls_ieurl <> '' Then
		...
		ls_web_link = ls_web_link + 'fileservice/index.aspx'
	End If
...
//	messagebox('setting weblink',ls_weblink_root)
	//check web link name
	if lower(ls_web_link) <> lower(ls_weblink_root) then
		Messagebox('ContractLogix Error','The web link setting is inconsistent with the current application link you are using.~r~nPlease go to System->Utilities->Web Link Settings to set the correct File Service Link.')
		is_web_link = ''
		Return -1
	end if
	//ping current cache
	if gf_ping_cache(ls_cache_name) = -1 then
		MessageBox('ContractLogix Error', 'Failed to connect the database ' +ls_cache_name + '.~r~nPlease go to System->Utilities->Web Link Settings to set the correct Cache Name.' )
		Return -1
	end if
End If
//end
...
public function decimal of_get_file_size (string as_filename, ref string as_unit);Decimal ldec_size
as_unit = ' KB'
...
this.Control[iCurrent+25]=this.ole_message
this.Control[iCurrent+26]=this.rb_ic
this.Control[iCurrent+27]=this.rb_oc
this.Control[iCurrent+28]=this.sle_sendbcc
this.Control[iCurrent+29]=this.cb_sendto
this.Control[iCurrent+30]=this.cb_sendcc
this.Control[iCurrent+31]=this.cb_sendbcc
this.Control[iCurrent+32]=this.cb_resize
this.Control[iCurrent+33]=this.cbx_clean
this.Control[iCurrent+34]=this.cb_paste_link
this.Control[iCurrent+35]=this.st_sending_status
this.Control[iCurrent+36]=this.cb_settings
...
inv_pop3 = Create n_cst_easymail_pop3 //Added By Ken.Guo 2015-07-27
...
IF isvalid(Message.PowerobjectParm) THEN
	...
	istr_ctx_email_weblink = istr_ctx_email //Added By Ken.Guo 2016-10-18
		...
		CHOOSE CASE Upper(istr_ctx_email.doc_from)
			...
			CASE "DM","DM_BROWSE"
				If UpperBound(istr_ctx_email.doc_id_list[]) > 0 then
					...
					//if istr_ctx_email.revision >= 0 Then
						//If of_save_attachment()	<> -1 Then 
							If istr_ctx_email.is_weblink = '1' Then
								...
								cb_settings.visible = True //Added By Ken.Guo 2016-08-24
							Else
					...
					if isvalid(w_dm_check_compare) then //Added By Jay Chen 11-16-2015 defaukt attachment is compared file
						of_add_attachment("attachment")
					end if
				End If
...
If isvalid(inv_pop3) Then Destroy inv_pop3 //Added By Ken.Guo 2015-07-27
end event
...
String  	ls_FullName[],ls_Merge_Before_File,ls_Merge_After_File,ls_link_arr[]
Blob		lblb_MsgData
...
If Left(istr_ctx_email.doc_from,4) <> 'CTXS' Then   	//Added If condition By Ken.Guo 2009-05-13.
	...
	If cb_paste_link.visible = true Then
		For i = 1 To Upperbound(istr_ctx_email.doc_id_list[])
			...
			End If			
		Next
		lnv_word.of_set_web_values( istr_ctx_email.doc_id_list[], istr_ctx_email_weblink.fullfilename_list[], ls_link_arr[], ls_link_arr[] )
	End If	
...
If Messagebox('ContractLogix','Are you sure you want to delete the attachment(s)?',Question!,YesNo!) = 2 Then Return
...
n_cst_email_checkin lnv_checkin


lnv_checkin = Create n_cst_email_checkin
OpenwithParm(w_email_send_settings,lnv_checkin.is_email_type )
destroy lnv_checkin
...

