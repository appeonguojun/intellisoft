...
long il_revision_count //Added By Jay Chen 02-25-2016
...
n_cst_webapi  inv_webapi //Added By Jay Chen 11-27-2014

//datastore lds_test //only used test 
end variables
...
public subroutine of_get_compute_relate_field (string as_name, integer ai_ctx_id)
public function string of_get_mergefield_name (string as_text)
public function integer of_useweb_mergedata_auto (long ai_export_id, long ai_ctx_id, string as_open_file, string as_target_file, boolean ab_mergeornot, str_doc_info astr_docinfo, long al_doc_id)
public function integer of_useweb_mergedata (long ai_export_id, long ai_ctx_id, string as_open_file, string as_target_file, boolean ab_merged, str_doc_info astr_docinfo, long al_doc_id)
public subroutine of_modify_word_property ()
public function integer of_get_revision_count (string as_filename)
public function integer of_activate (boolean as_bool)
end prototypes
...
If this.iole_parent.object.doctype > 0 Then this.iole_parent.object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
...
//Added By Ken.Guo 2016-05-30. workaround Task 524
Try 
	If iole_object.doctype > 0 Then iole_object.ActiveDocument.Saved = True 
Catch (Throwable th0)
End Try
...
li_rtn = ln_getfilename.of_getopenfilename("Select File",&
	 docpath, docname, "DOC",	"Doc Files (*.DOC),*.DOC"  )
...
try
	...
	If iole_object.doctype > 0 Then iole_object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
	...
	//Commented By Ken.Guo 2015-12-08 .task 475.but rollback task 115.
	//	//Added By Jay Chen 01-22-2015 Only reading when in No Eding Mode.
	//	if ab_readonly then
	////		iole_object.ActiveDocument.ProtectionType = wdAllowOnlyReading
	//		iole_object.SetReadOnly (true,""); 
	//	end if
	//	//end add
...
//Added By Ken.Guo 2015-08-10. close current document before open next  document.
//If gnv_user_option.of_get_option_value(gs_user_id, "close_word_before_open" ) <> '0' Then //Commented By Ken.Guo 2015-08-14
...
TRY
	...
	il_revision_count = lole_word.ActiveDocument.Revisions.Count //Added By Jay Chen 02-25-2016
...
//////////////////////////////////////////////////////////////////////
OLEObject lole_word
Boolean lb_trackchange_org //Added By Ken.Guo 2015-11-09
...
TRY
	...
	lole_word.Visible = FALSE
	lb_trackchange_org = lole_word.ActiveDocument.TrackRevisions  //Added By Ken.Guo 2015-11-09 save status
	lole_word.ActiveDocument.TrackRevisions = false //Added By Ken.Guo 2015-11-09 move here from bottom
	gnv_word_utility.of_modify_word_property( lole_word.Activedocument)	//Added By Mark Lee 06/20/2013 change for office 2013.
...
//	IF inv_word_util.of_unprotect_doc(lole_word.ActiveDocument) = -1 THEN //Alfee 10.08.2008
	...
	//lole_word.ActiveDocument.TrackRevisions = false //Commented By Ken.Guo 2015-11-09
	If isvalid(w_appeon_gifofwait) Then w_appeon_gifofwait.event timer()
	
	//Numbering
	inv_word_util.of_preview(lole_word.ActiveDocument, True)
	lole_word.ActiveDocument.TrackRevisions = lb_trackchange_org //Added By Ken.Guo 2015-11-09 restore status.
...
// $<add> 03.08.2007 by Alfee (Contract Logix Document Management)
//////////////////////////////////////////////////////////////////////
String ls_source_file,ls_error
...
TRY
	...
	if not ab_merged then
		inv_word_util.of_refresh_reqmnt_fields(lole_word,ai_ctx_id,ls_error) 
	end if
	//end
	IF inv_word_util.of_unprotect_doc(lole_word.ActiveDocument) = -1 THEN //Alfee 10.08.2008
		IF isvalid(w_export_progress) THEN CLOSE(w_export_progress)
		...
		RETURN -1
	END IF		
//	lole_word.ActiveDocument.TrackRevisions = false //Commented By Ken.Guo 2015-11-09 move to top
...
//	IF lole_word.ActiveDocument.Fields.Count < 1 THEN
	li_cnt = lole_word.ActiveDocument.Fields.Count + lole_word.ActiveDocument.Sections.Item(1).Headers.Item(1).Range.Fields.Count + lole_word.ActiveDocument.Sections.Item(1).Footers.Item(1).Range.Fields.Count
	IF li_cnt < 1 THEN //Modified By Jay Chen 08-19-2015 
		lole_word.ActiveDocument.Close(wdDoNotSaveChanges)
		lole_word.Quit(wdDoNotSaveChanges)
		lole_word.DisconnectObject( )
		destroy lole_word
	...
	END IF*/
	li_rtn = of_get_document_export_field(lole_word.ActiveDocument, ai_ctx_id,ai_export_id) //Added By Jay Chen 03-19-2014
	 //Added By Jay Chen 07-31-2014
	if li_rtn <> 1 then 
		if isvalid(lole_word) then
			lole_word.ActiveDocument.TrackRevisions = lb_trackchange_org //Added By Ken.Guo 2015-11-09 restore status.
			lole_word.ActiveDocument.saveas(as_target_file,0,false,'',false) //Added By Jay Chen 08-05-2014
			lole_word.Quit(wdDoNotSaveChanges)
			lole_word.DisconnectObject( )
			destroy lole_word
		end if
	...
	IF FileExists(ls_source_file) THEN //keep last export info
		...
		If isvalid(w_export_progress) Then w_export_progress.hpb_1.stepit( )
	ELSE
		gnv_debug.of_output( True,'Merge Failed. The export file is not exists. ' + ls_source_file + '.' )
		lole_word.ActiveDocument.Close(wdDoNotSaveChanges)
		lole_word.Quit(wdDoNotSaveChanges)
		lole_word.DisconnectObject( )
		destroy lole_word
	...
	gnv_debug.of_output( False,'Before Open Data Source.' + ls_source_file + '.' )
	...
	If ab_merged Then //The status may be different after saved as a new document - 04.15.2008
		...
		gnv_debug.of_output( False,'ab_merged = True' )
		//lole_word.ActiveDocument.BuiltInDocumentProperties.item[wdPropertyKeywords].value = 'merged'		
	Else
		gnv_debug.of_output( False,'ab_merged = False'  )
		lole_word.ActiveDocument.MailMerge.ViewMailMergeFieldCodes = False
		of_setmergestatus(lole_word.ActiveDocument, True)
	...
	gnv_debug.of_output(false, 'lole_word.ActiveDocument.saveas().' + as_target_file )
...
TRY
	...
	gnv_shell.of_delete_recent( as_open_file, true) //Added By Ken.Guo 2010-05-25. Delete Windows Recent Document.
	lole_word.Visible = FALSE
	lb_trackchange_org = lole_word.ActiveDocument.TrackRevisions  //Added By Ken.Guo 2015-11-09
	lole_word.ActiveDocument.TrackRevisions = false //Added By Ken.Guo 2015-11-09. move here from bottom.
	gnv_word_utility.of_modify_word_property( lole_word.Activedocument)	//Added By Mark Lee 06/20/2013 change for office 2013.	
	//Unprotect document if needed - Alfee 03.10.2008
	//inv_word_util.of_unprotect_doc(lole_word.ActiveDocument) 
	...
	Next	
	//Added By Jay Chen 08-19-2015
	IF i > li_cnt Then 
		OLEObject lole_header_fields,lole_footer_fields 
		lole_header_fields = lole_word.ActiveDocument.Sections.Item(1).Headers.Item(1).Range.Fields
		lole_footer_fields =  lole_word.ActiveDocument.Sections.Item(1).Footers.Item(1).Range.Fields	
		li_cnt = lole_header_fields.count 
		For i = 1 to li_cnt
			If lole_header_fields.Item[i].Type = wdFieldMergeField Then Exit
		Next	
		if  i > li_cnt Then
			li_cnt = lole_footer_fields.count 
			For i = 1 to li_cnt
				If lole_footer_fields.Item[i].Type = wdFieldMergeField Then Exit
			Next	
		end if	
	...
	Boolean lb_ismerged //Added By Jay Chen 03-06-2014
	lb_ismerged = of_ismerged(lole_word.ActiveDocument)
//	IF i > li_cnt OR (Not ai_export_id > 0) OR of_ismerged(lole_word.ActiveDocument) THEN	//Modified By Jay Chen 03-06-2014
	...
	//	lole_word.ActiveDocument.MailMerge.ViewMailMergeFieldCodes = 0 Then //Modify by ken for BugA05 in 200704IIIWeeks.
		...
		destroy lole_fields //03.13.2008
		destroy lole_word
		IF (Not ai_export_id > 0) OR lb_ismerged THEN //Modified By Jay Chen 03-06-2014
			RETURN -1
		else
			RETURN 1
		end if
...
//		RETURN -1
	...
	IF il_export_ctxid = ai_ctx_id and il_export_id = ai_export_id and Len(is_export_file) > 0 THEN
		ls_source_file = is_export_file	
		ids_document_export_field.reset() //Added By Jay Chen 08-05-2014
	ELSE
		ls_source_file = of_runexport(ai_export_id, ai_ctx_id)			
	END IF	
	IF FileExists(ls_source_file) THEN //keep last export info
...
app_filler.of_set_word_safemode(false, 1) //Added By Ken.Guo 2015-07-30
If lole_object.doctype > 0 Then lole_object.ActiveDocument.Saved = True //Added By Ken.Guo 2016-05-23. workaround Task 524
...
li_return = n_export.of_export_data_with_text_ctx_new( ai_export_id, ll_ctx_id, ls_path, 0, "","","",2,"" ,ids_document_export_field) //Modified By Jay Chen 03-19-2014

gnv_debug.of_output(False, 'of_runexport() = ' + String(li_return) + ', ls_path = ' + ls_path )
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 03-19-2014
//--------------------------------------------------------------------
...
//if ls_get_export_field = '1' then //Comment By Jay Chen 07-31-2014
	...
	For i = 1 to ll_cnt
		...
		If ll_type = wdFieldMergeField Then
			...
			if trim(ls_name) <> '' then
				...
				gnv_debug.of_output(true, 'n_cst_ovole_utils:of_get_document_export_field error. errtext is : ' + ls_text)
			end if
		End If
	Next	
...
//					ls_name = trim(right(ls_name,len(ls_name) - 10))
	...
	ls_errtext = right(ls_errtext, len(ls_errtext) - 2 )
	if len(ls_errtext) > 0 then
		messageBox("Caution on export" , "Get document field name(s) error. The error format text is :" + ls_errtext)
	end if
...
////only use test
//lds_test.SaveAs('c:\test.xls',excel!,True)
//return -1
////end test

//Added By Jay Chen 07-31-2014 remark:check export field whether or not exist in current export.
...
////only use test
//long a
//a = lds_test.insertrow(0)
//lds_test.setitem( a, "as_text", as_text)
////end test
...
//lds_test.setitem( a, "as_name", ls_name) //only use test
	
return ls_name
end function

public function integer of_useweb_mergedata_auto (long ai_export_id, long ai_ctx_id, string as_open_file, string as_target_file, boolean ab_mergeornot, str_doc_info astr_docinfo, long al_doc_id);//====================================================================
// Function: of_useweb_mergedata_auto
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
// 	value    long            ai_export_id
// 	value    long            ai_ctx_id
// 	value    string          as_open_file
// 	value    string          as_target_file
// 	value    boolean         ab_mergeornot
// 	value    str_doc_info    astr_docinfo
// 	value    long            al_doc_id
//--------------------------------------------------------------------
// Returns:  integer
//--------------------------------------------------------------------
...
Integer ll_rtn,li_rtn 
string ls_merge_fileid

//Setpointer(Hourglass!)

IF Len(Trim(as_target_file)) < 1 THEN RETURN -1

if isvalid(w_export_progress) then w_export_progress.hpb_1.stepit()

ll_rtn = inv_webapi.of_mergedocument(gs_accesstoken,'D',al_doc_id,ai_export_id,ai_ctx_id,ab_mergeornot,astr_docinfo,ls_merge_fileid)

if len(ls_merge_fileid) > 0 then
	li_rtn = inv_dm_utils.of_download_merge_file(ls_merge_fileid,as_target_file)
	if isvalid(w_export_progress) then	w_export_progress.hpb_1.position = 100
	if li_rtn = 1 then
		delete from em_attachment_temp where fileid = :ls_merge_fileid;
	end if
else
	return -1
end if
...
ll_rtn = inv_webapi.of_mergedocument(gs_accesstoken,'D',al_doc_id,ai_export_id,ai_ctx_id,ab_merged,astr_docinfo,ls_merge_fileid)
...
gnv_word_utility.of_modify_word_property( iole_document)	


end subroutine

public function integer of_get_revision_count (string as_filename);//====================================================================
// Function: of_get_revision_count
//--------------------------------------------------------------------
// Description:
//--------------------------------------------------------------------
// Arguments:
...
// Returns:  integer
//--------------------------------------------------------------------
// Author:	Jay Chen		Date: 02-25-2016
//--------------------------------------------------------------------
...
TRY
	lole_word.Application.NormalTemplate.Saved = TRUE //Alfee 01.17.2008	
	lole_word.Application.Documents.Open(as_filename,false,true)
	gnv_word_utility.of_modify_word_property( lole_word.Activedocument)	//Added By Mark Lee 06/20/2013 change for office 2013.
	lole_word.Visible = FALSE
...
If IsValid ( iole_object ) Then 
	If (iole_object.doctype = 2) Then
		iole_object.Activate(as_bool ) //Added By Jay 2017-01-18
	End If
End If
