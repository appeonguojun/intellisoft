...
type dw_occ from u_dw within w_bidq_import_document
end type
...
public subroutine of_check_status ()
end prototypes
...
n_cst_convert2pdf lnv_doctopdf  //Added By Jay Chen 09-30-2014
...
try
	...
	if not Pos(ls_Printer, "Contract Logix PDF Writer") > 0 then
		MessageBox("Warning", "The Contract Logix PDF Writer is not configured correctly!", Exclamation!)
		Return -1
	end if
	******************/
	...
	******************/
	//Added By Jay Chen 09-30-2014
	li_rtn = lnv_doctopdf.of_delete_file(ls_PDFTempFile)
	if li_rtn <> 1 then return -1
	//end
	...
	******************/
	lnv_doctopdf.of_printout_pdf(lole_Word.ActiveDocument) //Added By Jay Chen 09-30-2014
	lole_Word.ActiveDocument.Close(0)
	lole_Word.Quit(0)
...
public subroutine of_check_status ();//Added By Mark Lee 12/16/2014
if dw_document.rowcount() > 0 then 
	cb_setdef.enabled = true
else
	cb_setdef.enabled = false
end if 
end subroutine
...
this.Control[iCurrent+4]=this.cb_save2
this.Control[iCurrent+5]=this.cb_select
this.Control[iCurrent+6]=this.cbx_attachall
this.Control[iCurrent+7]=this.cb_scan
this.Control[iCurrent+8]=this.cb_preview
this.Control[iCurrent+9]=this.cb_clearcriteria
this.Control[iCurrent+10]=this.cb_search
this.Control[iCurrent+11]=this.cb_selectfile
this.Control[iCurrent+12]=this.dw_document
this.Control[iCurrent+13]=this.cb_save
this.Control[iCurrent+14]=this.cb_add
this.Control[iCurrent+15]=this.cb_delete
this.Control[iCurrent+16]=this.cb_attach
this.Control[iCurrent+17]=this.cbx_pdf
this.Control[iCurrent+18]=this.cb_setdef
this.Control[iCurrent+19]=this.cbx_preview
this.Control[iCurrent+20]=this.hpb_progress
this.Control[iCurrent+21]=this.cb_createqueue
this.Control[iCurrent+22]=this.cb_close
this.Control[iCurrent+23]=this.cb_deletequeue
this.Control[iCurrent+24]=this.dw_bidq
this.Control[iCurrent+25]=this.gb_view
this.Control[iCurrent+26]=this.gb_document
this.Control[iCurrent+27]=this.uo_search
this.Control[iCurrent+28]=this.gb_contract
this.Control[iCurrent+29]=this.lb_tempfile
this.Control[iCurrent+30]=this.st_progress
end on
...
//cb_stop.x = cb_log.x
//cb_log.x = cb_preview.x
////Modified by gavins 20161019  because saas upgrade apb6.5 to apb6.8 doc preview produce crash issue.
end event

event pfc_postopen;call super::pfc_postopen;dw_document.post of_SetDropDownCalendar(TRUE)				//Added By Mark Lee 09/19/2014
end event
...
//
li_Return = ln_getfilename.of_getopenfilename("Select File", &
		+ ls_FilePath, ls_FileName[], "DOC", &
		+ "Doc Files (*.DOCX;*.DOC),*.DOCX;*.DOC," &
		+ "Tif Files (*.TIF),*.TIF," &
		+ "Pdf Files (*.PDF),*.PDF," &
...
DataWindowChild ldwc_child, ldwc_child_by, ldwc_child_status
...
//this.of_SetDropDownCalendar(TRUE)				//can't use it
...
******************/
if lnv_doctopdf.of_check_pdfprinter() <> 1 then Return -1 //Added By Jay Chen 09-30-2014
...
event clicked;if dw_document.rowcount( ) > 0 then				//Added By Mark Lee 09/19/2014
	OpenWithParm(w_bidq_setdefaults, dw_document)
end if
end event
